...
OVERALL_ERROR_STATUS="RUNTIME_EXITCODE_NON_ZERO"
  ...
  case "$check_for" in
    ...
    "RUNTIME_EXITCODE_NON_ZERO")
      ...
      for backend in "${TARGET_BACKENDS[@]:1}"; do
        # Compare output.
        local DIFF
        DIFF=$(diff "${first_backend}_stderr.txt" "${backend}_stderr.txt")
        if [[ -n "$DIFF" && "$DIFF" == *"System.OverflowException"* ]]; then
          differ_exit_code=0
        fi
        rm "${backend}_stdout.txt"
        rm "${backend}_stderr.txt"
      done
      
      rm "${first_backend}_stdout.txt"
      rm "${first_backend}_stderr.txt"
...
function run_candidate_program {
  ...
  # Check if the exit code is non-zero. (interesting)
  if [[ $exit_code -eq 124 ]]; then
