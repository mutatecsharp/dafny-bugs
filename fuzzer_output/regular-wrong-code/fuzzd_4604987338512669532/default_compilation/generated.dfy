...
function fm2(p0: bool, p1: int, p2: bool, globalState: GlobalState): int {
	if ({true, !true} < {false, !false, false}) then |(map v0 : int | (595 <= v0) && (v0 < 0x184) :: (v0 / 0x3c8) := (0x231))| else |(map v1 : int | v1 in map[235 := set v2 : int | v2 in map[|(seq(0x9b, i0  => (0x29f)))| := |[412]|] :: (v2 + |(map v3 : int | (0x233 <= v3) && (v3 < 0x2bb) :: (v3 + 845) := (-0x261))|)] :: (v1 * -|multiset{-0xc3, -0x3e2, -313}|) := ("grrhk"))| / |map[false := true]|
}
...
function fm16(p0: int, p1: bool, globalState: GlobalState): set<int> {
	(set v0 : int | (-0x1ab <= v0) && (v0 < -0x2fb) :: (v0 % |[|[true, true]|, 0x3a9]|)) - {0x189}
}
...
function fm21(globalState: GlobalState): seq<int> {
	([0x296, 0x1b4, |(seq(0x3e4, i0  => (|(seq(526, i1  => ('y')))|)))|, 0xc8, |map[false := !false]|] + [|multiset([true])|, |"ad"|]) + [|[true]|, |["djuxym"]|]
}
function fm22(p0: bool, p1: char, p2: int, globalState: GlobalState): set<char> {
	if ({[0x2fc]} == {seq(0x23a, i0  => (|"ux"|))}) then set v0 : char | v0 in multiset{'x'} :: (v0) else {'j'} - (set v1 : char | v1 in map['w' := DC42(false).cf59] :: (v1))
}
...
function fm24(p0: bool, p1: int, globalState: GlobalState): map<int, bool> {
	(map[0xd6 := false] + map[701 := true]) + DC14(map v0 : int | (0x38a <= v0) && (v0 < 0x2b2) :: (v0 % 0x36b) := (true)).cf16
}
...
function fm49(globalState: GlobalState): D1 {
	DC4(0x2ee < --|map[false := true]|, |(if (!true) then seq(0x309, i0  => ('q')) else "hbf")|, !!true)
}
...
function fm54(p0: bool, p1: int, p2: map<string, set<bool>>, p3: bool, globalState: GlobalState): D6 {
	DC13(seq(0x23a, i0  => ('l')), -0x1c3 % |map[[false, false] := 0x1da]|)
}
...
function fm57(p0: set<int>, globalState: GlobalState): map<string, bool> {
	(map["qkxq" := false] + map[seq(0x8c, i0  => ('j')) := !!true]) + ((map v0 : string | v0 in ["np"] :: (v0) := (true)) + (map v1 : string | v1 in map["g" := -912] :: (v1) := (false)))
}
function fm58(p0: int, p1: int, globalState: GlobalState): seq<seq<int>> {
	([[|[false]|, |multiset{|[false]|}|]] + [[711, |[|[true]|]|]]) + ([[-511, 0xbf]] + DC44(seq(266, i0  => ([0x20c]))).cf63)
}
function fm59(globalState: GlobalState): multiset<seq<int>> {
	(multiset{[|map[!true := !!false]|, 0x3ca]} * multiset{seq(0x10f, i0  => (446))}) - (multiset{[591]} + multiset{[0x214, 0x139]})
}
...
function fm63(globalState: GlobalState): map<seq<int>, bool> {
	if (true) then map[DC3([0x13e]).cf3 := !false] + (map v0 : seq<int> | v0 in DC111(map[[-0x27e, 0x302] := |(seq(-0x1c, i0  => (|"s"|)))|]).cf163 :: (v0) := (false)) else map v1 : seq<int> | v1 in (set v2 : seq<int> | v2 in map[[0xf7, |"dq"|] := 0x253] :: (v2)) :: (v1) := (false)
}
function fm64(globalState: GlobalState): set<int> {
	if (multiset(seq(407, i0  => (0x352))) !! multiset{-0x19}) then if (!false) then {|[0x2b3, 0x379]|, 0xfd} else set v0 : int | (0xd5 <= v0) && (v0 < 0xd3) :: (v0 - -18) else DC23({-61, 0x2d3}).cf27
}
...
function fm72(p0: char, p1: int, p2: seq<bool>, p3: int, globalState: GlobalState): map<int, int> {
	map[|{!!true, false}| % 0x239 := 0x319]
}
function fm73(p0: char, p1: bool, p2: bool, globalState: GlobalState): seq<map<char, int>> {
	(seq(61, i0  => (map['n' := |"w"|]))) + ([map['n' := 0x351], map['o' := 0x203], map['a' := |map[|"rm"| := true]|], map['a' := 0x134], map v0 : char | v0 in (seq(457, i1  => ('f'))) :: (v0) := (|{0x1d4}|)] + [map['w' := 159], map['r' := |multiset{-0x2f7}|], map['j' := -0xf], map['v' := -|(seq(0x2c4, i2  => (|[map[false := |"dcuorfr"|]]|)))|]])
}
...
function fm81(p0: int, p1: int, p2: int, p3: bool, globalState: GlobalState): map<map<int, bool>, string> {
	match DC67(|map[--0x168 := -0xcc]|) {
		case DC66(cf98, cf99) => DC115(map[map v0 : int | (0x317 <= v0) && (v0 < 261) :: (v0 / -|[DC64(cf99).cf96]|) := (cf99) := "bwkx"]).cf166 + map[map v1 : int | (0x2cb <= v1) && (v1 < 0x159) :: (v1 - cf98) := (cf99) := "bypreflyn"]
		case DC67(cf100) => map[map[cf100 := true] := "ln"] + map[map[|{false, true}| := true] := "axx"]
		case DC65(cf97) => map[map[|"bj"| := true] := "t"] + (map v2 : map<int, bool> | v2 in [map[323 := true]] :: (v2) := (seq(677, i0  => ('s'))))
		case DC68(cf101) => map v3 : map<int, bool> | v3 in multiset{map[-0x29f := !!true], map[|[false]| := true], map[|{0x35c}| := false], map[-0x2f6 := !true]} :: (v3) := (seq(-61, i1  => ('w')))
	}
}
function fm82(p0: int, p1: bool, p2: bool, globalState: GlobalState): seq<set<int>> {
	seq(0x1ee, i0  => ({|map[|map[true := [true]]| := true]|, |"fkdnhpue"|} * DC23({|map[false := !false]|}).cf27))
}
...
function fm87(p0: int, p1: bool, globalState: GlobalState): D22 {
	DC48(false, |multiset{-|[0x48]|, |[true, true]|}| % |"kltuewwe"|)
}
...
function fm90(globalState: GlobalState): map<bool, map<char, bool>> {
	if ("thwvqyh" <= (seq(0x3cd, i0  => ('f')))) then DC117(map[false := map['a' := false]]).cf170 + map[true := map['k' := true]] else map[!false := map['s' := !true]]
}
...
function fm93(p0: int, globalState: GlobalState): map<bool, string> {
	(map[true := seq(402, i0  => ('c'))] + map[!false := DC13("ote", |[|"jate"|]|).cf14]) + map[true := "wmestkb"]
}
...
method m0(p0: int, p1: map<int, int>, p2: bool, p3: bool, globalState: GlobalState) returns (r0: int) {
	for i0 := p0 to p0 {
		...
		var v18 := DC84(p3, [v4.f26] < v17[|(seq(106, i5  => (v1)))| := v4.f27], |(v16 + v17)|, "wfqo", p0 <= i0);
		v10, v17, v18, globalState.f3, globalState.f3 := v10, v16 + v16, v18, p2, true;
		var v19 := new C4(v0, i0, v4.f27, i0 / 0x82, if (p2) then v3 else v3, if (!!p2) then p3 else p3, fm66(p2, true, globalState), p3);
	...
	if (!(p3 && p3)) {
		...
		v22[311], globalState.f4, r0, globalState.f3 := v21, true, p0, (|v24| % p0) <= |v30|;
		var v31: seq<int> := [p0];
		var v32: map<array<bool>, seq<int>> := map[v25 := v31[|fm103(p3, p2, p0, v29.fm12(p2, globalState), globalState)| := |"xnokb"|]];
		var v33: seq<seq<int>> := [v31];
		var v34: map<bool, seq<int>> := map[p3 := if (v25 in v32) then v32[v25] else (v33[p0])[p0 := -0x13b]];
		v34 := v34[fm1(p2, p0, globalState) := v31[p0 := 0xc6]];
		globalState.f4 := p2;
		var v35 := "qesfsv";
		v35 := v35;
		if (!((p0 % p0) <= 0x10a)) {
			globalState.f4 := v27[p0];
		...
		match (DC99()) {
			case DC98(cf147, cf148) =>
				...
				v62[294] := p2;
				v62[294] := if (fm1(p3, p0, globalState) ==> p2) then p3 else p2 ==> p2;
				var v63: map<char, char> := map[v50 := v50];
				v52 := (v52[p0 := if (v50 in v63) then v63[v50] else v50] + v52) + v52;
			...
			case DC99() =>
				...
				r0 := |(if (v67[797] in v69) then v69[v67[797]] else v68)| + v67[797];
				globalState.f4 := p2;
			case DC97(cf146) =>
				var v70 := DC89(p3);
				var v71 := DC90(v70);
				...
				var v73 := DC90(v72);
				v73 := v73;
				var v74: map<bool, seq<bool>> := map[!!(p0 <= p0) := (fm104(DC7(v52[p0 := 'e']), globalState)).cf11];
				var v75: seq<bool> := [p2];
				v74 := v74[p2 := v75];
				...
				v81 := v82[|v52| / DC81(p2, -0x1f5).cf118];
			...
			case DC100(cf149) =>
				...
				r0 := (if (p3 in v84) then v84[p3] else p0) % (p0 / -p0);
				globalState.f3 := p3;
				var v85: array<string> := new string[16];
				v85[155] := v52;
				v85[155] := fm105(p3, 'x', p2, globalState);
		}
	...
	for i10 := |v96| to |[p2]| {
		...
		if ((if (p0 in v97) then v97[p0] else i10) >= p0) {
			...
			var v100 := new C1(p0, [p3, p3, p2], p2 <==> !p3, v99[i10], fm1(false, i10, globalState));
			...
			var v111: array<map<D3, bool>> := new map<D3, bool>[20] [v103, fm106(-0x2a5, globalState), map v104 : D3 | v104 in v105 :: (v104) := (p2), v103, fm106(i10, globalState), v103, v103, v103 + fm106(|v99|, globalState), v103, (map v106 : D3 | v106 in map[v102 := v99] :: (v106) := (true)) + map[v102 := p3], map[v102 := v107.cf18] + v103, v103, v103, (map[DC7("pbxl").(cf9 := v99) := p3])[v102.(cf9 := v99) := p2], v103, v103, v103 + map[v102 := p2], v103, v103 + v110[p0], v103];
			v111[521] := v103;
			var v112: array<int> := new int[29];
			v112[962] := -(|v99| - v100.f24);
			...
			var v122: T3 := new C5(fm33(multiset{0x23e}, |v120|, globalState), v100.f24, v121[p0], true, v119.f23, p2);
			var v123: seq<T3> := [v122];
			var v124: seq<T3> := [v122, v122, v122, v122, v123[|v99|]];
			v101[884], v111[521], v112, v112[962], globalState.f4 := if (v118 in v116) then v116[v118] else v123 < v123, v103, v112, i10 + v100.f24, false;
			var v125: set<char> := {v98, v98, 'v'};
			var v126: array<T0> := new T0[2];
			var v127: multiset<array<T0>> := multiset{v126, v126};
			var v128: T2 := new C8(fm41(v122.f14, globalState), |p1|, v94, v101[884], 'o', p3, p0, v122.f14);
			var v129: map<bool, int> := map[!v101[884] := -0x9a];
			var v130: set<map<bool, int>> := {v129};
			var v131: C8 := new C8(v130, v100.f24, fm46(v101[884], p0, v112[962], globalState), p2, 'y', v128.f11, 0xd1, v112[962]);
			var v132: multiset<C8> := multiset{v131};
			v112[962], r0, v101[884] := (|v125| / p0) / p0, |v127|, "xnswcjp" == (v99[v122.f14 := v98] + DC41(|multiset{v128}|, |v132|, seq(0x346, i11  => (v122.f10)), v114.fm8(p2, globalState)).cf57);
		} else {
			var v133: array<bool> := new bool[24];
			var v134 := 'l';
			var v135: C10 := new C10(v133, -p0, [p2, p3, !p2], p3, v134, p2);
			v135 := v135;
			globalState.f3 := (i10 / i10) < p0;
			var v136 := "ypvwjd";
		...
		v146[365], r0, v148, globalState.f3 := v147, i10, v148, (v94 + (if (v149 in v151) then v151[v149] else v94)) <= (v94 + [p3]);
		var v152 := "kkgmu";
		...
		v149[6] := -i10 + i10;
		var v157: seq<seq<bool>> := [v94[p0 := p3], v94];
		v149[6] := |(v157 + ([v94] + v157))|;
	}
	var v158 := 'i';
	...
	if (p2) {
		...
		v170, r0, v173[842] := v170, p0 / -p0, if (!v94[v174.cf144] in v175) then v158 else v158;
		globalState.f3 := p3;
		var v176: map<bool, int> := map[v163.fm8(p3, globalState) := 0x38a];
		r0 := |(v176 + v176[p2 := v168.f28])|;
	} else {
		var v177: array<int> := new int[19](i13 => i13 / |(seq(791, i14  => (|(seq(0xd8, i15  => (multiset{p2})))|)))|);
		var v178 := DC6(v177);
		...
		v184[91] := v182;
		v184[91] := v182[false := p0];
		var v185: multiset<char> := multiset{'k', v158, v158};
		r0, v158 := p0 / -p0, v183[if ('f' in v185) then v185['f'] else p0];
		r0 := p0;
		globalState.f4 := p2;
		r0 := p0;
	}
	...
	var v190: set<bool> := {v94[p0], p3};
	var v191: array<int> := new int[24] [p0, p0, p0, p0 % p0, p0, |v186|, |(v95 + fm36(|p1|, globalState))|, p0, p0, |[p0, |v186|, p0, p0, 0x37d]|, p0, p0 % 0x3df, p0, p0, p0, -p0, p0, p0, -p0 * |"jhps"|, |(map v187 : string | v187 in v188 :: (v187) := (v186[|{!p2}|]))|, p0, |v189|, |v186|, |v190|];
	v191 := v191;
	r0 := p0;
}
trait T0 {
...
class C1 extends T1 {
	...
	function fm6(p0: int, globalState: GlobalState): string {
		("yrcvpwwdt" + "pdrsvnm") + ((seq(0x3bf, i0  => (f10))) + "lbbslmxui")
	}
	function fm5(p0: bool, globalState: GlobalState): int {
		f24
	}
	...
	method m1(p0: int, p1: int, p2: bool, globalState: GlobalState) returns (r0: map<int, bool>, r1: int, r2: int) {
		...
		v0[436] := fm1(f11, -628, globalState);
		v0[436] := p2;
		if (f13) {
			...
			v4 := v4;
			var v5: map<bool, int> := map[v0[436] := p1];
			v1.f21 := -(p1 / p1) * (|v5| - f24);
		} else {
			f13 := !f11;
			var v6 := "wklx";
			...
			r2 := p1;
			var v14: map<int, bool> := map[p1 := v0[436]];
			var v15 := DC14(v14);
			var v16: map<string, map<int, bool>> := map[v6 := v15.cf16];
			v16 := v16[v6[p0 := fm19(f11, p0, v0[436], globalState)] := v14];
		}
		
		var v17: array<array<bool>> := new array<bool>[23];
		v17[739] := v0;
		var v18: seq<int> := [p1, p1];
		v17[739], r1, r2 := v0, if (v18 == v18) then f24 else f24 / -p0, p0;
		var i0 := 0;
		while (f13)
			decreases 100 - i0
		{
			...
			var v20 := "v";
			v19[502] := v20[f24 := f10] + "v";
			v19[502] := v20;
			var v21: multiset<int> := multiset{p1};
			globalState.f4 := fm1(true, |v21|, globalState);
			globalState.f4 := v0[436];
			var v22: map<int, bool> := map[fm2(f11, -p1, false, globalState) := v0[436]];
			v0[436] := (if (p0 in v21) then v21[p0] else |v22|) < (p1 / p1);
		}
		v0[436] := f13;
		var v23 := "urk";
		var v24: multiset<int> := multiset{p1, -f24, |f12|, p0, p0};
		var v25: map<bool, bool> := map[fm1(v0[436], -p0, globalState) := f11];
		var v26: map<int, bool> := map[-688 := if (p2 in v25) then v25[p2] else p2];
		var v27: array<int> := new int[10] [|v23[p0 := f10]|, p1, f24, p1, -0x246, p1 + p1, -(p1 - |v24|), |v26|, f24, |v26|];
		forall i2 | 0 <= i2 < v27.Length {
			v27[i2] := i2 - |(seq(238, i3  => (f24)))|;
		}
	...
	method m2(p0: bool, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: multiset<seq<int>>, r3: seq<int>) {
		if (p1) {
			var v0 := DC4(f12[f24], f24, true);
			var v1 := DC5(v0);
			var v2: array<array<int>> := new array<int>[16];
			var v3: array<int> := new int[14](i0 => i0 % f24);
			v2[868] := v3;
			var v4: seq<array<int>> := [v3];
			var v5 := "xhcp";
			v1, v2[868], f24, globalState.f3 := v1, v4[|fm20(f13, f24, |v5|, |v5|, globalState)|], 847, false;
			var v6 := DC3(seq(0x2c7, i1  => (0x36e)));
			var v7: seq<int> := [f24];
			var v8 := DC3((v6.(cf3 := v7)).cf3 + v7);
			var v9: map<bool, int> := map[fm1(f13, f24, globalState) := f24];
			v8, v7 := DC3(fm21(globalState)).(cf3 := [f24, f24, f24]), ([f24, f24, f24])[if (f11 in v9) then v9[f11] else f24 := f24];
			f10 := f10;
			var v10 := DC9(f12);
			...
			v2[868][664] := f24;
			v2[868][664], globalState.f4 := f24, f11 && (f24 == f24);
		} else {
			...
			if (f24 != f24) {
				...
				v16 := fm20(false, f24, |f12|, f24, globalState) + v16[f24 := p2];
				var v17: array<map<bool, int>> := new map<bool, int>[2];
				var v18: map<int, array<map<bool, int>>> := map[0xba % f24 := if (f11) then v17 else v17];
				v18 := v18[f24 := v17];
				...
				v23, r0, globalState.f4 := v23, -((f24 * -f24) % (-f24 % f24)), ((v24[|v25| := |v26|])[-f24 := 78] * v24[f24 := 45]) >= (if (f24 in v27) then v27[f24] else v24);
				r0 := -|v25|;
			}
			...
			var v32 := "mpveanbw";
			f13, v29[514] := p2, (|v30| * f24) / |(v31 * multiset{f24, f24, |v32|})|;
		}
		...
		f11 := !(f24 > f24);
		var v34 := DC3(seq(0x345, i2  => (|map[map[f10 := 0x15] := f24]|)));
		var v35 := "fnetlw";
	...
	method m15(p0: bool, p1: int, globalState: GlobalState) returns (r0: bool, r1: seq<bool>, r2: int) {
		...
		v0 := v0[v4 !in v5 := v1[748] ==> f11];
		if (v1[748] || f13) {
			var v6: map<bool, int> := map[v1[748] := p1];
			f24 := f24 / (f24 / |v6|);
			f24 := p1;
			globalState.f3 := !p0;
			v1[748] := f13;
			r0 := !(if (f11) then fm1(f13, 0x209, globalState) else v1[748]);
		} else {
			var v7: set<bool> := {v1[748], f11};
			r0 := !(f24 >= f24) && !(v7 != v7);
			globalState.f3 := fm1(f13, p1, globalState);
			var v8: seq<int> := [p1, f24, f24, f24, f24];
			var v9: multiset<seq<int>> := multiset{v8, v8, v8, v8};
			v2 := if (|v9| > f24) then ("iycakdx" + v2)[|v2| := f10] else fm6(f24, globalState);
			globalState.f4 := !f11;
			var v10: map<bool, seq<bool>> := map[f13 := f12];
			var v11: seq<seq<bool>> := [if (true in v10) then v10[true] else f12];
			var v12 := DC4(f11, p1, v1[748]);
			var v13 := DC9([v1[748], f11]);
			var v14: map<int, seq<bool>> := map[f24 := f12];
			var v15: set<char> := {f10, f10, f10};
			var v16: array<seq<bool>> := new seq<bool>[28] [fm20(f13, |f12|, |f12|, 829, globalState), v11[f24], f12, (v11[-f24])[f24 := v12.cf4], f12, f12, f12, f12, f12, f12, f12, f12, [v1[748], v1[748], f11], f12, f12, f12, f12, fm20(f11, f24, f24, -0x1d9, globalState), v13.cf11, fm20(p0, p1, fm5(f13, globalState), -f24, globalState), [v1[748], f13, v1[748]], f12, f12, [f13], if (p1 in v14) then v14[p1] else f12, f12, [f11], v11[|v15|]];
			var v17 := DC8(v16);
			match (v17) {
				case DC8(cf10) =>
					var v18: array<char> := new char[11](i1 => v2[|v8|]);
					var v19: map<bool, array<char>> := map[p0 := v18];
					v19 := v19[false := v18];
					var v20 := new C0(p1, v1[748]);
					var v21: map<bool, int> := map[p0 := f24];
					v20.f21 := if (f13 in v21) then v21[f13] else p1;
					var v22: array<int> := new int[18](i2 => i2 / -0x20a);
					var v23: map<array<int>, bool> := map[v22 := v1[748]];
					var v24: map<map<array<int>, bool>, int> := map[v23 := v20.f21];
					v22[240] := if (v23 in v24) then v24[v23] else |(seq(0x1bd, i3  => (f10)))|;
					v22[240], globalState.f3, v20.f21, r1, v20.f21 := v20.f21, p0, (v20.f21 - -|(seq(552, i4  => (f10)))|) / |v2|, f12, |(f12 + (if (f11 in v10) then v10[f11] else f12))|;
			}
		...
		r1 := f12[f24 := f13] + (f12 + DC9(f12).cf11)[p1 := true];
		r2 := p1;
	}
}
...
class C2 extends T0, T5 {
	...
	function fm6(p0: int, globalState: GlobalState): string {
		DC13("pgiauhj", |[f14, 0x54]|).cf14 + (seq(0x2f5, i0  => (f10)))
	}
	...
	method m6(p0: int, p1: bool, globalState: GlobalState) returns (r0: int) {
			...
			decreases 100 - i0
		{
			...
			if (f17 != p0) {
				...
				v4[975], v5, v6, globalState.f3 := f10, v5, v6, p1;
				var v7: seq<bool> := [!!f11, v2 == fm6(|fm6(p0, globalState)|, globalState)];
				v7 := v7 + v7;
				var v8: array<int> := new int[7] [|v3|, 4, |"vicgsica"| + f27, f17, f27, 288, |v0[-847 := f14]|];
				v8 := if (p1) then v8 else v8;
				r0 := (-f14 / f14) * p0;
				f11 := p1;
			} else {
				var v9: set<bool> := {true};
				var v10: map<bool, set<bool>> := map[f12[f26] := v9];
				var v11 := DC28(v10);
				v10 := v11.cf34;
				...
				v13[725] := multiset(v15);
				v13[725] := v0;
				var v16: array<int> := new int[7](i4 => i4 / f14);
		...
		var v20: set<int> := {-f26};
		var v22: seq<set<int>> := [set v21 : int | v21 in v20 :: (v21 % |"rtkfwpa"|)];
		v19[258] := |([v20, v20] + v22)|;
		var v23 := DC4(f13, f27, f13);
		v19[258] := v23.cf5 / 0x205;
		if (f11) {
			var v24: array<bool> := new bool[23];
			var v25: map<bool, array<bool>> := map[v19[258] > 0x2e4 := v24];
			...
			var v36: array<bool> := new bool[16] [f11, f11, f11, !!f13, f12[v19[258]], fm39(v33, v35, globalState), p1, f11, p1, f11, f11, p1, p1, f13, f11, f13];
			...
			v37[480] := if (v27 in v38) then v38[v27] else v36;
		}
		
		for i5 := p0 / f27 to f26 - f16 {
			v0 := fm4(if (f13) then p1 else f11, globalState);
			var v39 := "srkpcrn";
			v39 := v39;
			r0 := f27;
			var v40: map<bool, int> := map[p1 := v19[258]];
		...
		for i6 := f17 to p0 {
			...
			match (v49) {
				...
				case DC24(cf28) =>
					...
					v74[227] := v72;
					v74[227] := new array<int>[22];
					var v75 := DC22(v0);
					var v76: seq<int> := [f27];
					var v77: seq<char> := [f10, f10];
					var v78: array<D10> := new D10[23] [DC22(v0), v75, v75, v75, v75, v75, DC22(multiset{p0}), v75, v75, v75, v75, v75, DC22(multiset{|v76|, f16, i6, |v76|}), DC22(v0), v75, v75, fm44(p1, 0x5, v76[f16], |v77|, globalState), v75, v75, v75, v75, v75, v75];
					var v79 := DC30(v78);
					var v80: seq<array<D10>> := [v78, v78, v79.cf38];
					v80 := [v78, v78, v78];
					var v81 := DC19(f13, v19[258]);
					var v82: map<int, int> := map[v81.cf22 := f16];
					v82 := v82;
					var v83, v84, v85 := m19(if (f11) then f16 else p0, globalState);
				case DC27(cf33) =>
					var v86 := "rjqh";
					var v87: array<seq<int>> := new seq<int>[1] [seq(706, i8  => (f27))];
					v87[850] := seq(0x155, i9  => (0x217));
					var v88: seq<int> := [f26];
					f10, globalState.f4, v86, v87[850], v19[258] := f10, true, v86 + (if (f11) then v86 else v86), (seq(0xee, i10  => (v19[258]))) + v88[i6 := -f17], |v88|;
					f11 := f13;
					v19 := v19;
					var v89: array<bool> := new bool[4];
					var v90: set<char> := {f10};
					v19, v19[258], v89, v19[258] := v19, (if (f13) then f26 else |v86|) * p0, v89, |v90|;
			}
			
			var v91: seq<map<bool, int>> := [map[p1 := -0x53]];
			var v92: seq<int> := [|v91|, i6, p0];
			var v93 := DC3(seq(0x3de, i11  => (f17)));
			var v95: array<seq<int>> := new seq<int>[22] [v92, v92 + v92, [f17], v93.cf3, fm45(if (-0x19e in v0) then v0[-0x19e] else 0x40, globalState), v92, [v19[258], f27] + v92, if (p1) then v92 else v92, v92, [f26, f16, v19[258], -0x3e6], seq(0x394, i12  => (f16)), v92[v19[258] := 906], (seq(0x36e, i13  => (f26))) + v92, v92, [0x174, -p0] + [i6], [0x39e, |(set v94 : int | v94 in map[f14 := p1] :: (v94 / 0x251))|, f17], seq(0x378, i14  => (0x55)), seq(49, i15  => (-f14)), v92 + [f14], v92, fm45(f14, globalState), seq(0x36b, i16  => (f17))];
			...
			v95, v96[306], f10, v19[258] := v95, v97 >= ((set v101 : seq<bool> | v101 in v100 :: (v101)) - v97), (if (p1) then DC26(v102[f27], f10, f11) else DC26('b', 'i', f11)).cf30, f26;
			var v103: multiset<bool> := multiset{v96[306]};
			v20 := v20 - fm47((fm48(v96[306], i6, DC3(v92), globalState)).cf21, v103, f11, globalState);
			globalState.f4 := f13;
		}
		r0 := f16;
	}
	method m7(globalState: GlobalState) returns (r0: seq<int>, r1: int, r2: bool, r3: array<int>) {
		...
		var v6 := DC1(f17);
		var v7: array<int> := new int[11] [f17 - f26, -(f26 + |v2|), f14 % f16, -708, |v5.cf28|, f27, f27, f26, f17, f17, v6.cf1];
		v7[504] := f16;
		v7[504] := f14;
		var v8: array<bool> := new bool[3];
		v8[260] := !f11;
		v8[260] := [f13, f11, f13] <= f12;
		v0 := (v0 - multiset{f17}) + (v0 + multiset(v4));
		var v9: seq<array<bool>> := [v8, v8, v8];
		var v10 := DC18(v9[v7[504]]);
		match (v10) {
			case DC19(cf21, cf22) =>
				...
				var v13 := DC5(v12);
				var v14: seq<D1> := [v12];
				var v15: array<D1> := new D1[4] [v13, v13, DC5(v14[fm10(f12[396], globalState)]), v13.(cf7 := v12)];
				var v16: seq<array<D1>> := [v15];
				var v17: array<array<D1>> := new array<D1>[26] [v15, v15, v15, v15, v15, v15, v15, v15, v15, v15, v15, v15, v15, v16[|(seq(0x177, i0  => ('g')))|], v15, v15, v15, v15, v15, v15, v15, v15, v15, v15, v15, v15];
				...
				v17, cf22, r2, cf22 := v17, |(v2 + "njlwqgjtx")|, v21 > (v21 * v21), cf22 * v11.fm5(!v8[260], globalState);
				var v22: map<int, int> := map[v11.f24 + v11.f24 := |"fyum"|];
				v22 := v22[-(if (!f11 in v19) then v19[!f11] else f14) % |v4| := if (v8[260]) then |v18["m" := |(seq(-0x282, i1  => (f14)))|]| else f26];
				globalState.f4 := fm1(f13, f26, globalState);
			case DC20(cf23, cf24) =>
				var v23 := DC1(f17);
				var v24 := DC2(DC2(v23));
				var v25 := DC2(v23);
				var v26: array<D0> := new D0[23] [v25, v25, v25.(cf2 := v23), v25, DC2(v23), v25, v25, v25, v25, v25, v25, v25, v25, v25, v25, v25, v25, v25, v25.(cf2 := v23), v25, DC2(fm51(v7[504], 0x351, globalState)), v25, v25];
				v26[941] := if (f11) then v25 else v25;
				v26[941] := v25;
				f10 := f10;
				var v27: array<C1> := new C1[9];
				var v28: seq<array<C1>> := [v27, v27];
				var v29: seq<seq<array<C1>>> := [v28, [v27], v28];
				cf23, v8[260], cf23, r1, r2 := (f14 + f27) / f27, f13, |([v27] + v29[v7[504]])|, f26, v8[260];
				v2, r1 := v2, v7[504];
			case DC18(cf20) =>
				...
				globalState.f4 := (multiset(([f11])[|f12| := v8[260]]) - v32) >= multiset{v8[260], v31.f22, f13, f11, v31.f22};
				if (f14 != 0x38a) {
					...
					globalState.f3 := (f27 / |v2|) >= (if (v4[f14] in v34) then v34[v4[f14]] else |v35|);
					...
					var v40: array<array<set<bool>>> := new array<set<bool>>[13] [v38, v39[f26], v38, v38, v38, v38, if (f13) then v38 else v38, v38, v38, v38, v38, v38, v38];
					v40[858] := if (fm1(f13, v7[504], globalState)) then v38 else v38;
					v40[858] := new set<bool>[28](i2 => {v31.f22});
			...
			case DC21(cf25) =>
				...
				if (v41 > (v41 - v41)) {
					...
					v43[478] := (if (v44 in v46) then v46[v44] else map[f13 := v8]) + v45;
					var v47: C0 := new C0(0x32, f13);
					var v48 := DC29(v47, v7, f11);
					f13 := v48.cf37;
					var v49: map<bool, int> := map[v47.f22 := f17];
					var v50: map<set<int>, int> := map[{|v49|} := f14 / -f26];
					v50 := v50[v41 := f16 % v7[504]];
				} else {
					var v51 := new C1(f16, f12 + f12, v8[260], f10, f11);
					var v52 := new C0(f16, v8[260]);
					r2 := f11;
					f13 := f14 >= (v52.f21 - f17);
			...
			decreases 100 - i4
		{
			...
			v7[504] := (if (f13) then |map[v4 := f13]| else f26) % (if (|v57| in v0) then v0[|v57|] else f17);
			v57 := v57[f14 := f16];
		}
		r0 := (v4 + v4) + (v4 + v4);
		r1 := f27;
		r2 := (fm46(f13, |v0|, f16, globalState) + f12)[f14 := v8[260]] < f12;
		r3 := v7;
	}
	method m1(p0: int, p1: int, p2: bool, globalState: GlobalState) returns (r0: map<int, bool>, r1: int, r2: int) {
		...
		var v4 := new C1(f14 - |v3|, (fm46(f13, |f12|, f17, globalState))[|[p2]| := f11], f11, f10, false);
		var v5: array<D3> := new D3[19];
		v5 := v5;
		var v6: array<D2> := new D2[1];
		var v7: multiset<array<D2>> := multiset{v6, v6, v6};
		v7 := v7 * (v7 + v7);
		var v8: array<int> := new int[11](i0 => i0 / 0xf1);
		v8 := v8;
	...
	method m2(p0: bool, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: multiset<seq<int>>, r3: seq<int>) {
		...
		match (v3) {
			case DC30(cf38) =>
				...
				r0 := f16 * |(fm47(p0, v4, f11, globalState) - v6[|v7[f14]|])|;
		...
		var v12: array<array<string>> := new array<string>[14] [v9, if (!p0) then v9 else v9, v9, if (f11) then v9 else v9, v9, v10.cf39, v11[f16], v9, v9, v9, v9, v9, v9, if (f13) then v9 else v9];
		v12[57] := v9;
		var v13: set<int> := {f14};
		var v14: C1 := new C1(f27, f12, {v0[f17]} > v13, f10, p1);
		...
		var v21 := DC20(f27, v16[v15 := f27]);
		v12[57], r3, r0, v14, v18 := v9, v0, --(if (f13) then |v13| else v20.cf15), v14, map[f27 := map[v21 := f16] + v17];
		v9[917] := fm6(f17, globalState);
		var v22 := DC4(p1, 0x6a, !p2);
		v14.f24, v9[917], globalState.f4 := f14, v14.fm6(f17, globalState), if (f13) then v22.cf4 else p2;
			...
			decreases 100 - i1
		{
			...
			if (p0 in (if (fm1(p2, |v19|, globalState)) then [p2] else (if (f26 in v25) then v25[f26] else f12[v14.f24 := false])[v14.f24 := p2])) {
				v14.f24 := -0x3cb;
				v15[949] := v14.f24;
				v15[949] := |v13|;
				f13 := p1;
				...
				globalState.f3 := f13;
				var v29: array<bool> := new bool[20] [p1, p1, p1, false, p0, p0, p1, p2, p0, p2, f13, f12[v14.f24], p0, f13, p2, p1, true, f13, !f13, p0];
				var v30: map<int, array<bool>> := map[f16 := v29];
				var v31: map<int, map<int, array<bool>>> := map[v14.f24 := v30];
				r0 := (|(if (f16 in v31) then v31[f16] else v30)| - |v13|) / v22.cf5;
				v14.f24 := f14;
				var v32 := new C0(--951, v14.f24 == 877);
				v15[879] := v14.f24;
				v15[879] := DC4(v32.f22, v32.f21, f11).cf5;
			}
			
			var v33: array<bool> := new bool[9];
			v33[509] := !p2;
			var v35 := DC15(f10);
			var v36: map<int, D8> := map[|v9[917]| := v35];
			v33[509] := ((map v34 : int | v34 in v0 :: (v34 / v14.f24) := (DC15(f10))) + v36) != v36;
		}
		var v37: map<int, bool> := map[f17 := f13];
		r0 := v14.f24 % (fm5(if (|v0| in v37) then v37[|v0|] else p0, globalState) - f17);
		r1 := ("bdqpc" + "akjviwoxd") + (v19 + fm6(f26, globalState));
		var v38: multiset<seq<int>> := multiset{[f27], seq(0x10, i2  => (|map[p2 := true]|)), v0, v0};
		r2 := v38 - v38;
		r3 := v0 + [v14.f24];
	}
	method m18(p0: bool, p1: bool, globalState: GlobalState) returns (r0: bool, r1: bool, r2: int) {
		...
		for i0 := f14 to |f12| {
			var v2: array<int> := new int[9](i1 => i1 % |[f11]|);
			v2[376] := i0 * i0;
			v2[376] := 0x291 + f27;
			var v3 := "bofxihi";
			...
			f11 := v6 < ((seq(-0x135, i2  => (v4))) + (seq(0xc8, i3  => (v4))));
			v2[376] := f17;
		...
		var v10 := "gnflehthh";
		v9[828] := v10;
		var v11: seq<string> := [v10, v10, "cxirxlub", v10[f16 := f10] + v10[f26 := v10[f14]]];
		var v12: multiset<string> := multiset{v10[-f27 := v10[f26]], v10};
		var v13: map<bool, int> := map[p1 := f16];
		var v14: seq<map<bool, int>> := [v13];
		v9[828], v10 := v11[if (fm39(v12, v14, globalState)) then |v10| else -0x115], (v10[f14 := f10])[f14 := f10] + (v10 + "kyensiou");
		var v15 := DC1(f14);
		var v16: map<bool, D0> := map[f11 := v15];
		v16 := v16[p1 := if (false) then v15 else v15];
		var v17: array<char> := new char[22];
		v17[446] := f10;
		v17[446] := if (!false) then 'u' else v9[828][|[false, p1]|];
		v9[828] := v10;
		r0 := f13;
	...
	method m19(p0: int, globalState: GlobalState) returns (r0: seq<int>, r1: string, r2: int) {
		...
		match (v1) {
			...
			case DC28(cf34) =>
				...
				if (multiset(f12) !! v14) {
					...
					globalState.f3 := !((|v16| / f14) >= f26);
					var v17: multiset<int> := multiset{f27};
					var v18: seq<int> := [f16, f27, f26, |v17|, f26];
					v6 := v6[multiset(v18) > v17 := f26];
					var v19 := DC33(v14);
					v14 := v19.cf41[f13 := 584];
					globalState.f3 := f17 == f14;
					var v20: map<char, bool> := map[f10 := true];
					var v21: array<bool> := new bool[21] [f13, !f13, fm39((multiset{"abyauevdd"})[("pp")[f16 := f10] := |v20|], v10, globalState), f11, f13, f11, f11, f13, f13, f11, false, f13, f13, f11, f11, f11, f13, f11, f13, !true, f13];
		...
		match (v33) {
			...
			case DC26(cf30, cf31, cf32) =>
				var v38 := new C1(|v31|, fm46(cf32, 0x275, f14, globalState) + fm46(f12[p0], f14, f17, globalState), cf32, cf30, f13);
				...
				match (v41) {
					case DC19(cf21, cf22) =>
						...
						v47[220] := (if (cf21 in v52) then v52[cf21] else v49) + v49;
					case DC20(cf23, cf24) =>
						...
						var v57: map<int, string> := map[cf23 := seq(0x3e0, i2  => (cf30))];
						var v58: map<D9, int> := map[v41 := v39.f24];
						var v59: seq<map<D9, int>> := [map[v41 := v39.f24], v58];
						var v60: map<int, bool> := map[0x315 := f13];
						var v61: array<map<D9, int>> := new map<D9, int>[24] [(if ((if (f16 in v57) then v57[f16] else v32) in v56) then v56[if (f16 in v57) then v57[f16] else v32] else v58) + map[v41 := |f12|], v58, v58, v58, map[v41 := |v32|], v58, v58, v58 + v58, map[v41 := fm2(v53.f22, |v29|, true, globalState)], v58, v58, map[DC18(v40) := |fm6(|f12|, globalState)|], v58 + v59[v38.f24], v58, v58 + v58, v58 + v58, v58, v58, v58, v59[f27], v58 + v58, v58[v41.(cf20 := v40) := 0xc4], v59[v53.f21], map[DC18(v40) := |v60|]];
					...
					case DC18(cf20) =>
						...
						v66 := |(if ((v38.fm6(0x17, globalState))[f27 := cf30] <= v32) then v32 else v32)|;
						v39.f24 := 0x14b;
						v38.f24 := v66;
					case DC21(cf25) =>
						...
						globalState.f3 := |(v67 + v67)| >= -|(seq(0x2d7, i3  => (f10)))|;
				}
		...
		r1 := if (false) then v32 else (v80[f26])[v77.f21 := f10] + v32;
		r2 := -f16;
	}
}
...
class C3 extends T2 {
	...
	method m1(p0: int, p1: int, p2: bool, globalState: GlobalState) returns (r0: map<int, bool>, r1: int, r2: int) {
			...
			decreases 100 - i0
		{
			...
			var v7 := DC13("owhc", p0);
			var v8 := DC0(f14);
			var v9: array<int> := new int[15] [f28, |v2| / p1, f28 - f14, f28, f28 * p0, -0x22a, if (v5.cf9 in v4) then v4[v5.cf9] else f14, p1, |(map v6 : string | v6 in [v7.cf14, v3, v3] :: (v6) := ({-0x333, f28}))|, p0, p0, -fm2(f13, f14, p2, globalState), v8.cf0 % f28, fm2(p2, f14, f11, globalState), f28];
			v1[207] := if (f11) then -|f12| else f14;
			...
			v16[262] := v4 - v4;
			v16[262] := multiset{"rvab", if (p2) then v3 else v3, "psemhu"};
		}
		var i2 := 0;
		while (!(f11 && f11))
			decreases 100 - i2
		{
			...
			v29[870] := f11;
			v29[870] := fm1(fm1(f13, p0, globalState), f14, globalState);
	...
	method m2(p0: bool, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: multiset<seq<int>>, r3: seq<int>) {
		if (f11) {
			...
			var v5 := "g";
			var v6 := new C2(|[f28, f28, |v1[f28]|, fm2(f11, f14, p1, globalState), |v2|]|, 827, fm55(f14, p0, f11, globalState), p1, f14, |v4|, 720, [p0, p1], f10 !in v5);
			f11 := !p2;
			...
			v13[985], globalState.f4, v15, r3 := v14, if (fm1(p1, |f12|, globalState)) then f11 else f13, map[0x34e := f14] + (map v16 : int | (0x360 <= v16) && (v16 < 0x1c8) :: (v16 - f28) := (f14)), v17 + [f28, f28];
			var v18: array<map<int, bool>> := new map<int, bool>[29];
			var v19: map<int, bool> := map[f14 := p2];
			v18[20] := v19;
			v18[20] := v19[fm2(f13, f28, true, globalState) := true];
			f13 := p2;
			r0 := f28;
		}
		...
		var v26 := "uggccql";
		r1 := v26 + v26;
		var v27: seq<int> := [v21[456]];
		var v28: multiset<seq<int>> := multiset{v27 + v27, (((seq(7, i2  => (f28))) + (seq(0x180, i3  => (f14))))[f28 := v21[456]])[-0x213 := f14]};
		r2 := v28;
		var v29: map<bool, seq<int>> := map[p2 := [v21[456], |v26|]];
...
class C4 extends T5 {
	...
	method m6(p0: int, p1: bool, globalState: GlobalState) returns (r0: int) {
			...
			decreases 100 - i0
		{
			...
			v2[405] := DC20(246, multiset{v2}).cf23;
			var v3 := DC7(v0);
			var v4: set<int> := {f16, f14};
			var v5 := DC23(v4);
			v2[405], v0, globalState.f3 := fm10(f11, globalState), v3.cf9, fm29(globalState) !! v5.cf27;
			v2[405] := p0;
		}
		...
		globalState.f4 := (if ((if (p1 in v6) then v6[p1] else v9) in v8) then v8[if (p1 in v6) then v6[p1] else v9] else f17) in (map[|v10| := multiset(seq(0x348, i2  => (f17)))] + v11);
	...
	method m7(globalState: GlobalState) returns (r0: seq<int>, r1: int, r2: bool, r3: array<int>) {
		...
		for i0 := 888 to -312 {
			...
			var v10 := "q";
			v3[641] := |((if (-f17 in v7) then v7[-f17] else v6) * v6)[v5 := i0 - |(if (v8 in v9) then v9[v8] else v10)|]|;
			r2 := if (DC16(f11).cf18) then f11 else v2.f22;
			f13, globalState.f3, v2.f21 := f14 < (f14 * 638), f13, i0;
		}
		var v11 := "mtrxyjc";
		var v12: map<D8, int> := map[DC16(f11) := f16 % |v11|];
		var v13 := DC16(true);
		...
		if (fm1(f11, |v15|, globalState)) {
			if (if (f14 in v15) then v15[f14] else f13) {
				...
				var v26: multiset<bool> := multiset{fm28(-f17, f12, globalState), false || f13, v25, f12[f14], f11};
				var v27: C0 := new C0(f14, fm28(|(seq(-0xd9, i1  => (f10)))|, f12, globalState));
			...
			match (v36) {
				case DC19(cf21, cf22) =>
					...
					v38 := map[cf21 := f12[cf22]];
					var v39: multiset<bool> := multiset{f13, false, f13};
					cf22 := (cf22 - 0xfe) / (if (f11 in v39) then v39[f11] else |f12|);
				...
				case DC20(cf23, cf24) =>
					...
					var v40: seq<string> := [seq(0x376, i3  => ('h')), "sofn", seq(0xad, i4  => (f10)), v11, v11];
					var v41: seq<int> := [f14];
				...
				case DC21(cf25) =>
					...
					var v56 := new C1(f14, f12, f11, if (f16 in v55) then v55[f16] else f10, v47[366]);
			}
			...
			if (f11) {
				...
				var v60: array<seq<bool>> := new seq<bool>[21] [f12, [f13], f12, f12, f12, f12, f12[|v59| := f13], f12, f12, [f13], f12, f12, f12, f12[|(seq(0x15, i8  => (f10)))| := f11], f12, f12, f12, f12, f12, f12, f12];
				var v61 := DC8(v60);
				var v62: map<int, char> := map[f17 := fm33(v59, f16, globalState)];
				var v63: seq<int> := [f16, f17 - f14, f17];
				v61, r0, v62 := v61.(cf10 := v60), v63, map[f17 % -f16 := f10];
				v11 := v11 + "scsoghc";
				var v64 := DC10();
				var v65 := DC9(f12);
				var v66 := DC11(v65);
				...
				v15, r2 := fm34(|v11| / f14, v64, f17, globalState), if (fm35(globalState) in v69) then v69[fm35(globalState)] else f11;
			} else {
				v0 := v0[f13 := f17];
				var v70: set<array<int>> := {v34};
				v0 := v0[v70 >= v70 := f17];
				var v71: array<string> := new string[16] [v11, v11, v11 + v11, v11, v11, v11, v11 + "xvtkapue", v11, v11, v11 + v11, "skk", v11, v11[f17 := f10] + v11, if (f11) then v11 else v11[|v11| := f10], v11, v11];
				v71 := v71;
			...
			if (v74 >= v74) {
				...
				v75[746] := "rxulrbmfc" + v11;
				v75[746] := seq(0xcb, i9  => (if (false) then f10 else f10));
				f13 := f13;
				...
				var v79 := new C1(v78[865], f12, !true, f10, f11);
				v34[488] := v78[f14];
				v34[488] := DC19(f13, 0x5c).cf22;
			...
			decreases 100 - i11
		{
			...
			v84 := v84;
			var v85: map<multiset<bool>, int> := map[multiset{true} := -f16 / f14];
			var v86: seq<int> := [0x25c, 0x279];
			v85 := v85[fm30(f11, f17, f14, f14, globalState) := v86[f17]];
			v11 := v11;
			r2 := f12[f17];
		}
		var v87: seq<int> := [|v11|, f17 % f17, f17, |v0|, f17 - f14];
		r0 := v87;
		r1 := (f14 / |{f14, f14}|) - f16;
		r2 := f12[|v14[f14 := f17]|] && f13;
		var v88: array<int> := new int[23](i12 => i12 / f14);
		r3 := v88;
	}
	method m1(p0: int, p1: int, p2: bool, globalState: GlobalState) returns (r0: map<int, bool>, r1: int, r2: int) {
		var v0: map<bool, bool> := map[f11 := f13];
		var v1 := "wynmbyi";
		var v2: array<bool> := new bool[17] [false, f11, f13, p2, f11 <== f12[f16], |[v0]| >= -p1, "efpxmwq" == v1, true, true, f13, f13, f11, f11, p2, f12[-f16], f13, f11];
		var v3 := DC13(v1, |v1|);
		v2[158] := p1 != v3.cf15;
		var v4 := DC16(f13);
		...
		v2[158] := (if (v6 in v7) then v7[v6] else v9) > ((set v10 : int | (0x335 <= v10) && (v10 < 379) :: (v10 - p1)) + v9);
		var v11: array<int> := new int[24];
		forall i0 | 0 <= i0 < v11.Length {
			v11[i0] := i0 - f16;
		}
		for i1 := if (v2[158] in v6) then v6[v2[158]] else 102 to p1 {
			var v12: seq<map<bool, bool>> := [v0];
			v0 := map[f11 := true] + (if (v2[158]) then v12[f16] else v0);
			var v13: seq<int> := [fm5(v2[158], globalState)];
			...
			if (f14 !in (if (fm1(f13, p0, globalState)) then v9 else v9)) {
				...
				v2[158] := fm28(p1, f12, globalState);
		...
		f11 := if (f13) then !((set v24 : int | (657 <= v24) && (v24 < 588) :: (v24 / p0)) !! v9) else if (p2) then f13 else p2;
		...
		v26[87] := v29;
		v11[794] := f17 - p1;
		v26[87], v2[158], v2[158], v11[794] := v29, p1 !in (set v30 : int | (0x19a <= v30) && (v30 < 0x2f) :: (v30 / f14)), p2, p1 % f14;
	...
	method m2(p0: bool, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: multiset<seq<int>>, r3: seq<int>) {
		...
		var v3 := "qukmfs";
		var v4 := new C1(|(map v2 : int | (594 <= v2) && (v2 < 0x39) :: (v2 % f16) := (p0))|, f12, p2, f10, fm1(p0, |v3|, globalState));
		globalState.f4 := !p2;
		var v5: seq<string> := [seq(877, i3  => (f10))];
		var v6: map<char, bool> := map[f10 := f11];
		var v7: array<string> := new string[15] ["q" + v3[f17 := f10], v3, v3 + v3, v3 + (seq(400, i0  => (f10))), if (f11) then v3 else seq(792, i1  => (f10)), (seq(0x2ee, i2  => (f10))) + "iksuwfaph", v5[f16], "arm", v3, "kg" + v3, v3, v3, v3, fm6(|v6|, globalState), v3];
		var v9 := DC15('g');
		var v10: map<D8, int> := map[v9.(cf17 := f10) := f16];
		v7 := new seq<char>[19] [seq(0x14a, i4  => (f10)), if (!f13) then seq(0x2a3, i5  => (f10)) else v3, v3, v3, fm6(-528, globalState), v3, "pqrt", v3, seq(-0x1f4, i6  => ('m')), seq(132, i7  => (f10)), v3, v4.fm6(|(map v8 : D8 | v8 in v10 :: (v8) := (p0))[v9.(cf17 := f10) := p2]|, globalState), seq(0xe, i8  => (f10)), v3, if (p2) then seq(0x24a, i9  => (f10)) else "mag", v3, v3 + v3, "bjvpf", v3];
		var i10 := 0;
		while (p1)
			decreases 100 - i10
		{
			...
			var v12: map<string, int> := map[v3 := |(seq(0x1e5, i11  => (f10)))|];
			v12 := v12["aonnf" + v3[v4.f24 := f10] := |v3|];
	...
	method m16(p0: string, globalState: GlobalState) returns (r0: bool) {
		var v0 := DC0(f17);
		var v1: array<int> := new int[4](i0 => i0 % f17);
		var v2: seq<array<int>> := [v1];
		var v3: map<D0, array<int>> := map[if (!true) then v0 else v0 := v2[f14]];
		v3 := v3[v0 := v1];
		...
		for i1 := |(multiset{|v4|} - v7)| to if (f14 in v8) then v8[f14] else f14 {
			...
			var v12: array<multiset<int>> := new multiset<int>[28] [v7, multiset{f14, f16, f16} + v7, v5 * v7, v5, v7, v5, v7, v7[|p0[|v10| := f10]| := 0x1a1], fm4(f13, globalState), v5, v5 - v5, v5 - v6.cf26, v5, v5, multiset{|(seq(-0x27b, i2  => (f10)))|}, v6.cf26, v5, v7, v7, multiset{|v11|}, v5, v7, v5, multiset{f17} - v7, v7 - v5, v7, v7, multiset{f17, i1}];
			v12[601] := v5;
			v12[601] := v7;
			var v13 := 0x2f0;
			v13 := f16;
			...
			decreases 100 - i3
		{
			...
			if (f13) {
				...
				var v26: array<bool> := new bool[19] [f13, fm1(f12[f17], f16, globalState), fm28(f14, f12, globalState), true, f13, fm4(f13, globalState) <= v5, v16 !in v19, true, f13, f11, f11, f12[v0.cf0], f13, v20.cf18, f11, (fm37(f10, |v15|, f11, DC27(v22), globalState)).cf32, f13, v16 == -446, f14 <= |(if (!f11 in v25) then v25[!f11] else [v23, v23, v23, v23])|];
				v17 := v17;
				globalState.f4 := v23.f11;
				var v27: seq<string> := [p0, p0, p0];
				var v28: array<string> := new string[25] [p0, p0, p0, p0, v27[v23.f17], p0, p0 + p0, p0, p0, p0, p0 + "ujdrqsn", p0, p0, p0, seq(-0x44, i5  => (f10)), "ivqbuaar", "bbi", p0[-v23.f17 := 'q'], "fn" + p0, p0, p0, p0, p0, p0, p0];
				v28[653] := p0;
				v28[653] := p0;
				var v29: set<int> := {if (v23.f11 in v4) then v4[v23.f11] else f16, f16, 0x19d};
				var v30: map<bool, bool> := map[true := v23.f13];
				v17[546] := if (v23.f13 in v30) then v30[v23.f13] else fm1(v23.f13, 0x255, globalState);
				v29, v17[546], v16 := {|{v23.f13}|}, v23.f12[f16], 0xc3;
			} else {
				globalState.f4 := f13;
				globalState.f3 := f11;
				var v31: map<int, char> := map[v16 := f10];
				v31 := map[if (f16 in v8) then v8[f16] else |f12| := f10] + v31;
				var v32: map<int, bool> := map[f16 % v16 := f11];
				v32 := map v33 : int | (0x4b <= v33) && (v33 < 0x2f0) :: (v33 % v16) := (f13);
				var v34 := new C0(v16, !f13);
			}
			
			var v36: seq<set<int>> := [set v35 : int | (0x91 <= v35) && (v35 < 0x3cf) :: (v35 % DC19(f13, v16).cf22)];
			var v37 := new C0(|v36| * f14, f11);
			var v38: C1 := new C1(f16, f12[v37.f21 := f11], v37.f22, f10, v37.f22);
		...
		if (|p0| > |fm53(f14, f13, f17, v39, globalState)|) {
			...
			v40[323] := fm54(f11, f14, v41, f13, globalState);
			v40[323] := fm54(f16 < f14, f14, v41, !f13 || f11, globalState);
			var v42 := "bbwiojagb";
			v42 := v42;
			...
			v49[745] := if (v4 in v52) then v52[v4] else !f11;
			r0 := f13;
			var v53 := "liebj";
		...
		if (|v58| != 0x33e) {
			...
			match (DC11(fm35(globalState))) {
				...
				case DC11(cf12) =>
					...
					v66[57] := false;
					var v67 := "exchtf";
					v66[57], v67 := f13, (seq(687, i8  => (f10))) + "tohqv";
					v1[562] := f14 + f16;
					v1[562] := (f16 / fm10(f13, globalState)) / f16;
					var v68: map<int, bool> := map[v1[562] := f13];
					v68 := v68;
					var v69 := DC13(seq(993, i9  => (f10)), |map[f13 := f17]|);
					var v70 := new C2(f14, f16, f10, !f11, f17, |v69.cf14|, 0xfe, [fm1(f11, |f12|, globalState)] + f12[v1[562] := f13], v66[57]);
			}
			
			if (f11) {
				var v71: multiset<char> := multiset{f10, fm33(v7, f14, globalState)};
				v8 := (v8 + v8)[-(if (f10 in v71) then v71[f10] else f17) := f17 / -509];
				...
				v75 := f17;
				var v78 := new C3(v0.cf0, f17, v76.f12[v76.f14 := v76.f13] + f12, fm1(!v76.f13, f17, globalState), f10, v5 >= v5);
				var v79: array<char> := new char[16];
				v79[451] := p0[|v5|];
				v79[451] := fm50(globalState);
			}
	...
	method m17(p0: int, p1: bool, p2: map<bool, int>, p3: bool, globalState: GlobalState) returns (r0: int, r1: int) {
		...
		match (v1) {
			case DC14(cf16) =>
				...
				if (fm1(f13, |v6|, globalState)) {
					...
					var v20 := DC8(v11);
					var v21: map<int, array<seq<bool>>> := map[-|v5| / f14 := v20.cf10];
					f13, f13, v11, v12, v13[525] := !v18.cf37, v16.f22, if ((f14 - f17) in v21) then v21[f14 - f17] else v11, v12, v15;
				} else {
					...
					var v27: map<string, set<bool>> := map[v24 := v25[|[v26, v26]|]];
		...
		if (f11) {
			...
			v45[757] := map[v42 := f11] + map[v42 := false];
			var v46: seq<int> := [f16 / |[v40.f28, v40.f28, f14, |v38|]|];
			var v47 := "knii";
			var v48: map<map<int, bool>, int> := map[v0 := f14];
			r0, v45[757], r1 := v46[|("fsy" + v47)|], v43 + v43, |v48|;
			f10 := f10;
			...
			var v50 := "agijmsl";
			globalState.f3 := |v50[-0x13d := f10]| != p0;
			if (!p3) {
				globalState.f4 := f14 > (f14 % f16);
				var v51: set<bool> := {f11, p1};
				var v52: map<int, seq<bool>> := map[|v51| := f12];
				var v53: map<bool, bool> := map[p3 := f13];
				var v54: map<string, map<bool, bool>> := map[(seq(0x3e4, i2  => ('i')))[|v52| := v50[f17]] := v53];
				...
				var v61 := DC29(v58, v60[f14], f13);
				var v62: seq<seq<bool>> := [f12];
				var v63: array<bool> := new bool[13];
				var v64: map<array<bool>, map<int, bool>> := map[v63 := v0];
				var v65: array<int> := new int[13] [f17 * 0x38a, |(v57[f11 := |v51|])[(v61.(cf37 := p3)).cf37 := |v57|]|, v58.f21, |v50|, |f12[|v62| := false]|, fm2(f11, -v58.f21, p3, globalState), -0x3ba / 607, f14, p0, -(v58.f21 / -f14), |(v64 + v64)|, p0 * f16, if (true) then v58.f21 else f16];
				v59[980] := p0;
				var v66: array<string> := new string[21];
				v66[817] := v50;
				globalState.f3, globalState.f3, v59[980], v66[817] := p3, p1, 0xef, v50;
				v58.f21 := |v50| / v58.f21;
			} else {
				globalState.f4 := p3;
				globalState.f3 := f11;
				...
				v76[603] := [v74, v74];
				var v77: map<seq<bool>, seq<C0>> := map[f12[p0 := f11] := v75];
				v73, r0, v76[603], r1 := v73, -f14, if (f12[f17 := p3] in v77) then v77[f12[f17 := p3]] else v75, -v71.fm10(f13, globalState);
...
class C5 extends T3 {
	...
	method m3(p0: multiset<map<int, int>>, globalState: GlobalState) {
		...
		match (v2) {
			case DC4(cf4, cf5, cf6) =>
				...
				cf5, cf5, cf5, globalState.f4 := f14 / f14, cf5, v1[|v3|], !(569 !in v4);
				cf5 := f14;
			...
			case DC3(cf3) =>
				...
				v12[345], v11, v13 := if (v18 in v17) then v17[v18] else fm5(f11, globalState), |v19| % v11, v13;
				var v20 := new C0(|v13| - v12[345], f11);
				v12[345] := v20.f21 % |(f12 + DC9(f12).cf11)|;
			case DC5(cf7) =>
				var v21 := "cwfidpny";
		...
		var i1 := 0;
		while (fm1(f11, 0x2ee % f14, globalState))
			decreases 100 - i1
		{
			if (i1 >= 100) {
				break;
			...
			v29[957] := v30;
			f10, v29[957], globalState.f3 := f10, v30, f11;
			globalState.f4 := f13;
			...
			match (v33) {
				case DC10() =>
					...
					v29[957], f13, v36[647], v28, v36 := v30, !f13, f14 / v34.f21, |(f12 + fm15(globalState))|, if (v34.f22 in v37) then v37[v34.f22] else v36;
					var v38 := "i";
					v29[957][919] := v38 <= v38;
					v29[957][919] := f13;
				case DC9(cf11) =>
					...
					v43 := v43 + v43;
					var v45: array<map<int, int>> := new map<int, int>[8](i3 => map v44 : int | (0x16d <= v44) && (v44 < 0x2cb) :: (v44 % |v1|) := (v28));
	...
	method m4(p0: int, p1: map<int, char>, p2: array<string>, p3: int, globalState: GlobalState) returns (r0: set<bool>, r1: array<int>, r2: bool, r3: bool) {
		for i0 := p0 to p0 {
			var v0: map<int, char> := map[p3 / |f12| := if (f11) then f10 else f10];
			v0 := v0 + p1;
			var v1 := "cvpugb";
			v1 := v1;
			...
			decreases 100 - i2
		{
			...
			v32[101] := DC4(f13, f14, f13);
			var v33: array<multiset<int>> := new multiset<int>[20](i4 => multiset(seq(864, i5  => (p3))) - multiset{0x283});
			v21[926] := f13;
			v22, v31, v32[101], v33, v21[926] := v31.f21, v31, v20, v33, v23.f11;
			v22 := (f14 + v22) * p3;
		}
	...
	method m1(p0: int, p1: int, p2: bool, globalState: GlobalState) returns (r0: map<int, bool>, r1: int, r2: int) {
		...
		for i0 := p0 to if (f11 in v0) then v0[f11] else p0 {
			...
			v1[708] := v2.(cf14 := v3);
			if (f13) {
				r1 := -((|f12| + -p0) / |(seq(-0x3c3, i1  => (map[p2 := f11])))|);
				var v4: set<bool> := {f13, p2};
				var v5: map<int, int> := map[|v3| := |v4|];
				var v7: seq<map<int, int>> := [v5, v5, map v6 : int | (0x5d <= v6) && (v6 < 0x2ba) :: (v6 * f14) := (i0), map[p0 := p0], v5[f14 := |(seq(-835, i2  => (f10)))|]];
				var v8 := new C0(|v7[388]|, f11);
				...
				v9 := (((seq(992, i4  => (43)))[-v8.f21 := fm5(f11, globalState)])[f14 := if (f13 in v0) then v0[f13] else f14] + (seq(251, i5  => (|v9|)))) + v9;
				...
				var v20 := DC7(v3);
				r1, v13.f24, r1, v3, globalState.f3 := v13.f24, (v13.fm18(f13, i0, f13, false, globalState).(cf14 := v3)).cf15, p1 / p0, (v20.(cf9 := v3)).cf9, f11;
				var v21: seq<int> := [137];
				var v22: map<seq<bool>, bool> := map[(f12[p0 := f13])[|v21| := f11] := true];
				v22 := v22[f12 := f13];
			}
			...
			match (DC22(v23)) {
				case DC22(cf26) =>
					f11 := !f11;
					var v24: seq<int> := [p1, p0 % -(if (|fm27(0x16, false, globalState)| in v23) then v23[|fm27(0x16, false, globalState)|] else |{i0}|), fm5(p2, globalState), p0 - i0];
					v24 := (seq(0x2c5, i6  => (p1))) + v24;
					r2 := f14;
		...
		var v30: array<int> := new int[14] [v26[|v27|], p1, p0, p0, p0, |v28|, f14, 627, |v29|, p1, p0, p1, f14, p0];
			...
			decreases 100 - i8
		{
			...
			var v38: T5 := new C2(f14, p1, f23, p2, p0, f14, |v28|, f12, v33[60][|v37[p1]|]);
			var v39: seq<T5> := [v38, v38];
			var v40: set<seq<T5>> := {v39};
			var v41: map<char, set<seq<T5>>> := map[v38.f10 := v40];
			var v42: seq<set<seq<T5>>> := [v40, v40];
			var v43: array<set<seq<T5>>> := new set<seq<T5>>[17] [v40 - {v39, v39}, {v39, v39}, v40, v40, v40, v40, v40 + v40, {v39, v39, v39, v39} + v40, v40, {[v38]}, if ('d' in v41) then v41['d'] else v40, v40, {v39, v39} + v40, v40, v42[|v28[v38.f17 := f10]|] + v40, v40 + {v39, [v38, v38]}, v40];
			v43[273] := {v39};
			v43[273] := v40 + v40;
			...
			v48, v51[682], r2, f11 := v48, v47.f22, p0, v52 > multiset{v51};
		}
		...
		v54 := v54[v28 <= v28 := if (f11 in v54) then v54[f11] else v55[v28 := |v28|]];
		var v56 := DC1(p0);
		var v57 := DC2(v56);
		...
		r1, r1, r2, f11 := fm5(f13, globalState), -match v57 {
			...
			case DC0(cf0) => p1
			case DC2(cf2) => p0
		}, (if (p2) then f14 else |v28|) % |{!f11, f11, true}|, v30 in v58;
		globalState.f3 := f13;
	...
	method m2(p0: bool, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: multiset<seq<int>>, r3: seq<int>) {
		...
		if ((v2 - v2) !! {f11}) {
			...
			var v8: array<int> := new int[29] [f14, |f12|, f14, f14, f14, 7, if (--0x121 in v4) then v4[--0x121] else f14, f14, v5.cf0, f14, f14, f14, f14, f14, f14, |(map v6 : int | (904 <= v6) && (v6 < 0x2ba) :: (v6 % f14) := (f14))|, f14, f14, f14, f14, f14, |v7|, f14, f14, f14, f14, 26, f14, 601];
			...
			v14 := v14[if (true) then set v15 : int | v15 in {v10.f21, 0x345} :: (v15 - |{true, false}|) else v12 := seq(0x2e3, i0  => (f10))];
			globalState.f4 := f13;
			f13 := v10.f22;
			...
			v19[558], globalState.f4, v0 := {f14} - v20, v21 == v21, v0 + v0;
			if (fm1(f13, 0x23e, globalState)) {
				var v22: C3 := new C3(f14, f14, f12, p1, f10, p1);
				v22 := v22;
				var v23: array<int> := new int[22](i2 => i2 / -0x2e3);
				v23[433] := fm5(true, globalState) % f14;
				v23[433] := f14;
				v17 := v17[f14 := v22.f28 != -v22.f28];
				f10 := f10;
				var v24 := "i";
				var v25 := new C2(|(v24 + v24)|, v22.f28, f10, f23 !in v24, if (p2) then v22.f28 else v22.f28, v23[433], -v22.f28, f12, v23[433] !in v21);
			} else {
				...
				var v30: array<int> := new int[5] [f14 * f14, f14, f14 % -f14, f14, if (f14 in v29) then v29[f14] else -360];
				v30 := v30;
				...
				r0 := |(v29 * multiset(v21))[|v32| := f14 / f14]|;
				var v33 := "stxnvc";
				r1 := v33;
				globalState.f3 := p1;
			}
		...
		v34[113] := v38 != ((set v39 : multiset<D12> | v39 in multiset{v37} :: (v39)) + (set v41 : multiset<D12> | v41 in v40 :: (v41)));
		var v42 := "v";
		var v43: seq<int> := [f14];
		var v44: seq<seq<bool>> := [f12];
		var v45 := new C2(f14, |v42|, f23, v34[113], -(f14 * 162), v43[f14], f14, v44[f14], false ==> p0);
		var i4 := 0;
		while (!v34[113])
			decreases 100 - i4
		{
			...
			i4 := i4 + 1;
			var v46: map<int, bool> := map[v45.f26 := v34[113]];
			v46 := v46 + v46;
			...
			var v50: seq<seq<int>> := [(seq(768, i5  => (-v45.f27)))[v45.f26 := |v49|]];
			var v51: multiset<seq<seq<int>>> := multiset{v50, v50[0x123 := v43]};
		...
		r2 := v55[v43 := fm2(v34[113], f14, false, globalState)] - fm59(globalState);
		r3 := v43;
	}
}
...
class C6 extends T4 {
	...
	function fm69(p0: bool, p1: int, p2: map<bool, int>, globalState: GlobalState): bool {
		if (f13) then (seq(0x2fb, i0  => (f10))) == "roxkic" else f11
	}
	method m5(p0: int, p1: int, p2: seq<D0>, p3: int, globalState: GlobalState) returns (r0: bool) {
		var v0 := -433;
		v0 := p1;
		...
		for i0 := p0 to |v1| / v0 {
			...
			globalState.f3 := -v5[|f12|] >= p0;
			r0 := true;
		}
		for i1 := p3 to v0 {
			f10 := f10;
			var v6: array<string> := new seq<char>[2](i2 => seq(927, i3  => (f10)));
			...
			var v12 := DC5(v11);
			v8[946] := p1 - fm9(f12, seq(-220, i4  => (-|map[p3 := f11]|)), v12, globalState);
			var v13: array<map<int, int>> := new map<int, int>[17];
			v13 := v13;
		}
		v0 := p3 * 949;
		var v14: array<int> := new int[1](i5 => i5 / f14);
		v14[406] := p1;
		v14[583] := fm5(f11, globalState);
		var v15 := "hlwvfvxwj";
		var v16: set<int> := {p0, p0, f14};
		var v17: seq<set<int>> := [v16];
		f13, globalState.f4, v14[406], v14[583] := (seq(418, i6  => (f10)))[|f12| := f10] < v15, !!f13, -|v17[f14]|, p0;
			...
			decreases 100 - i7
		{
			...
			v15, f15[767], f10, v20 := if (f11) then v15 else v15 + "vndudf", f13, (fm70(f14, globalState)).cf17, v20;
			v14[406] := f14;
			var v21: seq<array<int>> := [v14];
			var v22: array<array<int>> := new array<int>[11] [v14, v21[p3], v14, v14, v14, v21[|(seq(0x2de, i8  => (f10)))|], v14, v14, v14, v14, v14];
			v22[842] := v14;
			v22[842] := v14;
		}
		r0 := f13;
	...
	method m3(p0: multiset<map<int, int>>, globalState: GlobalState) {
		...
		match (v1) {
			case DC25(cf29) =>
				...
				var v5: set<map<bool, int>> := {v4[f14], fm0(f11, 0x364, f14, f13, globalState), v3};
				...
				v16 := v16[f12 := v2];
				var v17 := 501;
				v17 := |(if (f13) then v2[v17 := fm71(globalState)] else "nrkpw")|;
			...
			case DC26(cf30, cf31, cf32) =>
				...
				v24[141] := if ((v27 >= v27) in v25) then v25[v27 >= v27] else f12;
			...
			case DC27(cf33) =>
				...
				f15[675] := !(v34[f10 := -f14] >= multiset(v35));
				f15[675] := f14 > f14;
				globalState.f3 := f13;
				...
				if (f11) {
					...
					var v40 := DC4(f15[675], fm7(v39, f14, globalState), f13);
					var v41: multiset<bool> := multiset{v40.cf4, f15[675], fm69(f13, f14, map[false := fm5(f11, globalState)], globalState), f13, !f13};
					v41 := if (f12[f14]) then v41 else v41 * v41;
					var v42 := 0x387;
					v42 := 0x8d / f14;
					var v43: seq<int> := [v42];
					var v44: array<seq<int>> := new seq<int>[5] [(seq(455, i2  => (|f12|))) + v43, v43, v43, v43, v43];
					v44 := v44;
					v35 := v35;
					v37[406] := v42;
					v37[406], f15[675] := -f14, v41 > (v41 - multiset{f11, f11});
				} else {
					var v45: multiset<bool> := multiset{f11, !f13};
					var v46: map<int, bool> := map[f14 := f15[675]];
					...
					var v49: set<multiset<bool>> := {v45, v45, v48[f14]};
					f15[675] := v49 <= (if (f11) then set v50 : multiset<bool> | v50 in v48 :: (v50) else v49);
					var v51 := 0x3b9;
					v51 := v51;
					var v52: map<int, int> := map[f14 := f14];
					var v53 := m0(f14, v52, f12[v51], !f15[675], globalState);
					v35 := "uftcknuk";
					f15[675] := if (v51 < -v53) then f13 else f11;
				}
		...
		for i3 := f14 to f14 * |v56| {
			...
			var v58: array<seq<bool>> := new seq<bool>[6] [f12, [f11, f13], f12, f12[i3 := f13], f12, [true, if (f11 in v57) then v57[f11] else f11]];
			v58 := v58;
			f11 := f13;
			f10 := 'j';
			f13 := false;
			...
			decreases 100 - i4
		{
			...
			i4 := i4 + 1;
			globalState.f4 := f13;
			var v60: array<int> := new int[25](i5 => i5 % |(seq(131, i6  => ({f13})))|);
			v60[250] := f14;
			v60[250] := f14;
			if (false) {
				...
				v62[346] := v60;
				v62[346] := v60;
				var v63: map<string, string> := map[(seq(0x62, i8  => (v54.f23))) + v59 := seq(685, i9  => (v54.f23))];
				var v64: seq<int> := [f14, --0x1fd, -v60[250], v60[250]];
				v63 := v63[v59[|v0| := v54.f23] + "ffkdjmr" := v59[|[[|v64|]]| := f10]];
			} else {
				v60[250] := 0x14f / v60[250];
				var v65: multiset<string> := multiset{v59, v59};
				v60[250] := v54.fm7(v65, v60[250] * v60[250], globalState);
				var v66: multiset<int> := multiset{|(f12 + f12)|, v60[250] - f14};
				var v67: map<bool, bool> := map[f13 := f11];
				v60[250] := if ((|v67| % |(map v68 : int | v68 in v0 :: (v68 * |(seq(-481, i10  => (v54.f23)))|) := (v60[250]))|) in v66) then v66[|v67| % |(map v68 : int | v68 in v0 :: (v68 * |(seq(-481, i10  => (v54.f23)))|) := (v60[250]))|] else |f12|;
				v60 := v60;
			...
			decreases 100 - i11
		{
			...
			f11 := (f14 / v69) > fm2(true, -v69, f11, globalState);
			var v72: array<int> := new int[27](i12 => i12 + |v59|);
			var v74: seq<int> := [|f12|];
			v72[111] := |(map v73 : int | v73 in fm72('i', |v74|, f12, f14, globalState) :: (v73 + v69) := (false))|;
			v72[785] := 0x25a;
			f15[904] := f11;
			v72[111], v72[785], v69, f15[904] := v69, v69, f14, -0x20b > (f14 + f14);
		}
		for i13 := f14 + f14 to f14 {
			...
			var v78: array<string> := new seq<char>[2] [v59, ("mbte")[f14 := v54.f23]];
			var v79, v80, v81, v82 := v54.m4(-(i13 % i13), map[|v75| := 'i'] + v77, v78, f14, globalState);
			if (f13) {
				var v83, v84, v85, v86 := v54.m2(f15[677], !f13, if (fm8(true, globalState)) then true else f15[677], globalState);
				var v87: seq<multiset<int>> := [v76, multiset(v86), v76];
				var v88: array<multiset<int>> := new multiset<int>[12] [v87[f14], v76 * v76, v76, multiset{v83}, v76, v76, v76 - v76, multiset{0x11e}, v76, v76 + v76, v76 - v76, v76];
				v88[596] := v76;
				var v89 := DC41(i13, |f12|, v84, f11);
				v88[596] := v76[v89.cf55 / -f14 := |v59|];
				var v90: map<int, int> := map[|(multiset{f11})[f11 := |v59|]| := i13];
				...
				v93, globalState.f3, v97, v80 := [v98, v98, v98, map[v92 := v83], v98 + v98], f11, fm73(v54.f23, f15[677], f13, globalState), v80;
				var v99 := DC15(v54.f23);
				var v100: array<char> := new char[16] [v54.f23, 'b', v84[f14], f10, fm71(globalState), f10, 'q', f10, v84[i13], f10, v99.cf17, v54.f23, v84[v92.f27], v54.f23, v54.f23, v54.f23];
				v100 := v100;
				var v101 := new C2(i13, f14, 'k', f11, i13, |v84|, if (v81) then |map[v54.f23 := v92.f27]| else f14, f12, v81);
			} else {
				globalState.f4 := !f11;
				var v102: seq<int> := [f14];
				var v103: array<bool> := new bool[3] [f15[677], false, f13];
				var v104: map<int, array<bool>> := map[|(v102 + [f14])| := v103];
				v104 := v104;
				var v105 := new C3(f14 % 0x2df, i13, f12, !v82, v54.f23, v82);
				f15[677] := f15[677];
				var v106 := DC19(f15[677], |v79|);
				var v107 := new C2(if (f11) then f14 else -|f12|, v106.cf22, v59[|v59|], f13, -426, f14, |v59| - v105.f28, fm74(v59, f10, globalState), v81);
			}
			...
			var v111 := DC17(v110);
			v109[741] := if (v82) then v111 else v111;
			var v112: map<int, string> := map[i13 := v59];
			var v113 := DC36(f13, v82, v54.f23, f13, v108);
			v108, v108, v0, v108, v109[741] := f14 % |"isy"|, -(if (f15[677]) then |(if (i13 in v112) then v112[i13] else "mi")| else 0x24c) % 0x188, v0, f14 + v113.cf48, v111;
		}
	}
	method m4(p0: int, p1: map<int, char>, p2: array<string>, p3: int, globalState: GlobalState) returns (r0: set<bool>, r1: array<int>, r2: bool, r3: bool) {
		if (false <== f11) {
			...
			var v14 := new C5(f10, v13.f26, f12, |(seq(0x305, i0  => (f14)))| < v13.f26, f10, f13);
		} else {
			var v15: set<int> := {f14};
		...
		if (v22 !! v22[f11 := p3]) {
			...
			if (!f13) {
				...
				var v52: array<string> := new string[24] [v46, v46 + v46, v46, (v46 + v46)[v23.f21 := f10], v46, v46, "wtx" + v29.fm6(|v47|, globalState), v46, v46 + v46, v46, v46, "d", seq(0x4b, i1  => (f10)), v46 + v46, (fm6(f14, globalState))[p0 := f10] + (seq(0x214, i2  => (f10))), v46, v46, "hpx" + "suwxtjem", if (f12 in v48) then v48[f12] else v46[fm9(f12, [p3], v49, globalState) := f10], if (|v29.fm6(-133, globalState)| in v50) then v50[|v29.fm6(-133, globalState)|] else v46, "dcjrxt", v46, v46, v46[v51.cf15 := f10]];
				v52 := v52;
				v46 := v46;
			}
			...
			var v56 := new C5('n', fm7(v55["qphuybeh" := f14], f14, globalState), [f13, f13] + f12, f13, f10, p0 < |v22[f11 := -p0]|);
			var v57: seq<string> := ["xkxrtd"];
			var v58: map<bool, int> := map[f13 := -v56.fm7(multiset(v57), f14, globalState)];
			var v59 := new C0(p3, fm69(f13, |(seq(-0xe1, i3  => (f10)))|, v58, globalState));
			var v60 := DC10();
			var v61 := DC11(v60);
			match (v61) {
				case DC10() =>
					...
					v62[433] := f14;
					v62[433] := p3;
					globalState.f3 := v22[f11 := v59.f21] > v22;
					f11 := -(-p0 * |[f14]|) >= p0;
					var v63: seq<bool> := [f11];
					r2, v62[433], v63 := v59.f22, (if (fm8(v59.f22, globalState)) then v59.f21 else v59.f21) + |(if (v59.f22) then v54 else v54)|, f12[f14 := !f11] + (f12 + f12);
				case DC9(cf11) =>
					var v64: seq<int> := [|v54|];
					var v65: map<int, int> := map[|v64| := 0xd7];
					var v66: seq<int> := [|v65|];
					var v67: set<map<int, int>> := {fm72('u', -335, f12, p3, globalState), v65, v65};
					var v68 := DC57(v64[p3], [v59.f21, p0, v59.f21, p3, v59.f21], v67);
				...
				case DC11(cf12) =>
					...
					v71[107], v54, v72[389], v73 := if (fm8(!false, globalState) in v74) then v74[fm8(!false, globalState)] else f10, fm6(v59.f21, globalState), v59.f21, v73;
					var v75: set<char> := {v71[107]};
					var v76: set<array<bool>> := {f15, DC43(f15, v75, f13).cf60, f15};
					var v77: multiset<int> := multiset{0x0, -|v76|};
					v72[389] := -v72[389] / |v77|;
					var v79 := DC24(map v78 : int | (0x4c <= v78) && (v78 < -0x26b) :: (v78 - v72[389]) := (v56.f23));
					v79, v54 := v79, v54 + v54;
		...
		match (v83) {
			case DC19(cf21, cf22) =>
				...
				if (cf21) {
					...
					var v87: map<bool, seq<bool>> := map[v22 !! v85[|v86|] := f12];
					v87 := v87[f11 := [f11, f13]];
					var v88 := "ilghtb";
					...
					v91 := v91 + (seq(-0x2a7, i5  => (p0)));
					var v92 := DC18(v81[253]);
					v92 := v92;
			...
			case DC20(cf23, cf24) =>
				...
				var v118 := new C3(p0, -cf23, f12, (seq(-0x399, i7  => (v117.f28))) < (seq(0xb5, i8  => (|f12|))), f10, cf23 == p3);
				f15[951] := false <==> true;
				f15[951] := f13 <== true;
			...
			case DC21(cf25) =>
				...
				var v131: array<int> := new int[18](i9 => i9 / v124.f21);
				...
				if (f13) {
					...
					v124.f21, v124.f21 := if (!(v153[v154.cf78] && false)) then v124.f21 else f14, f14 - v124.f21;
					...
					v160[938] := 'a';
					var v161 := DC34(f13);
					f15[396], f15[491], globalState.f3, v160[938] := f11, v127 <= v127, v161.cf42, f10;
				} else {
					...
					f15[361] := v124.f22 <==> f11;
					v133, globalState.f3, f15[361] := v133[-(f14 + v165.f24) := f10], false, v124.f21 != 153;
					globalState.f4 := (v130 + v130) >= v130;
					f15[361] := (if (f11) then f11 else f13) && fm8(false, globalState);
		...
		forall i11 | 0 <= i11 < v81[253].Length {
			v81[253][i11] := !(f14 <= -f14);
		}
		v81[253] := f15;
		...
		var v176: multiset<char> := multiset{f10, f10, v175[p0]};
		var v177: array<int> := new int[24] [0x249, f14, fm9([f11], v170, v173, globalState), f14, p0, p3, --0x331 * 0x2c8, p0 / p0, if (f11) then 56 else p3, f14, p3 + p3, fm9(f12, v170, v173, globalState), |v174|, |(v176 * v176)|, |v175|, p3, p0, p0, f14, f14, f14, p0 * f14, p3, p0];
		r1 := v177;
	...
	method m1(p0: int, p1: int, p2: bool, globalState: GlobalState) returns (r0: map<int, bool>, r1: int, r2: int) {
			...
			decreases 100 - i0
		{
			...
			i0 := i0 + 1;
			var v0: array<int> := new int[19](i1 => i1 / |map[p2 := multiset{p0, f14}]|);
			...
			if (f11 <==> (f13 <==> p2)) {
				f15[948] := f11;
				v0[548] := f14;
				f15[948], v2, v0[548], r1 := p2, if (f11 <==> p2) then v2 else v2, p1 - p0, p1;
				f10 := f10;
				var v4: multiset<int> := multiset{v3.f24, |multiset(f12)|};
				var v5: map<bool, bool> := map[f15[948] := f13];
				var v6: seq<int> := [p0, p0];
				var v7: map<int, int> := map[v0[548] := v3.f24];
				var v9: map<bool, map<int, int>> := map[p2 := v7];
				var v10: set<map<int, int>> := {v7, map[|f12| := v0[548]], map v8 : int | v8 in v4 :: (v8 % v3.f24) := (|(seq(-0x15c, i2  => (f10)))[v3.f24 := f10]|), if (p2 in v9) then v9[p2] else v7[0x3cf := v3.f24]};
				var v11 := DC57(|v5|, v6, v10);
				var v12: array<multiset<int>> := new multiset<int>[9] [v4 + v4[v3.f24 := v0[548]], v4, v4, v4 - multiset(v11.cf84), v4, v4, v4, v4, multiset(v6)];
				v12[490] := v4;
				v12[490] := multiset{-0x3e7};
				var v13: map<bool, int> := map[!f15[948] := p0];
				r1 := |v13| % v0[548];
				var v14: map<int, bool> := map[v3.f24 := f11];
				var v15 := DC41(p0, v0[548], ("vjb")[f14 := 'k'], f11);
				v14 := v14[v0[548] := v2 <= v15.cf57];
			} else {
			...
			match (DC10()) {
				case DC10() =>
					...
					v0[233] := |(if (false in v29) then v29[false] else v2)| + f14;
					...
					v3.f24, v0[233], globalState.f3, f13, globalState.f4 := |(fm78(f10, f11, f11, globalState) + v30)[f11 && f11 := f10]|, p1, v33 != v33, f13, if (f10 in v34) then v34[f10] else if (!f13) then f11 else !f13;
					var v35: array<int> := new int[17];
					v35 := new int[22](i3 => i3 % p1);
					var v36: map<int, char> := map[v0[233] := 's'];
					v36 := v36[|v33| - v3.f24 := fm71(globalState)];
					globalState.f4 := !(0x24d == 0x39a) ==> f12[|v2|];
				case DC9(cf11) =>
					var v37: array<string> := new seq<char>[27] [v2, v2, v2, v2, v2, v2, v2, seq(0xd4, i4  => (f10)), "lfdwgry", v2, v2, v2, seq(752, i5  => (f10)), v2, v2, v2, v2, v2, "dlatlbs", v2, v2, v2[|cf11| := f10], v2, v3.fm6(|cf11|, globalState), fm6(v3.f24, globalState), v2, v2];
				...
				case DC11(cf12) =>
					...
					var v47: array<char> := new char[15] [fm71(globalState), f10, f10, if ((if (|multiset(fm6(0x334, globalState))| in v46) then v46[|multiset(fm6(0x334, globalState))|] else p1) in v45) then v45[if (|multiset(fm6(0x334, globalState))| in v46) then v46[|multiset(fm6(0x334, globalState))|] else p1] else f10, f10, 'c', f10, f10, f10, f10, f10, f10, f10, f10, v2[|v2|]];
					v47[121] := f10;
					v47[121] := 'i';
		...
		var v56 := "mvipcud";
		var v57 := DC58(v51);
		var v58: array<int> := new int[25] [p1, p0, p0 / |v50|, p0, |f12|, 0x28b, p1, p1, p0 - |v51|, |({v53} + v54)|, p1, -|fm79(globalState)| - p0, if (p0 in v55) then v55[p0] else 0x295, f14 - |v56|, p1, f14 / p1, f14, 0xe4 + p1, p1, f14, f14, |v57.cf86| * f14, p0, p0, 928];
		forall i8 | 0 <= i8 < v58.Length {
			v58[i8] := i8 / (|[seq(0x14a, i9  => (|v56|))]| / p0);
		}
		...
		if (f13) {
			...
			v58[43] := (if (v59 in v62) then v62[v59] else -p0) + f14;
			v58[43] := |f12| + |v56|;
			var v63: seq<string> := ["jnvojoblw"];
			var v64: map<int, bool> := map[p0 := f13];
			var v65: array<string> := new string[24] [v56 + (v63[|v64|])[p1 := f10], fm6(|"syutpi"|, globalState), v56, v56, v56 + v56, v56 + v56, v56 + "ktfcwjkwc", v56, v56 + v56, v56, fm6(|multiset{p1, f14}|, globalState), v56 + v56, v56 + v56, v56[f14 := 'a'], v56 + (seq(798, i11  => (f10))), v56, "vkncnvj", v56, v56 + v56, ("oihwvlpo")[f14 := f10], v56, "pvboctivj" + v56, v56 + v56, ("i")[p0 := f10] + v56];
			v65[247] := if (f11) then v56 else v56;
			v65[247] := v56;
			var v66 := new C0(|(if (!p2) then v49 else v49)|, f11);
			v66.f21 := -(if (p2) then -(v66.f21 - v58[43]) else f14);
		} else {
			r2 := p0 + p1;
			var v67 := new C1(p0, f12, f13 in f12, if (!f13 in v51) then v51[!f13] else f10, true);
			var v68: seq<string> := [v56 + v56, v56, v56];
			var v69: map<int, seq<int>> := map[p0 := seq(540, i12  => (|multiset{f13}|))];
			var v70: seq<seq<int>> := [v49];
			v56 := v68[if (f11) then p0 else |(if (|f12| in v69) then v69[|f12|] else v70[0x27c])|];
			v56 := v56;
			if (if (!fm1(f11, p1, globalState)) then f11 else !(|v49| == v67.f24)) {
				f15[353] := f13;
				f15[353] := f11;
				f13 := f15[353];
				var v71: map<int, bool> := map[p1 := f15[353]];
				var v72: map<bool, map<int, bool>> := map[f13 := v71];
				var v73: array<map<int, bool>> := new map<int, bool>[5] [v71, map[v67.f24 := f13], v71, v71, v71 + (if (f15[353] in v72) then v72[f15[353]] else fm80(f14, 'm', globalState))];
				v73[799] := v71;
				v73[799] := map[v67.f24 := f15[353]] + v71;
				globalState.f4 := false;
				var v74 := DC14(v71);
				v74 := v74;
			} else {
	...
	method m2(p0: bool, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: multiset<seq<int>>, r3: seq<int>) {
		...
		r0 := |v6|;
		r1 := seq(74, i3  => (DC15(f10).cf17));
		r2 := multiset{seq(0x34f, i4  => (f14)), v2 + v2, v2};
		r3 := v2;
	}
	method m21(p0: int, p1: bool, p2: bool, globalState: GlobalState) returns (r0: map<int, seq<int>>, r1: int, r2: int, r3: set<string>) {
		...
		for i0 := -|f12| to p0 {
			...
			if (!!(v2 <= v2)) {
				...
				var v16 := new C0(if (|(if (793 in v10) then v10[793] else map v11 : int | (-117 <= v11) && (v11 < 0x2d5) :: (v11 / f14) := (p1))| in v2) then v2[|(if (793 in v10) then v10[793] else map v11 : int | (-117 <= v11) && (v11 < 0x2d5) :: (v11 / f14) := (p1))|] else i0, f12[-|v15|]);
			...
			f15[95] := v29 !! (if (f12[f14] in v30) then v30[f12[f14]] else v29);
			var v31: set<int> := {p0, |v2|, p0 - p0, i0};
			f15[95], v31, r1 := (f14 >= i0) <==> f13, (v31 * v31) + (if (fm1(f13, p0, globalState)) then v31 else v31), f14 * p0;
		}
		if (!p2) {
			...
			if (if (v32 in v34[v35.cf73]) then f12[f14] else p2) {
				...
				f15[786] := f13;
				f15[249] := p2;
				f15[786], globalState.f4, f11, globalState.f4, f15[249] := p1, (if (p2) then f10 else f10) in v0, p1, p1, f13;
				r1 := f14 / (if (p2) then |v0[f14 := f10]| else f14);
				var v41 := DC25(f10);
				f10 := v41.cf29;
				...
				var v45 := new C4(if (f11 in v43) then v43[f11] else v42, 0xd4 % -|v44|, p0 % 0x157, f14, f12, p2, f10, f13);
				v44 := multiset{f11, f11, p1};
			}
			
			var v46: seq<int> := [0x1e9, p0];
			var v47 := DC48(p0 > |v46|, |(seq(-968, i2  => (f10)))|);
			match (v47) {
				case DC47(cf66, cf67, cf68) =>
					...
					v48[151] := if ((v50 * (if (|multiset{f11, if (p1 in v53) then v53[p1] else p2}| in v52) then v52[|multiset{f11, if (p1 in v53) then v53[p1] else p2}|] else multiset(seq(0x11a, i3  => (cf68))))) in v51) then v51[v50 * (if (|multiset{f11, if (p1 in v53) then v53[p1] else p2}| in v52) then v52[|multiset{f11, if (p1 in v53) then v53[p1] else p2}|] else multiset(seq(0x11a, i3  => (cf68))))] else v46[-0x1e];
					v48[151] := -0xfd;
			...
			match (v65) {
				...
				case DC15(cf17) =>
					...
					f10 := f10;
				case DC17(cf19) =>
					var v75: array<int> := new int[28](i4 => i4 % p0);
			...
			var v101: array<seq<seq<char>>> := new seq<seq<char>>[5] [v100, seq(624, i6  => (v0)), v100, v100[p0 := v0], v100];
			v101[72] := v100 + v100;
			v101[72] := v100;
		}
		...
		var v108 := new C2(p0, |v102|, f10, f11, |v103.cf57[|fm79(globalState)| := 'r']|, f14, p0 % p0, f12, v104[p0 := |v105|] == ([f14])[|v107| := -f14]);
		var i7 := 0;
			...
			decreases 100 - i7
		{
			...
			r2 := |(v108.fm6(-(v108.f26 - |v0|), globalState))[|v111[p2 := -0x2d]| + f14 := f10]|;
			r1 := v108.f27 / v108.f26;
...
class C7 extends T4 {
	...
	function fm5(p0: bool, globalState: GlobalState): int {
		if ((f29 > f29) in f29) then f29[f29 > f29] else f14 % 110
	}
	method m5(p0: int, p1: int, p2: seq<D0>, p3: int, globalState: GlobalState) returns (r0: bool) {
		...
		var v4 := DC41(if (f13 in v2) then v2[f13] else f14, |v3|, seq(117, i1  => ('q')), f13);
			...
			decreases 100 - i0
		{
			...
			if (f13) {
				...
				v7[189] := if (f13) then map[f11 := v8] else v9;
				v7[189] := v9;
				f11 := !f11;
				var v10: set<int> := {p1};
				var v11: array<array<int>> := new array<int>[18];
				var v12: map<int, set<int>> := map[f14 := v10];
				var v14 := DC23(if (p3 in v12) then v12[p3] else set v13 : int | (0x222 <= v13) && (v13 < 0xb9) :: (v13 / DC4(f13, p1, false).cf5));
				...
				var v19: multiset<string> := multiset{v17, v17[|v18| := v15.f23], seq(-0x1f4, i3  => (v15.f23)), v17, v17};
				var v20: array<int> := new int[22](i4 => i4 * -0xa3);
				var v21: multiset<array<int>> := multiset{v20, v20};
				var v22 := DC20(v16, v21);
				var v23: array<int> := new int[16] [fm7(v19, p0, globalState), p1, v16, p0, p0, f14, p1, p1, v16, -v22.cf23, 489, v4.cf56, -(p1 * |"eoxpk"|), |f29|, p3, if (p1 in v0) then v0[p1] else |([f11, f13])[|fm60(f14, v15.f23, f13, p0, globalState)| := f11]|];
				v23[676] := p3;
				f15[177] := f11;
				var v24: map<array<bool>, bool> := map[f15 := f11];
				v23[676], r0, v16, v16, f15[177] := p1, f11, v16, p3, |(v24 + v24[f15 := f11])| == |v17|;
				var v25: array<string> := new string[10](i5 => v17);
				var v26 := DC7(v17);
				v25[190] := (v26.(cf9 := "r")).cf9;
				var v27: map<bool, string> := map[f13 := seq(0x3d8, i6  => (v15.f23))];
				v25[190] := (if (f13 in v27) then v27[f13] else v17)[0x33c := v17[fm2(f15[177], f14, f15[177], globalState)]] + v17;
			}
			...
			v28, globalState.f4, v29, v32, v33[931] := v28, fm1(f11 || f11, p3 + -0xec, globalState), p3, v32, |v34|;
			f15[804] := f13;
			...
			decreases 100 - i7
		{
			...
			var v42: map<bool, D17> := map[f12[p1] := v41];
			v40[214] := |v42|;
			var v43: set<int> := {p3, p1};
			var v44: map<bool, seq<int>> := map[f13 := seq(0xdf, i8  => (if (!f11 in f29) then f29[!f11] else p1))];
			var v45 := DC42(f13);
			var v46: seq<seq<int>> := [[-p0, f14, p3]];
			v1, v39, v40[214], v40, v1 := fm61(v43, f13, f13, globalState), v39, fm5(f11, globalState), v40, (if (v45.cf59 in v44) then v44[v45.cf59] else v1) + v46[-p1];
			f15[74] := !f11;
			...
			case DC38(cf50) =>
				...
				match (v57) {
					case DC25(cf29) =>
						...
						v55 := v55 + (v55 + v55);
						v48[620] := v54;
						v48[620] := if (v54 in v0) then v0[v54] else v54;
				...
				match (v4) {
					...
					case DC42(cf59) =>
						var v73 := DC33(f29);
						var v74: array<D16> := new D16[28] [v73, v73, v73, v73, DC33(f29), v73, v73.(cf41 := f29), v73.(cf41 := f29), v73, v73, DC33(f29[f13 := |f12|]), DC33(f29), DC33(f29), v73, v73, v73, DC33(f29), DC33(multiset{f13}), v73.(cf41 := f29), v73, fm65(p3, p0, f11, globalState), v73, v73, if (f13) then v73 else v73, v73, v73, DC33(f29), DC33(v73.cf41)];
						v74[398] := v73;
						v74[398] := fm65(p0, p1 + p3, f11, globalState);
						v54 := p0;
						var v75: seq<map<int, bool>> := [v3];
						v54 := fm2(f11, |f29| % p0, !(v75 != v75), globalState);
						v54 := p3;
					case DC43(cf60, cf61, cf62) =>
						...
						var v79 := DC44((seq(661, i11  => (v76[910])))[-|f12| := v76[910]]);
						var v80: array<string> := new string[15] [v77, "aq", v77 + v77, v77, v77 + v77, v78[|v79.cf63|], "k", fm6(f14, globalState), fm6(|f29|, globalState), v77, "jls", v77, v77[|f29| := f10], v77[p3 := f10], seq(0x21d, i12  => ('l'))];
						v80[387] := v77;
						v80[387] := v77[p0 := fm66(cf62, f13, globalState)];
						v48 := v48;
						var v81: C1 := new C1(p0, f12, f13, f10, f11);
						var v82: seq<C1> := [v81];
						var v83: map<bool, C1> := map[f11 := v82[v81.f24]];
		...
		for i13 := f14 / p1 to |v90| {
			...
			match (v91) {
				case DC16(cf18) =>
					...
					var v94 := -0x2af;
					var v95 := "a";
					v94 := (v93.f28 / p0) + (if (f14 in v90) then v90[f14] else |v95|);
					f13 := f11;
				case DC15(cf17) =>
					...
					f15[935] := f13 ==> f13;
					f15[935] := f11;
					var v97: array<bool> := new bool[23](i14 => f15[935]);
					var v98: set<int> := {|v2|, -i13};
					var v99 := DC43(v97, fm67(|v98|, globalState), f15[935]);
					var v100: map<D19, array<bool>> := map[v99 := v97];
					var v101 := "elr";
					var v102 := new C2(|v100|, 0x197, v101[p0], [i13, p1, p3, |v1|] <= v1, 0x17c, p1, |f12| * i13, f12 + f12, v101 == (seq(0x1c1, i15  => (cf17))));
					var v103 := 0x142;
					var v104: array<seq<multiset<bool>>> := new seq<multiset<bool>>[27](i16 => [f29, f29]);
					var v105: seq<multiset<bool>> := [f29, f29[f13 := |fm61(v98, f13, f15[935], globalState)|], f29, f29, f29];
					v104[324] := v105;
					v50, r0, v103, v104[324] := v50, (if (f11) then p3 else |v101|) != |v1|, |(fm62(|v3|, f15[935], !false, globalState) + f12)| % i13, v105;
				case DC17(cf19) =>
					var v106 := 0x3dc;
					v106 := p0 - 0x23b;
					var v107 := "jjhowad";
					v107 := seq(167, i17  => (f10));
					f15[364] := f11;
					f15[364] := fm1(f13, i13 + p3, globalState);
					v106 := |(v0 * multiset{0x1df})| % |v2|;
			}
		...
		var v117 := "xpivoowxc";
		for i18 := f14 / p1 to |v117| {
			v48[351] := p3;
			v48[351] := p1;
			globalState.f4 := !(v1[fm5(f11, globalState)] < (-0x13f % v1[i18]));
			f10 := f10;
			f15[244] := f13;
			var v118: set<int> := {f14};
			f15[244], globalState.f3 := !false <==> (if (i18 in v3) then v3[i18] else f11), (set v119 : int | v119 in v118 :: (v119 - -0x297)) != v118;
		}
	...
	method m3(p0: multiset<map<int, int>>, globalState: GlobalState) {
			...
			decreases 100 - i0
		{
			...
			v0, f15[835], f15[697] := -254 + |v2[map[f11 := !f13] := v0]|, (if (f11) then f11 else f11) <== f11, (fm68(v0, |f12|, |(set v3 : int | (0x3c2 <= v3) && (v3 < 0x34b) :: (v3 % v0))|, 465, globalState)).cf18;
			if (f15[835]) {
				v0 := |([0x14b, v0] + ([f14])[if (true in f29) then f29[true] else v0 := f14])|;
				f15[835] := fm1(f15[835], f14, globalState);
				var v4 := "ilyljy";
				var v5: seq<string> := [fm6(|f12|, globalState)];
				var v6: array<int> := new int[24] [f14, v0, |f12|, v0, v0, |v4|, 0x108, |(seq(410, i1  => ('y')))|, v0, v0, v0, f14, |"ksxdwe"|, v0, f14, f14, f14, v0, fm7(multiset(v5), v0, globalState), v0, v0, f14, fm2(fm1(true, |multiset{v0}|, globalState), f14, f11, globalState), v0];
				var v7: map<array<int>, int> := map[v6 := f14];
				v0 := -f14 - |(v7 + map[v6 := f14])|;
				var v8: multiset<int> := multiset{f14, v0};
				v0 := |v1| % ((if (|v4| in v8) then v8[|v4|] else v0) / f14);
				var v9: C1 := new C1(f14, f12, f11, f10, f11);
				var v10: map<bool, C1> := map[f15[835] := v9];
				...
				var v16: set<bool> := {f15[835], !f11, f15[835], f11, f11};
				f15[835] := {!f11} != v16;
				...
				var v25: array<bool> := new bool[23] [false, f13, f15[835], f13, v0 > f14, v23 >= v23, false, f11, v20[72] !in map[|[f15[835]]| := |f12|], f13, f11, v24.cf44, f15[835], f13, f11, !f12[0x31c], f13, v21 != v21, f13, |v21| <= v20[72], f13, f15[835], f13];
				var v26: seq<set<bool>> := [v16];
				var v27: seq<int> := [|"xyovx"|, v20[72]];
				var v28: seq<seq<int>> := [v27];
				var v29: seq<int> := [v20[72] + |v28|, -0x19d, v0];
				v25, v0, v26, v27 := v25, v20[72] + v20[72], v26 + v26, v28[v0] + v29;
			}
			...
			decreases 100 - i2
		{
			...
			v31, v32, v33[273], v36 := --0x90, v32, v34, v36 + v36;
			var v37 := "ctidifphl";
			globalState.f4 := v31 > -(|v37| / v31);
		...
		match (v42) {
			case DC1(cf1) =>
				...
				v44[354] := v43;
				v44[354] := if (f13) then v43 else v43;
			case DC0(cf0) =>
				cf0 := (cf0 + f14) - f14;
				var v45 := "mgcl";
				...
				v41[294], v48, f11, f13 := fm1(f11, 0x2e9 + cf0, globalState), v48, !true, f11;
				var v49: multiset<string> := multiset{v45};
				var v50: map<bool, int> := map[fm7(v49, f14, globalState) <= 0x1e1 := f14];
				var v51: seq<bool> := [cf0 == 0x397];
				var v52 := DC4(!v41[294], cf0, false);
				globalState.f4, cf0, v50, v51 := !(v52.(cf5 := f14, cf4 := v41[294])).cf4, |{!true, f11}|, v50, v51 + (f12 + f12);
			case DC2(cf2) =>
				var v53: map<int, bool> := map[f14 := f11];
				var v54: multiset<map<int, bool>> := multiset{v53};
				var v56: set<int> := {f14};
				var v57 := DC39(|v54| > -f14, (set v55 : int | (0x389 <= v55) && (v55 < 454) :: (v55 / f14)) !! v56, false);
				...
				v59[80] := v42;
				f11, f13, v57, v59[80] := f13, true, DC39(f11, false, f11).(cf52 := f11), if (f13) then v42 else if (!f13) then DC0(f14) else v42;
				var v60 := 0x107;
				v60 := v60;
			...
			decreases 100 - i5
		{
			...
			var v69 := DC62(v68);
			var v70 := new C4(v67, 177, 872, -(|v69.cf92| - |v66|), f12[f14 := f13], fm1(f13, v65, globalState), f10, f13);
			globalState.f3 := false;
			v65 := 639;
		}
		var v71 := 0x256;
	...
	method m4(p0: int, p1: map<int, char>, p2: array<string>, p3: int, globalState: GlobalState) returns (r0: set<bool>, r1: array<int>, r2: bool, r3: bool) {
		...
		p2[573] := (if (p0 in v6) then v6[p0] else v5) + (if (v5 in v7) then v7[v5] else "mkaumtwd");
		if ((if (f13) then -0x24f else p3) <= (p3 % |"tinot"|)) {
			var v8 := 0x4a;
			var v9: multiset<string> := multiset{seq(112, i1  => ('p')), v5, p2[573], p2[573], p2[573]};
			v8 := fm7(v9, p3, globalState);
			var v10: map<int, D8> := map[|map[|f12| := f14]| := DC16(fm1(f13, v8, globalState))];
			var v11 := DC16(f11);
			v10 := map[p0 := v11] + v10;
			var v12: map<int, bool> := map[f14 := f13];
			v8 := -|(v12 + v12)[p3 := !f12[0x2f0]]|;
			var v13: map<char, int> := map[f10 := |map[|fm6(v8, globalState)| := f11]|];
			var v14: seq<int> := [|v13|, v8];
			var v15: set<int> := {v8};
			var v16 := new C0(v14[|v12|] - -0x1b5, v15 != {p0});
		...
		v20, v21 := p3 / f14, map v22 : int | v22 in v23 :: (v22 / --f14) := (-0x266);
		var v24: set<char> := {f10, f10};
		var v25 := DC43(f15, v24, f13);
		var v26: map<bool, array<bool>> := map[false := f15];
		var v27: seq<array<bool>> := [f15];
		var v28: array<array<bool>> := new array<bool>[20] [f15, f15, f15, v25.cf60, f15, f15, if (f11 in v26) then v26[f11] else f15, f15, f15, f15, v27[p0], f15, f15, f15, f15, f15, f15, f15, f15, f15];
		var v29: array<D25> := new D25[17];
		var v30: array<char> := new char[20](i2 => f10);
		var v31 := DC53(v30);
		v29[233] := v31;
		v28, v21, r3, v29[233], globalState.f4 := v28, v21, f11, v31, f13;
		...
		r0 := match v39 {
			...
			case DC46(cf65) => {true, !DC26(p2[573][p3], f10, f13).cf32, f11} + {f11}
		};
	...
	method m1(p0: int, p1: int, p2: bool, globalState: GlobalState) returns (r0: map<int, bool>, r1: int, r2: int) {
		...
		for i0 := |((map v1 : int | (0xe9 <= v1) && (v1 < 22) :: (v1 * p0) := (p2)) + v2)| to if (!f13) then |(map v3 : char | v3 in {f10, f10} :: (v3) := (p0))| else |fm75(f14, v4, p1, !f11, globalState)| {
			...
			var v6: map<bool, int> := map[f13 := p1 / 0x281];
			v6 := v6[p0 > f14 := |f29|];
			...
			if (f13) {
				...
				var v11: map<bool, C1> := map[f15[255] := v10];
				...
				var v16, v17, v18 := m20(if (f11) then v14[v9.fm5(false, globalState)] else v13, v15, f11 <==> false, f10, globalState);
				v10.f24 := v9.fm5(p0 != i0, globalState);
				var v19 := "gg";
				var v20: map<bool, string> := map[v18 := v19];
				var v21: seq<int> := [fm2(false, v10.f24, f15[255], globalState)];
				var v22: map<map<bool, string>, seq<int>> := map[v20 := v21];
				var v23: map<bool, bool> := map[f15[255] := f13];
		...
		var v30: map<int, array<int>> := map[f14 := v29[p1]];
		...
		var v38 := DC5(v37);
		var v39: array<map<int, array<int>>> := new map<int, array<int>>[10] [map[0x150 := v28], v30, v30, v30[-f14 := v28], if (true in v31) then v31[true] else v30, v30 + v30, map[p0 := v28], (if (-|v34| in v32) then v32[-|v34|] else v30)[f14 := v28], v35[fm9([p2], v36, v38, globalState)], v30 + v30];
		v39[37] := v30;
		v39[37] := v30;
		var i1 := 0;
		while (!f13)
			decreases 100 - i1
		{
			...
			r1 := fm2(v4 > v40[p1], p1, f11, globalState);
			...
			r1 := f14;
			f15[817] := |v2| == |v34|;
			f15[817] := !(if (f13) then (seq(0, i2  => (f10))) < v33 else f13);
		}
			...
			decreases 100 - i3
		{
			...
			f11 := true <==> f13;
			var v45 := DC7(v33);
			v44 := v44[fm2(p2, p1, f11, globalState) !in (seq(0x266, i4  => (|{v33}|))) := v33 <= v45.cf9];
			v28[104] := |v33|;
			globalState.f4, v28[104], globalState.f3 := fm1(f13, p0, globalState), p0, p2 && f13;
			var v46 := DC41(-v28[104], p0, v45.cf9, v28[104] >= p1);
			match (v46) {
				case DC41(cf55, cf56, cf57, cf58) =>
					...
					v28[104] := -(if (p2) then cf55 else fm7(v48, |v4|, globalState));
				case DC42(cf59) =>
					...
					var v53 := new C3(v28[104], |v4| / 0x1ac, f12, f13, fm71(globalState), f12 < [false]);
				case DC43(cf60, cf61, cf62) =>
					var v54 := DC32(v28[104]);
					var v55: multiset<string> := multiset{fm6(-0x57, globalState)};
					var v56: map<D15, int> := map[v54 := fm7(v55["kfn" := |fm6(f14, globalState)|], v28[104], globalState)];
					v56 := v56[v54 := v28[104] % -p1];
					var v57 := DC36(f13, cf62, f10, false, v28[104]);
					...
					var v68, v69, v70 := m20(v65, v66, v2[p1 := DC42(cf62).cf59] != (map v67 : int | v67 in v60 :: (v67 / |{f13}|) := (f13)), f10, globalState);
				case DC40(cf54) =>
					var v71: array<map<bool, int>> := new map<bool, int>[2](i5 => map[f13 := p0]);
					var v72 := new C4(v71, v28[104], p0 / v28[104], |v36|, f12, f13, f10, 33 < p0);
					f15[826] := f13;
					f15[826] := fm1(p2, p0, globalState);
					v28[104] := fm2(true, v36[f14], false, globalState);
					v28[104] := (p1 * |v33|) * -(p0 + cf54.f28);
			}
	...
	method m2(p0: bool, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: multiset<seq<int>>, r3: seq<int>) {
		if (f14 == f14) {
			...
			match (DC29(v0, v1, !p0)) {
				...
				case DC28(cf34) =>
					...
					globalState.f3, v15, globalState.f4, f15[633], f15[947] := if (f13) then p0 else !v16.cf32, v15, v17 > v17, p2, v0.f22;
					...
					var v24 := new C4(v22, |v23|, 0x1f8 / v0.f21, if (!p1 in f29) then f29[!p1] else 224, v21[128], true, f10, false);
			}
			...
			var v28: multiset<int> := multiset{v1[742]};
			var v29 := DC64(p0);
			...
			var v32 := DC5(v31);
			r0 := v30 / fm9(fm62(v1[742], fm8(true, globalState), f13, globalState), seq(0x91, i2  => (v0.f21)), v32, globalState);
			var v33: seq<seq<bool>> := [f12];
			var v34: C1 := new C1(-v0.f21, if (f13) then v33[0x130] else f12, v1[742] <= v0.f21, f10, p1);
			...
			if (p0) {
				...
				var v38: map<C4, string> := map[v37[f14] := "dy"];
				var v39 := "frsft";
				...
				v40[309] := if (false) then fm6(f14, globalState) else v39;
				var v41: map<bool, string> := map[p0 := v39];
				v40[309] := if (f12[f14] in v41) then v41[f12[f14]] else v39;
				var v42: array<map<int, multiset<char>>> := new map<int, multiset<char>>[26];
				v42[936] := map v43 : int | (-70 <= v43) && (v43 < 194) :: (v43 % f14) := (multiset{f10});
				var v44: multiset<char> := multiset{'q', 'p'};
				var v45: map<int, multiset<char>> := map[|fm6(f14, globalState)| := v44[f10 := |v39|]];
				v42[936] := v45 + v45;
				var v46 := new C1(f14 + |v40[309]|, f12 + [p1], p0, f10, f11);
				f10 := f10;
				var v47 := DC7("b");
				v46.f24 := (|v47.cf9| - f14) * f14;
			} else {
				var v48: array<seq<bool>> := new seq<bool>[27] [f12[f14 := fm8(f13, globalState)], f12, f12, f12, f12, f12, f12, f12, f12, f12, f12, f12, f12, f12, f12, f12, f12, f12, [p0], f12, f12, f12, f12, [!p2], f12, f12, f12];
				var v49 := DC8(v48);
			...
			if (f11) {
				...
				var v66: array<int> := new int[20] [f14, f14, |(multiset{f14, f14, 0x6a, f14, f14})[f14 := -f14]|, f14, f14, f14, |v59|, f14, 0x200, |v62|, v61[|v63|], -|"h"|, |v61|, if (v64 in v65) then v65[v64] else f14, v58.fm5(f11, globalState), f14, 0x39b, -0x10e, f14, v61[f14]];
				var v67: array<array<int>> := new array<int>[1] [v66];
				v67[429] := v66;
				r0, f15[949], v67[429] := f14, false, v66;
				f11 := p2;
				var v68 := DC7("yxnjhinyo");
				var v69: map<bool, array<int>> := map[f14 == f14 := v67[429]];
				var v70 := "yis";
				r0, r0, v68 := |v69|, f14, fm86(globalState).(cf9 := v70);
				var v71: multiset<multiset<bool>> := multiset{multiset{p0}};
				var v72 := new C2(f14, -v61[|multiset{p1}|], v58.f23, 0x116 == f14, |(v71 - v71)|, if (p1 in f29) then f29[p1] else f14, 0x23c, [p0, p2], f11);
				var v73, v74, v75, v76 := v72.m7(globalState);
			} else {
				var v77: set<int> := {-f14 % f14};
				var v78 := "gyhfrfv";
				v77, r1 := v77, v78;
				...
				var v82: seq<int> := [331];
				var v83: array<map<array<bool>, int>> := new map<array<bool>, int>[9] [v80, map[f15 := f14], v80, v80, v80, v80 + v80, v80[f15 := f14], v80, v80 + v81[|v82|]];
				v83[269] := v80;
				...
				r0, v83[269], f10, v84 := |map[f14 := v85]| * (|v87.cf92| / f14), v80 + (v80 + v80), if (p0 || p2) then if (f13 in v88) then v88[f13] else v58.f23 else f10, fm87(0x262, p0, globalState);
				...
				var v94: seq<array<set<bool>>> := [v93[f14]];
				v89[827] := v94[f14];
			...
			var v106: array<int> := new int[12] [-f14, f14 - |v96|, -v105.cf23, f14 % -0x3af, f14, f14, v99.f21, f14, f14, 0x34d, v58.fm7(multiset(seq(0x362, i4  => (seq(523, i5  => (f10))))), v99.f21, globalState), v58.fm5(f11, globalState)];
			v103[241] := |v102|;
			var v107: set<bool> := {p1};
			r0, v103[241] := |v98|, f14 * (f14 * |v107|);
		}
		...
		v111[599] := if (p2) then v115 * v115[-f14 := |(if (f14 in v116) then v116[f14] else v112)|] else v115;
		var v117 := "hps";
		var v118: array<int> := new int[11] [f14, f14, |v113|, 24, f14, f14, 0x115, |v117|, f14, f14, f14];
		var v119 := DC6(v118);
		var v120: seq<array<int>> := [v118];
		var v121: array<array<int>> := new array<int>[26] [v119.cf8, v120[f14], v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118, v118];
		v121[848] := v118;
		v121[848] := v118;
		...
		if (p2) {
			if (f11) {
				...
				var v140 := new C4(v138, -0x3a2, fm7(v139, f14, globalState), -f14, f12[f14 := p2], !p2, f10, p1);
			}
			...
			if (p2) {
				...
				var v159 := new C0(-fm5(f11, globalState), f12[-f14]);
				v121[848][293] := f14;
				var v160: seq<int> := [|v117|, f14];
				v121[848][293] := if (f15[799] || !f13) then fm9([p0], v160, v149, globalState) else -167 - f14;
				v121[848][293] := v121[848][293] % v121[848][293];
			}
		...
		r0 := |v163[v166[f14] := |v111[599]|]|;
		r1 := v117 + v117;
		var v167: seq<int> := [|v111[599]|];
	...
	method m20(p0: map<map<bool, C1>, array<bool>>, p1: array<int>, p2: bool, p3: char, globalState: GlobalState) returns (r0: array<int>, r1: char, r2: bool) {
		if (if (f11) then if (p2) then !f11 else f11 else p2 && f13) {
			...
			var v22: map<seq<array<bool>>, bool> := map[v21 := (fm89(f14, p3, f15[275], globalState)).cf93];
			v22 := v22[[v20] := p2];
			...
			var v26: set<int> := {f14};
			f13, v19, v23, f15[275], v23 := !!!f15[275], (if (f11) then "i" else v19) + (if (|v25[f13 := |v25|]| in v24) then v24[|v25[f13 := |v25|]|] else v19), v23, (v26 + v26) !! v26, fm2(!f13, |(v19 + v19)|, p2, globalState);
		...
		if (f11) {
			...
			var v37: map<seq<bool>, map<int, bool>> := map[f12 := fm80(f14, v36[v29], globalState)];
			var v38: seq<int> := [v29];
...
class C8 extends T2, T0, T5 {
	...
	method m1(p0: int, p1: int, p2: bool, globalState: GlobalState) returns (r0: map<int, bool>, r1: int, r2: int) {
		...
		v0[935] := v1;
		var v2: array<int> := new int[25](i1 => i1 / f16);
		...
		v0[935], v3, v4, globalState.f4, v8[256] := v1, v3, (DC3(v4).cf3[-0x395 * -f14 := 0x21])[|v10| := f14], p2, (map v11 : multiset<bool> | v11 in v12 :: (v11) := (v6))[v5 := if (f13) then v6 else v6];
		if (p2) {
			...
			v44[818] := f16 > DC4(f13, f16, v42.f13).cf5;
			var v45: multiset<int> := multiset{--|v6|};
			var v46: map<bool, multiset<int>> := map[f11 := v45];
			v44[818], v46 := v42.f13, (v46 + v46) + v46[v42.f13 := v45];
		}
		...
		if (f11) {
			var v50: array<map<bool, int>> := new map<bool, int>[28](i7 => map[v47[401] := p1]);
			var v51: set<char> := {f10};
			var v52 := new C4(v50, f16 + p1, |v51|, p1 / f14, f12, f13, 'm', p2);
			v2[533] := fm2(v47[401], |(seq(0x269, i8  => (v6[f16])))|, p2, globalState);
			v2[533] := -0x2f3;
			if (f13) {
				r1, r2 := f16, f16;
				var v53: array<map<bool, seq<int>>> := new map<bool, seq<int>>[11];
				var v54: map<bool, seq<int>> := map[v47[401] := v4];
				v53[142] := v54;
				v47, v53[142] := v47, v54 + v54;
				var v55, v56, v57, v58 := v52.m2(v47[401], v47[401], v47[401] <==> p2, globalState);
				var v59: array<int> := new int[18];
				v3[151] := v59;
				v3[151] := v59;
				var v60: C3 := new C3(f16, p1, f12, p2, f10, f11);
				var v61: seq<C3> := [v60, v60];
				var v62: map<seq<C3>, bool> := map[v61 + v61 := v47[401]];
				v62 := v62[v61 := p2];
			} else {
				var v63: array<set<int>> := new set<int>[20];
				v63 := v63;
				var v64: array<map<set<int>, int>> := new map<set<int>, int>[28];
				var v65: set<int> := {f16, p1, v2[533]};
				var v66: map<set<int>, int> := map[v65 := v2[533]];
				v64[759] := map[v65 := -f17] + v66;
				var v67: C0 := new C0(f14, f13);
				var v68: array<int> := new int[16](i9 => i9 / p0);
				var v69 := DC9(f12);
				globalState.f4, v64[759], v67, v68, v10 := !v52.fm28(f17, (v69.(cf11 := f12)).cf11, globalState), (v66 + v66) + (v66 + v66), v67, v68, v10;
				var v70 := DC29(v67, v68, f11);
				v68 := v70.cf36;
				v68 := new int[27];
				var v71: seq<array<bool>> := [v47, v47];
				var v72: multiset<array<bool>> := multiset{v47};
				var v73: map<multiset<array<bool>>, array<bool>> := map[if (v47[401]) then (multiset{v47})[v71[v2[533]] := fm10(p2, globalState)] else v72 := v47];
				v2[533], f11, v67.f21, v47, globalState.f4 := fm2(if (p2) then v67.f22 else f11, |f12|, !f13, globalState), p0 >= p1, 342, if (v72[v47 := f14] in v73) then v73[v72[v47 := f14]] else v47, !f13;
			}
			...
			r2 := f16;
			var v75 := DC19(f13, if (f13) then fm2(f11, f14, !v47[401], globalState) else f14);
			var v76: map<int, char> := map[f16 := f10];
			var v77 := DC24(v76);
			var v78: multiset<array<bool>> := multiset{v47};
			v75, globalState.f4, r2, v47[401], v77 := v75.(cf22 := |v6[f14 := f10]| / f14), ((multiset{v47})[v47 := p0] + v78) !! (if (v47[401]) then v78 else multiset{v47, v47}), (|v6| % p1) - f14, f13, DC24(v76);
			var v79: map<bool, bool> := map[f13 := f11];
			var v80: map<bool, bool> := map[false := fm1(v47[401], |v79|, globalState)];
			var v81: map<seq<int>, bool> := map[v4 := fm1(if (f11 in v79) then v79[f11] else v47[401], 0x331, globalState)];
			f11 := if (v4 in v81) then v81[v4] else !(v5 !! v5);
			var v82: map<int, array<bool>> := map[0x1f4 := if (v47[401]) then v47 else v47];
			...
			v47 := new bool[17] [f13, f13, f11, f13, f11, p0 != p0, p2, p2, v6 != (seq(0x2ba, i10  => (f10))), v47[401], p2, fm1(f13, |v84|, globalState), fm1(f11, 0x1c4, globalState), v85 > {v6}, (fm49(globalState)).cf6, v47[401], f13 !in f12];
		}
		...
		match (v86) {
			case DC8(cf10) =>
				...
				var v88: seq<array<int>> := [v2, v2, v87[fm10(p2, globalState)]];
				var v89: seq<seq<array<int>>> := [v88, v87, v88, v87];
				v87 := (v87[p1 := v2] + (v89[f14] + v87))[if (true in v5) then v5[true] else f17 := v2];
				r1 := f16;
				f13 := f13;
				var v90: seq<bool> := [p2];
				var v91: C0 := new C0(f17, true);
				var v92: map<bool, int> := map[f13 := p1];
				var v93: set<char> := {f10, v91.fm14(globalState), f10, v6[|v92|], f10};
				var v95 := DC29(if (false) then v91 else v91, v2, v93 > (set v94 : char | v94 in {f10} :: (v94)));
				var v96 := DC9(v90);
				v90, globalState.f3, v95, f10, globalState.f4 := v96.cf11, v47[401] && p2, DC29(v91, v2, f11), f10, v91.f22;
		}
	...
	method m2(p0: bool, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: multiset<seq<int>>, r3: seq<int>) {
			...
			decreases 100 - i0
		{
			...
			var v10 := DC23(fm47(f13, v9[f11 := f16], p0, globalState));
			match (v10) {
				case DC23(cf27) =>
					var v11: array<bool> := new bool[12];
					v11[915] := f13;
					v11[915] := DC26(f10, f10, true).cf32;
					r0 := f14;
					var v12 := DC6(v5);
					...
					v15[208] := v5;
					v15[208] := DC6(v5).cf8;
					var v16: map<bool, bool> := map[v11[915] := f12[f17]];
			...
			decreases 100 - i3
		{
			...
			v29 := v29[if (f14 in v31) then v31[f14] else v32[f16] := f13];
			v5[961] := f14;
			var v33: map<array<int>, int> := map[v5 := f16];
			v5[961] := if (v5 in v33) then v33[v5] else f14;
		}
		var v34: C3 := new C3(f14, |f12|, f12, true, f10, f11);
		var v35: seq<C3> := [v34];
		var v36 := "g";
		var v37 := DC40(v34);
		var v38: array<seq<C3>> := new seq<C3>[15] [DC38(v35).cf50 + v35, (v35 + v35)[-|v36| := v34], [v34, v34], v35, v35 + [v34, v34], v35, v35, v35, v35, v35, ([v34])[f17 := v34] + [v37.cf54, v34, v34, v34, v34], v35, v35, v35, v35];
		v38[518] := v35;
		var v39 := DC38([v34, v34]);
		v38[518] := v35 + (v39.(cf50 := v35)).cf50;
		r0 := f14 * f17;
		r1 := (v36 + v36) + v36;
	...
	method m6(p0: int, p1: bool, globalState: GlobalState) returns (r0: int) {
		...
		var v2 := DC9(f12);
		v1, r0 := seq(0x2a9, i0  => (DC32(f14).cf40)), f16 % |v2.cf11|;
		if (f10 in (seq(0x7a, i1  => (f10)))) {
			...
			var v12: map<int, int> := map[v3[794] := v7.f28];
			v7, f13, globalState.f3, v10 := v11[if (|v0| in v12) then v12[|v0|] else |v4|], f11, f13, DC12(v8);
			var v13: array<bool> := new bool[11](i3 => p1);
			v13[797] := |v6| == |v12|;
			r0, v13[797], v3[794] := 380 % v3[794], true, f14;
			...
			var v20: C3 := new C3(p0, f17, [f11, f11, f13, p1, v18[110]], v18[110], f10, p1);
			var v21 := DC40(v20);
			var v22: multiset<C3> := multiset{(v21.(cf54 := v20)).cf54, v20, v20, v20};
			var v23 := new C4(v19, f14, 367, if (v20 in v22) then v22[v20] else |f12|, f12, v18[110], f10, false);
			v18[110] := v18[110];
			var v24 := DC5(DC4(p1, f17, p1));
			var v25: map<D1, int> := map[v24 := f14];
			var v26: multiset<bool> := multiset{fm1(v18[110], 0x11a, globalState)};
			var v27 := DC33(v26);
			var v29: seq<D1> := [v24, v24];
			v25, r0, v18[110], v27, r0 := (map v28 : D1 | v28 in multiset(v29) :: (v28) := (v20.f28))[v24 := p0], f14, v18[110], v27, f14;
		}
		
		var v30: seq<seq<int>> := [v1];
		globalState.f4 := fm1(fm1(f11, f17, globalState), fm2(fm1(f13, |(seq(0x2f5, i6  => ('g')))|, globalState), |v30|, fm1(f11, -f16, globalState), globalState), globalState);
		var v31: array<string> := new string[3];
		var v32 := "sshofy";
		v31[22] := v32;
		v31[22] := v32;
		var v33 := DC13("wpuxe", f17);
		var v34: multiset<string> := multiset{seq(0x320, i8  => (f10)), v31[22], v33.cf14};
		var v35: array<bool> := new bool[23] [f13, p1, if (p1) then f11 else !f11, !(v34 !! v34), false, true, if (f11) then fm1(f13, p0, globalState) else p1, fm1(f11, fm2(f13, f14, f11, globalState), globalState), fm1(!false, f17, globalState), f11, p1, !f13, p1, !p1, fm1(f12[|v1|], f14, globalState), fm1(f13, -364, globalState), (seq(0x193, i9  => (f10))) != v32, p1, f13 <==> f11, p1, p1, f13, f14 > f16];
	...
	method m7(globalState: GlobalState) returns (r0: seq<int>, r1: int, r2: bool, r3: array<int>) {
		...
		var v2: T4 := new C7(v0[f13 := f14], v1, -142, f12, f11, f10, f11);
			...
			decreases 100 - i1
		{
			...
			match (v12) {
				...
				case DC35(cf43) =>
					...
					var v18: array<map<int, bool>> := new map<int, bool>[5] [v16, v16, map v17 : int | (455 <= v17) && (v17 < 0x3dd) :: (v17 % f16) := (v2.f11), map[v2.f14 := f13], v16];
					var v19: map<int, array<map<int, bool>>> := map[--f16 := v18];
					var v20 := DC45(if (900 in v19) then v19[900] else v18);
					r1, v15[13], v20, v2.f13 := v2.f14, v7, v20, false;
					v1[901] := v2.f11;
					v1[901] := !!(0xc1 > v2.f14);
					v5 := v5[v2.f13 := v2.f10];
				...
				case DC37(cf49) =>
					...
					v25, globalState.f3, v27, v2.f11 := v25, (v29 == v29) <==> !!v11, v27 * v30[|[!v2.f11]|], !v2.f13;
		...
		for i4 := 0xc4 to -f14 {
			...
			v39[994] := (if (f11 in v43) then v43[f11] else v42) + (v42 + v42);
		}
		var v44 := DC16(v2.f13);
		v44 := v44;
		r2 := f11;
	...
	method m13(p0: int, p1: int, globalState: GlobalState) returns (r0: bool, r1: string, r2: bool, r3: D1) {
		if (f13) {
			var v0: array<int> := new int[21](i0 => i0 / |map[f16 := f17]|);
			...
			v3[989] := f11;
			v3[989] := fm1(false, -p0 * |v2|, globalState);
			var v7: array<multiset<bool>> := new multiset<bool>[15];
			var v8 := DC48(false, f14);
			var v9: set<int> := {p1, f17};
			var v10 := new C6(v7, if (false) then v3 else v3, f14, f12, v8.cf69, if (v3[989]) then f10 else fm17(!f13, globalState), fm64(globalState) !! v9);
			var v11: multiset<seq<bool>> := multiset{f12[f14 := f13], f12, f12};
			v0[140] := |v11|;
			v0[140] := p0 * (if (false) then p1 else |"fhexkb"|);
		} else {
			...
			if (multiset{false, fm1(!true, p0, globalState), f11} >= (v12 * fm56(globalState))) {
				...
				var v17 := "qesb";
				globalState.f3 := !!(v12[f11 := |v17|] > v12);
				...
				v19[528] := (v20.(cf70 := p0)).cf69;
				v17 := v17;
				v18[841] := fm5(!v19[528] || f13, globalState);
			}
			
			var v21 := "bxop";
			r1 := v21 + (seq(0x24, i2  => (f10)));
			var v22 := 0x32f;
			v22 := p0 / f14;
			var v23: map<string, string> := map[v21 := fm6(fm10(f13, globalState), globalState)];
			v21 := if (v21 in v23) then v23[v21] else if (f13) then v21 else seq(65, i3  => (f10));
		...
		v26[936] := -0x388 / |v28|;
		var v29 := 54;
		v25, v27, v26[936], v29 := v25[true := p1], v27, p0, fm10(f11, globalState);
		v25 := v25[f11 := -p1];
		var i5 := 0;
		while (f13)
			decreases 100 - i5
		{
			...
			var v35: array<seq<array<bool>>> := new seq<array<bool>>[26] [v32, [v30], v32 + [v30], v32[p1 := v30], v32 + v33.cf103, v32, [v30, v30, v30], v32, v32, v32 + v32, v32, [v30], v32 + v32, [v30, v30], v32, if (f13) then v32 else v32[v29 := v31.cf20], v32, v32, v32, v32, v34[f14], v32, v32, v32 + v32, v33.cf103 + v32, v34[p0] + v32];
			v35[900] := [v30];
			v35[900] := (v32 + v32) + v32;
			var v37: map<bool, map<int, bool>> := map[f11 := map v36 : int | (731 <= v36) && (v36 < 0x74) :: (v36 + v26[936]) := (f11)];
			v37 := map[v28 == "umsn" := map v38 : int | (0xee <= v38) && (v38 < 757) :: (v38 % |v28|) := (f11)];
		}
		var v39: multiset<map<int, int>> := multiset{map[f14 := p1]};
		var v40: map<int, int> := map[f14 := |[v26]|];
		var v41: map<bool, map<int, int>> := map[f13 := v40];
		v39 := if (f11) then multiset{v40, if (f13 in v41) then v41[f13] else v40} else v39[map[v29 := f16] := |v28|] - v39;
		if (f13) {
			...
			v44 := fm47(f13, v45, f12[p1], globalState);
			...
			v49 := v49[f13 := v48];
			if (f13) {
				v26[936], v26[936] := v26[936], if (f14 >= p1) then -504 else -v26[936];
				var v50: seq<multiset<bool>> := [v45];
				var v51: array<multiset<bool>> := new multiset<bool>[15] [v45, multiset{f13, f13, f11}, v45, v45, v45, v45, v45, v50[f14], v45, v45, v45, v45, v45, multiset{f11}, multiset{f11, f13, false}];
				...
				var v58: map<bool, bool> := map[f13 := f11];
				var v59: array<char> := new char[25] [f10, f10, f10, f10, f10, f10, f10, 'm', f10, f10, if (f11 in v56) then v56[f11] else f10, f10, f10, f10, f10, f10, fm71(globalState), f10, v28[--|v57|], v28[|v58|], 'm', fm33(multiset{v26[936]}, |"x"|, globalState), f10, v28[f14], f10];
				var v60 := DC22(v57);
				v59[548] := fm33(v60.cf26, v42.f28, globalState);
				v59[548] := f10;
				v26[936] := 0x324 * v42.f28;
			} else {
			...
			globalState.f3 := f13;
			var v66: set<int> := {v26[936], |v27|, -0x12};
			...
			v26[936], v66, v71[212], v72 := DC1(fm10(f11, globalState)).cf1 / v73.cf82, v66 - (v66 - v66), v70, v72;
			if (false) {
				...
				v75 := new bool[21] [f13, fm1(f13, |v76|, globalState) <== f11, f16 == p1, if (f11) then f13 else !f13, f11, f11, true <== f13, fm1(f13, f17, globalState), f13, f12[f17], f13, f11 !in v67, true, f11, f11, f13, false, f11, !(v26 in multiset{v26, v26, v26, v26}), f13, f13];
				r1 := v28;
				f11 := f13 && f11;
				v26 := v26;
				var v77: seq<int> := [|v67|];
				...
				v26[936] := |(if (f11) then seq(-0x1d2, i6  => (f10)) else v28)|;
				globalState.f4 := f11;
		...
		r2 := !!(v86 > v86);
		r3 := fm84(0xff, p0 * v26[936], f12, p1, globalState);
	}
	method m14(globalState: GlobalState) returns (r0: int) {
		...
		var v3 := "h";
		var v4 := new C1(|v2[|v3|]|, f12, f13, v3[-f17], f11);
		var v6: map<bool, set<int>> := map[true := set v5 : int | (0x20e <= v5) && (v5 < 0x1bf) :: (v5 / v4.f24)];
		...
		v10 := v10[|fm75(f17, v11, f17, f13, globalState)| := map v12 : int | (750 <= v12) && (v12 < 0x19e) :: (v12 % v4.f24) := (v8)];
		...
		v18, v21, v3, globalState.f4, v0[690] := v18, DC44(if (v19 in v25) then v25[v19] else v20), (seq(0x2ec, i3  => (f10))) + ("rorffiar" + "aj"), v29 > (if (|v35.cf84| in v30) then v30[|v35.cf84|] else v29), f16;
		...
		match (v37) {
			case DC56(cf81, cf82) =>
				...
				var v43: map<array<int>, string> := map[v0 := seq(-741, i4  => ('e'))];
				v41, globalState.f3, v0[690] := fm92(v27.f22, f13, |(v42[f12 := f10])[f12 := 'j']| > 350, globalState), |v43| >= cf82, v27.f21 - v4.f24;
				v8 := v8[!v27.f22 := f17];
				var v44 := DC75(seq(0x20d, i5  => (v8)));
				var v45: seq<map<bool, int>> := [v8];
				if (|(v44.cf109[v4.f24 := fm0(f13, v0[690], -0x31f, true, globalState)] + v45)| > v4.f24) {
					v3 := v3;
					v13 := v13;
					v13[413] := v27.f22 !in f12;
					var v46: map<bool, map<int, int>> := map[f11 := v33];
					var v47: set<int> := {|(if (f13 in v46) then v46[f13] else v32)|, f17};
					v13[413] := |v47| <= |(map v48 : int | v48 in v47 :: (v48 * v4.f24) := (v4.f24))|;
					v4.f24 := v4.f24;
					var v49: array<array<int>> := new array<int>[24];
					var v50: map<string, array<array<int>>> := map[v3 := if (f13) then v49 else v49];
					v50 := v50[v3 := v49];
				} else {
					v0[690] := |f12| / (v4.f24 - |v8|);
					var v51: map<bool, C1> := map[f13 := v4];
					var v52: seq<string> := [v3];
					var v53: set<bool> := {f13};
					var v54 := DC1(v0[690]);
					v4.f24, v0[690], globalState.f3, v4.f24 := |v51[(v52[f17])[v27.f21 := f10] != v3 := v4]|, f16, ({cf81} - {v27.f22, f13, v23.fm28(cf82, f12, globalState), f13}) > v53, v54.cf1;
					var v55, v56, v57, v58 := v23.m7(globalState);
					v0[690] := fm10(f13, globalState);
					v56 := ---|((v3 + v3) + ("y" + v3))|;
				}
			...
			case DC57(cf83, cf84, cf85) =>
				...
				v0[690] := fm2(f13, |(v3 + v3)|, false, globalState);
				...
				v13[118], v61[815] := fm1(v3 <= v3, f17, globalState), ([f11 !in v62])[|v63| := v27.f22];
			case DC55(cf80) =>
				var v64: array<array<int>> := new array<int>[20];
				v64[798] := v0;
				var v65: map<int, array<int>> := map[v0[690] := v0];
				v64[798] := if (v4.fm5(v27.f22, globalState) in v65) then v65[v4.fm5(v27.f22, globalState)] else v0;
				v0 := v64[798];
				globalState.f4 := f11;
				globalState.f4 := |(seq(0x20b, i7  => (f10)))| > f14;
		}
...
class C9 {
	...
	method m11(p0: bool, p1: D0, p2: bool, globalState: GlobalState) returns (r0: T0, r1: int, r2: string) {
			...
			decreases 100 - i0
		{
			...
			var v10 := new C2(f19, |v6| * f19, v7, p0, f19, f19, f19, v5, v9 >= v9[seq(736, i1  => (f19)) := f19]);
		...
		var v31: seq<D26> := [v30];
		v24[575] := v31;
		v24[575] := v31 + fm94(0x1a3, true, false, v21.f10, globalState);
		v21.f11 := v21.f13;
		...
		if (!(if (v32 in v33) then v33[v32] else p0)) {
			...
			v36[15], v44, globalState.f3 := v26 != v26, v44, (v45 + v45) !in v46;
			var v47 := DC3(seq(344, i6  => (f19)));
			match (v47) {
				case DC4(cf4, cf5, cf6) =>
					r1 := f19;
					v36[15] := v21.f11;
					...
					v50, v36, globalState.f3, v37, v36[15] := v50, (DC43(v36, {v21.f10}, v21.f11).(cf60 := v36, cf61 := v51)).cf60, !v21.f13, v37 + v37, (|v45| - 0x33d) > f19;
				case DC3(cf3) =>
					r1 := f19;
					...
					v53[151] := v54;
					var v55: map<int, bool> := map[if (|map[v21.f10 := v36[15]]| in v34) then v34[|map[v21.f10 := v36[15]]|] else f19 := p0];
					v53[151] := if (if (v54.f28 in v55) then v55[v54.f28] else v21.f11) then v54 else v54;
				case DC5(cf7) =>
					...
					v57 := v57[v21.fm5(v36[15], globalState) := v56];
					v45 := v45;
			}
			...
			var v71 := new C2(f19, f19 % f19, v21.f10, v21.f11, if (f19 in v70) then v70[f19] else f19, f19, 683, v21.f12, if (fm1(false, f19, globalState)) then v21.f13 else !v21.f13);
		}
	...
	method m12(p0: seq<bool>, p1: bool, p2: bool, p3: D1, globalState: GlobalState) returns (r0: bool, r1: char, r2: int, r3: bool) {
		...
		if (match v15 {
			...
			var v16 := new C2(v0.f21, fm2(p1, f19, p2, globalState), v10.f23, true, f19, v0.f21 % f19, 0x46, [true, v0.f22, v0.f22], !!fm1(p2, 895, globalState));
			var v17 := DC27(DC26(v10.f23, 'm', !v0.f22));
			v17 := v17;
			if (v0.f22) {
				var v18: array<char> := new char[2];
				v18[513] := v10.f23;
				v18[513] := fm33(v3, fm2(p1, f19, v0.f22, globalState), globalState);
				var v19, v20, v21, v22 := v16.m7(globalState);
				var v23: map<int, char> := map[f19 := v10.f23];
				v23 := v23[--0x16 := v18[513]];
				globalState.f4 := v0.f22;
				var v24: multiset<seq<int>> := multiset{v6, v6};
				r2 := -0x2df / (if (v6 in v24) then v24[v6] else |p0|);
			} else {
				v0.f21 := |v7|;
				v0.f21 := -v16.f27;
				...
				v26[612] := v7;
				v25, r1, v26[612] := v25 + v25, fm19(p1, v16.f26 - |v7|, v0.f22, globalState), v7;
				r2 := v16.f27;
				var v27: array<int> := new int[19];
				v27 := v27;
			}
			
			r0 := (if (v0.f22) then f19 else v16.f27) != (v0.f21 / v0.f21);
			v0.f21 := f19;
		} else {
...
class C10 extends T4 {
	...
	method m5(p0: int, p1: int, p2: seq<D0>, p3: int, globalState: GlobalState) returns (r0: bool) {
		...
		if (f13) {
			...
			match (v2) {
				...
				case DC3(cf3) =>
					...
					v11, globalState.f3, v11 := p0, f15[355] !in v14, p1;
				case DC5(cf7) =>
					var v15 := 58;
					var v16 := "l";
					v15 := -(|v16| / v15);
					f11 := if (f13) then f11 else f15[355];
					var v17: map<int, int> := map[p1 := p1];
					var v18: map<map<int, int>, int> := map[v17 := p1 + p1];
					var v19: map<bool, int> := map[f15[355] := -0x3db];
					var v20 := DC0(if (false in v19) then v19[false] else p1);
					v0[732] := |f12|;
					var v21 := DC4(f13, v15, f15[355]);
					var v22: set<bool> := {f15[355], !f15[355]};
					var v23: map<char, D1> := map[f10 := v21.(cf5 := -|v22|, cf4 := f13)];
					v18, v20, v0[732], globalState.f4 := v18, v20, if (|v23| > p3) then v15 else v15, f13;
					var v24: array<D1> := new D1[29](i2 => v2);
					v24[675] := v2;
					var v25: array<set<bool>> := new set<bool>[14](i3 => v22);
					f15[355], v0[732], v24[675], v25, v0[732] := f15[355], p3, v2.(cf3 := v1), v25, p1;
			}
			...
			if (|"cyqan"| > v30.cf100) {
				...
				var v38: C8 := new C8(v33, p1, v34[f14] + f12, f11, f10, |v36| <= v37[p1], p3, p1 * 0x21b);
				...
				var v45 := DC35((multiset{f15})[f15 := -|v44|] - multiset{f15});
				var v46: seq<array<int>> := [v0];
				var v47 := DC33(v44);
				f15[426], f11, v0, v45, v44 := v29.f21 != |v27|, 800 == |{-0x3bd, p1, p3}|, v46[--(605 * p3)], v45, (if (fm1(v29.f22, v29.f21, globalState)) then v47.cf41 else v44) - v44;
			...
			decreases 100 - i5
		{
			...
			var v63: array<set<int>> := new set<int>[12] [v52, v52 * v52, v52, set v53 : int | (0x5 <= v53) && (v53 < 0x32d) :: (v53 % |map[f13 := f13]|), set v56 : int | v56 in v55 :: (v56 - -450), v52, v52, v57.cf27, fm29(globalState) - {fm9(f12, v55, v59, globalState)}, v52 + {f14, p1, p3, p3, |v61[p0]|}, v52 * (set v62 : int | v62 in v52 :: (v62 * -370)), fm29(globalState) - {p1, if (p0 in v60) then v60[p0] else f14, |v52|}];
			v63[426] := v52;
			v63[426] := (v52 + v52) * (v52 * v52);
			...
			v64[714] := f10;
			f15[493] := false;
			f15[493] := f11 && fm1(f11, p3, globalState);
			f15[493] := f13;
		}
		var v65 := DC0(f14);
		match (v65) {
			case DC1(cf1) =>
				v0[162] := 0x314;
				var v66 := DC51(|(seq(187, i6  => (f10)))|, f13, f11);
				v0[162], cf1 := p3, v66.cf73;
				var v67: multiset<string> := multiset{seq(451, i7  => (f10))};
				var v68 := "ieuaacm";
				var v69: set<string> := {seq(63, i8  => ('k')), v68};
				...
				var v72: map<array<int>, int> := map[v71 := 0x64 % |multiset{p1, |f12|}|];
				v72 := v72[v71 := p3];
				f11 := f13;
				cf1 := p3;
			case DC0(cf0) =>
				if (f11) {
					...
					var v78: multiset<string> := multiset{v76, v76[p1 := f10]};
					...
					v80, v81, f15[608], v82 := v80, v81 + v81, (-v83[cf0] > |f12[p3 := f11]|) !in v84, v82;
					var v85 := new C5(f10, |fm21(globalState)|, f12, f15[608], f10, f11);
					cf0 := |v81|;
					cf0 := v80.f19 * p1;
					var v86: array<map<bool, int>> := new map<bool, int>[28];
					var v87 := new C4(v86, p3, 0x334, f14, f12, !f11, v85.f23, f15[608]);
				}
				...
				if (f13) {
					...
					v96[604] := if (fm1(f11, p0, globalState)) then f15 else f15;
					v96[604] := f15;
					cf0 := -p0 % p3;
				...
				if (p1 > p0) {
					...
					f15[616] := f13;
					var v107: multiset<bool> := multiset{f12[p0], f13};
					f15[616], cf0, f10, cf0 := fm8(!f11, globalState), (if (f13) then 0x30a else f14) + (if (f13 in v107) then v107[f13] else 387), f10, p0;
				...
				var v118 := "eurck";
				var v119 := new C2(p1, p3, f10, false, f14, fm2(f13, p3, f12[|f12|], globalState), -(cf0 % |v118|), f12, f11);
			case DC2(cf2) =>
				r0 := f13;
				var v120 := 0x4;
				v120, v0 := f14, v0;
				...
				f15[375] := v122 !in (map v123 : multiset<char> | v123 in v124 :: (v123) := (v120));
				f15[375] := f13;
				if (f15[375] || f15[375]) {
					...
					v125[221], v126, v126 := v126 + (v126 + v126), (if (f15[375]) then (v127.(cf14 := v126)).cf14 else v126) + ("gisdxqkuj" + v126), v126;
					v120 := p1 + (0x95 - p3);
					var v128: multiset<int> := multiset{0x3c8};
					var v129 := new C0(|v128|, f11);
					globalState.f4 := f15[375] <== v129.f22;
				} else {
					var v130 := "d";
					v130 := v130 + v130;
					var v131: array<multiset<bool>> := new multiset<bool>[14];
					var v132: multiset<bool> := multiset{f15[375]};
					v131[403] := v132 * v132;
					var v133: map<bool, int> := map[f15[375] := f14];
					v131[403] := v132[true := -|v133| - f14];
	...
	method m3(p0: multiset<map<int, int>>, globalState: GlobalState) {
		...
		var v9: set<C6> := {v8};
		var v10: array<int> := new int[19] [v2 % v3.cf82, -(if (f11) then v1.f24 else v5[v2]), 206, v1.f24, v2, 0x326, if (f11) then v1.f24 else -f14, f14 - v2, v2 - v1.f24, v1.f24, |v6|, f14, |v9|, v1.f24, f14, v2 % f14, -0x3af, v1.f24, v2];
		var v11 := DC4(f13, -|[f13, !f13]|, true);
		var v12 := DC5(v11);
		v10[643] := if (f13) then fm9(f12, v5, v12, globalState) else f14;
		var v14: map<int, int> := map[-|fm61(set v13 : int | (0x82 <= v13) && (v13 < 0xf6) :: (v13 % 0x56), f11, !f11, globalState)| := v1.f24];
		v10[643], v14 := -f14, v14;
		var v15: seq<seq<int>> := [v5];
		var v16 := "uo";
		var v17: map<C6, string> := map[v8 := v16];
		v5, v10[643], v10[643], v10[643], v10[643] := fm75(v10[643] % |v15|, multiset{|[f10, f10]|, f14, v1.f24, |v16[f14 := f10]|}, v5[v10[643]], f11, globalState), |((if (v8 in v17) then v17[v8] else v16) + v16)|, v10[643], -v10[643], v2 - -v1.f24;
	...
	method m4(p0: int, p1: map<int, char>, p2: array<string>, p3: int, globalState: GlobalState) returns (r0: set<bool>, r1: array<int>, r2: bool, r3: bool) {
		var v0 := m9(p3 / 0x2f2, f12 != f12, globalState);
		var v1: map<int, seq<bool>> := map[f14 := f12];
		v1 := v1[f14 := f12[p0 := f11]];
		var v2 := DC0(v0);
		if (!match v2 {
			...
			var v3: map<int, int> := map[f14 / p0 := f14];
			v3 := v3[-p3 := p0];
			...
			v4[670] := if (f11) then f10 else f10;
			v4[670] := f10;
			var v5 := "by";
			var v6 := new C3(p0, 665, DC9(f12).cf11, p0 > p3, v5[|v5|], f13);
			if (f11) {
				globalState.f3 := f13;
				...
				var v9 := DC37(DC35(v8));
				var v10: multiset<int> := multiset{p0 % 896, |v5| * p3};
				var v12: map<int, bool> := map[f14 := f13];
				var v13: set<array<bool>> := {f15, f15};
				var v14: set<bool> := {f11, f13};
				globalState.f4, v9, v10, r3 := f13, v9, (v10 - (multiset{|(map v11 : int | v11 in v12 :: (v11 - (if (v6.f28 in v10) then v10[v6.f28] else v6.f28)) := (f14))|, fm2(f13, p0, f13, globalState)})[|v13| := |v14|]) - v10, f13;
				globalState.f3 := f11;
				f13 := f11;
			} else {
				var v15: seq<seq<bool>> := [f12, f12, f12];
				var v16: seq<int> := [v6.f28];
				var v17: seq<int> := [v6.f28, |[([0x1e2])[p3 := |map[!f13 := p0]|], v16, v16]|];
				var v18: map<int, bool> := map[|f12| := f13];
				var v19 := DC5(fm84(|v18|, p3, f12, |map[f10 := v6.f28]|, globalState));
				var v20 := DC5(v19);
				var v21 := DC5(v20);
				v0 := fm9(v15[f14], v17, v21, globalState);
				v5 := v5;
			...
			var v26: set<C4> := {v25};
			var v27 := new C5(v4[670], v0, f12, v26 >= {v25}, v4[670], fm12(f13, globalState));
		} else {
			v0 := f14;
			f15[398] := f11;
			f15[398] := false;
			...
			var v30 := "v";
			var v31: multiset<multiset<int>> := multiset{v28, (if (0x138 in v29) then v29[0x138] else v28)[|v30| := f14], v28, v28[p0 := f14]};
			...
			var v35: C7 := new C7(v33, v34, f14, f12 + f12, f15[398], f10, f11);
			var v36 := DC80(v35);
			v35 := v36.cf116;
			v30 := v30;
		}
			...
			decreases 100 - i2
		{
			...
			v0 := |(if (f13) then v38 else v38 + v38)[p0 := |v41| / v0]|;
			v0 := f14;
			var v42: map<bool, bool> := map[f11 := f11];
			var v43 := DC73(if (f12[v0] in v42) then v42[f12[v0]] else f13);
			match ((if (false) then v43 else v43).(cf107 := f13)) {
				case DC72(cf104, cf105, cf106) =>
					var v44: map<string, int> := map[v39[v0 := f10] := |fm96(cf104, 0x2b0, --v0, globalState)|];
					var v45: seq<map<bool, bool>> := [map[v40.f11 := false], v42];
					v44 := v44[v39 := v0 + |v45|];
					var v46: array<int> := new int[10](i3 => i3 % f14);
					v46[503] := v0;
					...
					var v50 := DC81(v40.f11, |v49[true := p0]|);
					v0, v0, v38, globalState.f3, v46[503] := p0 * --0x162, v0, v38[p3 + |v38| := -(if (|v47| in v37) then v37[|v47|] else v0)], if (v50 in v48) then v48[v50] else v40.f11, f14;
					v0 := -|(seq(0x181, i4  => ([!cf104, f13])))|;
					v37 := v37;
				case DC73(cf107) =>
					...
					var v66 := new C8(v57[v0], p3, [v40.f11, f11], |p1| == p0, v40.f10, f11, p3 * v58.cf98, if (v64 in v65) then v65[v64] else |v38|);
				...
				case DC71(cf103) =>
					...
					var v69: seq<seq<bool>> := [f12[p0 := f11]];
					var v70: map<bool, seq<bool>> := map[f13 := v69[-0x128]];
					var v71 := new C6(v67, f15, p0 / p0, if (true in v70) then v70[true] else f12, v40.f11, f10, 0x3be <= v0);
					v0 := (if (f11) then p0 else p3) % p0;
					r3 := !f11;
				case DC74(cf108) =>
					v0 := v0;
					...
					v72[838] := v74;
					f15[600] := true;
					v40.f11, v72[838], f15[600], v39 := f11, v74, if (false in v42) then v42[false] else true, "qayihs" + v39;
					v0 := v0;
		...
		match (v78) {
			case DC72(cf104, cf105, cf106) =>
				...
				var v81: seq<int> := [|v80|, -(f14 * p3), v80[p0] + p3];
				v80 := (v81 + [p0]) + v81;
				if (cf104) {
					...
					v82, f10, v82, v84, v85 := seq(0xa1, i7  => (v84.f10)), 'v', v84.fm6(v84.f14, globalState), v84, v85;
					v82 := v82;
					v82 := "hjcsnjkm";
				} else {
					globalState.f4 := f12[v0];
					...
					f11, v87[595], v0, v90[957] := !v88.f13, v91.cf119, |v92|, v80 + v81[v0 := v0];
				}
			...
			case DC73(cf107) =>
				...
				v76[599] := ---p3;
				var v97 := "hvoepgije";
				var v98: C1 := new C1(v0, [f11], f11, v97[0x338], cf107);
				var v99 := DC61(v98);
				v99 := v99.(cf91 := v98);
				v76[599] := v98.f24;
			case DC71(cf103) =>
				...
				v100[135] := [v102, v102] + v103;
				v100[135] := v103;
				var v104 := new C0(-p0 * p3, f13);
				var v105: map<string, bool> := map[v101 := f13];
				v105 := v105 + v105;
				v0, v0 := (0x33b + v104.f21) / (f14 * v0), v0;
			case DC74(cf108) =>
				f11 := f13 || f11;
				v0 := v0 / (if (f11) then v0 else |map[f11 := f13]|);
				...
				v106[499] := v113;
				v106[499] := map v114 : int | (0x3cf <= v114) && (v114 < 0x320) :: (v114 % f14) := (v112 + v111);
				v76[94] := p0;
				v76[94] := p3;
		}
		...
		r1 := v76;
		var v116: seq<int> := [p0];
		r2 := fm1(!!f12[p0], |v116| / p3, globalState);
		r3 := f13;
	}
	method m1(p0: int, p1: int, p2: bool, globalState: GlobalState) returns (r0: map<int, bool>, r1: int, r2: int) {
		...
		if (!!p2) {
			...
			var v12: array<int> := new int[11](i0 => i0 % p1);
			v12[95] := -(|f12| + f14);
			v12[95], v0 := p1, seq(0x311, i1  => (p0));
			var v13: seq<bool> := [!(v12[95] < v12[95]), p2, f11, v0 <= fm36(135, globalState)];
			var v14: multiset<bool> := multiset{f11, f11};
			var v15 := "atnhln";
			var v16 := DC76(f12[|v15|]);
			globalState.f3, v12[95], globalState.f4, v13 := true, -|(v13 + v13)|, p2, [multiset{true, f11} >= v14, p2, f13, f11, !v16.cf110];
			f15[841] := false;
			var v17: seq<string> := [v15];
			var v18 := DC41(|(seq(-0x1ab, i2  => (f10)))|, |"ehi"|, v17[v12[95]], p2);
			f15[841] := v18.cf57 != v15;
		}
		...
		var v25: map<int, D25> := map[|(seq(-47, i3  => ('v')))| := v24];
		var v26: seq<map<int, D25>> := [v25];
		r1 := -((if (f13) then -p1 else p0) % |(v26 + v26)|);
		var v27: array<char> := new char[10];
		v27[572] := if (f11) then f10 else f10;
		var v28 := "waxj";
		v27[572] := fm55(-|v28|, p2, f13, globalState);
		if (p2) {
			globalState.f4 := f11;
			r1 := -f14;
			...
			v29 := f12;
			if (f13) {
				var v30: map<string, string> := map[(seq(0x226, i4  => (v27[572]))) + v28 := v28];
				v30 := v30[v28 := fm6(|v28|, globalState)];
				var v31: C1 := new C1(|(v0 + (seq(156, i5  => (p0))))|, f12 + f12, p2, fm17(f13, globalState), p2);
				var v32 := DC61(v31);
				...
				var v44: map<string, int> := map[v28 := p1 % f14];
			...
			if (f13) {
				...
				var v50 := DC25('e');
				v49.f21 := |(v28 + v28[0x175 := v50.cf29])| - p1;
				globalState.f3 := (f14 / f14) >= p1;
				r1 := -(p1 / p0);
			} else {
				globalState.f4 := p2;
				r1, r1, r2 := 734, p0, p1;
				...
				var v53: array<int> := new int[2](i7 => i7 / -f14);
				...
				var v58: array<D9> := new D9[29] [v55, v55, v55, v55, v55, v55, DC20(|{f13}|, v54), v55, v55, v55, v55.(cf24 := v54), v55, v55, v55.(cf24 := v54), v55, v55.(cf23 := f14), v55, v55, v55, DC20(|v28|, v54[v53 := f14]), v55, v55, v55, v55, v55, v55, v55, DC20(622, v54), DC20(f14, if (|v57| in v56) then v56[|v57|] else multiset{v53})];
				v52[209] := v58;
				v52[209] := v58;
			}
			...
			var v66: T5 := new C2(|(seq(0x2af, i8  => (v60.f26)))|, v60.f27, f10, v59.f13, p1, -|v65|, v60.f27, v59.f12 + f12, v28 <= v28);
			v61[937] := v66.f16;
			var v67: map<int, T5> := map[v66.f16 + v66.f17 := v66];
			v66, v61[937] := if (v60.f26 in v67) then v67[v60.f26] else if (v66.f13) then v66 else v66, 498;
			v28 := ((seq(-666, i9  => (v59.f10))) + v28) + v28;
			var v68 := DC1(v66.f16);
			f13 := !(|v28| >= v68.cf1);
		}
	...
	method m2(p0: bool, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: multiset<seq<int>>, r3: seq<int>) {
		...
		for i0 := |((if (p2 in v2) then v2[p2] else "nrwfmjg") + v1[f14 := f10])| to --(|f12| + f14) {
			var v3: array<int> := new int[29](i1 => i1 / f14);
			...
			v3[276] := -(if (f13) then i0 else i0);
			var v13: set<set<bool>> := {{f11, p1}};
			var v14: seq<bool> := [v13 > v13, p2];
			var v15: set<bool> := {f13, f11};
			v3[276], v14 := f14, [|v15| >= i0, !(i0 != f14)];
			var v16: set<int> := {i0};
			m10(i0 / |v16|, globalState);
			r0 := f14 % v3[276];
		}
		var v17: map<seq<char>, bool> := map[v1 := f13];
		var v18: multiset<bool> := multiset{p1, true};
		var v19: array<bool> := new bool[20] [f14 <= 992, f11, p2, p2, f13, p0, p1, p2, p1, if ((seq(23, i2  => (f10))) in v17) then v17[seq(23, i2  => (f10))] else f13, p0, f11, p2, false, p2, p1, fm1(fm1(f11, 0x1da, globalState), f14, globalState), p0, v18 >= v18, !f13];
		...
		if (p2) {
			...
			v22 := v22[f14 % fm11(f12, p1, globalState) := if (f11) then p0 else p2];
			var v23: array<int> := new int[9](i3 => i3 * |(seq(0x2e8, i4  => (f14)))[f14 := -|v21|]|);
			var v24: map<bool, array<int>> := map[p2 := v23];
			v19, r0, v24 := f15, (f14 + f14) + (|"mew"| / |v1|), v24;
			globalState.f3 := p1;
			var v25 := DC26(f10, f10, !p2);
			var v26 := DC27(v25);
			var v27 := DC27(v26);
			var v28: map<D12, bool> := map[v27 := !(f14 >= f14)];
			var v29: map<map<int, int>, D12> := map[map[|(seq(-0x157, i5  => (f10)))| := f14] := v25];
			v28 := v28[DC27(if (v0 in v29) then v29[v0] else v25) := p0];
			var v30: map<multiset<bool>, int> := map[v18[false := f14] := |(f12 + f12)|];
			v30 := v30[v18 := f14];
		} else {
			...
			if (fm12(if (!p0) then p0 else f13, globalState)) {
				...
				v36[153] := f14;
				var v37 := DC9(f12);
				v36[153], v36[153] := (f14 % f14) / fm11([p0, false, p1], f13, globalState), if (f13) then 0x2f8 - 0x6c else |(map[f11 := f12] + (map[p1 := v37.cf11])[f13 := [!false]])|;
				globalState.f4 := f11;
				r0 := 0x3a9;
				globalState.f4 := f13;
			} else {
				var v38: array<seq<bool>> := new seq<bool>[23] [f12[f14 := p0], f12, f12, f12, f12, f12, f12, f12, f12, f12, f12, f12[|v1| := p0], f12, f12, [p1], f12, f12, f12, [p0], f12, f12, f12, f12];
				var v39 := DC8(v38);
				var v40: set<D4> := {v39};
				var v41: seq<set<D4>> := [v40, {v39}];
				var v42: array<set<D4>> := new set<D4>[13] [v40 * {v39, v39}, v40, {v39, v39}, v40, v40 + v40, v40 + v40, v40, v41[|v1|], v40, v40 - v40, if (p2) then v40 else v40, v40 - v40, v40];
				var v43 := DC87(v42);
				...
				v48[412] := |(v17 + map[v1 := v44.f13])|;
				var v49: seq<bool> := [v18 <= v18];
				var v50 := DC32(f14);
				var v51: set<D15> := {v50, v50};
				v48[412], v49, r0 := f14 * (f14 % v44.fm5(p0, globalState)), v44.f12, |v51| / f14;
				var v52 := new C3(v48[412], f14, v44.f12, v44.f13, f10, true);
		...
		var v56: multiset<seq<int>> := multiset{v54[f14] + fm75(f14, v55, |[f14]|, p2, globalState), v53};
		r2 := v56;
		r3 := (v53 + [f14])[f14 := f14] + v53;
	}
	method m9(p0: int, p1: bool, globalState: GlobalState) returns (r0: int) {
		var v0 := DC4(f11, f14, true);
		var v1: set<bool> := {p1, v0.cf4, f13, p1, f12[0x114]};
		var v2: map<array<bool>, set<bool>> := map[f15 := v1];
		v2 := v2;
		var v3: multiset<bool> := multiset{true};
		var v4: seq<int> := [-|f12[p0 := f13]|, if (f11 in v3) then v3[f11] else f14];
		var i0 := 0;
			...
			decreases 100 - i0
		{
			...
			var v9: set<int> := {|multiset(v5)|, if (f14 in v7) then v7[f14] else p0, |v8| / f14};
			v9 := fm64(globalState) - v9;
			var v10: array<int> := new int[22];
			v10[392] := f14;
			v10[392] := -p0;
			var v11 := new C2(f14, f14, f10, f11, f14 + |f12|, f14, v10[392] / f14, f12, f11);
			v10[392] := (v11.f26 - f14) % (f14 + -v10[392]);
		}
		for i1 := f14 to fm2(p1, |(map v12 : int | (0x2e <= v12) && (v12 < -253) :: (v12 + f14) := (f10))|, !!p1, globalState) {
			...
			var v21: array<bool> := new bool[22] [v1 > v1, !(if (0x3be in v18) then v18[0x3be] else f13), f12[f14], true, f11, f11, f11, p1, f11, f14 >= |v17|, p1, f14 < f14, f11, p1, !(f12 <= f12), i1 <= f14, |v20| != |v3|, p1, f13, f13, f11, p1];
			var v22: set<char> := {f10};
			var v23 := DC43(v21, v22, if (f11) then p1 else f13);
			v16[682], v21, v23 := v15.fm6(p0, globalState), v21, v23;
		}
		var v24 := DC89(f11);
		match (v24.(cf131 := f11)) {
			case DC88(cf130) =>
			...
			case DC90(cf132) =>
				...
				var v48: array<array<int>> := new array<int>[6] [v46, v46, v46, v46, v46, v47[0x2f9]];
				var v49: map<int, bool> := map[f14 := p1];
				var v50: multiset<int> := multiset{p0, f14};
				var v51 := DC66(p0, f15[967]);
				var v52: array<bool> := new bool[24] [f11, !(v1 > {if ((if (f14 in v50) then v50[f14] else f14) in v49) then v49[if (f14 in v50) then v50[f14] else f14] else !f11, f13}), p0 >= v51.cf98, f15[967], f13, p1, f13, f15[967], !f11, f13, f15[967], f13, f13, true ==> f15[967], f11, f11, !p1, p1, fm2(f13, |fm6(p0, globalState)|, p1, globalState) == p0, f14 == f14, true <== f13, f12[p0], f11, p1];
		...
		for i3 := f14 to 574 {
			...
			f15[821] := v59 != v59[v58 := v57.f28];
			f15[821] := if (f13) then p1 else f11;
			globalState.f4 := f13;
		}
		r0 := p0;
	}
	method m10(p0: int, globalState: GlobalState) {
		...
		var v1: seq<array<bool>> := [f15, v0[f14], f15, f15];
		var v2: map<bool, seq<array<bool>>> := map[f11 := [f15]];
		var v3 := DC76(f13);
		var v4 := DC71(v1);
		var v5: map<int, seq<array<bool>>> := map[f14 := v0];
		var v6: seq<int> := [f14];
		var v7: array<seq<array<bool>>> := new seq<array<bool>>[26] [v1, v0, [f15], if (!(v3.(cf110 := f13)).cf110 in v2) then v2[!(v3.(cf110 := f13)).cf110] else v4.cf103, if (p0 in v5) then v5[p0] else v1, v1 + v1, v0, v1[p0 := f15], v1, v0, v0 + v0, v1, [f15, f15], v0, v1, [f15], v1 + [f15], v1 + v1, v0 + v1, (v0[|v6| := f15] + v0)[p0 := f15], [f15] + v0, v1, v1, [f15], v1, v1 + v0];
		v7[752] := v0 + v1;
		var v8: map<int, int> := map[-604 := f14];
		var v9: set<map<int, int>> := {v8};
		var v10 := DC57(p0, v6[0x2e4 := |f12|], v9);
		var v11: set<char> := {f10};
		v7[752] := (v4.cf103[-v10.cf83 := f15] + v1)[0x378 % |v11| := f15];
		if (f11) {
			var v12: array<map<int, bool>> := new map<int, bool>[3];
			v12 := v12;
			globalState.f4 := f13;
			var v13: multiset<int> := multiset{p0};
			f11 := (f14 / p0) in (v13 - v13);
			...
			var v18: seq<char> := ['l'];
			v17 := DC43(v17, DC49(set v19 : char | v19 in (v18[p0 := f10])[p0 := f10] :: (v19)).cf71, f11).cf60;
		} else {
			var v20 := 0x330;
			...
			var v27: set<int> := {f14};
			v20, v21, v20, globalState.f4 := (v20 / p0) - (0x287 % f14), if (!f13) then {f11, true} + v23[v20] else v21, -|(v24 + (if (map[p0 := f14] in v26) then v26[map[p0 := f14]] else v24))| - f14, f13 ==> (|v27| > 641);
			...
			v33[684] := v35;
			var v36: array<int> := new int[5](i0 => i0 / v29.f24);
			v36[28] := |{f14, f14}|;
			v32, v33[684], v36[28], v20 := f12 + v32, v35, 0xdc, v20;
			var v37 := DC49(v11 * v11);
			var v38: T3 := new C5(v24[|f12|], -|fm6(|map[v29.f24 := f13]|, globalState)|, f12, f11, f10, f11);
			...
			v37, v36[28], v40, v38.f11, v20 := DC49(set v42 : char | v42 in v41 :: (v42)), v38.f14, v40 + v40, true, f14;
			f13 := |v24| <= v20;
		}
			...
			decreases 100 - i1
		{
			...
			if (if (if (f11 in v43) then v43[f11] else f11) then !f11 else f13) {
				...
				f15[704] := (v53 > v53) ==> (p0 >= |f12|);
				globalState.f3 := f11;
				var v54: array<bool> := new bool[13](i2 => f15[704]);
				var v55: set<bool> := {f15[704], false, f13};
				var v56 := new C7(v53, v54, --(-0x74 / |v55|), f12 + f12, !f15[704], fm17(f11, globalState), f11);
			}
			
			var v57: array<int> := new int[1](i3 => i3 * |multiset{map[p0 := f11]}|);
			var v58: map<bool, int> := map[f11 := f14];
			v57[428] := if (f11 in v58) then v58[f11] else p0;
			v57[428] := |(f12 + f12)|;
			v57[428] := f14;
		...
		match (match v61.(cf41 := v60) {
			...
			case DC29(cf35, cf36, cf37) =>
				...
				var v67: array<multiset<bool>> := new multiset<bool>[22] [v60, fm30(cf35.f22, cf35.f21, f14, f14, globalState), v60, v60, v60, fm30(cf35.f22, |v62|, |v65|, cf35.f21, globalState), v60, v60, v60, v60, v60, v60, v60, v60, v60, v60, v66[cf36[599]], v60, v60, v60, v60, v60];
				var v68 := new C6(v67, f15, f14, f12, |v62| > cf35.f21, v62[cf35.f21], f13);
			...
			case DC28(cf34) =>
				...
				match (v73) {
					...
					case DC42(cf59) =>
						...
						var v78: map<char, seq<int>> := map[f10 := v6[p0 := f14]];
						var v79: multiset<int> := multiset{f14, -|map[f11 := f10]|};
						var v80: array<seq<int>> := new seq<int>[26] [v6, v77[-0x10e], v6, v6, v6, v6, v6, v6, (if (f10 in v78) then v78[f10] else v6) + (seq(0x389, i5  => (p0))), v6, v6, v6, v77[f14] + v6, [p0, p0, f14], v6 + [p0], v6 + v6, v6, v6, [p0] + v6, v6, v6, v6, fm75(f14, v79, |v79|, cf59, globalState), v10.cf84[p0 := fm11(f12, cf59, globalState)], v6, v6];
						v80[852] := seq(-0x9, i6  => (-p0));
						var v81: multiset<char> := multiset{'m', f10};
						v80[852] := v6 + (fm45(-(if (f10 in v81) then v81[f10] else |v60|), globalState) + v6);
						var v82 := 0x27e;
						...
						v83.f21 := fm2(cf59, |(v72 + (seq(0x295, i8  => (f10))))|, v83.f22, globalState);
					case DC43(cf60, cf61, cf62) =>
						var v88: array<int> := new int[7];
						v88[38] := 0x396 * 0x108;
						v88[38] := 742;
						cf60[176] := !!(|v72| <= p0);
						cf60[176] := if (f13) then !false else cf62 ==> cf62;
						v88 := v88;
						v88 := v88;
					case DC40(cf54) =>
						f15[439] := f13;
				...
				var v92: seq<string> := [seq(332, i10  => (f10))];
				var v93: multiset<int> := multiset{p0};
				var v94: map<string, char> := map[v92[--0x269] + v72 := fm33(v93, f14, globalState)];
				...
				v98[718] := v95;
				v98[718] := v95;
				var v99: array<int> := new int[19](i11 => i11 / p0);
				v99[111] := f14;
				v99[111] := fm2(f11, -f14 + p0, f11, globalState);
		}
...
class C11 extends T4, T5 {
	...
	method m5(p0: int, p1: int, p2: seq<D0>, p3: int, globalState: GlobalState) returns (r0: bool) {
		...
		var v1 := "xpykharcm";
		var v2: seq<int> := [f16];
		var v3: array<int> := new int[11] [if (p0 in v0) then v0[p0] else 538, p0, p3, |v1|, |"x"|, f17, f14, 634, f16, v2[f16], f17];
		...
		for i0 := f16 to |v4| {
			...
			var v10 := new C7(multiset([false] + f12), f15, f14, f12, -|{f10}| == v2[v9.cf113], v8.f23, f13);
			v3[536] := 0xb4 + p3;
			v3[536] := v2[p3];
		}
		f10 := f10;
		if (f13) {
			...
			var v12 := new C3(v2[fm7(v11, p3, globalState)], p3, f12, f13, f10, f13);
			if ((f16 / f14) >= p3) {
				v3[740] := p1;
				v3[740] := f14;
				v1 := v1;
				...
				v13[301] := (v14 + v15[p1])[f12 := v16[f17]];
				...
				var v25: array<set<D4>> := new set<D4>[14] [v22, v22, v22, v22, {v23}, v22, {v23}, v22, v22, {v23, v23}, v22, v22, {v23, DC8(v21)}, v24[p3]];
				var v26 := DC87(v25);
				...
				v20 := if (fm8(false, globalState)) then -|v29| / f16 else 0x278;
			...
			if (if (f12 in v33) then v33[f12] else v34 == v34) {
				...
				v37 := (fm0(v35.f11, -p0, |v1[v12.f28 := f10]|, v35.f13, globalState))[(if (0x235 in v34) then v34[0x235] else |multiset{p3, v35.f14}|) >= v35.f14 := v12.f28];
				var v38: array<map<bool, int>> := new map<bool, int>[11](i2 => map[f13 := v35.f14]);
				v38[200] := v37;
				v38[200] := v37 + fm0(v35.f11, f16, p3, f11, globalState);
				...
				var v58: array<seq<map<D33, D0>>> := new seq<map<D33, D0>>[16] [v54, v54, v54, [v53, v53], [v53, v53[v51.(cf108 := DC74(v55[|v56|])) := v52], v53], v54, v54, (v54 + [v53])[p3 := v53], v54 + [v53], [v53], v54, [v53, map[v51.(cf108 := v50) := v52], v53[v51 := v52], v53, v53], v54, [map[DC74(v50) := v52], v53] + v54, v54, if (f13 in v57) then v57[f13] else v54];
				v58[301] := v54;
				v58[301] := (v54 + v54) + v54;
			...
			match (v66) {
				case DC51(cf73, cf74, cf75) =>
					...
					cf73, cf73, v3[630], cf73, v68 := f14, |v2| * p1, |(v1 + v1[f16 := f10])|, |(v70[{0x309} := fm2(cf74, -|v0|, cf74, globalState)] - DC91(multiset(v71)).cf133)|, map[v0[|f12| := 784] := cf74] + v68;
				...
				case DC50(cf72) =>
					...
					v3[515] := -0x156;
					v78, v3, v3[515], globalState.f3 := v78, v3, p1, f13;
					var v79: multiset<string> := multiset{v1};
					var v80: seq<map<int, int>> := [v0, v0];
					var v81: map<int, bool> := map[f14 := f11];
					var v82: array<int> := new int[23] [cf72.fm7(v79, 0x373, globalState), f17 % v2[0x188], f17, -f16 * p1, f14, 399, -(p0 * v3[515]), |v80|, v3[515], f14 + -f14, f14, p1, f14, f16, f16 % |v1|, |(f12 + f12)|, p0, f17, p1, |v81| - f14, p3, -p0, v2[p3]];
					v82, f10 := v82, f10;
					var v83: array<C0> := new C0[28];
					v83 := v83;
					f15[119] := f11;
					var v84: multiset<bool> := multiset{f13, false, f13, f11, f11};
					var v85 := DC33(v84[f13 := v3[515]]);
					...
					var v89: T2 := new C8(v88, 0x110, f12, !f13, cf72.f23, f12[p0], f14, p1);
					var v90: set<bool> := {f13, v86.f11};
					var v91 := DC62({v89.f11});
					var v92: map<int, set<bool>> := map[v3[515] := {true, f13}];
					...
					globalState.f4, globalState.f3, f15[119], v86 := {map[v89 := DC68(v95)]} < (v97 - v97), v86.f13, false <==> (-p1 < f14), if (v86.f13) then v86 else v86;
				case DC52(cf76) =>
					...
					var v102: C8 := new C8(v101[-f16], 0x141, f12, f11, 't', f11, 0x14a, -0x367);
					var v103: map<C8, seq<bool>> := map[v102 := f12];
					var v104 := new C5(f10, if (f11) then f14 else p3, (if (v102 in v103) then v103[v102] else fm46(f13, f14, f17, globalState))[-0x1f6 := f11], f13, f10, f11 || !f13);
					var v105 := 0x134;
					v105 := --(p3 * f17);
			...
			f10 := v1[fm5(f13, globalState)];
			var v110: array<multiset<bool>> := new multiset<bool>[5](i3 => DC33(DC33(multiset(f12)).cf41).cf41);
			var v111 := new C6(if (false) then v110 else v110, f15, f16, if (f13) then ([f13, !f13, f11])[p1 := !f11] else [f12[f14], f11, f13], f13, f10, f11);
			globalState.f4 := !!f11;
			var v112: map<bool, string> := map[f11 := v1];
			var v113: seq<string> := [v1];
			var v114: array<string> := new string[21] [v1, v1, v1 + v1, v1, v1, v1, v1 + fm6(f16, globalState), DC7(v1).cf9, v1, "kibj", v1, (DC84(f11, f13, p1, v1, f11).(cf123 := f13, cf122 := f13)).cf125, v1, v1, v1 + (if (!f11 in v112) then v112[!f11] else v1[|v1| := f10]), v1, v1, v1[f16 := f10], "cpqdcoml", v113[p0] + v111.fm6(685, globalState), v1];
			v114 := new string[24] [v1, fm6(p0, globalState), "jgw", v1, v1 + v1, "rmuft", v1, (seq(-0x27e, i4  => ('h'))) + v1, v1, fm6(-f17, globalState), v1, seq(0x301, i5  => (f10)), v1, v1, v1, v1 + v1, v1 + v1, v1, seq(0x27b, i6  => (f10)), seq(0xb8, i7  => (f10)), fm6(f16, globalState), v1, v1, v1];
		}
		
		f11 := f11;
		f13 := p0 < (f14 - |{f11}|);
		var v115: seq<seq<bool>> := [f12];
		v0 := v0[|(v115[p0] + [f11, f13, f13])| := if (true) then f16 else f17];
	...
	method m3(p0: multiset<map<int, int>>, globalState: GlobalState) {
		...
		for i0 := f14 % f14 to |(v0 + v0)| {
			...
			globalState.f3, v1, v2, f10 := f11, (f12 + v1) + (v1 + f12), if ((|v5| / f16) in v3) then v3[|v5| / f16] else v2, f10;
			...
			var v9 := new C4(v6, i0, |(v7 + v7)|, |v8| - |v8|, v1, f11 && f11, f10, v0[f14 := f10] != v0);
			f15[630] := f12 <= v1;
			f15[630] := f11;
			v0 := v0 + "ybmbc";
		}
			...
			decreases 100 - i2
		{
			...
			v17[885] := f14;
			v17[885] := f14;
			var v18: C7 := new C7(multiset{f13, fm8(false, globalState)}, f15, |(seq(-721, i3  => (f10)))|, f12, f13, f10, f11);
			var v19 := DC80(v18);
		...
		for i4 := -f14 to f16 {
			...
			var v32: seq<map<T0, D39>> := [v28 + (if (f13 in v29) then v29[f13] else v28)[v30[-|v31|] := v27], v28 + v28[v25 := v27]];
			...
			if (v25.f11) {
				...
				var v42: map<multiset<int>, int> := map[fm4(v39.f13, globalState) := v39.f14 / |[v39.f10]|];
				v22[380], v35, v36[70], v39, v40[854] := v34, fm100(fm7(v41, f17, globalState), f13, globalState), |v42|, v39, v25.f10;
				...
				v0, v39, v45, v46, v36[70] := (if (v39.f13) then v0 else v0[v39.f14 := f10]) + fm6(|(seq(185, i8  => (v0[if (v25.f11 in v34) then v34[v25.f11] else v39.f14])))|, globalState), v39, fm54(f16 >= |v31|, |v31| / v36[70], v49.cf146, !f13, globalState), if (v39.f13) then v46 else v46, (if (v39.f13) then i4 else -f16) - (f16 * v36[70]);
				v36[70] := 0x136;
				var v50: seq<int> := [0x1b1, v36[70]];
				var v51: seq<seq<int>> := [v50, seq(0x6e, i9  => (f17))];
				var v52 := DC3(v51[v36[70]]);
				var v53 := new C1((fm48(v39.f13, f14, v52, globalState)).cf22, v39.f12, v25.f11, if (false) then v0[f14] else f10, if (!v39.f11) then f13 else f13);
				...
				var v58: array<seq<bool>> := new seq<bool>[25] [f12, f12, f12, f12, f12, f12[f16 := v25.f11], f12, ([f11])[|v57| := f13], f12, f12, f12, f12, f12, f12, f12, f12, fm74(v0, v25.f10, globalState), f12, f12, [v25.f11], f12, f12, [f13, f11], f12, f12];
				var v59 := DC8(v58);
				var v60: array<D4> := new D4[3] [v59, v59, v59];
				v60[326] := DC8(v58).(cf10 := v58).(cf10 := v58);
				v60[326] := v59;
				...
				v62.f15[621], v66, v64.f13, globalState.f4, v67 := !!!f11, multiset{f16, 0x27a, |(v64.f12 + fm15(globalState))|, |v69[v68 := |v0|]|, f17 % -f17}, v62.f10 !in "h", v62.f13, v67;
			}
		...
		match (v70) {
			case DC78(cf112, cf113, cf114) =>
				...
				var v81: array<map<bool, int>> := new map<bool, int>[17] [fm0(cf112, f17, f14, f11, globalState), v74, v74, v74, v74, map[cf114 := f16], v74, map[!f13 := 743], v74, map[!f12[fm9(f12, v75, v77, globalState)] := f14], v74, v74, v74, v74, v80.cf87, v74, map[cf114 := f17]];
			...
			case DC77(cf111) =>
				...
				var v93 := DC5(v92);
				var v94 := DC5(v92);
				var v95: C8 := new C8(v87 - (set v88 : map<bool, int> | v88 in multiset{v86} :: (v88)), -|(v90[f17] + v89)|, f12, f11 || fm1(!f13, f14, globalState), f10, f13, -0x2de, fm9(f12, v91, v94, globalState));
				globalState.f3, f13, v95 := f13, fm8(f12[-f14], globalState), v95;
			...
			decreases 100 - i12
		{
			...
			v104[896] := |v110|;
			v104, v105, v106[866], v104[896] := v104, f15, f10, |v0|;
			...
			decreases 100 - i15
		{
			...
			var v117: array<int> := new int[4](i16 => i16 % f16);
			...
			var v127 := DC9([f13]);
			var v128: array<map<int, int>> := new map<int, int>[21] [v115, v115, (map v116 : int | (0x180 <= v116) && (v116 < 0x154) :: (v116 - 0x52) := (f16))[|v118| := |v119|], map[|v121| := f14] + v115, v115, v115, fm72('o', f14, f12, f14, globalState), map v122 : int | (0x19a <= v122) && (v122 < 0x362) :: (v122 * 0xb1) := (f16), v123[-0x10a], v115 + v115, v115, v115[if (f10 in v124) then v124[f10] else f14 := if (f10 in v124) then v124[f10] else fm2(false, -|v125|, f11, globalState)] + v115, v115 + (map v126 : int | (0xf4 <= v126) && (v126 < 0xb8) :: (v126 * |v114[f13 := f17]|) := (f17)), fm72(f10, f17, f12, f17, globalState), map[f16 := |v0|] + v115, v123[f16], map[|"ok"| := f17], (v115[f14 := f16])[f14 := f17], v115, v115, fm72(f10, if (f14 in v115) then v115[f14] else -f16, v127.cf11, f16, globalState)];
			v128[706] := v115 + v115;
			v128[706] := v115[f17 := f16];
			var v129: seq<bool> := [f13, f13];
			var v130 := 0x294;
			v129, f10, v130 := v129, if (f13) then f10 else f10, f16;
			var v131: map<bool, string> := map[f13 := seq(982, i17  => (f10))];
			...
			v134[116] := v0;
			globalState.f4, v133[150], f13, globalState.f3, v134[116] := f13, v132, f13, f13, "ph";
		}
	}
	method m4(p0: int, p1: map<int, char>, p2: array<string>, p3: int, globalState: GlobalState) returns (r0: set<bool>, r1: array<int>, r2: bool, r3: bool) {
		var v0 := "wvwkeheg";
		...
		var v3: array<int> := new int[21] [|v0|, f17, f17, f14, v1[p3], f14, f17, p0, |v0|, p3, p3, f17, f14, f16, f14, v1[f14], 0xb6 - f17, p3, |[f11, f11]|, p3, -fm7(multiset(v2), p3, globalState)];
		r1 := v3;
		var v4 := 0x162;
		var v5 := DC3(v1);
			...
			decreases 100 - i1
		{
			...
			v3[959] := f16;
			r3, v0, v4, r2, v3[959] := f13, fm6(f14, globalState), f14, !f11, p0;
			var v12: C0 := new C0(v1[f17], f11);
	...
	method m1(p0: int, p1: int, p2: bool, globalState: GlobalState) returns (r0: map<int, bool>, r1: int, r2: int) {
		...
		v9 := v9;
		var v10 := new C5(f10, |(seq(0x2e6, i0  => (f10)))|, f12, f11, f10, f11);
		if (f11) {
			v3 := -f16;
			f15[963] := f11;
			var v11: map<bool, int> := map[f11 := 935];
			var v12: map<map<bool, int>, bool> := map[v11 := true];
			f15[963] := if (v11 in v12) then v12[v11] else fm1(f13, -v3, globalState);
			globalState.f3, v3 := fm8(!f11, globalState), -|v6[442]| / v5;
			...
			v14[727] := v17;
			v13, globalState.f3, v14[727] := v13, f11, v17;
			...
			var v23: map<map<bool, seq<int>>, int> := map[if (|v6[442]| in v22) then v22[|v6[442]|] else v21 := v8.f19];
			v23 := v23[v21[f11 := fm45(v3, globalState)] := --707];
			v4 := |(((seq(0x2fe, i3  => (v10.f23))) + v7) + ("epueqdlr")[v8.f19 := f10])|;
			var v24: array<int> := new int[15];
			v24[353] := |(map[true := f13])[p2 := f13]|;
			var v25 := DC81(f11, v8.f19);
			v24[353] := v25.cf118;
			var v26: set<char> := {v7[0x23f]};
		...
		r0 := v31;
		var v32: multiset<string> := multiset{v6[442]};
		r1 := v8.f19 % v10.fm7(v32, p1, globalState);
		r2 := fm5(p2, globalState);
	}
	method m2(p0: bool, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: multiset<seq<int>>, r3: seq<int>) {
		var v0 := "cy";
		for i0 := |(if (p0) then ("um")[f16 := f10] else v0)| to f16 * |[p1, f11]| {
			r0 := f17 - f16;
			globalState.f4 := p0;
			...
			match (v1) {
				case DC47(cf66, cf67, cf68) =>
					...
					v2, cf68, v3, r0, f11 := v2 * v2, |v6|, v3, (if (v7 in v9) then v9[v7] else -345) / i0, if (f13) then f11 else v0 < "qpcysx";
				...
				case DC48(cf69, cf70) =>
					...
					var v13 := new C10(v12.cf60, f14 % f17, f12, p0, f10, !f13);
				...
				case DC46(cf65) =>
					...
					globalState.f3 := true;
					var v17: set<int> := {490};
					var v20: multiset<set<int>> := multiset{v17, v17, v17, v17, set v19 : int | v19 in (seq(260, i3  => (DC57(|cf65|, [f14, |{f11}|], {map v18 : int | (0x99 <= v18) && (v18 < -0x14c) :: (v18 / |[p2, p2]|) := (0x140), map[f14 := f17]}).cf83))) :: (v19 % -0x1d2)};
					var v21 := DC91(v20);
					v21 := v21;
					f15[140] := f13;
					var v22: set<bool> := {p2};
					...
					f15[140], v22, r0, f10 := |(if (p2) then v27 else v27)| < (if (f11) then |f12| else 882), {true, p0, f14 < f14, p0}, |(if (p1) then v0 + v0 else v0 + v0)|, f10;
			}
		...
		var v37: array<int> := new int[9] [|v33[f14]|, |map[-|v34| := v36]|, f14, f17, f14, 619, f14, f17, f14];
		match (f18.(cf8 := v37)) {
			case DC6(cf8) =>
				...
				match (v41) {
					case DC36(cf44, cf45, cf46, cf47, cf48) =>
						...
						v47 := v47[v46 := -f16 % f16];
					case DC35(cf43) =>
						f15[510] := if (p0) then true else p1;
						cf8[829] := f14;
						f15[98] := p1;
						var v48: set<int> := {|v0|};
						var v49: map<multiset<int>, int> := map[v40 := --f14];
						f15[510], cf8[829], r0, f15[98], v48 := f13, f17, f14 % (if (v40 in v49) then v49[v40] else f17), true, v48;
						f10 := f10;
						cf8[829] := DC63(f13, p2, cf8[829]).cf95;
						var v50: seq<bool> := [f14 <= f14, f11, f13];
						var v51: map<int, bool> := map[|[f13, !p2]| := p1];
						var v52: C0 := new C0(|v51|, f15[510]);
						var v53 := DC29(v52, v37, f15[510]);
						...
						var v60 := DC34(f15[510]);
						var v61: multiset<bool> := multiset{v52.f22, !p0, v52.f22, f13, v60.cf42};
						globalState.f4, cf8[829], globalState.f3, f15[510], v50 := v53.cf37, |(v59 + v59)|, |v61| !in v48, v0 <= v0, v50 + (f12 + v50);
					case DC37(cf49) =>
				...
				if (fm1(p2, f17, globalState)) {
					...
					var v72: map<int, int> := map[f16 := v71[f17]];
					v70[167] := v72;
					v69, f10, v70[167], globalState.f4, globalState.f4 := (v69 + v69) + v69, fm66(f13, p1, globalState), v72 + v72, !p2, (f17 / f14) > f17;
					...
					r0, globalState.f3, v73, r0, v33 := fm7(v74 * v74, v36.fm7(v74, f14, globalState), globalState), if (v76 in v77) then v77[v76] else !!(v76.f14 >= fm10(false, globalState)), v73, if (|{v76.f14}| in v70[167]) then v70[167][|{v76.f14}|] else v76.f16 + |v76.f12|, v33;
					v37[545] := f16;
					v37[545] := f16 * v76.f14;
				} else {
					var v78 := DC9(f12);
					...
					var v84 := DC3(v83);
					var v85 := DC5(DC5(v84));
					v37[352] := v36.fm9(f12, v83, v85, globalState) * (if (p1) then |v0| else f14);
					r0 := f16;
					var v86: seq<seq<int>> := [v83];
					var v87: set<int> := {|v86[f17]|};
		...
		if ((v94[p1 := |v36.fm6(|v96|, globalState)|] + (multiset(f12))[v95.fm39(v97, v98, globalState) := f16]) !in (v99 + v99)) {
			f11 := -v95.f26 <= f14;
			...
			var v103: map<int, string> := map[v95.f26 := seq(0x3b, i8  => (f10))];
			...
			r0 := (|v112.cf152| % v95.f27) / (f16 * (if (v113 in v115) then v115[v113] else v95.f27));
			var v116 := new C0(v95.f26, v95.fm39(fm43(v95.f27, v110.f11, |[v0[f16 := 'w']]|, p1, globalState), v98, globalState));
			v0, r1 := v0 + fm6(f14, globalState), (v0[v95.f26 := f10] + v0) + (v0 + v0);
			var v117: array<string> := new string[15] [v0, v0, v0, v0 + "mnh", v0, "ksqtb", fm6(v95.f27, globalState), v0, v0[f14 := v110.f10], v0, v0, v0, v0, v0, v0];
			v117 := v117;
			var v118: array<seq<bool>> := new seq<bool>[29](i10 => DC9(f12).cf11);
			v118 := new seq<bool>[29];
		}
		
		for i11 := |map[f17 := p2]| / (if (f11 in v34) then v34[f11] else f17) to f14 {
			...
			v119, v37[777], r1 := v119, if (f11) then |v94| else if (!p2) then v95.f26 else -|v120|, fm6(|v123|, globalState);
			var v124 := DC48(p1, v37[777]);
			var v125: seq<D22> := [v124, v124];
			var v126: set<int> := {|v125|};
			v119[698] := v126 != v126;
			v119[698] := p1;
			var v127 := new C5(f10, f14, f12, f12[i11], f10, !p2);
			var v128: map<string, set<int>> := map[v0 := v126];
			f11 := v128 != v128;
		}
		var v129: multiset<int> := multiset{f14, f17};
		var v130 := DC60(f16, v95.f26, true);
		r0 := |{(v94[!f11 := v95.f27])[f13 := -(if (v130.cf88 in v129) then v129[v130.cf88] else |(set v131 : int | (263 <= v131) && (v131 < 958) :: (v131 % v95.f27))|)], v94, v94}|;
		r0 := -v36.fm5(v95.f26 <= |fm64(globalState)|, globalState);
		r1 := (v0 + (seq(450, i12  => (f10)))) + v0;
		var v132: seq<int> := [f14];
		var v133: multiset<seq<int>> := multiset{v132, seq(0x27b, i13  => (DC47(f16, DC0(-v95.f27), 0x267).cf68)), v132, v132};
		r2 := v133;
		r3 := v132 + v132;
	}
	method m6(p0: int, p1: bool, globalState: GlobalState) returns (r0: int) {
		...
		r0, v0, v0, f13 := p0 / p0, if ((v5 == v5) in v1) then v1[v5 == v5] else v0, v0, p1;
		var i2 := 0;
		while ((f12 + f12) != f12)
			decreases 100 - i2
		{
			...
			var v10: seq<int> := [|(v6 + v6)|, 0x2ef, v7[v9.cf15], 367];
			v10 := v10;
			var v11 := DC67(f14);
			var v12 := new C9(f16 / v11.cf100);
		...
		var v26: array<seq<bool>> := new seq<bool>[15] [((f12 + f12)[|v14| := p1])[|v22| := p1], f12, v23.cf11, f12, f12, f12, v24[f16], f12[f14 := f13], v23.cf11, f12, f12, fm74(seq(0x111, i6  => ('i')), f10, globalState), f12 + fm20(if (f13 in v25) then v25[f13] else false, |f12|, v15.f24, 678, globalState), f12, f12 + ([f13, p1])[0x3a := !f13]];
	...
	method m7(globalState: GlobalState) returns (r0: seq<int>, r1: int, r2: bool, r3: array<int>) {
			...
			decreases 100 - i0
		{
			...
			var v2: seq<int> := [f17, f17, |(seq(0x24c, i1  => (f10)))|];
			...
			if (f14 <= f14) {
				...
				v12 := if (0x2b2 > f17) then v13[f14] else v12;
				f10 := f10;
				globalState.f3 := f13;
			} else {
				...
				var v28: set<int> := {f16};
				var v29 := DC23(v28);
				var v30 := new C7((multiset{v21.f11, v21.f13})[v21.f11 := v2[|(v29.(cf27 := v28)).cf27|]], f15, -f17 / 0x4f, [f11, v21.f11, v21.f13, f13] + f12, v21.f11, v21.f10, f11 && f13);
			}
			...
			var v37 := "d";
			globalState.f3, v33 := !((v35 * {f13, f13}) > (v35 - v35)), v36[|v6| + |v37|];
		}
			...
			decreases 100 - i2
		{
			...
			v42 := {v41, v41, (seq(0x6, i3  => ('e'))) + v41[f16 := f10], (seq(43, i4  => (f10))) + v41, v41};
			...
			v48[875] := v51[v50 := |(seq(386, i5  => (f17)))|];
			v48[875] := v51;
		}
		var v52: array<string> := new seq<char>[29](i7 => seq(-525, i8  => (f10)));
		...
		r2 := fm1(!f13, |(v57 + ([multiset{f17}, v56, multiset{f17, f16, f17}, v56, fm4(f13, globalState)])[0x1d3 := v56])|, globalState);
	...
	method m8(p0: map<int, int>, p1: char, globalState: GlobalState) returns (r0: int, r1: int, r2: int) {
		...
		var v4: map<array<int>, bool> := map[v3 := (DC73(f12[v0.f21]).(cf107 := f11)).cf107];
		f15[913] := if (v3 in v4) then v4[v3] else v0.f22;
		f15[913] := f11;
		...
		if (f13) {
			...
			if (f16 == |v5|) {
				r0 := v0.f21 / fm2(f15[913], f17, f13, globalState);
				...
				var v11: set<bool> := {0x1e7 <= f17, v0.f22, v0.f22 ==> f15[913], false, v0.f22};
				v11 := v11 * v11;
				var v12 := new C3(v0.f21, f16, f12, v0.f22, p1, f13);
				r1 := (v0.f21 % -f14) / -0x2f3;
			} else {
				var v13: array<bool> := new bool[14](i1 => true);
				var v14: map<int, multiset<array<bool>>> := map[f14 := (multiset{v13, v13, v13, v13, v13})[v13 := 0x22f]];
				var v15: multiset<array<bool>> := multiset{v13};
				f13 := !(multiset{v13, v13, v13} >= ((if (v0.f21 in v14) then v14[v0.f21] else v15)[v13 := f17] * v15));
				f15[913] := v0.f21 >= (if (-f14 in p0) then p0[-f14] else f16);
				var v17: seq<map<int, int>> := [fm72(f10, v0.f21, f12, f16, globalState), map v16 : int | (-814 <= v16) && (v16 < 532) :: (v16 % v0.f21) := (-v0.f21)];
			...
			v22 := v22;
			var v23: map<bool, int> := map[f15[913] := v0.f21];
			v3[46] := |v23| + f14;
			v3[46] := -0x379;
			var v24: multiset<string> := multiset{v20};
			var v25: seq<bool> := [!!!!v22.fm39(v24, [v23], globalState)];
			v25 := f12[-v0.f21 / (if (f11 in v23) then v23[f11] else -f16) := v0.f22];
			v23 := v23[f11 := |(map[f15[913] := f11])[f11 := v0.f22]| + v0.f21];
		} else {
			if (f15[913]) {
				var v26 := "makab";
				r1 := v0.f21 / (|v26| - v0.f21);
				f15[913] := v0.f22;
				var v27: set<bool> := {f15[913], v0.f22};
				var v28: seq<bool> := [if (v0.f22) then f15[913] else f13, v0.f22, f11, DC41(|multiset{v0.f22}|, |v27|, seq(-0x3a8, i2  => (f10)), f13).cf58];
				...
				v39 := v39[v33 := [v37]];
				var v40: array<bool> := new bool[2] [f15[913], v0.f22];
				...
				v44 := {map[v0.f22 := |v34|], map[f11 := fm2(f15[913], v0.f21, f11, globalState)], v43, v43, v43} + (v44 - v44);
				r1 := -0x9c - v37.f24;
				var v45: seq<bool> := [v0.f22];
				var v46 := DC9(v45);
				v45 := v46.cf11;
				var v47: C7 := new C7(v1, v40, f14, [v0.f22, v0.f22, v0.f22, f11, f11], v0.f22, f10, f15[913]);
				var v48: set<C7> := {v47};
				r2, r1, globalState.f4 := v0.f21, |(if (f15[913]) then {v47} else v48)| * (if (v0.f22) then f17 else v0.f21), !f13;
			}
			
			r2 := v0.f21;
			var v49: seq<map<int, int>> := [p0, p0];
			var v50 := new C2(f16, -(f14 - |v49[f16]|), f10, f11, -f17, v0.f21 / v0.f21, -(-v0.f21 + f16), f12 + f12, v0.f22);
			...
			match (v53) {
				case DC54(cf78, cf79) =>
					...
					v55 := v55[|fm75(f14, multiset{-v50.f26}, v0.f21, f11, globalState)| := f12[v0.f21]] + v55[f14 := v0.f22];
					r0 := f14;
					globalState.f3, v3, f13 := fm8(f15[913], globalState), v3, !v0.f22;
				case DC53(cf77) =>
...
method Main() {
	...
	var v1 := true;
	var v2: seq<bool> := [v1];
	var globalState := new GlobalState([v0, v0], v2, 646, false, false, v2 + v2, 0x39c, true, 0x399, map v3 : int | (0xd5 <= v3) && (v3 < 0xb3) :: (v3 * |{seq(0x371, i0  => ('y'))}|) := (v1));
		...
		decreases 100 - i1
	{
		...
		var v12: array<array<bool>> := new array<bool>[25] [v7, if (fm1(v1, v0, globalState)) then v7 else v7, v8[fm2(v1, v0, false, globalState)], v7, v7, v7, v7, v7, if (0x344 in v11) then v11[0x344] else v7, v7, v7, v7, v7, v7, v7, v7, v7, v7, v7, v7, v7, v7, v7, v7, v7];
		v12 := v12;
		var v14: map<bool, bool> := map[v1 := v1];
		var v15: map<int, int> := map[|fm3(v0, |v14|, v0, v0, globalState)| := v0];
		var v16 := m0(v0, (map v13 : int | (-0x2da <= v13) && (v13 < 0x2e6) :: (v13 / DC1(v0).cf1) := (v0)) + v15, v1, v1, globalState);
		var v17: map<seq<bool>, bool> := map[[v1, v1, !v1] := false];
		var v18: array<int> := new int[5] [|v17|, v16, v0, v16, v16 / v0];
		v18[126] := v16;
		v18[126] := v16 * (if (v1) then v16 else v0);
	}
	var v19 := 'm';
	globalState.f3, globalState.f4, v19, v1, v0 := v1, v1, v19, true, |map[v0 := v0]| % v0;
	var v20: seq<int> := [v0];
	var v21: seq<int> := [v0, v0, v0, |v20| * v0];
	var v22: array<int> := new int[28](i3 => i3 % -v0);
	var v23 := DC3(v21);
	var v24: seq<seq<int>> := [if (v1) then v23.cf3 else v21, v21 + v20];
	v0, globalState.f4, v21, v22, v6 := |([!v1, fm1(v1, v0, globalState), v1, v1] + (v2 + [v1]))|, v1, v24[v0], v22, v6 + v6;
	...
	for i4 := v0 to |v21| * |v26| {
		...
		v29[670] := {"kqbqrqbce", v6};
		var v30: set<string> := {v6 + v6, v6};
		v29[670], globalState.f3 := v30, fm2(!true, |(seq(-554, i6  => (v19)))|, v2[0x228], globalState) > i4;
		v6 := [if (v1) then v19 else v19];
		var v31 := m0(i4 % v0, map[i4 := v0], i4 == i4, v1, globalState);
		var v32 := DC1(v31);
		v32 := v32;
	}
	var v33: set<int> := {|([v0, -|fm4(v1, globalState)|])[|v21| := v0]|, v0};
	...
	if (v1) {
		...
		v44, v0 := (v44 + v44[877 := v22]) + (v44 + v44), (v0 % -v0) + |v37|;
		var v45: set<map<bool, int>> := {map[v1 := v0]};
		var v48 := new C8(v45, fm2(fm1(v1, |v6|, globalState), v0, fm1(v1, v0, globalState), globalState), v2, v0 in (set v47 : int | v47 in (set v46 : int | (-0x18f <= v46) && (v46 < 0x1bd) :: (v46 / v0)) :: (v47 % 0x221)), 'y', true, 487, v0 - v0);
	...
	var v59 := m0(v0 / v0, v25[v0 := |v6|], v2 != v2, v1, globalState);
	for i8 := v59 to v59 {
		if (false) {
			...
			var v73: C6 := new C6(v71, v72, v59, v2, v19 !in ("dhpgxwcxn")[|v37| := v66.f10], v6[v0], v66.f11);
			v73 := v73;
			var v74: set<char> := {v19, v66.f10, v19, v19, v19};
			var v75, v76, v77, v78 := v73.m2(v66.f11, false, 't' !in v74, globalState);
			var v79: seq<string> := [seq(0x32d, i9  => (v19)), v76, ("k")[v61 := v66.f10], v76];
			v59 := -(|v79[v59]| % |v5|);
		} else {
			...
			v19 := v19;
			v59 := i8;
			globalState.f3, globalState.f3, v59, v21 := true, v1, fm2(v1, -(i8 + v0), v1, globalState), seq(0x3e, i10  => (i8 * |v6|));
		}
	...
	if (v1) {
		...
		var v94: C11 := new C11(DC6(v22), v89, |v6|, v2, !v1, v19, !v1, if (|v6[|v26| := v19]| in v90) then v90[|v6[|v26| := v19]|] else -v0, |v93|);
		...
		v101[362], v0, v104, v105 := v6 + v6, v0 / (v105.f24 % v59), v104, v106[v59 * |v107|];
		...
		var v110: C8 := new C8(v109[v105.f24], v105.f24, v2, v1, v19, true, |{v1, v1}|, v59);
		...
		var v119: array<multiset<int>> := new multiset<int>[29] [v107, multiset{-0x303}, v107[v113.f14 := v113.f14], v107, v107, multiset{v105.f24}, multiset{v113.f14, |v6|, v113.f14}, multiset(v20), v107, v107, v107[-v0 := v113.f14], v107, v107, v107 * (v114.(cf26 := v107)).cf26, multiset{v110.fm10(v113.f13, globalState)} + (if (|v6| in v115) then v115[|v6|] else fm4(v1, globalState)), v107 + v107, multiset{586}, multiset{v0}, v107, v107, v107 * v107, multiset{|v117|}, v107, v107, (if (v0 in v115) then v115[v0] else fm4(v113.f11, globalState))[v105.f24 := |v107|], v107[v113.f17 := 0x351], v107[|v118| := v113.fm10(v1, globalState)] + v107, v107, v107];
		v119[821] := v107 + v107[v105.f24 := v105.f24];
		var v120 := DC6(v22);
		var v121: array<bool> := new bool[6](i11 => v1);
		var v122: C11 := new C11(v120, v121, v105.f24, v2, !v113.f13, v19, v1, v0, v105.f24);
		v113, v119[821], v101, v122 := v113, (multiset{v113.f16} + v107) * multiset{|v101[362]|}, v101, v122;
		...
		match (v124) {
			case DC72(cf104, cf105, cf106) =>
				v113.f10 := v113.f10;
				v22[707] := v113.f17;
				v22[707] := if (v113.f13) then v59 else v113.f14;
				var v125: multiset<string> := multiset{v6};
				var v126 := DC101(v125);
				var v127 := DC103(v126);
				var v128: multiset<char> := multiset{v113.f10, v113.f10};
				cf105[601] := v128 > multiset{v19, v113.f10};
				v127, v0, cf105[601], v0 := v127.(cf151 := DC103(v126)), 76, v113.f13, fm2(v113.f11, |v105.fm6(v105.f24, globalState)|, cf104, globalState);
				v2 := v113.f12;
			case DC73(cf107) =>
				var v129 := m0(v0, map[v116.f28 := v113.f14] + v25, v1, v113.f13, globalState);
				v4 := v4[v1 := -v113.fm5(v113.f13, globalState)];
				var v132: seq<set<int>> := [(set v130 : int | v130 in v21 :: (v130 / 0x11a)) + v33, v33 + v33, set v131 : int | (635 <= v131) && (v131 < 233) :: (v131 * v113.f17)];
				v132 := v132;
			...
			case DC71(cf103) =>
				...
				var v138: array<seq<bool>> := new seq<bool>[15](i13 => ([v113.f13, v113.f13])[v113.f14 := v113.f13] + v113.f12);
				v138 := v138;
				v121[919] := v1;
				v121[919] := v1;
			case DC74(cf108) =>
				v101[362] := v101[362] + "eihi";
				var v139: array<map<bool, set<int>>> := new map<bool, set<int>>[14];
				var v140: map<bool, set<int>> := map[v113.f11 := v33];
				v139[135] := v140[v113.f11 := v33];
				...
				v121[185] := [[v113.f13, v113.f11]] < v146;
				v105.f24, v139[135], v145, globalState.f4, v121[185] := 0x361, (v140 + v140) + v140, v145, v113.f11, v1;
				var v147 := DC0(v113.f17);
				var v148 := DC16(v113.f11);
				var v149 := new C8(v110.f20, v147.cf0, v2[v113.f14 := v1] + v113.f12, v121[185], v113.f10, if (v113.f13) then v113.f13 else v1, |v33| + v0, |[v148.cf18, v113.f13]|);
				var v150 := new C1(|v101[362]|, v2, !true, v19, v113.f14 > |"qhxia"|);
		}
		
		var v151: map<bool, multiset<int>> := map[v113.f13 := multiset{|v6|}];
		if ((v119[821] * multiset(fm36(v105.fm5(v113.f11, globalState), globalState))) > (v107 * (if (v113.f13 in v151) then v151[v113.f13] else v119[821]))) {
			v105.f24, v113.f11 := v113.f14 - (v116.f28 % |"ann"|), v113.f13 || (if (v113.f11) then fm1(v113.f13, v59, globalState) else v1);
			v121[804] := v113.f13;
			v121[804] := fm1(v1, 0x36e, globalState);
			var v152: multiset<seq<int>> := multiset{[v105.f24]};
			var v153 := DC86(v152);
			v121[804] := v21[0x369 := 0x7d] !in v153.cf128;
			v59, v107 := v59, multiset((v21 + (seq(0x258, i15  => (|v6|)))) + ((seq(98, i16  => (|v5|))) + v21));
			v105.f24 := v113.f17;
		} else {
			v121[199] := v113.f11;
			v121[199] := v122.fm8(v113.f11, globalState);
			globalState.f4 := v1;
			globalState.f4 := v113.f13;
			var v154 := DC78(v113.f13, -v113.f16, v113.f11);
			var v155: array<D35> := new D35[20] [v154.(cf113 := if (v113.f14 in v25) then v25[v113.f14] else v105.f24), DC78(v121[199], -v59, true), v154, v154, v154, v154, v154, DC78(v113.f13, v113.f14, v113.f11), v154, v154, fm102(globalState), v154, v154, v154, DC78(v113.f13, v59, !v113.f13), fm102(globalState), v154, v154, v154, v154];
			v155[642] := v154;
			v155[642] := v154;
			v121[199] := v24[v113.f14] != v21;
		}
