...
function fm4(p0: char, p1: string, p2: bool, globalState: GlobalState): seq<int> {
	(if (true) then DC14([DC13(|[|{0x282}|, |map[true := false]|, |(seq(abs(0x150), i0  => ('v')))|]|).cf17]) else DC14([0x2a3, 827, |map[718 := -0x195]|])).cf18
}
function fm5(p0: char, globalState: GlobalState): string {
	match DC8(true, false) {
		case DC7() => "ammm" + (seq(abs(832), i0  => ('o')))
...
function fm9(p0: int, p1: int, p2: int, globalState: GlobalState): set<char> {
	(set v0 : char | v0 in (seq(abs(0x3b7), i0  => ('y'))) :: (v0)) * {'f', 'm', 'i', 'f', 'x'}
}
...
function fm12(p0: map<int, int>, p1: set<int>, p2: int, p3: bool, globalState: GlobalState): map<int, string> {
	map[-0x1d0 := "creunkjsj"] + (map[|multiset{true}| := "ft"] + map[|"lvvvveiw"| := seq(abs(0x110), i0  => ('g'))])
}
method m0(p0: bool, p1: seq<bool>, globalState: GlobalState) returns (r0: string) {
	...
	for i1 := if (p0) then v1 else v1 to |map[v3 := |v6|]| {
		...
		var v12 := DC9(v11);
		var v13: array<seq<bool>> := new seq<bool>[22] [p1, p1[safeIndex(i1, |p1|) := !!p0], p1, p1, p1, p1, v9[safeIndex(v1, |v9|)], p1 + ([!true])[safeIndex(v3.f12, |[!true]|) := p0], p1 + p1, fm10(880, p0, v3.f12, i1, globalState), p1, p1, p1, if (p0) then p1 else [p0], fm10(v3.f12, p0, if (|v12.cf11| in v10) then v10[|v12.cf11|] else i1, i1, globalState), p1, if (p0) then p1 else p1[safeIndex(v1, |p1|) := p0], [p0, p0, p0, p0], if (p0) then p1 else p1, [p0, p0, p0, p0, !p0], [fm2(p0, p1, globalState), p0], p1];
		v13[safeIndex(907, v13.Length)] := p1;
		v8, globalState.f5, v13[safeIndex(907, v13.Length)], v3, globalState.f5 := v8, p0, p1, v3, p0;
		var v14 := DC7();
		var v15: set<D2> := {v14};
		if ((v3.f12 * |v15|) > i1) {
			...
			globalState.f8, v1, v2, globalState.f8 := !(p0 ==> true), i1 - safeDivisionInt(v17.f12, -i1), v2.(cf0 := if (!p0) then p0 else p0), !(p0 ==> p0);
			...
			var v22: array<int> := new int[4] [-460, v3.f12, safeModuloInt(v3.f12, v3.f12), v3.f12];
			v22 := v22;
			var v23: multiset<int> := multiset{fm0(-v1, map[p1 := p0], globalState)};
			var v24 := DC2(p0, |v23|);
			var v25: map<seq<bool>, bool> := map[v13[safeIndex(907, v13.Length)][safeIndex(9, |v13[safeIndex(907, v13.Length)]|) := p0] := p0];
			var v26: seq<map<seq<bool>, bool>> := [map[p1 := p0], v25, v25];
			v1, v1, v4, v24 := -v3.f12, if (v3.f12 in v23) then v23[v3.f12] else fm0(v3.f12, v26[safeIndex(i1, |v26|)], globalState), v4, v24;
			globalState.f5 := true;
			var v27: seq<int> := [fm0(|v7|, fm11(globalState), globalState), |v13[safeIndex(907, v13.Length)]|];
			var v28: map<char, int> := map[v4 := v3.f12];
			globalState.f5 := v27[safeIndex(0x23b, |v27|)] in (fm12(v10, v8, |v28|, p0, globalState))[v3.f12 := v7];
		...
		decreases 100 - i2
	{
		...
		if (v3.f12 == v3.f12) {
			...
			var v46: map<bool, array<int>> := map[p1[safeIndex(v3.f12, |p1|)] := v45];
			...
			v45[safeIndex(287, v45.Length)] := v3.f12;
			var v47: multiset<array<bool>> := multiset{v0};
			var v48: set<seq<int>> := {v41};
			var v49: seq<C0> := [v3];
			var v50: map<bool, seq<C0>> := map[v48 >= v48 := v49 + v49];
			v45[safeIndex(287, v45.Length)], v1, v1, v1, globalState.f8 := |(v47 + v47)| * v3.f12, v3.f12, v3.f12, |v50|, v37.f10 || v37.f10;
			v37 := v37;
			...
			v51[safeIndex(111, v51.Length)] := v53;
			v51[safeIndex(111, v51.Length)] := v53;
		} else {
			var v54 := DC1();
			v54 := v54;
			...
			v57[safeIndex(387, v57.Length)] := if (v37.f10) then v3 else if (v3.f12 in v58) then v58[v3.f12] else v3;
			v57[safeIndex(387, v57.Length)] := v3;
		}
...
method Main() {
	...
	var v2 := 147;
	var v3: multiset<seq<bool>> := multiset{[true], v1, v1[safeIndex(-758, |v1|) := false], v1[safeIndex(v2, |v1|) := !v0]};
	var v4 := DC0(v0);
	...
	if (!((v9 * v9) <= v9)) {
		...
		var v12: array<int> := new int[28](i0 => safeModuloInt(i0, v2));
		v12 := v12;
		var v13 := m0(v0, v1, globalState);
		v12[safeIndex(136, v12.Length)] := v2;
		var v14: set<bool> := {v0, v0, false, v0, v0};
		var v15: set<int> := {|v14|};
		v12[safeIndex(136, v12.Length)] := |v15| + v2;
		var v16: map<bool, bool> := map[v0 := v0];
		var v17: seq<int> := [if (!(if (true in v16) then v16[true] else v0)) then 446 else v12[safeIndex(136, v12.Length)]];
		var v18: map<bool, seq<int>> := map[v0 := v17];
		v17 := (v17 + (if (v0 in v18) then v18[v0] else v17)[safeIndex(v2, |(if (v0 in v18) then v18[v0] else v17)|) := v12[safeIndex(136, v12.Length)]]) + [v12[safeIndex(136, v12.Length)], 0x248];
	} else {
		v9 := v9 + fm1(v2, v0, v2, v0, globalState);
		globalState.f8 := DC4(v0, v6, v0).cf6;
		var v19 := DC3(v6);
		var v20 := DC5(v19);
		v5[safeIndex(158, v5.Length)] := false;
		v2, v8, v20, v2, v5[safeIndex(158, v5.Length)] := v2, v8, v20, v2, v0;
		v5[safeIndex(158, v5.Length)] := fm2(v5[safeIndex(158, v5.Length)], v1 + v1, globalState);
		globalState.f8, v5[safeIndex(158, v5.Length)] := v0 ==> v0, v5[safeIndex(158, v5.Length)];
	}
	...
	for i1 := v2 to v2 {
		...
		v9 := (multiset{v0, v0, v0, v0})[(if (v2 in v25) then v25[v2] else |v22|) <= i1 := abs(i1)];
		globalState.f8 := v0;
		if (!(v0 <==> !v0) ==> v0) {
			var v26 := 'f';
			...
			v5[safeIndex(470, v5.Length)] := true;
			var v29: array<int> := new int[2](i2 => safeModuloInt(i2, v2));
			v29[safeIndex(391, v29.Length)] := -v2;
			var v30 := DC6(fm3(globalState));
			var v31: set<bool> := {v0};
			var v32: seq<int> := [v2, -223];
			v5[safeIndex(470, v5.Length)], v2, v2, v29[safeIndex(391, v29.Length)] := (v30.cf8 - v31) !! (if (v0) then {v0, v0} else v31), safeDivisionInt(v2, v2), v32[safeIndex(i1 * i1, |v32|)], -(i1 + 0x145);
		} else {
			v22 := v22;
			v2 := i1;
	...
	var v38 := "toc";
	var v39 := 'y';
	var v40: array<int> := new int[11](i3 => safeDivisionInt(i3, v2));
	...
	v38 := v38;
	v2 := v2;
	var v43: map<int, map<int, bool>> := map[|map[v2 := v2]| := map v42 : int | (0x2d8 <= v42) && (v42 < 0x396) :: (safeDivisionInt(v42, v2)) := (v0)];
	v5[safeIndex(797, v5.Length)] := 927 !in v43;
	var v45: set<seq<bool>> := {v1};
	v5[safeIndex(797, v5.Length)], v39, v2, v2 := v0, if (true) then if (v0) then v39 else v39 else v39, safeModuloInt(v2, safeModuloInt(v2, fm0(v2, map v44 : seq<bool> | v44 in v45 :: (v44) := (v0), globalState))), 0x259 + v2;
	var v46: map<bool, bool> := map[v5[safeIndex(797, v5.Length)] := v0 <==> v5[safeIndex(797, v5.Length)]];
	var v48: seq<int> := [v2];
	v46 := v46[|v37| in (map v47 : int | v47 in v48 :: (v47 - v2) := (v37)) := v5[safeIndex(797, v5.Length)]];
	var v49 := new C0(v2, v0, v4);
	v2 := v49.f12 * (v49.f12 - v48[safeIndex(0x82, |v48|)]);
	for i5 := |v8| + v49.f12 to v2 {
		globalState.f8 := v0;
		var v50 := new C0(-(v49.f12 * |v6|), if (v49.f12 in v8) then v8[v49.f12] else true, v4);
		v5[safeIndex(797, v5.Length)] := !((fm8(globalState) * multiset(v48)) < v37);
		var v51: seq<C0> := [v50, v50, v50];
		if (|v51| == v2) {
			var v52 := m0(v5[safeIndex(797, v5.Length)], v1, globalState);
			var v53 := m0(v49.fm6(v0, v2, true, globalState), v1, globalState);
			globalState.f8 := v0;
			globalState.f5, v2 := v0, safeDivisionInt(-v49.f12, v49.f12 - -0x3d);
			v5[safeIndex(797, v5.Length)] := v0;
		} else {
			v2 := v50.f12;
			v2 := -safeDivisionInt(v2 - 0x1ca, -v49.f12);
			v39 := v39;
			var v54 := DC6({v5[safeIndex(797, v5.Length)]});
			var v55: map<D2, int> := map[v54 := v50.f12];
			v2 := if (v54 in v55) then v55[v54] else v49.f12 + v49.f12;
			var v56 := m0(v5[safeIndex(797, v5.Length)], v1, globalState);
		}
		
	}
	v5[safeIndex(797, v5.Length)] := (DC8(v0, v5[safeIndex(797, v5.Length)]).(cf10 := v5[safeIndex(797, v5.Length)])).cf9;
	var v57 := new C0(v49.f12, v0, DC0(v5[safeIndex(797, v5.Length)]));
	var v58 := m0(v5[safeIndex(797, v5.Length)], [v5[safeIndex(797, v5.Length)]], globalState);
	print v0, "\n";
	print v1 == [false, false], "\n";
	print v2, "\n";
	print v3 == multiset{[true], [false, false], [false, false], [false, true]}, "\n";
	print v4.cf0, "\n";
	print v5[0], "\n";
	print v5[1], "\n";
	print v5[2], "\n";
	...
	print v5[9], "\n";
	print v5[10], "\n";
	print v6 == map[false := 147], "\n";
	print v7.cf4, "\n";
	print v7.cf5 == map[false := 147], "\n";
	print v7.cf6, "\n";
	print v8 == map[1 := false], "\n";
	print globalState.f0 == multiset{[true], [false, false], [false, false], [false, true]}, "\n";
	...
	print globalState.f6 == map[false := 147], "\n";
	print globalState.f7 == map[1 := false], "\n";
	print globalState.f8, "\n";
	print globalState.f9, "\n";
	print v9 == multiset{true, false, false}, "\n";
	print v36 == map[2 := 147], "\n";
	print v37 == multiset{717}, "\n";
	print v38, "\n";
	print v39, "\n";
	print v40[0], "\n";
	print v40[1], "\n";
	...
	print v40[10], "\n";
	print |v41|, "\n";
	print v43 == map[1 := map[-104 := false, -105 := false, -106 := false, -107 := false, -108 := false, -109 := false, -110 := false, -111 := false, -112 := false, -113 := false, -114 := false, -115 := false, -116 := false, -117 := false, -118 := false, -119 := false, -120 := false, -121 := false, -122 := false, -123 := false, -124 := false, -125 := false, -126 := false, -127 := false, -128 := false, -129 := false, -130 := false, -131 := false]], "\n";
	print v45 == {[false, false]}, "\n";
	print v46 == map[false := false], "\n";
	print v48 == [594], "\n";
	print v49.f12, "\n";
	print v57.f12, "\n";
	print v58, "\n";
