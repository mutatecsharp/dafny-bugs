...
function fm1(p0: bool, p1: string, p2: int, p3: set<int>, globalState: GlobalState): multiset<string> {
	multiset{if (false) then "ugbrgumry" else seq(0x2b3, i0  => ('v')), "ex" + "mqbtpnfr", if (true) then "yqwpvy" else "i", "xu"}
}
...
function fm7(p0: bool, p1: int, p2: string, globalState: GlobalState): seq<string> {
	["mlix"] + (if (true) then ["ovwonibey", "grpdlwr", "smwee", "gbkcxo"] else seq(987, i0  => ("dbrwucen")))
}
...
function fm17(p0: int, p1: bool, p2: bool, globalState: GlobalState): multiset<int> {
	multiset{|[|{true, true}|, 0x35b]| / 0x19b, 0x22d, 0x35f, 0x231, -(324 % -0x124)}
}
function fm21(p0: bool, globalState: GlobalState): int {
	|{true}| / (|map[0x241 := !!false]| * -0x1e8)
}
function fm22(p0: int, p1: char, p2: int, globalState: GlobalState): set<int> {
	set v0 : int | (910 <= v0) && (v0 < 0x353) :: (v0 % |map[false := true]|)
}
function fm23(globalState: GlobalState): string {
	seq(0x37d, i0  => (if (false) then 'i' else 's'))
}
...
function fm28(p0: int, p1: bool, p2: bool, p3: bool, globalState: GlobalState): string {
	match DC44(map[|[|"pbsdg"|, |[false, false]|, 0x283, -|(seq(-0x36e, i0  => (|map[false := |[|(seq(0x367, i1  => ('u')))|]|]|)))|]| := map v0 : int | (-835 <= v0) && (v0 < 308) :: (v0 % 0x109) := (true)]) {
		case DC45(cf90, cf91) => (seq(0x61, i2  => ('o'))) + "rkyyri"
...
function fm38(p0: string, p1: bool, globalState: GlobalState): D7 {
	DC27(|(seq(-0x2f6, i0  => ('w')))| / -0x22f, 0x318 != 0x371, false)
}
function fm39(p0: int, p1: set<int>, globalState: GlobalState): D8 {
	DC31(-467 % --|"ntljmmwo"|, {false, false, true, !!true} < {!false}, [false] + [true])
}
function fm40(p0: int, globalState: GlobalState): seq<map<int, int>> {
	((seq(703, i0  => (map v0 : int | v0 in [0x335] :: (v0 + |map[0xd2 := 0xed]|) := (|{-|(seq(275, i1  => ('p')))|, |multiset{true, false}|}|)))) + [map[|"fswyqjca"| := |[-0xd4]|]]) + [map[-622 := 0x133], map[|[false]| := |map[|multiset{0x17e, -0xb6}| := "wooigd"]|], map[|(map v1 : int | v1 in map[-694 := true] :: (v1 / |"natxquurd"|) := (|[|[false, !false]|]|))| := 0x16d]]
}
...
function fm43(p0: int, globalState: GlobalState): map<int, int> {
	(map[0xd8 := |DC22([false, false]).cf40|] + (map v0 : int | v0 in map[0x25c := true] :: (v0 / |(seq(0x2a6, i0  => ('a')))|) := (|"tv"|))) + (map v1 : int | v1 in (map v2 : int | (0x174 <= v2) && (v2 < 976) :: (v2 % -0x188) := (map[true := !true])) :: (v1 * |multiset{!true}|) := (0x2f3))
}
...
function fm45(p0: bool, globalState: GlobalState): multiset<seq<int>> {
	(multiset{[0xe9], seq(0x92, i0  => (-0x156))} - multiset{seq(0x10e, i1  => (|[false, true, true]|))}) * (multiset{[-0x17b]} - multiset{[86, |{false}|], seq(337, i2  => (-|map[-0x73 := 'y']|))})
}
...
function fm48(p0: int, p1: bool, globalState: GlobalState): D0 {
	match DC32("qeoo") {
		case DC33(cf66, cf67, cf68) => if (!!true) then DC2(|(seq(75, i0  => ('a')))|, false, !!true) else DC2(cf67, true, true)
...
function fm50(p0: seq<bool>, globalState: GlobalState): seq<int> {
	if (|[false, true]| != -791) then [-0x2d7, |[map[true := false]]|] + [|[!true]|, -0x84, |[false, false]|] else (seq(0xc9, i0  => (0x1f5))) + [0x3d5]
}
...
function fm53(p0: bool, p1: int, p2: bool, globalState: GlobalState): set<multiset<bool>> {
	{DC24(false, false, multiset{!true}, true, |(seq(-0x305, i0  => ('p')))|).cf46 - multiset{!true, true, !false, false, false}, multiset{!!true}, if (!false) then multiset{false} else multiset([false]), if (false) then multiset{true} else multiset{true, false}}
}
function fm54(p0: bool, p1: int, globalState: GlobalState): map<int, bool> {
	(map v0 : int | (0x87 <= v0) && (v0 < 0x336) :: (v0 / |[|"cknpw"|]|) := (true)) + (map[|map[|(map v1 : set<bool> | v1 in map[{true} := false] :: (v1) := (false))| := 0x20]| := !true] + map[0x2d := true])
}
...
function fm57(p0: int, p1: char, p2: int, globalState: GlobalState): set<set<int>> {
	set v2 : set<int> | v2 in ((map v0 : set<int> | v0 in [{-|[-0x285, 749]|, |[|map[['v'] := false]|, 0x249]|, |(seq(0x183, i0  => (0x2b3)))|, 0x2bc, |{DC82(map[map[true := false] := map[DC37(-|[-0x3e3, -0x39a]|, false) := false]]), DC82(map[map[true := true] := map v1 : D10 | v1 in multiset{DC37(|map[0x8 := |(seq(371, i1  => ('i')))|]|, false)} :: (v1) := (true)])}|}] :: (v0) := (seq(854, i2  => ('u')))) + map[{|[false]|} := "o"]) :: (v2)
}
...
function fm62(p0: int, p1: bool, p2: int, globalState: GlobalState): D19 {
	DC59(|{true, false, true, false, true}| / -182, -|map[-0x3ad := false]|, 'e', !false, map[false := false] == map[false := false])
}
function fm63(p0: bool, p1: int, p2: int, globalState: GlobalState): multiset<int> {
	(multiset{0x2df, |{829}|} + multiset{64}) + (DC96(multiset{688}).cf160 - multiset([0x161, |(map v0 : int | v0 in (seq(-0x3cc, i0  => (-0x34b))) :: (v0 / 319) := (true))|]))
}
...
function fm66(p0: bool, p1: bool, globalState: GlobalState): map<bool, int> {
	match DC90("rjdxog", false) {
		...
		case DC89(cf149) => if (false) then map[true := |[|(seq(0x10a, i0  => (|[false]|)))|, --0x4e]|] else map[!false := 0xf7]
	}
}
...
function fm69(p0: int, p1: int, p2: int, p3: bool, globalState: GlobalState): set<map<int, int>> {
	{(map v0 : int | (0x39 <= v0) && (v0 < 0x390) :: (v0 + 0x12d) := (549)) + map[|multiset{map[!false := !false]}| := 0x13e], map[-0x30d := |multiset([|{423}|])|] + (map v1 : int | v1 in (seq(0x2e2, i0  => (|[0x219, 418]|))) :: (v1 % |map[!true := true]|) := (|(seq(538, i1  => (map[false := false])))|)), map[0x382 := |DC101(map v2 : multiset<int> | v2 in {multiset{|map[|(seq(0x3a2, i2  => ('n')))| := -|map[0x2e := !false]|]|}} :: (v2) := (-0x1b1)).cf169|] + (map v3 : int | (726 <= v3) && (v3 < 0x208) :: (v3 * |multiset([|[!true]|, |"pe"|])|) := (|"tduiv"|)), map[0x7c := --0x2e5] + map[-0x3ce := -0x17a], map[|{true, false}| := |DC10([|map[true := true]|]).cf22|]}
}
function fm70(p0: int, p1: int, p2: D22, globalState: GlobalState): seq<string> {
	(seq(0xd4, i0  => ("vsq"))) + ["cbqd"]
}
function fm71(p0: int, p1: string, globalState: GlobalState): seq<seq<bool>> {
	((seq(983, i0  => ([true, false]))) + [[false]]) + (DC104([[false]]).cf174 + [[!true, false, false]])
}
...
function fm78(p0: multiset<bool>, globalState: GlobalState): map<seq<int>, int> {
	map[seq(-0x23b, i0  => (|multiset{true, false}|)) := 0x1cb] + (map[[|map[true := 0x86]|] := |"tjlumjo"|] + map[[|"pbnknjehf"|] := |multiset([|map[false := false]|])|])
}
...
function fm81(p0: char, p1: string, p2: bool, globalState: GlobalState): seq<set<int>> {
	[{|"yhktue"|, 0x15}] + [set v0 : int | (0x57 <= v0) && (v0 < 0x1c) :: (v0 / |{0x102, 0x21a}|)]
}
...
function fm83(p0: bool, p1: int, globalState: GlobalState): set<multiset<int>> {
	({multiset{|map[false := false]|, -0x121, 919}, multiset{|(seq(431, i0  => ('j')))|}} - (set v0 : multiset<int> | v0 in [multiset([0x28b])] :: (v0))) * ({multiset{---0x2fd}, multiset{0x2e6}, multiset{|map[-0x2a9 := 'c']|, |(seq(17, i1  => (146)))|}} + {multiset{0x17}, multiset{0x39d, -0x13d, 0x346, |"verglajpg"|, |[|map[0x88 := !true]|, |[true, true, true, !true, false]|]|}})
}
...
function fm87(p0: int, p1: bool, globalState: GlobalState): map<seq<int>, bool> {
	(map[[|(seq(95, i0  => (0x16)))|, |[458]|] := false] + map[[|map[true := -0x1db]|] := false]) + map[[0x1bc] := true]
}
...
function fm90(globalState: GlobalState): D11 {
	DC40(|map[false := true]| % -|map[!!false := true]|, (map v0 : int | (-0xae <= v0) && (v0 < 702) :: (v0 % |[9, --644]|) := (true)) == map[0xe3 := false], |([false, true] + [true])|, true <==> false, |"db"| >= 0x256)
}
function fm91(p0: int, p1: bool, p2: bool, p3: int, globalState: GlobalState): seq<multiset<int>> {
	seq(0x20c, i0  => (multiset{0x7, |[false]|}))
}
...
method m0(p0: seq<set<D0>>, p1: bool, globalState: GlobalState) returns (r0: set<int>, r1: array<int>) {
	...
	match (v0) {
		case DC64() =>
			...
			v2[575] := v3 / DC8(|v4|, p1, v3, v3, v3).cf16;
			globalState.f5 := p1;
		...
		decreases 100 - i1
	{
		...
		var v28 := "g";
		v27[504] := v28 + v28;
		v27[504], v28 := v28 + "bcleus", seq(0x265, i2  => (if (p1) then 't' else 'e'));
	...
	v33[954] := v20;
	var v34: map<D11, char> := map[DC40(|map[v20 := v20]|, p1, v20, p1, p1) := v32[v20]];
	v33[954] := |(v34 + v34)|;
	var v35: array<map<bool, int>> := new map<bool, int>[29](i4 => map[p1 := v20] + map[!p1 := -v33[954]]);
	forall i3 | 0 <= i3 < v35.Length {
		v35[i3] := (map[p1 := 73] + map[true := |[p1, p1]|]) + (map[p1 := v33[954]] + map[p1 := |map[{DC11(v33[954], p1, 952).cf24, p1} := p1]|]);
	}
	var v36: array<char> := new char[10](i6 => if (!p1) then 't' else 'r');
	forall i5 | 0 <= i5 < v36.Length {
		v36[i5] := if (map[{false, p1, p1, p1} := v33[954]] == map[{p1, p1, p1} := v20]) then 't' else 'h';
	}
	r0 := {v33[954]};
	r1 := v33;
}
...
class C1 {
	...
	method m14(p0: array<bool>, p1: int, p2: map<bool, array<bool>>, p3: int, globalState: GlobalState) returns (r0: bool, r1: int, r2: string, r3: int) {
		...
		var v1 := new C0(v0, v0);
		var v2: seq<bool> := [v0];
		var v3: map<bool, int> := map[v0 ==> v2[f28] := f28];
		var v4 := 'j';
		...
		if (!false) {
			...
			var v25: array<map<C0, seq<bool>>> := new map<C0, seq<bool>>[8] [(map[v1 := v2])[v1 := fm27('v', v2[p1], globalState)], if (v1.f30) then v18 else map[v1 := v2], v19[|v22|] + v18, (v23.(cf37 := v18)).cf37, v18, if (v1.f30) then v18 else v18, if (p3 in v24) then v24[p3] else v18, v18];
			v25[891] := v18 + v18;
			v25[891] := v18;
			v1.f29 := v1.f30;
		}
		...
		if (v0) {
			...
			v26[93] := p3;
			v6[267], r3, r1, v1.f29, v26[93] := |fm29(p3, p1, globalState)| >= |"b"|, if (!v1.f30 in v3) then v3[!v1.f30] else p1, p3, v1.f29, if (v0) then 0x1e6 + f28 else f28;
			var v27 := DC19();
			var v28 := DC20(v27);
			match (v28) {
				case DC19() =>
					globalState.f7 := |f27| / f28;
					var v29 := DC5(fm30(f27, v1.f29, v6[267], |f27|, globalState));
					var v30: map<int, char> := map[|v3| := v4];
					v29, globalState.f7 := if (v1.f29) then v29 else DC5(v30), |f27|;
					r0 := true;
					globalState.f7 := p1 - v26[93];
				case DC18(cf37) =>
					v26 := v26;
					globalState.f7 := v26[93];
					v26 := v26;
			...
			var v41 := DC3(f28, v6[267]);
			var v42: set<D0> := {v41};
			var v43: seq<set<D0>> := [v42];
			var v44, v45 := m0(v43, !v1.f29, globalState);
			var v46: array<char> := new char[28](i1 => v4);
			v46[773] := v4;
			var v47: array<seq<bool>> := new seq<bool>[21](i2 => [v0, v6[267]]);
			var v48 := DC22([v1.f29, v6[267]]);
			v47[68] := v48.cf40;
			var v49 := DC0(v6[267] && v1.f29);
			var v50: multiset<int> := multiset{f28};
			var v51: map<multiset<int>, bool> := map[v50 := v6[267]];
			r3, v46[773], v47[68], v26[93], v49 := |(v51 + v51)[if (v6[267]) then v50 else v50 := v26[93] != v26[93]]|, v4, v2, f28, v49.(cf0 := v0);
			var v52: multiset<bool> := multiset{!v6[267]};
			var v53: map<char, multiset<bool>> := map[v4 := v52];
			v53 := v53[v46[773] := v52[v0 := p1]];
		} else {
			r1 := f28;
...
class C2 extends T1, T0 {
	...
	method m3(p0: D0, p1: seq<bool>, p2: int, p3: bool, globalState: GlobalState) returns (r0: seq<set<bool>>, r1: bool) {
		if (!p3) {
			...
			v0[113] := fm25(p2, v1, true, globalState);
			v0[113] := p2 * -|(if (f14) then p1 else p1)|;
			...
			if (f13) {
				...
				v12 := v12[v0[113] := v11];
				var v13: map<int, set<bool>> := map[v0[113] := v1];
				v13 := v13[p2 := v1];
				globalState.f5 := f14;
				var v14 := DC8(v0[113], v6.f29, |v3|, p2, p2);
				v14 := DC8(v0[113], v6.f30, v0[113], v0[113], p2).(cf20 := p2, cf19 := v0[113]).(cf20 := |p1| + |(seq(0x3a5, i2  => ('o')))|, cf18 := p2, cf19 := -(v0[113] * fm25(v0[113], v1, !true, globalState)));
			} else {
				var v15 := "yowmmq";
				var v16 := new C1(v15 + v15, p2);
				var v17 := new C1(v15, p2);
				v15 := "hgariu";
				var v18: multiset<bool> := multiset{v2[391], !p3, v2[391]};
				var v19: map<multiset<bool>, int> := map[v18 := v16.f28];
				v19 := v19[v18 + v18 := v0[113] * v16.f28];
				globalState.f5 := !!v2[391];
			}
			...
			if ((false <== p3) in (if (v20 in v22) then v22[v20] else v21)) {
				...
				v30 := v30[map v31 : int | (490 <= v31) && (v31 < 0x2e) :: (v31 % v28) := (p3) := v26 + (if (v28 in v32) then v32[v28] else seq(0x245, i3  => (v20)))];
			} else {
				...
				v35[219] := p2;
				v33[613], v35[219] := v34, p2 % p2;
				var v36: map<bool, seq<map<bool, int>>> := map[f13 := seq(-0x23a, i5  => (map[false := v35[219]]))];
				var v37: map<bool, int> := map[f13 := p2];
				var v38: seq<map<bool, int>> := [v37];
				v36 := v36[fm9(v20, globalState) := v38];
				var v39: seq<int> := [p2 + v35[219]];
				v39 := v39;
				var v40: array<map<int, bool>> := new map<int, bool>[11];
				v40 := v40;
				var v41 := "o";
				globalState.f8 := fm25(v35[219], DC26(v21).cf50, p3, globalState) % |v41|;
			}
			...
			match (v45) {
				case DC1(cf1, cf2, cf3, cf4) =>
					...
					var v48: array<array<string>> := new array<string>[13] [v46, v46, v46, v46, v46, v46, if ((seq(395, i6  => (v20))) in v47) then v47[seq(395, i6  => (v20))] else v46, v46, v46, v46, v46, v46, if (!f13) then v46 else v46];
					v48[315] := v46;
					var v49: array<int> := new int[7](i7 => i7 / |v43|);
					var v50: map<int, string> := map[|fm28(|[p2, p2, p2]|, false, true, true, globalState)| := v43];
					v49[371] := |(if (-151 in v50) then v50[-151] else v43)|;
					v48[315], v49[371], globalState.f7, v21 := v46, p2 * p2, p2, v21;
					var v51: map<bool, bool> := map[f13 := !f14];
					var v52: map<bool, int> := map[f14 := fm25(v49[371], v21, p3, globalState)];
					globalState.f5, globalState.f8, v20, v20, v43 := if (fm0(|(seq(0x2e7, i8  => (v20)))|, true, globalState)) then p3 else !fm9(v43[|v51|], globalState), |v52|, 'f', v20, seq(0xbb, i9  => (v20));
					var v53: multiset<bool> := multiset{f13};
					v51, globalState.f5, v49[371], globalState.f5, globalState.f5 := (map[true := f14] + v51[cf2 := !false])[v53 < v53 := f13], p1[p2 % |[f14]|], -(p2 * v49[371]), !(v42 >= v42), if (p3) then f13 else !cf2;
					globalState.f7 := p2;
				case DC2(cf5, cf6, cf7) =>
					...
					v55[655] := v54;
					r1, cf5, v55[655], cf7 := !(f14 ==> cf6), -fm25(0x386, {!f13, !p3}, p3, globalState), v54, p2 < (p2 + cf5);
				...
				case DC3(cf8, cf9) =>
					...
					globalState.f8 := p2;
					globalState.f7 := p2;
					globalState.f7 := -cf8 / fm25(-p2, v21, p3, globalState);
				case DC0(cf0) =>
					...
					v68 := v68[v67 := v20];
					var v69: seq<int> := [v67[251], p2];
					var v70: seq<seq<int>> := [([929])[v67[251] := v67[251]], v69 + v69];
					var v71: array<array<bool>> := new array<bool>[26];
					var v72: map<array<array<bool>>, seq<seq<int>>> := map[v71 := v70 + v70];
					globalState.f8, globalState.f5, v70 := v67[251], f14, if (v71 in v72) then v72[v71] else v70 + v70;
					var v73: map<int, bool> := map[fm25(v67[251], v21, f13, globalState) := v43 < "lohxbb"];
					v73 := map[v67[251] := if (|v73| in v73) then v73[|v73|] else p3];
				case DC4(cf10) =>
					var v74: multiset<int> := multiset{-519};
					var v75: seq<int> := [-p2, |multiset{v20, v20}|];
					var v76 := DC3(-p2, f13);
					var v77: array<int> := new int[11] [--p2 - p2, p2, 0x259 / (if (p2 in v74) then v74[p2] else |v75|), p2, fm25(v75[fm25(-0xe9, v21, f13, globalState)], fm33(p2, f13, f13, globalState), f14, globalState), 683, |v75|, p2, p2 % p2, p2 * v76.cf8, p2];
					v77 := v77;
					var v78: array<D7> := new D7[9];
					var v79: seq<array<D7>> := [v78, v78, v78];
					var v80: map<seq<array<D7>>, int> := map[(v79 + v79)[-|{v75}| := v78] := p2 * p2];
					var v81: map<int, int> := map[p2 := p2];
					v80 := v80[(v79 + v79)[|v81| := v78] := 911 % |v75|];
		...
		if (f14) {
			...
			if (v88.f30) {
				...
				var v103: array<set<map<char, bool>>> := new set<map<char, bool>>[8] [v98.cf61 * v97, (v98.(cf61 := fm34(v88.f30, v88.f29, p2, globalState))).cf61, v97, set v100 : map<char, bool> | v100 in (set v99 : map<char, bool> | v99 in v97 :: (v99)) :: (v100), v97 - v97, v97 + {v93}, v101[|v102|], v97];
				v103[830] := v101[346];
				v103[830] := v97;
				var v104: set<bool> := {v88.f30, v88.f29};
				globalState.f7 := -fm25(p2 + p2, v104, p2 == -p2, globalState);
				globalState.f7, globalState.f8, v88.f29 := p2, -(0x26f + p2), true || (v95 <= (seq(0x29, i14  => (v90))));
			} else {
				var v105 := "dia";
				var v106 := DC32(v105);
				var v107 := 'd';
				v105 := ((v105 + (seq(0x119, i15  => ('v')))) + v106.cf65)[p2 := v107];
				...
				v108[455] := v88.f29;
				globalState.f8, globalState.f8, globalState.f5, globalState.f7, v108[455] := p2, p2, f14, |v105|, f13;
			}
		...
		if (f13) {
			...
			if (v147.cf0) {
				...
				v148[935] := |map[|[f13, f13]| := f13]| >= (if (!v129.cf15 in v149) then v149[!v129.cf15] else fm25(v143.f28, {p3}, f13, globalState));
				var v151: set<int> := {v143.f28};
				var v152 := DC6(v151);
				globalState.f5 := p3 <== ((set v150 : int | (0x1c1 <= v150) && (v150 < 233) :: (v150 * -v143.f28)) !! v152.cf12);
				var v153: multiset<int> := multiset{-v143.f28, |(seq(-0x32, i18  => (v143.f28)))|, |v143.f27|};
	...
	method m4(p0: bool, p1: seq<int>, globalState: GlobalState) returns (r0: array<bool>, r1: int) {
		var v0: array<bool> := new bool[24](i0 => multiset{-0x216, |multiset([[f14, f14], [f13, f13, p0], [f14]])|, |{0x381}|} !! multiset(seq(269, i1  => (|{748, -0x2ac}|))));
		r0 := v0;
		var v1 := -595;
		var v2: set<bool> := {p0};
		var v3 := "vvrptnxt";
		var v4: set<int> := {fm25(v1, v2, f14, globalState), v1, v1 * |p1[|v3| := 583]|};
		v4 := v4;
		for i2 := v1 to v1 * v1 {
			var v5: seq<bool> := [f13];
			var v6: seq<seq<bool>> := [v5];
			var v7 := 'h';
			var v8: array<string> := new string[26] ["pqflgpy", v3, v3, v3, fm28(|multiset(v6[i2])|, f13, true, f14, globalState), v3, v3, v3, seq(-0x2c, i3  => ('s')), fm28(v1, f14, f13, f14, globalState), v3, v3, v3, v3, seq(0x117, i4  => (DC7(v1, 'x', f14).cf14)), v3, v3, seq(0xaa, i5  => ('j')), v3[i2 := v7], seq(0x201, i6  => (v7)), v3, v3, v3[i2 := v7], v3, v3, v3];
			var v9: map<array<string>, int> := map[v8 := v1];
			v9 := v9[v8 := |map[p0 := i2]|];
			var v10 := new C1(v3[|[p0]| := v7] + v3, |v3| / v1);
			v0[964] := p0;
		...
		v0[355] := f13;
		globalState.f5, v14, v0[355], globalState.f8, globalState.f8 := f14, v14, f14, |fm37(v3, globalState)|, v1;
		var v15 := new C1(v3, 557 % v1);
		if (f14) {
			...
			v19 := v19[DC32(seq(0x253, i7  => ('x'))).cf65 := v15.f28];
			...
			if (v0[355]) {
				...
				var v30: map<int, map<int, D7>> := map[0x172 % fm25(if (f14 in v27) then v27[f14] else v1, v2, p0, globalState) := v29];
				v30 := v30[-v15.f28 := v29];
				...
				globalState.f5 := (v0[355] <== fm0(v15.f28, (v35.(cf60 := p0, cf57 := f13)).cf59, globalState)) || f14;
			}
			
			var v36 := 'x';
			var v37: seq<bool> := [f13];
			var v38: multiset<bool> := multiset{v0[355], f13};
			v0[355], v0, globalState.f5 := if (DC7(v1, v15.f27[v15.f28], f14).cf15) then fm9(v36, globalState) else !(v2 >= v2), v0, multiset(v37) > v38;
			globalState.f7 := if (p1[v15.f28] <= v1) then v15.f28 else v1;
		}
		
		r0 := v0;
		r1 := (v1 + v15.f28) / v15.f28;
	}
	method m13(globalState: GlobalState) returns (r0: int, r1: int) {
		...
		if (!(v3 >= v3)) {
			...
			var v6 := 807;
			var v7: map<bool, multiset<int>> := map[if (f13 in v5) then v5[f13] else false := (multiset([|fm28(-0x21f, f14, f14, f13, globalState)|, v6, v6]))[v6 := 0x9]];
			r1 := |v7|;
			var v8: seq<int> := [v6];
			globalState.f7 := v6 % |v8|;
			var v9 := DC0(f13);
			var v10 := DC4(v9);
			var v11: array<seq<seq<int>>> := new seq<seq<int>>[2];
			v11[857] := seq(10, i1  => (v8));
			var v12: seq<seq<int>> := [v8];
			var v13: map<bool, int> := map[f14 := v6];
			var v14 := DC8(v6, !f13, -v6, |v13|, v6);
			var v15: set<map<bool, int>> := {v13, v13, v13};
			v10, v11[857], globalState.f5, r1, f13 := v10, v12, v14.cf17, fm25(v6, {f13, f14}, f14, globalState) + v6, !(v15 >= (v15 + v15));
		} else {
			var v16 := -0x1f6;
			var v17 := DC27(v16 % v16, !f13, !f14);
			var v18 := 'x';
			var v19 := "fajq";
			v17 := fm38(seq(0x268, i2  => ('t')), v18 in v19, globalState);
			var v20: map<char, bool> := map[v18 := false];
			v2[736] := !(if (v18 in v20) then v20[v18] else f13);
			v2[736] := f14;
			var v21: map<int, string> := map[v16 := seq(0x118, i3  => ('c'))];
			var v22: array<bool> := new bool[6](i4 => v2[736]);
			...
			v2[736] := multiset(v26) !! (multiset{v25[394], v16, v16, |v19|})[v25[394] := v25[394]];
		}
		...
		r1 := |v32[|(if (f14) then v36 else v36)| := v31]|;
	}
}
...
class C3 extends T2 {
	...
	method m5(p0: int, globalState: GlobalState) returns (r0: seq<int>, r1: set<seq<char>>, r2: bool) {
		if (f14) {
			...
			v9, f15[969], v10[428], f13, v17 := v9, !f32, v16, f13, (v17 + [f13, f13]) + [f13, v12.f14];
			var v18: array<bool> := new bool[16](i1 => f15[969]);
			v9[90] := v8;
			var v19: map<int, array<bool>> := map[p0 := v18];
			var v20: map<int, bool> := map[p0 := f14];
			v18, v9[90], globalState.f8 := if (v12.f13) then v18 else if (p0 in v19) then v19[p0] else v18, v8, (if (p0 in v13) then v13[p0] else |v20|) * p0;
			v11[428] := |(if (!false) then v17[p0 := v12.f14] else v17)|;
			var v21: array<array<bool>> := new array<bool>[21];
			v21[4] := v18;
			var v22 := 'd';
			var v23: map<char, bool> := map[v22 := f13];
			var v24 := DC40(|v23|, v12.f13, p0, f14, p0 != p0);
			v11[428], f15[969], v21[4], globalState.f7, v24 := p0 % (p0 - p0), if (false) then v12.f14 else v12.f13, if (p0 in v19) then v19[p0] else v18, p0, v24;
		...
		f15[606] := fm0(p0, false, globalState);
		var v31: set<int> := {p0};
		var v32: seq<bool> := [f32, v31 > v31];
		f15[606] := v32[-p0];
		if (f13) {
			globalState.f8 := p0 - (|v27| / 0x2a2);
			...
			var v37 := m16(v35, fm50([f15[606], f13], globalState) + [|v36|], v35.f27, globalState);
			var v38 := new C2(f13, f32);
			f15[606] := f15[606];
		} else {
			globalState.f5 := f13;
			var v39: array<int> := new int[26];
			v39[348] := |(seq(0x2fd, i3  => (p0)))|;
			v39[348] := p0;
			var v40, v41 := m17(f32, v39[348], globalState);
			globalState.f7 := 0x1fc;
			var v42 := DC12(fm0(-p0, !f32, globalState), f32, f15[606]);
			var v43: array<string> := new string[4] [v27 + v27, v27, v27, v27 + v27];
			v43[341] := v27;
			v42, f15[606], v43[341] := v42, f32, "roit";
		}
		...
		if (p0 == p0) {
			...
			var v47: multiset<int> := multiset{|"dttfcj"| / -p0, p0, p0, |v45|, -|v46|};
			...
			v48[834] := p0;
			v48[834] := p0 + (p0 * fm10(p0, f32, globalState));
			var v50: set<bool> := {f32};
			var v51: multiset<bool> := multiset{v32[if (|v50| in v47) then v47[|v50|] else v48[834]] !in v32};
			v51 := v51;
		} else {
			globalState.f5 := f13;
			var v52 := new C2(f15[606], f32);
			globalState.f7 := p0 - p0;
			f13 := |(v31 * v31)| == (p0 - p0);
			var v53: seq<int> := [484, -0x11, p0, p0];
			var v55: multiset<int> := multiset{|v27|, p0};
			var v56: array<int> := new int[29](i5 => i5 / p0);
			...
			var v63 := DC54(v28, DC36(!f15[606], v56, p0));
			var v64: map<bool, D16> := map[!true := v63];
			v62[732] := |v64|;
			var v65: array<multiset<int>> := new multiset<int>[18];
			v65[849] := multiset(v53);
			v62[732], v65[849] := if (f14) then fm10(p0, true, globalState) else p0, if (f14) then v55 else multiset{p0} - v55;
		}
		
		var v66: map<int, bool> := map[p0 := f14];
		r0 := fm50([if (fm10(p0, f15[606], globalState) in v66) then v66[fm10(p0, f15[606], globalState)] else f14], globalState);
	...
	method m6(p0: bool, globalState: GlobalState) {
		...
		if (f14) {
			...
			globalState.f8, v11, v12[917], v17[227], globalState.f7 := -v2.f28 - v2.f28, ((set v20 : map<int, int> | v20 in v19 :: (v20)) - {v18, v18, v18, v18}) - v11, -(v2.f28 / v1), v13.(cf72 := v12).(cf71 := |v2.f27|), v2.f28 % v1;
			globalState.f8 := v2.f28;
			v12[917] := if (f14) then |v9| else v2.f28;
			...
			v22 := map[|v9| := v25.cf46];
			v4 := v4[p0 <== f32 := v2.f27[v12[917] := v3] <= (seq(0x200, i2  => (v3)))];
		} else {
			globalState.f5 := (f13 ==> f13) <==> (if (f32) then f13 else fm9(v3, globalState));
			f13 := f13;
			var v26: seq<bool> := [f14];
			var v27: seq<seq<bool>> := [v26];
			var v28: multiset<int> := multiset{|v27[v1]|, v2.f28};
			var v29: map<bool, char> := map[f14 := v3];
			globalState.f7 := (|v0[|v28| := if (v5 in v29) then v29[v5] else v3]| - |v0|) % v1;
			var v30: seq<int> := [v1];
			var v31 := DC10(v30[v2.f28 := v2.f28]);
			var v32: set<bool> := {f32, v5};
			var v35: set<int> := {|v0|};
			globalState.f8, v31, globalState.f5, v5, globalState.f5 := |v0| / |v32|, v31, f14, if (p0 in v4) then v4[p0] else (set v33 : int | v33 in map[v2.f28 := seq(-451, i3  => ('r'))] :: (v33 - -|(set v34 : D11 | v34 in [DC40(|[multiset{0x192}, multiset{|map[!true := 0x9a]|, |[[-0x179, 345, 0xc1, |"hydfq"|, -0x305]]|}, multiset{|map[-0x367 := false]|}, multiset([|map[true := true]|])]|, true, -0x36d, true, false)] :: (v34))|)) > v35, v2.f27 == v0;
			v8[930] := p0;
			v8[930] := f32;
		}
		...
		v36[993] := v2.f27;
		v36[993] := v0;
		var v37: multiset<string> := multiset{v36[993]};
	...
	method m3(p0: D0, p1: seq<bool>, p2: int, p3: bool, globalState: GlobalState) returns (r0: seq<set<bool>>, r1: bool) {
		...
		if (v0) {
			...
			v2 := [v2[|{f14}|], f15, f15, f15];
			r1 := f32;
			f32 := f14 && (p1 < p1);
		} else {
			var v3: array<int> := new int[8](i0 => i0 % 434);
			v3[559] := v1;
			v3[559] := (fm10(-v1, f32, globalState) - |multiset(p1)|) + -v1;
			var v4 := DC0(v0);
			match (v4) {
				case DC1(cf1, cf2, cf3, cf4) =>
					var v5: set<int> := {v1, fm10(v3[559], fm0(v1, f32, globalState), globalState), v3[559], v1};
					var v6 := DC36(f13, v3, |v5|);
					var v7: seq<array<int>> := [v6.cf74];
					v3 := v7[v3[559]];
					var v8 := "otgs";
					var v9 := new C1(v8 + v8[v3[559] := fm2(cf2, globalState)], v3[559]);
					globalState.f7 := v9.f28 + v9.f28;
					var v10 := DC48(fm54(!f13, v3[559], globalState));
					v10 := DC48(map[p2 := p3]);
				case DC2(cf5, cf6, cf7) =>
					globalState.f5 := false;
					var v12: array<set<int>> := new set<int>[24](i1 => (set v11 : int | v11 in multiset{|[v1, cf5]|} :: (v11 / |map[!true := false]|)) - {p2, v3[559]});
					var v13 := DC55(v12);
					...
					v19 := (v19 * v19)[v18 := -|v21[-v3[559]]|];
				case DC3(cf8, cf9) =>
					globalState.f8 := cf8;
					var v22 := 'v';
					var v23 := DC5(map[v3[559] := v22]);
					var v24 := DC16(v3, |(seq(0x306, i2  => ('l')))|, v23);
				...
				case DC0(cf0) =>
					...
					var v31 := "yimtrdef";
					var v32: set<int> := {|v31|};
					f15[727] := multiset(v30 + v30) !! multiset{v32, v32};
					var v33: map<bool, bool> := map[cf0 := f13];
					var v34: set<map<bool, bool>> := {v33, map[f13 := v0], v33};
					var v35: multiset<bool> := multiset{f15[727], f32, v34 !! v34};
					...
					var v39: map<int, array<char>> := map[v3[559] := v38];
					f15[727], globalState.f5, v35, f15[727] := v36 in v37, if ((v39 != v39) in v33) then v33[v39 != v39] else false, v35 - v35, p3;
					var v40 := 't';
					v40, v1 := v40, v1;
				...
				case DC4(cf10) =>
					...
					v0 := p1[if (p3 in v43) then v43[p3] else v3[559]];
					globalState.f8 := v1 % |{p3}|;
					globalState.f7 := v3[559] % p2;
			}
			
			var v44 := new C0(v0, !!true);
			var v45 := "cimc";
			var v46 := DC8(p2, v44.f30, v3[559], |map[f32 := v1]|, v3[559]);
	...
	method m4(p0: bool, p1: seq<int>, globalState: GlobalState) returns (r0: array<bool>, r1: int) {
			...
			decreases 100 - i0
		{
			...
			if (f14) {
				...
				var v7: array<D8> := new D8[17] [v5, DC30(v4).(cf61 := {v3, v3}), v5, v5, v5, v5, v5, v5, if (v6[v0]) then v5.(cf61 := {v3, v3}) else fm55(map[v0 := |("oipnwvo")[-418 := v1]|], v0, f14, globalState), v5, v5, DC30(v4), v5, v5, v5, v5, v5];
				v7[48] := v5;
				var v8: set<bool> := {false, f14};
				f32, v7[48] := (|v8| / v0) == v0, v5.(cf61 := v4 * v4);
				...
				v12[62] := v14;
				v10, v11, globalState.f5, v12[62] := v10, v11 + v11, v13[fm10(v0, f14, globalState)], v14;
				var v15: array<int> := new int[27];
				v15 := v15;
				var v16: set<bool> := {f13, v13[v0]};
				globalState.f5 := (v16 !! v16) || f14;
				var v17: seq<char> := [v1];
				var v18: set<array<int>> := {v15};
				var v19 := DC23(v18, v1, v1);
				var v20: array<char> := new char[9] [v1, v1, v17[v0], v1, v1, v1, v1, v19.cf43, v1];
			...
			var v44: array<set<D15>> := new set<D15>[24] [DC58(v42.cf108).cf108, v41, v41, v41, v41 - v41, v41 * v41, v41, v41, v41, v41, v41, v41, v41, {v40, v40}, v41 + v41, v41, v43[v0], v41 * v41, v41 + v41, v41 - v41, v41, v41 * v41, v41, v41];
			v44[480] := v41;
			v44[480] := v41;
			if (p0) {
				f15[199] := !f14;
				var v45 := "ewtmpgn";
				f13, f15[199], v0 := f14, f32, |v45|;
				v1 := v1;
				...
				v46[780] := v48.(cf23 := v0, cf25 := p1[|v49|]);
				v46[780] := v48;
				globalState.f7 := 642;
				var v50: set<bool> := {p0};
				...
				v0 := if (v60 in v61) then v61[v60] else |(if (v1 in v62) then v62[v1] else [0xee, v0])| / v0;
				v0 := |((v59 + v59) + v59)|;
			}
	...
	method m16(p0: C1, p1: seq<int>, p2: string, globalState: GlobalState) returns (r0: bool) {
		...
		if (v3 == multiset{p0.f28, p0.f28, p0.f28}) {
			...
			v6 := multiset{false, v2[p0.f28], p0.f28 != p0.fm26(v7, v9, globalState)};
			...
			if ((p0.f28 + |"jmhqfh"|) > (p0.f28 % p0.f28)) {
				...
				v20 := v20[p0.f28 / p0.f28 := v19];
				var v21: array<bool> := new bool[13];
				var v22: seq<array<bool>> := [f15, v21, f15];
				globalState.f5, v21, globalState.f5, globalState.f7, globalState.f5 := f14 || f14, v22[--0xbd - p0.f28], true, -0x2fd, !true;
			} else {
				var v23: set<seq<int>> := {p1};
				globalState.f8, v23, globalState.f7, globalState.f5, globalState.f7 := |p0.f27|, v23, |(p0.f27 + "kux")| % 0x372, f13, -(|multiset(p1)| % (if (f14) then -p0.f28 else p0.f28));
				...
				v25, globalState.f7, v26, v27[241], globalState.f7 := v25, p0.f28, (seq(-0x382, i0  => ('j'))) + [v14], p0.fm27(fm8(--p1[p0.f28], globalState), p0.f28 == -p0.f28, globalState), -v28.cf82 - p0.f28;
				var v29: array<array<T1>> := new array<T1>[20];
				var v30: T1 := new C2(f13, f14);
				var v31: map<bool, T1> := map[f32 := v30];
				var v32: array<T1> := new T1[18] [v30, if (v27[241][p0.f28] in v31) then v31[v27[241][p0.f28]] else v30, v30, v30, v30, v30, v30, v30, v30, v30, v30, v30, v30, v30, v30, v30, v30, v30];
				v29[590] := v32;
				v29[590] := v32;
			...
			var v34, v35 := m17(f14, |v11| / p0.f28, globalState);
		...
		if (true) {
			...
			globalState.f7 := p0.f28 + (|(map v45 : char | v45 in v47 :: (v45) := (f14))| % p0.f28);
			...
			if (v48.cf63) {
				...
				var v52: array<int> := new int[11](i2 => i2 % |p1|);
				...
				f15[33] := v54 !! v54;
				var v55: map<bool, int> := map[f15[33] := -|(seq(719, i3  => (v46)))|];
				var v56: map<string, map<bool, int>> := map[p0.f27 := v55];
				var v57: set<bool> := {f15[33]};
				v56 := v56[fm51(v57, |v3|, globalState) := v55];
				var v58: map<bool, bool> := map[f13 := f13];
				globalState.f5 := ({|[f15[33], f15[33]]|, p0.fm26(v46, v58, globalState)} * {-854, |v58|}) !! v54;
			...
			v60 := v60;
			var v61 := new C1(p0.f27 + p0.f27, if (f13) then p1[p0.f28] else p0.f28);
		} else {
			var v62 := 'j';
			var v63: array<int> := new int[17](i5 => i5 * p0.f28);
			var v64: map<bool, string> := map[true := p0.f27];
			v63[250] := -|(if (f14 in v64) then v64[f14] else p2)|;
			var v65: map<int, bool> := map[|p1| := f13];
			var v66: map<map<int, bool>, string> := map[v65 := p2];
			v62, r0, v63[250] := v62, v62 in (if (map[0x92 := f13] in v66) then v66[map[0x92 := f13]] else p2), p0.f28;
			if (f14 || f13) {
				v65 := (v65 + v65) + v65;
				var v67: multiset<seq<int>> := multiset{(p1 + p1)[99 := p0.f28]};
				globalState.f8 := |v67|;
				...
				globalState.f5 := v2[|v70|];
				globalState.f5 := f32;
			} else {
				var v71 := DC7(v63[250], v62, false);
				var v72: array<bool> := new bool[25] [if (f32) then f14 else true, f32, f14, p0.f28 != 0x44, f32, f32 <==> f13, fm9(v62, globalState), v71.cf15, f13, f14 <== f32, true, fm9(v62, globalState), fm0(|multiset{v63[250], |v3|}|, f13, globalState), true, if (true) then f14 else false, f14, f13, f32, f14, fm9(v62, globalState) || false, f13, f32, -|v2| == v63[250], f13, f13];
				var v73: map<int, int> := map[v63[250] := v63[250]];
				...
				v72 := new bool[28] [v3 == multiset(p1), f13, f13 <== f32, !fm0(p0.f28, f13, globalState) && f32, f14, (seq(-0x1a9, i6  => (p2[v63[250]])))[0x3ca := v62] <= "wos", if (f32) then fm0(|v73|, true, globalState) else !false, f13 <== f14, f14, f13, f14, f32, fm0(if (f13 in v74) then v74[f13] else v63[250], f14, globalState), f13, f13, if (f13) then f32 else false, f32, f32, f13, f14, fm0(p0.f28, f13, globalState), f13, !f13 in v2, (if (p0.f28 in v75) then v75[p0.f28] else p0.f27) < p2, fm0(p0.f28, !!f32, globalState), if (v62 in v76) then v76[v62] else f32, f32, !f14];
				var v77 := "jqr";
				v72[651] := p0.f28 != |p2[|p0.f27| := v62]|;
				var v78: map<char, char> := map[v62 := v62];
				var v79 := DC8(p0.f28, f14, v63[250], p0.f28, v63[250]);
				v77, f13, globalState.f8, v72[651] := p0.f27, (if (!fm0(-137, f13, globalState)) then p0.f28 else p0.f28) <= |v78|, --548, fm9(v62, globalState) || fm0(v79.cf16, fm0(v63[250], f13, globalState), globalState);
				v36 := v36;
				v63[250] := p0.f28;
			...
			globalState.f5, v63[250], globalState.f8 := f14, v63[250], v81.cf58;
			v62, v63[250] := v62, 572;
		}
		
		f15[247] := f13;
		f15[247] := if (f14) then f14 else f14;
		var v82: array<bool> := new bool[10] [f15[247], f15[247], f32, f15[247], f13, fm0(0x2f2, true, globalState), f15[247], f15[247], f13, f14];
	...
	method m17(p0: bool, p1: int, globalState: GlobalState) returns (r0: bool, r1: int) {
		...
		v7[793] := (v6 + (if (|map[0x6a := f32]| in v9) then v9[|map[0x6a := f32]|] else map[v3 := true])) + v6;
		...
		match (v11) {
			case DC50() =>
				...
				if (f32) {
					...
					var v19 := DC60(v18);
					var v20: array<multiset<D6>> := new multiset<D6>[13] [fm60(p1, fm0(p1, v14.f30, globalState), if (p0 in v16) then v16[p0] else v15, p1, globalState), multiset{v17}, v18, v18, v18, multiset{v17, v17}, v18, v18, v18, v19.cf114 * v18, v18, multiset{v17}, v18[v17 := |v12|]];
					var v22 := DC40(|(map v21 : int | v21 in v2 :: (v21 + p1) := (false))|, v14.f29, p1, f32, f32);
					var v23: seq<bool> := [p0, true, f32];
					v20[512] := multiset{fm61(fm10(v22.cf80, p0, globalState), p1, f13, |v23|, globalState)};
					var v24: seq<seq<bool>> := [[v14.f30], v23];
					v20[512] := (multiset{v17, DC22([p0])})[DC22([p0]).(cf40 := v23) := |(v24 + v24)|];
					v3[416] := p1;
					globalState.f7, v14.f29, globalState.f5, v15, v3[416] := p1 * -|[v12]|, false, f32, 'p', -fm10(p1, !fm9(v15, globalState), globalState);
			...
			case DC51(cf96, cf97, cf98, cf99, cf100) =>
				...
				if (v31 !! multiset{p0}) {
					...
					var v35: map<int, map<int, bool>> := map[v33[cf99] := v34];
					var v36 := DC44(v35);
					...
					var v45 := DC25(v44);
					f15[977] := f32;
					v45, cf100, cf100, f15[977] := v45, (cf99 - 0x2cd) / cf100, |v33|, f14;
				} else {
					cf100 := -p1;
					...
					var v51: array<string> := new string[17](i2 => v48[-cf96 := 'b']);
					v51[364] := v48;
					v51[364] := v48;
					globalState.f8 := (p1 * cf96) % p1;
				}
				
				var v52: map<int, int> := map[cf100 := 0x274];
				var v53: seq<map<int, int>> := [v52];
				var v54: map<int, bool> := map[|v53| := p0];
				var v55 := DC8(cf100 / -p1, p0, cf99, cf100 + |v54|, p1);
				match (v55) {
					case DC7(cf13, cf14, cf15) =>
						...
						var v60 := DC15(fm0(0x1a6, fm0(v57[cf96], f13, globalState), globalState), v59);
						var v61 := "mmmo";
						v56 := v56[v60.cf31 := v61[-cf99]];
						...
						var v66: array<seq<bool>> := new seq<bool>[24] [[cf98, cf15, true, p0], (v62 + [cf15])[fm10(|(seq(0x10, i3  => (cf14)))|, f32, globalState) := f32], [p0] + v62, v62, fm56(|multiset(v62)|, globalState), v62, v62, v62, v63[cf100] + v62, [f14], [cf15], v62, v62, v64.cf64, v62, v62, v62, v62 + (if (false in v65) then v65[false] else [f32, cf98]), v62 + v62, (v63[cf100])[cf13 := fm0(cf13, cf98, globalState)], [f13, true], v62, v62, v62];
						v66[276] := [cf98];
						v66[276] := [f14, f14] + (v64.cf64 + [false, cf15]);
						cf13 := p1;
						cf14 := cf14;
					case DC8(cf16, cf17, cf18, cf19, cf20) =>
						...
						var v73: array<string> := new seq<char>[29] [(seq(763, i4  => (v67))) + v70[cf100 := v67], v70, if (!f13) then "iawcbya" else "ja", v70 + v70, fm51(v0, cf96, globalState), (seq(-0x313, i5  => (v67))) + "in", v70 + v70, seq(-0x311, i6  => (v67)), seq(-490, i7  => (v67)), v70 + v70, if (cf17 in v72) then v72[cf17] else v70, "s", v70, v70, v70, v70 + v70, v70, v70, if (f32) then v70 else if (p0 in v72) then v72[p0] else seq(0xe0, i8  => (v67)), v70, seq(0x2e2, i9  => (v67)), v70, "neutdkdx", seq(0x1ad, i10  => ('v')), v70, v70, v70, seq(0x308, i11  => (v67)), v70[-cf20 := v67]];
						v73[657] := v70;
						f15[633], cf99, r1, v71[382], v73[657] := p0, p1, cf20, v68, "vfpbudcke";
						var v74: seq<set<bool>> := [v0];
						f15[633] := cf17 || (v74[cf18] != v0);
						v3[805] := cf96;
						v3[805], f15[633] := fm10(cf20 + cf20, cf17 ==> cf17, globalState), f14;
					case DC6(cf12) =>
						globalState.f8 := p1;
					...
					case DC9(cf21) =>
						...
						v77 := v77 + v77[cf100 := cf96];
						var v78 := 'x';
						var v79: map<bool, string> := map[false := ("pimoc")[0x39a := v78]];
						var v80 := "ovsmkbbmh";
				...
				v85 := v85[v87[p1] := (map v88 : int | (-953 <= v88) && (v88 < 0x17c) :: (v88 - cf100) := (-|v2|)) + (map v89 : int | (515 <= v89) && (v89 < 771) :: (v89 - cf96) := (cf96))];
				globalState.f5 := cf98;
			case DC49(cf95) =>
				var v90: array<string> := new seq<char>[17](i12 => (seq(0x205, i13  => ('b'))) + (seq(0x26f, i14  => ('a'))));
				v90[4] := seq(-0x58, i15  => ('o'));
				v90[4] := "gshqobx";
				if (p0) {
					var v92 := DC57(p1);
					globalState.f8 := -(|(map v91 : int | (0x394 <= v91) && (v91 < 974) :: (v91 * p1) := (true))| / v92.cf107);
					f15[538] := v0 !! v0;
					var v93: seq<bool> := [f13, f32, f32];
					v90[4], f15[538] := "wbcrr", v93[|(v90[4] + v90[4])|];
					var v94 := new C0(f14, fm0(|v90[4]|, true, globalState));
					globalState.f5 := v93[p1];
					var v95 := 'y';
					var v96, v97 := m0(v1, fm9(v95, globalState), globalState);
				} else {
					var v98: multiset<bool> := multiset{f13};
					v98 := v98[f32 := p1] * multiset{f14, f32, p0, false};
					var v99: map<int, array<bool>> := map[p1 := f15];
					var v100 := DC15(f14, v99);
					r1 := fm10(|v90[4]|, v100.cf31, globalState);
					globalState.f7 := |(v90[4] + v90[4])|;
					var v101: seq<int> := [p1, |v90[4]|, p1, p1, p1];
					...
					r0 := if (true) then p0 else if (v104.cf109 in v103) then v103[v104.cf109] else !v105[|v105|];
				}
				
				if (p0) {
					var v106 := new C1(v90[4], p1);
					...
					var v116 := DC7(if (v113 in v114) then v114[v113] else p1, v112, v90[4][p1] !in v115);
					...
					v119 := v119[f14 := v118[|fm51(v0, p1, globalState)| := p1]];
					var v120: array<map<bool, int>> := new map<bool, int>[1](i16 => map[!p0 := p1]);
					f13, globalState.f7, v120, r1, globalState.f7 := f13, -p1, v120, p1 + |v4|, p1 / p1;
					v90[4] := v90[4];
					var v121 := DC5(v113);
			...
			case DC52(cf101) =>
				...
				v133 := v133[p1 % |v2| := |v133|];
				var v134 := 'y';
				v130 := v130[p1 := v134] + (v130 + v130);
		}
		...
		v138[889] := new array<bool>[19] [f15, f15, f15, f15, f15, f15, if (p0) then v139[p1] else f15, f15, f15, v139[p1], f15, f15, f15, f15, f15, f15, f15, f15, f15];
...
class C4 extends T2 {
	...
	function fm8(p0: int, globalState: GlobalState): char {
		if ((set v1 : char | v1 in (set v0 : char | v0 in multiset(seq(387, i0  => ('r'))) :: (v0)) :: (v1)) > {'l', 'i'}) then 'l' else 'o'
	}
	function fm9(p0: char, globalState: GlobalState): bool {
		'c' !in "lnlfgx"
	}
	...
	method m5(p0: int, globalState: GlobalState) returns (r0: seq<int>, r1: set<seq<char>>, r2: bool) {
		...
		v1[405] := p0;
		v0, f15[916], globalState.f7, v1[405] := v0, f31, |[v1]|, p0 + p0;
		for i1 := 629 to p0 {
			var v2 := new C1(seq(0x62, i2  => ('c')), v1[405]);
			globalState.f7 := p0;
			var v3 := DC19();
			var v4: map<D5, D7> := map[DC20(v3) := DC29(false, v2.f28, f31, f15[916]).(cf60 := true, cf57 := f31)];
			var v5 := DC20(v3);
			var v6 := DC29(f14, |[f14]|, f13, true);
			v4 := v4[v5 := v6];
			var v7 := 'i';
			var v8: seq<bool> := [f31, f14];
			var v9: map<char, bool> := map[v7 := v8[v2.f28]];
			v9 := map[v7 := false] + v9;
		}
		...
		var i4 := 0;
		while ((DC31(0x3d6, f13, v11).(cf62 := v1[405])).cf63)
			decreases 100 - i4
		{
			if (i4 >= 100) {
				break;
			...
			i4 := i4 + 1;
			var v12: seq<int> := [v1[405] * -0x324];
			r0 := v12;
			if (f31) {
				var v13: array<char> := new char[29];
				var v14 := 'y';
				v13[833] := v14;
				v13[833] := fm2(f31, globalState);
				var v15: map<array<int>, bool> := map[v1 := false];
				var v16: map<bool, bool> := map[f15[916] := f31];
				...
				f15[916], r0, globalState.f7, v1[405], v1[405] := f31, v12, |v18|, v1[405] % p0, p0;
				globalState.f7 := v1[405] - -0x1ff;
				var v19: array<seq<bool>> := new seq<bool>[25];
				var v20 := DC37(p0, f15[916]);
				var v21: map<int, bool> := map[v1[405] := f14];
				v19, v20, r2 := v19, v20, if (false) then !(if (|v12| in v21) then v21[|v12|] else f13) else if (false) then f13 else f14;
			} else {
				r2 := f15[916];
				globalState.f7 := p0;
				r0 := v12 + v12;
				var v22: set<bool> := {f15[916]};
				var v23 := DC27(|v11|, !f13, v22 != {f31, f15[916], f14, f15[916], f13});
				var v24 := 'y';
				var v25: map<char, int> := map[v24 := v1[405]];
				v23 := DC27(p0, |v25| < v1[405], f15[916]);
				var v26: map<string, char> := map[v0 := v24];
				globalState.f7 := -|v26|;
			}
			
			var v27 := 'y';
			var v28: map<int, int> := map[v1[405] := p0];
			var v29 := DC7(p0, v27, !false);
			var v30: array<char> := new char[29] ['u', v27, 'f', v27, v27, v27, fm2(fm0(if (p0 in v28) then v28[p0] else p0, f15[916], globalState), globalState), v27, v27, v27, fm8(v1[405], globalState), 's', v27, v27, v27, v27, fm2(f14, globalState), fm2(f13, globalState), v27, v27, 'd', v27, v29.cf14, v27, v27, v27, v27, v27, v27];
			v30 := new char[25](i5 => v27);
			var v31 := new C2(f13, v1[405] >= v1[405]);
		}
		var v32: set<int> := {fm10(p0, f15[916], globalState)};
		var i6 := 0;
		while (p0 in v32)
			decreases 100 - i6
		{
			...
			i6 := i6 + 1;
			globalState.f5 := v11[p0];
			var v33 := 'f';
			var v34: map<bool, bool> := map[f13 := f14];
			v1[405], v33, v34, v11, v0 := p0, v33, v34, v11, ("gs" + v0) + (v0 + v0);
			match (DC12(v33 in v0, f31 <==> f13, fm0(p0, f31, globalState))) {
				case DC11(cf23, cf24, cf25) =>
					...
					var v40 := DC11(cf23, f15[916], cf23);
					var v41: array<bool> := new bool[23] [if (v11[cf25]) then fm9(v33, globalState) else cf24, f15[916], true, if (f31) then fm0(cf23, cf24, globalState) else cf24, f14, false, v36.cf45, f31, false, f14, !f13 ==> f15[916], f13, 0x24 < cf25, f31, f15[916], (if (|v39| in v38) then v38[|v39|] else v37) < v37, !cf24, f14 ==> f13, f14, cf24, v40.cf24, false, fm0(cf23, !f31, globalState)];
					v41 := v41;
					var v42: map<array<bool>, bool> := map[v41 := f15[916]];
					f15[916] := if (v41 in v42) then v42[v41] else f15[916];
					v34 := if ({v1[405], 0x9f} !! v32) then v34 + v34 else v34;
				case DC12(cf26, cf27, cf28) =>
					f13 := f31;
					f15[916] := f31;
					var v43: map<int, int> := map[p0 := 0x23e];
					var v44: set<bool> := {cf27, f13};
					var v45: multiset<int> := multiset{p0, if (|v44| in v43) then v43[|v44|] else p0, v1[405]};
					globalState.f7, globalState.f7 := |v45| * p0, v1[405] % p0;
					var v46: map<bool, int> := map[false := p0];
					v46 := v46[v0 <= v0 := v1[405]];
				case DC10(cf22) =>
					v33 := v33;
					v0 := v0;
					globalState.f5, r2, f13 := if (f14 && f15[916]) then f31 else !f15[916], f13, f13;
					var v47 := DC11(|v0|, f31, v1[405]);
					var v48: map<int, int> := map[fm10(|v0|, f13, globalState) := v1[405]];
					globalState.f5, v33, f13, f15[916], globalState.f5 := !(p0 != v1[405]), v33, !f14 <== f14, v47.cf23 <= (if (v1[405] in v48) then v48[v1[405]] else -p0), fm0(p0, f31, globalState);
				case DC13(cf29) =>
					var v49: seq<int> := [v1[405]];
					v32, globalState.f5 := v32 - (set v50 : int | v50 in v49 :: (v50 - -134)), f14;
					globalState.f8 := if (f15[916]) then v1[405] else 642;
					var v51: multiset<array<int>> := multiset{v1, v1};
					globalState.f7, v51, globalState.f8 := -((|v34| / |fm43(|(seq(134, i7  => (p0)))|, globalState)|) / -|v0|), (v51 + v51) - v51, p0 + |"nxrni"|;
					var v52: set<bool> := {f15[916], f13};
					f15[916] := v52 >= v52;
			}
			...
			if (f31) {
				f15[916] := f15[916];
				f15[916] := f13;
				globalState.f5 := fm0(v1[405], f31, globalState);
				var v53 := new C1(v0, fm10(p0, f31, globalState));
				v11, globalState.f5, globalState.f8, v33 := v11, !(f15[916] ==> f13), p0 % v53.f28, if (f14) then v33 else v33;
			} else {
				var v55: multiset<int> := multiset{-p0, p0, -p0, p0};
				f15[916] := (set v54 : int | (-0x4 <= v54) && (v54 < 0x2f7) :: (v54 % p0)) > (set v56 : int | v56 in v55 :: (v56 * |"dmrfa"|));
				globalState.f8 := p0;
				v0 := v0[v1[405] := v33];
				var v57 := new C2(true, f14);
				globalState.f7 := v1[405];
			}
			
		}
		var v58: map<int, string> := map[p0 := v0];
		v58 := v58[if (f31) then v1[405] else |v0| := v0];
		r0 := [p0];
	...
	method m6(p0: bool, globalState: GlobalState) {
		...
		if (f31) {
			...
			v5 := fm44(f14, f14, v2, v5[v0], globalState) + v5;
			if (f31) {
				f13 := f13;
				var v6: array<set<D6>> := new set<D6>[2];
				var v7: seq<array<set<D6>>> := [v6, v6, v6, v6, v6];
				v6 := v7[v0 * fm10(v0, p0, globalState)];
				var v8: seq<bool> := [!(v5[v0 := |"bppbk"|] != v5)];
				var v9 := DC41();
				var v10: map<D11, seq<bool>> := map[v9 := v8];
				v8 := if (v9 in v10) then v10[v9] else v8;
				v1 := v1[v4 < v4 := |v4| != v0];
				var v11: set<bool> := {f31};
				var v12: set<seq<int>> := {v5[v0 := -fm10(v0, false, globalState)], v5};
				var v13 := DC3(v0, !false);
				var v14: array<int> := new int[10] [|multiset{f13, p0, f14}| - v0, v0, fm10(v0, !f13, globalState), |v11| % v0, |v12|, v0, -v0, v13.cf8, |v1|, fm10(v0, f14, globalState)];
				v14 := v14;
			} else {
				var v15: array<array<int>> := new array<int>[27];
				var v16: array<int> := new int[17];
				v15[867] := v16;
				f13, v15[867] := 'a' !in v4, v16;
				globalState.f8 := fm10(v0 % v5[|v1|], f14 <==> f13, globalState);
			...
			f13 := f13;
			var v24 := "unkxwrtd";
			globalState.f5 := (seq(14, i0  => (v2)))[v0 := v2] != v24;
		...
		var v27 := DC27(|(seq(-605, i1  => (v2)))|, f14, -0xad != v0);
		...
		var v33: seq<int> := [v0];
		var v34: multiset<seq<int>> := multiset{v33[v0 := v0], seq(0x3b6, i2  => (v0)), v33};
		var v35: multiset<bool> := multiset{!v28.f30};
		globalState.f7, f15[300], v31, v0, v32 := |(v34 * fm45(v28.f30, globalState))|, f14 !in v35, v31, |v1|, v32 + (if (v28.f29) then map v36 : int | (-776 <= v36) && (v36 < -0x19d) :: (v36 % |(seq(0x3d9, i3  => (v2)))|) := (f31) else v32);
	...
	method m3(p0: D0, p1: seq<bool>, p2: int, p3: bool, globalState: GlobalState) returns (r0: seq<set<bool>>, r1: bool) {
		...
		v1 := v1[p2 + v6[fm10(p2, f13, globalState)] := (v8.(cf13 := p2)).cf14];
		var i0 := 0;
			...
			decreases 100 - i0
		{
			...
			i0 := i0 + 1;
			var v9 := "xrtyw";
			globalState.f7 := (|v9| / 0x250) % 157;
			f15[304] := !fm0(p2, f13, globalState);
			var v10: array<char> := new char[1];
			var v11: multiset<array<char>> := multiset{v10, v10};
			f15[304] := v11 !! v11;
			globalState.f7 := p2;
			var v12: multiset<int> := multiset{p2};
			var v13: array<bool> := new bool[9] [!(fm46(p2, false, f31, 414, globalState) >= v12), if (f14) then f14 else p3, f14, f13, !fm9(v7, globalState), false <== f15[304], !f14, !f31, f15[304]];
			v13 := v13;
		}
		f15[449] := p3;
		f15[449] := p1[p2];
		...
		var v15 := DC8(0x179, multiset(p1) !! multiset{f31, f13}, if (f15[449]) then p2 else if (f31 in v0) then v0[f31] else p2, if (false) then p2 else p2, p2);
		match (v15) {
			case DC7(cf13, cf14, cf15) =>
				cf15 := f15[449];
				var v16 := "u";
				...
				if (v17[fm10(|v18|, cf15, globalState)] < (v16 + v16)) {
					...
					v19[363] := v21 + DC44(map v22 : int | (94 <= v22) && (v22 < 0x396) :: (v22 + |v16|) := (map[p2 := false])).cf89;
					v19[363] := v21 + (v21 + v21);
					globalState.f8 := (cf13 % p2) % cf13;
					var v23: map<bool, char> := map[cf15 := 'x'];
					var v24: array<char> := new char[27] [v7, v7, cf14, if (f31) then v7 else v16[0x2af], fm8(cf13, globalState), cf14, 'g', cf14, 'k', 'r', 'k', fm2(cf15, globalState), v7, 'g', 'v', if (f15[449] in v23) then v23[f15[449]] else v7, cf14, v7, cf14, fm2(f31, globalState), cf14, v7, v7, fm8(cf13, globalState), v7, 'p', v7];
					...
					var v26: array<bool> := new bool[24] [f31, p3, cf15, if (false in v18) then v18[false] else false, f14, false, f15[449], f13, true, f14, cf15, cf15, f13, !f13, false, f31, f13, f31, v25.cf57, p3, cf15, fm9(cf14, globalState), p3, true];
				...
				v14[291] := p2;
				var v32: array<char> := new char[13](i5 => v16[p2]);
				var v33 := DC46(v32);
				v31, v14[291], v32, globalState.f5 := v31, p2 * cf13, v33.cf92, f13;
				globalState.f5 := cf15;
			case DC8(cf16, cf17, cf18, cf19, cf20) =>
				var v34 := new C2(f13, cf17);
				var v35 := "alyxuwoku";
				v35 := v35;
				f15[449] := v34.fm9('g', globalState);
				cf20 := cf16;
			case DC6(cf12) =>
				v6 := v6;
				globalState.f5 := p3;
				if (!(p2 == p2)) {
					...
					var v37: multiset<array<int>> := multiset{v14, v36[p2]};
					f15[449] := v14 !in v37[v14 := |fm47(f15[449], p2, globalState)|];
					f13 := !!fm0(|(seq(519, i6  => (v7)))|, p3, globalState);
					var v38 := DC6(cf12);
					var v39 := DC9(v38);
					var v40: map<int, D2> := map[p2 % p2 := v39];
					globalState.f7, f15[449] := -|v40|, f13;
					var v41 := "uggyclfn";
					globalState.f8 := |v41| * p2;
					var v42: seq<string> := [seq(0x9a, i7  => (v7)), v41, v41, v41, v41];
					var v43: map<string, int> := map[v42[p2] := p2];
					var v44: seq<seq<int>> := [[p2]];
					var v45: seq<seq<int>> := [[p2, p2], v44[fm10(|(seq(477, i8  => (v7)))|, p3, globalState)]];
					v43 := v43["s" := -p2 + |v45[p2]|];
				} else {
					globalState.f5 := f13;
					...
					var v48: map<int, bool> := map[-0x3d8 := f15[449]];
					var v49 := DC48(v48);
					v47[330] := v49.cf94;
					v47[330] := v48 + v48;
					f15[449] := p3 ==> f14;
					var v50: map<array<int>, seq<string>> := map[v14 := seq(262, i9  => ("cmtbwhml"))];
					var v51 := "uac";
			...
			case DC9(cf21) =>
				f15[449], globalState.f7, f13, f15[449], globalState.f5 := !(-|[v8.cf15]| == p2), 0x384, f13, f15[449], f14;
				var v55 := DC3(p2, f13);
				var v56: set<int> := {p2, fm10(p2, !v55.cf9, globalState)};
				f15[449] := (v56 + v56) <= (v56 + v56);
	...
	method m4(p0: bool, p1: seq<int>, globalState: GlobalState) returns (r0: array<bool>, r1: int) {
		...
		var v6: map<bool, bool> := map[v1[v2] := if (v4 in v5) then v5[v4] else f31];
		var v7 := "ysse";
		var v8: seq<string> := [v7];
		v0, globalState.f7, v6, globalState.f5, r1 := v0, p1[v2 - --|v8[v2]|], (v6 + v6) + v6, v2 <= fm10(v2, f14, globalState), v2;
		var v9 := 'm';
		var v10: multiset<char> := multiset{v9};
		for i1 := |v10| to v2 {
			v9 := v9;
			f13 := !!f13;
			var v11: map<bool, char> := map[v1[|map[i1 := true]|] := v9];
			...
			match (DC36(f13, v4, v2)) {
				case DC35(cf70, cf71, cf72) =>
					...
					var v13: map<int, string> := map[cf70 := seq(-0x194, i2  => (v9))];
					v4[330] := |v13|;
					v4[330] := 0x1b2;
					var v14 := DC2(v4[330], f13, p0);
					...
					var v17: map<int, int> := map[v4[330] := i1];
					var v18: set<bool> := {p0};
					var v19: seq<array<D0>> := [v15];
					v15 := if ((if (|v18| in v17) then v17[|v18|] else v4[330]) in v16) then v16[if (|v18| in v17) then v17[|v18|] else v4[330]] else v19[-|(map v20 : int | (-0x1ab <= v20) && (v20 < 0x137) :: (v20 / i1) := (cf70))|];
					var v21: map<int, array<int>> := map[cf71 := cf72];
					var v22: seq<array<int>> := [cf72, cf72];
					var v23: array<array<int>> := new array<int>[17] [cf72, cf72, cf72, cf72, cf72, cf72, if (142 in v21) then v21[142] else cf72, cf72, cf72, cf72, cf72, v22[0x232], cf72, cf72, cf72, cf72, cf72];
					var v24 := DC49(v23);
					...
					v26[902] := v23;
					v26[902] := new array<int>[12] [cf72, cf72, cf72, cf72, cf72, cf72, cf72, cf72, cf72, cf72, cf72, cf72];
				case DC36(cf73, cf74, cf75) =>
					v7 := "vblretf" + (seq(0x3e5, i3  => (v9)));
				...
				case DC37(cf76, cf77) =>
					...
					v30[335] := v31;
					v30[335] := multiset{cf77, f31, i1 > cf76, f14, f14};
					v6 := v6[f14 := cf77];
					var v32: map<seq<int>, int> := map[p1 := i1];
					globalState.f7, v30[335], v9 := i1 * (if ((seq(-0xb, i4  => (-v2)))[cf76 := cf76] in v32) then v32[(seq(-0xb, i4  => (-v2)))[cf76 := cf76]] else i1), (v30[335] * v30[335]) - v30[335], v9;
				...
				case DC34(cf69) =>
					...
					var v42, v43 := m0(v41[v2 := v40], f14, globalState);
		...
		for i5 := v2 to v2 {
			...
			v51[116] := if (f13) then v4 else v4;
			var v52: multiset<int> := multiset{i5, v2};
			f15[144] := v52 > multiset{i5, v2, v2, i5, |v7|};
			var v53 := DC1(map[v2 := |v52|], f13, p1, f15);
			f13, v51[116], f15[144], globalState.f5 := !(if (true) then f31 else p0 ==> false), v4, f13, v53.cf2;
			if (v1[v2]) {
				f13 := !p0;
				var v54: array<bool> := new bool[16] [f15[144], f15[144], f15[144], f15[144], f13, p0, f14, f13, f15[144], p0, f13, p0, p0, !p0, true, f14];
				var v55: map<bool, array<bool>> := map[f15[144] := v54];
				var v56: seq<array<bool>> := [if (p0 in v55) then v55[p0] else v54];
				var v57 := DC56(v56);
				globalState.f8 := |v57.cf106|;
				r1 := v2;
				v51[116] := new int[26](i6 => i6 - |v1|);
				...
				v62 := v62[v61 := fm10(|p1|, !(if (f13 in v6) then v6[f13] else p0), globalState)];
				v51[116] := v51[116];
				var v63: map<int, char> := map[|p1| := v9];
				f15[144] := (if (--|v7| in v63) then v63[--|v7|] else 'b') !in fm42(v2, globalState);
				v7 := v7[p1[i5] := v9];
			}
			
			var v64: map<int, int> := map[if (fm10(i5, f14, globalState) in v52) then v52[fm10(i5, f14, globalState)] else i5 := p1[i5]];
			var v65: seq<map<int, int>> := [v64];
			var v66: set<bool> := {f31, f13};
			var v67: array<bool> := new bool[3] [f14, !true, f15[144]];
			globalState.f8, globalState.f5, globalState.f5, globalState.f8, r0 := v2, v64[i5 := i5] != v65[i5], p0, |fm51(v66, v2, globalState)|, v67;
		}
		if (f13) {
			globalState.f5 := !(if (f31 in v6) then v6[f31] else f13);
			globalState.f5 := v1[|p1|];
			v4[223] := fm10(v2, f13, globalState) / v2;
			v4[223], globalState.f7, globalState.f7, r1, globalState.f5 := |fm56(p1[|p1|], globalState)|, |(v7 + v7)[v2 := if (f14) then v9 else v9]|, if (f14) then v2 else |[v9, 'p']|, v2, f13;
		...
		if (f13) {
			...
			var v75 := DC59(|map[p0 := p0]|, |(v72 * v72)|, v9, fm9(v9, globalState), v74 == (multiset{v73})[v73 := v2]);
			match (v75) {
				case DC59(cf109, cf110, cf111, cf112, cf113) =>
					...
					var v80: seq<seq<int>> := [v76[|v79|]];
					var v81: map<char, int> := map[cf111 := cf110];
					v3 := v3[v2 := (seq(0xc7, i7  => (DC57(cf110).cf107))) < v80[if (cf111 in v81) then v81[cf111] else v2]];
					f13 := !cf112;
			...
			globalState.f5 := v1[v2];
			v4 := v4;
			var v85 := DC3(276, f31);
			var v86: set<D0> := {v85};
			var v87 := DC65(multiset{v2});
			var v88: seq<D0> := [v85];
			var v90: seq<set<D0>> := [v86, fm59(f14, v87.cf117, p0, |(seq(0x196, i8  => (v2)))|, globalState), set v89 : D0 | v89 in v88 :: (v89)];
			...
			match (v106) {
				case DC35(cf70, cf71, cf72) =>
					v7 := v7 + v7;
					cf72 := new int[24](i10 => i10 * (if (-0x26f in v94) then v94[-0x26f] else |(seq(0x1c1, i11  => (v9)))|));
					v0[742] := v7;
					v0[742] := v7;
					v0[742] := v0[742];
				case DC36(cf73, cf74, cf75) =>
		...
		if (true) {
			...
			if ((f13 <==> f14) ==> p0) {
				...
				v128[697] := DC13(DC10(p1));
				var v129 := DC13(DC10(p1));
				globalState.f8, v124, v126, v127[989], v128[697] := v2, v124, {v125, v125} + {v125, v125, v125}, if (f14) then fm56(v2, globalState) else v1 + v1, v129;
				v9 := v9;
			...
			var v136, v137 := m0(v135 + v135[fm10(|"tanv"|, f13, globalState) := v134], p0, globalState);
			var v138: multiset<int> := multiset{v2};
			var v139 := DC65(v138);
			f15[732] := v139.cf117 !! v138;
			var v140: multiset<bool> := multiset{f13, f14};
			var v141 := DC29(p0, v2, f14, p0);
			f15[732] := !(if (v140 >= fm67(v141.cf59, p0, v2, globalState)) then p0 else f31);
			var v142 := DC23({v4}, v9, v9);
			v142 := v142;
			globalState.f7 := |(seq(937, i14  => (v9)))|;
		...
		var v155 := DC33(|v7|, |v152[f13 := if (-|v154| in v153) then v153[-|v154|] else v2]|, v2);
		var v156: seq<map<int, int>> := [map[|v152[f14 := v2]| := v2]];
		r1 := v155.cf68 + (v2 / |v156|);
	}
	method m15(p0: int, p1: array<bool>, p2: int, globalState: GlobalState) returns (r0: seq<int>) {
		...
		if (if (f31) then f31 else f13) {
			...
			var v15: array<map<int, bool>> := new map<int, bool>[14] [fm54(f13, -p2, globalState), v11, v11, map[|v1| := !f31], v11 + v11, fm54(f13, p0, globalState), map[p0 := f13], (if (v3 in v12) then v12[v3] else v11) + v11, map v13 : int | v13 in v14 :: (v13 / 560) := (f31), v11 + v11, v11, v11, v11, v11];
			...
			var v20: seq<C3> := [v19];
			var v21: array<C3> := new C3[10] [v19, v20[p0], v19, v19, v19, v19, v19, v19, v19, v19];
			...
			var v28: array<bool> := new bool[7] [f13 && f14, f14, v23[p0], v23[|v26|] <== v27.cf77, (fm49(DC11(p2, f13, |v25|).cf23, p0, f31, globalState)).cf86, f13 <==> f14, !f13];
			...
			p1[800] := f13;
			var v41: set<int> := {p2};
			globalState.f7 := p0 % (|v41| + -p0);
			var v42: map<int, int> := map[p2 := p2];
			var v43 := DC1(v42, f14, [p0, |v30|], f15);
			var v44: seq<bool> := [if (p1[800] in v30) then v30[p1[800]] else !fm9(v5, globalState), (v43.(cf2 := f14)).cf2, f13];
			if (v44[p2 - p2]) {
				var v45 := DC37(p2, p1[800]);
				p1[800] := v45.cf77;
				globalState.f7 := p0 / p2;
				v3[33] := |fm56(|v1|, globalState)|;
				v3[33], globalState.f5 := p0, f31;
				v3[33] := fm10(v3[33], p1[800], globalState) + v3[33];
				var v46 := DC68(v1);
				var v47: seq<int> := [p0, v3[33], p0, p2, |v46.cf120|];
				var v48 := new C2([v3[33], p2] < v47, if (f31) then f13 else f31);
			} else {
				...
				v49, globalState.f8, v50[527], v51, globalState.f5 := v49, p2, fm8(|v1|, globalState), v51, if (!f31 <==> f14) then p1[800] ==> v44[if (p0 in v52) then v52[p0] else p0] else f14;
			...
			var v57: multiset<bool> := multiset{p1[800]};
			var v58 := DC40(p0, f31, |v57|, p1[800], f13);
		...
		for i1 := p0 * v60.cf55 to p0 {
			...
			v64[444] := fm2(f31, globalState);
			v61, v62[151], v64[444], v63 := if (f14) then v61 else multiset(v63), v63, v5, fm56(i1, globalState);
			var v65: array<string> := new string[18](i3 => v0 + v0[p2 := v5]);
			v65[395] := v0;
			v65[395] := "cjkcxv";
			var v66: map<map<int, bool>, seq<bool>> := map[map[p2 := false] := v62[151]];
			var v67: multiset<array<int>> := multiset{v3};
			globalState.f7, v5, v66, v65[395], v65[395] := if (v3 in v67) then v67[v3] else |map[f14 := f14]| % p2, v5, if (f31) then v66[fm54(f31, p2, globalState) := v62[151]] else v66, v65[395], v65[395];
			v1 := v1['e' in v0 := i1];
		}
		v0 := v0;
		var v68 := DC19();
		...
		match (v69) {
			case DC19() =>
				...
				var v72: seq<int> := [|v71|];
				f15[117], globalState.f7, globalState.f7, r0, v70 := f31, -p2, p0, v72, v70;
				if (f15[117]) {
					var v73: set<bool> := {f13, f31};
					var v74: seq<set<bool>> := [v73, v73];
					globalState.f5 := v73 > v74[-p0];
					var v75: array<char> := new char[5](i4 => v5);
					v75[216] := v5;
					v75[216] := v5;
					v5 := if (f15[117]) then 'e' else v75[216];
					globalState.f8 := v72[p0];
					var v76: set<int> := {|v73|};
					var v77: map<int, int> := map[|v70| := --DC37(|v76|, f14).cf76];
					var v78: array<multiset<bool>> := new multiset<bool>[6];
					v78[633] := if (f14) then v71[f14 := |v0|] else v71;
					v77, v78[633] := v77 + v77, fm67(f13, f31, fm10(p0, f13, globalState), globalState) * v71;
				} else {
					...
					v79[970] := set v82 : string | v82 in v81 :: (v82);
					var v83 := DC37(p2, f15[117]);
					var v84: seq<bool> := [f31];
					var v85: map<seq<int>, int> := map[if (v83.cf77) then fm50(v84, globalState) else v72 := |[p0, p0, p0, -p0, p2]|];
					v85 := v85[v72 := if (f15[117]) then p0 else p2];
					f15[117], globalState.f8 := !f13, p0;
				}
			...
			case DC18(cf37) =>
				...
				var v97: seq<int> := [p2];
				var v98: map<bool, string> := map[f13 := v0[|v97| := v5]];
				globalState.f8, v96 := v88.fm10(p2, false, globalState), [v0, "fcp", (if (f13 in v98) then v98[f13] else v0) + v0, v0, (v0 + (seq(0xe9, i5  => (v5))))[p0 := v5]];
			case DC20(cf38) =>
				v3[590] := p2;
				v3[590] := |(("ve" + v0) + v0)|;
				var v99: map<int, array<bool>> := map[0xce := f15];
				var v100 := DC15(f13, v99);
				var v101: map<D4, int> := map[v100 := v3[590]];
				var v102: map<int, array<bool>> := map[|v101| := f15];
				var v103 := DC15(f14, v102);
				var v104: map<int, int> := map[|{p0}| := v3[590]];
				var v105: set<int> := {p0};
				var v106: multiset<set<int>> := multiset{v105, {|{v3[590], v3[590]}|}};
				var v107: seq<multiset<set<int>>> := [v106, (multiset{v105})[{p0} := |v1|], v106, v106];
				var v108: multiset<bool> := multiset{f14, f13, f14, f13, f31};
				var v109: seq<int> := [p2, v3[590]];
				var v110: map<int, bool> := map[|v104| := f14];
				var v111: array<int> := new int[18] [if (f31) then |"s"| else 0x377, v3[590] - v3[590], p0 % v3[590], v3[590] % v3[590], v3[590], fm10(-v3[590], v100.cf31, globalState), p2 * (if (p2 in v104) then v104[p2] else 0x271), p0, p2, v3[590], fm10(v3[590], f31, globalState), |v107[|v0|]|, v3[590], v3[590], p0 - p0, p2, |v108|, v109[|v110|]];
				var v112: multiset<int> := multiset{v3[590]};
				globalState.f8, v111 := if (p2 >= p0) then 0x37 % v3[590] else p0, if (!(v112 > v112)) then v111 else v111;
...
class C5 extends T1, T2 {
	...
	method m3(p0: D0, p1: seq<bool>, p2: int, p3: bool, globalState: GlobalState) returns (r0: seq<set<bool>>, r1: bool) {
		...
		var v6 := 'i';
		var v7: array<int> := new int[1](i0 => i0 % p2);
		var v8: multiset<int> := multiset{p2};
		var v9 := DC5(map[|v8| := v6]);
		var v10 := DC16(v7, |v2|, v9);
		var v11: set<D4> := {v10};
		var v12: array<int> := new int[13] [p2 % |v4|, p2, p2, -(if (fm9(v6, globalState)) then p2 else -0x2a1), p2, fm10(p2, f13, globalState), if (!false) then 243 else |v11|, p2, p2 + p2, p2, p2, p2, p2 / 0x2a9];
		v12 := v7;
		var i1 := 0;
		while (f14)
			decreases 100 - i1
		{
			...
			var v20: set<int> := {v17.f28, |v19[p2]|};
		...
		r1 := f14;
		var v25: array<bool> := new bool[2](i3 => p1[-p2]);
		...
		r0 := if (v2 in v41) then v41[v2] else ((seq(0x2c4, i4  => (v38))) + v39)[p2 := v38];
	...
	method m4(p0: bool, p1: seq<int>, globalState: GlobalState) returns (r0: array<bool>, r1: int) {
		...
		for i1 := v6 to v6 {
			...
			v7[38] := (if (f14 in v9) then v9[f14] else v10) + v10;
			var v11: seq<char> := [v3, v3, v3];
			var v12: set<array<bool>> := {f15};
			v3, v7[38], globalState.f5 := v11[v6], v10 + (v10 + v10), {f15} <= (v12 - v12);
		...
		for i2 := |v24| to v6 {
			...
			var v27: array<string> := new string[8] ["oocelbl", v24, v24, fm28(v6, f13, false, p0, globalState) + v24, if (true) then v24 else v25[v6], v24 + v24, if (0x2b0 in v26) then v26[0x2b0] else "cioi", fm28(|fm37("drku", globalState)|, f13, p0, f14, globalState)];
			v27[117] := v24;
			v27[117] := v24;
		...
		r1 := |(v30[if (v32.(cf11 := v34[|v35| := v3]) in v33) then v33[v32.(cf11 := v34[|v35| := v3])] else v6])[v6 := f14]|;
	}
	method m5(p0: int, globalState: GlobalState) returns (r0: seq<int>, r1: set<seq<char>>, r2: bool) {
		globalState.f5 := false;
		var v0: seq<bool> := [true, f14, false, f14];
		var v1 := "aakrarafl";
		var v2: seq<bool> := [f13, !f13, v0[|v1|], f14, !f13];
		var v3: multiset<seq<bool>> := multiset{v2, v2};
		var v4: set<bool> := {f13};
		var v5 := DC37(p0, f13);
		var v6: set<int> := {p0, |v4|};
		var v7: array<int> := new int[25] [p0, p0, p0, 0x3b8, p0, -p0 - |(seq(739, i0  => ('m')))|, if (v2 in v3) then v3[v2] else 716, fm25(p0, v4, f14, globalState), p0, p0, p0, p0, 808, p0, p0, |(v4 - {!v5.cf77, !f14})|, p0, p0, 414 + fm25(p0, v4, f13, globalState), |v6| + 0x1f3, p0, p0, |fm28(p0, f13, f14, f14, globalState)|, p0, p0];
		v7 := v7;
		...
		match (v11) {
			case DC22(cf40) =>
				v9 := v9;
				globalState.f7 := -(p0 / p0) / 0x2d7;
				var v12 := new C2(!(if (f13) then f13 else f14), f13);
				v7[925] := -|v1| + |[p0]|;
				f15[666] := f13;
				var v13: map<int, int> := map[p0 := fm10(|v1|, f14, globalState)];
				globalState.f8, v7[925], f15[666] := -|((seq(85, i1  => (map[p0 := p0])))[p0 := v13] + fm40(p0, globalState))|, p0, f13;
			case DC23(cf41, cf42, cf43) =>
				...
				globalState.f7, v7, globalState.f8, v7[815], globalState.f5 := -p0, v7, (p0 - p0) % fm10(|(if (v7 in v14) then v14[v7] else v16[p0])|, true, globalState), p0, f13;
				r2 := v6 >= (set v17 : int | (338 <= v17) && (v17 < 0x167) :: (v17 % v7[815]));
				globalState.f8 := |v1|;
				var v18: map<string, bool> := map["bpcdigp" := f14];
				var v19: map<int, array<bool>> := map[v7[815] := f15];
				var v20: map<map<string, bool>, map<int, array<bool>>> := map[v18 := v19];
				v7[815] := |(map[0x241 := f15] + (if (map[seq(861, i2  => (DC7(p0, cf43, f14).cf14)) := f14] in v20) then v20[map[seq(861, i2  => (DC7(p0, cf43, f14).cf14)) := f14]] else v19))|;
			case DC24(cf44, cf45, cf46, cf47, cf48) =>
				...
				match (v21) {
					case DC1(cf1, cf2, cf3, cf4) =>
						...
						cf4[88] := if (v24.(cf73 := v23.f30, cf75 := 0x362) in v25) then v25[v24.(cf73 := v23.f30, cf75 := 0x362)] else cf44;
					case DC2(cf5, cf6, cf7) =>
						f15[930] := cf7;
						f15[930] := cf6;
						globalState.f7 := cf5;
						...
						v27[9] := v26;
						var v28 := DC24(f14, f13, cf46, f15[930], cf48);
						var v29: map<bool, int> := map[f14 := p0];
						var v30: seq<int> := [cf48];
						var v31 := DC10(v30);
						v27[9] := new bool[27] [false, cf45, f13, if (cf44) then cf47 else cf7, cf44, f14, cf44, cf44, true, f14, f15[930] <==> f15[930], f14, cf7, cf47, false, cf44, f14, v2[cf5], cf7, cf47, fm0(cf5, cf6, globalState), v28.cf45, false, fm9(v9, globalState), false, v29 != v29, fm24(v1, v31, cf44, globalState)];
						var v32: seq<seq<bool>> := [v0 + (fm41(v9, |v29|, cf48, p0, globalState))[p0 := cf44]];
						v32 := v32;
					case DC3(cf8, cf9) =>
						var v33 := DC14(v8);
						v33 := v33;
						...
						v34[129] := v7;
						v34[129], cf48, cf9, cf8 := v7, p0 * -p0, cf47, -cf8;
						var v35: map<bool, array<int>> := map[f14 := v34[129]];
					...
					case DC0(cf0) =>
						...
						v46, f15[472], f13, v48 := v46, cf46 < cf46, !(cf44 !in v2), f15;
					case DC4(cf10) =>
						globalState.f8 := fm10(p0, fm9(v9, globalState), globalState);
						f13 := cf44;
						var v49 := new C0(fm10(p0, cf47, globalState) > 0x1a3, cf44);
						var v50 := new C1((seq(0x2cd, i3  => (v9))) + v1, |v1|);
				}
				
				var v51: multiset<int> := multiset{|(seq(756, i4  => (v9)))|, |v1|};
				globalState.f8 := (497 + |v51|) / cf48;
				var v52: map<char, bool> := map[v9 := f14];
				v2 := (v0 + (v2 + fm41(v9, -p0, |v52|, p0, globalState)))[cf48 := cf47];
			case DC21(cf39) =>
			...
			case DC25(cf49) =>
				...
				if (!f14) {
					...
					var v70 := new C1(v1, p0 + |v69[|(seq(0x1cd, i6  => (|v2[p0 := false]|)))|]|);
					f15[190] := v65.f30 || v65.f29;
					...
					f15[190] := if (v71 in v72) then v72[v71] else v73 in multiset{v73, v73, v73, map[v65.f29 := p0]};
					...
					var v78 := DC39((multiset(v77))[v74 := v70.f28]);
					v76 := v76[v7 := if (true) then v78.cf79 else v75];
					v65.f29 := if (v65.f30) then f13 else v1 != v70.f27;
					f15[190] := v65.f29;
				} else {
					v7[976] := -p0;
					var v79: seq<int> := [p0, -p0, |(v1 + (seq(-0x1f9, i7  => ('j'))))|];
					v1, r0, globalState.f8, v7[976], r2 := v1 + v1, v79, -674, p0, v65.f29;
					var v80: array<bool> := new bool[16];
					v80 := f15;
					f13 := if (v65.f30) then if (v7[976] in v66) then v66[v7[976]] else !v65.f29 else !true;
					globalState.f8 := p0;
		...
		globalState.f7, v81[433], f15[775], globalState.f5, v83[830] := p0, v9, true, fm0(p0, v85 !in v86, globalState), (v84 + v84) + v84;
			...
			decreases 100 - i9
		{
			...
			globalState.f5 := f14;
			v7[393] := p0;
			var v90: multiset<int> := multiset{fm10(fm10(v84[p0], false, globalState), f15[775], globalState)};
			v7[393] := if (p0 in v90) then v90[p0] else |(if (f15[775]) then v83[830] else seq(0x1f8, i10  => (p0)))|;
			var v91: map<bool, bool> := map[f15[775] := true];
			var v92: multiset<bool> := multiset{true, if (f15[775] in v91) then v91[f15[775]] else false, f13, f15[775]};
			v9, v7[393], f15[775], f15[775], globalState.f5 := v81[433], (p0 * |v4|) + p0, f13 in (v2 + [f15[775]]), !(v92 != v92), false;
			var v93: multiset<char> := multiset{v81[433], v9};
			f13, v81[433], v93 := f15[775], v9, v93;
		}
		var v94: map<int, bool> := map[p0 := f15[775]];
		v2 := fm41(v81[433], p0, |v94| % 0x309, p0, globalState);
		r0 := v84;
		var v95: set<seq<char>> := {fm28(|v1|, f14, f13, !true, globalState), v1, v1};
		r1 := v95;
		var v96: map<bool, bool> := map[f15[775] := false];
		r2 := -p0 >= (|v96| + p0);
	}
	method m6(p0: bool, globalState: GlobalState) {
		if (f14) {
			...
			match (v11.(cf65 := "yqqxm")) {
				case DC33(cf66, cf67, cf68) =>
					...
					var v13 := DC24(f14, f13, v12 - multiset([p0, p0]), p0, cf68 % cf68);
					...
					var v19 := "houval";
					f15[162], globalState.f5, globalState.f5, v14[342], v15 := f13, cf68 > (-0x1a * |v17|), (fm33(cf68, v16.fm9(v7, globalState), v16.f14, globalState) + v18) <= v18, v7, [|v19|];
				...
				case DC32(cf65) =>
					...
					v24[667] := cf65;
					var v25: seq<bool> := [p0];
					v23, v24[667] := fm50(v25[v22 := f14], globalState), if (f14) then cf65 else cf65;
					f15[688] := !f14;
					globalState.f8, f15[688], v5[991], globalState.f5, globalState.f5 := v5[991], p0, -v22 / v22, f13, fm0(v22 / v5[991], f14, globalState);
					var v26: set<bool> := {p0};
					var v27 := DC57(fm25(v5[991], v26, f14, globalState));
					var v28 := DC66(0x276);
					var v29: map<D22, bool> := map[v28 := f13];
					v27, globalState.f5, globalState.f5, v22 := v27, f13, f13, |(v29 + v29)| / v22;
			}
			
			f15[783] := p0;
			var v30: array<char> := new char[19](i1 => v7);
			v30[858] := v7;
			v5, f15[783], v30[858] := v5, p0, fm2(f13, globalState);
			var v31 := 0xa8;
		...
		var v38: multiset<int> := multiset{v37[v33]};
		var v39: map<bool, bool> := map[p0 := p0];
		var v40: array<int> := new int[26] [v33, v33, v33 + v33, v33, v33 + v33, |fm33(-951, !f13, p0, globalState)|, if (false) then v33 else |v34|, v33 * -v33, v33, if (f13 in v35) then v35[f13] else |v36|, v33, v33, v33, v33, v33, v33, v33 - v33, |v38|, v33, v33, 120 / v33, v33 - v33, |v39|, v33, v33, fm10(v33, f13, globalState)];
		var v41: set<int> := {v33};
		v40[936] := v33 - |v41|;
		v40[936] := v33;
		var i2 := 0;
			...
			decreases 100 - i2
		{
			...
			i2 := i2 + 1;
			globalState.f5 := (v33 + v40[936]) > (v33 % v33);
			v33 := v33;
			globalState.f7 := |v34|;
	...
	method m11(globalState: GlobalState) returns (r0: seq<bool>, r1: map<bool, int>, r2: int, r3: int) {
			...
			decreases 100 - i0
		{
			...
			r0 := v5;
			v1 := -v1;
			v0[31] := |fm28(v1, false, f14, f14, globalState)|;
			var v6 := 'i';
			...
			v0[31] := if (v7 in v10) then v10[v7] else v1;
		}
		var v11: seq<bool> := [f14, f14, f13];
		r0 := v11[v1 + |map[v1 := v1]| := f14];
		var v12 := 'g';
		...
		if (match DC9(v13) {
			...
			f15[327] := if ((if (f13) then v17 else (v17[v1 := [443, |v19|]])[v1 := [v20.cf55]]) in v18) then v18[if (f13) then v17 else (v17[v1 := [443, |v19|]])[v1 := [v20.cf55]]] else f13 <== f14;
			if (f14) {
				var v21: array<bool> := new bool[17];
				var v22 := new C3(f14, v21, fm0(|v15|, false, globalState), f15[327]);
				v16 := v16 + v16;
				f13 := v22.f32;
				f15[327] := -v1 <= (|v14| - fm25(v1, {!v22.f32}, f15[327], globalState));
				var v23: map<bool, char> := map[f15[327] := 'c'];
				globalState.f7 := |v23|;
			} else {
				globalState.f8 := 712;
				globalState.f5 := f13;
				var v24: array<bool> := new bool[2](i1 => f14);
				var v25: map<array<bool>, bool> := map[v24 := f15[327]];
				var v26: array<bool> := new bool[14] [f14, f13, f15[327], if (v24 in v25) then v25[v24] else f15[327], f13, f13, f15[327], f15[327], f13, f13, f13, true, f14, f14];
				var v27: map<int, int> := map[v1 := |v14|];
				var v28: set<map<int, int>> := {v27};
				var v29 := "fnervxff";
				var v30 := new C4(f13, v24, v28 >= fm69(|v29|, v1, v16[|[|v11|, v1]|], f13, globalState), 0xfb < -v1);
				var v31 := new C4(v29 == v29[v1 := v12], v26, f13, f13);
				f13 := v1 < v1;
			}
			...
			if (v1 !in v34) {
				...
				v33.f29, v0[33], v33.f29, v37[396], v41 := (v42 != v42) !in (v43 - v43), v1, f13 && f13, v0, v41;
			} else {
				var v44: array<array<array<bool>>> := new array<array<bool>>[24];
				var v45: seq<array<bool>> := [f15];
				var v46: map<bool, int> := map[f13 := v1];
				var v47: array<array<bool>> := new array<bool>[20] [f15, f15, f15, v45[|v46|], f15, f15, f15, f15, f15, f15, f15, f15, f15, f15, f15, f15, f15, f15, f15, f15];
				v44[524] := v47;
				v44[524] := v47;
				var v48 := "truco";
				v48 := v48 + (v48 + v48);
				...
				var v51: array<string> := new string[13] [(v49[v1])[v1 := v12], v48 + v48, v48, v48, v48, (if (v33.f30 in v50) then v50[v33.f30] else "pujtcau") + v48, v48, seq(-0x27f, i2  => (v12)), v48, v48 + v48, (seq(309, i3  => (v12))) + v48, v48, v48 + "sv"];
				v51[961] := v48;
				v51[961] := v48;
				r3 := v1;
				v51[961] := (seq(942, i4  => (v12))) + v51[961];
			}
			
			if (v33.f29) {
				var v52 := DC68(map[v33.f29 := -|"hs"|]);
				v0[227] := |(v52.cf120 + map[f13 := v1])|;
				var v53 := "e";
				var v54: C1 := new C1(v53 + v53, -0x2d1);
				var v55: seq<int> := [|v34|];
				f15[371] := v11[v1];
				v0[227], v54, v55, f15[371] := v54.f28 - v1, v54, v55, f14 || v33.f30;
				...
				globalState.f7, f13, r2, globalState.f8 := if ((v1 % -|v63|) in v60) then v60[v1 % -|v63|] else v1, v58.f29, -(if (v33.f29) then v1 else v1), |(map v64 : int | v64 in v34 :: (v64 - |v60|) := (v1))|;
			...
			f15[457] := if ((v65[f14 := v1] + v65) in v66) then v66[v65[f14 := v1] + v65] else f14;
		...
		for i5 := |(v67 + v67)| to 0x269 * v1 {
			...
			v0[838] := fm10(|(seq(0x213, i6  => (v12)))|, f14, globalState);
			v0[838] := i5;
			var v71: multiset<int> := multiset{v69, v69, |{f13, f13}|};
			v71 := v71;
		}
		r0 := (v11[v1 := f13])[v1 := f14];
	...
	method m12(globalState: GlobalState) returns (r0: int, r1: seq<int>) {
			...
			decreases 100 - i0
		{
			...
			var v8: seq<int> := [v0[697]];
			var v9: seq<int> := [|v8|];
			var v10 := new C2(v0[697] == |(map v7 : int | v7 in v9 :: (v7 / v1) := (v1))|, f13);
			var v11 := "qye";
			var v12: set<int> := {v1};
			var v13 := 'a';
			var v14: array<string> := new string[15] ["riqdxf", v11, v11[|v12| := v13], v11, v11, "esiup", v11[v1 := v13], v11, v11, v11, v11, v11, v11, v11, v11];
			var v15 := DC70(v14);
		...
		for i1 := |(map v19 : string | v19 in v21 :: (v19) := (475))| * v0[697] to v0[697] {
			f13 := if (f14) then f13 else f14;
			var v22: array<string> := new string[2];
			var v23: seq<array<string>> := [v22];
			var v24: array<array<string>> := new array<string>[14] [v22, v23[581], v22, v22, v22, v22, v22, v22, v22, v22, v22, v22, v22, v22];
			v24[786] := v22;
			v24[786] := v22;
			var v25: map<int, int> := map[-(i1 * v0[697]) := i1];
			v25 := v25[787 / v1 := v1];
			...
			if (v26.cf26) {
				...
				var v31 := new C1(v20, v30[|[v29.f30]|]);
				var v32 := 'y';
				var v33: map<char, array<bool>> := map[v32 := f15];
				globalState.f8 := v0[697] + fm10(-|[if ('g' in v33) then v33['g'] else f15]|, f14, globalState);
				var v34: array<array<bool>> := new array<bool>[15];
				v34[534] := f15;
				v34[534] := f15;
				var v35: set<int> := {-v1 + v31.f28, |v20|};
				var v36 := DC37(|multiset{f13}|, f14);
				globalState.f8, globalState.f7, v29.f29, v35 := -(v31.f28 + v0[697]), |(if (v36.cf77) then v5 else v5 + v5)|, v29.f30 ==> f13, fm37(v20, globalState) - (v35 * v35);
			} else {
				var v37: map<bool, seq<string>> := map[f14 := [v20, seq(500, i2  => ('l')), seq(88, i3  => ('f')), v20]];
				var v38: seq<string> := [v20, v20];
				var v39 := DC66(i1);
				var v40 := 'r';
				var v41: map<char, seq<string>> := map[v40 := [v20, seq(-0x39b, i4  => ('o'))]];
				var v42 := DC32(v20);
				var v43: map<bool, int> := map[!!f14 := 0x27b];
				var v44: array<seq<string>> := new seq<string>[18] [(if (f13 in v37) then v37[f13] else v38) + v38[0x310 := "lpcro"], v38, fm70(v0[697], i1, v39, globalState), [v20, v20, v20], if (f13) then ["ubmpmiv", v20] else v38, v38, v38, if (v40 in v41) then v41[v40] else [fm28(-935, f14, f14, f14, globalState), v42.cf65], v38 + v38, v38, seq(0x180, i5  => (v20)), v38 + v38, v38, v38, v38 + (v38[v0[697] := v20])[v0[697] := v20], v38, seq(944, i6  => (v20)), fm70(|v43|, |v20|, v39, globalState)];
				v44[195] := v38 + [v20];
				var v45: array<char> := new char[17];
				v45[463] := v40;
				globalState.f5, v44[195], v45[463] := true, v38, v40;
				v0[697] := -408 * i1;
				var v46 := new C2(v20 == v20, f14);
				f15[679] := f14;
				f15[679] := f14;
				v20, globalState.f8, globalState.f7 := v38[if (f14) then i1 else i1], v1, i1;
			}
		...
		var v49 := DC11(v1, f13, v0[697]);
		var v50: seq<D3> := [v49];
		for i7 := |fm44(f13, f14, 'n', v0[697], globalState)| to |v50| {
			var v51: multiset<array<bool>> := multiset{f15, f15, f15, f15, f15};
			globalState.f7 := if (f15 in v51) then v51[f15] else v1 / 791;
			var v52: seq<multiset<int>> := [v47];
			globalState.f5 := (v47 * v47) < (v52[v1] - v47);
			var v53 := 'k';
			v53 := v20[v0[697]];
			globalState.f5 := v47 < (v47 * v47);
		}
		r0 := v0[697];
		var v54: seq<int> := [v0[697]];
		r1 := (v54 + v54) + (v54 + v54[|{seq(423, i8  => (|map[v1 := f13]|))}| := |v54|]);
	}
}
...
class C6 extends T1 {
	...
	method m3(p0: D0, p1: seq<bool>, p2: int, p3: bool, globalState: GlobalState) returns (r0: seq<set<bool>>, r1: bool) {
		if (f13) {
			...
			match (v1) {
				case DC7(cf13, cf14, cf15) =>
					...
					var v4: map<bool, seq<int>> := map[p1[p2] := [|v3|]];
					v2 := v2 + (v2 + (if (p3 in v4) then v4[p3] else v2[f25 := cf13]));
					globalState.f5 := f14 ==> cf15;
					var v5: array<int> := new int[1] [f25];
					v5 := v5;
					var v6: seq<array<int>> := [v5, v5];
					var v7: map<seq<array<int>>, int> := map[[v5, v5, v5, v5, v6[p2]] := p2];
				...
				case DC6(cf12) =>
					...
					v15[605] := v16;
					var v17 := "ajdis";
					var v18: seq<string> := ["hpol", "inwqrh", seq(-0x2db, i1  => (v0)), v17];
					globalState.f8, globalState.f8, v15[605] := (f25 % f25) - p2, |v18| + p2, v16.(cf87 := v0);
					v17 := v17[|(cf12 * {f25})| := v0];
					globalState.f7 := --((f25 * p2) % f25);
				case DC9(cf21) =>
					globalState.f7 := f25;
			...
			v24 := v24;
			var v25: array<int> := new int[4](i3 => i3 / f25);
			var v26 := "mk";
			v25[329] := |v26|;
			var v27: set<bool> := {f13};
			var v28: map<bool, set<set<int>>> := map[f14 := fm57(f25, v0, fm25(f25, v27, !true, globalState), globalState)];
			var v29: set<int> := {|(seq(0x2dc, i4  => (|multiset{f13}|)))|};
			var v30: set<set<int>> := {v29};
			v25[329] := f25 / (if (f13) then |(if (p3 in v28) then v28[p3] else v30)| else |v26|);
			if (p1[0x3c6]) {
				var v31 := DC50();
				...
				v31, v0, globalState.f8, globalState.f7, v32 := v31, v0, fm25(|fm56(f25, globalState)|, v27, if (-f25 in v35) then v35[-f25] else f14, globalState) * v25[329], if (607 in v36) then v36[607] else v25[329], v32;
				v26 := v26 + v26;
				var v37: seq<seq<bool>> := [p1, p1];
				var v38: set<char> := {'v', v26[p2]};
				var v39: array<seq<seq<bool>>> := new seq<seq<bool>>[28] [v37, v37, [p1, p1], v37, v37, seq(-0x2d2, i7  => (p1)), fm71(|v38|, (seq(0x1d7, i8  => (v0)))[v25[329] := v0], globalState) + [p1], [[f13]], v37, [[p3, f13]], v37 + v37, [p1, p1, p1], v37 + v37, v37[0x389 := p1], v37, seq(0x1cd, i9  => (p1)), v37 + v37, [p1, p1, p1, p1, [f13]] + v37, v37, v37, seq(0x28, i10  => (p1)), ([p1, p1])[f25 := p1], v37, v37, v37, v37, [p1, p1, p1], v37];
				v39[642] := v37 + (seq(0x2ea, i11  => (p1)));
				var v40: seq<int> := [v25[329]];
				var v41: multiset<bool> := multiset{p3, f14};
				v39[642] := fm71(|v40| % |v41|, v26, globalState);
				var v42: set<array<int>> := {v25};
				var v43: array<char> := new char[28] [v0, v0, v0, 'e', v26[26], v0, if (p3) then v0 else v0, v0, v0, 'r', v0, 'b', v0, v0, 'o', v0, v0, v0, v0, v0, v26[|"gxux"|], v0, v0, v0, DC23(v42, v0, v0).cf42, fm2(f14, globalState), v0, v0];
				v43[89] := fm8(f25, globalState);
				v43[89] := DC23(v42, v0, v0).cf42;
				var v44 := new C4(f13, v33, f13 <==> f13, false);
			} else {
				var v45: multiset<bool> := multiset{p3, f13, fm9(v0, globalState), f14};
				v29 := {v25[329], |v45|} * (set v46 : int | (642 <= v46) && (v46 < 0xda) :: (v46 * f25));
				var v47: map<int, int> := map[|v45| := f25];
				var v48: map<int, bool> := map[|v26| := fm0(if (v25[329] in v47) then v47[v25[329]] else v25[329], p3, globalState)];
				var v49: array<bool> := new bool[11] [p3, |v48| == v25[329], f13, p3, p3, f13, v0 !in fm51({f14}, v25[329], globalState), p3, false, p3, p3];
				v49[464] := f25 > v25[329];
				var v50: multiset<array<int>> := multiset{v25, v25, v25};
				globalState.f5, v49[464] := p3, v50 !! multiset{v25, v25, v25, v25};
				v26 := v26;
				v49[464] := v26 != v26;
				...
				var v54: seq<set<int>> := [{|v26|, f25, p2, -v25[329], |multiset{v49[464]}|}, {v25[329], f25, -v25[329]}, {if ('e' in v53) then v53['e'] else v25[329], v25[329]}, {v25[329]}, v29];
				v51, v25[329], v52, globalState.f7, v25[329] := v51, --f25, v52, |v54|, p2;
			}
			
			var v55 := DC8(v25[329], f14, v25[329], p2, v25[329]);
			var v56: seq<D2> := [v55, v55, v55, v55, v55];
			var v57: map<bool, int> := map[f14 := p2];
			if (((seq(-530, i13  => (DC8(p2, f14, |map[f13 := v25[329]]|, p2, v25[329])))) + v56[|v57| := v55]) != (v56 + v56)) {
				...
				v60[915] := v63;
				var v64 := DC28(v58, v25[329], 0x177);
				v59[894], v60[915] := v64.cf54, v63 + v63;
				globalState.f8 := -(|v26| + v25[329]);
				...
				f13, v69, v70[329], v25[329] := v29 > (if (p3) then v29 else v29), map[fm73(globalState) := f14 && p3], p1, DC51(f25, v71, p3, |p1|, if (|v26| in v72) then v72[|v26|] else f25).cf100;
				v58[146] := v27 >= v27;
				var v73: array<string> := new string[24];
				var v74: seq<array<string>> := [v73, v73, v73];
				v58[146], v73, globalState.f7 := v26[v25[329] := v0] != v26, v74[p2], f25;
			} else {
				...
				var v78: map<D11, int> := map[v75 := if (false) then |(seq(0x35, i16  => (v0)))| else |v77|];
				var v79: seq<bool> := [p3];
				v78, v79, globalState.f5 := (map v80 : D11 | v80 in multiset{v75, v75, v75} :: (v80) := (-v25[329])) + (v78 + v78), ([p3, p3] + p1) + v79, |v26| < -(v25[329] % v25[329]);
				var v81: array<seq<int>> := new seq<int>[5](i17 => [f25, p2] + [v25[329]]);
				v81 := v81;
				var v82: multiset<string> := multiset{if (v76.fm9(v0, globalState)) then v26 else v26, seq(-0x3d3, i18  => (v0))};
				v82 := v82;
				var v83: array<bool> := new bool[18];
				var v84: map<int, bool> := map[p2 := v76.f13];
				var v85 := DC27(v25[329], if (f25 in v84) then v84[f25] else p3, !fm0(f25, p3, globalState));
				var v86: map<array<bool>, D7> := map[v83 := v85];
				v86 := v86[v83 := v85];
				globalState.f8 := v25[329] * f25;
			}
			...
			r1, v87, v89 := f14, seq(0x2f3, i19  => ('k')), [p3, p3, f14] + v89[|[false, f14]| := p3];
			var v90 := 'h';
			v90 := if (!f14) then v90 else v90;
			globalState.f5 := (f25 / f25) != |v88|;
			...
			if (f14) {
				...
				r1 := v95[480];
				var v98: array<set<int>> := new set<int>[10](i20 => set v97 : int | v97 in (seq(407, i21  => (p2))) :: (v97 * -0x262));
				var v99 := DC55(v98);
				v99 := v99.(cf105 := v98);
			} else {
		...
		if (fm0(p2 + |[v108, v108, v108]|, f13, globalState)) {
			...
			var v118: seq<set<int>> := [fm37(v108, globalState) * v117, fm37(v108, globalState), fm37(seq(0x2b3, i24  => ('r')), globalState)];
			v118 := v118 + v118[f25 := v117];
		...
		var v122: set<int> := {p2};
		var v123: array<int> := new int[25] [f25, f25, fm25(f25, {f14, p3}, f14, globalState), f25, p2, p2, -fm25(f25, v120, p3, globalState), |v120|, f25, f25, f25, f25, p2, p2, |v108|, 268, 0x3ae, p2, p2, v121[p2], p2, p2, f25, p2, |v122|];
		...
		v126[238] := f13;
		v125, v126, globalState.f7, v126[238] := v125, v126, f25, (v122 !! v122) in (v120 * v120);
		var v127: map<bool, bool> := map[p3 := v126[238]];
		...
		match (v129.(cf79 := v128)) {
			case DC40(cf80, cf81, cf82, cf83, cf84) =>
				...
				var v131 := DC39(v128);
				match (v130.(cf88 := if (v126[238]) then v131 else fm49(-0x20e, -f25, cf83, globalState))) {
					case DC40(cf80, cf81, cf82, cf83, cf84) =>
						var v132 := DC32(v108 + v108);
						v132 := v132;
						v123, v126[238] := v123, true;
						v126 := v126;
						v123[844] := p2;
						v123[576] := cf82;
						var v133: multiset<int> := multiset{p2, |p1|, p2, cf80, p2};
						v123[844], globalState.f7, v123[576], v108, f13 := if (f25 in v133) then v133[f25] else if (f13) then cf80 else p2, (-(if (f25 in v133) then v133[f25] else p2) % cf82) / 959, p2, v108, fm0(cf80, if (f14) then cf83 else v126[238], globalState);
					case DC41() =>
						var v134 := 'p';
						var v135: T1 := new C5(p3, cf84, v126);
						var v136: map<T1, string> := map[v135 := v108];
						globalState.f5, cf83, globalState.f7 := fm9(v134, globalState), cf83, (0x16e - -f25) - (|v136| % cf80);
						var v137: map<bool, array<int>> := map[v126[238] := v123];
						v137 := v137[f14 := v123];
						var v138: multiset<int> := multiset{cf82, cf82};
						var v139 := new C5(v138 !! (multiset{f25})[p2 := 0x2a1], v135.f13, if (true) then v126 else v126);
						globalState.f7, v108, v134 := cf82, v108, v134;
					case DC42(cf85, cf86, cf87) =>
						var v140 := new C5(if (v126[238]) then !fm0(f25, cf86, globalState) else v126[238], p1[cf82], v126);
						cf85 := 'j';
						var v141: multiset<array<bool>> := multiset{v126, v126};
						cf82 := (cf80 * |[cf80, f25, |v141|, f25, cf80]|) + cf80;
						v123[327] := f25;
						v126[238], v123[327] := cf83, cf80;
					case DC39(cf79) =>
						globalState.f8 := p2;
					...
					case DC43(cf88) =>
						...
						v122 := if (if ((p1[|v148| := cf84])[|(if (cf80 in v149) then v149[cf80] else v108)| := p3] in v147) then v147[(p1[|v148| := cf84])[|(if (cf80 in v149) then v149[cf80] else v108)| := p3]] else cf83) then v122 else v122 * v122;
						v123[152] := 0x39d;
						var v150: map<int, int> := map[|"tt"| := --p2];
						v123[152] := (if (cf82 in v150) then v150[cf82] else -cf80) % 0xef;
						var v151 := DC32(seq(0xf2, i26  => (v146)));
						var v152: multiset<string> := multiset{v108, v151.cf65, v108, v108 + v108, seq(926, i27  => (v146))};
						v126, cf83, v152 := v126, cf84, multiset{v108, v108, v108};
						v150 := v150[p2 := v123[152]];
				}
				
				var v153: array<string> := new string[27];
				var v154 := 'c';
				v153[754] := (if (v126[238]) then v108 else "kyh")[cf82 := v154];
				v153[754], v123, v126 := "gxbbxq" + v108, v123, v126;
				var v155 := new C0(v126[238], f14);
				cf84, cf84, cf84 := !!fm0(p2 % p2, f14, globalState), v126[238], !cf84;
			case DC41() =>
				r1 := p2 == (f25 - |v108|);
				globalState.f5 := true;
			...
			case DC39(cf79) =>
				...
				match (v169) {
					case DC61() =>
						var v170 := 'n';
						var v171 := DC23({v123}, fm2(v126[238], globalState), v170);
					...
					case DC60(cf114) =>
						globalState.f7 := v121[|(map v181 : int | v181 in (set v182 : int | (-0x10e <= v182) && (v182 < 0x2e5) :: (v182 + p2)) :: (v181 % |['v']|) := (p2))| + f25];
						var v183: array<map<seq<bool>, bool>> := new map<seq<bool>, bool>[28];
						var v184: map<seq<bool>, bool> := map[p1 := v126[238]];
						v183[101] := v184;
						var v185: array<string> := new string[17];
						var v186: array<D10> := new D10[24](i29 => DC34(([v120])[f25 := v120]));
						v186[254] := fm74(v108, v127, globalState);
						var v187 := 'e';
						var v189: seq<seq<bool>> := [v168[944], p1];
						var v190 := DC22(v168[944]);
						var v191: seq<set<bool>> := [v120, v120, v120];
						var v192 := DC34(v191);
						v183[101], v168[944], v185, v186[254] := (fm75(--|p1|, v187, globalState) + (map v188 : seq<bool> | v188 in v189 :: (v188) := (v126[238]))) + v184, v190.cf40, v185, v192;
						globalState.f7 := p2;
						globalState.f5 := !p3;
					case DC62(cf115) =>
						v126[238] := f25 != 0x94;
						...
						var v194 := new C5(f14, f14, v193[f25]);
						v123[47] := f25;
						var v195 := 'd';
						v123[47] := -(-0x19 * -|v108[p2 := v195]|);
				}
			...
			case DC43(cf88) =>
				if (f13) {
					...
					globalState.f7 := -f25;
					globalState.f8 := fm25(f25, {f14}, v126[238], globalState);
					r1 := (f13 !in p1) || !!f14;
					v123 := v123;
					f13 := !!f14;
				}
				...
				v126[238] := p2 >= (|v197| / |v108|);
				globalState.f7 := f25 % p2;
				globalState.f7 := f25;
		}
		...
		var v202: seq<string> := [("dusn")[p2 := v201], v108, seq(0x6, i31  => (v201)), v108];
		f13 := (-0x2b9 - f25) != |v202[p2]|;
	...
	method m4(p0: bool, p1: seq<int>, globalState: GlobalState) returns (r0: array<bool>, r1: int) {
		...
		v6[304] := if (p0) then f13 else true;
		var v7: set<bool> := {f14};
		f13, v6[304], globalState.f5 := fm9(v4, globalState), !!fm0(f25 + |multiset(v1)|, f14, globalState), f14 && !(v7 < v7);
		var i0 := 0;
		while (!v6[304])
			decreases 100 - i0
		{
			...
			var v13 := new C4(!true, v6, v6[304], multiset{v8, {f25, |v11|}} >= v12);
			globalState.f5 := true;
			...
			var v17: array<map<bool, map<bool, bool>>> := new map<bool, map<bool, bool>>[23] [v14, v15[f25], v14 + v14, v14, v14, v14, v14, v14, fm76(|v8|, f25, v13.f31, v13.f31, globalState) + v14, fm76(f25, |multiset{p0}|, p0, v13.f31, globalState), v14 + v14, v14, v15[f25], if (v6[304]) then if (f25 in v16) then v16[f25] else v14 else v14, v14[p0 := v0], v14, v14, v14 + fm76(f25, f25, v6[304], f14, globalState), v14, map[f13 := v0] + v14, v14, v14 + v14, v14 + map[f14 := v0]];
			v17[324] := v14;
			v17[324] := v14;
			var v18 := DC2(f25, v13.f31, v6[304]);
			var v19 := new C1(fm28(v18.cf5, v13.f31, v13.f31, false, globalState) + (seq(0x2, i1  => (v4))), f25);
		}
		var v20: map<int, bool> := map[f25 := fm0(|p1|, f13, globalState)];
		var v21: map<bool, map<int, bool>> := map[p0 := v20];
		var v22: set<int> := {f25, -f25};
		var v23: array<map<int, bool>> := new map<int, bool>[24] [v20, v20, (map[f25 := v6[304]])[f25 := f14], v20, v20, v20, v20, if (p0 in v21) then v21[p0] else v20, v20, v20, v20, map[|v5| := !v6[304]], v20, v20[|v1| := v6[304]], v20, map[|p1| := f13], v20, v20, v20[|v22| := v6[304]], v20, v20, v20, v20, fm54(f14, |map[fm25(f25, v7, p0, globalState) := v6[304]]|, globalState)];
		var v24: array<array<map<int, bool>>> := new array<map<int, bool>>[6] [v23, v23, v23, v23, v23, v23];
		v24[277] := v23;
		v6[304], v24[277] := !(v6[304] <== v6[304]), v23;
		globalState.f8 := f25;
		v4 := v4;
		...
		var v29 := DC5(v28);
		var v30 := DC16(v26[|v27|], f25, v29);
		match (DC17(v30)) {
			case DC15(cf31, cf32) =>
				if (if (v1 == v1) then fm0(0x13e, p0, globalState) else !(f25 > -f25)) {
					var v31 := DC12(cf31, v1[f25], p0);
					...
					v6[304], f13, v25[72], v25[72], v36 := p0, (|v22| % f25) <= (|p1| - v36.fm26(v4, v0, globalState)), f25, v25[72] * (v36.fm26(v4, v0, globalState) - 0x4e), if (fm9('j', globalState)) then v37[v36.f28] else v36;
					v27 := v27[f14 := f25];
				} else {
					globalState.f8 := f25;
					cf31 := f14;
					var v38: multiset<int> := multiset{f25, f25, f25, f25};
					var v39 := DC8(-fm25(f25, v7, v6[304], globalState), cf31, f25, f25, if (f25 in v38) then v38[f25] else f25);
					v39 := v39;
				...
				var v46 := new C4(cf31, v45.cf54, fm28(fm25(f25, v7, v44, globalState), v6[304], false, cf31, globalState) == (seq(0x128, i2  => (v4))), f13);
				v6, globalState.f7, globalState.f5, v6[304], globalState.f7 := v6, f25, v4 in "jo", false <==> v46.f31, 0x1b1;
			case DC16(cf33, cf34, cf35) =>
				...
				match (v48.(cf39 := v1 + v1)) {
					case DC22(cf40) =>
						cf33[358] := cf34;
						var v49 := DC3(cf34, v6[304]);
						var v50: map<int, int> := map[|v5| := cf34];
						var v51 := DC16(cf33, f25, cf35);
						var v52: multiset<D4> := multiset{v51};
						cf33[358] := (v49.cf8 % cf34) + (if (f25 in v50) then v50[f25] else |v5[if (v51 in v52) then v52[v51] else f25 := v4]|);
						v6[304] := !((cf34 * f25) >= p1[cf33[358]]);
						cf33[358] := f25;
						cf34 := |v5| / f25;
					case DC23(cf41, cf42, cf43) =>
						var v53: map<bool, string> := map[f14 := seq(29, i3  => (cf42))];
						globalState.f7 := -0x192 + (|(if (!p0 in v53) then v53[!p0] else v5)| + f25);
						var v54 := DC37(f25, f13);
						cf33[829] := v54.cf76 * cf34;
						var v55: map<map<bool, bool>, bool> := map[v0 := v1[f25]];
						var v56: multiset<int> := multiset{cf34, f25, -612, -f25};
						cf33[829], globalState.f8, f13, globalState.f8 := |v55|, (if (f14) then 0x1a0 else cf34) % -(f25 - -|v20|), v56 > v56, cf34;
						cf42 := v4;
						globalState.f8 := cf34 / cf34;
					case DC24(cf44, cf45, cf46, cf47, cf48) =>
						var v57: C4 := new C4(f14, v6, f14, cf47);
						v57, f13 := v57, f13;
						var v58: array<map<seq<int>, int>> := new map<seq<int>, int>[22];
						var v59: map<seq<int>, int> := map[p1 := |fm28(|v22|, f14, if (cf48 in v20) then v20[cf48] else cf47, v6[304], globalState)|];
						v58[686] := v59 + map[p1 := f25];
						v58[686] := fm78(cf46, globalState);
						cf44, globalState.f8 := !v6[304], cf48;
						...
						var v64: set<map<char, int>> := {map[v4 := f25], map v60 : char | v60 in (if (f25 in v61) then v61[f25] else v62) :: (v60) := (-DC24(f14, cf44, cf46, !v6[304], |v20|).cf48), v63};
						...
						v67[755] := if (f14) then set v68 : map<char, int> | v68 in (seq(0xf5, i4  => (v63)))[cf34 := map[v4 := |v1|]] :: (v68) else v64;
						v67[755] := fm79(0x161, globalState);
					case DC21(cf39) =>
						r1 := fm25(f25, {!v6[304], true, true}, true, globalState);
						v29 := v29;
						var v69: multiset<seq<bool>> := multiset{cf39};
						var v70 := DC45(f25 - |v69|, f25);
						var v71: multiset<bool> := multiset{f13};
						var v72 := DC59(0x13, p1[|v71|], v4, true, f14);
						var v73: array<char> := new char[9] ['m', 'f', fm2(v6[304], globalState), v4, v4, 'x', v72.cf111, 'x', v4];
					...
					case DC25(cf49) =>
						...
						var v77 := DC72(v76[v75 := fm25(f25, {v6[304]}, f13, globalState)]);
						var v78 := new C4(!p0, v6, v75 !in v77.cf127, v6[304]);
						var v79: seq<array<bool>> := [if (true) then v6 else v6, v6];
						var v80: multiset<bool> := multiset{f14};
						v6 := v79[|(v80 + v80)|];
						v25[796] := -(v78.fm10(|(map v81 : multiset<int> | v81 in multiset{multiset{f25}} :: (v81) := (cf34))|, false, globalState) / f25);
						v25[796], globalState.f7, v75, v5 := cf34, f25, v75, v5 + (if (v6[304]) then v5 else v5);
			...
			case DC14(cf30) =>
				...
				if (p0 ==> p0) {
					...
					v85 := v85;
					var v86 := new C3(v6[304], v6, f13, if (f14) then true else f14);
					v25[161] := f25;
					v25[161] := f25;
					var v87: map<seq<bool>, int> := map[v1 := f25];
					v6[304] := v87 == v87;
				} else {
					var v88: array<char> := new char[18] [v4, v4, v4, v4, v4, v4, v5[f25], v4, 'q', v4, v4, v4, v4, v4, v4, v4, v4, 'd'];
					...
					v93 := v93[v91 := v92];
					var v94: seq<string> := [seq(932, i5  => (v4)), fm51(v7, f25, globalState)];
					globalState.f5 := v94 <= v94;
					v92 := v92;
					v1 := v1 + v1[4 := v6[304]];
					var v95 := DC37(f25, false);
					v1, v6[304] := v1, v95.cf77;
				}
			...
			case DC17(cf36) =>
				...
				v96[564] := (v5 + v5) + v5;
				var v97 := DC70(v96);
				var v98: map<D24, string> := map[v97 := seq(245, i6  => (v4))];
	...
	method m10(p0: int, globalState: GlobalState) returns (r0: seq<array<bool>>, r1: bool, r2: bool) {
		...
		v3[423] := v4 == (map v5 : string | v5 in v6 :: (v5) := (f25));
		var v7 := DC61();
		v3[423] := match v7 {
			case DC61() => f13
			case DC60(cf114) => f13
		...
		match (v11.(cf36 := v12)) {
			case DC15(cf31, cf32) =>
				var v13: set<int> := {p0};
				var v14 := DC28(v3, |[if (("fp")[if (f25 in v1) then v1[f25] else |v13| := v9] in v4) then v4[("fp")[if (f25 in v1) then v1[f25] else |v13| := v9]] else f25]|, f25);
				v14 := v14;
				var v15: map<int, bool> := map[p0 := v3[423]];
				var v16 := DC36(false, v8, p0);
				var v17: seq<array<int>> := [v8, v16.cf74, v8];
				var v18: map<char, array<int>> := map[v9 := v8];
				var v19: array<array<int>> := new array<int>[17] [v8, v8, if (cf31) then v8 else v8, v8, if (if (p0 in v15) then v15[p0] else f14) then v8 else v8, v8, v8, v17[p0], v8, v8, v8, v8, v8, v8, v8, v8, if (v9 in v18) then v18[v9] else v8];
				var v20 := DC69(f13, |v0|, 0x34c, f14);
				var v21: seq<bool> := [v20.cf121, cf31];
				var v22: map<seq<bool>, array<int>> := map[v21 := v8];
				v19[140] := if (v21 in v22) then v22[v21] else v8;
				var v23: map<bool, bool> := map[v3[423] := cf31];
				var v24: multiset<map<bool, bool>> := multiset{map[cf31 := true], v23 + map[v3[423] := cf31], map[f13 := v3[423]]};
				var v25 := DC1(v1, cf31, [p0], if (|v0| in cf32) then cf32[|v0|] else v3);
				v9, v19[140], v24, r1 := v9, v8, v24[map[f13 := v25.cf2] := f25] - (multiset{v23, v23} + v24), v3[423];
				var v26 := DC3(|v21|, f13);
				var v27: set<D0> := {v26};
				var v28: seq<set<D0>> := [v27, {v26}, v27, v27, v27];
				var v29, v30 := m0(v28 + (seq(0x207, i0  => (v27))), fm0(f25, false, globalState), globalState);
				if (v3[423]) {
					var v31: seq<set<int>> := [v29, v29, {f25}];
					var v32 := DC75(v31);
					var v33: set<bool> := {cf31, f13, v3[423], v3[423]};
					...
					var v38: array<seq<set<int>>> := new seq<set<int>>[28] [v31 + v32.cf132, v31, v31, v31, v31, (seq(-0x282, i1  => (v31[f25]))) + v31, v31, v31, v31, fm81(v9, v0, v21[p0], globalState), v31, v31, v31, v31 + v31, fm81('d', "xaatg", v3[423], globalState), v31, v31, v31, v31 + v31, (seq(0x87, i2  => (v29))) + v31, v31, v31, if (f13) then seq(0x397, i3  => (v29)) else v31[p0 := v13], (if (!f14 in v35) then v35[!f14] else [v13, v13, {p0, -p0}])[p0 := v29], (seq(589, i4  => (v29))) + v31, fm81(v9, v0, cf31, globalState), v31, [set v37 : int | v37 in v36 :: (v37 % -0x16b), v13]];
					var v39 := DC42(v9, f13, v9);
					v38[638] := fm81(v39.cf85, v0, f14, globalState);
					v38[638] := v31;
					globalState.f7 := p0 + p0;
					v30[63] := p0;
					v30[63] := -|map[v30 := cf31]| - f25;
					globalState.f7 := f25 * f25;
					var v40: multiset<bool> := multiset{f13};
					var v41 := new C0(v30[63] > f25, v40 < v40);
				} else {
					globalState.f7 := (if (v3[423]) then f25 else f25) + f25;
					var v42: set<bool> := {f14};
					v8[750] := |v42|;
					v8[750] := f25;
					var v43 := new C1(v0, |(v0 + (seq(0x1dc, i5  => (v9))))|);
			...
			case DC16(cf33, cf34, cf35) =>
				v0 := v0 + v0;
				var v45: multiset<bool> := multiset{v3[423]};
				cf33[722] := if (f13) then cf34 else |v45|;
				cf33[722] := cf34;
				v3[423] := f13 <== v3[423];
			...
			case DC14(cf30) =>
				v0 := v0;
				globalState.f7 := p0 % p0;
				var v47 := new C4(f13, v3, f25 < p0, v3[423]);
				var v48: map<bool, bool> := map[v3[423] := v3[423]];
				globalState.f5 := v48[!v47.f31 := v47.f31] == (v48 + v48);
			case DC17(cf36) =>
				var v49: C5 := new C5(v3[423], f13, v3);
				v8[815] := -|{v0, v0}| + p0;
				var v50: seq<bool> := [f13, v3[423], f14, v3[423]];
				var v51: multiset<seq<int>> := multiset{fm50(v50, globalState)};
				var v52: seq<int> := [-f25];
				v49, globalState.f8, r2, v8[815] := v49, p0, f14, (if (v52 in v51) then v51[v52] else f25) / p0;
				if (v3[423]) {
					var v53 := DC69(!f14, --688 % v8[815], f25 - |v52|, fm9(v9, globalState) <== f13);
					v53 := v53.(cf123 := v8[815], cf122 := -f25);
					v3[423] := f14;
					var v54: array<D9> := new D9[10];
					var v55 := DC32(v0);
					v54[784] := v55;
					v54[784] := v55;
					v1 := v1[v8[815] := v8[815] % f25];
					var v56: set<int> := {p0};
					v3, v8[815], globalState.f7, v8[815] := v3, -p0 - |(v56 - v56)|, v49.fm10(f25, if (f14) then v3[423] else v3[423], globalState), p0;
				} else {
					globalState.f5 := !f13;
					var v57: array<set<bool>> := new set<bool>[29](i6 => {f13} + {v3[423]});
					var v58: map<int, bool> := map[|fm50(v50, globalState)| := v3[423]];
					var v59: set<bool> := {f13, f14, f13, if (|multiset{|v0|}| in v58) then v58[|multiset{|v0|}|] else true, v3[423]};
					v57[384] := v59;
					v57[384] := DC26(v59).cf50;
					v58 := v58[v8[815] := f14];
					var v60: map<bool, int> := map[f13 ==> false := |v0|];
					var v61: multiset<bool> := multiset{f14, f14};
					globalState.f5, v7, v60, globalState.f7 := true, v7, v60, if (f13) then |(v61 - multiset{f13})| else v49.fm10(|multiset{|(seq(437, i7  => (v9)))|}|, false, globalState);
					var v62: multiset<set<bool>> := multiset{v57[384], v59, {fm0(f25, f14, globalState), f14, f14, v3[423]}};
					var v63: seq<set<bool>> := [v57[384], v59];
					var v64: map<int, set<bool>> := map[-0x1b3 := v59];
					var v65: seq<multiset<set<bool>>> := [v62];
					var v66: array<multiset<set<bool>>> := new multiset<set<bool>>[11] [fm82(f25, f25, fm25(f25, v59, v3[423], globalState), v8[815], globalState), v62[v57[384] := p0], v62, multiset{v59, {v3[423]}, v59, {f13}, v59}, multiset(v63) + v62, multiset{v59} - multiset{v59}, multiset([if (p0 in v64) then v64[p0] else v57[384], v57[384]]), v62, v62, v62, v62 + v65[|{v8[815], p0}|]];
					v66[618] := v62;
					v66[618] := fm82(v8[815], v8[815], f25, 0x11c, globalState);
				}
				
				var v67: set<bool> := {f13};
				v0, v3[423], f13 := (v0 + v0)[-(712 * p0) := v9], false, v50[fm25(v8[815], v67, f14, globalState)];
				var v68: map<int, bool> := map[p0 - v8[815] := f25 == v8[815]];
		...
		v70[986] := v3;
		var v71: seq<bool> := [!v3[423], false, fm0(|{v0}|, f14, globalState), v3[423]];
		var v72: seq<array<bool>> := [v3];
		v70[986], v3[423], r1, v71 := v72[f25], f13, f13, v71;
		var v73: map<char, bool> := map[v9 := v3[423]];
		var v74: array<char> := new char[15];
		var v75: map<array<char>, array<array<bool>>> := map[v74 := v70];
		var v76 := DC78(v70);
		var v77: map<bool, array<array<bool>>> := map[v3[423] := v70];
		var v78: map<int, array<array<bool>>> := map[779 := v70];
		var v79: array<array<array<bool>>> := new array<array<bool>>[21] [v70, v70, v70, v70, v70, v70, v70, v70, v70, v70, v70, v70, v70, if (if (v9 in v73) then v73[v9] else v3[423]) then v70 else v70, v70, if (v74 in v75) then v75[v74] else v70, v70, v70, (v76.(cf135 := if (v3[423] in v77) then v77[v3[423]] else v70)).cf135, if (p0 in v78) then v78[p0] else v70, v70];
		v79[203] := v70;
		globalState.f5, v79[203], globalState.f8, v3[423] := f14, v70, f25, !(p0 > f25) && f13;
		var v80 := DC69(true, f25, f25, f14);
		v3[423] := v80.cf121;
		var v81 := DC56(v72);
		r0 := v81.cf106;
		r1 := if (fm0(f25, f13, globalState)) then f13 || f14 else !f14;
		var v82: map<bool, bool> := map[v3[423] := f14];
...
class C7 extends T1 {
	...
	function fm9(p0: char, globalState: GlobalState): bool {
		(if (f13) then DC8(|(seq(0x12f, i0  => (0x369)))|, f14, |(map v0 : int | (-0x27 <= v0) && (v0 < 0x1e8) :: (v0 + |map[f24 := f14]|) := ("nvwk"))|, |map[f24 := f14]|, 70) else DC8(|{|"f"|}|, f14, |{-0x199}|, 262, |[f14, f24]|)).cf17
	}
	method m3(p0: D0, p1: seq<bool>, p2: int, p3: bool, globalState: GlobalState) returns (r0: seq<set<bool>>, r1: bool) {
		...
		if (f14) {
			...
			match (v8) {
				case DC7(cf13, cf14, cf15) =>
					...
					v0, v0[379], v10, v11, globalState.f5 := v0, p2, v10, fm23(globalState), (v7 - v7) > {if (p3 in v12) then v12[p3] else 0x14d};
					cf15 := f24;
					var v13: multiset<string> := multiset{seq(-0x30e, i3  => (cf14))};
					var v14: multiset<int> := multiset{55};
					var v15: array<bool> := new bool[22] [fm0(p2, f14, globalState), false, !p3 && false, v13 > v13, true, cf15, cf15, v14 !! v14, f24, false, f14, if (p3) then f13 else p3, f24, cf15, f24, f13, false, v11 <= ("yucfbp")[0xfa := cf14], f14, cf13 > p2, f14, f14 <== p3];
				...
				case DC8(cf16, cf17, cf18, cf19, cf20) =>
					...
					globalState.f5 := |p1[cf16 := if (p2 in v21) then v21[p2] else v20[457]]| < -0xcf;
					var v22: map<int, seq<bool>> := map[cf16 := p1];
					var v23: seq<array<int>> := [v0, v0];
					v20[457] := p1 <= (if (|v23| in v22) then v22[|v23|] else [p3]);
					var v24 := "ufjtmta";
					f13 := v24 == "ltbmwxajy";
				case DC6(cf12) =>
					...
					v26 := v26;
				case DC9(cf21) =>
					globalState.f8 := (if (f13) then -p2 else p2) % (p2 * p2);
					globalState.f8 := p2;
					var v30: map<char, seq<bool>> := map['e' := p1];
					var v31 := 'a';
					globalState.f5 := fm0(p2 % p2, (if (v31 in v30) then v30[v31] else p1) != p1, globalState);
			...
			match (v34) {
				case DC79() =>
					...
					globalState.f8 := v35[|v36.cf65|] + |((v37[-p2 := p2])[p2 := |"hxr"|] - v37)|;
					globalState.f7 := p2 / p2;
					var v38: map<int, bool> := map[0xac * p2 := !f24];
					v0[184] := p2;
					v38, v0[184] := (if (f24) then v38[0x30 := f24] else v38)[p2 := true], p2 % p2;
				case DC78(cf135) =>
			...
			v55[175] := v57;
			v55[175] := v57;
			var v58 := DC33(v0[309] + v0[309], v0[309], p2);
			v0[309], v58, globalState.f7 := v0[309], v58, -0x14;
			v56 := v56;
			r1 := p3;
		}
	...
	method m4(p0: bool, p1: seq<int>, globalState: GlobalState) returns (r0: array<bool>, r1: int) {
		...
		if (fm9(v0, globalState)) {
			...
			v3 := v3[v1 >= v1 := f14];
			var v4: map<int, int> := map[-v1 := v1 / 0x1c5];
			v4 := v4 + v4;
			globalState.f7 := v1;
			var v5: seq<bool> := [f13];
			...
			match (v15.(cf38 := v14)) {
				...
				case DC18(cf37) =>
					...
					globalState.f7 := v1;
				case DC20(cf38) =>
					var v16 := new C3(p0, v6, v5[|v2|], |v4| <= v1);
			...
			var v17 := 0x18a;
			globalState.f8 := v17 * (v17 % 0x155);
			var v18: multiset<bool> := multiset{if (f24) then f13 else f13};
			v18, globalState.f8, globalState.f7, f13, globalState.f8 := v18[f13 := v17], v17, v17, f13, v17;
			globalState.f7 := -0x20;
			var v19 := "hdnkcvjd";
			var v20 := DC32(v19);
			if (v0 !in (v20.cf65 + v19)) {
				...
				f13 := false;
				var v22: array<int> := new int[19](i1 => i1 / v17);
		...
		v28 := match DC12(f24, f24, !f13).(cf26 := f24, cf28 := f14) {
			...
			case DC10(cf22) => (seq(0x29d, i2  => (v0))) + v28
			case DC13(cf29) => v28
		};
		globalState.f5 := p0;
		...
		r1 := if (p0 in v33) then v33[p0] else 0x1b0 % v30;
	}
}
...
class C8 extends T2 {
	...
	function fm10(p0: int, p1: bool, globalState: GlobalState): int {
		(if (!f13) then -400 else |{f22}|) / (0x34c % |(seq(-0x28f, i0  => (map[|{0x2a2, -0x46}| := |[f22, true]|])))|)
	}
	function fm8(p0: int, globalState: GlobalState): char {
	...
	method m5(p0: int, globalState: GlobalState) returns (r0: seq<int>, r1: set<seq<char>>, r2: bool) {
		...
		if (v0.cf24 <== (|v1| >= p0)) {
			...
			var v3 := new C6(p0 / p0, DC11(|v2|, f13, p0), f22, f14);
			...
			var v13: seq<int> := [0x41, v4[660]];
			var v14: set<bool> := {f23};
			var v15: array<multiset<bool>> := new multiset<bool>[21] [v5, v5[f23 := 413], v5, v5, v5[f23 := v3.f25], fm67(f13, v8.cf77, |v9|, globalState), v5, v10[p0] * v5, v5, v5, v5, v5, v5[f22 := if (f23 in v12) then v12[f23] else |v13|], v5, v5, v10[p0] * v5, v5, fm67(false, f23, v3.f25, globalState) - v5, fm67(f13, v9[-p0], |v11|, globalState), v5 + v5, fm67(f23, v9[-|v2|], |v14|, globalState)];
			...
			var v22: array<int> := new int[10] [p0, p0, p0, p0 % |v16|, p0 % p0, if (map["chfcqvski" := true] in v19) then v19[map["chfcqvski" := true]] else p0, 0x3aa, p0 + v21.cf75, p0 * |v16|, -(p0 + |v17|)];
			var v23: set<int> := {p0};
			var v24 := DC6(v23);
			...
			v20[629] := v27[p0];
			v20[629] := v27[fm10(p0, f14, globalState) % |v17|];
			...
			if (v29.cf28) {
				...
				var v32: multiset<int> := multiset{if (v31[|v17|] in v30) then v30[v31[|v17|]] else v20[629]};
				var v33 := 'v';
				var v34: map<bool, multiset<int>> := map[f13 ==> f14 := multiset(v27)];
				f23, v17, globalState.f7, globalState.f8, v32 := v33 in (v17 + v17), v17, v20[629], if (v25.f31 in v30) then v30[v25.f31] else p0, if (f22 in v34) then v34[f22] else multiset{p0, p0};
				globalState.f7 := -v20[629];
				globalState.f7 := v20[629] / v20[629];
				var v35 := DC10(v27);
				var v36 := DC5(map[v20[629] := v33]);
				globalState.f5, globalState.f5, v20[629], v20[629] := fm20(v35, v31 + [v31[v20[629]]], v36, |(seq(0xcf, i1  => (v33)))| + p0, globalState), !!f22 <==> v25.fm9(v17[p0], globalState), -0x195 * p0, 569;
				f13 := f23;
			} else {
				var v37 := DC28(f15, v20[629], v20[629]);
				var v38: seq<array<bool>> := [f15, v37.cf54, f15, f15, f15];
				v38 := v38 + v38;
				var v39: seq<bool> := [f13];
				v39 := ((v39 + v39) + v39)[0x33f := false];
				...
				v43, v20[629], globalState.f5 := v43, -p0, !([v44, DC68(map[f13 := v20[629]]), v44, v44.(cf120 := v45)] != v47[v20[629]]);
				globalState.f7 := |[|map[v20[629] := f14]| != |v17|, v25.f31, f23]|;
		...
		globalState.f5 := 947 <= |(v51 + v51)[0x39c := -0x110]|;
		var v52 := new C2(false, f14);
		var v53 := "l";
		globalState.f8, globalState.f8 := p0, p0 / |(v53 + v53)|;
		...
		if (if (true) then f23 else !f14) {
			...
			match (v56) {
				case DC50() =>
					...
					var v61 := DC10(seq(0x164, i3  => (p0)));
					var v62: seq<bool> := [f23];
					var v63 := DC5(map[-|v51| := v57]);
					var v64: array<bool> := new bool[19] [f23, f23, f15[106], v52.fm9('t', globalState), fm20(v61, v62, v63, 0x2d1, globalState), f14, true, true, f13, f13, f23, true, f15[106], f14, true, !f22, f14, f23, f22];
					v60[703] := [v64];
					v60[703] := [v64];
				case DC51(cf96, cf97, cf98, cf99, cf100) =>
					...
					cf98 := {multiset{cf99}, v66[v65[587] := 0x1d9], (DC65(v66).(cf117 := v66)).cf117} > fm83(f14, -256, globalState);
				...
				case DC49(cf95) =>
					...
					var v74: set<D0> := {DC1(map v72 : int | v72 in v51 :: (v72 / p0) := (p0), f14, seq(0x303, i4  => (-0x3d7)), f15), v73};
				...
				case DC52(cf101) =>
					...
					f15[914] := (v79 - multiset(v80)) > multiset{f14};
					var v81 := 'j';
					...
					var v86 := new C4((seq(204, i6  => (p0))) == v51, v85, f22, f22);
			}
			...
			if (v88[p0]) {
				globalState.f7 := |v88| % p0;
				f15[894] := if (f23) then f14 else f14;
				f15[894] := !((f23 !in fm56(p0, globalState)) <== f14);
				var v89 := 'k';
				var v90 := DC5(map[0x23 := v89]);
				globalState.f5 := fm20(DC10(v51[p0 := p0]), v88 + v88, v90, p0, globalState);
				var v91: set<int> := {p0};
				var v92: map<set<int>, bool> := map[v91 := f13];
				v92 := v92;
				v88 := v88[p0 := f15[894]] + (v88 + v88);
			} else {
				globalState.f8 := p0;
				var v93: multiset<int> := multiset{0x239 % p0, if (false) then p0 else p0, p0};
				...
				var v101: array<map<multiset<D15>, string>> := new map<multiset<D15>, string>[9] [v98[p0] + v97, v97, v97, v97, if (p0 in v99) then v99[p0] else v97, v97[v100 := v53] + v97, v97, v97, v97];
				v101[253] := v97 + v97;
				var v102 := 'b';
				globalState.f5, globalState.f5, v101[253], globalState.f5 := f22, v52.fm9(v102, globalState), v97, f14;
				var v103: array<C6> := new C6[27];
				var v104: C6 := new C6(|v1|, v0, f22, f14);
				v103[470] := if (f13) then v104 else v104;
				...
				v103[470], v95[154], f22, globalState.f5, v53 := v104, v106[-p0], [v107, DC19()] < ([v107, v107] + v108), !({|v109|, |v53[-p0 := v102]|} !! v110), "ilxakikjs";
			}
			...
			match (if (f13) then v111 else v111.(cf76 := |v53|)) {
				...
				case DC36(cf73, cf74, cf75) =>
					...
					var v119 := 'q';
					v118 := v118[cf75 := fm41(v119, cf75, 0x1a6, cf75, globalState)];
				case DC37(cf76, cf77) =>
					var v120 := 't';
					v120 := v120;
					f15[16] := cf77;
					...
					v122 := v122[f15[16] := v121];
					var v123: set<bool> := {f13};
					var v124: array<bool> := new bool[10];
					var v125: array<int> := new int[17] [cf76, |v123|, p0, cf76, cf76, -p0, |(multiset{v124, v124})[v124 := |v53|]|, p0, p0, |{p0, cf76, cf76, cf76, p0}|, |v51|, p0, -p0, p0, --p0, cf76, p0];
				...
				case DC34(cf69) =>
					globalState.f7 := p0;
					var v128: T1 := new C7(f14, !true, !v88[0x2ff]);
					v128 := v128;
			...
			if (fm0(0x3ae, f22, globalState)) {
				...
				v136[322] := p0;
				var v137 := 's';
				v136[322], v137 := 0x8c, v137;
				var v138: set<bool> := {f23};
				var v139 := new C6(fm25(v136[322], v138, f22, globalState), DC11(v136[322], !f22, |[v136[322], v136[322]]|), f23, f22);
				v136, v136[322] := v136, v139.f25;
			} else {
				...
				f15[702] := p0 == |v146|;
				var v147 := DC82(v142);
				v145[542], f23, f15[702], globalState.f5, globalState.f8 := v142 + v147.cf138, f13, !(if (f22 in v140) then v140[f22] else f14), true, p0 - p0;
				var v148 := new C7(f14, !(!f23 || !f23), !!!f22);
				var v149: set<int> := {p0};
				f13 := v149 >= v149;
				f15[702] := v148.f24 <==> f14;
				f22 := false;
			}
	...
	method m6(p0: bool, globalState: GlobalState) {
			...
			decreases 100 - i0
		{
			...
			var v4: array<int> := new int[12](i1 => i1 % v0);
			v4[121] := 0x373 - v0;
			var v5: seq<int> := [v0];
			var v6 := DC10(v5);
		...
		for i2 := v0 to fm25(v0, v15, f13, globalState) {
			...
			if (multiset(v16) >= v17) {
				...
				v18[989], v20, globalState.f7, globalState.f8, globalState.f8 := v0, v20, i2, v0, i2;
				var v21 := "amakeh";
				var v22: map<bool, string> := map[v14.f32 := v21];
				v22 := v22[v21 != v21 := v21];
				v18[989] := v0;
				globalState.f7 := v18[989];
				var v23 := new C1(v21, v18[989]);
			} else {
				var v24 := 'a';
				...
				var v31: seq<int> := [i2];
				var v32: map<char, bool> := map[v24 := i2 in v31[i2 := i2]];
				v32 := v32[v24 := f14];
				v31 := v31;
			}
			
			globalState.f8 := (0x2fa + |v17|) % v0;
			if (if (v14.f32) then v14.f32 <== !f23 else f22) {
				var v33: seq<int> := [i2, v0];
				var v34 := DC11(v0, f14, v0);
				var v35 := 'q';
				var v36 := new C6(-(v33[i2] * v0), v34, f23, v14.fm9(v35, globalState));
				...
				var v50 := "nqf";
				var v51 := 'r';
				var v52: array<string> := new string[16] [if (v14.f32) then v50 else "anco", seq(0xfc, i3  => ('a')), v50 + "p", (seq(61, i4  => ('b')))[i2 := v51], v50, "ydtjpgl" + v50[v0 := v51], v50 + (seq(0x307, i5  => (v51))), v50 + v50, v50, v50, "mvk", v50, "emgwmmtv", v50, v50, v50];
				v52[181] := v50;
				var v53: map<int, bool> := map[v0 / fm21(v14.f32, globalState) := fm0(v0, true, globalState) <==> v14.f32];
				var v54: array<C6> := new C6[6];
				var v55: multiset<array<bool>> := multiset{f15};
				globalState.f5, v52[181], v53, globalState.f8, v54 := v55 < v55[f15 := v0], v50, v53 + fm54(f14, -0x235, globalState), (|(map v56 : int | (676 <= v56) && (v56 < 0xe8) :: (v56 * v0) := (i2))| * 0x3db) % v0, v54;
				v14.f32 := !v14.f32;
			...
			if (f23) {
				...
				var v72 := "xeuu";
				var v73: map<int, string> := map[v0 % |v72| := v72];
				v73 := v73[-|(if (f14) then v72 else seq(-0x244, i6  => ('o')))| := v72];
				...
				v83[86] := if (v14.f32) then v73 else if (i2 in v84) then v84[i2] else v73;
				f22, v83[86], f23 := v78 !! v78, v73, v14.f32;
	...
	method m3(p0: D0, p1: seq<bool>, p2: int, p3: bool, globalState: GlobalState) returns (r0: seq<set<bool>>, r1: bool) {
		...
		var v2 := DC5(v1);
		var i0 := 0;
		while (fm20(DC10(seq(0x317, i1  => (p2))), p1, v2, p2, globalState) && f23)
			decreases 100 - i0
		{
			if (i0 >= 100) {
				break;
			...
			var v5 := DC68(v4);
			v5 := v5;
			globalState.f8 := (p2 / |p1|) + |(fm56(0x24c, globalState) + p1)|;
			r1 := f14;
			var v6: array<int> := new int[25];
			v6 := v6;
		}
		var v7: array<D7> := new D7[12](i2 => if (p3) then DC29(f23, p2, f22, f13) else DC29(f13, |{p3, f14}|, !true, p1[-p2]));
			...
			decreases 100 - i3
		{
			...
			f15[316] := (if (f14 in v10) then v10[f14] else p2) >= p2;
			var v11: map<int, int> := map[p2 := p2];
			var v12: multiset<int> := multiset{fm21(f15[316], globalState), 0xd0};
			var v13: seq<int> := [if (-fm10(|v12|, !f22, globalState) in v11) then v11[-fm10(|v12|, !f22, globalState)] else p2];
			var v14: map<int, seq<int>> := map[p2 := seq(0x2d, i5  => (|v13|))];
			var v15: array<seq<int>> := new seq<int>[26] [v13 + v13, v13[p2 := --p2], v13, v13 + v13, fm50([f13], globalState), v13, v13, seq(0x170, i4  => (p2)), v13 + v13, v13, v13, v13, [p2] + v13, v13, v13, [0x2fe], v13 + v13, v13, if (p2 in v14) then v14[p2] else [p2], v13, v13, v13 + v13, seq(0x221, i6  => (-581)), v13 + v13, seq(0x289, i7  => (-p2)), [p2]];
			v15[976] := seq(0x242, i8  => (0x3ca));
			v15[976] := v13;
			var v16 := DC77();
			...
			if (p3) {
				...
				v21 := v21[!p3 := p2 % |v23|];
				globalState.f8 := p2;
				var v24: multiset<D7> := multiset{v7[335], v8, v8};
				v24 := v24;
				f15[316] := (-|v21| + p2) >= p2;
		...
		if (f14) {
			if (f14) {
				globalState.f5 := fm0(DC27(|(seq(0x318, i9  => (v0)))|, f14, f13).cf51, !false, globalState);
				var v25: array<map<string, bool>> := new map<string, bool>[28];
				v25[200] := map[seq(0x3c4, i10  => (v0)) := p3];
				var v26 := "yec";
				...
				f15[941] := (set v31 : map<bool, bool> | v31 in v30 :: (v31)) >= v32;
				globalState.f7, r1, f15[941] := p2 + 0x385, DC0(f22).cf0, f13;
			...
			globalState.f7 := (p2 % -0x119) * p2;
			var v45: seq<int> := [p2, p2, p2, p2, 0x14f];
			var v46 := "nijkfr";
			var v47: multiset<bool> := multiset{f23};
			var v48: set<int> := {-(if (!f22) then p2 else p2), v45[-|v46|] * |v47|, p2, 0x2b4};
			...
			globalState.f5, globalState.f5, v52[811], v0 := (v54 + v54) == ({v53} - v54), !p3, f15, v0;
			...
			decreases 100 - i11
		{
			...
			if (f14) {
				...
				v60 := seq(0x175, i13  => (p2));
				globalState.f7 := p2 / p2;
				var v62 := "trmp";
				v62 := "pbsl" + ("j" + (seq(-0x3c3, i14  => (v0))));
				var v63: array<int> := new int[20];
				var v64: seq<array<int>> := [v63, v63];
				var v65: map<array<int>, int> := map[v64[-p2] := p2];
				...
				globalState.f8 := v68.f28;
				var v69: array<int> := new int[20](i15 => i15 / |v68.f27|);
				v69[279] := v68.f28;
				f15[547] := f13;
				var v71: map<int, int> := map[v68.f28 := v68.f28];
				v69[279], globalState.f8, f15[547] := p2 * v68.f28, -p2 + |((map v70 : int | (735 <= v70) && (v70 < 0x194) :: (v70 / v68.f28) := (p2)) + v71)|, v61.f31;
				var v72: map<bool, char> := map[f23 := v0];
				v72 := v72[f13 := fm2(p3, globalState)];
			}
			
			var v73 := "ljy";
			v73 := ("lx" + (seq(195, i16  => ('d')))) + v73;
		}
		var v74: seq<set<bool>> := [v59, v59];
		r0 := [v59] + v74;
		var v75: multiset<bool> := multiset{p3, p3};
		var v76: seq<multiset<bool>> := [v75];
		var v77: map<seq<multiset<bool>>, bool> := map[v76[p2 := multiset{p3, fm0(fm21(f13, globalState), f14, globalState)}] := f13];
	...
	method m4(p0: bool, p1: seq<int>, globalState: GlobalState) returns (r0: array<bool>, r1: int) {
		...
		var v3 := "saw";
		f23, v0 := if (true) then f13 else multiset(p1) > v2, v3[|p1|];
			...
			decreases 100 - i1
		{
			...
			i1 := i1 + 1;
			r1 := fm25(|(map v7 : int | (-0x84 <= v7) && (v7 < 655) :: (v7 / |DC32(v3).cf65|) := ('f'))|, {false}, f14, globalState);
			var v8: array<int> := new int[4];
			globalState.f5, v8, f15[650], f23 := f23, v8, f23, fm23(globalState) != (seq(0xfb, i2  => (v0)));
			v8[527] := |v3|;
			var v9: map<char, char> := map['l' := v0];
			var v10: map<int, map<char, char>> := map[v1 := v9];
			v8[527] := (if (v4.f14) then |v10| else v1) % -v1;
			if (f15[650]) {
				f22 := -v1 != 150;
				var v11: map<int, map<int, int>> := map[v8[527] % v1 := fm43(v1, globalState)];
				var v12: map<int, int> := map[v1 := v1];
				v11 := v11[v8[527] := if (f13) then v12 else v12];
				v8[527] := -v8[527];
				var v13 := DC3(|multiset(DC1(v12, false, p1, v4.f15).cf3)|, f15[650]);
				var v14: set<D0> := {DC3(v8[527], f22), DC3(v8[527], f22), v13, v13};
				var v15 := DC87(v14);
				var v16: seq<set<D0>> := [v14];
				var v17 := DC12(v4.f14, fm0(v8[527], v4.f14, globalState), f13);
				var v18, v19 := m0([v15.cf146, v14] + v16, v17.cf26, globalState);
				var v20 := new C4(v5 != v5, v4.f15, f14, false);
			} else {
				var v21: map<int, char> := map[v8[527] := v0];
				var v22 := DC22(v5);
				...
				v26, f15[650], v3 := v29[v1], v4.fm8(|(seq(-0x193, i3  => (v0)))|, globalState) !in "vcgx", v3[-0x1ad := v0];
				var v30: array<char> := new char[10](i4 => v0);
				var v31: map<int, array<char>> := map[v8[527] := v30];
				var v32: multiset<array<char>> := multiset{if (v8[527] in v31) then v31[v8[527]] else v30, v30};
				var v33 := DC72(v32);
				var v34 := DC74(v33);
				...
				var v39: array<map<bool, multiset<D26>>> := new map<bool, multiset<D26>>[19] [v36, v36, v36, map[v4.f13 := v35], map[v4.f13 := v35], v36 + v36, v37[v8[527]], if (if (v8[527] in v38) then v38[v8[527]] else v4.f13) then v36 else v36, v36, v36, v36 + v36, v36, v36 + v36, map[f22 := v35], if (v4.f13) then v36 else v36, v36, v36 + v36, v36, v36 + v36];
				var v40 := DC10(p1);
				var v41 := DC89(v35);
				v39[271] := v36[fm20(v40, v5, DC5(v21), v1, globalState) := v41.cf149];
				v39[271] := v36;
				var v42: seq<array<int>> := [v8, v8];
				var v43 := DC36(p0, v8, v1);
				var v44: array<array<int>> := new array<int>[29] [v8, v42[0x2ad], v8, v8, v8, v8, v8, v8, v8, v8, v8, v8, v43.cf74, v8, if (v4.f13) then v8 else v8, v8, v8, v8, v8, v8, v8, v8, v8, v8, v8, v8, v8, v8, v8];
				v44 := v44;
				globalState.f8 := -v8[527];
				var v45: map<bool, seq<bool>> := map[true := v5];
				var v46: map<map<bool, seq<bool>>, int> := map[v45 := 755];
				v46 := v46[v45 + map[f15[650] := v5] := |v5| / v1];
			}
		...
		for i5 := fm10(-v1, !f13, globalState) to v1 {
			...
			v47[912] := v1;
			v47[912] := fm21(false, globalState) % |v5|;
			globalState.f5 := f13;
			var v48 := DC32(v3[-v47[912] := v0]);
			v3 := v48.cf65;
			var v49: T0 := new C2(v4.f14, v4.f14);
			var v50: map<bool, T0> := map[v47[912] == v1 := v49];
			v50 := v50[v4.f13 := v49];
		}
		r0 := v4.f15;
		r1 := v1;
...
class C9 extends T0 {
	...
	function fm9(p0: char, globalState: GlobalState): bool {
		f21 !in (if (f13) then [f21, f20] else DC10(seq(0x2f2, i0  => (f21))).cf22)
	}
	...
	method m9(p0: bool, p1: int, p2: bool, p3: multiset<D2>, globalState: GlobalState) returns (r0: array<map<bool, bool>>, r1: bool, r2: int) {
		if (p0) {
			...
			v3[609] := v5;
			var v6 := "yyohfhkh";
			var v7: map<char, int> := map[v6[f20] := p1];
			var v8: seq<bool> := [p2];
			...
			globalState.f7, v3[609] := -(if ((if (f20 in v11) then v11[f20] else v9) in v10) then v10[if (f20 in v11) then v11[f20] else v9] else p1) + f20, v5;
			v4 := v4;
			f21 := 0x3a1;
			f13 := p0;
		} else {
			...
			var v19: seq<multiset<int>> := [v18, v18, multiset{f21}, v18, multiset(seq(0xe2, i0  => (f21)))];
			var v20: array<int> := new int[24] [|v16|, v17[f20], fm18(globalState), fm18(globalState), p1, f20, p1, f21, p1, p1, p1, 451, f20, -p1, -f21, -f21, 0x17d, -|v17| * |v19[0x9c]|, 895, f21, -f21, 0x25d, f20, p1];
			v20[858] := f21;
			v20[858] := 0x64;
			var v21: array<char> := new char[9];
			v21[594] := v14;
			var v22 := DC3(p1, p0);
			v21[594] := (DC7(v22.cf8, v14, f13).(cf15 := !f13)).cf14;
			var v23: set<D0> := {v22, DC3(p1, p2)};
			var v24, v25 := m0([v23, v23], p0, globalState);
			var v26 := DC10(v17[|"f"| := f21]);
			match (v26) {
				case DC11(cf23, cf24, cf25) =>
				...
				case DC10(cf22) =>
					...
					var v43: multiset<bool> := multiset{!v42[f20]};
					var v44 := new C3(|v43| < f20, v40, f13, p0);
					globalState.f8 := if (!(v20[858] <= p1)) then f20 else v20[858];
				case DC13(cf29) =>
					var v45: seq<array<int>> := [v25, v20];
					var v46: array<array<int>> := new array<int>[25] [v20, v25, v25, v25, v20, v20, v20, v20, v25, v20, v25, v20, v25, v20, v25, v20, v20, v25, v25, v20, v20, v45[f21], v20, v25, v25];
					v46[686] := v20;
					var v47: set<bool> := {p2};
					v46[686] := if ({f13} !! v47) then v20 else v20;
					var v48: array<bool> := new bool[21](i4 => p0);
					var v49: map<int, bool> := map[v20[858] := true];
					var v50 := DC73(f13, true, v49);
					var v51 := new C8(f13, if (true) then fm9(v14, globalState) else p2, v48, p0, (v50.(cf128 := f13, cf129 := p2)).cf128);
					v24 := v24;
					v20[858] := -(f21 % v20[858]) * v20[858];
			}
		...
		var v53 := DC45(f21, -(if (v52[-515]) then p1 else f20));
		v53 := v53;
		...
		var v65: map<int, int> := map[if (p1 in v63) then v63[p1] else |v59| := |v64| / p1];
		...
		if (v52 < v52) {
			if (p1 != (p1 - --f20)) {
				...
				var v73: seq<string> := [seq(0x2ac, i8  => (v57)), v59];
				v59, v69[68], v70, v59 := v73[f20] + fm51(v56, f20, globalState), f20, v70 + (v70 + v72), seq(0x65, i9  => (v57));
				v66 := v66;
			} else {
				...
				v74[705] := if (if (f13) then p0 else p0) then v75 else map[f13 := |v76|];
				...
				var v83: array<int> := new int[27] [|v78|, 882, f21, p1, |v79|, f20, f21, if (false in v64) then v64[false] else f20, p1, p1, |v65[p1 := p1]|, f20, p1, f20, |v80|, p1, f21, p1, p1, f21, |v82|, f20, |v59[p1 := v57]|, p1, f21, |v52|, 858];
				v77[540] := if (p2) then v83 else v83;
				var v84: seq<array<int>> := [v83];
				v77[540] := v84[p1 / |v59|];
				var v85 := DC23({v83}, v57, v57);
				var v86: array<char> := new char[27] [v57, v57, v57, v59[|v65|], if (p2) then v57 else v57, v57, v57, v57, v57, v57, v57, v57, v57, 'o', v57, v57, v57, v85.cf42, v57, v57, v57, v57, v57, v57, v57, v57, 'y'];
				v86[184] := v57;
				v86[184] := v57;
				globalState.f7 := p1;
			...
			v89[264] := p2;
			v89[264] := fm0(0x78 + |v52|, f20 != |map[p0 := p0]|, globalState);
			...
			v93[841] := v92;
			v93[841], v89[264] := if (f13) then v92 else v92, f13;
			var v94 := DC76(v89[264], v89[264]);
			var v95 := DC29(p2, f20, f13, false);
			var v96: array<D27> := new D27[23] [v94, v94, v94, v94, v94, v94, v94, v94, v94, DC76(p2, p2), v94, v94, v94, v94, fm85(!p2, v95, globalState), DC76(f13, v89[264]), v94, v94, v94, v94, v94, v94, DC76(p0, p0)];
			v96[182] := v94;
			v96[182] := v94;
		} else {
			var v97 := DC90(v59, p0);
			...
			var v100: array<seq<string>> := new seq<string>[25] [v98, if (p2) then v98 else [v59, v59], v98, v98, v98, v98, [fm28(p1, f13, f13, p0, globalState), v59[f21 := 'o']], [v59, seq(0x2c6, i11  => (v57)), v59, v59, v59], v99[f21], [v59, "gbgk"] + v98, ["cxrx"], v98[f21 := "kpj"], v98, seq(-599, i12  => (seq(0x1d0, i13  => (v57)))), v98, v99[p1], ["g"] + [v59, "uqcb", if (p2 in v58) then v58[p2] else v59, v59], v98, [v59] + [v59], v98, v98, v98, ["ehvrdah", v59], seq(666, i14  => (v59[|v56| := v57])), v98];
			v100[997] := [v59, v59];
			v100[997] := ["ahn"];
			globalState.f5 := f13;
		...
		if (f13) {
			...
			v101 := v101 + v101;
			if (true) {
				var v102: set<int> := {f20, |v59[f20 := v57]|, |"idcph"|};
				...
				globalState.f7 := -|((v104 + v104) * {v103, v103, v103, v105[f21], v103})|;
				globalState.f8 := f21;
				...
				v113[152] := p1;
				r2, v113[152], globalState.f8, v59 := if (!true) then f20 else if (p2) then p1 else p1, 841 + f20, if (f13) then |(v59 + v59)[-782 := v57]| else f20, "bay" + v59;
			...
			var v128: C4 := new C4(p0, v127, p2, v52[f20]);
			var v129: set<int> := {f20};
			var v130: array<bool> := new bool[25] [p2, p0 ==> p0, !(if (p2) then p0 else p0), v52[p1], if (v122 in v126) then v126[v122] else p0, p0, p0, 0x2de > |map[v128 := v57]|, !f13, f13, f13, !(|"c"| > f20), !p0, f13, p1 >= |map[true := f13]|, true, true, p0, p2, if (v128.f31) then p0 else !v128.f31, f13, v59 < v59, v129 !! v129, true, if (p0) then p0 else true];
			v130[357] := f21 == fm21(!p0, globalState);
			v130[357] := fm0(if (p2) then f21 else p1, p0, globalState);
			globalState.f5 := f21 == (f21 % |v64|);
			v59, v59 := v59, v59 + v59;
		} else {
	...
	method m8(globalState: GlobalState) returns (r0: D0, r1: int) {
		...
		for i0 := f19 - f19 to f19 + -0x160 {
			if (f13) {
				...
				var v11: multiset<int> := multiset{if (f13) then f19 else i0, |(v9 + v9)|, |v10| / i0};
			...
			v14[937] := f19 / i0;
			v14[937] := i0 - i0;
			f18 := f18[|(map v15 : int | (0x11e <= v15) && (v15 < 0x281) :: (v15 + i0) := (f13))| < f19 := v14[937]];
		}
		globalState.f5 := if (f13) then f13 else f13;
		if (f13) {
			var v16 := "imgpdwrrx";
			var v17: seq<int> := [f19, f19, f19, f19, f19];
			var v19: array<int> := new int[9] [f19, f19 + f19, |v16|, f19, -f19 * v17[f19], f19, f19, fm14(!f13, set v18 : int | (362 <= v18) && (v18 < -0x142) :: (v18 % |multiset{f19, f19}|), globalState), 0x3c2];
			v19[297] := f19 * f19;
			var v21: map<int, bool> := map[f19 := f13];
			v19[297], r1, f13 := f19 + |(map v20 : int | (0x23f <= v20) && (v20 < 588) :: (v20 / f19) := (f13))|, -|(v21 + v21)| % f19, f13;
			var v22: map<int, int> := map[v19[297] := |[f19, f19]|];
			var v23: seq<multiset<int>> := [multiset{v19[297], f19}];
			var v24: multiset<int> := multiset{if (|(multiset{v19[297]})[|v23[v19[297]]| := f19]| in v22) then v22[|(multiset{v19[297]})[|v23[v19[297]]| := f19]|] else f19};
			var v25 := 's';
			var v26: set<multiset<int>> := {v24, fm17(f19, fm9(v25, globalState), f13, globalState), fm17(0x8a, f13, f13, globalState), v24, v24};
			if (v26 != v26) {
				var v27: set<int> := {f19};
				var v28 := new C9(v19[297], v19[297] - |v27|, v19[297] >= 583);
				v28.f21 := -f19;
				...
				v29[134] := |"ndgpfm"| > |['y']|;
				v29[134] := !((v19[297] == v19[297]) && f13);
			} else {
				globalState.f7 := |map[f13 := v19[297]]|;
				...
				v36 := v36 + v36;
				r1 := 79;
				var v37: map<map<bool, int>, int> := map[f18 := v19[297]];
				v37 := v37[f18 := f19];
			}
			...
			var v39: map<int, array<bool>> := map[f19 % |f18| := v38];
			v39 := v39[-0x15b := v38];
			var v40 := "cc";
			var v41: map<int, bool> := map[f19 := f13];
			var v42: map<int, int> := map[f19 := |v40|];
			var v43: array<int> := new int[24] [|(v40 + v40)|, |v41| % f19, f19, f19, |v42|, f19, -f19, |f18| - f19, -0x2ab, fm14(f13, fm52(globalState), globalState), f19, f19, f19, if (f13 in f18) then f18[f13] else f19, f19, f19 * f19, f19, f19 % f19, f19, f19 - 803, f19, 0x22e, -f19, f19 % f19];
			var v44: map<seq<bool>, int> := map[[f13, f13] := f19];
			var v45: seq<bool> := [f13];
			v43[848] := if (v45 in v44) then v44[v45] else f19;
			v43[848] := f19;
			var v46 := 'k';
			...
			var v50: array<seq<bool>> := new seq<bool>[6] [v48[if ((if (f13 in v49) then v49[f13] else true) in f18) then f18[if (f13 in v49) then v49[f13] else true] else v43[848]], (v45 + v45)[f19 := false], v45, v45, v45, v45];
			v50[922] := v45;
			var v51: map<bool, array<char>> := map[true := v47];
			v47[670], v50[922], globalState.f5, r1, globalState.f5 := v46, v45, f13, |(v40 + v40)[v43[848] := v46]|, |v51| <= f19;
			var v52: seq<int> := [|v41|, v43[848], v43[848], |v40|, 839];
			...
			v53 := fm87(v43[848], v54.cf56 > f19, globalState);
		}
		...
		var v63: map<multiset<D4>, multiset<bool>> := map[v62 := (multiset{!f13})[f13 := f19]];
		...
		v58[878] := f19;
		var v67 := DC32(v55);
		v58[878], v55 := f19 + f19, v67.cf65;
		var v68: multiset<int> := multiset{f19};
		var v69 := DC2(if (|"bkqi"| in v68) then v68[|"bkqi"|] else v58[878], f13, !f13);
		r0 := v69.(cf6 := f13);
		r1 := -((f19 / f19) + v58[878]);
	}
}
...
class C11 {
	...
	method m7(p0: int, p1: bool, globalState: GlobalState) returns (r0: int, r1: array<bool>) {
		...
		for i0 := p0 to v0[p0] {
			...
			v0 := ((seq(64, i1  => (v2.f25))) + (seq(777, i2  => (p0)))) + [-0x134];
			v1[463] := p0 % -i0;
		...
		v5, v6[156], globalState.f7, globalState.f5 := v5[v4 := p1] + (v5 + v5), f17, p0 / p0, (v7 !! multiset{if (p1 in v4) then v4[p1] else p1, p1}) ==> p1;
		for i3 := p0 to p0 {
			...
			v10[201] := v11[if (p1) then |v13| else p0];
			if (p1) {
				...
				var v18 := 'j';
				var v19: seq<string> := [f17, f17, f17, f17[p0 := v18] + fm28(p0, v9.f29, v9.f30, v9.f30, globalState), f17];
				var v20: map<int, set<array<int>>> := map[p0 := {v16} + v17];
				v6[156], v17, globalState.f5 := v19[i3], if (p0 in v20) then v20[p0] else v17, false;
				...
				r0 := |((([[v23], v24] + (seq(40, i5  => (v24))))[p0 := [v23]])[DC2(p0, v9.f30, v25.cf28).cf5 := seq(-0x10c, i6  => (DC30(v22)))] + v26)[i3 * i3 := v24 + v24]|;
				...
				v28[960] := v29;
				globalState.f7 := i3;
				var v30: array<seq<string>> := new seq<string>[19](i7 => [v6[156]]);
				var v31 := 'q';
				var v32: seq<string> := [f17[0x3a := v31], "akjruou", f17, f17];
				v30[273] := v32;
				var v33: map<int, bool> := map[-0x37b := v12[p0]];
				...
				v9.f29, globalState.f8, v30[273], v9.f29 := v34.(cf89 := v35) in map[v34 := v40], p0 / |map[false := v9.f30]|, v32, i3 >= |v41|;
				var v42: multiset<int> := multiset{i3};
				var v43: map<multiset<bool>, bool> := map[v7 := false];
				v6[156] := v6[156][if (|f17| in v42) then v42[|f17|] else p0 := v6[156][|v43|]];
				var v44: set<int> := {p0};
				var v45 := DC6(v44);
				var v46 := DC9(DC9(v45));
				var v47 := DC9(v46);
		...
		var v53: seq<bool> := [p1, p1, 0x2f2 <= -|(map v50 : int | v50 in v52 :: (v50 % p0) := (p1))|];
		v53 := fm56(0x123, globalState);
		var v54: array<int> := new int[11](i8 => i8 % p0);
		v54[227] := p0;
		v54[227] := p0;
		...
		r0 := match v57 {
			case DC66(cf118) => |v0[p0 := cf118]| * cf118
			case DC65(cf117) => -|(map v58 : int | v58 in (seq(0x2ad, i9  => (p0))) :: (v58 / p0) := (0x207))| % |v0[p0 := p0]|
			case DC67(cf119) => |f17|
		};
...
class C12 extends T2 {
	...
	method m5(p0: int, globalState: GlobalState) returns (r0: seq<int>, r1: set<seq<char>>, r2: bool) {
		if (f13) {
			...
			globalState.f8 := fm10(|v2[|v4|]|, f14, globalState) * f16;
			v1 := v1;
		...
		if (if (f13) then v22 != v22 else f13 <==> !f14) {
			...
			var v26: seq<D2> := [v25];
			var v27: seq<seq<D2>> := [seq(67, i0  => (v25))];
			v26 := v27[f16];
			var v28: map<bool, bool> := map[f14 := f13];
			globalState.f8 := (|v28[f13 := f14]| % p0) * (f16 * p0);
			var v29: C11 := new C11("ve");
			v29 := v29;
		} else {
			var v31 := DC75(seq(0x34f, i1  => (set v30 : int | v30 in [p0] :: (v30 % |multiset{--743}|))));
			var v32 := "xifghyyv";
			...
			var v36 := 'x';
			globalState.f8, v31, v32, r2 := f16, if (v32 < v32) then v31 else fm88(|v35[p0]|, globalState), v32[p0 := v36], true;
			globalState.f7 := p0;
		...
		if (f14) {
			...
			f15[71] := f13;
			if (f15[71]) {
				var v70 := DC42(v65[|fm50([v67.f24], globalState)|], fm0(p0, !f14, globalState), v69);
				var v71: multiset<map<bool, bool>> := multiset{map[true := f14]};
				var v72 := DC43(if (!v67.f24) then v70 else DC39(v71[map[f15[71] := f15[71]] := |v65|]));
				var v73: array<set<char>> := new set<char>[3];
				var v74: multiset<bool> := multiset{f14, false};
				var v75 := DC77();
				v72, v73, globalState.f8, globalState.f5 := fm89(|(v74 * v74[f15[71] := 0x188])|, f14, v75, globalState), v73, (|fm50(v42, globalState)| % p0) - p0, f13;
				v39[455] := p0;
				v39[455], globalState.f8, v66 := |fm51(v22, f16, globalState)|, p0, v66 + (v66 + v66);
				var v76: map<int, int> := map[--v39[455] := v39[455]];
				v76 := v76[p0 := v39[455]];
				var v77: map<int, string> := map[p0 := v65];
				v65 := v65[|v77| := v69];
				var v79: seq<int> := [p0];
				var v80: array<bool> := new bool[27](i3 => v42[|v79|]);
				...
				v83 := v83[v65 + v65 := v82];
				v65 := v65[-p0 := v69];
				var v84 := new C7(!f15[71], f14, !v42[p0]);
				var v85: array<bool> := new bool[5];
				var v86: map<bool, int> := map[f15[71] := -f16];
			...
			if (!f14) {
				...
				var v101: map<map<int, bool>, multiset<int>> := map[v97 := v99[|v96| := |v100|] + v99];
				...
				v39[522], globalState.f8 := |v101|, |v107|;
				v39[522] := p0;
		...
		var v123: set<seq<char>> := {v122[f16] + [v120, v120]};
		r1 := v123;
		r2 := !false;
	}
	method m6(p0: bool, globalState: GlobalState) {
		...
		var v4 := 'j';
		var v5: set<char> := {v4};
		var v6: array<int> := new int[23] [f16, |v0|, |v2|, |(map v3 : char | v3 in v5 :: (v3) := (!f13))|, |v1| % f16, f16, f16, f16 - -f16, f16 / f16, f16, -f16 / f16, -DC59(f16, f16, v4, !f13, f13).cf109, f16, f16, f16, f16, -f16 - f16, -f16, f16, f16, -0x189 % f16, -0x177, f16];
		forall i0 | 0 <= i0 < v6.Length {
			v6[i0] := i0 / f16;
		}
		f13 := !f13 && f14;
		var v7: seq<string> := [seq(-0x1f4, i1  => (v4))];
		var v8: seq<int> := [f16, |multiset{v1, seq(0xf0, i2  => (v4))}|, f16, f16, f16];
		var v9: array<string> := new string[23] [v1, v1, v1, v1, v1, "pvinqwpkc", v1, v1, v1, v1 + v1, v1, v7[f16], v1[f16 := fm2(p0, globalState)], v1, v1, v1, v1, v1, v1, fm23(globalState) + v1, "xibtqu", "skkuik", v1[|v8| := v4] + v1];
		v9[763] := v1;
		var v10: seq<set<int>> := [v0, v0];
		var v11 := DC75(v10);
		v9[763] := match v11 {
			case DC76(cf133, cf134) => seq(-0xd7, i3  => (v4))
			case DC77() => v1
			case DC75(cf132) => v1
		};
		globalState.f5 := true;
		...
		v13 := v13[(seq(0x3a4, i4  => (v4)))[|v14| := v4] := f16] + v13;
	}
	method m3(p0: D0, p1: seq<bool>, p2: int, p3: bool, globalState: GlobalState) returns (r0: seq<set<bool>>, r1: bool) {
		var v0 := DC3(p2, true);
		...
		v7[986] := -(p2 % (if (p3 in v10) then v10[p3] else |(seq(0x27c, i0  => ('i')))|));
		var v11 := "xkk";
		...
		var v15: array<seq<int>> := new seq<int>[7] [v14, v14 + v14, ([|[v7[986]]|])[0x112 := v7[986]] + [f16, f16], v14, v14 + [f16, p2], v14, v14];
		v15[958] := v14[|v14| := |"qytlstdy"|];
		v15[958] := v14 + v14;
		v7[986] := 95 / (v7[986] * f16);
		if (f14) {
			...
			match (v16) {
				case DC90(cf150, cf151) =>
					v7[986] := -f16;
					var v17 := 'p';
					var v19 := DC87(set v18 : D0 | v18 in multiset{DC3(v7[986], f14), fm92(p3, globalState), v0.(cf8 := |v11|).(cf9 := fm9(v17, globalState)), v0} :: (v18));
					v19 := DC87(v3);
					var v20: map<int, bool> := map[p2 := f14];
					globalState.f5 := v14[f16] == |v20|;
					globalState.f8 := -((if (!!cf151 in v1) then v1[!!cf151] else v7[986]) - p2);
				case DC89(cf149) =>
					v2 := v2 - multiset(v15[958]);
					var v21: map<int, bool> := map[p2 * p2 := f13];
					v21 := v21[|v15[958]| := fm9('g', globalState) <== false];
					var v22 := 'o';
					var v23: array<string> := new string[10] [fm23(globalState) + v11, v11[v7[986] := v22], seq(675, i1  => (v22)), v11, v11 + v11, v11, "kuyxbyfa" + v11, v11 + v11[|fm46(f16, p3, f14, v7[986], globalState)| := v22], "bonav", (fm23(globalState))[p2 := v22]];
					var v24: map<bool, array<string>> := map[!!(if (p3) then f14 else f14) := v23];
					v23 := if ((v7[986] >= 0x3da) in v24) then v24[v7[986] >= 0x3da] else v23;
					globalState.f5 := !f14;
			}
			...
			v26 := v26[v25 := v7[986] / p2];
			var v27: seq<array<bool>> := [f15];
			var v28: array<array<bool>> := new array<bool>[14] [f15, f15, f15, f15, f15, f15, f15, f15, f15, f15, f15, f15, v27[f16], f15];
			v28[218] := f15;
			v28[218] := f15;
			v28[218][274] := p3;
			v28[218][274] := p1[v7[986]];
		} else {
			...
			if ((p3 <== !p3) ==> f13) {
				...
				v34[4] := f15;
				v34[4] := f15;
				v6 := fm16(false, v7[986], globalState);
				var v35 := DC12(f13, p3, f13);
				v1 := v1[v35.cf28 := v7[986] % |p1|];
				globalState.f7 := -v7[986];
				var v36: seq<array<bool>> := [v34[4]];
				v36 := v36 + v36;
			} else {
				var v37: map<int, bool> := map[p2 := !((seq(-0x1c5, i3  => ('i'))) != v11)];
				v37 := v37[p2 := p3];
				...
				r1 := (v7[986] == p2) <==> (|(map[p3 := -821])[f13 := |multiset{'j'}|]| <= |v37|);
			}
	...
	method m4(p0: bool, p1: seq<int>, globalState: GlobalState) returns (r0: array<bool>, r1: int) {
		var v0 := "qpr";
		var v1 := 'd';
		for i0 := fm10(|v0[|v0| := v1]|, f14, globalState) to f16 {
			f15[782] := f14;
			f15[782] := f13;
			f13 := p0;
			var v3: set<int> := {f16};
			var v4: map<int, bool> := map[|v3| := f14];
			if ((map v2 : int | (0x195 <= v2) && (v2 < 0x1cb) :: (v2 % DC11(|multiset{i0}|, f14, f16).cf25) := (f13)) != (v4 + v4)) {
				var v5: map<int, map<int, bool>> := map[i0 := fm54(f15[782], i0, globalState)];
				var v6 := DC44(v5);
				...
				v8[979] := |map[v1 := i0]|;
				var v9: seq<bool> := [!!f14, f15[782], f14, f14, f14];
				v8[979], globalState.f5, globalState.f8 := i0, if (f16 in v4) then v4[f16] else !v9[f16], i0;
				r1 := |v9|;
			} else {
				globalState.f5 := DC76(p0, p0).cf133;
				var v10: T0 := new C9(f16 / i0, if (f15[782]) then f16 else f16, f16 == f16);
				v10 := v10;
			...
			var v14: set<bool> := {false, f13, f15[782]};
			var v15: map<bool, int> := map[p0 := i0];
			var v16: C10 := new C10(v15, |v0[f16 := v0[0x27a]]|, f13);
		...
		for i2 := f16 to |v0| {
			if (v23.f23) {
				...
				v24[351] := f16;
				v24[351] := i2;
				var v25 := new C11(seq(533, i3  => (v1)));
				v24[351] := i2;
			} else {
				r1 := i2 + (f16 - i2);
				var v26: map<int, char> := map[i2 := v1];
				var v27: set<bool> := {f13, v23.fm20(DC10(seq(0xf4, i4  => (i2))), v20, DC5(v26), i2, globalState)};
				var v28: array<int> := new int[15] [|v0|, i2 % i2, f16, -(f16 * f16), i2, fm25(i2, v27, p0, globalState), i2, fm25(f16, v27, f13, globalState), i2, f16, f16, -i2, |v0|, 0x188, i2];
			...
			globalState.f7 := (|v33| - f16) + (|v0| % i2);
			var v34: array<int> := new int[24];
			v34[869] := |v0|;
			v34[869] := 0x1bb;
			var v35: seq<string> := ["xjncnc", v0, v0];
			var v36: multiset<string> := multiset{"ttrgn", v35[v34[869]], v0};
			v36 := v36;
		}
		for i5 := f16 / -f16 to f16 {
			...
			var v44: array<set<bool>> := new set<bool>[18] [v40 - v40, v40, v40, v40, if (f16 in v43) then v43[f16] else v40, {f15[396], v23.f23}, v40, {f13, v23.f23}, v40, v40, v40, {v23.f23, v23.f22, p0}, v40 + v40, v40, v40, {f15[396], v23.f22}, v40, v40 - {v23.f23, v23.f23, DC31(i5, !v23.f22, v20).cf63, f14}];
			v44[754] := {v20[i5]};
			v44[754] := v40 - v40;
		}
		...
		var v52: map<multiset<int>, bool> := map[v46[f16 := |v51|] := p0];
		var v53 := DC85(f16);
		var v54: seq<D31> := [DC85(|map[f16 := f16]|), v53];
		var v55: array<int> := new int[19] [f16, f16, v45.cf122, if (v23.f23) then f16 else if (v0 in v47) then v47[v0] else |p1|, f16, f16, -f16 - f16, f16, |(v49.cf152 + (if (f14 in v50) then v50[f14] else v49.cf152))|, f16, f16, 0x15b, f16, f16, f16 % |v21|, f16 % |(seq(0x10a, i7  => (v1)))|, f16, f16 / |v52|, -|v54|];
		forall i6 | 0 <= i6 < v55.Length {
			v55[i6] := i6 / |{DC90(v0, v23.f23).cf150}|;
		}
		r0 := f15;
		r1 := -(|[f13]| + f16);
	}
...
class C13 {
	...
	method m1(p0: int, p1: multiset<bool>, p2: bool, globalState: GlobalState) returns (r0: bool, r1: bool) {
		...
		var v8 := "tdufexj";
		globalState.f7, r0, v0[504], v4 := f12, (DC2(f12, v7.cf2, p2).(cf5 := 6)).cf6, v8 == (if (p2) then v8 else v8), v4;
		...
		for i0 := v10 to f12 {
			...
			var v15: array<int> := new int[6] [v6[v10], v10, fm5(globalState), v10, 391, |(if (v0[504]) then fm6(v10, |v12|, p2, globalState) else v14)|];
			v15[2] := |v6|;
			var v16: seq<string> := [v8];
			var v17: array<seq<string>> := new seq<string>[27] [(seq(0x351, i1  => (v8)))[|v8| := fm4(109, globalState)], v16, v16[v10 := v8], v16, [seq(726, i2  => ('y'))], v16 + v16, v16, v16 + v16, v16 + v16, v16[f12 := v8], [v8], (seq(0x1ce, i3  => (v8))) + v16, v16 + v16, v16, seq(132, i4  => ("h")), v16, seq(-0x6c, i5  => ("w")), v16, v16, v16, v16, v16, v16, v16, v16[-v13 := v8], fm7(true, fm5(globalState), v8, globalState), v16];
			v17[540] := v16;
			v15[2], v17[540], globalState.f8, globalState.f8 := fm5(globalState), v16, fm5(globalState), i0;
			v10 := p0;
			r0 := if (!v0[504]) then true else v0[504];
			if ((v15[2] * v10) !in (v5 - v5)) {
				v15[2] := v13;
				var v18 := new C8(v0[504], false, v0, !p2 || v11, if (p2) then p2 else v11);
				v6, v15[2] := v6, f12;
				var v19 := DC11(-103, !p2, fm21(false, globalState));
				var v20: C6 := new C6(v15[2] / p0, v19, v9 == v9, (if (v15[2] in v5) then v5[v15[2]] else v10) >= p0);
				v20 := v20;
				v15[2] := -f12 / (p0 / 0x283);
			} else {
				var v21: set<int> := {|v12|};
				...
				v21 := v22.cf12 * (v23[p0] * fm52(globalState));
				var v24: map<bool, int> := map[v11 := v6[p0]];
				v15[2] := |fm3(v11, v8, v8, v24[v11 := v10], globalState)| % v10;
				var v25 := DC33(v10, 0x342, v15[2]);
				var v26: map<D9, bool> := map[v25 := p2];
				r0 := v12[|v26[v25 := v0[504]]|];
				var v27: map<seq<int>, int> := map[seq(0x2b0, i6  => (105)) := v15[2] + v10];
				v27 := v27[v6 := i0];
		...
		if (p2) {
			...
			var v31: array<array<bool>> := new array<bool>[14] [v30[-v10], v0, v0, v0, v0, v0, v0, v0, v0, v0, v0, v0, v0, v0];
			v31[600] := v0;
			v31[600] := v30[v13];
			var v32 := new C0(f12 <= fm5(globalState), v0[504]);
			var v33 := 'r';
			...
			v11 := p2;
			var v36: set<int> := {v13};
			var v38: array<int> := new int[14] [-0x37c, v13 + v13, 0x394, 0x33a, |(set v37 : int | v37 in v36 :: (v37 + |[0x2ac]|))|, v13, v13, v10, f12, v13 * -0x20d, |v8|, f12 / v13, v13, -(if (p2) then 0x11b else f12)];
			v38[73] := f12;
			v38[73] := fm21(v0[504], globalState) * v10;
			var v39 := DC84(p2, 0x274, f12);
			if (v39.cf140) {
				v8 := v8;
				v11 := fm0(|fm43(v13, globalState)|, v0[504], globalState);
				v8 := v8;
				var v40: array<multiset<int>> := new multiset<int>[17](i7 => v5);
				v40[735] := v5 + v5;
				v40[735] := (multiset{-v13, p0, f12, v13, 575} * v5)[p0 := v38[73]];
				var v41 := 'i';
				v8 := v8[v13 := v41] + (seq(-0x4, i8  => (v41)));
			} else {
				...
				var v46: array<multiset<array<bool>>> := new multiset<array<bool>>[28] [v42, v43.cf157, v42, v42, v42 + v42, v42, v42, v42, v42, v42 * multiset{v0}, v42, v42, v42 - v42[v0 := -v13], v42, v42 - v42, v42, v44[f12], v42 * v42, v42, v42, v42, v42, multiset{v0} * v42, v42, if (v13 in v45) then v45[v13] else v42, v42, v42, v42];
				v46[195] := v42;
				v46[195] := v42;
				globalState.f7 := v10;
				v38[73] := |p1| * fm21(v11, globalState);
				v8 := "i";
				var v47 := 'v';
				var v48: multiset<char> := multiset{v47, 'n'};
				var v49: map<bool, multiset<char>> := map[v11 := v48];
				var v50: map<int, multiset<char>> := map[v38[73] := v48];
				var v51 := DC71(multiset(v8));
				var v52: array<multiset<char>> := new multiset<char>[26] [(if (!true in v49) then v49[!true] else v48) * v48, if (v13 in v50) then v50[v13] else v48, v48, multiset{v47, v47}, v48, fm93(f12, v11, p2, globalState), v48, if (p2 in v49) then v49[p2] else fm93(p0, true, true, globalState), v48 - v48, multiset{v47, v47}, v48, v48, v48, v48, v48, v48, multiset{v47, v47}, (multiset{v47, v47})[v47 := v10], v48, multiset{v47, v47, v47, v47, v47}, v48, v48, v51.cf126, (multiset(v8))[v47 := f12], v48, v48];
				v52[309] := v48;
				var v53: map<int, int> := map[p0 := |v8| / f12];
				var v54: map<bool, string> := map[v11 := v8];
				v52[309], globalState.f8, v38, v53, v8 := v48 - v48, v13 - f12, v38, v53 + v53, (if (v0[504] in v54) then v54[v0[504]] else v8) + v8;
			}
			...
			if (v8 == v8) {
				...
				v10 := (-|map[v38[73] := |v5|]| - v6[f12]) - v38[73];
			}
		...
		v64 := v64;
		var v65 := 'f';
		var v66: map<char, bool> := map[v65 := v0[504]];
		r0 := (v66 != v66) ==> !false;
		r1 := !p2;
	}
	method m2(p0: int, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int, r1: bool, r2: seq<bool>, r3: int) {
		...
		var v2: array<int> := new int[20](i0 => i0 % f12);
		v2 := v2;
		if (!(f12 != p0)) {
			...
			globalState.f8, v3[715], r3, v9[235] := -p0, v10[|v6.f17| * 0x21b], -p0, v7;
			v1 := v1[p2 := 0xdb];
			var v11: seq<bool> := [p1];
			var v12: multiset<int> := multiset{f12, p0};
			var v13: set<bool> := {true};
			var v14: array<bool> := new bool[19] [v11[|v12|], p2, p1, p1, p1, fm0(-|v13|, p2, globalState), p1, p2, !p1, p1, p1, p2, p2, !p1, p2, p1, !p2, false, if (false in v0) then v0[false] else true];
			...
			if (p2) {
				...
				globalState.f7 := f12 / |{v16.f31, v16.f31}|;
				v14[952] := fm0(p0, !v16.f31, globalState) <==> p2;
				v14[952] := p2;
				v14[952] := true;
			}
		...
		var v44: array<map<int, int>> := new map<int, int>[15] [v39, v39, map v40 : int | (0x29 <= v40) && (v40 < 0x17a) :: (v40 / v38[214]) := (f12), v39, if (v42.cf24 in v41) then v41[v42.cf24] else v39, v39, v39, map v43 : int | (179 <= v43) && (v43 < 0x173) :: (v43 % 0x13c) := (0xa3), v39, v39, v39, map[|v1[if (p2 in v0) then v0[p2] else p1 := f12]| := p0], v39, map[p0 := -f12], v39];
		v44[645] := (map v45 : int | v45 in [p0, p0, |multiset(seq(0x3d4, i2  => (DC84(p1, f12, f12))))|, f12, p0] :: (v45 - f12) := (f12)) + v39;
		var v46 := "wj";
		var v47 := 'k';
		v44[645], v46 := v39 + v39, v46[p0 / p0 := v47];
		var v48: seq<int> := [f12, f12];
		var v49: C6 := new C6(-v48[p0], v42, p1, p1);
		var v50: seq<C6> := [v49];
		var v51: map<bool, seq<C6>> := map[p1 := v50];
		var v52: map<seq<bool>, int> := map[[p2, p1, p2] := f12];
		var v53: multiset<char> := multiset{v47, v47, 'b', v47};
		r0 := -(|v51| % (if (p2) then if (v35 in v52) then v52[v35] else |v53| else v49.f25));
		r1 := p2;
		r2 := (v35[v49.f25 := v35[f12]])[p0 := false] + v35[p0 := p2];
		r3 := fm5(globalState);
	}
}
...
method Main() {
	...
	for i0 := |v6| to 911 {
		var v7 := "vmdqtyq";
		v7 := (if (v4) then seq(553, i1  => ('m')) else seq(0x63, i2  => ('m'))) + v7;
		var v8: array<bool> := new bool[20](i3 => v4);
		v8[799] := !v4;
		var v9: multiset<bool> := multiset{v4, v4};
		var v10: set<multiset<bool>> := {v9[v4 := v3], v9[v4 := i0]};
		v8[799], globalState.f5, globalState.f8 := v10 >= v10, false, i0;
		var v11 := 'j';
		v11 := v11;
		var v12 := DC3(|multiset{v4, v4}|, v8[799]);
		v5 := v5[if (v8[799]) then v4 else v4 := (v12.(cf9 := v4)).cf9];
	}
	var v13: multiset<bool> := multiset{v4, v4};
	var v14: seq<bool> := [v13 > v13];
	var v15 := "cyqvbuvx";
	var v16: set<int> := {|v15|, v3, v3};
	v14 := ((v14 + v14) + ([v4] + v14))[v3 := fm1(v4, "huwfv", v3, v16, globalState) > fm1(v4, seq(0x10a, i4  => (v15[v3])), |v13|, v16, globalState)];
	var v17: array<string> := new string[29];
	v17[48] := v15;
	var v18 := 'k';
	v17[48] := ("r")[0x370 := v18];
	var v19: map<string, bool> := map[v15 := false];
	var v20 := DC2(-v3, !v4, if ((seq(0x24c, i5  => ('y'))) in v19) then v19[seq(0x24c, i5  => ('y'))] else !v4);
	v20 := DC2(v3, v3 != v3, !v4);
	var v21: set<bool> := {v4, v4, v3 != v3};
	v21 := v21;
	var v22: array<char> := new char[16] [v18, v15[763], v18, v18, v18, v18, v18, v18, 'p', v18, fm2(v4, globalState), if (!v4) then v18 else v18, v18, v18, v18, v18];
	v22[948] := v18;
	v22[948] := fm2(v4, globalState);
	var v23 := DC3(v3, v4);
	...
	if (match v20 {
		...
		v27[15] := v4;
		v27[15], globalState.f7 := !(v4 <==> v4), -(v3 * (v3 - -v3));
		v27[15] := v3 > v3;
		var v28: map<array<bool>, int> := map[v27 := v3];
		var v29: map<map<array<bool>, int>, bool> := map[v28 + v28 := v4];
		v29 := v29[v28 := v4];
		var v30: map<seq<int>, bool> := map[v6 + (seq(950, i6  => (v3))) := v27[15]];
		v30 := v30[v6 + v6 := v27[15]];
		globalState.f7 := -(-v3 / v3);
	} else {
		v4 := v4;
		globalState.f5, globalState.f8 := v15 <= v17[48], v3 - |v14|;
		...
		globalState.f7, v17[48], v33 := v3 - (|v34| / v3), v17[48], v33;
		...
		v35[306] := v33;
		v35[306], globalState.f8 := v33, (v3 - |v15|) % v3;
	}
	...
	v36[61], globalState.f7, v41 := v41.fm9(v22[948], globalState), |map[0x7b := v41.f22]| % v3, v41;
	if (v41.f23) {
		v18, globalState.f8 := v22[948], v3 % v3;
		var v42: map<int, bool> := map[v3 := v36[61]];
		var v43, v44, v45 := v41.m5(-|v42| - v3, globalState);
		var v46: multiset<int> := multiset{v3};
		v3 := -v6[if (v41.f22) then -0x338 else if (v3 in v46) then v46[v3] else v3];
		var v47: map<bool, array<bool>> := map[v41.f22 := v36];
		var v48: seq<map<bool, array<bool>>> := [v47];
		v36[61], v38.f13, v47, v36[61] := v3 <= |v15|, !v38.fm9(v22[948], globalState), v48[-v3], v41.f22;
		v41.m6(v38.f13 || v36[61], globalState);
	} else {
		globalState.f8 := 0x23;
		...
		if (v3 > fm5(globalState)) {
			...
			v0[872] := fm5(globalState);
			var v53: map<bool, map<bool, int>> := map[v41.f23 := v37];
			v0[872] := --(if (if (v36[61]) then v41.f22 else v41.f22) then v52.f25 else |((if (v41.f22 in v53) then v53[v41.f22] else v37) + v37)|);
		} else {
			...
			v55[100], v17[48], v38.f13, v57, v58 := DC24(if (true in v5) then v5[true] else false, v54.fm9(v18, globalState), multiset{v38.f13, v4}, v41.f22, v3).cf46 * multiset{if (v3 in v58) then v58[v3] else v38.f13}, (v15[-|(fm28(v3, v41.f23, v4, v54.f24, globalState) + (seq(0x264, i8  => (v22[948]))))| := v22[948]])[v3 := v22[948]], v38.f13, v57, (v58 + v58) + ((map v59 : int | (-627 <= v59) && (v59 < 742) :: (v59 + 0x2a8) := (v36[61])) + map[v3 := v41.f23]);
			globalState.f7 := v3 - v3;
		}
		
		if (v36[61]) {
			var v60: map<int, bool> := map[v3 := v41.f23];
			v60 := map[|(v17[48] + v17[48])| := "bljayf" != v15];
			var v61: map<int, seq<bool>> := map[-(-v3 + |fm23(globalState)|) := v14];
			v61 := v61[v3 := [v41.f22, true, false, v36[61]]];
			v0[242] := v3 - -v3;
			v0[242] := v3;
			var v62: map<int, map<bool, bool>> := map[v3 := map[true := v41.f23]];
			var v63: seq<map<bool, bool>> := [v5];
			var v64: array<map<bool, bool>> := new map<bool, bool>[17] [v5, v5, v5, v5, map[v38.f13 := v38.f13], v5, if (v0[242] in v62) then v62[v0[242]] else v5, v5, v5, v5, v5, v5, v5, v5 + v5, v63[-v3], v5 + v5, v5];
			v64[192] := if (v38.f13) then v5 else v5;
			v64[192] := map[v41.f22 := v41.fm9(v18, globalState)];
			v19 := v19[v15 := false];
		} else {
			...
			globalState.f7, globalState.f5 := v3 - v6[|v17[48]|], (v68 * v68) > multiset{|v69|, 518};
			var v70: map<char, bool> := map['y' := v65.f14];
			v70 := v70;
			var v71: array<C4> := new C4[23];
			v71 := v71;
			var v72: map<seq<bool>, int> := map[v14 := |v17[48]|];
			var v73, v74 := m0([v24[if (v14 in v72) then v72[v14] else v3]], v41.f22, globalState);
		}
		
		v41.m6(v36[61], globalState);
	}
	
	v0[400] := v3;
	v41.f22, globalState.f7, v0[400] := !v36[61], 0x1e, 301 / fm21(v38.f13, globalState);
	v0[400] := v0[400];
	var v75 := DC59(|v14|, v3, v18, v41.f22, v4);
	var v76: set<D19> := {v75};
	var v77: map<int, set<D19>> := map[v0[400] := v76];
	v36[61] := (if (v0[400] in v77) then v77[v0[400]] else v76) <= v76;
	globalState.f7 := -v0[400] % v0[400];
	forall i9 | 0 <= i9 < v17.Length {
		v17[i9] := "lxgelo" + v15;
	}
	var v78 := DC10(v6);
	var v80 := DC5(map v79 : int | v79 in v6 :: (v79 % |{'y'}|) := (DC7(v3, v22[948], v4).cf14));
	var v81 := new C3(if (v41.f22) then v41.f23 else !!v41.fm20(v78, v14[v0[400] := v38.f13], v80, v3, globalState), v36, true, 149 <= |{v18, v22[948]}|);
}