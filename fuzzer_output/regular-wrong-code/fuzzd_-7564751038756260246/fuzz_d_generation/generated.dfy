...
function fm5(p0: int, p1: bool, globalState: GlobalState): map<int, int> {
	map[|[false, true, !true, !false, false]| := --(|[true]| % 0xc0)]
}
...
function fm9(globalState: GlobalState): D3 {
	if (false) then DC4(seq(0x360, i0  => ('n'))) else DC4(seq(-0x193, i1  => ('x')))
}
function fm10(p0: bool, p1: int, p2: int, globalState: GlobalState): map<map<int, int>, bool> {
	match if (false) then DC15(--|map[0x3d7 := |map[false := true]|]|, |map[false := multiset{true}]|) else DC15(|[[!false]]|, |"ujipcla"|) {
		...
		case DC14(cf20) => map[map v1 : int | v1 in map[0x33 := true] :: (v1 * |{false, false}|) := (0x1a4) := false] + map[map v2 : int | v2 in multiset{|(map v3 : D2 | v3 in multiset{DC3([true, true])} :: (v3) := (-0x21e))|, |(map v4 : int | v4 in [|(seq(0x2a0, i0  => ('a')))|] :: (v4 + 0x195) := (190))|, |(seq(344, i1  => (83)))|, 0x15e} :: (v2 / |{!true}|) := (-|map[false := |[true, false, true]|]|) := !true]
		case DC17(cf28) => map v5 : map<int, int> | v5 in [map[|[true]| := |{0x292, |(seq(528, i2  => ('j')))|, -688, -524, -555}|], map[|multiset([|"fvgbw"|, |map[true := 0x18a]|])| := -341]] :: (v5) := (false)
	}
}
function fm11(p0: int, p1: int, p2: map<bool, int>, globalState: GlobalState): map<bool, int> {
	map[false := -|(set v0 : int | (0x1ec <= v0) && (v0 < 0x349) :: (v0 - -|[0x100]|))|] + (map[!!true := |map[|"orpdwcbhu"| := |(seq(893, i0  => ('g')))|]|] + map[false := |(map v1 : int | v1 in [|multiset{false, false}|] :: (v1 + 0x1bd) := (|map[true := 908]|))|])
}
method m0(p0: bool, p1: int, p2: char, p3: bool, globalState: GlobalState) {
		...
		decreases 100 - i0
	{
		...
		var v10: array<int> := new int[21] [p1, p1 / p1, v3.cf7, p1, -p1, p1, p1, |v4|, |v5|, p1, |v4|, p1, 0x20e, p1, p1, p1 / |v7[p1 := v6]|, p1, |v4|, p1, |(v9 + v9)|, p1];
		v10[786] := |v0[p3 := true]|;
		v10[786] := p1;
	}
	var v11: seq<int> := [-p1];
	...
	var v16: map<int, bool> := map[v11[|v12|] := v14 in v15];
	var i1 := 0;
		...
		decreases 100 - i1
	{
		...
		if (!!!p0) {
			...
			var v20: array<bool> := new bool[22](i2 => |multiset{p3}| < v11[p1]);
			...
			v20[304] := -(v11[|v22|] - -(if (p1 in v23) then v23[p1] else |v25|)) > |v17|;
			globalState.f8 := 0xb6;
			var v26: set<C0> := {v13, v13};
			var v27: map<int, set<C0>> := map[p1 := v26];
			v20[304] := v26 !! (if (p1 in v27) then v27[p1] else v26);
		} else {
			var v28 := false;
			globalState.f8, v28, v12, v28 := v11[0x88 + p1], !v28, v12, p2 in v12;
			...
			var v36: array<int> := new int[19] [p1, 293, p1, if (p2 in v33) then v33[p2] else p1, |(v11 + v11)|, p1, -fm4(p0, p1, globalState), p1, p1, |map[v32 := |v11|]|, -DC5(false, 0x296).cf7, p1, 185, |v34| + p1, p1 % p1, |(seq(0x35e, i3  => (0x146)))|, p1, -257 % p1, |(v35 + map[p1 := p1])|];
			v36[249] := 506;
		...
		var v41: map<array<int>, string> := map[v40 := v12 + v12[p1 := 'e']];
		var v42 := DC5(true, p1);
	...
	match (fm9(globalState)) {
		...
		case DC4(cf5) =>
			...
			var v46 := DC10([p1]);
			v45 := v45[(v11[|v46.cf14| := v11[p1]])[p1 := p1] := "imvdwrgjx"];
			var v47 := new C0();
			globalState.f0 := p1;
			var v48: array<int> := new int[10];
			v48[289] := |cf5| + |(map v49 : int | (0x190 <= v49) && (v49 < 0x25a) :: (v49 / p1) := (p2))|;
			v48[289] := p1;
	}
	...
	for i4 := p1 to -(p1 + p1) {
		...
		var v55: map<array<bool>, bool> := map[v54 := v51[p1]];
		globalState.f0, globalState.f2, v53[646], v55, globalState.f8 := -((if (p0) then p1 else i4) + (i4 % i4)), if (p3) then v12 else ("br")[p1 := p2] + v12, i4 % p1, v55, i4 / (p1 - p1);
		if (false) {
			v54[643] := p3;
			v54[643] := if (!!p3) then !(p1 != i4) else false && p0;
			var v56: map<bool, bool> := map[p0 := v54[643]];
			var v57: map<int, map<bool, bool>> := map[-v53[646] := v56];
			v57 := v57[-i4 := v56];
			v54[643] := p3;
			...
			var v64 := true;
			v64 := v53[646] != i4;
			v64 := v11 < (v11 + v11[v53[646] := p1]);
			var v65: seq<multiset<C0>> := [multiset(v14) * multiset{v52}, multiset(if (p0) then v14 else v14), multiset((if (p0) then [v52] else v14)[p1 := v13])];
			v13, v52, v65 := v13, v13, v65;
			var v66 := DC8();
			v66 := v66;
			var v67: array<seq<map<bool, int>>> := new seq<map<bool, int>>[11];
			var v68: map<bool, int> := map[p0 := |multiset{v51[p1]}|];
			var v69: seq<map<bool, int>> := [fm11(|v11|, v53[646], v68, globalState)];
			v67[913] := v69;
			var v70: set<int> := {p1};
			v64, v67[913], v53[646], globalState.f0 := if (v12 <= v12) then v64 else p0, v69, v53[646], v53[646] / |v70|;
		}
		...
		var v73: map<C0, bool> := map[v13 := v54[400]];
		var v74 := DC14(v73);
		var v75: map<multiset<int>, int> := map[v50 := |v74.cf20[v13 := v54[400]]|];
		v72 := v72[|v75| < -p1 := v54[400]];
	}
}
class C0 {
	constructor () {
...
method Main() {
	...
	match (v11) {
		case DC0(cf0) =>
			if (!(!(v9 > v9) <== v2)) {
				m0(if (v2) then fm0(globalState) else v8[v6], |multiset(v8)|, v4, v2, globalState);
				var v12: set<bool> := {false};
				var v13: seq<int> := [|v8|];
				var v14: map<set<int>, int> := map[v9 - {|v12|, -0x134} := v13[v6] + v6];
				...
				v1[178] := fm2(v18, v2, v6, globalState);
				var v19: set<bool> := {v2};
				var v20: map<char, string> := map[v4 := seq(0x1ae, i1  => (v4))];
				var v21: map<set<bool>, bool> := map[v19 := v20 != v20];
				v21 := v21[v19 := v2];
				globalState.f0 := v6;
				globalState.f2 := v0;
				m0(v8[v6], v6, v4, v2, globalState);
			}
			...
			if (v22.cf1) {
				...
				var v29: map<C0, seq<char>> := map[v26 := if (!v2 in v28) then v28[!v2] else seq(0x17, i2  => (v4))];
				var v30: array<seq<char>> := new seq<char>[8] [[v4, 'g', v4, v4], if (v26 in v29) then v29[v26] else seq(-93, i3  => (v4)), v0, v0, v0, seq(123, i4  => ('t')), v0, v0];
				v27[264] := v30;
				v1[325] := v4;
				var v31: array<C0> := new C0[27];
				var v32: map<array<C0>, array<seq<char>>> := map[v31 := v30];
				v27[264], v23, v1[325], globalState.f0 := if (v31 in v32) then v32[v31] else v30, v23, v4, v6;
			} else {
				var v33: array<array<char>> := new array<char>[27];
				v33[214] := v1;
				v33[214], globalState.f8 := v1, -(v6 + v6);
				...
				v9, v6, globalState.f0, v2, v35 := v36[v6 * v6], fm4(v2, v6, globalState), v6 / v6, (|v37| == |v38|) ==> (v2 && v2), (v35 + v35[v2 := true])[fm0(globalState) := v2];
				var v39: seq<map<int, int>> := [v37];
				var v40: map<int, bool> := map[v6 := v2];
				var v41: map<int, bool> := map[|{if (v6 in v40) then v40[v6] else v2}| := v2];
				var v43: array<map<int, int>> := new map<int, int>[10] [v37, v39[0x2b], fm5(0xb7, v2, globalState), v37 + v37, v37, v37, fm5(|"aywqiw"|, v2, globalState) + v37, map[0x179 := |(set v42 : int | v42 in v41 :: (v42 * 51))|], v37 + fm5(v6, v2, globalState), v37];
				v43[809] := map[v6 := |v8|];
				...
				v43[809], v45, v47, v2 := v37[v6 := v6] + (if (v6 in v48) then v48[v6] else v37), v45, v47 + (v47 + map[v11 := v46]), v6 >= v6;
				v2 := false;
			}
	...
	v3[849] := v2;
	v3[849] := !v2;
	var v49: seq<int> := [fm4(v3[849], v6, globalState)];
	var v50: map<seq<int>, int> := map[v49 := v6];
	var v51: map<bool, int> := map[v3[849] := |v0[0x7a := v4]|];
	var v52: map<map<seq<int>, int>, map<bool, int>> := map[v50 := v51];
	v52 := v52[map[v49 := v6] + (map v53 : seq<int> | v53 in v50 :: (v53) := (|v9|)) := map[v3[849] := v6]];
	if (v2 || (v6 >= v6)) {
		globalState.f8 := v6;
		var v54: map<array<bool>, bool> := map[v3 := v2];
		v54 := (v54 + v54)[v3 := true];
		var v55: seq<string> := ["pnjsvda"];
		var v56 := DC3((v8[|v55| := v2])[|v0| := false]);
		v8 := v56.cf4;
		var v57: C0 := new C0();
		var v58: seq<C0> := [v57];
		var v59: set<C0> := {v57};
		var v60: map<int, C0> := map[v6 := v57];
		var v61 := DC4(seq(0xf4, i5  => (v4)));
		var v62: seq<D0> := [v11, fm6('i', v61.cf5, v2, globalState)];
		m0({v58[v6], v57, v57, v57, v57} !! v59, |(v60 + map[v6 := v57])|, v4, v62 != [v11, v11], globalState);
		...
		var v68: map<bool, seq<D3>> := map[!v3[849] := if (v3[849] in v67) then v67[v3[849]] else [v65]];
		var v69 := DC6(v66);
		v67 := v67[v3[849] := v69.cf8];
		var v70: map<int, bool> := map[v6 := v2];
		v3[849] := v70 != fm7(globalState);
		v3[849] := !v2;
		v9 := v9;
	}
	...
	forall i7 | 0 <= i7 < v71.Length {
		v71[i7] := (map[DC3(v8) := v3[849]] + (map v72 : D2 | v72 in (map v73 : D2 | v73 in (seq(0x35e, i8  => (DC3([false])))) :: (v73) := (map[!true := v6])) :: (v72) := (v3[849]))) + (if (v3[849]) then map[DC3(v8) := !v3[849]] else map[DC3(v8) := v3[849]]);
	}
	var v74 := new C0();
	...
	for i9 := fm4(false, |v8[79 := DC1(v3[849]).cf1]|, globalState) to |v78| * v6 {
		var v79: seq<array<bool>> := [v3, v3, v3];
		v79 := [v3];
		var v80 := DC9(v77);
		var v81: seq<array<int>> := [v77, v80.cf13, v77, v77];
		v81 := v81;
		var v82 := DC5(v2, v49[i9]);
		v3[849] := v6 > (v82.cf7 * -0x318);
		v3[849], v3[849], globalState.f8, v9, v74 := !v2, (v10 * v10) < v10, |(([v3[849]] + v8) + [v2, v3[849]])|, {v6} - v9, v74;
	}
}