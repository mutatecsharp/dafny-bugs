...
function fm17(p0: int, p1: bool, p2: int, p3: bool, globalState: GlobalState): seq<int> {
	(seq(0x284, i0  => (0x398))) + [0x210, 0x2df, 73]
}
...
function fm20(p0: bool, globalState: GlobalState): D4 {
	if (false) then DC14(true, ['e'], true, |[-0x2a0]|) else DC14(false, seq(0x215, i0  => ('h')), true, 0x2b)
}
function fm21(p0: bool, p1: int, p2: int, p3: bool, globalState: GlobalState): map<string, string> {
	map[seq(-939, i0  => ('o')) := "ghevnoh"] + (map["kvlieijo" := "iynkbshu"] + map["qh" := "owl"])
}
...
function fm24(globalState: GlobalState): D0 {
	match DC22(seq(784, i0  => (|map[0x33a := false]|))) {
		case DC23(cf39) => DC0(0x12f)
		case DC22(cf38) => DC0(|multiset{|[false]|}|)
	}
}
function fm25(p0: int, p1: bool, globalState: GlobalState): D4 {
	if (!false ==> !true) then DC14(!true, seq(-0x189, i0  => ('r')), true, -|[true, true, false, false, false]|) else DC14(true, ['t'], false, 0x2e0)
}
...
function fm27(globalState: GlobalState): map<int, int> {
	map v0 : int | (0x22 <= v0) && (v0 < -0x62) :: (v0 % -682) := (--0x3d0)
}
function fm28(globalState: GlobalState): seq<string> {
	if (!(|[true]| !in multiset{|map[|(map v0 : int | (375 <= v0) && (v0 < 0x40) :: (v0 - |map[false := -0x85]|) := (false))| := |(map v1 : char | v1 in ['f'] :: (v1) := (0xc0))|]|, 0x3b2, |"nyibe"|})) then (seq(0x2d6, i0  => ("ivarjnlq"))) + (seq(-0x2db, i1  => (seq(-0xfb, i2  => ('d'))))) else ["h"]
}
...
function fm30(p0: bool, p1: bool, globalState: GlobalState): seq<int> {
	((seq(548, i0  => (0x152))) + [-0x1b8, -656, |{--|multiset([map[true := true], map[true := true]])|}|, |multiset{true, false}|]) + [-|{false}|]
}
...
function fm40(p0: int, p1: char, p2: bool, globalState: GlobalState): D7 {
	DC23(|(seq(-0x2cd, i0  => (|multiset{false}|)))| >= |"plum"|)
}
...
function fm42(p0: int, p1: int, p2: char, p3: bool, globalState: GlobalState): D5 {
	DC18(|map[0x39 := false]| / --0x3ad, 0x34d)
}
function fm43(p0: int, p1: bool, p2: int, globalState: GlobalState): string {
	if (!(0x2f6 >= 0x124)) then (seq(0x240, i0  => ('s'))) + "udraodytg" else seq(804, i1  => ('a'))
}
...
function fm46(p0: bool, p1: bool, p2: int, p3: bool, globalState: GlobalState): map<int, int> {
	map[-0x91 := |{!false, true, false, !false}|] + (map v0 : int | v0 in (map v1 : int | (0x39c <= v1) && (v1 < -0xd5) :: (v1 - |[false]|) := (true)) :: (v0 * |multiset([!false])|) := (--|multiset{|(seq(60, i0  => ('v')))|, --896, 0x281, |"govbc"|, 0x150}|))
}
function fm47(p0: string, globalState: GlobalState): map<map<int, int>, seq<bool>> {
	if ({true} >= {true}) then map[map[-|[|"au"|]| := |[|{|"hwghh"|, 899}|]|] := [false]] else map[map v0 : int | (0x3a6 <= v0) && (v0 < 0x3aa) :: (v0 * 0x70) := (0x352) := [false]] + (map v1 : map<int, int> | v1 in (seq(69, i0  => (map[|"oarr"| := 0x2b9]))) :: (v1) := ([true]))
}
...
function fm55(p0: map<int, int>, p1: int, p2: int, globalState: GlobalState): seq<int> {
	(seq(0xf2, i0  => (--0x362))) + [0x336]
}
function fm56(globalState: GlobalState): multiset<bool> {
	multiset{!(if (!!!true) then false else false), |map[true := 0x3d]| >= |multiset{false, !true}|, false, false, 0x2b6 > |(seq(0x220, i0  => ('y')))|}
}
...
function fm58(p0: seq<string>, p1: int, p2: int, p3: int, globalState: GlobalState): string {
	("kgmp" + (seq(0x136, i0  => ('e')))) + (if (!!false) then "ctvdgg" else "p")
}
...
function fm61(p0: int, p1: bool, p2: bool, p3: int, globalState: GlobalState): map<int, bool> {
	(map[|(map v0 : seq<int> | v0 in [[0x26d, -0x32c], seq(942, i0  => (|map[0x5 := |multiset([0x315])|]|))] :: (v0) := ('l'))| := false] + map[|(seq(0x11d, i1  => (|[-836, 0x37f]|)))| := false]) + map[--|[0x109, |[0x2ac]|]| := false]
}
...
function fm68(p0: bool, p1: bool, p2: bool, globalState: GlobalState): map<int, map<int, bool>> {
	map[|map[|multiset{0x1cb, |"wfuk"|}| := false]| := map[0x17a := !false]] + (map[|(seq(-0x386, i0  => ('k')))| := map[|[false, false, false, true]| := true]] + map[|[|(map v0 : int | (0x1b6 <= v0) && (v0 < 0xe2) :: (v0 - 880) := (false))|]| := map[|[true]| := false]])
}
...
function fm70(p0: int, p1: bool, globalState: GlobalState): D12 {
	DC39(!!({70, |[|map[421 := false]|]|} !! (set v0 : int | (73 <= v0) && (v0 < 0x2) :: (v0 + 0x9e))), 0x353 % -0x3a4)
}
function fm71(p0: int, p1: int, p2: D14, p3: bool, globalState: GlobalState): map<int, seq<int>> {
	map[0x130 / |[!!DC14(false, seq(294, i0  => ('p')), !false, 824).cf24]| := if (true) then [180] else seq(-0x90, i1  => (|"culue"|))]
}
function fm72(p0: int, p1: int, globalState: GlobalState): D10 {
	DC30(seq(0x318, i0  => ('x')), (seq(671, i1  => ('k'))) != "qif", 242 / -0x15f)
}
...
function fm74(globalState: GlobalState): map<bool, map<bool, int>> {
	match DC35(DC32(|(map v0 : int | (0x232 <= v0) && (v0 < 0x9) :: (v0 % 603) := (false))|, 0x3a4)) {
...
function fm75(p0: bool, p1: int, p2: int, p3: int, globalState: GlobalState): seq<set<int>> {
	([{|[717]|}, {-0x3}] + [{-406}, {0x279, -0x4f}]) + (if (true) then [{|map[|"gntq"| := true]|, |(map v0 : int | (0x233 <= v0) && (v0 < 440) :: (v0 + |map[false := |multiset([777, |"tkska"|])|]|) := (0x13))|}] else seq(0x337, i0  => (set v1 : int | (-0x360 <= v1) && (v1 < 84) :: (v1 * 0x283))))
}
...
function fm78(p0: int, p1: int, globalState: GlobalState): map<D20, bool> {
	(map[DC61(map[false := |map[false := false]|]) := false] + map[DC61(map[false := -0x2d2]) := false]) + map[DC61(map[true := |(set v1 : int | v1 in (map v0 : int | v0 in map[-|map[|{!false}| := true]| := true] :: (v0 % |(seq(0x2ff, i0  => ('p')))|) := (|multiset{-0x3e4}|)) :: (v1 * |[|[|[|[true, false]|, 0x312]|, -0x35a, |"pjl"|]|]|))|]) := false]
}
...
method m0(p0: bool, p1: bool, globalState: GlobalState) {
	...
	if (p0) {
		...
		var v10: map<map<bool, char>, string> := map[v9 := seq(-0x313, i0  => ('x'))];
		...
		var v17: map<bool, int> := map[!v12[v1] := v1];
		var v18: array<int> := new int[28] [|v0|, v1, |v3[v1 := fm5(v4, globalState)]|, v1, 896, v1, 0x3e0, v1, v1, |(v6 + {v5})|, v1 % |v7|, v1 - |"nxyn"|, |v8| - v1, v1 - v11.cf84, |v12[v1 := p1]| % v1, if (p0) then v1 else -v1, -(v1 / v1), v1 / -0x1e9, v1, if (p1 in v17) then v17[p1] else -|v0|, v1 % v1, v1, v1, 912 + v1, v1 % v1, 0x267, v1, v1];
		v18[798] := -v1 * v1;
		globalState.f1, v18[798], v5 := p0, --((|fm19(p1, globalState)| / -0x25a) % |"uebbsqjp"|), v5;
		v18[798] := 936;
		...
		v20[281], v22, globalState.f1, v23, globalState.f4 := v19, v22, p0, v23, -(v1 + (-v18[798] - v1));
	} else {
		...
		v27[734] := -0x6f;
		v27[734] := if (p0) then |map[v0 := v0]| else v1 % v1;
		var v28: multiset<int> := multiset{v27[734], v1};
		var v29: set<bool> := {p0, p1, !p0, p0, !p0};
		var v30: map<bool, multiset<int>> := map[p0 := v28[|v29| := -0x3b8]];
		v30 := v30;
		...
		decreases 100 - i3
	{
		...
		var v44: set<C2> := {v43};
		var v45 := DC71(|((if (v1 in v38) then v38[v1] else v37) + v37)|, v39, v40, (v41[|v44| := v0])[v1 := v0]);
		match (v45) {
			case DC71(cf113, cf114, cf115, cf116) =>
				globalState.f1 := (if (p1) then p1 else p0) && (if (p0) then p0 else p1);
				var v46: map<string, char> := map["llt" := v0[v1]];
				...
				var v50: array<seq<char>> := new string[21] [v0, v0, v0, v0, if (p1) then v0 else [v0[759], v43.f24, v43.f23, v43.f24], v0 + v0, [v43.f23, v32.fm8(v1, cf113, v49, globalState)], v0, v0, seq(0x97, i4  => (v42)), v0 + ['c', v43.f24, v43.f24], v0 + v0, v0, v0, v0, v0, [v43.f23], v0, v0, v0, v41[0x20e]];
				v50 := v50;
			case DC72(cf117, cf118, cf119, cf120, cf121) =>
				var v51: array<string> := new seq<char>[20](i5 => cf121);
				var v52: T2 := new C7(|cf121[v1 := 'm']|, v51, p0, v33);
				var v53: map<string, T2> := map[v0 + v0 := v52];
				var v54: seq<T2> := [v52];
				v53 := v53[cf121 := v54[cf118]];
				var v55: seq<seq<bool>> := [v52.f18, v52.f18, v33, v33];
				var v56 := DC85(v55);
				var v57: multiset<bool> := multiset{p0};
				globalState.f1, globalState.f4, globalState.f1 := v43.fm6(v0, if (p0) then cf118 else 0x21d, |(if (p0) then v55 else v56.cf145)|, v43.f23, globalState), --v1, (v1 % cf117) != |v57[p0 := v1]|;
				var v58: array<int> := new int[29](i6 => i6 + DC84(v1, |multiset{cf118, v1}|, p0).cf142);
				var v59: map<int, bool> := map[cf118 := p1];
				v58[592] := v32.fm7(p0, v59, globalState);
				v58[592] := (cf117 + v1) * v43.fm7(p1, v59, globalState);
				var v60: array<seq<bool>> := new seq<bool>[20] [v52.f18, v52.f18, [p0], v33, v33, [p0], v52.f18, v33, v52.f18, v52.f18, v52.f18, v52.f18, v33, [p0, p0], v33, [p1], v52.f18[v1 := false], v52.f18, v52.f18, [p0]];
			...
			case DC70(cf112) =>
				...
				var v73: array<int> := new int[15] [|v65|, |"wlionyfpy"|, -v1 / v1, v1, v1, -0x296, v1 / -v1, v1, |v0| + v1, |(seq(846, i7  => (v1)))|, v1, 812, |(v69 + v69)|, v1 + |v72|, v1 * v1];
				v73[255] := |(multiset(seq(0x1b2, i8  => (v43.f24))) + multiset{v43.f23, v42})|;
				var v74: map<seq<bool>, bool> := map[v33 := false];
				v73[255] := (if (if (v33 in v74) then v74[v33] else true) then 0x2c8 else v1) % v1;
				var v75: seq<seq<bool>> := [[v64.f17]];
				var v76: array<seq<bool>> := new seq<bool>[19] [v75[v1], v33, v33, v33, v33, v33, v33, v33, v33, [p0], v33, ([true, p0])[v73[255] := true], v33, fm59(!v43.fm6(v0, v73[255], v73[255], v43.f24, globalState), globalState), v33, v33, v33, [p1], v33];
				var v77 := DC42(v33, v63, p1, v76);
				var v79: set<int> := {v1, v73[255], v73[255], v64.fm7(v77.cf71, map v78 : int | (0x20c <= v78) && (v78 < 0x3ce) :: (v78 + |multiset{p1}|) := (p0), globalState) * v73[255]};
...
class C0 {
	...
	function fm14(globalState: GlobalState): int {
		match if (false) then DC3('q') else DC3('a') {
			...
			case DC3(cf5) => |"vfuvqv"| / |(seq(0x1c6, i0  => (|(seq(0x2d9, i1  => (DC12(map[|"pygnrmn"| := true]))))|)))|
			case DC7(cf12) => 290
		}
	}
}
...
class C1 extends T0 {
	...
	function fm48(p0: string, p1: bool, p2: string, p3: bool, globalState: GlobalState): bool {
		0x3b9 < |((seq(0x67, i0  => ('a'))) + "pixj")|
	}
	function fm49(p0: set<bool>, p1: string, p2: int, globalState: GlobalState): map<int, int> {
		((map v0 : int | v0 in [|map[true := 0x10c]|] :: (v0 + --0x3c4) := (0xc7)) + map[|[|(map v1 : int | v1 in {-0x2e8} :: (v1 - 211) := (false))|]| := -0x304]) + (map[|(seq(0xac, i0  => ('a')))| := 0x383] + map[-0xf3 := |"next"|])
	}
	method m17(p0: bool, p1: bool, p2: set<bool>, p3: int, globalState: GlobalState) returns (r0: map<map<int, bool>, seq<int>>, r1: array<array<bool>>, r2: int) {
		for i0 := p3 to p3 + p3 {
			if (p0) {
				...
				var v1 := 'p';
				var v2 := DC9(v1, p3, "pd", |(seq(-0x19, i1  => (v1)))|, p3);
				v0[765] := v2.cf14;
				v0[765] := v1;
				var v3: seq<int> := [751, p3];
				globalState.f4, globalState.f1 := |map[v3 <= v3[i0 := p3] := fm50(globalState)]|, p3 < i0;
				...
				var v9 := "ga";
				v8[6] := if (fm0(globalState)) then v9 else v9;
				var v10: seq<bool> := [p0];
				var v11: map<seq<bool>, string> := map[v10 := seq(-0x3c9, i2  => ('m'))];
				var v12 := 'c';
				v8[6] := (if (v10 in v11) then v11[v10] else seq(269, i3  => ('n')))[p3 := v12];
				globalState.f4 := i0;
				var v13: array<int> := new int[16](i4 => i4 / i0);
			...
			var v19: seq<string> := [v17, v18.cf25, v17 + v17, seq(0x15f, i5  => ('j')), v17];
			var v20: map<int, seq<string>> := map[i0 := v19];
			v19 := (v19 + (if (i0 in v20) then v20[i0] else v19)[i0 := v17]) + v19;
			var v21: array<bool> := new bool[27](i6 => multiset{p0} > multiset{p1});
			v21[998] := !p1;
			v21[998] := !p1;
			var v22: seq<int> := [i0, i0, i0];
			if (|(v22 + v22)| > (p3 / i0)) {
				v21[998] := multiset{p0} !! multiset{p0};
				var v23: map<bool, bool> := map[p0 := p0];
				var v24 := DC31(v23);
				var v25: array<map<bool, bool>> := new map<bool, bool>[29] [v23, map[p0 := v21[998]], v23, v23, v23, v23, v23, map[v21[998] := p1], v23, v23, v23, v23, v23, v24.cf52, v23, v23, map[v21[998] := v21[998]], v23, map[p0 := !p1], v23, v23, v23, v23, v23, v23, v23, fm51(i0, p1, globalState), v23, v23];
				var v26 := 'e';
				var v27: seq<bool> := [DC5(DC0(p3), v25, v21[998], |v17|, v26).cf8, p0, v21[998]];
				globalState.f2 := v27 + v27;
				globalState.f1 := !true;
				var v28: map<int, bool> := map[i0 := v21[998]];
				var v30: set<map<int, bool>> := {v28 + (map v29 : int | v29 in multiset{p3} :: (v29 % p3) := (true))};
				v30 := v30 + v30;
				...
				globalState.f1 := p1;
				var v32 := 'c';
				var v33: map<bool, bool> := map[false := v21[998]];
				var v34: seq<map<bool, bool>> := [v33[p0 := v21[998]]];
				var v35: array<map<bool, bool>> := new map<bool, bool>[20] [v33, v33, v33, v33, v33, map[p0 := p0], v33, v33, map[false := p0], v33, map[true := p0], v33, v33, v33, v34[|[p0]|], v33, v33, v33, v33, v33];
			...
			decreases 100 - i7
		{
			...
			var v50: multiset<int> := multiset{if (v47 in v48) then v48[v47] else p3, v40[971], 0x1e8, if (p1 in v49) then v49[p1] else v40[971], v40[971]};
			var v51 := 'e';
			var v52: map<char, bool> := map[v51 := p1];
			var v53: map<int, int> := map[v40[971] := |v52[v51 := p1]|];
		...
		if (p0) {
			...
			v54[660], globalState.f9, globalState.f4, v54[779] := -|v55| < p3, p3, p3, p1;
			v54 := v54;
			var v56 := new C0();
			...
			var v65: array<bool> := new bool[20] [p3 > |v58|, p0, p1, p0, p0, false, fm0(globalState), fm0(globalState), fm6(v59, fm50(globalState), |v60|, v61, globalState), p1, p1, !true, true, p0, p2 !! p2, v63 == ([p3, p3, p3, p3, p3])[|v60| := p3], false, multiset(v63) >= multiset{|v64|}, p0, p0];
			v65 := new bool[25];
			globalState.f1 := p1;
			var v66: array<seq<multiset<bool>>> := new seq<multiset<bool>>[10](i9 => seq(0x2b8, i10  => (multiset{p0})));
			var v67: seq<bool> := [p1, p0];
			var v68: multiset<bool> := multiset{v67[0x3d6]};
			var v69: seq<multiset<bool>> := [v68, v68];
			v66[87] := (v69 + v69[0x6c := v68])[p3 := multiset{false, p0}];
			var v70 := DC41(v69);
			v66[87] := v70.cf68;
			var v71: multiset<int> := multiset{p3 / |v68|, p3, -fm50(globalState)};
			var v72: C0 := new C0();
			var v73 := DC38(v72, p3);
			var v74: array<C0> := new C0[7] [v72, if (true) then v72 else v72, v72, v72, v73.cf63, v72, v72];
			v74[431] := v72;
			v71, globalState.f1, v74[431] := multiset{v72.fm13(globalState), p3 / |v59|, p3, v72.fm14(globalState)}, v67[p3] <==> fm6(v59, p3, p3, v61, globalState), v72;
			globalState.f1 := {fm6(v59, p3, v72.fm14(globalState), v61, globalState)} > (if (!p0) then p2 else p2);
		}
		
		forall i11 | 0 <= i11 < v40.Length {
			v40[i11] := i11 % -(|(set v75 : char | v75 in {'y'} :: (v75))| - p3);
		}
	...
	method m18(p0: int, p1: bool, p2: int, globalState: GlobalState) returns (r0: D3, r1: bool, r2: string) {
		...
		var v2 := 'u';
		var v3: set<char> := {v2};
		var v4: array<int> := new int[1] [p0 / |v3|];
		v4[55] := p0;
		v4[55] := p2;
		var v5 := DC4();
		if (match v5 {
			...
			if (!p1) {
				...
				v4, v4[55] := if (v4[55] in v8) then v8[v4[55]] else v4, (p0 % p0) % (if (!p1) then p0 else v4[55]);
				var v9 := new C0();
				globalState.f9 := v4[55];
				var v10 := new C0();
			} else {
				var v11 := DC22(v0);
				var v12: map<int, int> := map[v4[55] := p2];
				var v13: multiset<char> := multiset{v2, v2, v2};
				v11, v1 := DC22((if (p1) then fm55(v12, 0x240, p0, globalState) else v0)[p2 := |v13|]), v1 + v1;
				r1 := p1;
				var v14 := new C0();
				v4 := v4;
				var v15 := new C0();
			...
			globalState.f1, v16, globalState.f4 := !p1, v16, |fm56(globalState)| / v4[55];
			globalState.f1 := p1;
			globalState.f9 := v4[55];
		} else {
			globalState.f4 := p0;
			...
			v18 := v18[p1 := fm57(p1, p1, p2, v4[55], globalState)];
			var v19 := new C0();
			v2 := v2;
			var v20: array<seq<int>> := new seq<int>[26];
			v20[551] := [p2];
			var v21: multiset<int> := multiset{v4[55]};
			var v22: map<int, bool> := map[|v21| := false];
			var v23: seq<map<int, bool>> := [v22, v22];
			var v24: set<bool> := {p1};
			v20[551] := (v0[|v23| := 0x3d2 / p2])[|(v24 + v24)| := v4[55]];
		}
		
		if (p1) {
			var v25: array<bool> := new bool[11](i0 => p1);
			v25[799] := v1 == (seq(-0x33c, i1  => (v2)));
			v25[799] := !(p1 || p1);
			var v26: set<int> := {|v1|};
			var v27: array<string> := new seq<char>[3] [seq(570, i2  => (v2)), fm58(["mplo", "gwlrkcc", v1], --0x33f, 655, |v26|, globalState), v1];
			v27[410] := v1;
			globalState.f9, v27[410], v25[799] := p2 / |v1|, v1, if (if (false) then v25[799] else false) then v25[799] else v25[799];
			var v29: array<set<int>> := new set<int>[21](i3 => (set v28 : int | (0x2e6 <= v28) && (v28 < 765) :: (v28 - p2)) - v26);
			v29[909] := v26;
			v29[909] := fm57(p1, true, v4[55], -v4[55], globalState);
			v25[799] := v25[799];
			var v30: multiset<bool> := multiset{p1, true};
			var v31: seq<bool> := [v25[799]];
			globalState.f1 := (v30[v31[|v0|] := p0])[fm0(globalState) := v4[55]] < v30;
		} else {
			var v32 := DC9(v2, p0, v1, v4[55], 0x3dd);
			v2 := v32.cf14;
			var v33: C0 := new C0();
			var v34 := DC38(v33, v4[55]);
			v34, r2, r2, v4[55] := v34, "hbijsffo", if (true) then seq(0x2a2, i4  => (v2)) else "pg" + v1, p2;
			...
			v38[234] := if (p0 in v39) then v39[p0] else "qbbbyd";
			v38[234] := v1;
		}
...
class C2 extends T1, T2 {
	...
	method m3(p0: bool, p1: int, p2: char, globalState: GlobalState) returns (r0: D1, r1: int, r2: bool) {
		...
		var v3: map<int, char> := map[v2[p1] := f24];
		v3 := v3[p1 := p2];
		var v4 := DC21(DC20(v1));
		...
		var v10: array<map<D6, seq<int>>> := new map<D6, seq<int>>[21] [v5, v5 + v5, v5, (map[v4 := v2])[v4 := v2], v5 + v5[v4 := v2], v5, v5, v5 + v5, v5, v5, map[v4 := v2], v5, if (p0) then map[v4 := seq(0xd9, i1  => (p1))] else v5, v5, v5, v6.cf48, v5, v5, map[v4 := fm41(v7, p1, {p0, p0}, p1, globalState)], (map[DC21(v8) := [p1]])[v4 := seq(-0x9f, i2  => (p1))], map[v4 := [fm7(true, v9, globalState)]] + v5];
		v10[517] := v5 + v5;
		v10[517] := v5;
		if (p0) {
			var v11 := new C0();
			...
			if ([fm7(p0, v9, globalState)] == v2) {
				...
				v1[613] := if (p0 in v12) then v12[p0] else p0;
				...
				var v23: array<multiset<D5>> := new multiset<D5>[12] [v19 * v19, v20[v2[p1]] + v19, v19 - multiset{v18}, multiset{v18, v18}, multiset(v21), multiset{fm42(|v22|, |"arqfffve"|, f24, f17, globalState)}, multiset(v21 + v21), v20[p1], v19, v19 + v19, v19, v19];
				v1[613], v23, v9, r1, v1[613] := |[v22]| >= v0.fm13(globalState), v23, v9, (p1 / p1) * |(fm43(--p1, p0, p1, globalState))[p1 := f24]|, p2 in "om";
				var v24: array<int> := new int[27](i3 => i3 + p1);
				v24[104] := p1;
				var v25 := DC27(p1, -0x1a4, -p1, -0x18c);
				globalState.f4, globalState.f9, v24[104], globalState.f4, r2 := p1, p1, p1, v25.cf46, p0;
				var v26 := new C0();
			} else {
				...
				v33[826] := |v34|;
				v33[826], globalState.f1 := v30, p0;
				var v35: map<bool, string> := map[v31 := "d"];
				v32[532] := fm6(if (true in v35) then v35[true] else v7, 927, v33[826], f24, globalState);
				var v36: set<int> := {v30};
				v32[532] := v36 >= v36;
			}
			...
			if (p0) {
				...
				v44, globalState.f4, v45, v47 := v44, p1, v45 + (v45 + map[f18 := p1]), seq(0x299, i5  => (v46));
				v1[83] := f17;
				v1[83] := p0;
			} else {
				var v48 := 'x';
			...
			r2, globalState.f1, v52[610], globalState.f4 := !fm6(v7 + v54.cf16, |v55|, 0xc2, f23, globalState), (p1 * p1) >= p1, "sywg" + v7, -(p1 * p1);
			v1[558] := p0;
			var v56: map<D10, bool> := map[v6 := true];
			v1[558] := if (v6 in v56) then v56[v6] else f17;
			var v57: array<set<bool>> := new set<bool>[9](i6 => {v1[558]});
			var v58 := DC27(|v52[610]|, p1, -28, p1);
			var v59: map<array<set<bool>>, int> := map[if (fm0(globalState)) then v57 else v57 := (v58.(cf46 := p1)).cf43];
			v59 := v59[if (v1[558]) then v57 else v57 := p1];
			var v61: multiset<bool> := multiset{v1[558], f18[p1], f17};
			var v62: set<int> := {|v61|};
			var v63: seq<set<int>> := [set v60 : int | v60 in v2 :: (v60 % -0x18b), {p1, p1}, v62, {p1, -p1}];
			var v64: array<int> := new int[8] [p1, 0xaa, p1, p1, p1, -p1, |v63[p1]|, |([v1[558]] + f18)|];
			v64[263] := |(f18 + [v1[558]])|;
			v64[263] := -0x364;
		}
		...
		for i7 := p1 to p1 {
			...
			match (v67) {
				case DC9(cf14, cf15, cf16, cf17, cf18) =>
					...
					var v74: array<int> := new int[21] [cf18 / cf18, p1, cf18, i7, cf15 + p1, cf18, i7, 0xd4, cf15, |cf16|, cf18, cf18, cf15, cf15, p1 % |fm46(!v72.cf39, !p0, cf15, p0, globalState)|, i7 - i7, cf18, cf18, |v69| * cf15, --(cf18 - cf15), -(|(seq(0x34d, i8  => (v9)))| - |v73|)];
					v74[472] := fm7(p0, v9, globalState);
					v71, v74[472] := fm47(seq(0xda, i9  => (p2)), globalState), if (cf18 in v70) then v70[cf18] else -(p1 * i7);
	...
	method m4(p0: seq<char>, p1: set<bool>, p2: int, p3: array<int>, globalState: GlobalState) {
		...
		v1[658] := new int[6] [0x29a, |({p2} * (set v2 : int | (0x41 <= v2) && (v2 < -0x292) :: (v2 + p2)))|, if (f17) then p2 else p2, |(v3 + map[f18 := f17])|, p2, 0x2b8];
		globalState.f9 := p2;
		var i1 := 0;
		while (!f18[p2])
			decreases 100 - i1
		{
			...
			globalState.f1, globalState.f1, globalState.f4, v7[972] := (v9 - (if (f23 in v13) then v13[f23] else v9)) != v9, f17, p2, f17 && f17;
			match (v10.(cf49 := p0).(cf50 := f17, cf49 := v10.cf49)) {
				case DC30(cf49, cf50, cf51) =>
					var v14 := 'f';
					v14 := f23;
					globalState.f4 := -(766 % p2);
					...
					globalState.f9, v1[658], v18 := cf51 * (p2 % |multiset([f17])|), v0, v18[p3 := p2];
				case DC29(cf48) =>
					var v19: map<map<int, bool>, int> := map[map[p2 := f18[p2]] := p2 * p2];
					var v20: map<bool, bool> := map[v7[972] := v7[972]];
					v19 := v19 + (v19 + map[v11 := |v20|]);
					globalState.f1 := !v7[972];
					var v21 := DC9(f23, p2, p0, p2, p2);
					var v22: map<bool, string> := map[v7[972] := v21.cf16];
					v22 := v22[!false <==> f17 := "kpedi" + p0];
					var v23: seq<array<bool>> := [v7, v7, v7];
					var v24: array<array<bool>> := new array<bool>[12] [v7, v7, v7, v7, v7, v7, v7, v7, v7, if (v7[972]) then v7 else v7, v23[p2], v7];
					v24[748] := v7;
					var v25 := 'p';
					var v27: map<int, char> := map[|v20| * p2 := f24];
					v24[748], v7[972], v25 := v7, if (if (v7[972]) then f17 else v7[972]) then v7[972] else v9 <= (set v26 : int | (0x16 <= v26) && (v26 < 734) :: (v26 - p2)), if ((p2 / -p2) in v27) then v27[p2 / -p2] else v25;
			}
	...
	method m5(p0: bool, p1: int, p2: map<bool, D3>, p3: char, globalState: GlobalState) returns (r0: int) {
		...
		var v3: array<string> := new string[27] [v1, v1, v1, seq(121, i0  => (p3)), "fakedqm", v1, "a", v1, v2[p1], v1, v1, v1, "mdrcwvpq", v1, v1, "ccvom", v1[v0[432] := f23], v1, v1, fm43(p1, p0, p1, globalState), v1, v2[p1], v1, seq(-0x2f0, i1  => (f24)), "ei", v1, v1];
		var v4 := DC11(0x3ce, v3);
		var v6: set<int> := {-v0[432], -p1, v0[432]};
		var v7: set<int> := {v0[432], |v6|, p1};
		var v8: map<bool, int> := map[p0 := |v7|];
		var v9: set<int> := {p1, v0[432], if (p0 in v8) then v8[p0] else -p1};
		var v10: map<bool, bool> := map[p0 := f17];
		r0, globalState.f1 := (v4.(cf20 := v0[432])).cf20, ((set v5 : int | (0x165 <= v5) && (v5 < 619) :: (v5 * 0x4c)) - v6) >= {fm7(p0, map[|v10| := p0], globalState)};
		var i2 := 0;
		while (f18[p1])
			decreases 100 - i2
		{
			...
			if (if (p0 in v10) then v10[p0] else f17) {
				...
				v12[136] := multiset(f18) + multiset{f17};
				var v13 := DC0(p1);
				...
				var v15 := DC5(v13, v14, p0, |(seq(-0x57, i4  => (p3)))|, f24);
				var v16: set<char> := {f24};
				var v17: multiset<bool> := multiset{p0, v16 != v16, f17};
				v11[247], v12, globalState.f1, globalState.f1, v12[136] := p0, v12, v15.cf8, -|v1| > -482, v17;
				var v18: map<int, int> := map[-(|"f"| + v0[432]) := p1];
				v18 := v18[0x173 * v0[432] := |f18|];
				globalState.f4 := |v10| - v0[432];
				var v19: array<array<char>> := new array<char>[8];
				var v20: array<char> := new char[1];
				v19[203] := v20;
				v19[203] := v20;
				globalState.f4 := v0[432];
			} else {
				var v21: array<char> := new char[5](i5 => f24);
				var v22: map<char, int> := map[f24 := -p1];
				var v23: map<int, seq<bool>> := map[p1 := f18];
				v21[772] := fm8(p1, if (p3 in v22) then v22[p3] else p1, v23, globalState);
				v21[772] := f24;
				var v24: array<map<bool, int>> := new map<bool, int>[25](i6 => map[p0 := v0[432]]);
				...
				v24[588] := v8 + (if (|v28| in v27) then v27[|v28|] else map[false := p1]);
				v24[588] := v8 + (v8 + v8);
				var v29 := new C0();
				var v30 := DC0(p1);
				var v31 := DC2(v30);
			...
			var v34: map<array<int>, int> := map[v0 := v0[432]];
			var v35 := DC25(v34);
			match (v35.(cf41 := v34)) {
				case DC26(cf42) =>
					v1 := (v1 + v1) + (v1 + v1);
				...
				case DC27(cf43, cf44, cf45, cf46) =>
					globalState.f2 := f18;
					globalState.f1 := f18[v0[432]];
					var v38: multiset<bool> := multiset{f17};
					globalState.f4 := cf44 % (|v38| + cf43);
					v10 := v10[f17 := cf45 != p1];
				case DC25(cf41) =>
					v0 := v0;
					v3[832] := v1;
					v3[832] := v1 + (v2[|map[f17 := v0[432]]|] + v1);
					var v39: set<bool> := {f17};
					var v40: array<bool> := new bool[26] [v39 > {f17}, p0, p0, p0, fm0(globalState), p0, false, false, f17, !true, v0[432] > v0[432], false, f17, v0[432] == p1, f18[v0[432]], f17, v9 !! v7, f17, f17, f17, p0, f17, if (f17) then !f17 else p0, true, f17, false && f17];
					v40 := if (f17) then v40 else v40;
					var v41: array<D1> := new D1[12];
					var v42: array<map<bool, bool>> := new map<bool, bool>[21](i8 => v10);
					var v43 := DC5(DC0(|f18|), v42, p0, v0[432], p3);
					v41[199] := v43;
					globalState.f1, v0[432], globalState.f1, v41[199] := !f17, |v10| - 0xf6, true, v43;
				case DC28(cf47) =>
					v0 := v0;
					...
					var v49 := DC5(v47, v48, false, v0[432], f23);
					var v50: seq<D3> := [DC11(v0[432], v3)];
					var v51: multiset<seq<D3>> := multiset{v50};
					var v52: seq<seq<D3>> := [v50, v50[v0[432] := v4], v50];
					v44, globalState.f1, v45 := v46[v49.cf9], !(v51 < (multiset(v52) * v51)), v45;
					var v53: map<int, bool> := map[if (true in v8) then v8[true] else |{p0}| := !f17];
					v0[432] := -fm7(v0[432] >= p1, v53[-v0[432] := p0], globalState);
					v0[432] := 0x245;
			}
			...
			v54 := if (p1 in v55) then v55[p1] else fm8(v0[432], -|v10|, v56, globalState);
			var v57: array<T0> := new T0[16];
			var v58: T0 := new C1();
			v57[574] := v58;
			var v59: array<bool> := new bool[4];
			v59[484] := p0;
			var v60: map<int, int> := map[v0[432] := v0[432]];
			v1, v57[574], v59[484], globalState.f1 := fm58(v2, |v1|, v0[432], |fm57(f17, p0, v0[432], |f18|, globalState)|, globalState), v58, f17, if (f17) then p1 in v60 else p0 ==> p0;
		}
		v10 := v10[p0 := false];
		var i10 := 0;
		while (p0)
			decreases 100 - i10
		{
			...
			if (f17) {
				...
				globalState.f4 := if (p0) then v0[432] else |v64|;
				globalState.f9 := -v0[432];
				globalState.f9 := v0[432];
			...
			var v71: multiset<array<int>> := multiset{v0, if (|f18| in v69) then v69[|f18|] else v70[643], if (false) then v0 else v0};
			var v72 := DC43(v71);
			v67[857], v67, v71 := if (p0 in v10) then v10[p0] else f17, v67, v71 + v72.cf73;
			globalState.f1 := 0x293 > (p1 % p1);
		}
		var i11 := 0;
		while (p0 <==> p0)
			decreases 100 - i11
		{
			...
			i11 := i11 + 1;
			var v73: map<map<int, bool>, int> := map[(map[v0[432] := f17])[p1 := p0] := p1];
			var v75: map<int, bool> := map[|v8| := p0];
			var v76: seq<map<int, bool>> := [v75];
			v73 := (map v74 : map<int, bool> | v74 in v76 :: (v74) := (p1)) + map[v75 := v0[432]];
			var v77: multiset<bool> := multiset{!f17};
			var v78: multiset<int> := multiset{0x5};
			globalState.f4 := if (!(multiset{f17, f17} == v77)) then v0[432] * |v1| else if (v0[432] in v78) then v78[v0[432]] else p1;
			v1, globalState.f9 := v1[v0[432] := p3], if (f18[|(seq(0x1fd, i12  => (f23)))|]) then -v0[432] else p1;
			globalState.f1 := f17;
		}
		r0 := fm50(globalState);
	}
	method m6(globalState: GlobalState) returns (r0: string, r1: bool) {
		...
		match (v7) {
			case DC42(cf69, cf70, cf71, cf72) =>
				...
				r1, r1, globalState.f4, v4[879] := cf71, cf71, v1 / v1, if (v1 in v10) then v10[v1] else -0x122 <= |v11|;
			case DC41(cf68) =>
				...
				v12[312] := (if (false) then v7.(cf69 := if (|v15| in v14) then v14[|v15|] else fm59(f17, globalState)) else DC42(f18, v7.cf70, f17, v6)).(cf70 := v4, cf69 := [f17, f17]);
				var v16 := new C1();
				var v17: array<int> := new int[26];
				globalState.f9 := |{v17, v17}|;
				var v18 := DC23(f17);
				var v19: array<D7> := new D7[22] [fm60(f17, |f18|, f17, globalState), DC23(v7.cf71), v18.(cf39 := f17), v18, v18, v18, v18, v18, v18, v18, v18, v18, v18, if (f18[v1]) then v18 else v18, DC23(!f17), v18, v18, v18, v18, v18, DC23(f17), fm60(f17, -v1, f17, globalState)];
				v19[749] := v18;
				v19[749] := v18;
		}
		...
		var v31: array<int> := new int[6] [v29[v1] + |v30|, v1, v1, |v25|, v1, v1];
		v31[146] := v1;
		v31[146], globalState.f9 := v1, -v1;
		r0 := (("kpqr" + v23) + v23)[v31[146] + |v23| := f24];
		r1 := !!fm0(globalState);
	}
	method m15(p0: D4, p1: int, globalState: GlobalState) returns (r0: bool, r1: int, r2: bool, r3: array<bool>) {
		if (f17) {
			if (f17) {
				var v0: map<bool, int> := map[f17 := p1 / p1];
				v0 := v0[f17 := -p1];
				var v1 := "sil";
			...
			match (v14) {
				case DC32(cf53, cf54) =>
					...
					v25 := v25[cf54 / |v26[p1]| := !(if (f17) then f18[cf54] else true)];
					globalState.f9 := p1;
				case DC33(cf55, cf56, cf57) =>
					...
					var v36: set<int> := {|v35|, cf56, |(seq(0x101, i1  => (cf57)))|};
					...
					v41, globalState.f9 := v41, cf57 - (cf56 / cf57);
				...
				case DC31(cf52) =>
					var v50: seq<int> := [|f18|];
					var v51 := DC3(f24);
					var v52 := DC9(f24, p1, seq(-0xb0, i3  => ('a')), p1, |f18|);
					...
					var v58: array<array<char>> := new array<char>[28] [v53, v53, v53, v53, v53, v53, v53, v53, v53, v53, v53, v54[p1], v57.cf3, v57.cf3, v53, v53, v53, v53, v53, v57.cf3, v53, v53, v57.cf3, v53, v53, v53, v53, v53];
					var v59 := "vbbyh";
					...
					var v63 := DC30((seq(910, i4  => (f24))) + v59, f17, |([f17] + f18)|);
					v63 := DC30("j", !(p1 > p1), p1);
					v56[387] := f17;
					v56[387] := f17;
				case DC35(cf61) =>
			...
			if (fm0(globalState)) {
				...
				var v68: array<bool> := new bool[4] [f17, if (f17) then f17 else f18[|v66|], v67 !! multiset{f17, f17, f17, f17}, f17];
				v68[962] := f17;
				v68[962] := true;
				var v69: map<bool, int> := map[f17 := p1];
				var v70: seq<map<bool, int>> := [v69 + map[v68[962] := fm50(globalState)]];
				v70 := v70;
				var v71: map<bool, string> := map[f17 <==> true := v65];
				v71 := v71[f17 := v65];
				r0 := v68[962];
				var v72: map<int, bool> := map[p1 := v68[962]];
				var v73: multiset<int> := multiset{-0x135};
				var v74: map<bool, bool> := map[v68[962] := f17];
				var v75: map<int, bool> := map[fm7(f17, v72, globalState) / p1 := f18[if (p1 in v73) then v73[p1] else |v74|]];
				v75 := v75[p1 := f17];
			} else {
		...
		if (f23 in v86) {
			...
			match (v88) {
				...
				case DC18(cf33, cf34) =>
					...
					globalState.f1 := v96 == [seq(-0x2bb, i7  => (f24))];
					v82[630] := f17;
					var v97 := DC20(v82);
					...
					v82[630] := !(|v98| <= |v100|);
					cf33 := |fm58(v96 + v96, if (v82[630]) then -0x3a8 else p1, -158, 879, globalState)|;
					var v101: multiset<int> := multiset{|(multiset{v82[630], v82[630]} * v84)|};
					v101 := v101;
				case DC16(cf29) =>
					r2 := false;
					var v102 := 'r';
					globalState.f9, v102, r0 := p1 / p1, fm53(p1, f17, if (f17) then true else f17, 491, globalState), !fm0(globalState);
					r2 := f17;
					var v103 := new C1();
			}
			...
			globalState.f9 := p1 - |((map v105 : int | v105 in v107[p1 := -p1] :: (v105 % |f18|) := (v86)) + map[fm50(globalState) := if (p1 in v108) then v108[p1] else v86])|;
		} else {
			var v109: C0 := new C0();
			...
			match (v110) {
				...
				case DC38(cf63, cf64) =>
					...
					v121[946] := -cf64;
					v121[946] := cf64 + p1;
					var v122: seq<string> := [seq(0x87, i9  => (f24))];
					var v123: map<int, seq<string>> := map[v121[946] := v122];
					var v124: map<seq<string>, bool> := map[if (|f18| in v123) then v123[|f18|] else DC47(v122[p1 := v86]).cf78 := f17];
					var v125 := DC47(v122);
					v124 := v124[v125.cf78 := f17];
					globalState.f4 := v121[946];
				case DC39(cf65, cf66) =>
					var v126 := new C0();
					globalState.f9 := p1 / |v86|;
					var v127: set<bool> := {false, cf65};
					v82[364] := !(v127 >= {fm0(globalState)});
					...
					v82[364], globalState.f4 := fm6(v86, cf66, 349, f24, globalState), |v129|;
				...
				case DC36(cf62) =>
					...
					cf62 := cf62;
					var v135 := 'w';
					var v136: map<int, string> := map[p1 := seq(0x3b3, i10  => (f24))];
					var v137: seq<int> := [p1, p1, p1];
					var v138: array<string> := new string[19] [v86, v86, v86, if (|v137| in v136) then v136[|v137|] else v86, "udkyui", "vdumxuc", seq(-952, i11  => (v135)), v86, seq(0x39e, i12  => ('f')), v86[|v86| := f24], ("tvctg")[p1 := f23], v86, v86, v86, v86, v86, v86, v86, v86];
			...
			v149[554] := if (p1 in v150) then v150[p1] else p1;
			v149[554] := -|v150|;
			r2 := f17;
			r2 := !(f17 <== f17);
		}
	...
	method m16(p0: int, globalState: GlobalState) returns (r0: int) {
		if (f17) {
			...
			if (f17 <==> f17) {
				...
				v0[576] := f17;
				v0[576] := (|f18| - p0) > p0;
				var v9: array<int> := new int[1](i1 => i1 % p0);
				v9[946] := p0;
				var v10: map<bool, int> := map[v0[576] := if (f17) then p0 else p0];
				v9[946] := if (f17 in v10) then v10[f17] else p0 / p0;
				v0[576] := v0[576];
				var v11: set<int> := {v9[946], if (v0[576]) then |{f23}| else v9[946], |multiset(f18)|, |f18|};
				v11 := v11;
				m0(v0[576], v0[576] || v0[576], globalState);
			}
			...
			var v12 := "yrm";
			var v13: seq<string> := [seq(0x124, i3  => (f24))];
			var v14 := DC9(f23, |v13[p0]|, v12, p0, -0x1ce);
			var v15: array<string> := new string[17] [v12, if (!true) then "odladbb" else seq(645, i2  => (f24)), v14.cf16 + v12, v12, v13[p0], v12, v12, v12 + v12, "s", "fqsdblgtk", v12, v12, v12, v12, v12, v12, v12];
			v15[455] := if (f17) then (fm43(p0, f17, |v12|, globalState))[p0 := f24] else v12;
			v15[455] := seq(0x1, i4  => ('l'));
			globalState.f1 := f17;
			globalState.f1 := f17;
			globalState.f1 := f18 == f18[|(map v16 : char | v16 in map[f24 := f17] :: (v16) := (p0))| := true];
			...
			var v21: set<int> := {p0 % (if (0x39a in v20) then v20[0x39a] else p0), p0};
			var v22 := DC45(f17, v17);
			var v23: map<int, bool> := map[p0 := true];
			var v24: multiset<char> := multiset{f23};
			var v25: array<bool> := new bool[28] [f17, false, f17, f17, !v22.cf75, f17, f17, if (p0 in v23) then v23[p0] else true, f17, f17, f17, f17, v24 !! v24, true, f17, f17, f17, f17, !((seq(0x2c0, i6  => (multiset{f17}))) < (seq(-0x281, i7  => (multiset{f17})))), f17, f17, f17, true, p0 != p0, f17, f17 <==> f17, !f17, f17];
			v25[504] := f17;
			var v26: map<bool, string> := map[f17 := v15[455]];
			v18, v21, v25[504] := v18, v21, if (f17) then f17 in v26 else f17;
		}
			...
			decreases 100 - i8
		{
			...
			var v30: array<string> := new string[14] ["qpxeq", "ph", v27, v27 + v27, "vuvghcegr", v27[p0 := f23], if (f17) then "nkes" else v27, v27 + v27, v28.cf49, v27, fm58(v29, p0, p0, p0, globalState), v27, v27, (seq(0xfc, i9  => (f24))) + v27];
			v30[784] := v27;
			var v31: seq<int> := [p0];
			var v32: multiset<seq<int>> := multiset{v31, [-|f18|]};
			v30[784] := fm58(v29 + v29, p0, if (v31 in v32) then v32[v31] else p0, p0, globalState);
			...
			var v37 := DC14(fm0(globalState), v30[784], v35[656], p0);
			if ((v36 + {v37.cf24, v35[656]}) > v36) {
				globalState.f1 := v35[656];
				var v38: array<int> := new int[13];
				v38[374] := |fm43(p0, f17, p0, globalState)|;
				var v39: map<int, bool> := map[p0 := f17];
				v38[374] := -(if (fm0(globalState)) then fm50(globalState) else p0 - fm7(v35[656], v39, globalState));
				var v41 := DC22(seq(0x365, i10  => (|(map v40 : int | (0x199 <= v40) && (v40 < 386) :: (v40 / v38[374]) := (f17))|)));
				v31 := v41.cf38;
				v38, globalState.f1, globalState.f1, v38[374], v38[374] := DC10(v38).cf19, (p0 % v38[374]) >= p0, false, p0, --p0 + 0x1e3;
				v35[656] := f17;
			} else {
				var v42: map<int, bool> := map[p0 % p0 := f18[|v27|]];
				v42 := v42[p0 := v35[656]];
				var v43: C0 := new C0();
				var v44: map<int, C0> := map[-(p0 / |f18|) := v43];
				var v45 := DC11(-p0, v30);
				var v46: seq<C0> := [v43, v43];
				v44 := v44[v45.cf20 + p0 := v46[|v30[784]|]];
				var v47: array<array<bool>> := new array<bool>[13];
				v47[599] := v35;
				globalState.f1, v35[656], v47[599], globalState.f4 := (0x3b3 + p0) <= p0, (p0 - -0x3b4) <= p0, if (f17) then v35 else v35, p0;
				...
				var v52: array<int> := new int[23] [p0, |map[v35[656] := f23]|, p0, p0, -872, fm50(globalState), p0, p0, p0, |map[|v49| := v35[656]]|, p0, |v50|, |v34|, p0, 0xf1, |v27|, p0, p0, p0, p0, p0, |v51|, v31[p0]];
				var v53: map<int, array<int>> := map[p0 / |("eonh")[p0 := f23]| := v52];
				v53 := v53[if (f18[|v29[p0]|]) then p0 else p0 := v52];
				var v54: map<bool, string> := map[false := v30[784]];
		...
		match (v66) {
			...
			case DC27(cf43, cf44, cf45, cf46) =>
				...
				var v78: set<int> := {|v76|, p0, fm7(f17, map v77 : int | (0x262 <= v77) && (v77 < 0x359) :: (v77 / cf46) := (f17), globalState), cf44};
				var v79: C0 := new C0();
				...
				var v83: set<int> := {cf43 % cf45, p0, p0, cf44, v72[|v82|] - cf43};
				...
				v83, v84[596], v85[784], v81[666], globalState.f1 := (if (f17) then v83 else v78) * (v78 - {-cf46, 59, cf45}), true, v84, cf45, !((set v87 : string | v87 in DC52(v86).cf88["jwdfxbro" := -0x315] :: (v87)) > v88);
			...
			case DC25(cf41) =>
				...
				v92[831] := v93;
				globalState.f4, globalState.f2, globalState.f4, v92[831] := -0xde, fm59(f17, globalState), p0, v93;
				globalState.f1 := f17;
			case DC28(cf47) =>
...
class C3 extends T1 {
	...
	method m3(p0: bool, p1: int, p2: char, globalState: GlobalState) returns (r0: D1, r1: int, r2: bool) {
			...
			decreases 100 - i0
		{
			...
			v4[98] := p0;
			v0[988], v1, v4[98] := p1, p2, f17;
			var v5: multiset<bool> := multiset{p0, !true};
			var v6 := "vdbclbek";
			var v7: set<int> := {p1, v0[988], p1, |v6|, p1};
			var v8: map<int, int> := map[v0[988] := p1];
			var v10: set<set<int>> := {v7 * (set v9 : int | v9 in v8 :: (v9 - |{true}|)), fm37(f17, v4[98], globalState), if (false) then v7 else v7, v7};
			v4[98], v5, v4[98], v10 := v4[98], v5, p0, v10;
			var v11: map<int, array<bool>> := map[|(seq(0x237, i1  => ('b')))| := v4];
			v11 := v11[p1 := v4];
			var v12: array<seq<int>> := new seq<int>[17](i2 => [-|v6|]);
			v12[347] := fm38(DC26(v4[98]).cf42, -v0[988], globalState);
			v12[347] := fm38(f17, if (f17 in v5) then v5[f17] else v0[988], globalState) + [p1, p1, p1, p1];
		}
		if (!(if (p0) then f17 else p0)) {
			...
			var v19: seq<seq<bool>> := [v16[if (p1 in v17) then v17[p1] else p1], v18];
			var v20: array<int> := new int[21](i4 => i4 % |map[f17 := p1]|);
			var v21: map<array<int>, char> := map[v20 := p2];
			var v22: array<bool> := new bool[27] [f17, f17, |v15[p1]| != |v16|, false, p0, !p0, p0, p0, p2 !in (seq(388, i3  => (p2))), if (p0) then f17 else true, p0, !f17, f17, true, f17, p0, f17, f17, p0, fm0(globalState), p0, false, true, v18[fm7(f17, v13, globalState)], p1 < p1, v20 in v21, p0];
			v22[76] := p0 || f17;
			var v24: array<seq<map<int, int>>> := new seq<map<int, int>>[26](i5 => [map[p1 := p1], map[0x324 := p1]] + (seq(0x159, i6  => (map v23 : int | (0x1c4 <= v23) && (v23 < 0x306) :: (v23 / |{p0, p0}|) := (p1)))));
			...
			v22[76], globalState.f4, v24[570] := p0, v29.cf27 - (p1 / |v18|), [v26];
			v22[76], globalState.f9 := v28 == (seq(0x5, i7  => (p2))), p1;
			var v30: map<int, char> := map[p1 := p2];
			var v31: seq<string> := ["xob"];
			v22[76] := fm6(v28 + (v28[p1 := if (676 in v30) then v30[676] else p2])[p1 := p2], fm7(p0, v13, globalState), |multiset{v28, seq(-0x47, i8  => (p2)), v31[p1], v28, ("qdhc")[p1 := p2]}|, if (false) then p2 else p2, globalState);
			r2 := f17;
			v13 := v13[p1 := p0];
		} else {
			...
			v32[730] := v33 <= v33;
			var v34 := "tffpjdbdf";
			v32[730] := v34 !in fm39(globalState);
			var v35: seq<int> := [p1, 0x8d];
			var v36: map<int, array<bool>> := map[-|v35| := v32];
			var v37: seq<bool> := [v32[730]];
			var v38: seq<seq<bool>> := [v37, v37, v37];
			globalState.f9, v32, v32[730] := p1, if ((if (p0) then |multiset(v38[p1])| else p1) in v36) then v36[if (p0) then |multiset(v38[p1])| else p1] else v32, fm6(v34, p1, p1, p2, globalState);
			var v39 := new C0();
			v32[730] := false;
			var v40: array<string> := new string[20];
			v32[730], v40, v32 := !p0, v40, v32;
		}
		...
		if (v41 < (v41 + v41)) {
			...
			var v64 := DC42(v61, v62[p1], f17, v63);
			v60[265] := v61 <= (v64.(cf71 := f17).(cf69 := [f17])).cf69;
			var v65: array<int> := new int[10];
			var v66 := DC45(false && fm0(globalState), v65);
			match (v66) {
				case DC44(cf74) =>
					v60[265], cf74 := p0, p1 - 0x2d9;
					var v67: map<bool, bool> := map[p0 := p0];
					var v68: map<bool, char> := map[v60[265] := p2];
					var v69: map<map<bool, char>, string> := map[v68 := seq(0x27, i11  => (p2))];
					...
					v71, v72[12] := v71, v73[fm50(globalState) % cf74];
					r2, globalState.f9 := v61[cf74], -(p1 + p1);
					globalState.f1 := cf74 >= p1;
				case DC45(cf75, cf76) =>
					m0(v60[265], cf75, globalState);
					var v74 := new C2(p2, p2, true, v61 + v61);
					var v75: set<bool> := {cf75};
					var v76 := new C2(v41[p1], if (v60[265]) then v74.f23 else v74.f23, v75 <= v75, v61);
				...
				case DC43(cf73) =>
					...
					var v83: set<bool> := {v60[265], v60[265]};
					var v84: map<set<bool>, bool> := map[v83 := v60[265]];
			...
			if (f17) {
				var v87: map<int, int> := map[if (v60[265]) then p1 else p1 := p1];
				...
				var v91: array<string> := new string[13] [v41, v41, seq(291, i13  => (p2)), v41, seq(-0x208, i14  => (p2)), "rxygjmo", fm58(v90, v65[200], |v41|, -p1, globalState), v41, v90[|map[v65[200] := p0]|], v41, "jqxcf", v41, v41];
				var v92 := DC11(0x146, v91);
				var v93: seq<array<string>> := [v91];
				var v94: array<array<string>> := new array<string>[20] [v91, v91, v91, v92.cf21, if (!fm6(v41, -0x8a, v65[200], p2, globalState)) then v91 else v91, v91, v91, v91, v91, v91, v91, v93[p1], v91, v91, v91, v91, v91, v91, v91, v91];
				v94[131] := v91;
				v94[131] := new string[27];
				globalState.f1 := v65[200] == (v65[200] - p1);
			} else {
				var v95: multiset<bool> := multiset{v60[265], p0, f17};
				v60[265] := !v61[|v95| % p1];
				var v96 := new C1();
				...
				var v101: seq<seq<D2>> := [v99[p1 := v98], v99[p1 := DC8(v97)], v99];
				v100 := multiset(v101) * v100;
				var v102 := 'x';
				v102, v60[265], globalState.f9 := if (v60[265]) then if (p0) then v102 else v102 else p2, p1 < -p1, |(v41 + "bwumxn")|;
			}
			...
			v109, v41, v41, globalState.f4, globalState.f9 := multiset{p1, |v112[0x113 := p1]|}, v41, "voiuyy", fm50(globalState), p1 / p1;
			var v113: set<int> := {p1};
			var v115: array<int> := new int[15] [p1, p1, p1, -(p1 % |v113|), p1, p1 / p1, p1, 0x367, p1, if (p0) then p1 else -0x39f, p1, fm7(p0, map v114 : int | (795 <= v114) && (v114 < 827) :: (v114 + p1) := (f17), globalState), p1, -p1, p1];
			v115[60] := p1;
			var v116: map<bool, bool> := map[p0 := p0];
			var v117: map<bool, map<bool, bool>> := map[f17 := v116];
			v115[60] := |(if (f17 in v117) then v117[f17] else v116)| + p1;
			var v118: array<bool> := new bool[1];
			var v119: map<int, array<bool>> := map[-v115[60] := v118];
			var v120, v121, v122, v123 := m13(v119, p2, map[v118 := p1], globalState);
			if (v120 || p0) {
				globalState.f1 := f17;
				var v124: map<bool, int> := map[p0 := v121];
				v124 := v124[-v121 >= -v115[60] := p1];
				var v125: C2 := new C2(p2, p2, f17, fm59(f17, globalState));
				var v126: seq<C2> := [v125];
				var v127: array<seq<C2>> := new seq<C2>[15] [v126, v126, v126, v126 + [v125, v125], v126, v126, v126, (v126 + v126)[|v110| := v125], v126, v126, v126 + v126, v126, v126, v126 + v126, v126];
				v127 := v127;
				v41 := v41;
				var v128: array<array<int>> := new array<int>[22];
				v128[770] := DC45(v120, v115).cf76;
				...
				v128[770] := new int[27] [v115[60], 0x1b2, v131[v122], v122, p1 / v121, 0x13d % |(seq(0x1bc, i15  => (p2)))|, 350, 0x203, -0x12d, v115[60], p1, if (0x3d6 in v109) then v109[0x3d6] else |v41|, v121 / v121, p1, v115[60] / v121, v121, v121 + v122, 961, v122 - |v133|, if (p0) then p1 else p1, -0x153, 0x225, v115[60] % 0x38b, v115[60] % |v123|, v121, v129.fm7(v120, v134, globalState), |[p1, p1]|];
			} else {
				v115 := v115;
				var v135 := new C0();
				...
				var v142: array<map<string, int>> := new map<string, int>[20](i16 => map[v41[|multiset{|map[p0 := v115[60]]|, |v109|}| := p2] := |[v139]|]);
				var v143: map<string, int> := map[v41 := v121];
				v142[198] := v143;
				var v145: multiset<string> := multiset{v41, v41, v41, v41, seq(689, i17  => ('x'))};
				var v146: map<C0, bool> := map[v135 := v138];
				v142[198] := ((map v144 : string | v144 in v145 :: (v144) := (|map[|map[v115[60] := v41]| := true]|)) + map[v41 := v140]) + map[v41 := |v146|];
				globalState.f1 := !v138;
			}
			
			var v147: seq<int> := [|v110|];
			var v149: seq<string> := [v41];
			var v150: map<bool, int> := map[v120 := v115[60]];
			var v151: set<seq<int>> := {v147, v147, fm55(map v148 : int | v148 in v109 :: (v148 * p1) := (-v121), |v149[|v150|]|, v115[60], globalState), v147, v147};
		...
		r1 := (if (f17 in v152) then v152[f17] else p1) % |v156|;
		globalState.f9 := p1;
		...
		var v161 := DC0(DC51(v158, |(set v159 : int | v159 in fm64(p1, -0xe2, globalState) :: (v159 % 319))|, p2, v160, p1).cf87);
		var v162: array<map<bool, bool>> := new map<bool, bool>[2];
		var v163 := DC5(v161, v162, p0, p1, p2);
		var v164 := DC7(v163);
		r0 := v164;
		r1 := (if (f17) then |v41| else -0xbe) / fm50(globalState);
		r2 := fm0(globalState);
	}
	method m4(p0: seq<char>, p1: set<bool>, p2: int, p3: array<int>, globalState: GlobalState) {
		...
		v9[139] := v8;
		v3[780], globalState.f1, globalState.f1, v9[139] := v4, fm6("dpufryn", p2, p2, v5, globalState), f17, v8;
		...
		match (match DC47(v10).(cf78 := v10) {
			...
			case DC4() =>
				var v11: seq<int> := [p2];
				globalState.f9 := -(|(if (v3[780][|v11[p2 := |p0|]|]) then p0 else p0)| % p2);
				var v12: array<D19> := new D19[20](i0 => DC58(p1, 0x2ab, v11[p2]));
				...
				v14[804] := v15;
				var v16: multiset<string> := multiset{p0[p2 := v5]};
				globalState.f1, globalState.f9, globalState.f1, globalState.f1, v14[804] := p0 in v16, p2, f17, f17, v15 + ((map v17 : char | v17 in [v5, v5, v5] :: (v17) := (p0)) + map[v5 := "g"]);
			...
			case DC5(cf6, cf7, cf8, cf9, cf10) =>
				...
				var v20 := DC13(cf9);
				var v21: set<D4> := {v20};
				p3[490], globalState.f1 := p2, v21 > v21;
				v4 := fm59(cf8, globalState);
				var v22 := DC23(f17);
				var v23: map<int, int> := map[if (cf8 in v2) then v2[cf8] else cf9 := p3[490]];
				var v24: map<bool, map<int, int>> := map[true := v23];
				p3[490], globalState.f9, globalState.f1, globalState.f1, globalState.f1 := (p3[490] + p2) * (if (false in v2) then v2[false] else cf9), fm7(f17, v1, globalState) / (-p2 + 0x32a), v22.cf39, v24 != v24, fm6(p0 + (seq(744, i1  => (cf10))), -p2, cf9, v5, globalState);
				...
				var v30: array<int> := new int[19] [0x118, cf9, p3[490], p3[490], p3[490], if (v5 in v26) then v26[v5] else p3[490], cf9, |(seq(0x37d, i2  => (p2)))|, |v28|, cf9, |v29|, |p0|, |fm65(f17, f17, globalState)|, cf9, p3[490], 0x16b, p2, p3[490], |v2|];
				var v31: set<array<int>> := {v30, v30};
				var v32: map<map<bool, multiset<bool>>, int> := map[v25 := p3[490] * |v31|];
				v32 := v32[v25 := -p3[490]];
			case DC6(cf11) =>
				...
				v37, globalState.f4, globalState.f9, v37, globalState.f4 := v37, cf11 * cf11, cf11, v38[cf11 + -cf11], p2;
				p3[882] := cf11 % p2;
				p3[882] := cf11;
				globalState.f1 := f17;
			case DC3(cf5) =>
				...
				v1 := v1;
			case DC7(cf12) =>
				var v42: seq<multiset<bool>> := [multiset(v3[780]), v2];
				var v43: seq<seq<multiset<bool>>> := [v42];
				var v44 := DC41(v43[p2]);
				var v45: array<seq<multiset<bool>>> := new seq<multiset<bool>>[18] [v42, v42[p2 := v2], v42, seq(0x234, i3  => (v2)), v42, v42, v42, v42, v42, v42 + [v2], seq(906, i4  => (v2)), v44.cf68, v43[p2], if (f17) then [v2[f17 := |[p0]|], multiset{f17, f17}] else v42, v42, [v2], v42[p2 := multiset{f17}], v42 + v42];
				v45[982] := v42;
				v45[982] := v42[p2 := v2] + ([v2, v2] + (seq(308, i5  => (v2))));
				globalState.f9 := (p2 / p2) + |map[p2 := p3]|;
				...
				if (if (((map v50 : map<bool, int> | v50 in v51 :: (v50) := (f17)) + v47) in v49) then v49[(map v50 : map<bool, int> | v50 in v51 :: (v50) := (f17)) + v47] else f17 <== f17) {
					...
					v55[433], globalState.f9, v56[782] := fm53(if (p0 in v57) then v57[p0] else p2, if (f17) then f17 else f17, p2 == |v4|, |p0| % v54.fm13(globalState), globalState), p2 / p2, fm66(globalState);
					v52[769] := true;
					var v58 := DC63([v52, v52]);
					var v59: seq<array<bool>> := [v52];
					v52[769] := v58.cf104 == (v59 + v59);
					...
					p3[222] := if (p0 in v60) then v60[p0] else p2;
			...
			decreases 100 - i8
		{
			...
			var v64: array<bool> := new bool[18] [f17, f17, f17, f17, false, fm51(p2, f17, globalState) == v62, v4[-909], f17 <== f17, f17, f17, -p2 == p2, f17, f17 ==> f17, true, f17, f17, f17, if (if (p1 in v63) then v63[p1] else f17) then true else true];
			v64[938] := true <==> f17;
			...
			var v73: set<int> := {588};
			var v74 := DC10(p3);
			v64[938], v65, globalState.f1, globalState.f4, v72[758] := !f17, if (f17) then v65 else v10[p2], v73 >= v73, -|map[v74.cf19 := p3]|, v67;
			globalState.f1 := (p2 + p2) > (|v65| % p2);
		...
		match (DC50(if (f17) then f17 else !f17, v76, f17)) {
			case DC49() =>
				...
				var v80 := DC5(v77, if (v5 in v79) then v79[v5] else v78, (seq(0xb7, i10  => (v5))) <= p0, p2, v5);
				var v81: T0 := new C1();
				v80 := DC5(v77, v78, v76[426], |map[v81 := f17]|, v5).(cf7 := v78).(cf6 := v77, cf9 := fm50(globalState));
				var v82 := new C0();
				v76[426] := f17;
			...
			case DC48(cf79) =>
				...
				p3[445] := if ((if (p2 in v89) then v89[p2] else v87) in v88) then v88[if (p2 in v89) then v89[p2] else v87] else p2;
				var v90: set<char> := {v5, v5};
				var v91: seq<int> := [p2];
				var v92 := DC39(f17, |v91|);
				var v93: seq<array<array<int>>> := [v85, v85];
				v85, v0, globalState.f4, globalState.f4, p3[445] := if (v90 >= fm67(0x190, v5, [|p0|], v92, globalState)) then v85 else v93[p2], v0, if (p2 in v0) then v0[p2] else p2, p2, p2;
				v76[127] := true;
				v76[127] := f17;
				if (f17) {
					globalState.f4 := if ((fm42(p2, p2, 'a', f17, globalState)).cf34 in v86) then v86[(fm42(p2, p2, 'a', f17, globalState)).cf34] else p3[445];
					globalState.f1 := f17;
					globalState.f1 := v5 in "fmiu";
					p3[445] := (p2 * |v87|) % fm7(f17, v1, globalState);
					var v94 := new C0();
				} else {
					globalState.f9 := p3[445];
					v76[127] := (seq(0x25a, i11  => (v5))) == (p0[p3[445] := v5] + p0);
					var v95: array<int> := new int[11] [p2, p2, (if (576 in v0) then v0[576] else 0x78) / p2, -(p2 / fm50(globalState)), p3[445], 0xc4, p3[445], -(-p3[445] * p2), |p0|, p2, p3[445] * p2];
					v95 := v95;
					m0(v76[127], !((if (p3[445] in v0) then v0[p3[445]] else p3[445]) == p2), globalState);
					globalState.f1 := f17;
				}
				
				var v96: array<bool> := new bool[20](i12 => v76[127]);
				var v97: array<char> := new char[15];
				var v98 := DC1(if (v3[780][p3[445]]) then {v4[0x7d]} else p1, v96, v97);
				match (v98) {
					case DC1(cf1, cf2, cf3) =>
						globalState.f9 := p3[445];
						globalState.f1 := !((0x90 / p2) >= p3[445]);
						var v99 := new C1();
						var v100: C0 := new C0();
						var v101: map<int, C0> := map[p3[445] := v100];
						v76[127], v100, globalState.f4 := f17, if ((|p0| - p3[445]) in v101) then v101[|p0| - p3[445]] else v100, if (p2 in v86) then v86[p2] else p3[445];
					case DC0(cf0) =>
						v87, p3[445], globalState.f9 := fm57(true, v76[127], p2 % p3[445], cf0, globalState), ---p2 + p3[445], (|v91[cf0 := p3[445]]| % 811) / |p1|;
						m14(p1, f17, p0, globalState);
						globalState.f4, globalState.f4 := cf0, p3[445];
						var v102 := DC47(v10);
						var v103: multiset<D16> := multiset{v102, v102, v102, v102};
						v76[127], p3[445], globalState.f1, globalState.f1, globalState.f1 := if (fm6("ww", p2, |v103|, v5, globalState)) then false else f17, 0x135, (fm6(p0, cf0, p2, v5, globalState) || !v76[127]) <==> v76[127], p0 <= (seq(322, i13  => (v5))), f17;
					case DC2(cf4) =>
						v76[127] := v76[127];
						var v104 := "cch";
						globalState.f1, globalState.f9, v104 := f17, if (false) then fm7(v76[127], v1, globalState) else p3[445], p0 + (seq(862, i14  => (v5)));
						globalState.f4 := p2;
						v76[127] := p2 > p2;
				}
	...
	method m13(p0: map<int, array<bool>>, p1: char, p2: map<array<bool>, int>, globalState: GlobalState) returns (r0: bool, r1: int, r2: int, r3: set<bool>) {
			...
			decreases 100 - i0
		{
			...
			var v4 := DC66(v3);
			var v5: seq<C1> := [v3];
			var v6: array<C1> := new C1[29] [v3, if (f17) then v3 else v3, v3, v3, v3, v3, v4.cf108, v3, v5[v0], v3, v3, v3, v3, v3, v3, v3, v3, v3, v3, if (f17) then v3 else v3, v3, v3, v3, v3, v3, v3, v3, v3, v3];
			var v7: array<map<bool, bool>> := new map<bool, bool>[9](i1 => map[f17 := f17]);
			var v8: map<array<map<bool, bool>>, array<C1>> := map[v7 := v6];
			v2, v6, r1, globalState.f4 := v2[f17 := v0], if (v7 in v8) then v8[v7] else if (f17) then v6 else v6, v0, v0;
			r2 := -v0;
	...
	method m14(p0: set<bool>, p1: bool, p2: string, globalState: GlobalState) {
		...
		v3[48] := f17;
		var v4: seq<bool> := [f17];
		var v5 := DC30("hyvctl", v4[|"ibxvwec"|], v2);
		var v6: multiset<int> := multiset{v5.cf51, v2};
		globalState.f4, globalState.f9, v1[395], v3[48] := (|v6| - v2) + v2, |([f17] + (v4 + v4[v2 := f17]))|, v2, f17;
		var v7: seq<string> := [fm43(-443, !v3[48], v1[395], globalState)];
		var v8 := 'u';
		var v9: array<string> := new string[7] [p2, p2, v0, (fm58(v7, v1[395], v2, -|v0|, globalState))[v2 := v8], (seq(0x1c0, i0  => (v8)))[if (v1[395] in v6) then v6[v1[395]] else v1[395] := 'a'], p2, seq(709, i1  => (v8))];
		var v10 := DC11(v1[395], v9);
		match (v10) {
			case DC11(cf20, cf21) =>
				if (v3[48]) {
					var v11 := new C2(v8, v8, v3[48], v4);
					var v12: array<D6> := new D6[17];
					var v13: set<array<D6>> := {v12};
					var v14 := DC67(v13);
					v13 := v14.cf109;
					globalState.f1 := {!f17, v3[48]} >= p0;
					globalState.f1 := p1;
					v3[48] := 't' !in "uthcw";
				} else {
					var v15: seq<int> := [cf20];
					var v16: set<int> := {cf20};
					v1[395], v15, v1[395] := (if (v3[48]) then cf20 else |v16|) * 0x4e, (v15 + v15)[v1[395] := -(cf20 - 0xec)], -(v1[395] % 0x3bc);
					var v17 := new C0();
				...
				v2 := fm50(globalState);
				if (p1) {
					v1[395] := |((if (true) then v0 else "uu") + p2)|;
					cf20 := v2;
					var v20 := new C0();
					v3[48] := if (f17) then !v3[48] else v3[48];
					var v21: seq<int> := [cf20, |[p1]|];
					v3[48] := v3[48] <==> (v21 < v21);
				} else {
					v3[48], globalState.f4 := |[false]| != (v1[395] * cf20), cf20;
					var v22: map<bool, int> := map[v4[-cf20] := v2];
					v22 := v22;
					var v23: seq<int> := [-v1[395]];
					globalState.f4 := |(((seq(0xcf, i2  => (-|[f17]|))) + v23) + (seq(-506, i3  => (|map[v8 := cf20]|))))|;
					v3[48] := v3[48];
					cf20 := v1[395] + -cf20;
				}
				
				if (v3[48]) {
					globalState.f9 := v1[395];
					var v24: map<int, string> := map[v1[395] / 0x113 := "mf"];
					v24 := v24[v2 := v0];
					var v25 := new C0();
					...
					var v30: map<int, bool> := map[v2 := v4[v2]];
			...
			case DC10(cf19) =>
				...
				if (true) {
					...
					globalState.f4 := v2;
					var v37: map<int, bool> := map[v1[395] := v3[48]];
					v1[395] := |v37| / v1[395];
					...
					v41 := v41[v37[v1[395] := f17] := v40];
					globalState.f1 := v3[48];
					v1[395] := (-627 - v1[395]) / -v1[395];
				}
		...
		v43[623] := fm51(v1[395], false, globalState);
		var v44: map<int, array<int>> := map[v2 := v1];
		var v45: array<array<int>> := new array<int>[5] [v1, if (v2 in v44) then v44[v2] else v1, v1, v1, v1];
		v45[357] := if (f17) then v1 else v1;
		var v46: map<bool, bool> := map[f17 := p1 && f17];
		v3, v43[623], globalState.f9, globalState.f9, v45[357] := v3, v46, 0x33d, v1[395], v1;
		v3[48] := (v4 + v4) != v4;
		for i5 := v2 to v2 {
			v9[386] := fm43(i5, v3[48], v2, globalState);
			var v47: map<array<int>, int> := map[v1 := i5];
			var v48 := DC25(v47);
			v1[395], v9[386], v48 := i5, (p2[i5 := v8] + v0) + p2, v48;
			var v49: array<D9> := new D9[5];
			var v50 := DC27(0x1c5, -0x34a, v2, v2);
			var v51 := DC28(v50);
			v49[269] := v51.(cf47 := v50);
			globalState.f9, v0, v1[395], v49[269] := v2, v9[386][v1[395] := v8], |(map v52 : char | v52 in (seq(0x373, i6  => (v8))) :: (v52) := (v3[48]))| * 0x276, DC28(v50);
			var v53: multiset<bool> := multiset{v3[48], p1, f17 && p1};
			v53 := v53;
			globalState.f1 := f17;
		}
	}
...
class C4 extends T0, T1 {
	...
	function fm6(p0: string, p1: int, p2: int, p3: char, globalState: GlobalState): bool {
		"byx" != ((seq(298, i0  => (f22))) + "ulb")
	}
	function fm7(p0: bool, p1: map<int, bool>, globalState: GlobalState): int {
		(|"dstj"| / ---|map[|"xwv"| := f17]|) * (|[f17, f17]| / 0xb7)
	}
	function fm8(p0: int, p1: int, p2: map<int, seq<bool>>, globalState: GlobalState): char {
		'i'
	}
	...
	method m3(p0: bool, p1: int, p2: char, globalState: GlobalState) returns (r0: D1, r1: int, r2: bool) {
		...
		match (v2.(cf4 := if (p0) then v1 else v1)) {
			case DC1(cf1, cf2, cf3) =>
				...
				if (v4 != "dae") {
					...
					v0[80], v0[80] := f17, true;
					var v8: map<string, set<bool>> := map[v4 := cf1];
					var v9: seq<bool> := [f17, p0];
					var v10: multiset<int> := multiset{p1, p1};
					var v11: map<bool, string> := map[v9[if (p1 in v10) then v10[p1] else v3.fm14(globalState)] := v4];
					v8 := v8[if (false) then v4 else if (v0[80] in v11) then v11[v0[80]] else v4 := fm23(f22, p1, globalState)];
				} else {
			...
			case DC0(cf0) =>
				if (v0[80]) {
					var v15 := DC0(p1 - p1);
					var v16 := "lmcld";
					v0[80], globalState.f9, v15, r2, globalState.f4 := !fm6((seq(408, i1  => (f22))) + v16, cf0, cf0, p2, globalState), if (true) then fm7(f17, map[0x22b := f17], globalState) else p1, fm24(globalState), fm0(globalState), cf0;
					var v17: seq<bool> := [f17];
					var v18: map<int, bool> := map[|v17| := f17];
					var v19: seq<int> := [|multiset{fm7(p0, v18, globalState)}|];
					var v20: array<D0> := new D0[2] [v15, DC0(v19[p1])];
					...
					globalState.f1 := v0[80];
					v16 := "cr";
					var v23: set<bool> := {v17[p1]};
					var v24: map<bool, bool> := map[p0 := f17];
					var v25: seq<map<bool, bool>> := [v24, v24, v24, map[f17 := v0[80]]];
					var v26: array<map<bool, bool>> := new map<bool, bool>[13] [v24, v24[v0[80] := v0[80]], map[p0 := f17], v24, v24, v24, v24, v25[0x294], v24, map[v0[80] := f17], v24, v24, v24];
					var v27 := DC5(DC0(p1), v26, v0[80], cf0, p2);
					v23 := {v27.cf8};
				} else {
					var v28: seq<bool> := [f17, v0[80], v0[80]];
					v0[80] := fm22(v28, f22, globalState);
					v0[80] := cf0 <= p1;
					var v29: map<int, bool> := map[p1 / cf0 := if (p0) then p0 else p0];
					var v30 := "ndhwwr";
				...
				var v33: array<string> := new seq<char>[3](i2 => seq(0x219, i3  => ('k')));
				var v34 := DC11(cf0, v33);
				var v35: seq<int> := [p1];
				var v36: map<int, bool> := map[cf0 := f17];
				var v37: array<int> := new int[12] [p1, cf0, 0x8e, v34.cf20, v35[p1], cf0, p1, p1, v35[cf0], cf0, fm7(p0, v36, globalState), cf0];
				var v38: map<array<int>, bool> := map[v37 := true];
				v38 := v38;
				globalState.f9 := cf0;
				v0[80], globalState.f9, v0 := p1 == |v36|, -cf0, v0;
			case DC2(cf4) =>
				m0(true, !fm0(globalState), globalState);
				globalState.f9 := 0x268;
				var v39: array<int> := new int[22](i4 => i4 % p1);
				var v40: map<int, int> := map[p1 := 58];
				v39[325] := -(396 * (if (p1 in v40) then v40[p1] else p1));
				v39[325] := fm7(v0[80], map v41 : int | (814 <= v41) && (v41 < 88) :: (v41 + p1) := (true), globalState) * p1;
				var v42 := DC0(p1);
				var v43: array<map<bool, bool>> := new map<bool, bool>[20](i5 => map[v0[80] := !v0[80]]);
				var v44 := DC5(v42, v43, false, v39[325], f22);
				var v45: array<D1> := new D1[5] [v44, v44, v44, DC5(v42, v43, p0, v39[325], p2), v44];
				var v46: multiset<array<D1>> := multiset{v45, v45, v45};
				if (v46 > v46) {
					var v47: multiset<bool> := multiset{v0[80]};
					v0[80] := if (v0[80]) then v47 !! v47 else v0[80];
					var v48: seq<bool> := [p0];
					v0[80] := v48 != v48;
					var v49 := new C0();
					globalState.f9 := p1;
		...
		match (fm25(p1, p0, globalState)) {
			case DC13(cf23) =>
				...
				var v54: seq<bool> := [v0[80]];
				var v55: seq<multiset<bool>> := [v53[v54[cf23] := p1] - v53, v53 - v53];
				v55 := v55;
				var v57: map<int, int> := map[p1 := |(set v56 : int | (0x186 <= v56) && (v56 < 271) :: (v56 + -213))| / cf23];
				v57 := v57[p1 := cf23];
			case DC14(cf24, cf25, cf26, cf27) =>
				var v58 := new C0();
				v0[80] := cf25 < cf25;
				var v59: map<int, bool> := map[cf27 - cf27 := v0[80]];
				var v60: array<map<bool, bool>> := new map<bool, bool>[10](i6 => map[p0 := p0]);
				var v61 := DC5(DC0(p1), v60, v0[80], cf27, f22);
				...
				match (DC4()) {
					case DC4() =>
						...
						var v64: array<D4> := new D4[17] [v63, v63, v63, v63, v63, v63.(cf26 := p0, cf27 := cf27), v63, v63, v63.(cf27 := |cf25|, cf24 := !false, cf25 := ([f22])[cf27 := f22]), v63, v63.(cf27 := |fm26(globalState)|, cf24 := cf26), v63, v63, fm25(p1, false, globalState), v63, v63, v63];
						v64 := if (cf24) then v64 else v64;
					case DC5(cf6, cf7, cf8, cf9, cf10) =>
						globalState.f1 := !p0;
						cf9 := cf27;
						var v65: array<string> := new seq<char>[11] [cf25, (cf25 + cf25)[cf9 := f22], cf25[0x3cd := p2], seq(0x2c6, i7  => (cf10)), cf25 + "rvvs", cf25, cf25 + cf25, cf25, seq(0x3c5, i8  => (cf10)), cf25 + cf25, cf25];
						v65 := v65;
						var v66: array<char> := new char[17];
						var v67: set<bool> := {cf24};
						var v68: set<int> := {|cf25|, cf27, cf27, cf27, p1};
						var v70: map<bool, bool> := map[DC1({true}, v0, v66).cf1 !! v67 := v68 > (set v69 : int | (0 <= v69) && (v69 < 0x3a4) :: (v69 % cf27))];
						v70 := v70;
					case DC6(cf11) =>
						var v72 := DC13(fm7(p0, (map[cf11 := cf26])[p1 := cf26], globalState));
						var v73: seq<bool> := [true, cf24];
						globalState.f9 := (DC14(false, seq(0x2c1, i9  => (f22)), true, p1).(cf27 := fm7(f17, map v71 : int | v71 in [v72.cf23, |[fm22(v73, p2, globalState), f17]|] :: (v71 * cf27) := (p0), globalState))).cf27;
						...
						var v80: map<bool, D2> := map[v0[80] := v79];
						var v81 := DC18(cf11, |v80|);
						globalState.f1, v74[77], v75[236], globalState.f1 := fm6(if (cf24) then cf25 else cf25, -p1, -v81.cf33 + v61.cf9, p2, globalState), p1, v76, p0;
					...
					case DC7(cf12) =>
						...
						r1, v0[80], v0[80], r2 := (0x259 % |v87|) % v58.fm13(globalState), p0, f17, fm22(v87, p2, globalState);
						cf24 := cf24;
				}
			...
			case DC12(cf22) =>
				var v88 := new C0();
				if (f17) {
					var v89: multiset<bool> := multiset{!p0, f17, p0, v0[80]};
					var v90 := DC19(v89);
					var v91: array<multiset<bool>> := new multiset<bool>[9] [v89, v89, v89, v89, v89, multiset{v0[80], f17}, v89, v90.cf35 - v89, v89];
					v91[902] := (multiset{true, p0})[v0[80] := |(seq(0xe5, i11  => (p1)))|];
					v91[902] := v89 + v89[v0[80] := p1];
					var v92: set<bool> := {v0[80], p0};
					...
					v95 := v95[v94 := p0];
					cf22 := cf22[-0x395 := fm6(seq(0x34c, i12  => (f22)), p1, p1, p2, globalState)];
					var v96: map<bool, seq<string>> := map[v0[80] := ["iqqijrbyp"]];
					v96 := v96[true := seq(0xb3, i13  => ("bx"))];
				} else {
					var v97 := 'w';
					v97 := p2;
					var v98 := "cn";
					var v99: multiset<int> := multiset{|v98|};
					var v100: map<multiset<int>, bool> := map[v99 := v0[80]];
				...
				if (false) {
					...
					v104 := DC14(!v0[80] ==> !true, v105, f17, 0xa);
					r1, v0[80] := p1, v0[80];
					...
					var v109 := "ubqaaswiq";
					var v110: map<int, string> := map[-406 := (seq(0x227, i14  => (p2)))[p1 := f22]];
					var v111: seq<int> := [p1];
					var v112: seq<string> := [if (f17) then v109 else v109, v109 + (if (v111[p1] in v110) then v110[v111[p1]] else seq(0x1a3, i15  => (f22))), v109, v109, v109];
					...
					v114, globalState.f1, globalState.f4, v0[80] := fm29(v115 + [f17, p0, f17, f17, false], f22, globalState), map[!f17 := v118] != v119, -(p1 / p1), f17;
					var v120: set<bool> := {v0[80]};
					...
					globalState.f9, v118[476], v0[80], v111 := p1 - p1, |v109| / -p1, v124 != v124, v111;
					v118[476] := if (p0) then v118[476] else p1;
				}
				
				var v125: seq<bool> := [p0, p0, v0[80], !p0, p0];
				var v126: map<bool, int> := map[v125 < v125 := p1];
				v126 := v126[f17 := p1];
			case DC15(cf28) =>
				var v127 := "nilqtapg";
				var v128: map<int, bool> := map[|v127| := !v0[80]];
				var v129: set<int> := {p1};
				...
				if (false !in v134) {
					...
					var v138: multiset<bool> := multiset{v0[80]};
					var v139: array<seq<int>> := new seq<int>[16] [v133, v133, v133, seq(769, i17  => (p1)), if (!p0) then v133 else v133, fm30(!f17, false, globalState), v133, v133, ([p1, p1, p1])[|v127| := p1], ([|(set v135 : int | v135 in v133 :: (v135 - |multiset{true}|))|, p1])[|v127| := p1] + [p1, p1], fm30(v0[80], f17, globalState), v133, v133, v136.cf38 + v133, v137[|v138|] + v133, v133];
					v139 := new seq<int>[21];
					var v140: seq<bool> := [!p0, v0[80], v0[80], v0[80], f17];
					globalState.f4 := |((v134 + map[f17 := |v140|]) + v134)|;
					r2, v0[80] := if (v0[80]) then v140[p1] else f17, v133 != v133;
					globalState.f1 := fm6(v127, p1, p1, f22, globalState) <== v0[80];
					v0[80] := true;
				} else {
					var v141: array<int> := new int[19];
					v141[673] := p1;
					v141[673] := p1 * |v133|;
					var v142: map<int, seq<int>> := map[p1 := ([p1])[v141[673] := v141[673]]];
					var v143: array<seq<int>> := new seq<int>[17] [v133, v133, (seq(-38, i18  => (-v141[673]))) + v133, v133, v133, [v141[673]], v133 + v133, v133, v133, v133, seq(0x2b2, i19  => (v141[673])), if (0x10d in v142) then v142[0x10d] else v133, v133, [p1] + v133, v133, v133, v133];
					var v144: map<int, int> := map[p1 := p1];
					v143[218] := [if (v141[673] in v144) then v144[v141[673]] else v141[673], p1, if (v141[673] in v144) then v144[v141[673]] else v141[673]];
					v143[218] := ([p1, p1, v141[673], p1, 874])[0x7f := v141[673]];
					var v145: array<string> := new string[27];
					v145 := v145;
					v127, v141[673], v0[80] := fm26(globalState), 0x220, v134 in [v134];
					globalState.f4 := p1 / v141[673];
				}
				
				var v146: map<int, int> := map[p1 - p1 := p1];
				var v147: seq<string> := [v127];
				v146 := v146[0x2f6 := |(v147[p1] + v127)|];
		}
		...
		globalState.f9 := |fm31(([-0x139])[p1 := -|v148|], p1 / p1, v151, v152 + [p0], globalState)|;
		var v153: seq<int> := [p1];
		var v154: seq<seq<int>> := [v153];
		var v155 := DC22(v153);
		var v156: array<seq<int>> := new seq<int>[11] [v153, v153, seq(0x36a, i20  => (p1)), v153, v154[p1], v153 + v153, v153, v153, fm30(true, f17, globalState), v155.cf38, v153];
		v156[139] := v153[p1 := p1];
		v156[139] := v153;
		if (!!f17) {
			var v157: map<int, bool> := map[-0x2de := true];
			var v158: map<int, char> := map[fm7(p0, v157, globalState) := v148[|map[-p1 := p0]|]];
			...
			v161 := v161[v159 + map[map v162 : int | (0x1ce <= v162) && (v162 < -0x31d) :: (v162 % 0x190) := (p2) := |v148|] := v160];
			var v163: map<bool, bool> := map[f17 := p0];
			globalState.f1 := v152[|v163|];
			...
			match (fm32(f17, p0, true, globalState).(cf35 := v164)) {
				case DC20(cf36) =>
					...
					var v168: set<bool> := {multiset{v166} > multiset(v167), v0[80], p0, p0, false};
					v168 := v168;
					globalState.f1 := f17;
					var v169 := 'h';
					var v170: array<char> := new char[2];
					v170[827] := f22;
					var v171: array<int> := new int[17](i21 => i21 / p1);
					var v172 := DC10(v171);
					var v173: seq<map<bool, D3>> := [map[v0[80] := v172]];
					var v174: map<bool, int> := map[p0 := 791];
					var v175: multiset<int> := multiset{p1 * p1, -(0x38e - |v152|), -|v174|};
					v169, v170[827], v173, v169, v175 := 'p', p2, v173, p2, v175;
					var v176: map<int, int> := map[0x364 := 987];
					var v177 := DC24(multiset{v176});
					globalState.f9 := |v177.cf40|;
				case DC19(cf35) =>
					globalState.f4 := (p1 - -p1) / (p1 - -748);
					var v178: multiset<int> := multiset{p1};
					v0[80] := (multiset(v156[139]) * multiset(v156[139])) > v178;
					globalState.f1 := v0[80];
				...
				case DC21(cf37) =>
					...
					globalState.f1, v184, v148, globalState.f9, v0[80] := v0[80], v184, ("y")[|v185| := 'w'], p1 / p1, fm0(globalState);
			}
			
			match (DC18(-822, -0x2f1)) {
				case DC17(cf30, cf31, cf32) =>
					globalState.f1 := v0[80] <== p0;
					var v186: array<multiset<int>> := new multiset<int>[18](i22 => multiset{|v163[p0 := v0[80]]|} + multiset{709, cf30});
					var v187: multiset<int> := multiset{cf30};
					v186[381] := v187 + v187;
					var v188: set<bool> := {p0, f17, f17};
					var v189: array<char> := new char[10];
					var v190 := DC1(v188, v0, v189);
					var v191: map<int, D0> := map[cf30 - -|v156[139]| := v190];
					globalState.f9, v0[80], r2, globalState.f1, v186[381] := |v191|, (cf31 / p1) == cf31, f17, !f17, multiset{cf31};
				...
				case DC18(cf33, cf34) =>
					m11(p1, |(if (fm0(globalState)) then v156[139] else v153)|, v0[80], p0, globalState);
					var v196 := DC23(f17);
					var v197 := DC14(!v196.cf39, v148, v0[80], cf33);
					v0[80] := v197.cf26;
					globalState.f1 := false;
					...
					var v202: map<bool, int> := map[v0[80] := cf33];
					v0[80], globalState.f4, r1, v200 := |(v157 + v157)| == |v201[cf33 := cf34]|, |"vfjmssa"| + |v148|, |v202|, v200;
				case DC16(cf29) =>
					var v203: multiset<multiset<bool>> := multiset{v164, multiset(v152)};
					var v204: seq<map<int, bool>> := [v157];
					globalState.f4 := fm7(v164 in v203, v204[p1], globalState);
					var v205: map<bool, int> := map[v0[80] := p1];
			...
			globalState.f9, globalState.f4, globalState.f1 := if ((p1 <= fm7(f17, map[p1 := false], globalState)) in v164) then v164[p1 <= fm7(f17, map[p1 := false], globalState)] else p1 % p1, -p1, false;
		} else {
			var v216 := new C0();
			var v217 := new C0();
			if (if (fm22(v152, 'c', globalState)) then v0[80] else v0[80]) {
				v0[80] := v0[80];
				var v218: array<map<bool, D2>> := new map<bool, D2>[22];
				var v219: array<array<map<bool, D2>>> := new array<map<bool, D2>>[22] [v218, v218, v218, v218, v218, v218, v218, v218, v218, v218, v218, v218, v218, v218, v218, v218, if (v0[80]) then v218 else v218, v218, v218, v218, v218, v218];
				v219[65] := v218;
				var v221: seq<seq<bool>> := [v152];
				v219[65], v0[80], globalState.f1, r2, globalState.f1 := v218, p0, v0[80], p0, !fm22((if (v152[|(set v220 : int | (-0x2ba <= v220) && (v220 < -0x2c0) :: (v220 + -p1))|]) then v221[p1] else v152)[p1 := p0], f22, globalState);
				v0[80] := true;
				var v222: set<int> := {p1};
				v222 := v222;
				var v223 := new C0();
			...
			match (v227) {
				case DC20(cf36) =>
					...
					var v233 := DC0(p1);
					var v234: map<bool, bool> := map[v0[80] := v0[80]];
					var v235: seq<map<bool, bool>> := [fm36(p1, v230, globalState)];
					var v236: array<map<bool, bool>> := new map<bool, bool>[18] [v234[p0 := p0], map[p0 := v0[80]], v234, v234, v234, v234[f17 := false], v234, map[f17 := p0], v234, v234, v234, map[if (f17 in v234) then v234[f17] else p0 := f17], map[v0[80] := v0[80]], v234, map[v152[p1] := f17], v235[p1], v234, v234];
					var v237: array<int> := new int[12] [0x62, p1, |v229|, fm7(f17, v230, globalState), p1, p1, |v232|, |v153|, p1, p1, DC5(v233, v236, f17, p1, p2).cf9, |(seq(0x45, i23  => (f22)))|];
					var v238: map<array<int>, int> := map[v237 := p1];
					v238 := DC25(v238).cf41;
					v231[725] := |(set v239 : int | (0x2ec <= v239) && (v239 < 900) :: (v239 / p1))| * |v148|;
					var v240: multiset<array<int>> := multiset{v237};
					var v241: map<multiset<int>, multiset<array<int>>> := map[multiset{p1} := v240];
					var v242: multiset<int> := multiset{|v152|, |(seq(815, i24  => (p1)))|};
					v231[725] := -(|(if (v242 in v241) then v241[v242] else v240)| + p1) / (p1 / p1);
					globalState.f4 := p1;
				case DC19(cf35) =>
					r2 := !((|v148| / v153[p1]) == p1);
					...
					var v246: map<int, int> := map[p1 := |(v152[p1 := v0[80]])[|v245| := p0]|];
					...
					v248 := v248;
					v0[80] := if (!p0) then p0 else if (f17) then !fm6(v148, p1, p1, 'a', globalState) else false;
				case DC21(cf37) =>
					var v249: array<int> := new int[3];
					var v250: map<bool, int> := map[p0 := p1];
					v249[92] := if (f17 in v250) then v250[f17] else 0x382;
					v249[92] := p1;
					globalState.f1 := f17;
					m11(v249[92], v249[92], false, v0[80], globalState);
					var v251 := 'g';
					var v252: map<bool, bool> := map[p0 := f17];
					var v253: seq<map<bool, bool>> := [v252];
					v251, v249[92], globalState.f1 := v251, (|v253| / p1) % p1, false;
			}
			
		}
		
		var v254 := DC0(p1);
		var v255: map<bool, bool> := map[p0 := v0[80]];
	...
	method m4(p0: seq<char>, p1: set<bool>, p2: int, p3: array<int>, globalState: GlobalState) {
		...
		if (f17 in v0) {
			...
			globalState.f1 := !f17;
			var v1: array<map<int, int>> := new map<int, int>[1](i0 => map[p2 := |[p2, |map[p2 := f17]|, -p2, p2]|] + map[-|map[|(seq(175, i1  => (0xb8)))| := map[|{p2}| := f17]]| := |map[map[f17 := f17] := f17]|]);
			v1 := v1;
			var v2 := DC26(p2 < p2);
			match (v2) {
				case DC26(cf42) =>
				...
				case DC27(cf43, cf44, cf45, cf46) =>
					...
					globalState.f9, v8, v9, v10 := (if (f17) then cf46 else cf45) * (|v0| % cf46), [p2, |v10|, 50], v11[|(v0 + v0)|], "cs";
					globalState.f1 := !f17;
					p3[496] := cf44;
					var v12 := DC14(f17, p0, f17, cf43);
					p3[496], globalState.f9 := cf44, (p2 % |v12.cf25|) / 589;
				case DC25(cf41) =>
					globalState.f1 := f17;
					var v13: set<int> := {p2 % p2, p2 * p2};
					var v14: multiset<bool> := multiset{f17};
					var v15: multiset<int> := multiset{p2, p2, |v14|};
					v13 := set v16 : int | v16 in v15[p2 := p2] :: (v16 * |(seq(0x1e2, i2  => (0x39a)))|);
					globalState.f1 := f17;
					var v17: seq<int> := [p2, -106];
					globalState.f9 := |(seq(0x299, i3  => (|(seq(-336, i4  => (|v13|)))|)))| - v17[p2];
				case DC28(cf47) =>
			...
			match (DC44(-p2)) {
				case DC44(cf74) =>
					...
					var v25: multiset<seq<bool>> := multiset{v0, [false, f17, f17, f17, f17] + [f17], if (false) then v0 else v0, v0[-|multiset(v24[cf74 := p2])| := f17], v0};
					cf74 := |v25|;
				...
				case DC45(cf75, cf76) =>
					cf76[85] := p2;
					cf76[85] := p2;
					m12(f17 ==> f17, -p2, -cf76[85] / cf76[85], f22, globalState);
					var v28 := "spbbdmnj";
					var v29 := DC60(cf75);
					var v30: C0 := new C0();
					var v31: map<C0, int> := map[v30 := p2];
					var v32: map<D12, map<C0, int>> := map[DC38(v30, cf76[85]) := v31];
					globalState.f4, globalState.f9, globalState.f9, v28, v29 := --cf76[85] - (p2 / p2), |v32|, cf76[85], p0, v29;
					var v33: set<int> := {p2, p2, cf76[85], -0xab, cf76[85]};
					cf75 := v33 !! (v33 - v33);
				case DC43(cf73) =>
					var v34 := "aembg";
					p3[784] := p2;
					var v35 := 'q';
					v34, p3[784], v35, globalState.f9 := "mler", p2, f22, fm50(globalState);
					var v36: seq<int> := [p2, p3[784]];
					var v37: map<seq<int>, bool> := map[v36 := f17];
					var v39: map<seq<bool>, int> := map[v0 := |v0[p2 := f17]|];
					var v40: map<seq<int>, int> := map[v36 := if ([f17, !f17] in v39) then v39[[f17, !f17]] else p3[784]];
					v37 := map v38 : seq<int> | v38 in v40 :: (v38) := (f17);
					var v41: multiset<int> := multiset{279};
					var v42: set<multiset<int>> := {multiset((seq(0x25b, i5  => (p2))) + (seq(373, i6  => (p3[784])))), v41, v41, v41, v41 - v41};
		...
		var v47: map<int, bool> := map[|(if (f17) then v45 else map v46 : int | (0x2f1 <= v46) && (v46 < 0x311) :: (v46 / p2) := (|v0|))| := f17];
		...
		if (f17) {
			...
			if (!f17) {
				...
				globalState.f1, globalState.f1 := v0[p2], fm64(|p1|, |v52|, globalState) < fm64(p2, p2, globalState);
				...
				var v61: multiset<bool> := multiset{f17};
				globalState.f9, globalState.f4, globalState.f2, p3[671], globalState.f9 := p2, v58[0x251] / |(v59.(cf22 := v60[-|v61|])).cf22|, v0 + fm59(f17, globalState), p2 % p2, -(p2 + p2) - p2;
			...
			v67[429] := v69;
			globalState.f1, v66, globalState.f1, v67[429] := p2 <= p2, p0[p2 := v65.f23], !f17, v69;
			var v70: set<char> := {'l'};
			if ((v70 < v70) <==> f17) {
				globalState.f1 := f17;
				v66 := "xn";
				...
				var v73: array<char> := new char[24] [v65.f23, v65.f24, v65.f23, v65.f23, v65.f24, v65.f24, v65.f24, f22, f22, v65.f23, v65.f23, 'g', 'l', v65.f24, v65.f23, v65.f23, f22, v65.f23, v65.f24, f22, v65.f23, v65.fm8(238, p2, map v72 : int | (0x126 <= v72) && (v72 < -0x3a3) :: (v72 / 0x35e) := (v0), globalState), f22, v65.f24];
				globalState.f1 := v73 !in [v73, v73, v73, v73];
				globalState.f1 := f17;
				v68 := v68;
				var v74: seq<string> := [seq(546, i8  => (v65.f24))];
				var v75: seq<string> := [v74[p2], "spvotyik"];
				v74 := v75;
		...
		if ((v80 + v80) >= v80) {
			...
			v78 := v78 + (v78 + fm30(f17, f17, globalState))[p2 := p2];
			var v84 := "xxovxial";
			var v85 := DC30(("nikofhbja")[60 := f22], f17, p2);
			...
			if (f17) {
				var v87 := new C0();
				globalState.f1, globalState.f9 := if (v86[351]) then v86[351] else p2 < p2, -52 / -v87.fm14(globalState);
				globalState.f4 := p2 % (p2 + p2);
				p3[100] := p2;
				var v88: array<seq<bool>> := new seq<bool>[1] [v0];
				v88[525] := v0;
				p3[100], v88[525] := p2 - p2, v0;
				var v89 := DC30(p0, v86[351], p3[100]);
				p3[100] := |([f22, f22, f22, f22] + v89.cf49)| * (p2 + p2);
			} else {
	...
	method m11(p0: int, p1: int, p2: bool, p3: bool, globalState: GlobalState) {
		...
		var v4: array<int> := new int[6] [p1, 0x22a, p1, 0xc0, |v3|, |fm28(globalState)| / p0];
		...
		match (v11) {
			...
			case DC29(cf48) =>
				...
				v5[474] := p3;
				v5[474] := f17;
				var v15: array<bool> := new bool[13] [f17, false, v5[474], p3, v5[474], p3, p2, f17, p3, v5[474], p3, p3, v5[474]];
				...
				match (if (fm0(globalState)) then v18 else DC34(p2, p3, 437)) {
					case DC32(cf53, cf54) =>
						...
						var v19 := "x";
						v19 := fm58(seq(0x9a, i3  => (v19)), p1, p1, cf54, globalState);
						var v20: map<int, seq<bool>> := map[v18.cf60 % |multiset{p1, p1}| := v13 + v13];
						globalState.f4, v20, globalState.f1 := p1, v20, p2;
					case DC33(cf55, cf56, cf57) =>
						v5[474] := p3;
						var v21: map<bool, bool> := map[f17 := f17];
						var v22: set<int> := {p1, |v21| * cf55, v9[cf57]};
						v22 := v22;
						...
						var v27 := DC24(v26[0x11]);
						v27 := v27;
						var v28: array<D21> := new D21[7];
						v28 := v28;
					case DC34(cf58, cf59, cf60) =>
						v0[91] := cf60;
						v0[91] := if (v5[474]) then p1 else 0x2a2;
						var v29: map<int, int> := map[cf60 := if (p2) then 0x387 else cf60];
						v29 := v29 + v29;
						v5[474] := v5[474];
						var v30: multiset<bool> := multiset{v5[474]};
						var v31: map<bool, int> := map[|v30| == cf60 := p1];
						v31 := (fm45(cf60, cf58, fm22([true], f22, globalState), cf58, globalState) + v31)[f17 := cf60 - v0[91]];
					case DC31(cf52) =>
						globalState.f9 := p1;
						var v32 := "bstxmfi";
						var v33: map<char, string> := map[f22 := seq(0xbc, i4  => (f22))];
						var v34: array<string> := new string[16] [v32 + (if (f22 in v33) then v33[f22] else v32), v32, v32 + v32[p0 := f22], seq(641, i5  => (f22)), v32 + (seq(-422, i6  => (f22))), v32, "i", v32, v32, v32, v32, v32, v32, (seq(0x203, i7  => (f22)))[0x6a := 't'], v32, (seq(-0x7b, i8  => (f22))) + v32];
						v34[519] := "uwgisqs";
						v34[519] := (v32 + v32)[p0 + p1 := f22];
						var v35: set<bool> := {v5[474]};
						globalState.f4 := |v35|;
						globalState.f1 := fm6(v32, p0, p0, f22, globalState);
					case DC35(cf61) =>
						var v36: set<bool> := {p3};
						var v37: multiset<bool> := multiset{!(v36 >= v36), p0 >= p0, false, !v5[474]};
						...
						v5[474], v39, v42, globalState.f4 := v5[474], (fm68(p2, p2, if (p2 in v43) then v43[p2] else p3, globalState))[p1 := v44], v42 + v42[v41 := |v36|], |fm58(v46, p1 / fm50(globalState), -|(v44 + v44)|, -(fm7(!v5[474], map v47 : int | (-272 <= v47) && (v47 < 0x2d) :: (v47 * p1) := (v5[474]), globalState) + p0), globalState)|;
						v5[474] := f17;
				}
	...
	method m12(p0: bool, p1: int, p2: int, p3: char, globalState: GlobalState) {
		...
		if (p1 == (p1 / p2)) {
			...
			var v6: map<bool, map<multiset<int>, int>> := map[true := map[v5[|(seq(0x271, i0  => (v1.f23)))|] := p2]];
			...
			v10[54], globalState.f4, v12[410] := v15, if (-0x2b2 in v18) then v18[-0x2b2] else |v18| % p2, f17;
			globalState.f1 := !v12[410];
			globalState.f4 := p2 - ((if (v9[0x314] in v18) then v18[v9[0x314]] else p1) + -p1);
		} else {
			var v19 := DC6(p2);
			var v20 := "twseyr";
			...
			v22[807] := {v23, v23, v23, v23};
			v22[807] := {v23, v23, v23};
			if (("ieygymdu" <= v20) <== p0) {
				var v24: array<int> := new int[28](i3 => i3 / p2);
				v23[444] := v0[p2];
				v24, globalState.f9, globalState.f4, v23[444] := v24, -|fm51(p1 % p1, v1.fm6(v20, p2, 0x3cb, v1.f23, globalState), globalState)|, 622, !p0;
				var v25: array<bool> := new bool[1];
				var v26: seq<array<bool>> := [v25, v25];
				v26 := v26 + DC63(v26).cf104;
				globalState.f9 := p2 / (p1 % p2);
				var v27: map<bool, char> := map[v23[444] := p3];
				...
				v30, v23[444], globalState.f4, v31, v24 := map[|(v32 + v32)| := f17], v33.cf39, -(if (v23[444]) then p2 else p1) % v35.cf64, map[v20 + (seq(888, i4  => (v1.f23))) := if (v36 in v37) then v37[v36] else v24], v24;
				v24[821] := 910;
				v24[821] := fm50(globalState);
			} else {
				var v38: set<int> := {p1};
				v38 := v38;
				...
				var v48: map<int, multiset<int>> := map[|v41[p2 := |v45|]| := v47];
				var v49: seq<multiset<int>> := [v47];
				var v50: array<multiset<int>> := new multiset<int>[25] [if (|v41| in v48) then v48[|v41|] else v47, v47, v47, v47, v47, v47 * v47, v47, multiset{|v45|}, v47 + multiset{p1, p1, p1}, v47, v49[|map[p1 := 0x3a1]|], v47 + v47, multiset{v41[p2], -0x10e}, fm64(p2, p1, globalState), v47, v47, v47, v47 - v47, v47, v47, multiset{p2, p2, |v0|, p1}, v47, v47, v47, v47 - v47];
				v50[428] := v47 + v47;
				var v51: multiset<char> := multiset{'h'};
				v50[428] := (multiset{p2})[p1 := |v51|];
			}
			
			var v52: set<int> := {p2, p1};
			if ((v52 * v52) > v52) {
				var v53: array<array<int>> := new array<int>[18];
				var v54: array<int> := new int[21](i5 => i5 / p2);
				v53[182] := v54;
				var v55: seq<int> := [p2];
				...
				v53[182] := new int[29] [-0x30c, |v20|, p1 - p1, --p2, |fm59(p0, globalState)|, p2 - |v56|, p2, p1, p1, -p1, if (f17) then p1 else p2, --0x234, p1, fm7(v58.cf80, v59, globalState), p2, v55[p1], p2 * p1, p1 + p1, fm7(v60.cf100, v59, globalState), |v61|, 0x388, fm7(p0, v59, globalState), |(seq(0x2f, i6  => (v1.f23)))|, p2, p1, p2, fm7(f17, v59, globalState), |v62|, -(p1 - -p2)];
				var v63: multiset<bool> := multiset{f17};
				globalState.f9 := ((if (p0 in v63) then v63[p0] else fm7(f17, v59, globalState)) / p2) * p2;
				v23[544] := p0;
				var v64: map<bool, int> := map[f17 := |v20|];
				var v65: array<seq<bool>> := new seq<bool>[10](i8 => v0);
				var v66 := DC42([false], v23, p0, v65);
				v23[544] := (p1 + |multiset((seq(0x202, i7  => (|map[false := -0x2c2]|)))[if (p0 in v64) then v64[p0] else p1 := p1])|) >= (|v66.cf69| / p2);
				var v67 := DC40(DC39(p0, p2));
				v67 := v67;
				v54[770] := v1.fm7(v23[544], v59, globalState);
				...
				globalState.f1, v69[994] := f17, if (p0) then v72 + (if (p1 in v73) then v73[p1] else v72) else v72;
				var v74: C0 := new C0();
				v74 := v74;
				var v75: multiset<int> := multiset{p1};
				globalState.f1 := v75 <= (v75[p1 := p1])[p1 := p1];
				var v76 := DC4();
				...
				var v83: array<int> := new int[26](i9 => i9 % p1);
				v82[331] := v83;
				var v84: set<char> := {v1.f23};
				globalState.f4, globalState.f1, v81, v82[331], globalState.f1 := p2, v84 !! v84, (v81 + v81) - v81, v83, f17;
			}
			...
			decreases 100 - i10
		{
			...
			var v87: map<seq<multiset<int>>, bool> := map[(seq(-388, i11  => (multiset{|v0|, |{f17, f17}|, p1}))) + v86 := f17];
			v87 := v87[v86 + (seq(0x297, i12  => (v85))) := p0];
			var v88 := "twitb";
			v88 := "emahmexr" + v88;
		...
		if ((p2 > p2) <== fm0(globalState)) {
			...
			v91[494] := false;
			globalState.f9, globalState.f1, v91[494] := p1, false, f17 && p0;
			globalState.f1 := v91[494];
			...
			v101[452] := v103 == v104[p1];
			var v105: array<array<bool>> := new array<bool>[29];
			v105[308] := v101;
			v105[308] := v101;
			if ((if (v101[452]) then false else true) <== (p1 <= p1)) {
				var v106 := v1.m16(p1, globalState);
				globalState.f1 := v101[452];
				v101 := new bool[1] [v101[452]];
				var v107: map<bool, int> := map[p0 := -v106];
				globalState.f1, globalState.f4 := p0, if (v101[452] in v107) then v107[v101[452]] else p1;
				v100[405] := v1.f23;
			} else {
				globalState.f4 := 409;
				var v108: C0 := new C0();
				...
				var v117: array<int> := new int[11] [236, if (0xfc in v111) then v111[0xfc] else p2, |v112| * p1, fm7(f17, v113, globalState), -0x1b5, |(v111 + v111)|, p2, p2, |v114|, p2, v116[p1]];
				v117[961] := p1;
				var v118: seq<array<bool>> := [v105[308], v101];
				globalState.f4, v117[961] := p2 * (-|v118| / p2), (p2 + v116[p1]) * (p1 - p2);
				v100[405] := v112[if (f17) then |v112| else v117[961]];
			}
			
			v100[405] := v1.f23;
		}
		
		var v119: array<int> := new int[22](i15 => i15 / -p1);
		var v120 := "nikcd";
		v119[335] := |(v120 + v120)|;
		...
		v119[335], v121[348], v123, v124 := -|(v0 + v0)|, fm43(p1, v125 !! v125, |v120|, globalState), v123, v124;
	}
}
	...
	method m9(p0: seq<int>, p1: int, p2: int, p3: array<int>, globalState: GlobalState) returns (r0: array<seq<int>>, r1: int, r2: int) {
		...
		if ((v1 + v1) in (seq(0x3b0, i0  => (v1)))) {
			...
			if (false <==> (if (f21 in v2) then v2[f21] else f21)) {
				...
				var v9: array<multiset<int>> := new multiset<int>[10] [multiset{p1} * multiset(p0), v5, multiset(p0), v5, v6[|v5|], v5, v5, fm12(v8, globalState), v5, v5];
				...
				v16[335] := if (p2 in v17) then v17[p2] else v15;
				var v18: multiset<array<bool>> := multiset{v14, v13[201], v13[201], v14, v13[201]};
				var v19 := 'p';
				var v20 := DC14(f21, [v19], f21, p1);
				v16[335], globalState.f1, v18 := v15, !(|(v20.cf25 + (seq(0x3d3, i3  => (v19))))| != -p2), v18;
				var v21 := "ckoc";
				v21 := v21;
			}
		...
		for i4 := p1 to |fm15(globalState)| {
			...
			match (v36.(cf27 := p1)) {
				...
				case DC14(cf24, cf25, cf26, cf27) =>
					...
					r2 := cf27;
					var v42 := DC3(v32);
					var v43: array<char> := new char[22] [v32, v32, v32, v32, v32, v32, v32, v32, v32, v32, 't', v32, v42.cf5, v32, v32, v32, v32, v32, cf25[p2], v32, fm16(-p1, if (i4 in v34) then v34[i4] else f21, globalState), v32];
					v43 := v43;
				case DC12(cf22) =>
					p3[926] := p2;
					globalState.f1, p3[926], globalState.f1 := fm0(globalState), -i4, f21 <==> f21;
					v36 := v36;
	...
	method m10(p0: set<bool>, globalState: GlobalState) returns (r0: string) {
		...
		match (DC12(v5)) {
			case DC13(cf23) =>
				...
				match (v6) {
					case DC13(cf23) =>
						...
						globalState.f9 := (|(if (|v10| in v8) then v8[|v10|] else v7)| / fm11(f21, v11, f21, v12, globalState)) * v13[v2];
						globalState.f1 := f21;
						var v14 := DC0(cf23);
						var v15: map<string, D0> := map[v4 := v14];
						var v16: seq<string> := [v4, "lvnhyan", v4, v4];
						v15 := v15[v16[|(seq(0x2c8, i0  => ('i')))|] := v14];
						v2 := -0x1bc;
					case DC14(cf24, cf25, cf26, cf27) =>
						...
						v19[392] := v27;
						var v28: seq<int> := [|v4|];
						globalState.f9 := (|v28[v2 := |(seq(0x256, i2  => (v20)))|]| / |v4|) / 0x29e;
					case DC12(cf22) =>
						v0[989] := -cf23;
						...
						globalState.f4, v0[989], v31, v32[899], v34 := fm11(false, v33 + v33, f21, v35, globalState), fm11(f21, v33, f21, v35, globalState), v31, v29, if (false !in p0) then v34 + v34 else v34;
						var v36: seq<int> := [v0[989]];
						var v37: array<int> := new int[26];
						var v38: array<set<array<int>>> := new set<array<int>>[8];
						v38[158] := v1 - v1;
						v32[899], v36, globalState.f1, v37, v38[158] := v29, v36 + [fm11(f21, v33, f21, DC12(v5), globalState)], f21, v37, {v37};
						var v39: set<int> := {v0[989]};
				...
				v47, v48, v49[817], v3, v53 := (map[v0 := v46])[v0 := multiset{f21}], v48, if (true) then v50 else v50, v3[|v4| - 0x15f := f21], v53;
			case DC14(cf24, cf25, cf26, cf27) =>
				v4 := v4;
				match (fm18(f21, cf26, true, globalState)) {
					case DC4() =>
						...
						v0[876] := cf27 * (if (cf27 in v55) then v55[cf27] else 0x38d);
						v0[876] := -(|[f21, cf24, true]| + cf27);
						var v56 := new C0();
						globalState.f1 := false;
					case DC5(cf6, cf7, cf8, cf9, cf10) =>
						cf10 := 'k';
					...
					case DC6(cf11) =>
						...
						v0[358] := cf27 % cf11;
						v0[358] := cf11 + 0x192;
						cf24 := !((if (cf24) then !false else f21) || (v0[358] <= cf11));
					case DC3(cf5) =>
						cf26 := f21;
						...
						v61[799] := v0;
						v61[799] := v0;
						var v62: map<int, int> := map[if (false) then |fm19(f21, globalState)| else cf27 := cf27 % v2];
						var v63: C0 := new C0();
					...
					case DC7(cf12) =>
						...
						var v70: map<int, map<string, string>> := map[|(if (true in v69) then v69[true] else map[cf26 := cf26])| % cf27 := fm21(!cf26, cf27, cf27, cf26, globalState)];
				...
				match (v76) {
					case DC1(cf1, cf2, cf3) =>
						...
						var v80 := 'l';
						var v81 := DC9(v80, cf27, v4, --cf27, v0[449]);
						var v82: seq<D2> := [v81];
						v82 := v82[cf27 % v0[449] := v81];
					...
					case DC2(cf4) =>
						...
						v0[95] := if (-0x1f7 in v91) then v91[-0x1f7] else v2;
						var v92: map<bool, map<T1, multiset<bool>>> := map[f21 := v90];
						v72[156], globalState.f1, v90, v0[95] := !((if (v88.fm6(v4, cf27, -0x249, 'i', globalState)) then cf24 else !f21) || f21), fm0(globalState), (if (cf24 in v92) then v92[cf24] else v90[v88 := v89]) + map[v88 := multiset{v88.f17}], v2 + v2;
						var v93: array<int> := new int[18];
						var v94: map<int, array<int>> := map[cf27 := v93];
						v94 := v94[-0x109 + v2 := v93];
						var v95: seq<int> := [v2];
						var v96: seq<seq<int>> := [(seq(869, i8  => (-|cf25|))) + v95];
						v96 := [v95 + v95];
			...
			case DC15(cf28) =>
				...
				globalState.f1, globalState.f1, globalState.f9 := v103 != (v104[v2] + map[v102 := f21]), f21 ==> (if (f21) then f21 else v105.cf50), v2;
				var v106: seq<int> := [v2];
		...
		var v114: map<string, int> := map["j" := v113[v2]];
		var v115 := DC52(v114);
		match (v115) {
			case DC53(cf89, cf90) =>
				v3 := v3;
				v2 := v2;
				v0[355] := v2;
				globalState.f1, globalState.f1, v0[355], v108 := f21, !f21, cf90, fm53(cf89, f21, f21, cf89, globalState);
				var v116: array<bool> := new bool[7];
				var v117: seq<array<bool>> := [v116, v116, v116];
				var v118: set<array<bool>> := {v116, v117[cf90]};
				v118 := v118 + v118;
			case DC54(cf91) =>
				var v119 := new C3(f21);
			...
			case DC55(cf92) =>
				...
				v125[609] := v126 == v126;
				v125[609] := fm0(globalState);
				v125 := v125;
				var v127: T2 := new C2(v109.f22, v109.f22, v125[609], v3);
				v127 := v127;
				globalState.f1 := f21;
			case DC52(cf88) =>
				r0 := v4;
				globalState.f1 := f21;
				var v130: multiset<map<int, int>> := multiset{fm46(f21, f21, v2, f21, globalState), map v128 : int | (0xb3 <= v128) && (v128 < 573) :: (v128 % |(map v129 : int | v129 in map[v2 := v4] :: (v129 + v2) := (v2))|) := (v2)};
				var v131 := DC24(v130);
				var v132: map<D8, bool> := map[v131 := f21];
				v132 := (v132 + v132) + v132;
				v0[686] := v2;
				var v133: map<int, int> := map[v2 := v2];
				globalState.f9, globalState.f9, globalState.f9, v0[686] := -((v2 - v2) + 82), v2 * v2, -0x3d6, (v2 * 770) * (if (v2 in v133) then v133[v2] else -v2);
			case DC56(cf93) =>
				var v134 := DC39(f21, |v4|);
				v0[456] := |multiset{v134, DC39(f21, v2)}| + |p0|;
				var v135 := DC58(p0, |fm43(|v113|, f21, 0x1a0, globalState)|, v2 - -v2);
				v0[456], v2, globalState.f9, v135 := v2 % v2, v2, -v2, fm69(if (f21) then v111 else v111, v4, f21, globalState);
				var v136: array<bool> := new bool[14](i10 => f21);
				v136[902] := f21;
				v136[902] := f21;
				v5 := v5[fm50(globalState) / v0[456] := f21];
				globalState.f1 := fm0(globalState);
		}
...
class C6 extends T2 {
	...
	method m5(p0: bool, p1: int, p2: map<bool, D3>, p3: char, globalState: GlobalState) returns (r0: int) {
		...
		v2[502] := p1;
		v1, v2[502], globalState.f1 := (seq(0x192, i0  => (v0))) + v1, 882 - |v1|, p0 !in f18;
		var v3 := DC4();
		match (v3) {
			case DC4() =>
				...
				if (v7.f21) {
					...
					v10 := [v9[v8.(cf67 := v12) := p1], v9 - v9[v8 := v2[502]], v9, v9[v8 := p1], multiset{v8}];
					...
					var v18: map<int, bool> := map[v17[p1] := v7.f21];
					var v19: map<int, int> := map[v2[502] := p1];
					globalState.f1, globalState.f4, globalState.f1, globalState.f9, globalState.f1 := -p1 > 0xed, if (p0) then p1 else p1, if (v2[502] in v18) then v18[v2[502]] else v7.f21, v2[502] - -0x14a, |(v19 + v19)| > v16.fm7(!p0, v18[v2[502] := v7.f21], globalState);
					var v20 := new C3(p0);
					v2[502] := v2[502];
			...
			case DC5(cf6, cf7, cf8, cf9, cf10) =>
				...
				if ((v21 - v21) >= multiset([p0, p0] + f18)) {
					var v22 := DC3(cf10);
					v22 := v22.(cf5 := cf10);
					globalState.f4 := fm50(globalState) / fm50(globalState);
					v2 := v2;
					globalState.f1 := cf8;
					...
					var v27: map<int, bool> := map[v2[502] := cf8];
					cf9 := -(if (v26 > v26) then cf9 / v2[502] else |v27|);
				} else {
					var v28: map<int, int> := map[cf9 := cf9];
					cf8 := fm50(globalState) > (if (v2[502] in v28) then v28[v2[502]] else cf9);
					cf8 := fm0(globalState);
				...
				var v32 := DC39(cf8, v2[502]);
				v32 := fm70(v2[502], f18[v2[502]], globalState);
				var v33: map<int, int> := map[0x1cd := v2[502] % cf9];
			...
			case DC6(cf11) =>
				...
				v38[191] := if (map[p0 := p1] in v40) then v40[map[p0 := p1]] else p0;
				v38[191] := p0;
				v2[502] := fm50(globalState);
				globalState.f1 := p0;
				globalState.f1 := p0;
			case DC3(cf5) =>
				var v41: set<bool> := {p0, p0};
				globalState.f1 := !((|"ge"| / p1) in fm64(-p1, |v41|, globalState));
				cf5 := v0;
				var v42: multiset<bool> := multiset{p0, p0};
				if ((if (p0) then v42 else v42[p0 := v2[502]]) > v42) {
					globalState.f1 := p0;
					globalState.f1 := true;
					cf5 := fm53(0x2ab, false, p0, v2[502], globalState);
					v2 := v2;
				...
				match (DC32(p1, v2[502])) {
					case DC32(cf53, cf54) =>
						globalState.f1 := p0;
						var v44: map<map<int, multiset<bool>>, int> := map[map[cf53 := v42] := 668];
						v2[502] := if (map[-cf54 := v42] in v44) then v44[map[-cf54 := v42]] else p1;
						v2 := v2;
						var v45 := new C0();
					case DC33(cf55, cf56, cf57) =>
						...
						v46 := v46[p1 := p0];
						var v47 := DC9(v0, |v1|, seq(-0x3a8, i4  => (p3)), cf57, p1);
						cf5 := v47.cf14;
					...
					case DC34(cf58, cf59, cf60) =>
						...
						v50 := v50[[cf58, cf58] != f18 := v2[502]];
						var v51: map<string, int> := map["c" := v2[502]];
						var v52 := DC52(v51);
						v52 := if (v41 <= {!cf59}) then if (cf59) then v52 else v52 else DC52(v51);
						var v53: seq<int> := [-cf60];
						var v54: map<int, map<int, seq<int>>> := map[v2[502] := map[0x2a := v53]];
					...
					case DC35(cf61) =>
						...
						var v66: seq<int> := [|v65|];
						v64[428] := multiset(v66 + v66);
						var v67: multiset<int> := multiset{0x22e};
						var v68 := DC68(v67);
						v64[428] := v68.cf110;
						var v69: seq<seq<int>> := [v66, v66];
						var v70: multiset<seq<int>> := multiset{v66, v69[p1]};
	...
	method m6(globalState: GlobalState) returns (r0: string, r1: bool) {
		...
		if (v2) {
			...
			var v9: multiset<bool> := multiset{!f18[-v7]};
			...
			if (v0[458]) {
				...
				var v13: map<int, int> := map[v10 := |v12.cf16[v7 := v1]|];
				v5[608] := if (v10 in v13) then v13[v10] else -0xae;
				var v14: array<D7> := new D7[11];
				var v15: set<bool> := {v0[458]};
				var v16 := DC22([|v15|, |v11|, v7]);
				v14[552] := v16;
				v5[608], globalState.f1, v14[552], v0[458], v1 := v7, v9 > (if (v0[458]) then v9 else v9), v16, v2, v1;
				v1 := v1;
				var v17: array<int> := new int[1](i0 => i0 * |v6|);
				var v18 := DC45(v0[458], v17);
			...
			var v27: seq<multiset<bool>> := [multiset(if (v0[458]) then fm59(false, globalState) else [!v2]), v26, v26 * fm56(globalState)];
			globalState.f4, v24, v25 := |v27|, if (v0[458]) then v24 else v24, v25;
			var v28: map<int, bool> := map[0x3d8 := v2];
			var v29 := 0x5;
			v5[56] := v25.fm7(v0[458], v28[v29 := v2], globalState);
			v5[56] := fm50(globalState) - v29;
			v2 := v2;
			var v30: multiset<string> := multiset{"lqnmvvgv"};
			var v31: map<int, multiset<string>> := map[-v29 := v30];
			v31 := v31[if (v2) then v5[56] else v5[56] := v30 * v30];
			var v32: map<char, int> := map[v25.f24 := v29];
			var v33: seq<map<char, int>> := [v32];
			var v34: seq<int> := [v5[56]];
			var v35: map<bool, int> := map[v0[458] := |v34|];
			globalState.f9 := |v33[-|v35|]|;
		}
		
		v0[458] := v0[458];
		var v36: map<bool, bool> := map[true := v0[458]];
		var v37 := 0x30a;
		var v38 := "edhhu";
		var v39 := DC51(v36, v37, v1, map[map[v0[458] := v38[v37]] := v38], v37);
		...
		if (match v41 {
			...
			match (v42.(cf23 := v37)) {
				case DC13(cf23) =>
					var v43: seq<int> := [v37];
					var v44 := DC22(v43);
					var v45: multiset<D7> := multiset{if (v0[458]) then v44 else v44};
					var v46: set<bool> := {fm0(globalState), v0[458], fm0(globalState), v2, v2};
					v45, globalState.f4 := v45, v37 * (|v46| % cf23);
					var v47 := new C1();
					v0[458] := v37 < cf23;
					globalState.f9 := v37;
				case DC14(cf24, cf25, cf26, cf27) =>
					...
					var v49 := DC25(v48);
					var v50: map<bool, map<array<int>, int>> := map[v0[458] := v48];
					...
					v54 := v54;
					var v55: map<bool, int> := map[v0[458] := v37];
				...
				case DC12(cf22) =>
					...
					var v60: multiset<bool> := multiset{v2, v0[458], v2};
					var v61: seq<multiset<bool>> := [v60];
					globalState.f4, v0[458], v38, globalState.f4, v59 := v37, v60[v0[458] := v37] != v61[|v38|], (v38 + v38[-v37 := v1]) + v38, v37, v59;
					var v62: set<seq<int>> := {v59};
					var v63: seq<set<seq<int>>> := [v62];
					var v64: map<bool, int> := map[v0[458] <== false := v37 % |v63[v37]|];
					v64 := v64;
					r1 := v3.f18[v37];
					v5[614] := -v37;
					v5[614] := v37;
				case DC15(cf28) =>
					...
					v67 := v67;
					v5[355] := -0x1ec;
					v5[355] := |f18[0xd4 := v0[458]]| / v37;
			}
			
			var v68 := new C1();
			var v69: seq<string> := [fm43(v37, v2, v37, globalState)];
			var v70: set<bool> := {false};
			v0[458] := (v38 + fm58(v69, v37, |v70|, v37, globalState)) <= ("esoxuann")[v37 := v1];
			var v71 := DC14(v0[458], v38, v2, v37);
			v2 := v71.cf24;
		} else {
			var v72: multiset<string> := multiset{v38, v38};
			v5[878] := (if (v38 in v72) then v72[v38] else v37) / 0x1a4;
			v5[878] := v37;
			var v73: array<int> := new int[1](i2 => i2 - v37);
			v73 := v73;
			v0[458] := v2;
			globalState.f9 := ---((fm72(v37, v37, globalState)).cf51 / v37);
			var v74: map<int, bool> := map[fm50(globalState) * fm50(globalState) := v2];
			v0[458], v74, v1 := !v0[458], v74[-0x40 := v2], v1;
		}
		...
		match (v76) {
			case DC64(cf105, cf106) =>
				if (if (v0[458]) then false else v0[458]) {
					...
					globalState.f4, v85 := |(if (true <==> cf105) then v86 + v86[v37 := |v87|] else seq(772, i3  => (v37)))[if (v37 in v80) then v80[v37] else v37 := v37]|, v85;
					v2 := cf106;
				}
				
				var v88: map<int, char> := map[fm50(globalState) := v1];
				if (v37 >= |v88|) {
					var v89: multiset<bool> := multiset{fm0(globalState), v37 <= v37, v0[458], v37 >= |v38|, v37 in fm73(v38[v37], globalState)};
					globalState.f4 := if ((v37 < |(map v90 : int | (419 <= v90) && (v90 < 0x1ae) :: (v90 % |v38|) := (v37))|) in v89) then v89[v37 < |(map v90 : int | (419 <= v90) && (v90 < 0x1ae) :: (v90 % |v38|) := (v37))|] else v37 + |v75|;
					globalState.f9 := v37;
					var v91: set<int> := {v37 - |v89|};
					v91 := fm37(!v0[458], v0[458], globalState);
					var v92: set<bool> := {v2};
					globalState.f1 := ({cf106, cf106} + v92) != (v92 * v92);
					var v93: seq<int> := [v37];
					r1 := -v93[if (v37 in v40) then v40[v37] else v37] != v39.cf84;
				} else {
					var v94: C1 := new C1();
					var v95: set<int> := {v37};
					var v96: map<C1, bool> := map[v94 := v3.f18[|v95|]];
					var v97: map<int, bool> := map[|v96| % v37 := true];
					v97 := v97[v37 + v37 := v95 >= v95];
					globalState.f9 := -|v38|;
					var v98: array<seq<bool>> := new seq<bool>[10](i4 => [cf105]);
					var v99 := DC42(v3.f18, v0, cf105, v98);
					var v100: multiset<bool> := multiset{cf105, v0[458], v99.cf71};
					var v101: seq<int> := [|v38| - v37, v37, v37 - -208, fm50(globalState)];
					globalState.f1, globalState.f9, globalState.f4, v0[458] := if ("eiejogx" != "uhyvgwdk") then v37 >= v37 else v0[458], if (!!v0[458] in v100) then v100[!!v0[458]] else -v37, v101[v37], !(v95 <= (if (v0[458]) then {v37} else v95));
					globalState.f1, r0 := cf105, seq(0x10d, i5  => (v1));
					globalState.f9 := 607 * v37;
				}
				
				globalState.f9 := (0x19 / v37) - v37;
				var v102 := DC0(v37);
				...
				if (|(map[v37 := |[v1, v104.cf10, v1]|] + v40)| < v37) {
					...
					var v107: array<seq<char>> := new string[25] [v38, [v1, v1] + v38, v38, v38, v38, v38, v38, [v1], v38, seq(0x24d, i7  => (v1)), (seq(0x269, i8  => (v1))) + v38, v38, v38 + v38, v38, v38, v38, v38, v38[v37 := v1] + v38, v38, v38, v106[fm50(globalState)], v38, seq(-0x240, i9  => (v1)), [v1, 'a'] + v38, v38];
					v107[939] := v38;
					v107[939] := fm58(v106, v37, v37, v37, globalState);
					v38 := v107[939];
					var v108: C1 := new C1();
					...
					var v113: map<string, bool> := map[v38 := v0[458]];
					r1, cf105, cf105, globalState.f1, v37 := !!(v0[458] <==> cf106), v37 > (|v112[v37 := v37]| + |f18|), (if (!v0[458]) then v38 else v107[939]) == (v107[939] + v107[939]), v3.f18[if (-v37 in v40) then v40[-v37] else -v37], |v112| % |v113|;
				} else {
					...
					v115 := v116[v37 * v37];
					var v117: map<bool, array<bool>> := map[if (!v0[458]) then cf105 else cf106 := v0];
					v117, v0 := map[v0[458] := v0], v0;
					v114 := v114[v37 := v0[458]];
					cf106 := !(v2 ==> v2);
					var v118 := DC0(v37);
					var v119 := DC2(v118);
					var v120 := DC2(v119);
			...
			case DC63(cf104) =>
				var v122: seq<string> := [(fm19(v0[458], globalState))[v37 := v1]];
				var v123: map<string, map<int, int>> := map[v122[|v38|] := fm27(globalState)];
				v123 := v123[v38 := v40];
				var v124: multiset<bool> := multiset{true};
				var v125 := m8(multiset(v3.f18) !! v124, v124, v37 / -v37, v37, globalState);
				v38 := (fm26(globalState) + v38) + "iq";
				var v126: seq<map<bool, bool>> := [v36];
				var v127: set<map<bool, bool>> := {v126[v125], v36, v36};
				var v128: map<char, bool> := map['k' := v3.f18[v125]];
				var v129: seq<map<char, bool>> := [v128, map[v1 := v0[458]]];
				var v130 := DC70(v129);
			...
			case DC65(cf107) =>
				...
				v5[852] := v37 - v37;
				v5[408] := -v37;
				var v135: multiset<bool> := multiset{v0[458], v0[458], false};
				var v136: seq<multiset<bool>> := [v135, v135, v135];
				v5[852], globalState.f1, v5[408], v136 := -v37, v2, v37, v136;
				var v137: array<D5> := new D5[9];
				v137[40] := v133;
				v137[40] := if (v2) then v133 else v133;
		}
		
		var v138: map<array<bool>, bool> := map[v0 := v0[458]];
		r1 := |v138| == (if (v2) then v37 else v37);
		r0 := (if (false) then seq(-279, i10  => (v1)) else v38)[v37 * v37 := v1];
		r1 := v2;
	}
	method m7(p0: array<int>, p1: bool, p2: array<int>, p3: bool, globalState: GlobalState) {
			...
			decreases 100 - i0
		{
			...
			var v5 := DC13(v3);
			v5, globalState.f9, globalState.f4, globalState.f1 := if (p3) then v5 else v5, v4, if (p1 in v2) then v2[p1] else 0x17c % v4, p1 <==> p3;
		}
		...
		for i1 := 0x2b6 to if (v6 in v8) then v8[v6] else v7 {
			if (if (DC23(p3).cf39) then p3 else true) {
				...
				v22[762] := p3;
				var v23 := "uoolsvjcy";
				globalState.f1 := (v23 + v23) != ("m")[i1 := v17];
			} else {
				globalState.f1 := p1;
			...
			p0[851] := i1;
			p0[851], v7, globalState.f4, globalState.f1 := i1, i1, (i1 + v7) + 255, fm0(globalState);
			...
			var v36: array<int> := new int[22] [v7, i1, |v28| / i1, p0[851] % v7, |{v7}|, |(map[p3 := v28] + v32)|, if (p3) then fm50(globalState) else v7, i1, i1, i1, |v28|, -i1 % |v33|, v7, fm50(globalState), |(v8 + v8)|, |f18| - 0x3a6, i1, -i1, v31.fm11(p3, map[p3 := p1], p1, DC12(map[i1 := p1]), globalState), |v34|, v7, if (p1 in v35) then v35[p1] else i1];
			v36, globalState.f1 := v36, p3;
		}
		var v37: map<bool, int> := map[p3 := v7];
		var v38: map<map<bool, int>, int> := map[v37 := v7 * v7];
		v38 := v38[v37 + v37 := v7 % |[p3]|];
	...
	method m8(p0: bool, p1: multiset<bool>, p2: int, p3: int, globalState: GlobalState) returns (r0: int) {
		...
		if (match v3 {
			...
			if (!false) {
				...
				v0[137] := v15 in (v17 + v17);
				var v18: map<char, bool> := map[v10 := f18[p3]];
				...
				v20, v26 := fm75(v0[137], p3, p3, p3, globalState) + v27[p2], DC72(0x200, p3 - p2, v22, (map[v5.f17 := v23])[v5.f17 := v23], [v10] + v4);
			} else {
				var v28 := "rmmsy";
				v28 := v28;
				var v29: array<string> := new string[28];
				var v30 := 'w';
				v29[815] := ("ruqbejjv")[p2 := v30];
				v29[815] := v28;
				...
				v34 := v34[|v29[815]| + p3 := v1 + v1];
			}
			
			v0[69] := p0;
			var v35 := "hkicvxf";
			var v36: set<string> := {v35};
			v0[69], globalState.f1 := false, v36 >= {v35, v35};
			var v37: map<bool, int> := map[true := p2];
			var v38: map<int, int> := map[(if (!v0[69] in v37) then v37[!v0[69]] else p2) - |f18| := p3 % |v35|];
			v38 := v38[0x14b := p2];
			...
			var v43 := 'i';
			var v44: map<bool, char> := map[v0[69] := v43];
			v40[120] := fm76(p3, |v42[p2]|, globalState) + v44;
			var v45: seq<map<bool, char>> := [v44];
			var v46: array<int> := new int[2] [p3, p2];
			var v47 := DC45(p0, v46);
			var v48: map<bool, bool> := map[v0[69] := v47.cf75];
			v39, v40[120], globalState.f9, v43 := v39, v45[if (p0) then if (v0[69] in p1) then p1[v0[69]] else p3 else |v48|], p3, v43;
			v46[767] := p2;
			v46[767] := (p2 + p2) % 295;
		} else {
			var v49 := "piiraifk";
			...
			var v51 := DC38(v50, p3);
			var v52 := 'm';
			globalState.f9, globalState.f1, globalState.f9, v49, globalState.f9 := v51.cf64, p0, p2, "rovyyjii" + (if (p0) then seq(176, i5  => ('o')) else v49[v1[p3] := v52]), fm50(globalState);
			var v53 := DC23(p0);
			match (v53) {
				case DC23(cf39) =>
					...
					v57[985] := p3;
					v57[985] := |v2|;
					var v59: map<int, bool> := map[-v57[985] := !cf39];
					globalState.f9 := if (cf39) then |(if (cf39) then (map[p3 := cf39])[v57[985] := cf39] else v59)| else v57[985] - v57[985];
					var v60: multiset<bool> := multiset{p0};
					v60 := v60[v49 != "co" := v57[985]];
				case DC22(cf38) =>
					var v61: map<seq<int>, bool> := map[[-|v49|] := p0];
					var v62: array<char> := new char[10];
					v62[255] := if (fm0(globalState)) then 'x' else v52;
					var v63: seq<string> := [v49, v49];
					v61, v62[255], v63 := map[cf38 := p0], v52, fm28(globalState);
		...
		if (v0 in (v73 - v73)) {
			v0[288] := !p0;
			v0[288] := (fm50(globalState) + p2) >= p3;
			globalState.f1 := -p3 != p2;
			var v74: array<int> := new int[18](i6 => i6 - |(seq(484, i7  => ('t')))|);
			var v75: map<seq<bool>, int> := map[[v0[288], v0[288], p0] := p3];
			v74[879] := |v75|;
			v74[879] := p3;
			m0(v0[288], p0, globalState);
			var v76 := "euny";
			var v77 := 'a';
			v76 := (if (p0) then v76 else v76) + (fm19(p0, globalState))[p3 := v77];
		} else {
			var v78: array<string> := new string[5];
			var v79: map<array<string>, array<bool>> := map[v78 := v0];
			v79 := v79[v78 := v0];
			globalState.f1 := !(p3 > (p3 / v1[p2]));
...
class C7 extends T1, T2 {
	...
	method m3(p0: bool, p1: int, p2: char, globalState: GlobalState) returns (r0: D1, r1: int, r2: bool) {
			...
			decreases 100 - i0
		{
			...
			var v10: seq<map<int, int>> := [map v4 : int | v4 in map[p1 := p0] :: (v4 / p1) := (f19), map v5 : int | v5 in v6 :: (v5 - f19) := (p1), v9[f19], v8, v8];
			var v11: set<bool> := {p0};
			var v12: array<bool> := new bool[3] [p0, f17, p0];
			var v13: array<char> := new char[24](i1 => p2);
			var v14: seq<D0> := [DC1(v11, v12, v13)];
			v1[698], r2, v3 := (f19 % |v10|) * |v14|, !!p0, v3;
			var v15 := 's';
			f20[303] := v0;
			v12[184] := f17;
			v1[698], v15, f20[303], v12[184] := fm7(p0, map v16 : int | (0x170 <= v16) && (v16 < 0x356) :: (v16 + v1[698]) := (false), globalState), v15, v0, p0;
			var v17 := DC0(p1);
			var v18: array<map<bool, bool>> := new map<bool, bool>[24](i2 => map[p0 := p0]);
			var v19 := DC5(v17, v18, !f17, v1[698], v15);
			var v20 := DC5(v17, v19.cf7, f17, p1, 'd');
			var v21 := DC5(v17, v18, v12[184], v20.cf9, v15);
			v21 := v20;
			var v22 := DC12(v7);
			var v23: set<int> := {p1};
			f19 := (f19 - fm7(false, v22.cf22, globalState)) * -(|v23| % f19);
		}
		var v24 := new C0();
		...
		var v37: set<int> := {|v36|};
		v26, v26, v29, globalState.f4, r2 := (if (p1 in v30) then v30[p1] else (seq(897, i5  => ('o')))[f19 := p2]) + v26, fm58(v31[f19 := v26], f19 - f19, v32.cf66, p1, globalState), v29, v24.fm13(globalState), fm10(if (p0) then map v33 : int | v33 in v34 :: (v33 % 608) := (f17) else v35, f19, v37, p2, globalState);
		v25 := v25;
		var v38: array<D19> := new D19[24];
		var v39 := DC60(!f17);
		v38[38] := if (f17) then v39 else fm77(0x264, p0, globalState);
		v38[38] := v39;
		var v40 := DC3(p2);
		var v41 := DC7(v40);
		r0 := v41;
		var v42: map<bool, bool> := map[p0 := f17];
		r1 := (|v34| + |v42[p0 := false]|) / f19;
		r2 := f17;
	}
	method m4(p0: seq<char>, p1: set<bool>, p2: int, p3: array<int>, globalState: GlobalState) {
		...
		forall i1 | 0 <= i1 < v9.Length {
			v9[i1] := if (f18[|multiset(f18)|]) then p0[f19 := 'k'] else (if (true) then seq(0x372, i2  => ('t')) else p0)[|multiset{0x144}| := 'd'];
		}
		...
		if (!f17) {
			...
			if (f17) {
				...
				v12 := v12;
				v12[795] := f19;
				v12[795] := |map[v10[119] := p2]| * |p0|;
			} else {
				var v13: map<int, bool> := map[|f18| := f17];
				var v14 := DC12(v13);
				var v15: multiset<D4> := multiset{v14};
				globalState.f4 := -(p2 - f19) % (if (v14 in v15) then v15[v14] else f19);
				var v16: array<T2> := new T2[18];
				globalState.f2, v16 := f18 + f18, v16;
				var v17: map<int, set<bool>> := map[f19 := p1];
				v17 := v17[f19 % -0x2a6 := p1];
				var v18: seq<string> := [p0];
				var v19: multiset<bool> := multiset{f17, f17};
				f20[404] := fm58(v18, p2, if (f17 in v19) then v19[f17] else f19, -0x86, globalState);
				var v20: seq<array<bool>> := [v10];
				f20[404], globalState.f1, globalState.f1 := p0 + (p0 + fm26(globalState)), (v20 + v20[f19 := v10]) <= v20, f17;
				globalState.f1 := f17;
			}
			...
			p3[366], p3[454], globalState.f4, globalState.f1, v25 := f19, p2 * p2, p2 + (|p1| / p2), f17, v25;
			globalState.f4 := p3[366] - (f19 / f19);
			var v26: map<int, set<int>> := map[f19 := fm37(f17, f17, globalState)];
			var v28: multiset<bool> := multiset{false, f17};
			var v29: multiset<multiset<bool>> := multiset{v28, v28[f17 := p2]};
			var v31: set<multiset<bool>> := {v28};
			var v32: set<bool> := {!({p2, p3[366]} >= (if (f19 in v26) then v26[f19] else set v27 : int | v27 in v24 :: (v27 % 0xe))), (set v30 : multiset<bool> | v30 in v29 :: (v30)) !! v31, f17};
			v32 := p1 + p1;
		} else {
			f19 := p2;
			var v33 := 'y';
			...
			if (fm59(f17, globalState) != v34.cf125) {
				...
				var v40 := DC72(p2, DC59(fm35(v37, p2, globalState), p2).cf99, v10, v39, seq(-0x31c, i4  => (v36.f24)));
				v9[926] := v40.cf121;
				var v41: map<int, bool> := map[p2 := f17];
				var v42 := DC9(v36.f23, |v41|, p0, p2, -f19);
				v9[926] := v42.cf16;
				f19 := if (f17) then f19 / f19 else f19;
			} else {
				...
				v49[975] := v50;
				globalState.f1, v49[975], globalState.f4 := v50.f21, v50, 0x2a7 * f19;
			}
			
			var v51: seq<string> := [p0];
			globalState.f9 := f19 + |(if (fm6(v51[f19], p2, p2, v33, globalState)) then p0 else p0)[p2 := v33]|;
			p3[499] := |p1|;
			globalState.f1, p3[499], globalState.f1 := f17, p2 % f19, f17;
		...
		if (!f17) {
			...
			v54[976] := v55;
			v54[976] := if (f17) then v55 else v55;
			...
			var v66 := 'o';
			var v67 := DC72(|(seq(748, i6  => ('b')))|, p2, v10, v65, [v66, v66, 'o']);
			match (DC72(f19, fm7(f17, v63, globalState), v10, v67.cf120, seq(0xd1, i7  => (v66)))) {
				case DC71(cf113, cf114, cf115, cf116) =>
					globalState.f1 := f17;
					var v68 := DC3(v66);
					var v69 := DC7(v68);
					...
					var v75: C2 := new C2('t', v73.f22, f17, v74[0x3d]);
				...
				case DC72(cf117, cf118, cf119, cf120, cf121) =>
					...
					var v80: map<D20, bool> := map[DC61(v79[|(seq(0x1d2, i8  => (p2)))|]) := f17];
					var v82 := DC61(v64);
					var v83: seq<D20> := [v82];
					...
					var v91: array<array<int>> := new array<int>[8] [p3, p3, v90[cf117], p3, p3, p3, p3, p3];
					v91[109] := p3;
					v91[109] := p3;
					cf121 := cf121 + p0;
				case DC70(cf112) =>
					...
					v92, p3[360], v94 := p0, fm50(globalState), v94[p2 := p3];
					globalState.f9 := p3[360] + (-p2 / p2);
					...
					v95 := v97[|(if (f17 in v98) then v98[f17] else f18)|];
					v61[203] := f18;
					v61[203] := f18;
			}
			
			v10[688] := f19 <= f19;
			v10[688], globalState.f1 := fm6(fm19(!f17, globalState), p2, f19, v66, globalState), |p0| == |v62|;
			var v99: array<char> := new char[5](i9 => v66);
			v99[800] := v66;
			var v100: map<set<int>, bool> := map[{-f19} * v62 := true];
			var v101: multiset<int> := multiset{|v62|};
			v99[800], v100, globalState.f4, v10[688] := v66, v100[{if (f19 in v101) then v101[f19] else f19, -p2} - v62 := f17], f19 / p2, f17;
			var v102: multiset<bool> := multiset{f17, v10[688], v10[688], fm10(v63, p2, v62, v99[800], globalState), f18[p2]};
			globalState.f1 := -((if (true in v102) then v102[true] else f19) + |f18[p2 := v10[688]]|) <= |(v53 + v53)|;
		}
	...
	method m5(p0: bool, p1: int, p2: map<bool, D3>, p3: char, globalState: GlobalState) returns (r0: int) {
		...
		for i0 := -670 to |v1| {
			var v2: array<int> := new int[24](i1 => i1 % --0x5f);
			v2[342] := i0;
			v2[342] := (f19 / f19) / i0;
			...
			v11[630] := false;
			v11[630] := p0;
			var v12: map<int, string> := map[v2[342] := seq(-963, i3  => (p3))];
			var v13: map<seq<int>, int> := map[seq(0x367, i2  => (p1)) := v2[342] % -|v12|];
			var v14: seq<seq<bool>> := [f18];
			r0, v13, v11[630], globalState.f1, v4 := v2[342] % 0x18f, if (f17) then v13 else v13, f18 <= ([false] + [p0]), true && p0, multiset(([f17] + v14[898]) + [p0, f17]);
			v5[255] := p3;
			v5[255], v11[630], v11[630] := v0[i0 / v2[342]], !p0, if (f17) then f17 else p0;
		}
		for i4 := if (p0) then f19 else f19 to p1 {
			...
			v18[927] := if (f19 in v22) then v22[f19] else v20;
			var v23 := DC0(-i4);
			...
			v15[264], v16[94], v0, v18[927] := v24.cf10, v17, v28.cf121, v20;
		...
		var v43: map<set<int>, seq<bool>> := map[v41 := v42[p1]];
		var i7 := 0;
		while (|v43| == 110)
			decreases 100 - i7
		{
			...
			decreases 100 - i8
		{
			...
			i8 := i8 + 1;
			v0 := (v0 + (v0 + v0))[p1 := p3];
			var v46: set<bool> := {false, v29 == v29[fm50(globalState) := f19], p0};
			v46 := if (f17) then v46 else v46;
			var v47: seq<string> := ["f"];
			globalState.f4 := |(v47[967] + "vsfvvkwo")|;
	...
	method m6(globalState: GlobalState) returns (r0: string, r1: bool) {
		...
		var v2: map<seq<bool>, int> := map[v1[144] := 0x99];
		var v3: seq<int> := [|v2|];
		v0[982] := if ((seq(46, i0  => (f19))) <= v3) then f17 else !!f17;
		var v4: map<int, int> := map[f19 := f19];
		var v5: map<map<int, int>, int> := map[v4 := f19];
		var v6: set<int> := {f19};
		var v7: map<bool, bool> := map[f17 := true];
		var v8: map<int, int> := map[|v5| / |v6| := -|v7|];
		var v9 := "dgclxuk";
		v0[982], v8, r0, v0[982], v0[982] := true, (v4 + v4) + v4, v9, f17, v6 < v6;
		if (f19 <= -f19) {
			globalState.f9 := f19;
			...
			var v16: set<bool> := {v0[982], fm6(v9, f19, f19, v14, globalState)};
			var v17: map<set<bool>, char> := map[fm23(v14, fm7(v0[982], v15, globalState), globalState) * v16 := v14];
			v17 := v17;
			v14 := v14;
			var v18: seq<string> := [v9];
			var v19: seq<string> := [fm58(v18, 602, f19, -0x19e, globalState) + "cx"];
			r0, r1, globalState.f9 := v19[fm7(true, map[f19 := f17], globalState)], false, f19 + |(v7 + v7)|;
			...
			if (!(f18[f19] <==> (|v20| < f19))) {
				...
				var v28: array<int> := new int[28](i1 => i1 % f19);
				var v29: map<array<int>, bool> := map[v28 := f17];
				globalState.f4 := |((v29 + map[v28 := f17]) + map[v28 := v0[982]])|;
				var v30: map<seq<bool>, array<bool>> := map[f18 + f18 := v0];
				v0 := if (f18 in v30) then v30[f18] else v0;
				globalState.f4 := f19;
				globalState.f9 := f19;
				var v31: map<map<int, bool>, bool> := map[v15 := v0[982]];
				globalState.f1 := v31[v15 := f17] == v31;
			}
			
			var v32: array<array<int>> := new array<int>[15];
			var v33: array<int> := new int[8](i2 => i2 / |v9|);
			v32[655] := v33;
			v3, r1, globalState.f1, globalState.f4, v32[655] := (v3 + fm30(v0[982], f17, globalState)) + v3, f17, f17, f19 - f19, v33;
		}
...
class C8 {
	...
	method m2(p0: int, globalState: GlobalState) returns (r0: map<bool, string>) {
		...
		if ((p0 <= p0) && v0) {
			...
			v2[531] := fm0(globalState);
			var v17: multiset<array<int>> := multiset{v3};
			v0, v16, globalState.f1, v2, v2[531] := !v0, v16, v0, v2, (v17 + v17) <= v17;
			var v18: map<int, bool> := map[0x265 := p0 == p0];
			v18 := v18[p0 := v2[531]];
			var v19: seq<bool> := [true, v2[531]];
			var v20 := DC74(p0, v2[531], v19, v9);
			var v21 := new C5(if (v0) then v20.cf124 else v2[531]);
		} else {
			var v22: array<D18> := new D18[3];
			var v23: map<string, int> := map["slje" := p0];
			var v24 := DC52(v23);
			v22[907] := v24;
			v22[907] := v24.(cf88 := v23 + v23);
			...
			match (v32) {
				...
				case DC33(cf55, cf56, cf57) =>
					...
					v2[613] := if (|(seq(0x50, i1  => (v27)))| <= |v36|) then v0 else v0;
					globalState.f1 := v2[613];
					v0 := v2[613];
					var v37: set<bool> := {!v2[613]};
				...
				case DC34(cf58, cf59, cf60) =>
					...
					var v42: C7 := new C7(|v39| / cf60, v40, !(if (cf58) then fm0(globalState) else cf58), v41);
					var v43: array<seq<int>> := new seq<int>[21];
					var v44: seq<seq<bool>> := [v41 + v41];
					var v45 := DC23(v0);
					var v46 := DC23(v45.cf39);
					var v47: map<int, bool> := map[0x207 := v0];
					v2[676], v42, globalState.f9, v43, v44 := (cf58 || cf59) <==> false, v42, cf60 + v42.fm7(v46.cf39, v47, globalState), v43, [v41];
					cf58 := fm0(globalState);
					var v49: multiset<seq<bool>> := multiset{v41, v41};
					var v50: map<seq<bool>, bool> := map[v41 := false];
					var v51 := new C7(|((map v48 : seq<bool> | v48 in v49 :: (v48) := (cf58)) + v50)|, v40, v2[676], v41 + v41);
					var v52: set<int> := {|[p0]|};
					cf59 := {cf60} == (v52 + v52);
				case DC31(cf52) =>
					var v53: array<string> := new seq<char>[2] [seq(0x10b, i2  => (v27)), v29];
					v53[52] := v29;
					v53[52] := fm43(|([v0])[p0 := v0]| - 78, p0 < p0, p0, globalState);
					var v54: multiset<C3> := multiset{v26, v26, v26};
					globalState.f9 := |v54| % p0;
					globalState.f9 := p0;
					v25[896] := p0;
					v25[896] := -p0;
				case DC35(cf61) =>
					...
					v55[693] := seq(-0x4f, i3  => (v27));
					globalState.f1, v2[226], v0, v55[693] := v0, !v0, v0, v29 + v29;
					var v56: seq<bool> := [v0];
					var v57: C7 := new C7(p0, v55, v2[226], v56 + v56);
					v57, globalState.f4, v2[226] := v57, v57.f19, v2[226];
			}
...
class C9 {
	...
	method m1(p0: bool, globalState: GlobalState) {
		...
		v4 := v4;
		globalState.f4 := f15 / f16;
		globalState.f1, globalState.f1, v3, globalState.f1 := p0, p0, v3[f15 / v4[f16] := v0], (v3 + v3) <= (v3 + v3);
		...
		for i0 := |v7| to f15 {
			...
			var v15: seq<bool> := [false, v14[fm1(globalState)]];
			...
			match (v18) {
				case DC4() =>
					...
					v1[283] := |(seq(-0x3bd, i1  => (DC3(v0).cf5)))|;
					v19, v1[283], v9[627] := v19, i0, p0;
					var v20: array<D1> := new D1[25](i2 => DC6(if (v9[627] in v13) then v13[v9[627]] else |v4|));
					var v21 := DC8(v20);
					var v22: seq<array<D1>> := [v20, v20];
					var v23: array<array<D1>> := new array<D1>[7] [v20, v20, v20, v20, v21.cf13, v20, v22[f15]];
					v23[720] := v20;
					v23[720] := v20;
					v7 := v7[v1[283] := i0];
				case DC5(cf6, cf7, cf8, cf9, cf10) =>
					...
					v24, globalState.f9, cf9, v9[474] := v24, |(v25 - multiset{cf8})| / (if (cf8) then -538 else i0), fm1(globalState), cf8;
					v1[32] := 841;
					v1[32] := cf9;
					cf9 := cf9;
				case DC6(cf11) =>
					...
					v28[108] := if (p0) then v8 else {p0, p0};
					var v29 := DC14(true, [v27.f23], false, f15);
					v28[108] := {v29.cf24, !p0} - {p0, true, p0};
					globalState.f1 := p0;
				case DC3(cf5) =>
					var v30: array<seq<bool>> := new seq<bool>[15] [v15, v15, [p0], v15, v15, [false], v15, v14, v15, v15, v15, v14[f15 := !p0], v14, v15, v14];
					var v31 := DC42([p0], v9, p0, v30);
					v3, globalState.f4, globalState.f4, globalState.f9, globalState.f1 := v3, 654, f16 + (f15 / f15), |(v31.cf69 + (v15 + v15))|, p0;
					globalState.f1 := (0x3b7 !in [f16, f15]) ==> false;
					v9[277] := p0;
					v9[277] := 721 < -0x66;
					globalState.f1 := v9[277];
				case DC7(cf12) =>
					var v32: map<string, array<int>> := map[v3 := v1];
					v32 := v32[v3 + v3[-715 := 't'] := v1];
					v3 := v3;
					v9 := v9;
					v9[845] := !(v15 <= v14);
					v9[845] := false;
...
method Main() {
	...
	var v6: map<map<int, int>, map<int, bool>> := map[map v3 : int | (0x3c2 <= v3) && (v3 < 0xa2) :: (v3 % |"skaqexbx"|) := (-|"osnu"|) := v5];
	...
	globalState.f1 := v11[v4];
	if (v0) {
		globalState.f9 := v4;
		var v12 := 'v';
		var v13 := "xcrb";
		var v14: multiset<char> := multiset{v12, v12, v13[v4], v12};
		v14, globalState.f1, globalState.f2 := (multiset{v12} * v14) - v14, v0 <==> fm0(globalState), v11[|v13| := v0];
		var v15: set<bool> := {v0};
		...
		if (v0 <== v0) {
			...
			var v23 := new C3(v0);
			var v24: array<int> := new int[9](i0 => i0 % v4);
			var v25 := DC45(v0, v24);
			...
			v27[361] := v25;
			v1[733] := v0;
			globalState.f1, v27[361], v1[733] := fm2(v0, v0, globalState) !in v13, v25.(cf75 := !v0), false;
			v4, v24 := v4, v24;
		} else {
			...
			v0 := (-206 / v4) == |v31|;
			var v32 := new C0();
			var v33: map<int, int> := map[123 % v4 := |"roxuoorfm"|];
			v33 := v33[|v13| := v4];
		}
		...
		var v43 := 'g';
		var v45: seq<set<int>> := [v42, v42, set v44 : int | v44 in map[v4 := v43] :: (v44 % 0x243)];
		var v46: map<bool, int> := map[v0 := |v45[-0x132]|];
		var v47: map<map<bool, int>, map<C3, bool>> := map[v46 := v41];
		v39[521] := if (v46 in v47) then v47[v46] else v41;
	}
	...
	v1[924] := v0;
	var v51 := DC78(v50);
	v50, v1[924] := v51.cf133, v0;
	if (v1[924]) {
		globalState.f4 := v4 + v4;
		v4 := if (v0) then v4 else 0x1dd;
		var v52 := "gjee";
		var v53 := DC9('n', 0x6d, v52, v4, 0x3ab);
		var v54: map<bool, bool> := map[v1[924] := v49.fm6(v52, v4, v4, v53.cf14, globalState)];
		var v55: seq<map<bool, bool>> := [v54, v54];
		v55 := if (v0) then v55 else [map[v1[924] := v0], map[v0 := v1[924]]];
		v0 := v0;
		var v56 := DC17(v4, v4, v4);
		var v57 := DC59(v56, v4);
		globalState.f9 := (v57.cf99 - 1) / v4;
	} else {
		var v58 := new C7(v4, v2, v0 && v1[924], v11);
		if (!v1[924]) {
			v4 := v58.f19;
			globalState.f1 := v1[924];
			...
			var v63 := "vkfqdet";
			v1[924] := (|v63| >= v58.f19) ==> v0;
			var v64: map<int, int> := map[v4 := v58.f19];
			var v65: multiset<bool> := multiset{v1[924], v1[924], v0};
	...
	if (v4 < v4) {
		globalState.f2 := v11;
		if (v49.fm22([v1[924], v0, v0], 'd', globalState)) {
			globalState.f9 := v4;
			...
			var v87: seq<string> := [v86[v4]];
			v84, v85 := v86[v81[169]], v85;
			...
			v1[924], v91 := v0, v92[v4];
			var v93 := DC61(map[v1[924] := -0x3d1]);
			var v94: map<bool, int> := map[v0 := v4];
			v93 := DC61(v94);
			var v95: C1 := new C1();
			var v96 := "a";
			var v97: map<int, seq<bool>> := map[0x271 := [false, v1[924]]];
			var v98: seq<map<int, seq<bool>>> := [v97[v50.fm7(false, v5, globalState) := v11]];
			var v99: map<bool, map<int, seq<bool>>> := map[v1[924] := v98[v4]];
			var v100: seq<int> := [v4, v4, |v11|, v4, v4];
			v95, v48, v1[924], v94, v96 := v95, v49.fm8(v4, v4, if (v0 in v99) then v99[v0] else v97, globalState), !v1[924] in v11, v94[v49.fm22(v11, v49.fm8(v4, v4, v97, globalState), globalState) := v100[v4]], ("osrvo")[v4 := v49.f22] + (seq(0x195, i2  => ('k')));
			var v101 := DC63(v7);
			var v102 := DC65(v101);
			var v103 := DC65(v102);
			var v104: array<D21> := new D21[17] [v103, v103, DC65(v102), v103, DC65(v101), DC65(v102).(cf107 := DC64(v0, v1[924])), v103, v103, v103, v103, v103, v103, v103.(cf107 := v101), v103, DC65(v102), v103, DC65(v101)];
			v104 := v104;
			var v105: multiset<bool> := multiset{v0};
			var v106: set<char> := {v49.f22};
			var v107: map<int, multiset<bool>> := map[v4 := v105[v0 := |v106|]];
			var v108: set<int> := {v4, |(if (v4 in v107) then v107[v4] else v105)|};
			var v109: C5 := new C5(v108 !! v108);
			globalState.f9, globalState.f9, v109 := |(v96 + (seq(-0x14d, i3  => (v49.f22))))| + 0x18e, 122, v109;
		}
		
		var v110: C2 := new C2(v49.f22, v49.f22, v1[924], v11);
		var v111: map<int, C2> := map[v4 := v110];
		var v112: array<C2> := new C2[25] [v110, v110, v110, v110, v110, v110, v110, v110, v110, if (v1[924]) then v110 else v110, v110, if (v1[924]) then v110 else v110, v110, v110, v110, v110, v110, v110, if (v50.fm7(v0, map[v4 := v1[924]], globalState) in v111) then v111[v50.fm7(v0, map[v4 := v1[924]], globalState)] else v110, v110, v110, v110, v110, if (v0) then v110 else v110, v110];
		v112[884] := v110;
		v112[884] := if (v4 < v4) then v110 else v110;
		var v113 := DC79();
		var v114: C5 := new C5(v1[924]);
		var v115 := DC82(v114);
		...
		v121[149] := (if (v110.fm7(v1[924], map[v4 := v0], globalState) in v123) then v123[v110.fm7(v1[924], map[v4 := v0], globalState)] else v119) + v119[v4 := 0x38e];
		...
		var v126, v127, v128 := v50.m3(v1[924], v4, v49.f22, globalState);
		var v129: array<seq<int>> := new seq<int>[15];
		var v130: seq<int> := [v4, 0x354, v4];
		v129[306] := v130;
		var v131: map<bool, seq<int>> := map[v0 := (seq(0x26d, i4  => (v4))) + (seq(0x3e, i5  => (|v11|)))];
		var v132: array<int> := new int[17];
		var v133 := DC45(v0, v132);
		v129[306] := if (!v133.cf75 in v131) then v131[!v133.cf75] else v130;
		...
		v135[152] := v134;
		var v136: C7 := new C7(v130[v127], v2, (seq(0x1c6, i6  => (v49.f22))) == "ybmkvtvrc", if (v1[924]) then v11 else [v128, v0, v0, v1[924]]);
		var v137: seq<C7> := [v136, v136, v136, v136];
		v135[152], globalState.f4, v11, v136 := v134, v127 * v136.fm7(v11[fm5((multiset{seq(-0x21, i7  => (v49.f22))})["saqnagdv" := v127], globalState)], v5, globalState), v11, v137[v136.f19 / v136.f19];
	...
	var v149: map<array<C6>, bool> := map[v148 := v1[924]];
	var v150 := DC67(v147);
	v1[924], v142, v50, v143[911] := if (v148 in v149) then v149[v148] else v142.f17, if (!v1[924]) then v142 else v142, v50, v150;
	var v151 := "onuohp";
	var v152: seq<string> := [v151];
	v151 := (seq(0x39e, i8  => ('x'))) + v152[v4];
	globalState.f1 := if (v0) then v142.f17 else v1[924];
	var v153: multiset<int> := multiset{v4, -(v4 % |v5|)};
	var v154: set<char> := {v49.f22, v49.f22};
	var v155: set<int> := {v4, v4, |v154|, v4, v4};
	globalState.f4 := if (v4 in v153) then v153[v4] else |v155| / |v11[v4 := v142.f17]|;
	v1[924] := v4 < |v155|;
	var v156, v157, v158 := v50.m3(v142.f17, v4 / v4, v49.f22, globalState);
}