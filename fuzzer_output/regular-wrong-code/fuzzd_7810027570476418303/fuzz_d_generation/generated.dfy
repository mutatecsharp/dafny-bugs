...
function fm0(p0: int, p1: string, p2: int, globalState: GlobalState): seq<int> {
	(seq(788, i0  => (|"dbr"|))) + ([|"gysnkcfxa"|] + [0x1a6])
}
...
function fm11(p0: D3, globalState: GlobalState): seq<bool> {
	[if (true) then true else true, true, !(DC39(multiset{|(seq(0x30e, i0  => ('h')))|, 382, |[-0x330, |map[false := 0x35]|]|, |map[{'b', 'x'} := -|"eqi"|]|}) !in {DC39(multiset([0x2c8]))}), |map[true := |[false]|]| < 0x53]
}
function fm12(globalState: GlobalState): string {
	(if (true) then "fmhga" else seq(747, i0  => ('o'))) + ("eq" + "vx")
}
...
function fm17(globalState: GlobalState): char {
	match DC36(map v0 : int | (0x189 <= v0) && (v0 < 0x21e) :: (v0 % |map[--0x231 := -246]|) := (map v1 : int | (-991 <= v1) && (v1 < 0x38) :: (v1 - 0x1d9) := (true))) {
...
function fm20(p0: bool, p1: bool, globalState: GlobalState): set<int> {
	DC5(set v0 : int | (0x4 <= v0) && (v0 < 0xef) :: (v0 / |[771]|)).cf6 - {|map[768 := !false]|, -|[|[false, false, !true, false, true]|, |[true]|, 0x26f, 0x1c]|}
}
...
function fm22(p0: D8, p1: int, p2: seq<int>, globalState: GlobalState): D7 {
	DC17({[|[DC47([|[false]|, -0x1d], true, 468).cf90]|, |{false}|], [-|"ofew"|]} <= {[28, |(seq(241, i0  => ('s')))|, |[!false, false, !true, true]|, -|"pgy"|, DC95(true, "i", 0x261).cf174], [-0x1ce]}, |map[!true := 'n']|, true, multiset{918, |"y"|} > multiset{0x156})
}
function fm23(p0: map<bool, bool>, p1: D5, p2: int, globalState: GlobalState): seq<multiset<int>> {
	if (false) then [multiset{0x15d}, multiset{437, |"raswr"|, 0x34c, 0x201}] + [multiset{|['x', 't']|}] else [multiset([|(map v0 : int | v0 in multiset{|map[257 := 'g']|} :: (v0 - |"vewl"|) := (|map[false := |(seq(0x24b, i0  => (-24)))|]|))|]), multiset{|{|multiset{false, false, true}|, -153}|}] + (seq(-720, i1  => (multiset{|[false]|})))
}
function fm24(p0: int, p1: D6, p2: int, globalState: GlobalState): map<int, int> {
	if (---0x1b2 !in (map v0 : int | (403 <= v0) && (v0 < 181) :: (v0 - -0xc) := (|(seq(0x2f8, i0  => ('i')))|))) then (map v1 : int | (318 <= v1) && (v1 < 0xfd) :: (v1 + 0x11a) := (-0x32)) + map[-0x37a := -960] else map v2 : int | (981 <= v2) && (v2 < 583) :: (v2 / |(set v3 : int | v3 in [0x174] :: (v3 * |multiset{false, true}|))|) := (436)
}
function fm26(p0: int, globalState: GlobalState): multiset<int> {
	multiset(seq(0x22e, i0  => (|[-0x2b6]|))) * (multiset([|[!!true]|, 0x2e2, |[false]|, |[0x337]|, |[false, false]|]) + multiset(seq(-29, i1  => (-0x21e))))
}
...
function fm28(p0: int, globalState: GlobalState): string {
	match if (!!!true) then DC28(true, true) else DC28(true, true) {
		...
		case DC26(cf50) => seq(392, i0  => ('d'))
		case DC30(cf59) => "ihxbhxy" + (seq(-30, i1  => ('h')))
	}
}
...
function fm33(p0: int, globalState: GlobalState): map<map<bool, bool>, int> {
	(map v0 : map<bool, bool> | v0 in map[map[!!true := false] := seq(0x2e9, i0  => ('l'))] :: (v0) := (-0x295)) + ((map v1 : map<bool, bool> | v1 in [map[true := !false]] :: (v1) := (0x14e)) + (map v2 : map<bool, bool> | v2 in {map[true := !false]} :: (v2) := (|multiset{|{true}|}|)))
}
function fm34(p0: int, p1: int, p2: bool, globalState: GlobalState): D1 {
	if ((seq(-0x6e, i0  => (DC55()))) != [DC55()]) then DC1([|map[false := true]|, |(map v0 : seq<int> | v0 in (map v1 : seq<int> | v1 in map[[|map[true := -0x1ca]|] := 0x358] :: (v1) := (true)) :: (v0) := (-|map[714 := [DC89(|"jodwibs"|, |[|multiset{'e', 'b'}|, |map[|[|[|(seq(0xde, i1  => ('w')))|, -0x56, |[true]|, 536]|]| := |multiset{0x313}|]|, 680, 0x3a5]|, |['b']|)]]|))|]) else DC1(seq(90, i2  => (|(seq(30, i3  => ('j')))|)))
}
...
function fm37(p0: bool, globalState: GlobalState): map<int, string> {
	(map[0x4a := "gvuku"] + map[0x29c := "pmyviaxhi"]) + map[|(seq(-362, i0  => ('c')))| := "tm"]
}
function fm38(p0: bool, globalState: GlobalState): set<int> {
	(if (true) then {-|(seq(0x41, i0  => ('v')))|, 574, 95} else DC5({|map[[!!false] := true]|}).cf6) - ({|[true]|, 0x1c8, 336} * {|map[|"ipnac"| := true]|})
}
...
function fm43(globalState: GlobalState): map<int, set<D3>> {
	(map v0 : int | v0 in {|map[0x2e9 := 'u']|} :: (v0 / 0x84) := (set v2 : D3 | v2 in {DC5({878}), DC5(set v1 : int | (658 <= v1) && (v1 < 538) :: (v1 % |(seq(265, i0  => ('p')))|))} :: (v2))) + ((map v3 : int | v3 in (set v4 : int | (-894 <= v4) && (v4 < 67) :: (v4 / 0x139)) :: (v3 + |"i"|) := ({DC5({0x146, -0x36b, 295, |"qqwjm"|, 388})})) + map[0x17f := set v6 : D3 | v6 in DC122(multiset{DC5(set v5 : int | (-0x276 <= v5) && (v5 < -0x90) :: (v5 * -0xc8))}).cf214 :: (v6)])
}
...
function fm53(p0: bool, p1: int, globalState: GlobalState): seq<set<int>> {
	[{|"aankqsff"|, -|[false, true, true, true]|, |map[!false := !true]|, 0x1c4}] + (if (false) then seq(0x99, i0  => (set v0 : int | v0 in multiset([|multiset{false}|]) :: (v0 % 0x330))) else [{|multiset{|DC5({836}).cf6|}|, |multiset{|"jjfrnfyif"|, |[true, !false, false]|}|}])
}
function fm54(p0: bool, p1: int, globalState: GlobalState): D15 {
	DC40(-(|{|(set v0 : int | v0 in multiset([0xb9, 0x327]) :: (v0 / |(set v1 : int | (448 <= v1) && (v1 < 0x333) :: (v1 * |"xoycxewkj"|))|))|}| / 0x260))
}
...
function fm57(globalState: GlobalState): map<int, D0> {
	map[---(0x3d1 / |DC37([true], false, true).cf71|) := DC0('i')]
}
...
function fm61(p0: int, p1: int, globalState: GlobalState): map<int, bool> {
	map[|(seq(0x24c, i0  => ('o')))| := true] + (map v0 : int | (0x20f <= v0) && (v0 < 0x26c) :: (v0 % 0x1cf) := (!!true))
}
function fm62(globalState: GlobalState): D4 {
	DC8(0x87 - -0x1a5, |map[-0x183 := DC93(true, |"h"|, false).cf168]| - |(map v0 : map<int, bool> | v0 in (map v1 : map<int, bool> | v1 in map[map[|"aji"| := true] := true] :: (v1) := (false)) :: (v0) := (0x1dd))|, !(false !in map[!true := false]), DC119(map[-114 := 'y']).cf207, (seq(-0x36c, i0  => ('m'))) + ['c'])
}
...
function fm66(p0: int, p1: multiset<D18>, p2: int, p3: char, globalState: GlobalState): map<int, D5> {
	(map[|map[true := |[false]|]| := DC11(false, true)] + (map v0 : int | v0 in {|(set v1 : int | (0xf5 <= v1) && (v1 < -0x6f) :: (v1 / -88))|} :: (v0 - |[411]|) := (DC11(false, true)))) + map[195 := DC11(false, true)]
}
function fm69(p0: int, globalState: GlobalState): multiset<int> {
	multiset{-994, |DC123(true, false, seq(780, i0  => ('n'))).cf217|, |DC52(map[!true := |{-0x1dc}|]).cf105|, 0x44 / -0x269}
}
...
function fm71(p0: int, p1: bool, globalState: GlobalState): map<int, D0> {
	(map[|[|map[-|"pyeyxodu"| := |"wa"|]|]| := DC0('x')] + map[-0x164 := DC0('d')]) + (map v0 : int | v0 in map[|[0x14e, 0x7a]| := |(seq(0x77, i0  => ('o')))|] :: (v0 - 3) := (DC0('d')))
}
function fm72(p0: int, p1: bool, p2: D7, p3: bool, globalState: GlobalState): string {
	"cdngej" + ("lqr" + (seq(0x125, i0  => ('t'))))
}
...
function fm79(p0: int, p1: bool, p2: int, globalState: GlobalState): multiset<seq<int>> {
	multiset{seq(0x1c8, i0  => (0x17e))} - multiset{[|"sv"|]}
}
function fm80(p0: bool, globalState: GlobalState): D8 {
	DC22(0x2b7 <= |[seq(566, i0  => (|{true, false}|))]|, 0x25d + -867, -|({{'m', 'm'}} - (set v0 : set<char> | v0 in (seq(-367, i1  => ({'g', 'c'}))) :: (v0)))|)
}
function fm81(p0: int, p1: int, p2: int, globalState: GlobalState): D18 {
	match DC39(multiset([0x2ec])) {
		case DC40(cf78) => DC49(cf78, map[{false} := seq(0x350, i0  => ('y'))], |"kruepef"|, cf78, cf78)
...
function fm82(p0: int, globalState: GlobalState): map<set<bool>, string> {
	if (true <==> false) then map[{true} := "lby"] + map[{false} := "gww"] else map v0 : set<bool> | v0 in (seq(0x1f8, i0  => ({!!true}))) :: (v0) := ("obklwn")
}
...
function fm84(globalState: GlobalState): D14 {
	if (false) then DC38(-0x101, 899, 267) else DC38(0x37c, 0x35, |(seq(682, i0  => (|{false, true}|)))|)
}
...
function fm86(p0: bool, p1: set<bool>, p2: int, p3: bool, globalState: GlobalState): map<seq<int>, D7> {
	((map v0 : seq<int> | v0 in {[719, |multiset{true, true}|]} :: (v0) := (DC18(|{map[false := !true]}|))) + map[seq(71, i0  => (-|(seq(644, i1  => (|"odyncqvj"|)))|)) := DC18(--617)]) + ((map v1 : seq<int> | v1 in [[|"qxjsyj"|]] :: (v1) := (DC18(|[true]|))) + (map v2 : seq<int> | v2 in [[0x1e9, |"y"|, 0x2a8, -|multiset{true, false}|]] :: (v2) := (DC18(111))))
}
...
function fm93(p0: int, p1: int, p2: map<int, D34>, globalState: GlobalState): map<int, map<bool, int>> {
	map[0x23d / --0x1c7 := if (false) then map[true := -0x356] else map[false := -0x172]]
}
function fm94(p0: bool, p1: string, globalState: GlobalState): D18 {
	DC48(0x57 / -0x1e9, false, true)
}
...
function fm96(p0: char, p1: bool, p2: int, globalState: GlobalState): D19 {
	match DC123(true, true, "k") {
		...
		case DC124(cf218) => DC51(-0x82, |[true, false, true]|, ['g'], |[|map[true := |(seq(-0x36d, i0  => ('u')))|]|, --8, |multiset{|[false]|}|, |multiset{false}|]|)
	}
}
...
class C1 extends T0 {
	...
	method m0(p0: seq<int>, p1: int, p2: string, p3: bool, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
		...
		var v2: seq<bool> := [p3];
		v0[225] := ((if (p3 in v1) then v1[p3] else fm3(p1, globalState)) + |v2|) == (0x237 + fm3(p1, globalState));
		for i1 := p1 to p1 {
			...
			var v6 := DC15(v5);
			globalState.f6, v6, globalState.f5, v0[225], v0[225] := v3, DC15(fm29(v0[225], p1, fm30(i1, p1, p3, v0[225], globalState), globalState)), v0, !p3, p3;
			var v7: seq<int> := [v3[933]];
			v7 := p0;
		}
		for i3 := p1 / |p2| to p1 * -p1 {
			var v8: array<multiset<set<int>>> := new multiset<set<int>>[15](i4 => multiset{{i3}});
			var v9: map<bool, array<bool>> := map[v0[225] := v0];
			...
			var v15: set<bool> := {false, v0[225], p3, p3};
			var v16: seq<seq<int>> := [if (|v15| in v14) then v14[|v15|] else p0, seq(688, i5  => (p1))];
			globalState.f2 := if (v0[225]) then |v16[0x3d1]| else p1;
		}
		...
		var v25: array<int> := new int[2](i7 => i7 % p1);
		var v26 := DC9(v25, v0[225], v17);
		var v27: map<int, bool> := map[v26.cf16 := p3];
		var v28 := DC0('h');
		var v29: map<int, D0> := map[p1 := v28];
		v0[225] := if ((v17 + -p1) in v27) then v27[v17 + -p1] else fm2(v29, v17, fm3(p1, globalState), globalState);
		r0 := p1 / (if (p3) then v17 else p1);
	...
	method m1(globalState: GlobalState) {
		...
		if (v0) {
			...
			var v7: array<int> := new int[10](i0 => i0 / 0x3cc);
			v7[614] := -v1;
			v7[614] := v1;
			v2 := v2;
			var v8: array<bool> := new bool[5];
			var v9: multiset<int> := multiset{v1};
			var v10: seq<int> := [|v9|, v7[614]];
			...
			if (true) {
				...
				var v28: array<multiset<char>> := new multiset<char>[29] [v18, v22[|[false]|], v18, multiset{v17}, v18, v18 * v18, multiset(seq(-0x210, i1  => (v17))) + v18, v18, multiset{v17, v17, v17}, multiset{v17}, v18, v18, multiset{v17, v17, v17, 'l', v17}, multiset{v17} * multiset{v17}, v18, multiset{v17, v17, v17} - multiset(v25.cf13), v26.cf60, multiset(v16), v18, v18, (multiset{v17, v17, v17})[v17 := v15.f22] * v18, if (v15.f22 in v27) then v27[v15.f22] else v18, v18, v18, v18 * v18, v18 - multiset{v17, v17, v17}, v18 + v18, v18 + v18, v18];
				v28[809] := v18 * v18;
				v28[809] := multiset{v17, v17, v17};
				var v29: array<bool> := new bool[19](i2 => !v15.f21);
				v29[618] := v0;
				v29[979] := v17 !in v16;
				v29[618], v29[979], v15.f22, globalState.f2, globalState.f6 := --(if (false) then v1 else 0x2e9) <= v15.f22, v15.f21, v15.f22, v15.f22 + v15.f22, v19;
				...
				v36[501] := if (v15.f21) then v35 else v35.(cf21 := v34, cf23 := v30);
				var v37: array<int> := new int[8](i3 => i3 + 495);
				v33, v36[501], globalState.f6 := {!v0, v15.f21, v15.f21}, v35, v37;
				var v38: array<char> := new char[11];
				v38[19] := v30;
				v38[19] := v30;
				v1 := (v1 * v15.f22) / v15.f22;
			...
			v41[724], v0, v0, v0 := v42, fm2(v45, v48.cf10, v15.f22, globalState), v1 != v15.f22, v15.f21;
		...
		for i5 := 0x1d2 to v1 / v53[-v1] {
			...
			v54[372] := v56;
			var v59 := "xhy";
			v54[372] := (v59 + (seq(573, i6  => (v58)))) != v59;
		}
	}
	method m9(p0: array<bool>, p1: bool, globalState: GlobalState) returns (r0: int, r1: bool, r2: seq<int>, r3: char) {
			...
			decreases 100 - i1
		{
			...
			if (if (p1) then !p1 else p1) {
				...
				v28 := v28[v22 := (seq(-0xb7, i4  => (v7))) + fm31(DC8(|v22|, |v20|, p1, v26, v22), v5, p1, globalState)];
				...
				p0[2], r1, r0 := v30 > (set v32 : seq<char> | v32 in v31 :: (v32)), v33.cf44, fm3(v5, globalState);
			}
			...
			if (true) {
				...
				v35[873] := v36;
				v35[873] := new int[24](i6 => i6 * v5);
				var v37 := DC0(v7);
				var v38: map<int, D0> := map[v5 := v37];
				r1 := fm2(v38 + v38, v5 / v5, v5, globalState);
				var v39: array<array<bool>> := new array<bool>[13];
				v39[471] := p0;
				var v40 := DC17(p1, fm3(v5, globalState), p1, p1);
				v39[471], r1 := p0, v40.cf34;
				var v41: set<int> := {v5};
				r1 := fm2(v38 + map[v5 := v37], |v41| / v5, v5, globalState);
			} else {
				...
				v51 := v51;
				var v52: array<int> := new int[6](i7 => i7 / v44.f22);
				...
				v52[425] := -v42.f22 * (|v55| * v42.fm7(|v56|, v49, p1, true, globalState));
			}
		...
		var v59 := 'k';
		match (DC21(p0, !true, fm2(map v57 : int | (0xd9 <= v57) && (v57 < 0x12c) :: (v57 % v5) := (DC0('q')), v5, if (p1 in v58) then v58[p1] else v5, globalState), if (p1) then v59 else v59)) {
			case DC20(cf37, cf38, cf39) =>
				r3 := v59;
				var v60: map<int, bool> := map[0x1f0 - cf37 := cf39];
				v60 := v60[cf37 := p1];
				cf39 := p1;
				globalState.f2 := cf37 / cf37;
			case DC21(cf40, cf41, cf42, cf43) =>
				...
				var v66: array<array<int>> := new array<int>[25] [v61, v61, v61, if (cf41) then if (v5 in v63) then v63[v5] else v61 else v61, v61, v61, v61, v61, v61, v61, v61, v61, v61, v61, v61, v61, v61, v61, v64[v5], v61, v61, if (!cf42) then v61 else v61, if (p1) then v61 else v61, v65.cf14, v61];
				v66[904] := v61;
				v66[904] := new int[23](i9 => i9 - v5);
				p0[958] := v6 > v6;
				var v67: map<bool, array<int>> := map[cf41 := v61];
				var v68: map<bool, bool> := map[true := p1];
				var v69: seq<int> := [-|v68|];
				p0[958], v67, globalState.f2, cf41 := (0x329 + |v69|) > v5, v67, v5, false;
				...
				if (cf42) {
					...
					var v77: array<map<int, array<bool>>> := new map<int, array<bool>>[21] [v73, map[|v74| := cf40], v73, v73, v73, v73, v73, v73 + v73, v73, DC33(v73).cf64, v73, v73, v73 + v73, v73, if (p1) then v73 else v73, v73 + v73, v75[v5] + map[v5 := cf40], v73, v73, v76.cf64 + v73, v73];
					v77[950] := v73;
					v77[950] := v73 + (map[|v69| := cf40])[v5 := cf40];
					...
					v61[89] := fm3(|v81|, globalState);
					var v82: set<array<int>> := {v61};
					v61[89], v82, v66[904] := fm3(v5, globalState) - v5, v82 + v82, v66[904];
					...
					var v87: map<int, int> := map[|v86[v5]| := v61[89]];
			...
			case DC19(cf36) =>
				...
				var v92 := DC1(seq(0x1d4, i11  => (|[v91.f22]|)));
				...
				if (p1 <==> (v93 > v93)) {
					...
					var v96: array<string> := new string[10] [v90, (seq(-0x2c0, i12  => (v59))) + "amqyl", v90, v90 + ("oayu")[|v90| := v59], "gwnkp" + v90[v91.f22 := v59], v90, v90 + v90, fm31(v95, v91.f22, p1, globalState), "tufdol", v90];
					cf36, r1, v96 := cf36, false, v96;
					var v97: seq<int> := [v91.f22];
					var v98 := new C0(v91.f22 >= v97[v91.f22], v91.f22);
					...
					var v105: map<int, multiset<int>> := map[v104.cf52 := (fm35(p1, true, globalState))[v5 := -v91.f22]];
					v105 := v105[0x381 := v6];
		...
		for i13 := v5 to v5 {
			...
			if (p1) {
				...
				v6 := fm35(p1, v123[i13 := p1] < [p1, p1], globalState);
				var v124: map<int, int> := map[|[p1]| := v5];
				var v125: set<int> := {(if (i13 in v124) then v124[i13] else v5) / v5, |(if (p1) then fm36(p1, true, globalState) else v123)|};
		...
		var v137: map<int, string> := map[v5 := if (0xa3 in v136) then v136[0xa3] else seq(-0x377, i15  => ('o'))];
		r1 := v136[v5 := v135] == (fm37(p1, globalState) + fm37(p1, globalState));
		var v138: array<int> := new int[3](i16 => i16 - v5);
		v138[149] := -(v5 / v5);
		v138[149] := v5;
		var v139: set<bool> := {p1, false};
		r0 := 0x4c + (|v139| + v138[149]);
		r1 := p1;
		var v140: seq<int> := [v5];
		r2 := v140;
		r3 := fm30(v138[149], |v135| % -v138[149], p1, !p1, globalState);
	}
}
...
class C2 extends T0, T2 {
	...
	function fm2(p0: map<int, D0>, p1: int, p2: int, globalState: GlobalState): bool {
		!({|multiset{false, false}|} != {-0x1c6, |(seq(0x3b1, i0  => ('b')))|})
	}
	function fm3(p0: int, globalState: GlobalState): int {
		if (-0xe9 != |[false]|) then -|(if (false) then {|[|map['q' := false]|, 49, |[-0x2de]|, 26, |(set v0 : map<int, bool> | v0 in (seq(0x333, i0  => (map[|map[false := false]| := true]))) :: (v0))|]|, 0x25} else set v2 : int | v2 in (map v1 : int | (0x169 <= v1) && (v1 < 0x187) :: (v1 / |[0x1ec, |[true, true]|]|) := (!true)) :: (v2 + |[true]|))| else 0x185
	}
	...
	method m0(p0: seq<int>, p1: int, p2: string, p3: bool, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
			...
			decreases 100 - i1
		{
			...
			if (v1.f22 == p1) {
				...
				var v5 := new C0(v4[p1 := v1.f22] > fm26(v1.f22, globalState), p1);
				...
				var v16: array<int> := new int[14] [if (v1.f21) then v1.f22 else v1.f22, |(p2[|v7| := v0] + p2)|, v1.f22, |v2|, v1.f22, p1, p1, v5.f22, v1.f22, -(v5.f22 + |v10|), -(v1.f22 * |v11|), |p2|, v5.f22, p0[-|v15|]];
				globalState.f6, globalState.f2 := v12, |v8|;
				var v17 := new C0(v1.f21, v5.f22 - p1);
				v6[242] := v1.f21;
				var v18 := DC0(v0);
				var v19: map<int, D0> := map[|p0| := v18];
				v6[242] := fm2(if (v5.f21) then v19 else v19, v1.f22, v17.f22 / v17.f22, globalState);
				...
				var v29 := DC22(p3, v1.f22, |((map v26 : int | v26 in v27 :: (v26 + p1) := (v23[-v1.f22]))[v1.f22 := v1.f21] + v28)|);
				var v30: array<bool> := new bool[10];
				globalState.f5, v25[127], v29, v1.f22 := v30, v0, v29, v1.f22;
				var v31 := true;
				v31 := v1.f21;
				var v32: array<int> := new int[12](i2 => i2 % v1.f22);
				var v33: map<array<int>, C0> := map[v32 := v1];
				v33 := v33[v32 := v1];
				var v34: map<int, D0> := map[v1.f22 := DC0(v25[127])];
				v31 := fm2(v34, v1.f22, |((seq(0x381, i3  => (v25[127]))) + p2)|, globalState);
			}
			...
			if (v1.f21) {
				...
				v35[219] := |(v36 - (v36 * multiset{!v1.f21}))|;
				...
				v38[958] := (p0[v1.f22] / (if (p1 in v39) then v39[p1] else 0x2b2)) != v1.f22;
				var v41: seq<bool> := [v1.f21, v1.f21];
				var v42: set<int> := {p1};
				var v43: map<seq<bool>, set<int>> := map[v41 := v42];
				var v44 := new C0((set v40 : int | v40 in p0 :: (v40 / 0x227)) > (if (v41 in v43) then v43[v41] else v42), 0xd3);
				v41 := v41;
			} else {
				...
				v46[698] := {|v47|};
				v46[698] := v45;
				var v48: array<int> := new int[3](i5 => i5 % -0x1b3);
				v48[50] := |[v1.f21, !v1.f21, v1.f21]|;
				v48[50] := if (!v1.f21) then v1.f22 else v1.f22;
				globalState.f2 := v1.f22 / 0x240;
				var v49: array<bool> := new bool[29];
				v49[237] := !p3;
				v49[237] := (seq(0x40, i6  => (v0))) <= p2;
			...
			if (v1.f21) {
				...
				var v54: seq<bool> := [if (!v53[|p0|]) then v1.f21 else v1.f21, true];
				var v55 := true;
				...
				v59[372] := v1.f22 == -0x156;
				var v60: multiset<bool> := multiset{p3, v1.f21};
				var v61: multiset<int> := multiset{-v1.f22};
				var v62: array<seq<int>> := new seq<int>[11] [p0 + p0, p0, p0, p0[|v60| := v1.f22] + p0, p0, p0, [v1.f22, |v61|, 62], p0 + (seq(-628, i7  => (|p2|))), p0[p1 := v1.f22], fm0(|v53|, "egmmpy", |p2|, globalState), [v1.f22, |p2|] + p0];
				v62[979] := p0 + p0;
				v57[450] := v51.fm3(0x139, globalState);
				globalState.f2, v59[372], globalState.f2, v62[979], v57[450] := v1.f22, |v54| != v51.fm3(v1.f22, globalState), v1.f22 % p1, fm0(v1.f22, p2, if (v55) then fm3(p1, globalState) else p1, globalState), if (v1.f22 in v61) then v61[v1.f22] else -0x1d2;
				v55 := v1.f21;
				var v63 := DC25(v59[372]);
				...
				v66[257] := if (!v67[-0x74]) then p1 else v1.f22 - -|v67[p1 := v65]|;
				var v68: set<int> := {|(p2 + p2)|};
				v66[257], v68 := v1.f22, v68;
				v65 := p3;
				globalState.f1 := p2[v1.f22];
			}
		...
		var v74: array<char> := new char[24] [v0, v0, v0, v0, fm27(!p3, v1.f21, p1, globalState), v0, v0, if (v1.f21 in v70) then v70[v1.f21] else p2[|[p1]|], v0, v0, v0, p2[p1], fm30(v1.f22, p1, p3, v1.f21, globalState), v0, v0, fm27(if (v72[p3 := fm39(v1.f21, v1.f22, v1.f22, globalState)] in v73) then v73[v72[p3 := fm39(v1.f21, v1.f22, v1.f22, globalState)]] else v1.f21, v1.f21, |"tvfi"|, globalState), v0, v0, v0, 'f', v0, v0, v0, v0];
		forall i8 | 0 <= i8 < v74.Length {
			v74[i8] := v0;
		}
		globalState.f2 := --v1.f22 * p1;
		r0 := fm25(p3, v1.f21, v1.f22, globalState) % p1;
		var v75: multiset<int> := multiset{p1, |"a"|};
		var v76: seq<bool> := [v1.f21];
		var v77 := DC28(v76[v1.f22], p3);
	...
	method m4(p0: int, p1: set<map<char, int>>, p2: int, p3: bool, globalState: GlobalState) returns (r0: int, r1: bool) {
		...
		for i0 := |(v0 + v0)| to 948 {
			...
			if (p3) {
				...
				r1 := v5[v1.fm3(p0, globalState)] <= (v4 + v4);
				var v6: multiset<bool> := multiset{p3, p3};
				var v7: map<int, int> := map[p0 := p0];
				var v8: array<int> := new int[13] [-0x114, i0, |fm0(i0, seq(0xb2, i1  => (v2)), p2, globalState)|, |(v6[p3 := p2] * v6)|, |[!!p3, p3, p3]|, p2, -p0, -|v0[i0 := -0x6f]|, p0 + p0, p0, v1.fm3(|map[i0 := 0x4c]|, globalState), p0 - i0, -fm3(|v7|, globalState)];
				v8[793] := i0;
				var v9: array<bool> := new bool[21](i2 => p3);
				var v10: seq<array<bool>> := [v9];
				v8[793] := |(v10[i0 := v9] + v10)|;
				v9[754] := !(!p3 || p3);
				var v11 := "eoip";
				v9[754] := v2 !in v11;
				v11 := v11;
				...
				v0 := [|(v12 + v12)|, v0[v0[997]]];
				var v14: array<bool> := new bool[2](i3 => {p0} > (set v13 : int | (0x111 <= v13) && (v13 < 0x17) :: (v13 % i0)));
				v14[687] := p3 || p3;
				var v15: map<bool, int> := map[p3 := p2];
				var v16: map<string, map<bool, int>> := map[v12 := v15];
				v14[687], v15, globalState.f2 := p3, if (v12 in v16) then v16[v12] else v15 + v15, fm3(0xd5, globalState) % (if (p3) then p0 else p2);
				var v17 := new C0(v14[687], fm3(p2, globalState));
				var v18 := new C1();
			}
			
			if (false) {
				var v19 := "mtpymbdc";
				globalState.f2 := (fm3(-v1.fm3(i0, globalState), globalState) % |{v19, v19, seq(0x2dc, i4  => (v2)), v19}|) + v1.fm3(i0, globalState);
				var v20: map<int, bool> := map[|v19| := p3];
				var v21: multiset<int> := multiset{-p2, p2};
				r1 := (if (|v21| in v20) then v20[|v21|] else p3) ==> p3;
				r1 := !p3;
				globalState.f2 := |v0|;
				var v22: map<int, seq<char>> := map[p0 := seq(0x80, i5  => (v2))];
				var v23: seq<string> := [v19, v19];
				var v24: array<int> := new int[11] [p2, |(if (p2 in v22) then v22[p2] else v19)|, i0 % p2, -p0, -fm3(p2, globalState), -p0, p0, -56 - p0, p0 + |v23[p0]|, i0, |(v19 + v19)|];
				var v25: map<bool, bool> := map[false := p3];
				v24[645] := p2 - |v25|;
				v24[645] := |((v19 + (seq(0xa2, i6  => (v2)))) + (v19 + v19[|(seq(0x365, i7  => (v2)))| := v2]))|;
			} else {
				var v26 := "swtajd";
				var v27: set<int> := {i0};
				var v28 := DC1(v0);
				...
				var v38: array<int> := new int[23] [p0, p0, p0, p0 - p2, -p0, p0, p0, v31.cf74, |[v32]|, |v26| + p0, |[p3]|, |((if (i0 in v33) then v33[i0] else v26) + v26)|, p0, i0, p2, 0x3e3, if (true) then if (p2 in v35) then v35[p2] else p2 else if (p0 in v37) then v37[p0] else i0, p2 * |"tjxltg"|, p0, p0, p0, v0[-p2], p2];
				...
				v38[492] := |(v40 + (if (p3 in v41) then v41[p3] else v40))|;
				var v42 := DC20(0x3b2, p0, p3);
				v38[492], globalState.f2 := v42.cf37, p0;
				r1 := p3 || (p3 in v32);
				var v43: multiset<string> := multiset{v26 + v26[|v36| := v2], fm31(DC8(p0, |v0|, p3, v3, v26), -i0, p3, globalState), if (p3) then v26 else v26[0x254 := v26[v38[492]]], v26[i0 := v2] + v26, v26};
				v43 := v43;
				var v44: map<array<int>, string> := map[v38 := v26];
				v38[492] := |(v44 + v44)| + p2;
				var v45: array<bool> := new bool[2](i8 => p3);
				v45[52] := p3;
				var v46: map<int, D0> := map[|(seq(0x386, i9  => (|(seq(133, i10  => (v38[492])))|)))| := fm40(p3, v38[492], v26, globalState)];
				v45[52], v38[492] := v1.fm2(v46, |[--380]|, p2, globalState), |{p3, p3, p3, p3}| - (p2 - v38[492]);
			}
		...
		if (!!p3 || p3) {
			...
			var v49 := DC39(v48);
			r0, v49 := |"hg"|, v49;
			var v50 := DC16(p3 <== p3, p0 % p2, p3, p0);
			match (v50) {
				case DC16(cf27, cf28, cf29, cf30) =>
					...
					var v60 := DC9(v58[if (cf29 in v59) then v59[cf29] else p2], cf29, cf30);
					globalState.f2 := -v60.cf16 % p0;
				case DC17(cf31, cf32, cf33, cf34) =>
					var v61: seq<seq<int>> := [v0, v0];
					var v62: multiset<seq<seq<int>>> := multiset{(seq(-0x3db, i11  => (v0))) + v61};
					...
					v64[93] := if (fm2(v67, p0, p2, globalState) && false) then v65 else fm30(p0, cf32, cf31, cf31, globalState);
					var v68: array<bool> := new bool[26];
					v68[675] := if (p3) then false else cf31;
					var v69: C1 := new C1();
					...
					globalState.f2, globalState.f5, r1, v68[675], v69 := v69.fm3(|(map[cf34 := p0] + v70)|, globalState), v68, cf34 || (cf32 == p0), !(cf32 >= p2) <==> cf33, if ((|v73| - p2) in v72) then v72[|v73| - p2] else v69;
					var v74: set<int> := {p2};
					var v75: array<multiset<int>> := new multiset<int>[7] [if (cf33) then v48 else multiset(fm0(p0, "eldkn", 0x376, globalState)), fm35(cf33, cf33, globalState), multiset{|v74|, |[v68[675], cf33]|}, v48 + v48, v48, v48, multiset{p2}];
					v75[748] := v48;
					var v76: array<int> := new int[25];
					v68[675], v47, globalState.f6, v75[748] := false || cf34, v47, v76, v48;
				case DC18(cf35) =>
				...
				case DC15(cf26) =>
					...
					var v94: set<int> := {if ((v92[true := v91])[v90[p2] := v91] in v93) then v93[(v92[true := v91])[v90[p2] := v91]] else p2, p2};
					v94 := v94;
			}
			
			globalState.f2 := p0 / (|v0| * 0x59);
			globalState.f1 := 'o';
			globalState.f1 := 't';
		} else {
			...
			if (v95 != (v95 + v95)) {
				...
				var v104: array<C0> := new C0[29] [v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v102, v103[p0], v102];
				...
				r1, globalState.f5, globalState.f2 := p2 <= |v99|, v106, p2 % (p2 - p2);
				r1 := p3;
				...
				var v109 := new C0(p3, -(p0 / p2));
				var v110: array<int> := new int[5](i12 => i12 / v109.f22);
				v110[256] := v109.f22;
				...
				r1, v110[256], v111[808], v114 := (multiset(seq(-0x3c5, i14  => (p0))) !! v115) <==> p3, (p2 + |(seq(0x352, i15  => (p2)))|) * (if (p2 in v108) then v108[p2] else p0), if (p3) then p3 else p3, v114;
				var v116: map<multiset<int>, bool> := map[v115 := p3];
				v111 := new bool[19] [p2 < 189, p3, if (false) then p3 else p3, if (!v111[808]) then v109.fm8(globalState) else p3, v115 in v116, v111[808], v109.f21, v109.f21, !(v111[808] ==> v95[p2]), v109.f21, p3, p3 <==> v111[808], !v109.f21 && v109.f21, v95[v110[256]], p3, v109.f21, p3 ==> v109.f21, p3, v111[808]];
			}
		...
		for i16 := p2 to p2 {
			...
			match (v119) {
				...
				case DC31(cf60) =>
					...
					var v127: array<int> := new int[21](i18 => i18 / DC3(v122.f21, p2).cf4);
			...
			v130[480] := v117;
			v129, v130[480] := fm41(v117, p3, globalState), v117;
			r0 := i16;
	...
	method m5(globalState: GlobalState) returns (r0: int, r1: seq<set<bool>>, r2: int, r3: int) {
		...
		if (if (v5 in v11) then v11[v5] else v3) {
			var v13: set<int> := {|(map v12 : string | v12 in fm42(v6, globalState) :: (v12) := (("brbt")[-793 := v6]))|, v5, v5};
			var v14 := "rmbg";
			var v15: array<string> := new string[8](i0 => v14);
			v15[7] := ("srkaqou")[v5 := v14[v5]];
			v13, v14, v3, v15[7], v3 := v13, v14, true, v14, v3;
			var v16: array<bool> := new bool[12](i1 => v3);
			var v17: seq<bool> := [v3];
			var v18: set<seq<bool>> := {v17};
			v16[305] := v18 > v18;
			v16[305] := v3;
			var v19: multiset<string> := multiset{v15[7], v15[7] + v14[v5 := v6]};
			...
			if (v3) {
				...
				globalState.f2 := -v5;
				var v20: C1 := new C1();
				globalState.f1, v16[305], v20, r0, v3 := if (v16[305]) then v6 else v6, v16[305], v20, v5, v5 < v5;
				r0 := -(v5 - v5);
				v3 := v17[-v5];
			} else {
				var v21 := new C1();
				var v22: array<seq<int>> := new seq<int>[14](i3 => (seq(0xc2, i4  => (v5))) + [|map[|v14| := v5]|]);
				var v23: array<array<bool>> := new array<bool>[14];
				v23[119] := v16;
				v6, v16[305], v22, v16[305], v23[119] := v15[7][-(if (v16[305]) then v5 else v5)], v3, v22, v3, v16;
				var v24: map<string, string> := map[("qi")[v5 := v6] := v14];
				var v25: map<int, char> := map[v5 := v6];
				var v26 := DC8(v5, v5, !v16[305], v25, seq(-0x193, i5  => (v6)));
				var v27: multiset<int> := multiset{v5};
				var v28: map<bool, multiset<int>> := map[v3 := v27];
				var v29: seq<int> := [|v28|];
				v3, r3, globalState.f2 := v3, |(if (fm31(v26, v29[v5], v3, globalState) in v24) then v24[fm31(v26, v29[v5], v3, globalState)] else v14)|, 0x6b;
				r2 := |((v17 + v17[v5 := v16[305]]) + v17)|;
				r0 := v5 + v5;
			}
			...
			var v32 := DC45(v31);
			v30[837] := {v31, v32.cf87, map[v3 := v3]};
			var v33 := DC32(0x122, v3, v3);
			var v34: set<map<bool, bool>> := {v31 + v31, v31, map[v3 := v3]};
			v1[331], r2, v30[837] := v33.cf61, v5, v34;
			var v35: set<int> := {v5, v1[331], v5};
			var v36: map<int, set<int>> := map[v5 := v35];
			var v37: array<int> := new int[11];
			v35, globalState.f6, globalState.f6 := v35 * ((if (v5 in v36) then v36[v5] else v35) - fm38(v3, globalState)), v37, v0[767][fm3(v1[331], globalState)];
			var v38 := "elqxnm";
			var v39: map<bool, char> := map[v5 <= |v38| := v6];
			r3, v39 := v1[331], (map[v3 := v6] + v39) + map[v3 := v6];
			if (v1[331] > v1[331]) {
				...
				v43 := v43;
				var v44: C0 := new C0(v42.fm2(v7, |v39|, v5, globalState), v1[331]);
				v44 := v44;
				...
				v48 := v48[v1[331] := v31];
				var v49: seq<int> := [v1[331], v1[331], v5];
				v49 := ((seq(0x3ac, i9  => (v5))) + v49) + v49;
				v1[331] := |fm43(globalState)|;
				var v50: array<array<int>> := new array<int>[3];
				var v51: array<bool> := new bool[1](i10 => v4 < DC26(v4).cf50);
				v51[556] := v3;
				var v52 := DC42(v1[331], |v38|);
				var v53: multiset<D16> := multiset{v52, v52, DC42(0x367, -|v38|), v52, v52};
				var v54 := DC0(v6);
				var v55: map<bool, int> := map[v3 := 718];
				var v56 := DC20(0x2f9, v1[331], v3);
				var v57 := DC22(v3, v1[331], v1[331]);
				var v58: map<int, int> := map[0xf := v5];
				v1[331], v3, v3, v50, v51[556] := fm25(v53 > v53, fm2(map[v1[331] := v54], |[v1[331], if (v3 in v55) then v55[v3] else v1[331], 0x8d]|, v5, globalState), v1[331], globalState), if (v3) then v3 else v56.cf39 <== v3, if (if (true) then if (v1[331] in v9) then v9[v1[331]] else v3 else !v3) then v3 else v57.cf44, v50, v58 != (v58 + v58);
				v3 := v3;
			}
			
			var v59: multiset<int> := multiset{|"cvwlohc"|};
			var v60: seq<int> := [if (v5 in v59) then v59[v5] else v5];
			var v61: seq<int> := [v5, |v38|, v1[331] - |v60|];
			var v62: map<string, int> := map[v38 := v1[331]];
			var v63: map<bool, int> := map[v3 := v1[331]];
			var v65: map<string, set<int>> := map[seq(0x157, i11  => (v6)) := v35];
			var v66: multiset<multiset<bool>> := multiset{multiset{v3}};
			var v67: map<int, int> := map[v1[331] := v1[331]];
			v60, v62, globalState.f2, r0 := if (v3) then ([v5])[|v63| := 0x399] else v60, map v64 : string | v64 in v65 :: (v64) := (|v38| / v1[331]), v1[331] + |(v66 - fm44(v5, true, globalState))|, (if (|v38| in v67) then v67[|v38|] else v5) + |map[|map[v3 := v6]| := !true]|;
		}
		
		var v68: seq<bool> := [v3, v3, if (v5 in v9) then v9[v5] else v3, v3];
		var v69: set<seq<bool>> := {v68, v68[v5 := false]};
		...
		var v73: map<D8, int> := map[DC22(v3, v5, v5) := -v5 % v5];
			...
			decreases 100 - i13
		{
			...
			v1[735] := v5 % (|(set v76 : int | v76 in v75 :: (v76 % -137))| - v5);
			var v77: array<D11> := new D11[10];
			v77[420] := DC28(!v3, false).(cf54 := !v3);
			v1[735], r3, v77[420] := if (v3 in v70) then v70[v3] else v1[735], v5, if (false) then DC28(v3, v3) else DC28(v3, v3);
		...
		r1 := v80 + (v80[-996 := {true, v3, false, true, v3}] + v80);
	...
	method m2(p0: bool, p1: int, p2: int, globalState: GlobalState) returns (r0: array<int>, r1: bool, r2: seq<bool>) {
		...
		var v3: seq<int> := [p2];
		var v4 := DC20(|v3[p1 := p1]|, p1, p0);
		var v5: map<bool, bool> := map[p0 := v4.cf39];
		var v6: seq<bool> := [p0, p0];
		var v7: map<bool, bool> := map[if (!!v6[p1] in v5) then v5[!!v6[p1]] else p0 := p0];
		v7 := v7[p0 <==> p0 := p2 < p1];
		if (p0 <==> p0) {
			...
			match (v17) {
				case DC16(cf27, cf28, cf29, cf30) =>
					cf30 := 693;
					var v18: array<int> := new int[25](i1 => i1 / cf28);
					v18[641] := fm3(cf28, globalState);
					...
					v21[194] := v22 == v22;
					v18[641], v20, v21[194] := -(|v0| * cf28), DC39(v19[|(map v23 : int | (804 <= v23) && (v23 < 492) :: (v23 % cf30) := (cf29))| := cf30]).(cf77 := v19), p0;
					var v24: map<int, char> := map[0x3a0 := v14];
					var v25: multiset<D4> := multiset{if (true) then DC8(p2, p2, v21[194], v24, v0) else DC8(p1, |v0|, p0, v24, v0)};
			...
			v51[957] := if (p0) then 'a' else 'c';
			var v52: set<int> := {p1};
			var v53 := DC29(p0, p1, p2, p1);
			var v54 := DC21(v50, p0, v53.cf55, v14);
			v7, v49[649], v51[957], globalState.f1 := v7, v50, fm30(p2, -p2, {p2} <= v52, p0, globalState), v54.cf43;
		} else {
			...
			v55[580] := p0 || fm2(v58, v61.cf57, |v62|, globalState);
			var v63: set<char> := {v56, v56, v56, v56};
			r1, v55[580], globalState.f2, r1 := fm2(map[p1 := DC0(v56)], p2, |v63|, globalState), p0, p2 / |v59|, p0 && p0;
			r2 := fm36(v55[580], fm2(v58, fm25(!p0, p0, p2, globalState), p2, globalState), globalState);
			var v64: map<bool, int> := map[p0 := p2];
			var v65: map<map<bool, int>, bool> := map[v64 := false];
			v55[580] := if (v64 in v65) then v65[v64] else fm2(v58[|v6[0x3d9 := p0]| := v57], p2, |[p0, v55[580]]|, globalState);
			globalState.f2 := -(p1 % -p1);
			var v66: set<array<bool>> := {v55, v55};
			var v67: set<string> := {"uik", v0, v0[p2 := v56]};
	...
	method m3(p0: seq<bool>, p1: string, p2: int, p3: bool, globalState: GlobalState) {
		for i0 := p2 to p2 {
			...
			var v9: seq<array<int>> := [v7, v8[p2]];
			var v10 := DC50(v8);
			globalState.f2, v1[971], globalState.f2, globalState.f5, v3[17] := p2, p1, |v4| + p2, v3, if (fm2(v6, -p2, |v10.cf100|, globalState)) then p2 != p2 else p3;
			var v11 := DC37(p0, !v3[17], v0);
			var v12: seq<D14> := [v11, v11, v11];
			var v13: array<seq<D14>> := new seq<D14>[22] [v12, v12, v12, v12, v12, v12, v12, [v11, v11], v12, v12, v12, v12[|p1| := fm45(p2, v0, -i0, i0, globalState)], seq(0x10, i1  => (v11)), v12, v12, v12, v12, [v11, v11], [DC37([true], v3[17], p3)], [v11], v12, seq(0x195, i2  => (v11))];
			...
			if (v17 >= (v17 - v17)) {
				var v18 := DC29(v3[17], |(seq(560, i3  => ('b')))|, p2, p2);
				var v19: seq<D11> := [v18];
				var v20: seq<int> := [i0];
				var v21: set<int> := {|v20|};
				...
				var v25 := DC51(|"no"|, i0, v1[971], i0);
				v24[133] := v25;
				v24[133] := v25;
				v7[246] := i0;
				v7[246] := if (p3) then v20[p2] else i0;
				globalState.f2 := fm25(v7[246] <= v7[246], p0[i0], i0, globalState);
				var v26 := new C1();
			} else {
				globalState.f2 := p2;
				var v27: seq<string> := [v1[971]];
				globalState.f2 := (|v27[i0]| % i0) * p2;
				var v28: T0 := new C1();
				var v29: map<T0, int> := map[v28 := i0];
				var v30 := DC52(v4);
				var v31: array<map<bool, int>> := new map<bool, int>[15] [v4, fm39(v0, |v29|, -463, globalState), if (v0) then map[v0 := p2] else v4, v4, v30.cf105, v30.cf105, v4, map[p3 := i0] + v4, map[!v3[17] := i0], v4 + v4, v4[p3 := i0] + v4, v4, v4, v30.cf105[p3 := i0], v4];
				v31[681] := v4;
				v31[681] := v4 + v4;
				var v32: array<array<int>> := new array<int>[10] [v7, v7, v7, v7, v7, v7, v7, v9[i0], v7, v7];
				v32[574] := v7;
				v0, v32[574], globalState.f2 := p2 !in map[-499 := p2], v7, p2;
		...
		for i6 := 0x282 to p2 * 0x220 {
			...
			var v42: C0 := new C0(!p3, -|(seq(0x2a8, i7  => (v36)))|);
			v41[804] := v42;
			v41[804] := v42;
			v40 := p3;
		}
		...
		var v50: seq<seq<int>> := [v47[|v49|]];
		v44 := if ((DC11(p3, p3).cf19 && p3) in v45) then v45[DC11(p3, p3).cf19 && p3] else multiset(v47[p2]);
	}
}
...
class C3 extends T0 {
	...
	method m0(p0: seq<int>, p1: int, p2: string, p3: bool, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
		...
		var i0 := 0;
		while ((p1 % (if (p3 in v0) then v0[p3] else p1)) != p1)
			decreases 100 - i0
		{
			if (i0 >= 100) {
				break;
			...
			decreases 100 - i1
		{
			...
			var v9 := 'n';
			v7, globalState.f1, globalState.f2 := (v8 + {v7}) > v8, v9, p0[p1];
		...
		if (p3) {
			...
			var v14 := 'u';
			r0, globalState.f2, v12, globalState.f1 := -(-p1 * (|p2| / p0[|v13|])), p1, v12, v14;
			...
			v19[927] := v16;
			v15[250] := p3;
			v17, v18[836], v19[927], v16, v15[250] := fm20(p3, p3, globalState), p1, (p2 + "kqsp") + p2, ("n" + "rvgjfwpb") + v16, p3;
		} else {
			var v20 := 'k';
			var v21 := DC0(v20);
		...
		var v44: map<int, map<int, map<int, bool>>> := map[p1 % p1 := map[p1 := map[|[v42]| := v42]]];
		v44 := v44[p1 + p1 := map v45 : int | (0x2b9 <= v45) && (v45 < 0xce) :: (v45 * p1) := (map[|p2| := v42])];
		r0 := p0[p1] + (p1 + p1);
		var v46: set<int> := {p1, |p0|, p1, p1, p1};
		var v47 := 'x';
		var v48: multiset<int> := multiset{|v46|, p1 % 0x37f, |p0[p1 := -p1]| - |fm24(p1, DC13(p2, p3, v47, p3), p1, globalState)|, p1, -p1};
		r1 := v48;
	}
	method m1(globalState: GlobalState) {
		var v0 := true;
		if (!v0) {
			...
			var v6: seq<map<int, bool>> := [v5, map[|(seq(0xae, i0  => (v1)))| := v2.f21]];
			v4[357] := v6;
			var v7: seq<int> := [v1];
			var v8 := 'g';
			var v9 := DC13("exv", v1 !in v7, v8, v2.f21);
			v3, v4[357], v9, globalState.f2, v1 := v3, v6, v9, v2.f22, v2.f22;
		} else {
			...
			globalState.f2, v10[866], globalState.f2, v10[474] := v13[|v15|], -v11, v11, v11;
			v0 := v0;
			var v16 := DC20(v10[866], |[v13]|, v0);
			var v17: map<D8, int> := map[v16.(cf39 := v0, cf37 := v11) := fm3(v10[866], globalState)];
			var v18 := new C0(v0, if (v16 in v17) then v17[v16] else v11);
			var v19: seq<bool> := [true, v0, v0];
			var v20 := 's';
			globalState.f1, v19 := v20, v19;
			v14[115] := true;
			v0, v10, v14[115], v11 := v0, v10, multiset{true, v18.f21} >= multiset{!v0}, |v12| % |v12|;
		}
		...
		var v26 := "gxgxaho";
		var i2 := 0;
		while (fm2(if (|(map v25 : int | (0x65 <= v25) && (v25 < 540) :: (v25 + v21) := (|map[|DC26(DC26({false, v0}).cf50).cf50| := v21]|))| in v24) then v24[|(map v25 : int | (0x65 <= v25) && (v25 < 540) :: (v25 + v21) := (|map[|DC26(DC26({false, v0}).cf50).cf50| := v21]|))|] else v23, -v21, v21 / |v26|, globalState))
			decreases 100 - i2
		{
			if (i2 >= 100) {
				break;
			...
			v28[651], v0, globalState.f1, v0, v0 := v21, v30 > (v30 * v30), v22, v0, v31 >= {v21, v21};
		...
		if (v0) {
			...
			v38[193] := v21 + |v39|;
			v38[193] := if (v0) then v21 else v21;
			var v40 := DC27(!v0, v38[193]);
			v40 := v40.(cf51 := !v0);
			var v41: T2 := new C2();
			...
			var v44: array<int> := new int[29](i3 => i3 % v21);
			var v45: seq<array<int>> := [if (v0) then v44 else v44, v44];
			globalState.f6 := v45[v21];
			v44 := v44;
			globalState.f1 := v22;
		...
		for i4 := -(v21 - |v26|) to v21 {
			...
			var v50: map<int, bool> := map[v49[i4] - v21 := v0];
			var v51: map<bool, int> := map[true := 0x234];
			v0, v50, globalState.f2, globalState.f2 := fm2(v23, i4, v21, globalState), v50[i4 := !v0] + v50, 0xc0, (if (v0 in v51) then v51[v0] else v21) / i4;
			var v52: set<bool> := {v0};
...
class C4 extends T2 {
	...
	method m4(p0: int, p1: set<map<char, int>>, p2: int, p3: bool, globalState: GlobalState) returns (r0: int, r1: bool) {
		...
		var v2 := 's';
		var v3: map<int, string> := map[p0 := v0[|v1| := v2]];
		var v4: array<string> := new string[1] [if (p0 in v3) then v3[p0] else v0[|map[p0 := f24]| := v2]];
		var v5 := DC12(v4);
		v4 := v5.cf20;
		...
		for i2 := p2 + p2 to p0 {
			var v9: seq<int> := [p0];
			var v10: map<seq<int>, multiset<bool>> := map[v9 := v1[p3 := i2]];
			v10 := v10;
			var v11: seq<bool> := [false];
			var v12: map<int, D0> := map[p2 := DC0(v2)];
			var v13 := DC13(fm14(|v11|, 0x39d, p2, p3, globalState), p3, v0[p0], fm2(v12, 0x217, i2, globalState));
			v13 := v13;
			if (p3) {
				f24 := v13.cf22;
				var v14: multiset<int> := multiset{p0};
				var v16, v17 := m8(if (p2 in v14) then v14[p2] else p2, map v15 : int | v15 in multiset(v9[i2 := p2]) :: (v15 % p2) := (p0), globalState);
				v16 := p3;
				...
				var v22: set<bool> := {true, v8[820]};
				var v23: set<int> := {v19[192], |v22|};
				var v24 := DC5(v23);
				v21[623] := v24.cf6;
				v21[623] := v23 * v23;
			}
	...
	method m5(globalState: GlobalState) returns (r0: int, r1: seq<set<bool>>, r2: int, r3: int) {
		...
		var v1: array<int> := new int[29](i0 => i0 / v0);
		...
		match (v2) {
			case DC8(cf9, cf10, cf11, cf12, cf13) =>
				...
				var v19: array<bool> := new bool[28] [true, f24, cf11, 'v' !in "iuiemgy", v3 <= v4, cf11, cf11, f24, v4 > v3, (if (v0 in v5) then v5[v0] else |v5|) != |v6|, cf11, v8 > v8, cf11, cf13 == cf13, fm2(v10[v0 := DC0(v7)], cf9, v0, globalState), f24, (if (|v11[cf9]| in v5) then v5[|v11[cf9]|] else |v13|) >= cf9, v15 != v15, cf11, cf11, v0 >= v0, cf11 || f24, multiset(v16) != v17, f24, v18.cf3 !in v16, f24, f24, cf11];
				v19[967] := f24;
				v19[967] := f24;
				...
				if (|v20| > v0) {
					...
					v12 := map[DC3(v19[967], -v0).cf3 := cf10];
					v19[967] := v19[967] !in [v19[967]];
					r3 := 0x3cd;
				} else {
					var v21: map<bool, char> := map[!fm2(v10, -cf9, cf9, globalState) <== v19[967] := v7];
					v21 := v21[v19[967] := v7];
					var v22: array<T1> := new T1[20];
					globalState.f2, f24, v22, f24, cf11 := cf9, v19[967], v22, v19[967], |(v16 + v16)| < cf9;
					var v23: map<int, seq<char>> := map[cf9 := cf13];
					v23 := v23[--cf10 := cf13];
					var v24: array<char> := new char[12] ['c', v7, 'c', v7, v7, v7, v7, if (v19[967]) then v7 else v7, v7, v7, v7, v7];
					v24[777] := v9.cf0;
					v24[777] := v7;
					f24 := v19[967];
				}
			...
			case DC9(cf14, cf15, cf16) =>
				match (DC9(v1, f24, v0)) {
					...
					case DC7(cf8) =>
						...
						v42 := v43[cf16] + (v42 + v42);
						var v44 := "ws";
						var v45: multiset<string> := multiset{v44};
						var v46: seq<multiset<string>> := [v45];
						var v47: map<array<int>, multiset<string>> := map[cf14 := (v46[|v44|])[v44 := cf16]];
						v47 := v47[v2.cf14 := v45];
						var v48: map<bool, int> := map[f24 := cf8.f22 * -0xc2];
						var v49: multiset<bool> := multiset{cf15, cf8.f21};
						cf16 := ---(if ((v49 >= v49[cf8.f21 := cf8.f22]) in v48) then v48[v49 >= v49[cf8.f21 := cf8.f22]] else cf8.f22);
		...
		var v66 := "pqyq";
		v65 := DC22(|v66[-v0 := fm17(globalState)]| < v0, 0x248, v0);
		var v67: seq<int> := [v0];
		v67 := (if (f24) then v67 else v67) + v67;
		var v68: array<array<seq<int>>> := new array<seq<int>>[21];
		var v69: seq<bool> := [f24];
		var v70 := 'o';
		var v71: array<seq<int>> := new seq<int>[20] [v67, v67, v67, seq(381, i2  => (v0)), [v0, -|v69|], v67, v67, v67, v67, fm0(v0, v66, |[v70, v70]|, globalState), v67, [v0], v67, v67, v67, v67, v67, seq(0x17, i3  => (v0)), [v0, v0], v67];
		v68[361] := v71;
		v68[361] := v71;
		r0 := v0 * v0;
	...
	method m2(p0: bool, p1: int, p2: int, globalState: GlobalState) returns (r0: array<int>, r1: bool, r2: seq<bool>) {
		var v0: array<int> := new int[20](i0 => i0 % p1);
		...
		for i3 := fm3(p2, globalState) / p1 to fm3(-p2, globalState) {
			var v15 := "s";
			v15 := "lg" + v15;
			r1 := i3 < i3;
		...
		r0 := v0;
		r1 := v1 !! v1;
		r2 := [!p0, p0, p1 >= p2, v2[264], v2[264]];
	}
	method m3(p0: seq<bool>, p1: string, p2: int, p3: bool, globalState: GlobalState) {
		f24 := p3;
		var v0: map<int, string> := map[0x2de % 0x2f2 := p1];
		var v1: seq<string> := ["npvw"];
		v0 := v0[p2 := v1[p2]];
		var v2: T0 := new C2();
		var v3: seq<T0> := [v2];
		var v4: multiset<int> := multiset{|v3[p2 := v2]|};
		var v5 := 'h';
		var v6 := DC0(v5);
		var v7, v8 := m8(-p2, fm18(|p1|, f24, !p3, v4[|fm46(p2, f24, p2, fm2((map[p2 := v6])[p2 := v6], p2, p2, globalState), globalState)| := p2], globalState), globalState);
		var v9: map<bool, seq<bool>> := map[v7 := p0];
		var v10: map<int, bool> := map[p2 := p3];
		var v11: array<seq<bool>> := new seq<bool>[22] [p0, p0, p0, p0 + p0, p0, p0, p0, p0 + p0, p0, p0, p0, p0, p0, p0, p0 + [v7], p0[(DC38(p2, 0x1a6, |(seq(-469, i0  => (p2)))|).(cf75 := 0xac)).cf75 := f24], p0, if (v7 in v9) then v9[v7] else p0, [p3, v7, true], p0, p0, [f24, if (p2 in v10) then v10[p2] else !f24, f24] + p0];
		v11[528] := p0 + p0;
		var v12: array<int> := new int[29];
		var v13: seq<int> := [|v10|];
		var v14: seq<seq<int>> := [v13];
		v12[806] := p2 * -fm3(|v14|, globalState);
		var v15: set<bool> := {v7, v7};
		var v16 := DC26({v7});
		v11[528], v12[806], v15 := p0, |v13| / p2, match v16 {
			...
			decreases 100 - i1
		{
			...
			i1 := i1 + 1;
			var v18: C0 := new C0(f24, v12[806]);
			v18 := v18;
			v7 := if (f24) then v18.f21 else v7;
			var v19 := new C0(v18.f21, -v18.f22);
			globalState.f2 := -v12[806] - p2;
		}
		var v20: map<int, seq<int>> := map[v12[806] := seq(0x3ba, i3  => (|v10|))];
		for i2 := |(if (v12[806] in v20) then v20[v12[806]] else [fm3(v12[806], globalState)])| to p2 {
			if (p3) {
				v0 := v0[i2 * 564 := p1];
				globalState.f1 := v5;
				v7, v7 := fm35(v7, f24, globalState) !! v4, !(v2.fm3(v12[806], globalState) >= (i2 + p2));
				globalState.f2 := |p1[|("y" + p1)| := v5]|;
				globalState.f1 := v5;
			} else {
				f24 := p0 <= [v7, v7, p3];
				...
				v21[256] := v4 != v4;
				v21[256] := v7;
				globalState.f2 := v13[if (!v7) then fm3(v12[806], globalState) else v12[806]];
				var v22: map<bool, int> := map[f24 := i2];
				var v23: map<int, char> := map[--v12[806] := v5];
				v13 := if (v21[256]) then [(DC8(|v22|, p2, v7, map[i2 := v5], p1).(cf13 := p1, cf12 := v23, cf11 := p3)).cf9] else if (|v15| in v20) then v20[|v15|] else seq(0x227, i4  => (v12[806]));
				v13 := (v13 + (fm0(fm3(v12[806], globalState), "ex", -v12[806], globalState) + [i2, i2]))[i2 := v12[806] % 0x273];
			}
			
			globalState.f2 := (if (f24) then p2 else p2) * (v12[806] + p2);
	...
	method m0(p0: seq<int>, p1: int, p2: string, p3: bool, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
			...
			decreases 100 - i1
		{
			...
			if (DC11(p3, f24).cf18) {
				...
				var v11: array<int> := new int[2](i3 => i3 % p1);
				v11[983] := fm3(v7.f22, globalState);
				v11[983] := v7.f22;
				var v12: map<seq<int>, int> := map[[v7.f22] := p1 / v7.f22];
				var v13: array<array<int>> := new array<int>[6];
				v13[976] := v11;
				v12, v13[976] := v12, v11;
			} else {
				var v14: seq<bool> := [false];
				f24, f24 := v2, if (v14[p1]) then false else f24;
				var v15: array<int> := new int[18](i4 => i4 - |("vyy")[|v14| := 'h']|);
				v15[152] := p1;
				var v16: map<bool, int> := map[v2 := p1];
				v15[152] := (if (p3 in v16) then v16[p3] else p1) * p1;
				...
				v20[232] := v18 >= v18;
				var v21 := 'e';
				globalState.f1, v2, v20[232], globalState.f2, v2 := v21, f24, p3, |p2|, !f24;
				globalState.f2 := 0xf4;
			}
			...
			var v29: array<D2> := new D2[29] [v23, v23, v23, v23, DC2(v2), DC2(fm2(v25, |"sjagqpdv"|, p0[|v1|], globalState)), if (f24) then DC2(p3) else v23, v23, v23, v23, fm47(f24, v28.cf30, p3, globalState), v23, DC2(p3), v23, v23, v23, v23, DC2(v2), v23, v23, DC2(!f24), v23, DC2(f24), v23, fm47(f24, |p0|, p3, globalState), fm47(v2, p1, f24, globalState), v23, v23, v23];
			v29[77] := v23.(cf2 := fm2(v25, -p1, p1, globalState));
			v29[77] := v23;
		}
		...
		v32[894] := fm17(globalState);
		var v33 := 'i';
		v30[514], f24, v32[894], r0 := v31, p3, v33, (p1 / p1) + p1;
		f24 := !((p1 % p1) > p1);
		var v34: array<int> := new int[10](i6 => i6 % p1);
		v34[448] := |p2|;
		var v35: map<int, D0> := map[p1 := DC0(v33)];
		var v36 := DC0('x');
		var v37: map<int, bool> := map[p1 := f24];
		var v38: seq<bool> := [f24];
		v34[448] := |[fm2(v35[p1 := v36], |v37|, fm3(p1, globalState), globalState)]| % |v38|;
		var v39: multiset<int> := multiset{v34[448]};
		r0 := v34[448] + (if (p1 in v39) then v39[p1] else 846);
		var v40 := DC45(fm19(globalState));
		r1 := match v40 {
			case DC45(cf87) => v39 - v39[v34[448] := p1]
		};
	}
	method m1(globalState: GlobalState) {
		...
		var v5 := DC29(false, v3, v3, v4[|v4|]);
		var v6: set<char> := {fm17(globalState)};
		v5 := DC29(f24, v3 - v3, v3, |v6|);
		globalState.f2 := (v3 % v3) / (if (false) then v3 else -v3);
		for i2 := v3 to if (f24) then v3 else v3 {
			var v7 := new C1();
			var v8: map<bool, int> := map[f24 := v3];
			var v9: set<bool> := {f24, (if (v2[504] in v8) then v8[v2[504]] else v3) >= i2, f24};
			v9 := v9 + v9;
			var v10: map<seq<set<bool>>, int> := map[seq(-431, i3  => (v9)) := 507];
	...
	method m8(p0: int, p1: map<int, int>, globalState: GlobalState) returns (r0: bool, r1: array<char>) {
		...
		var v7 := "shmkpwpai";
		var v8 := DC2(f24);
		var v9: array<bool> := new bool[28] [f24 ==> f24, f24 && f24, f24, f24, p0 == -p0, f24, v1 != v1, f24, f24, f24, f24, p0 in v1, f24, !f24, f24, v6[p0], f24, v7 != v7, fm17(globalState) !in "ibrmonla", v8.cf2, f24 && !f24, !f24, f24, f24, f24, f24, f24, f24];
		v9[948] := f24 && v6[711];
		v9[948] := f24;
		for i0 := p0 to -(p0 - p0) {
			var v10: seq<int> := [p0, i0];
			match (fm48(DC51(i0, v10[p0], v7, i0), globalState)) {
				case DC32(cf61, cf62, cf63) =>
					var v11: array<array<D11>> := new array<D11>[20];
					var v12: array<D11> := new D11[3];
					v11[872] := v12;
					var v13: map<seq<int>, int> := map[[|v10|, 0x31a] := cf61];
					var v14: array<int> := new int[14] [cf61, (if (i0 in p1) then p1[i0] else 0x105) / (if (v10 in v13) then v13[v10] else |v10|), |"wgvu"|, |{cf62, cf63, cf63, v9[948]}|, cf61, p0, |v6[p0 := true]|, fm3(i0, globalState), cf61, i0, |(fm41(v2, v9[948], globalState) + v0)|, |v6|, p0, |v6|];
					v14[900] := i0;
					var v15 := DC29(cf63, i0, i0, i0);
					var v16: seq<D11> := [v15];
					v11[872], v14[900], cf63, v9[948] := v12, i0, (v15 in v16) <==> f24, !(!v9[948] && !f24);
					v14[900] := v14[900];
				...
				case DC31(cf60) =>
					...
					var v19: seq<string> := [v7, v7, seq(0x2fd, i1  => (v2)), seq(0x115, i2  => (v2)), "omffohc"];
					var v20: T0 := new C2();
					var v21: seq<T0> := [v20];
					globalState.f2 := |v18[[v9[948], f24, f24] + ([DC13(v7, v9[948], v2, v9[948]).cf22])[|fm0(i0, v19[p0], |v6|, globalState)| := !!v9[948]] := |v21|]|;
					var v22 := DC0(v2);
					v9[948], v6 := v9[948], if (v20.fm2(map[i0 := v22], i0, -p0, globalState)) then v6 + v6 else v6 + v6;
					f24 := v9[948];
			}
			...
			var v28: map<int, array<int>> := map[v23.fm3(p0, globalState) := v27[p0]];
			v24[867] := DC53(|v28|);
			...
			match (v31) {
				case DC27(cf51, cf52) =>
					...
					var v35: array<map<seq<int>, char>> := new map<seq<int>, char>[19] [v33 + v33, v33, map[v10 := v2] + v33, map[seq(0x20d, i4  => (|p1|)) := v2] + v33, v33, v33, v33, v33, v33, v33, v33, v33, map[v10 := v2], v33, v33, v33, map[[p0] := v2] + v33, v33 + map[[p0, -cf52] := v2], map v34 : seq<int> | v34 in map[[i0] := p0] :: (v34) := (v2)];
					v35[624] := v33;
					r0, v32, cf52, v35[624] := v10 == v10, map[cf51 := f24 <==> false], cf52, (v33 + v33)[v10 + v10 := v2];
				case DC28(cf53, cf54) =>
					v26[350] := p0;
					var v36 := DC38(i0, p0, |{p0, |(seq(0xbe, i5  => (0x11c)))|, -i0}|);
					var v37: multiset<int> := multiset{v36.cf75};
					f24, v26[350], cf53 := i0 < |fm18(i0, true, f24, v37, globalState)|, |v10|, i0 <= p0;
					cf54 := v9[948];
					var v38 := new C1();
					var v39: seq<map<bool, bool>> := [fm19(globalState), map[cf54 := v9[948]]];
					var v40: map<int, D7> := map[0xa := DC16(cf54, |v39|, cf53, i0)];
					var v41 := DC16(v9[948], 0x127, v9[948], 0xc5);
					v40 := v40[p0 := v41] + v40;
				case DC29(cf55, cf56, cf57, cf58) =>
					...
					globalState.f2, v43, v26[13], v26[620], v9[948] := |(v7 + [fm27(cf55, false, cf57, globalState)])|, if ((cf55 <== false) in v44) then v44[cf55 <== false] else v43, v45.cf37, i0, v6[cf56 * p0];
				case DC26(cf50) =>
					globalState.f2 := p0;
				...
				case DC30(cf59) =>
					...
					var v52: map<multiset<int>, seq<int>> := map[v51 := seq(0x101, i6  => (DC16(v9[948], p0, v9[948], p0).cf30))];
			...
			decreases 100 - i7
		{
			...
			var v55: map<bool, bool> := map[v54.cf27 := v9[948]];
			globalState.f2 := |v55|;
			var v56: seq<array<bool>> := [v9, v9];
			var v57 := DC13(fm28(|v56|, globalState), v9[948], v2, v9[948]);
			var v58: seq<int> := [p0 % 0x33b];
			r0, v9[948], v57, globalState.f2, v9[948] := v9[948], v9[948], v57, |v58[p0 := p0]|, !v9[948];
		}
		var v59 := new C1();
		var v61: map<bool, bool> := map[v9[948] := false];
		var v62 := DC51(|(map v60 : int | v60 in map[--|v61| := p0] :: (v60 - |v6|) := (v6))|, |v7|, v7, 0x273);
		for i8 := p0 to p0 * v62.cf104 {
			globalState.f2 := i8;
			var v63 := DC16(p0 > i8, 0x2c5 * i8, v9[948], i8 * |v7|);
			v63 := v63;
			...
			if ({v9, v9} >= v64) {
				...
				var v67 := DC9(v66, v9[948], -846);
				var v68 := DC25(false);
				var v69 := DC6(v9);
				var v70 := DC43(v69, |(seq(-0x3b, i9  => (p0)))|, f24, v9[948]);
				var v71: map<char, bool> := map[fm27(v67.cf15, f24, p0, globalState) := v68.cf49 <== v70.cf84];
				var v72: T0 := new C3();
				var v73: set<T0> := {v72, v72};
				v71 := v71[fm30(p0, |v73|, f24, v9[948], globalState) := f24 <== false];
				var v74: map<bool, int> := map[f24 := p0];
				var v75: seq<int> := [|v74|];
				var v76: multiset<C1> := multiset{v59};
				var v77 := DC56(p1);
				var v78 := DC13(v7, v9[948], v2, v9[948]);
				var v79: array<map<int, int>> := new map<int, int>[21] [map[|v75| := p0] + (p1[i8 := 0xb5])[|fm15(globalState)| := 0x21d], (map[p0 := -|v76|])[|v74| := i8], map[p0 := p0] + p1, (v77.(cf110 := p1)).cf110, p1, p1 + p1, p1, map[p0 := 698], fm24(p0, v78, i8, globalState) + map[|v7| := 0x2ad], p1, p1, p1, p1, p1 + p1, p1[p0 := 926], p1, p1 + map[p0 := p0], map[|v6| := i8] + p1, fm18(i8, true, v9[948], v65, globalState) + map[0x202 := p0], map[p0 := i8], p1 + fm24(i8, v78, |v7|, globalState)];
				v79[584] := p1;
				v79[584] := p1;
				var v80: array<char> := new char[8];
				v80[978] := 'j';
				v80[978] := if (f24) then v2 else v2;
			} else {
				var v81 := DC30(fm49(p0, globalState));
				var v82: map<D11, bool> := map[v81 := false];
				globalState.f2 := |v82| - p0;
				var v83: map<bool, int> := map[v9[948] := p0];
				v83 := v83[f24 := --|v6| * i8];
				var v84 := DC54(f24, v9[948], p0);
				var v85: seq<D20> := [v84, v84, DC54(v9[948], f24, |map[i8 := v9[948]]|), v84];
				var v86: seq<int> := [-|v85| + i8];
				globalState.f2 := |v86|;
				v9[948] := v9[948];
				var v87: multiset<int> := multiset{i8, p0};
				var v88: map<int, char> := map[p0 := v2];
				var v89: set<bool> := {v9[948], f24};
				var v90: seq<array<bool>> := [v9, v9];
				var v91: array<int> := new int[24] [-i8, p0, i8, 108, p0, i8, p0, |(v87[p0 := i8])[|v61| := |v7|]|, i8, |multiset{i8, p0, i8, 0x299}|, i8, |DC8(p0, |v7|, v9[948], v88, v7).cf13| / p0, i8 % -|v6|, |((seq(0x1a9, i10  => (i8))) + v86)|, |v89| * i8, p0, p0, p0, if (v9[948] in v83) then v83[v9[948]] else p0, |v90|, p0 + i8, i8, i8, i8];
				v91[364] := i8;
				v91[364] := |(multiset{v9[948]} * (multiset(v6) + v0))|;
			}
...
class C5 extends T1 {
	...
	method m2(p0: bool, p1: int, p2: int, globalState: GlobalState) returns (r0: array<int>, r1: bool, r2: seq<bool>) {
		...
		for i0 := |v0| % p1 to p1 {
			...
			var v9: seq<set<bool>> := [{p0, p0, p0, p0}, {false}, v8[p1], {p0, p0, p0, p0}, {!p0, p0}];
			var v10: multiset<set<bool>> := multiset{v9[|v4|]};
			...
			if (p0) {
				...
				var v16 := DC73(v15);
				var v17: multiset<int> := multiset{|v16.cf135|, |(seq(-0x2b, i1  => (p2)))|, p2, i0, |v12|};
				var v18 := 'b';
				...
				v21, globalState.f2 := v22[p1 / p1], i0;
				...
				v5[297] := if ((true <== p0) in v29) then v29[true <== p0] else if (p0) then v26 else v26;
				...
				var v34 := DC57(v33);
				v32[887] := map[p1 := v34.(cf111 := v33)];
				var v35: map<int, D22> := map[p1 := v34.(cf111 := v33)];
				v32[887] := v35 + (map[i0 := v34] + v35);
				...
				var v44 := DC8(|v42|, i0, p0, v43, v5[297]);
				...
				var v48 := DC71(v47);
				var v49: map<set<set<int>>, D26> := map[v40[fm3(p2, globalState)] := v48];
				v49 := v49[v37 - v37 := v48];
			}
		...
		v53[239] := v54;
		var v55 := 'k';
		globalState.f2, v52, globalState.f2, v53[239] := fm3(|v0| * p2, globalState), v52, if (p0) then |v0| % p1 else -|map[v55 := -p2]| + p2, v54;
		var v56 := new C3();
		var v57 := "lyspocqf";
		...
		r1 := p1 != p1;
		var v61 := DC0(v55);
		r2 := if (p0) then ([v56.fm2(map[-0x252 := v61], p1, p1, globalState)])[p1 := p0] else ([p0])[p1 := p0];
	}
	method m3(p0: seq<bool>, p1: string, p2: int, p3: bool, globalState: GlobalState) {
		var i0 := 0;
		while (p3)
			decreases 100 - i0
		{
			...
			var v2: array<D18> := new D18[8] [v0, DC47(v1, p3, p2), v0, v0.(cf90 := p3), v0, DC47(v1, p3, p2), v0, if (p3) then DC47(seq(0xed, i1  => (|[v1]|)), p3, p2) else v0];
			v2[112] := v0;
			var v3 := 'h';
			var v4: seq<string> := [p1];
			v2[112] := v0.(cf90 := v3 !in v4[p2]);
			var v5: set<bool> := {p3};
			globalState.f2 := |v5|;
			var v6 := false;
			...
			v6 := v8 > v8;
			globalState.f2 := -((p2 / 0x273) * fm3(fm3(p2, globalState), globalState));
		}
		...
		if (match v18 {
			...
			v9[856] := fm2(v22, p2, -395, globalState);
			var v23: set<string> := {"yvqj"};
			v9[856] := v23 <= (set v24 : string | v24 in v23 :: (v24));
			var v25: map<int, bool> := map[p2 := p3];
			var v26: map<int, map<int, bool>> := map[p2 := v25];
			var v28: seq<map<int, bool>> := [v25];
			var v29 := DC36(map v27 : int | v27 in v28[|v25|] :: (v27 - p2) := (v25));
			var v30: multiset<bool> := multiset{p3};
			var v31: map<multiset<bool>, seq<bool>> := map[v30 := p0];
			var v32: map<int, int> := map[p2 := |v31|];
			var v33: set<bool> := {v9[856], p3, true, p3};
			var v34: array<D14> := new D14[20] [DC36(v26), v29, v29, DC36(v26).(cf70 := v26), v29, v29, v29, v29, v29, v29, v29, v29, v29, v29, v29, v29, v29, fm70(p3, ([|v32|])[p2 := p2], false, v33, globalState), v29, v29];
			v34[949] := v29;
			var v35: array<bool> := new bool[27](i5 => v9[856]);
			var v36: set<array<bool>> := {v35, v35, v35};
			var v37 := DC70(v9[856], v25, p2, v9[856], |p1|);
			var v38 := DC42(|map[p2 := p2]|, p2);
			var v39: set<map<D16, bool>> := {map[v38 := false]};
			v9[856], v34[949], v18, v9[856], v9[856] := fm2(v22, p2, |v36|, globalState), DC36((v26[0x228 := v25])[p2 := v25]).(cf70 := v26), v18, !v37.cf128, v39 == v39;
			var v40: map<map<bool, bool>, int> := map[v13 := fm3(p2, globalState)];
			var v41 := DC24(v40);
			match (v41) {
				case DC25(cf49) =>
					var v42 := "plcqtj";
					v42 := if (v9[856]) then p1 else v42;
					v9[856] := p3 && p3;
					var v43: map<int, char> := map[p2 := v16];
					var v44 := new C4(true, if (false) then DC8(|v14[p2 := p2]|, p2, cf49, v43, [v16, v16]) else DC8(p2, if (cf49 in v30) then v30[cf49] else p2, false, v43, seq(-0x185, i6  => (v16))));
					globalState.f1 := v16;
				case DC24(cf48) =>
					globalState.f2 := (p2 + p2) + p2;
					v9[856] := v9[856];
					var v46: map<int, char> := map[|(set v45 : int | (0x377 <= v45) && (v45 < 0x1c3) :: (v45 % 208))| := v16];
					var v47: map<char, map<bool, char>> := map[v16 := map[!p3 := v16]];
					var v48: map<bool, char> := map[p3 := 'w'];
					v46 := v46[0x266 := p1[|(if (fm68(p2, globalState) in v47) then v47[fm68(p2, globalState)] else v48)|]];
					v32 := v32[p2 / p2 := -0x1f0];
			}
	...
	method m0(p0: seq<int>, p1: int, p2: string, p3: bool, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
		...
		v0, v0, globalState.f5, v0 := p3, !(v4.cf13 < (("j")[fm3(|v5|, globalState) := v2])[-0x3ab := v2]), v6, fm2(v7 + v9[p1 := v8], -p1, p1 + p1, globalState);
	...
	method m1(globalState: GlobalState) {
			...
			decreases 100 - i0
		{
			...
			if (v0) {
				...
				var v6: set<bool> := {v0};
				var v7: array<array<int>> := new array<int>[27] [v3, v3, v3, v3, v3, v3, v3, v3, if (v0) then v3 else v3, v3, v3, v3, v3, v3, DC9(v3, v0, v4).cf14, v3, v3, v3, v3, v3, v3, v3, v3, v3, v3, v5[|v6|], v3];
				v7[976] := v3;
				v7[976] := v3;
				v0 := v0;
				var v8 := "etvbwfn";
				...
				var v11: array<bool> := new bool[14] [v0, v0, v1.fm2(v10, |(seq(498, i1  => ('r')))|, v4, globalState), v0, v0, v0, v0, v0, v0, v0, v0, v0, v0, v0];
				...
				v19, v22, v23 := {"wgiwpcpu" + "eqnfdp", v18}, (if (v0) then set v24 : int | (-0x1df <= v24) && (v24 < 0x332) :: (v24 % v21) else {v21}) * v22, v23.(cf2 := v28 != v28);
				v0 := v0;
				...
				var v35: set<array<int>> := {v33[921], v32, v32, v32, v34.cf14};
		...
		for i5 := v49 % |v50| to |v51| {
			var v52: map<bool, int> := map[v0 := fm3(i5, globalState)];
			var v53: array<bool> := new bool[14];
			var v54 := DC6(v53);
			var v55 := DC0(v48);
			var v56: seq<bool> := [fm2(map[|(seq(0xde, i6  => (v48)))| := v55], v49, v49, globalState), v0];
			var v57 := DC54(v0, v0, i5);
			var v58: seq<D20> := [v57, v57];
			var v59: seq<seq<D20>> := [v58, v58];
			var v60: set<bool> := {v0};
			var v61: array<int> := new int[22] [|(v52 + map[DC43(v54, i5, v0, v0).cf84 := i5])|, 248, if (v0) then i5 else |map[i5 := v53]|, v49, |map[|v56| := v49]| - v49, v49 + v49, 259, i5, i5, v49, 0x262 + |v50|, v49 * v49, |(v59[0x295] + v58)|, 0x2fc, v49, -i5, i5 * v49, v49, v49 / i5, i5, |v60| / i5, v49];
			v61[793] := v49;
			var v62: seq<int> := [v49, v49, |multiset{v52}|];
			globalState.f2, v61[793] := (if (v0) then v49 else i5) - -|v62|, i5;
			v0 := v60 >= (v60 + v60);
			v53[977] := v0;
			v61[793], v0, v0, v53[977], v0 := v49, v0, v0, v0, v0;
			var v63: multiset<bool> := multiset{v0};
			var v64: set<int> := {v49, if (v0 in v63) then v63[v0] else v49, v49};
			var v65 := DC17(v53[977], v49, v0, v0);
			var v66: map<set<int>, string> := map[v64 := fm72(i5, v53[977], v65, v53[977], globalState)];
			v66 := v66[{v61[793], |(fm73(v0, v61[793], v53[977], v49, globalState)).cf50|, v49, |v50|} := v50];
		}
		var v67: seq<string> := [seq(0x282, i7  => (v50[v49])), seq(0xbe, i8  => (v48))];
		var v68: map<bool, bool> := map[if (-|v67[v49]| in v51) then v51[-|v67[v49]|] else v0 := v0];
		if (if (v0 in v68) then v68[v0] else v0 <== !v0) {
			v49 := v49;
			var v69: seq<int> := [v49];
			var v70: set<int> := {|v69|};
			globalState.f2 := |(v70 + (set v71 : int | (0x115 <= v71) && (v71 < 603) :: (v71 + v49)))| / (v49 - v49);
			...
			v73[238] := v0;
			var v74: multiset<int> := multiset{v72[803], 0x2c3};
			globalState.f1, v72[803], v73[238], globalState.f5, v0 := v48, |v74|, v0, v73, v49 == v49;
		} else {
			globalState.f2 := v49;
			v0 := v0;
		...
		if (v0) {
			...
			v78[899] := |fm72(|(if (v79.cf120) then map[v0 := v49] else map[v0 := v49])|, v0, v82, !false, globalState)|;
			v78[899] := v78[899] * v49;
			var v83 := new C2();
			v0 := v0;
			v0 := v0;
			...
			v84[277] := v0;
			v84[277] := v49 > (v49 * |"epvp"|);
			var v85: array<multiset<int>> := new multiset<int>[3](i9 => multiset([DC64(|multiset(seq(-0x1c5, i10  => (v49)))|, v84[277], v0).cf121]) * multiset([|multiset{v49, v49}|]));
			var v86: seq<int> := [|[v84[277]]|, -v49, v49, -0x3a0];
			v85[424] := multiset{v86[v49]};
			var v87: array<int> := new int[20];
			var v88 := DC9(v87, v0, |v51|);
			var v89: multiset<int> := multiset{v49};
			v84[277], globalState.f2, v84[277], v49, v85[424] := v84[277] ==> v88.cf15, fm3(v49, globalState) % v49, v48 in "jb", fm3(v49, globalState), v89;
			var v90: multiset<bool> := multiset{v0};
			v49 := |(v90 - v90[false := v49])|;
			globalState.f6 := v87;
			v67 := v67[if (v84[277]) then -|fm74(654, v49, |multiset{v84[277]}|, globalState)| else v49 := if (v0) then v50 else seq(0x1d0, i11  => (v48))];
		}
...
class C6 extends T1 {
	...
	function fm3(p0: int, globalState: GlobalState): int {
		-(|[f26]| / |(seq(0x31f, i0  => ('k')))|) % (0x36d / |[f26, false]|)
	}
	method m2(p0: bool, p1: int, p2: int, globalState: GlobalState) returns (r0: array<int>, r1: bool, r2: seq<bool>) {
		var v0: seq<bool> := [p0];
		...
		v1[346] := if (true) then f26 else true;
		v1[346] := p2 < 992;
		...
		var v8 := "qd";
		var v9 := DC54(v1[346], f26, |v0|);
		var v10 := 'b';
		var v11: array<string> := new seq<char>[8] [seq(0x1b6, i1  => ('g')), v8, fm63(v9.cf107, p1, v2.f21, |v8|, globalState), v8[p2 := v10], v8, v8, v8, "kefim"];
		var v12 := DC12(v11);
		var v13 := DC14(v12);
		var v14 := DC14(v13);
		...
		v18[627] := v7;
		var v19 := DC1(seq(0x32b, i2  => (v2.f22)));
		var v20 := DC58(fm3(v2.f22, globalState), v19);
		f26, v1[346], v5, v18[627] := (if (v2.f21) then p1 else p1) <= v2.f22, match v20 {
			case DC58(cf112, cf113) => v0[v2.f22]
			case DC59() => !!v0[p2]
			case DC57(cf111) => p2 >= v2.f22
		}, DC37(v0[v2.f22 := f26], f26, v1[346]), v7;
		var v21: array<seq<array<char>>> := new seq<array<char>>[21];
		var v22: multiset<int> := multiset{p2};
		var v23: array<char> := new char[23] [v10, v10, v10, v10, v10, 'b', v10, v10, fm64(p2, |v22|, v1[346], v2.f22, globalState), v10, v10, v10, v10, v10, v10, v10, 'c', v10, v10, v10, 'e', v10, v10];
		var v24: seq<array<char>> := [v23, v23];
		v21[508] := v24;
		v21[508] := DC69(v24).cf127[p1 := v23];
		var v25: map<int, char> := map[|v0| := v10];
		var v26 := DC8(v2.f22, v2.f22, f26, v25, v8);
		var v27 := new C4(!v1[346], v26);
		v10 := 'v';
	...
	method m3(p0: seq<bool>, p1: string, p2: int, p3: bool, globalState: GlobalState) {
		...
		var v4 := DC8(|p1[p2 := 't']|, |v0|, p3, v2, v3.cf13);
		...
		globalState.f2 := match v8 {
			...
			case DC48(cf92, cf93, cf94) => |(seq(0xa6, i0  => (v1)))| * p2
	...
	method m0(p0: seq<int>, p1: int, p2: string, p3: bool, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
		...
		v0[147] := |p2|;
		f26 := p3 ==> p3;
		globalState.f2 := fm3(|p2| % p1, globalState);
		var i0 := 0;
		while (p3)
			decreases 100 - i0
		{
			...
			var v8: multiset<array<bool>> := multiset{v7[p1], v6};
			...
			match (v10) {
				case DC27(cf51, cf52) =>
					...
					v1 := fm63(cf51, p1, f26, p0[if (DC23(v11) in v13) then v13[DC23(v11)] else -0x76], globalState);
					f26 := f26;
					...
					var v20: array<map<int, D5>> := new map<int, D5>[8] [map[0x106 := DC11(p3, f26)], v16 + v16[v0[147] := v15], map[fm3(-|v8|, globalState) := v15], v16, fm66(v0[147], v17, p1, v3, globalState), v16, map[|v18| := v15] + (if (true in v19) then v19[true] else v16), v16[v0[147] := v15]];
					v20[100] := v16 + map[|[p1, p1]| := DC11(cf51, f26)];
					var v21: array<seq<bool>> := new seq<bool>[26];
					var v22: seq<array<seq<bool>>> := [v21];
					v14[608], v20[100], f26, f26, v21 := p0, v16, cf51, cf51, if (f26) then v22[-272] else v21;
					var v23: seq<bool> := [cf51, cf51, f26, cf51, p3];
					var v24: map<int, seq<bool>> := map[cf52 := v23];
					v0[147] := |(if ((-p1 % v0[147]) in v24) then v24[-p1 % v0[147]] else fm65(f26, globalState) + v23)|;
				case DC28(cf53, cf54) =>
					globalState.f2 := |v1|;
					cf54 := p3;
					var v25: C2 := new C2();
					v25 := v25;
					globalState.f2 := v0[147];
				case DC29(cf55, cf56, cf57, cf58) =>
					var v26: array<string> := new string[13](i3 => "wehje");
					v26[495] := p2;
					v26[495] := seq(0x3bf, i4  => (v3));
					r0 := p1;
					cf55 := cf55 <== p3;
					var v27: array<seq<int>> := new seq<int>[12];
					v27[620] := p0 + (seq(80, i5  => (p1)));
					v27[620] := p0 + (seq(0x22c, i6  => (p1)));
				case DC26(cf50) =>
					var v28: T1 := new C5();
					globalState.f2, v1, v28 := v0[147] % v0[147], p2, v28;
					f26 := p3;
					f26 := v1 == p2;
					v3 := v3;
				case DC30(cf59) =>
		...
		for i7 := p1 to p1 {
			if (p1 != p1) {
				...
				v0[147], f26, v34, v35[977], f26 := if (false) then i7 else |v36|, v37 >= v37, v34, v0[147] != (|v36| % -0x13a), f26;
				var v38: C1 := new C1();
				var v39 := DC0(p2[v0[147]]);
				var v40: map<int, D0> := map[p1 := v39];
				var v41 := 'r';
				v38, v35[977], globalState.f1 := v38, v38.fm2(if (p3) then map[v0[147] := v39] else v40, v0[147], if (false) then i7 else i7, globalState), v41;
				globalState.f2 := v0[147];
				...
				var v47 := DC0(v44);
				var v48 := DC53(if (f26 in v46) then v46[f26] else v0[147]);
				var v49: seq<map<int, D0>> := [map[i7 := v47], map[v0[147] := v47]];
				var v50: array<bool> := new bool[21] [p3, f26, f26, p3, f26, p3, f26, f26, !p3, (if (|fm75(v0[147], globalState)| in v45) then v45[|fm75(v0[147], globalState)|] else v44) !in p2, p3, v0[147] != (if (f26 in v46) then v46[f26] else |p2|), p3, true, f26, fm2(map[p1 := v47], p1, v48.cf106, globalState), p3, true, fm2(v49[v0[147]], |p2|, v0[147], globalState), v44 !in "ykxddirv", f26];
				var v51: map<int, int> := map[v0[147] := p1];
				var v52: map<int, bool> := map[p1 := true];
				var v53: T0 := new C2();
				var v54: map<int, T0> := map[v0[147] := v53];
				v50[127] := DC16(p3, if (|v52| in v51) then v51[|v52|] else |v54|, f26, v0[147]).cf29;
				var v55: map<bool, bool> := map[f26 := p3];
				v50[127] := if (!f26 in v55) then v55[!f26] else f26;
				r0 := 750;
				var v56: set<int> := {v53.fm3(i7, globalState)};
				m10(v56, globalState);
				var v57: map<int, D0> := map[v53.fm3(v0[147], globalState) := v47];
				var v58: set<bool> := {f26, true, f26};
				f26 := v53.fm2(v57 + v57, if (v50[127]) then v0[147] else p1, |v58| - v0[147], globalState);
			...
			var v62 := DC58(i7, v61);
			var v63: set<int> := {v0[147]};
			v59[185], v0[147], v62 := fm69(p1, globalState), p0[|v63| + v0[147]], DC58(v0[147], DC1(p0)).(cf113 := v61);
	...
	method m1(globalState: GlobalState) {
		...
		var v10: map<int, int> := map[v2 % v2 := v2];
		v10 := v10[975 := v2];
		var i0 := 0;
			...
			decreases 100 - i0
		{
			...
			if (!(if (if (v9.f24) then v9.f24 else v9.f24) then v9.f24 else v2 > v2)) {
				...
				v9.f24, v12[275], globalState.f1, v9.f24 := v9.f24, v0[|(v13 + v13)|], v6, if (f26) then v9.f24 else true;
				var v14: array<int> := new int[11](i1 => i1 + v2);
				v14[432] := v2;
				v14[432] := -f27.cf4;
				var v15: map<map<string, int>, bool> := map[map[seq(-837, i2  => (v12[275])) := v14[432]] := --v14[432] > |[v9.f24, !true, v9.f24]|];
				...
				var v22, v23 := v9.m4(v19.f22, v21 + v21, -v2, v17[33] in v18, globalState);
			}
		...
		v27 := v27[|(v28 + (multiset{-|[f26]|})[|v29| := v2])| := v9.f24];
	}
	method m10(p0: set<int>, globalState: GlobalState) {
		var v0 := 0x20c;
		globalState.f2 := fm3(fm3(v0, globalState), globalState);
		var v1: seq<int> := [|p0|];
		var v3: seq<set<int>> := [p0, p0, set v2 : int | (0x11a <= v2) && (v2 < -0x26d) :: (v2 % v0), p0, p0];
		var v4: C5 := new C5();
		var v5: map<bool, C5> := map[f26 := v4];
		for i0 := v1[|v3[v0]|] to |v5| / v0 {
		...
		if (f26) {
			...
			var v21 := "usnudcrhs";
			var v22: array<int> := new int[6] [v0, |map[f26 := v10[v4.fm3(fm3(|v20|, globalState), globalState)]]|, |p0| - 0xdb, fm3(v0, globalState), |v21|, v0];
			v22[913] := v0;
			v22[913] := v0 / (v0 / |{v4.fm2(v12, -v0, v0, globalState)}|);
			f26 := f26;
			var v23: map<int, int> := map[v22[913] := v0];
			var v24: set<bool> := {v4.fm2(v12, |v23|, v22[913], globalState)};
			f26 := (if (f26) then v24 else {f26}) == v24;
		} else {
			var v25 := new C5();
			var v26 := "iwawautd";
			v10, v26, globalState.f2, globalState.f2, globalState.f2 := v10 + v10, v26, v4.fm3(v0 % v0, globalState), v0, v0;
			globalState.f2 := if (if (f26) then f26 else v10[v0]) then -v0 - v0 else v0;
			var v27: array<int> := new int[16](i1 => i1 / v0);
			var v28 := 'l';
			var v29 := DC13(v26, false, v28, f26);
			v27[826] := if (v29.cf24) then v0 else v0;
			v27[826] := v0;
			var v30: seq<array<int>> := [v27, v27, v27];
			v30 := v30 + v30;
		}
		
		var v31: array<int> := new int[3](i2 => i2 % v0);
		v31[505] := v0;
		v31[505] := v0;
		var v32 := "cbvmwrxwe";
		var v33: multiset<int> := multiset{-v0};
		for i3 := |v32| % |v33| to v31[505] {
			v14[821] := f26;
			var v34 := DC22(f26, -0x248, v31[505]);
			globalState.f2, v14[821], globalState.f2, v31[505], f26 := if (i3 < v31[505]) then v31[505] else v0, !v4.fm2(fm71(v0, f26, globalState), -(v31[505] * 0x2f0), |v32|, globalState), if (fm2(v12, v0, i3, globalState)) then v0 else v34.cf45, v0, f26;
			var v35: map<bool, int> := map[v4.fm3(v31[505], globalState) >= v31[505] := v0 % v0];
			v35 := v35;
			var v36: map<bool, bool> := map[f26 := v14[821]];
			v31[505], v0, f26 := i3, |v32|, if (false in v36) then v36[false] else f26;
			var v37 := DC2(f26);
			var v38 := DC4(v37);
			match (v38) {
				case DC3(cf3, cf4) =>
					var v39: T0 := new C2();
					var v40: map<T0, bool> := map[v39 := i3 != v31[505]];
					var v41: array<bool> := new bool[9] [v14[821], !!false, true, v39.fm2(map[v0 := v11], -129, cf4, globalState), !v14[821], true, cf3, cf3, cf3];
					var v42: map<int, array<bool>> := map[v0 := v41];
					v40 := v40[v39 := v10[|v42|]];
					var v43: map<int, int> := map[v0 := v31[505]];
					var v44: multiset<bool> := multiset{cf3, f26};
					var v45 := DC64(if (v0 in v43) then v43[v0] else |map[v14[821] := v0]|, v44 !! v44, f26);
					var v46 := 'j';
					var v47: map<int, char> := map[v31[505] := v46];
					var v48 := DC8(i3, i3, true, v47, v32);
					globalState.f2, v45, v32, globalState.f2, v35 := -|v48.cf13|, v45, v32[i3 := v46], |v10|, v35 + v35;
					globalState.f5 := v41;
					var v49: set<bool> := {cf3};
					v41 := new bool[22] [cf3, cf3, f26, v14[821] ==> f26, cf3, v49 !! v49, v14[821], !f26, f26, if (v14[821]) then v10[cf4] else f26, f26, v14[821], f26, if (cf3) then !!v39.fm2(v12, v31[505], v31[505], globalState) else cf3, cf3, !cf3, v14[821], -0x80 > 0x33e, v4.fm2(v12, |v33|, |v1|, globalState), cf3, f26, f26];
				case DC2(cf2) =>
					var v50: C0 := new C0(v14[821], -v31[505]);
					var v51 := DC7(v50);
					var v52 := DC16(fm2(v12, i3, v0, globalState), v50.f22, v50.f21, v50.f22);
					var v53: set<bool> := {v14[821]};
					var v54 := 'c';
					var v55: set<set<bool>> := {{f26}, {f26, f26}};
					globalState.f2, v51, f26, v52 := |([|v53|, v50.f22, v50.f22, -v0, |v1|] + v1)| % (0x2bf % v0), DC7(v50), (fm63(v14[821], 0x25, v50.f21, i3, globalState))[i3 := v54] != v32, DC16(v0 in v1, |v55|, f26, |v32|);
					v31[505] := |map[f26 := v50.f21 <== true]|;
					v31[505] := |v32|;
					...
					var v60: map<int, string> := map[v31[505] := v32];
					var v61: seq<string> := [seq(0x2e, i5  => (v54)), if (i3 in v60) then v60[i3] else v32];
					v56[457] := new bool[12] [f26 in v59, !false, f26 || v14[821], p0 > p0, (seq(414, i4  => (v54))) < v32, v50.f22 != v50.fm7(i3, v61, v50.f21, cf2, globalState), false, multiset(v1) <= v33, cf2, v14[821], cf2, if (v50.f21) then v50.f21 else !false];
				case DC4(cf5) =>
					v14[821] := f26;
					var v62 := new C2();
					var v63 := DC47(v1, f26, v31[505]);
					var v64: map<D18, int> := map[v63 := if (f26) then v0 else v31[505]];
					var v65: C0 := new C0(v14[821], v31[505]);
...
class C7 extends T1 {
	...
	method m2(p0: bool, p1: int, p2: int, globalState: GlobalState) returns (r0: array<int>, r1: bool, r2: seq<bool>) {
		...
		var v7: seq<seq<char>> := [[v3], seq(0x103, i1  => (v3))];
		var v8 := DC8(fm3(|(seq(0x225, i0  => (v3)))|, globalState), p2, p0, v6, v7[p2]);
		if ((if (p0) then v3 else v3) in (v4 + v8.cf13)) {
			var v9: array<bool> := new bool[3] [p0, p0, false];
			var v10 := DC3(true, p2);
			var v11: map<array<bool>, D2> := map[v9 := v10];
			v1[735] := |(v11 + v11)|;
			v1[735] := if (p0) then p1 else p1;
			globalState.f2 := p1 % -v1[735];
			...
			var v15: multiset<int> := multiset{v14.cf16, p2, |v0|, p2, -p2 / p1};
			...
			decreases 100 - i2
		{
			...
			i2 := i2 + 1;
			var v19 := new C2();
			v4 := ((if (p0) then v4 else v4) + (v4 + v4))[p2 := fm56(p0, globalState)];
		...
		if (!(p0 <== p0)) {
			...
			if (p0) {
				...
				var v33: set<int> := {if (p2 in v31) then v31[p2] else |v32[p1 := v5]|};
				r1, v33, globalState.f2 := (321 - p1) != p1, v33, p2;
				var v34 := DC61(fm57(globalState));
				v26[807] := fm2((v34.(cf115 := v28)).cf115[p1 := v27], p1 + p1, p2, globalState);
				v26[807] := false;
			...
			v7 := fm58(map[seq(0x3a0, i5  => (v3)) := |multiset(v38)|], v39 + v39, globalState);
			match (fm59(|[v1, v1]|, p0, globalState)) {
				case DC27(cf51, cf52) =>
					var v40: seq<bool> := [cf51];
					var v41: multiset<int> := multiset{p1};
					r2 := v40 + (v40 + v40)[|[cf52, |multiset{cf52, p1, p2}|, |v41|]| := p0];
				...
				case DC28(cf53, cf54) =>
					...
					var v48 := DC45(v47);
					var v49: map<int, map<bool, bool>> := map[|(if (v26[807]) then v4 else v7[-p1])| := v48.cf87];
					var v50: seq<C0> := [v46];
					var v51: multiset<bool> := multiset{v46.f21};
					var v52: multiset<int> := multiset{186, |v39|, |v51|, v46.f22};
					var v53 := DC3(cf53, p2);
					v45[692], v26[807], v49, v46.f22, cf53 := v50[-|(v52 - v52)|], v53.cf3, fm60(false, 0x126, globalState), -p1, cf53 ==> true;
					globalState.f2 := fm3(p2, globalState) / v46.f22;
					var v54 := DC29(cf53, -(v46.f22 + p1), v46.f22 % p2, v46.f22);
				...
				case DC29(cf55, cf56, cf57, cf58) =>
					...
					var v58 := DC26(v0);
					var v59 := DC29(v26[807], cf56, cf57, cf56);
					var v60: seq<set<bool>> := [v0, v0];
					var v61: array<set<bool>> := new set<bool>[21] [{v26[807], p0, v26[807], p0}, fm1(v4, cf55, cf55, globalState), {p0, true}, v0, v0, v0 + {cf55}, v0 + v0, v0 * v0, fm1(v4, cf55, cf55, globalState), v0 * v0, v0, v58.cf50 * v0, v0 * v0, v0, v0, v0, v0, {false, v26[807], v59.cf55}, {v57.fm2(v28, |v39|, -0x3ae, globalState)}, {v26[807], v26[807], cf55, cf55}, v60[cf56]];
					v61[989] := v0;
					v61[989] := v0;
				case DC26(cf50) =>
					r1 := p0;
					var v62: map<map<int, char>, bool> := map[v6 := v26[807]];
					var v64: seq<bool> := [p0];
					var v65: map<map<int, char>, seq<bool>> := map[map[p1 := 'b'] := v64];
					v62 := (map v63 : map<int, char> | v63 in v65 :: (v63) := (v26[807])) + map[v6 := v26[807]];
					var v66 := new C1();
					v26[807] := !true;
				case DC30(cf59) =>
					var v67: map<array<int>, bool> := map[v1 := v26[807]];
					var v68: map<array<bool>, bool> := map[v26 := if (v1 in v67) then v67[v1] else v26[807]];
					var v69: multiset<int> := multiset{p1, p1, p2};
					v26[807] := (if (v26 in v68) then v68[v26] else v26[807]) <==> (v69 <= v69);
					var v70: map<bool, int> := map[v26[807] := p2];
					v70 := v70[v26[807] := (if (false in v70) then v70[false] else 0x2e0) - p2];
					var v71: map<bool, bool> := map[false := v26[807]];
					v71 := v71[true := p0];
					v1[753] := |v5|;
					v1[753] := p1 * p2;
			}
			...
			v75[714] := p0;
			v75[714] := if (!(!p0 <== p0)) then p0 else p2 != p1;
			var v76: map<bool, int> := map[v75[714] := if (p1 in v5) then v5[p1] else p1];
			v76, globalState.f2 := v76, p1;
			var v77: C4 := new C4(false, v8);
			var v78: map<bool, C4> := map[p0 := v77];
			globalState.f2 := -(-(-p2 - -p2) % |(v78 + v78)|);
		}
			...
			decreases 100 - i6
		{
			...
			v80[638] := fm61(p2, p1, globalState);
			var v81: multiset<array<int>> := multiset{v1, v1};
			v80[638] := map[if (v1 in v81) then v81[v1] else p2 := p0];
			if (v3 !in (seq(-927, i7  => (v3)))) {
				var v82 := new C2();
				v80[638] := (v80[638][p2 := p0] + v80[638]) + v79;
				...
				var v87 := DC31(v86);
				v83[806], v85, v87, r1 := p1 == 0x163, v85, v87, p0;
				var v88 := new C1();
				r1 := p0;
			} else {
				r1 := p0;
	...
	method m3(p0: seq<bool>, p1: string, p2: int, p3: bool, globalState: GlobalState) {
		...
		var v2: set<int> := {p2};
		var v3: array<int> := new int[7] [|(v0 - v0)|, p2, p2, -(if (p3 in v1) then v1[p3] else p2), p2 / p2, p2, -|[v2, v2]| / p2];
		var v4: map<bool, bool> := map[false := p3];
		var v5 := 'r';
		var v6 := DC0(v5);
		var v7 := DC1(seq(0x314, i0  => (p2)));
		var v8: seq<int> := [p2, p2];
		var v9 := DC58(0x322, v7.(cf1 := v8));
		v3[484] := |((map[|v4| := [p3]])[p2 := p0[254 := p3]])[|[fm2(map[|v0| := v6], p2, v9.cf112, globalState)]| := p0]|;
		v3[484] := p2 + |p0|;
		var v10 := new C0(false <== p3, p2);
		forall i1 | 0 <= i1 < v3.Length {
			v3[i1] := i1 + (v3[484] + v10.f22);
		}
		var v11: map<char, bool> := map[v5 := v10.f21];
		var v12: map<int, map<char, bool>> := map[v3[484] := v11];
		var v13 := DC46(v12);
		...
		var v19: array<bool> := new bool[7] [v17[v3[484]] >= v16, true, p3, v18 !! v18, p0 != p0, p0 == p0, p3];
		v19[67] := !p3;
		v19[67] := p3;
		...
		v19[67] := match v21 {
			...
			case DC30(cf59) => v19[67]
		};
		var v22: map<int, char> := map[v3[484] := v5];
		var v23 := DC8(|p1|, v3[484], v10.f21, v22, p1);
		var v24: T2 := new C4(v10.f21, v23);
		v24 := v24;
	}
	method m0(p0: seq<int>, p1: int, p2: string, p3: bool, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
		var v0: map<int, int> := map[|(seq(0x13c, i1  => ('s')))| := p1];
		var v1: array<seq<int>> := new seq<int>[15] [p0, p0, p0, p0, fm0(p1, p2, p1, globalState) + p0, p0[p1 := p1], ([p1, p1])[p1 := |p2|], (if (false) then [p1, p1, -p1, p1, |v0[p1 := p1]|] else p0)[p1 := p1], p0 + p0, p0, p0, p0, p0, p0 + p0, p0];
		forall i0 | 0 <= i0 < v1.Length {
			v1[i0] := p0;
		}
		var v2 := 'j';
		...
		match (v5) {
			case DC62() =>
				...
				v6[36] := p3;
				v6[36] := p3;
				if (if (v6[36]) then v6[36] else fm2(v4, -fm3(277, globalState), |p2|, globalState)) {
					globalState.f2 := |p0|;
					...
					var v9: T1 := new C5();
					v8[846] := v9;
					v8[846] := if (v6[36]) then v9 else v9;
					var v10 := DC21(v6, v6[36], !DC27(v6[36], |(seq(0x2ab, i2  => (map[true := p1])))|).cf51, v2);
					var v11: seq<D8> := [v10];
					var v12: map<int, bool> := map[p1 := p3];
					var v14: map<int, seq<D8>> := map[|(seq(0x55, i3  => (map v13 : D11 | v13 in (seq(-0x247, i4  => (DC27(p3, p1)))) :: (v13) := (p1))))| := v11];
					var v15: seq<seq<D8>> := [v11, v11];
					var v16 := DC75([v10, DC21(v6, p3, v6[36], v2)]);
					var v17: array<seq<D8>> := new seq<D8>[18] [v11[|v12| := v10], v11, v11, v11, v11, v11, if (|p2| in v14) then v14[|p2|] else v11, v11, v11, v11, v11, v11, v15[0x150] + v11, ([v10, v10])[p1 := v10] + [v10, DC21(v6, v6[36], p3, 's')], v11, v11 + v11[p1 := v10], [v10.(cf42 := v6[36], cf41 := v6[36]).(cf43 := 'b', cf40 := v6), v10], v16.cf137 + [v10]];
					v17, r0, v6[36] := v17, |multiset(p0 + p0)|, p3;
					var v18: multiset<int> := multiset{p1};
					v6[36], v6[36] := !(v18 > v18), !((|(seq(640, i5  => (v2)))| + p1) != p1);
				} else {
					var v19: T1 := new C5();
					var v20: map<T1, bool> := map[v19 := p1 > p1];
					var v21: multiset<bool> := multiset{v6[36], v6[36]};
					var v22: map<bool, int> := map[p3 := -|v21|];
					var v23: seq<map<bool, int>> := [v22, v22];
					var v24: map<bool, map<bool, int>> := map[v6[36] := v23[453]];
					var v25: map<int, map<bool, int>> := map[if (v6[36]) then p1 else p1 := if (p3 in v24) then v24[p3] else v22];
					var v26: set<array<bool>> := {v6, v6, v6, v6};
					var v27: array<int> := new int[6];
					var v28 := DC9(v27, v6[36], p1);
					v6[36], v20, v25, globalState.f2 := |v26| == -(|v22| % v19.fm3(|p0|, globalState)), v20, v25, -(v28.(cf14 := v27)).cf16;
					globalState.f2 := |p2|;
					v6[36] := v6[36];
					v1[12] := [769];
					var v29: seq<seq<int>> := [p0];
					var v30: multiset<int> := multiset{p1, -0x29c, 0x2f3, |v22|, p1};
					var v31: seq<bool> := [v6[36], p3];
					var v32 := DC53(p1);
					v1[12], v6[36], v6[36], v6, globalState.f2 := ((v29[p1] + p0) + p0)[p1 := if (p3) then |v30[p1 := |v31|]| else 0x1be], p3 || (-p1 < 0x292), if (p3) then |(seq(0x44, i6  => ('s')))| >= p1 else v6[36], v6, v32.cf106;
					globalState.f6 := new int[5] [-353, p1, p1, p1, |p2| + |map[v6[36] := p1]|];
				}
				...
				if (!!p3) {
					...
					var v41: map<bool, bool> := map[v39 := v6[36]];
					var v42: map<int, map<bool, bool>> := map[p1 := v41];
					var v43: map<int, multiset<char>> := map[p0[|v40|] := multiset{p2[|v42|]}];
					...
					var v46: seq<bool> := [v6[36], p3];
					var v47 := new C0(v6[36], |v46|);
					var v48: multiset<char> := multiset{v2, v2, v2};
					globalState.f5, v6[36], v6[36], globalState.f2, v48 := v6, v47.f21, p3, p0[-v47.f22], multiset{v2};
					var v49: array<int> := new int[4](i7 => i7 * v47.f22);
					globalState.f6 := v49;
					var v50 := "xkcq";
					v50 := p2;
					var v51: array<map<bool, int>> := new map<bool, int>[18];
					var v52: map<array<map<bool, int>>, bool> := map[v51 := v6[36]];
					v52 := v52[v51 := true];
				}
				
				var v53: set<bool> := {v6[36]};
				var v54 := DC29(!p3, |v53|, p1, |p0|);
				var v55: map<D11, bool> := map[v54 := p3];
				var v56: array<int> := new int[6];
				var v57: seq<bool> := [v6[36]];
				v6[36], globalState.f1, v6[36] := if (v54 in v55) then v55[v54] else DC9(v56, v6[36], |v57|).cf15, v2, p3;
			case DC63(cf116, cf117, cf118, cf119, cf120) =>
				globalState.f2 := DC16(!false, cf119, cf120, 666).cf30;
				cf120 := false;
				var v58: seq<string> := ["yh", "qwed"];
				if (!(fm55(multiset(v58), cf120, 0x1d8, globalState) == (seq(0x372, i8  => (v2))))) {
					...
					v61 := v61 * (v61 + v61);
					var v62: array<int> := new int[1](i9 => i9 % p1);
					var v63: C1 := new C1();
					var v64: map<array<int>, C1> := map[v62 := v63];
					var v65: seq<bool> := [cf120, cf120];
					var v66: multiset<int> := multiset{p1};
					var v67: array<int> := new int[25] [p1, -|v64|, p1, v63.fm3(p1, globalState), p1, p1, cf119, 0x392, |multiset(v65[cf119 := p3])|, cf119, cf119, cf119, p1, cf119, p1, p1, |v66|, if (p1 in v0) then v0[p1] else cf119, |map[v63.fm2(v4, |v65|, |multiset{cf118}|, globalState) := v0]|, p1, 0xfb, 0x50, p1, cf119, cf119];
					var v68: seq<array<int>> := [v67, v67, v67, v67, v62];
					var v69: array<array<int>> := new array<int>[12] [v62, v67, v67, if (cf116) then v68[0x202] else v67, v67, v67, v67, v62, v62, v67, v62, v67];
					v69[244] := v67;
					v69[244] := v67;
					var v70 := new C2();
				...
				cf119 := (fm3(0x3d1, globalState) - p0[fm3(cf119, globalState)]) * cf119;
			case DC64(cf121, cf122, cf123) =>
				var v77: seq<bool> := [p3];
				var v78: array<int> := new int[2];
				v78[904] := p1;
				var v79 := DC42(|"d"|, cf121);
				v77, v78[904], globalState.f2 := v77, v79.cf80, cf121;
				cf122 := v78[904] > v78[904];
				var v80: map<int, char> := map[p1 := fm56(cf122, globalState)];
				var v81 := DC8(v78[904], v78[904], cf122, v80, p2);
				globalState.f2 := v81.cf10 / 0x1e9;
				globalState.f2 := fm3(cf121 / |p2|, globalState);
			case DC61(cf115) =>
				...
				v0 := v0[v82[198] := |v83|];
				var v84: set<bool> := {fm2(map[p1 := v3], v82[198], p1, globalState), p3, p3};
				var v85: set<int> := {|p2[p1 := v2]|};
				var v86: map<set<bool>, set<int>> := map[v84 := v85];
				v86 := v86[v84 := set v87 : int | (117 <= v87) && (v87 < -0x117) :: (v87 % p1)];
			...
			decreases 100 - i11
		{
			...
			var v104: array<string> := new string[27] [p2, (seq(-807, i12  => (v2))) + p2, seq(291, i13  => (v2)), "lot", p2, (seq(0x3b8, i14  => ('d'))) + p2, p2 + p2, seq(958, i15  => (v2)), p2, seq(0x2ee, i16  => ('x')), p2, p2, p2 + p2, v101.cf21, v102[-306] + "oeyfty", p2, p2, p2, p2, p2, p2, p2, p2 + (seq(646, i17  => (v2))), p2 + (if (p3 in v103) then v103[p3] else "raxa"), p2, p2, p2];
			v104[137] := p2;
			var v105: map<int, char> := map[p1 := v2];
			var v106: multiset<string> := multiset{"nvqelbhel", p2};
			var v108: map<map<int, char>, bool> := map[map v107 : int | v107 in p0 :: (v107 / |p2|) := (v2) := p3];
			v104[137] := DC8(p1, p1, p3, v105, fm55(v106, if (v105 in v108) then v108[v105] else v94, p1, globalState)).cf13;
			var v109: map<int, array<int>> := map[fm3(p1, globalState) := v95];
			v109 := v109[-p1 := v95];
		}
		var v110 := true;
		var v111: multiset<seq<int>> := multiset{seq(93, i18  => (0xd)), [|p2|], [0xa2], p0};
		var v112: multiset<char> := multiset{v2};
		v110 := v111 == fm79(-|v112|, v110, p1, globalState);
		v93 := v93 + fm75(|p2|, globalState);
		var v113: C5 := new C5();
		var v114: seq<C5> := [v113];
		if (!!!(p1 >= (|v114| % v113.fm3(p1, globalState)))) {
			var v115: multiset<int> := multiset{0x27c};
			var v116: array<int> := new int[3](i19 => i19 / 0x2e7);
			...
			match (if (v115 >= v115) then v117 else v117) {
				case DC8(cf9, cf10, cf11, cf12, cf13) =>
					...
					v118[900], v110, globalState.f2, v121 := v119 + v119, p3, cf10 % cf9, v121;
					var v122: array<bool> := new bool[23](i21 => v110);
					v122[227] := cf11;
					var v123 := DC22(v110, cf9, cf10);
					v122[227] := v2 !in (if (v123.cf44) then "mwooiam" else cf13);
					var v124: array<array<bool>> := new array<bool>[27];
					v0, v124, globalState.f2, globalState.f2 := map[p1 := p1] + (map v125 : int | (0xdc <= v125) && (v125 < 0x279) :: (v125 - |map[|map[v115 := v122[227]]| := cf11]|) := (cf9)), v124, cf9, cf9;
					cf10 := cf9;
				case DC9(cf14, cf15, cf16) =>
				...
				case DC7(cf8) =>
					...
					var v136: array<set<int>> := new set<int>[14] [v131, v132, v131, v132 + v131, v132, v131, {fm3(p1, globalState)} * fm78(true, globalState), v131 - (set v133 : int | v133 in p0[cf8.f22 := cf8.f22] :: (v133 / -0xf3)), v131, v131 * v132, v135[cf8.f22], v132 + v132, v132, v132];
					v136[141] := v132;
					...
					v136[141], r0, v137[186], v130.f26, v139 := v132, p1, fm56(v142.cf55, globalState), p3, map[v110 := v138];
			...
			if (fm3(p1, globalState) <= (p1 - p1)) {
				...
				var v146: map<int, bool> := map[|v145| / -p1 := v144.f26 <== !v110];
				...
				var v154: map<array<bool>, string> := map[v153 := p2[p1 := v2]];
				v151[7] := new int[9] [0x245, p1 - p1, |v152|, p1, -0x396, |(if (v110) then map[v153 := p2] else v154)|, -p1, p1, 886];
				var v155 := new C3();
			...
			if (v113.fm3(|(seq(0xbe, i25  => (p1)))|, globalState) > p1) {
				var v158 := DC48(p1, v110, false);
				var v159: C3 := new C3();
				var v160: seq<C3> := [v159];
				var v161: map<bool, seq<C3>> := map[v158.cf93 := v160];
				v161 := v161[v110 := v160];
				v113.m3([p3], p2 + (seq(0x3a1, i26  => (v2))), p1, p3, globalState);
				v110 := p2 < "e";
				r0 := p1 * (p0[-p1] / p1);
				...
				v167[513] := if (p3 in v168) then v168[p3] else v116;
				v167[513] := v116;
				v110 := p3 ==> p3;
				var v169 := DC62();
				v169 := v169;
				...
				v171[230] := v173;
				v171[230] := v173 * (multiset{v172, v172} + multiset{DC64(p1, v110, v110), v172});
			}
			...
			v180[69] := v110;
			var v182: seq<map<int, int>> := [v0, map v181 : int | (-923 <= v181) && (v181 < 0x27b) :: (v181 / p1) := (-v178.f22)];
			v180[927] := p3;
			v180[69], v182, v180[927] := p3, [v0[|v112| := p1], v0], (0x1e5 - |p2|) < v178.f22;
		}
		
		var v183: set<int> := {p1, -p1, p1, |"vbbvdyp"|};
		var v184: map<bool, int> := map[true := p0[p1]];
		var v185: set<set<int>> := {v183, {|v184|, p1}};
		r0 := (|p0| - v113.fm3(p1, globalState)) % |v185|;
	...
	method m1(globalState: GlobalState) {
		...
		var v2: map<array<int>, int> := map[v1 := |(seq(239, i0  => (|map[v0 := -|(seq(0x28b, i1  => ('s')))|]|)))|];
		v1[173] := |v2|;
		...
		var v6 := 0x2a4;
		var v7: map<bool, set<bool>> := map[v5[v6] := v4];
		...
		v0, v1[173], v3[939], v8[460], globalState.f2 := |(v11 + v11)| < |v12|, |v10| / v6, v0, {v0, if (v0) then v0 else v0, v6 <= v6, v0, v0}, fm3(v6, globalState);
		var v13: array<seq<bool>> := new seq<bool>[15](i2 => v5);
		v13[869] := v5;
		var v14: array<array<int>> := new array<int>[22];
		v1[173], v13[869], v14, v1[173] := v6, v5, v14, -(v1[173] % v1[173]);
		var v15: set<int> := {v6};
		var v16: seq<set<int>> := [v15];
		var v17 := DC5(v16[DC32(0x32e, v0, true).cf61] - v15);
		v17 := DC5(v15 - fm78(v3[939], globalState));
		var v18 := 'c';
		var v19: seq<char> := [v18];
		var v20 := DC8(v6, v1[173], v3[939], map[v1[173] := v18], if (v3[939]) then [v18, fm56(v3[939], globalState)] else v19);
		match (v20) {
			case DC8(cf9, cf10, cf11, cf12, cf13) =>
				var v21: map<int, bool> := map[-0x50 := cf11];
				var v22: map<int, int> := map[v1[173] := |v21|];
				v22 := map[|v4| := |v22|] + v22;
				var v23: seq<map<bool, int>> := [v11, map[v0 := cf10], v11, v11];
				var v24: array<map<bool, int>> := new map<bool, int>[22] [v11, v11, v11, v11[v0 := v1[173]], v11, v11, v11 + v11, v11, v23[0x63], v11[true := cf10] + v11, v11, v11, v11, v11, v11 + v11, v11 + v11, v11, v11, v11 + map[v3[939] := v1[173]], v11, v11, v11 + v11];
				v24[999] := v11;
				v24[999] := v11 + (v11 + v11);
				if (cf9 <= 646) {
					v13[869] := v5;
					globalState.f2 := cf9;
					var v25: map<int, array<bool>> := map[|(seq(-0x16f, i3  => (v19)))| := v3];
					var v26 := DC33(v25);
					var v27 := DC33(v26.cf64);
					var v28: set<string> := {cf13, cf13, v19};
					v26, cf11, cf10, globalState.f2, v3[939] := v26, v28 !! (v28 * v28), (cf10 % v6) * v6, cf10 / |v22|, v0;
					globalState.f2 := cf10;
					globalState.f6 := v1;
				} else {
					var v29: C0 := new C0(v3[939], cf9);
					var v30 := DC7(v29);
					v30 := DC7(v29);
					var v31: T2 := new C2();
					var v32: multiset<T2> := multiset{v31};
					v32 := v32;
					v19 := (cf13 + cf13)[|"ubmcut"| := v18];
					v1[173] := cf9;
					var v33: seq<string> := [cf13, v19];
					var v34: multiset<int> := multiset{v1[173], cf10};
					var v35: multiset<seq<int>> := multiset{[|[v0, cf11]|, cf10], v10};
					v1 := new int[29] [v31.fm3(cf10, globalState), v29.fm7(|v15|, v33, cf11, v3[939], globalState), v1[173] / 0x369, cf10, 0xbf * v29.fm7(cf10, seq(357, i4  => (v19)), v3[939], false, globalState), v31.fm3(--v6, globalState), cf10 + v29.f22, v29.f22, if (v29.f22 in v34) then v34[v29.f22] else cf10, cf10, cf10 % cf9, |{v1[173], |v19|, v29.f22}|, v6, v1[173] - cf9, v29.fm7(v29.f22, v33, v29.f21, v3[939], globalState), v6, |(if (cf11) then v19 else [v18])|, v29.f22, --v29.f22, cf10, if (false) then cf10 else 0x202, v29.f22, v6, |(v8[460] * v8[460])|, 332, v6, v1[173], |v19| - v31.fm3(fm3(|v10|, globalState), globalState), |v35|];
				}
				
				var v36 := DC63(v12 !! fm75(v6, globalState), v18, false <== v0, cf10, cf11);
				var v37 := DC66(v14);
				var v38: array<D25> := new D25[7] [v37, v37, v37, v37, v37, DC66(v14), v37];
				v36, v38, cf10 := v36, v38, (|v5| * cf9) / (cf10 / cf9);
			case DC9(cf14, cf15, cf16) =>
				var v39 := DC32(v6, cf15, v3[939]);
				globalState.f2 := v39.cf61;
				v5 := v13[869] + v5;
				var v40 := DC1([v6, cf16, v1[173]]);
			...
			case DC7(cf8) =>
				...
				match (v45) {
					case DC76(cf138, cf139) =>
						...
						var v54: seq<map<int, bool>> := [map v51 : int | (988 <= v51) && (v51 < 0x3c) :: (v51 - |(map v52 : int | (465 <= v52) && (v52 < -0x392) :: (v52 % cf8.f22) := (cf8.f21))|) := (cf8.f21), v53];
						var v55: map<int, string> := map[|v54| := v19];
						globalState.f1, v3[939], globalState.f2, cf139 := 'c', if (v48[|v49|] in v47) then v47[v48[|v49|]] else cf139, if (v3 in v50) then v50[v3] else |(if (|v19| in v55) then v55[|v19|] else v19)[v1[173] := v18]|, v13[869][cf8.f22];
						var v56: map<int, multiset<bool>> := map[cf8.f22 := multiset(v5)];
						var v57: map<bool, multiset<bool>> := map[cf8.f21 := if (v1[173] in v56) then v56[v1[173]] else fm75(v6, globalState)];
						var v58 := DC60(v12);
						v19, v3[939] := v19, !(multiset(v13[869]) <= (if (cf139 in v57) then v57[cf139] else v58.cf114));
					...
					case DC77(cf140, cf141, cf142, cf143, cf144) =>
						var v61: map<set<bool>, array<int>> := map[v8[460] := v1];
						v61 := v61 + v61;
						...
						v65 := v65[!cf8.f21 := if (true) then map v66 : multiset<int> | v66 in {v62[-cf8.f22 := cf8.f22]} :: (v66) := (0x33a) else v63];
						v3[939] := v0;
						cf144 := cf8.f22 == cf8.f22;
					case DC75(cf137) =>
						var v67: seq<array<bool>> := [v3];
						v1[173], globalState.f1, cf8.f22, cf8.f22 := -(v6 * |(v67 + v67)|), v18, if (DC37(v5, v3[939], cf8.f21).cf73) then v1[173] * cf8.f22 else v6 - v1[173], cf8.f22;
						globalState.f6 := v1;
						var v68 := DC32(v6, v0, v3[939]);
						var v69: map<D12, array<bool>> := map[v68 := v3];
						v69 := v69[v68 := v67[0x2bf]];
						var v70: multiset<int> := multiset{cf8.f22, -|v19|, |v13[869]|, cf8.f22, -v6};
						var v71: map<multiset<int>, bool> := map[v70 * v70 := v3[939]];
						v71 := v71[v70 := v5[|v8[460]|]];
				}
		...
		var v74: map<array<map<int, int>>, bool> := map[v73 := v3[939]];
		v3[939] := !v3[939] <==> (v3[939] || (if (v73 in v74) then v74[v73] else false));
	}
}
...
class C8 extends T1 {
	...
	function fm3(p0: int, globalState: GlobalState): int {
		match DC0('d') {
			case DC0(cf0) => |map[f23 := !f23]| + |(seq(-0x1e4, i0  => (cf0)))|
		}
	}
	...
	method m2(p0: bool, p1: int, p2: int, globalState: GlobalState) returns (r0: array<int>, r1: bool, r2: seq<bool>) {
		for i0 := 299 to p2 {
			...
			match (v7) {
				case DC3(cf3, cf4) =>
					...
					v15[821] := DC5(v13);
				case DC2(cf2) =>
					var v16: array<seq<bool>> := new seq<bool>[7](i1 => [v0.f21] + ([v5.f21])[|multiset{v0.f21}| := v0.f21]);
					v16 := v16;
					var v17: array<int> := new int[28](i2 => i2 - |{{v0.f21}}|);
					var v18: seq<int> := [v0.f22];
					v17[517] := -177 + v18[-0xfb];
					var v19 := 'x';
					var v20 := "dfdp";
					var v21: seq<string> := [("ylekotrl")[fm3(v0.f22, globalState) := v19], seq(-0x288, i3  => (v19)), v20, v20];
					v5, v17[517], v0.f22, r1 := v5, 0xaa, v0.fm7(v0.f22, v21 + (seq(0x1a5, i4  => (v20))), v0.f21, 0x1d2 == p1, globalState), v0.f21;
				...
				case DC4(cf5) =>
					var v25 := "uwhwcx";
					var v26: seq<int> := [-fm3(|(seq(658, i5  => ('t')))|, globalState)];
					var v27: seq<int> := [|v25| * |v26|, fm3(p2, globalState), i0, 0x209, v5.f22 / i0];
					v26 := [-v5.f22];
					v5.f22 := |v2|;
					var v28 := DC1(v27);
					v28 := v28;
		...
		if (f23) {
			...
			if (v29.f21 <== (v29.f22 >= p2)) {
				var v40 := "fjdkibmi";
				f23 := if (v29.f22 != v29.fm7(p2, [seq(-0x2d1, i7  => ('o')), v40, v40], !f23, true, globalState)) then v29.f21 else v40 == v40;
				v29.f22 := v29.f22 * |v30|;
				f23 := v29.f21;
				var v41 := DC2(v29.f21);
				v41 := v41;
				...
				var v45 := new C0(v44[p2], p2);
				var v46 := 'e';
				var v47: map<int, char> := map[|(seq(725, i8  => ('j')))| := v46];
				...
				var v56: array<int> := new int[18] [v45.f22 % v29.f22, v45.f22 - |v43|, v30[-0x144], v29.f22 - v29.f22, v29.f22 % v45.f22, p1, p2, 472, v29.f22, -(v45.f22 % v29.f22), v29.f22, v29.f22, v45.f22, v45.f22 + |fm11(v52, globalState)|, 0x339, |(if (|fm12(globalState)| in v54) then v54[|fm12(globalState)|] else v53[v29.f21 := |[-|v55|]|])[f23 := p1]|, v45.f22, |v30|];
				globalState.f6 := v56;
				var v57 := DC7(v29);
				...
				var v65: array<map<seq<int>, bool>> := new map<seq<int>, bool>[15] [map v60 : seq<int> | v60 in v62[-v45.f22] :: (v60) := (v29.f21), map[v30 := v29.f21], v63 + map[v30 := v29.fm8(globalState)], v63, v63 + v63, v63, v63, fm13(p1, v29.f22, v64.cf15, f23, globalState), v63, v63, v63, v63, v63 + v63, v63, v63];
				var v67: map<seq<int>, int> := map[v30 := v45.f22];
				v65[946] := map v66 : seq<int> | v66 in v67 :: (v66) := (v45.f21);
				var v68 := DC10(v58);
				v48, v59, v44, v65[946] := seq(934, i9  => (v46)), {v68.cf17, v58, v58}, [p2 <= v29.f22], v63;
			}
			
			if (v29.f21) {
				var v70: seq<map<int, char>> := [map v69 : int | v69 in v30 :: (v69 / v29.f22) := ('c')];
				...
				v32[809] := v29.f21;
				var v74: T2 := new C2();
				v32[809] := if (false) then f23 else p2 != |multiset{v74, v74}|;
				v32[809] := v29.f21;
				var v75: seq<bool> := [v32[809]];
				var v76: seq<bool> := [v75[v29.f22]];
				var v77: multiset<int> := multiset{|(v76 + v75)|, v29.f22 - p2, -879, p1};
				v77 := v77;
			}
			
			v29.f22 := v29.f22 / -0x358;
			var v78 := "rut";
			var v79: map<string, int> := map[v78 := |(v30 + v30)|];
			v79 := v79[seq(0x139, i10  => ('m')) := p1];
	...
	method m3(p0: seq<bool>, p1: string, p2: int, p3: bool, globalState: GlobalState) {
		if (p3) {
			...
			if (p3) {
				...
				globalState.f2, f23, v8[399], globalState.f2, f23 := -v11.cf83, true, v1, -(|v6| * p2) % p2, fm2(v13, |p0|, p2, globalState);
				globalState.f2 := p2;
				var v14: array<D2> := new D2[8](i2 => DC3(p3, p2));
				var v15 := 'x';
				var v17: map<int, int> := map[p2 := p2];
				var v18: map<bool, map<int, int>> := map[!f23 := v17];
				var v19: seq<char> := [v15, fm27(fm2(map v16 : int | (979 <= v16) && (v16 < 0x1a9) :: (v16 % p2) := (DC0(v15)), |(if (f23 in v18) then v18[f23] else v17)|, p2, globalState), p3, p2, globalState)];
				v1[767] := p2;
				var v20: map<bool, bool> := map[p3 := p3];
				v14, v19, globalState.f1, v1[767], v20 := v14, ([fm30(-p2, p2, f23, p3, globalState), v15, fm27(f23, fm2(v13, p2, p2, globalState), p2, globalState), v15])[p2 := v15], v15, fm3(p2, globalState), v20;
			} else {
				...
				v21[297] := !!fm2(v25, -p2, p2, globalState);
				var v26: multiset<bool> := multiset{p3};
				v22, v21[297], globalState.f5, f23, f23 := ([v21, v21, v21, v21, v21])[p2 := v21], if (false || f23) then v26 !! v26 else p3, v21, f23, f23;
				globalState.f2 := p2;
				globalState.f2 := fm3(p2, globalState) * p2;
				var v27 := "fqk";
				globalState.f2, v27 := p2, v27;
				var v28: map<char, string> := map[v23 := seq(0x7, i5  => (v23))];
				v27 := ((seq(-0xe9, i4  => (v23))) + "spigjbiqu") + (if ('n' in v28) then v28['n'] else v27);
			}
		...
		var v31 := DC29(f23, if (p3) then p2 else p2, |(if (f23) then [p2] else v30)|, |((seq(35, i6  => (0xa3)))[|fm36(p3, p3, globalState)| := |v30|])[p2 := p2]|);
		...
		match (v39) {
			case DC8(cf9, cf10, cf11, cf12, cf13) =>
				v36[586] := p2 % 0x2f2;
				v36[586] := p2 % (cf10 - cf10);
				var v40: set<int> := {cf9};
				var v41 := DC8(|v40|, 336, p3, cf12, cf13);
				var v43 := 'a';
				var v44: multiset<char> := multiset{v43};
				var v46, v47 := v37.m0(fm0(p2, fm31(v41, |(map v42 : char | v42 in v44 :: (v42) := (p3))|, p3, globalState), 0x19a, globalState), p2, p1, v37.fm2(map v45 : int | (0x1b9 <= v45) && (v45 < -319) :: (v45 % -cf9) := (DC0('t')), v36[586], |v30|, globalState), globalState);
				var v48 := DC18(v46);
				var v49: array<D7> := new D7[12] [v48, v48, v48, v48, DC18(cf10), if (p3) then v48 else v48, DC18(|[p3]|), v48, fm50(v43, v36[586], cf11, 0x341, globalState), v48, v48, v48];
				v49[528] := DC18(v46);
				v49[528] := v48.(cf35 := cf10 * |cf13|);
				cf11 := true <==> f23;
			case DC9(cf14, cf15, cf16) =>
				cf15 := f23;
			...
			case DC7(cf8) =>
				...
				if (p1[|v54| := v55] != (p1 + p1)) {
					...
					var v76: array<bool> := new bool[22] [cf8.f21, fm2(map v72 : int | v72 in v73 :: (v72 * 0xe8) := (DC0(v55)), v68.f22, 0xfa, globalState), p0 < p0, v74 == v74, cf8.f21 && p3, cf8.f21, f23, v74 >= v74, v68.f21, cf8.f21, !cf8.f21 && p0[v68.f22], cf8.f22 > -v68.f22, f23, p3, v75.cf73, cf8.f21, v68.f21, false, p3, p3, !v68.f21, p3];
				...
				match (v77) {
					case DC37(cf71, cf72, cf73) =>
						...
						globalState.f2, globalState.f6, v80, cf73 := p2 + (180 / cf8.f22), if (p2 in v81) then v81[p2] else v36, v80, v37.fm2(map[|v82| := v78.(cf0 := v55)] + v79, cf8.f22, cf8.f22 / p2, globalState);
						var v83: map<string, int> := map[p1 := 0x114];
						v83 := v83[p1 := -969];
						var v84: seq<C3> := [v37];
						var v85: set<seq<C3>> := {v84, v84};
						var v86: seq<set<seq<C3>>> := [v85];
						v85 := (v85 - v85) * (v85 * v86[|(seq(610, i11  => (p2)))|]);
					case DC38(cf74, cf75, cf76) =>
						f23 := cf8.f21;
						globalState.f2 := cf8.f22;
						var v87: array<array<int>> := new array<int>[22];
						var v88: array<char> := new char[8] [p1[cf76], fm17(globalState), 'a', v55, v55, fm17(globalState), v55, if (cf8.f21) then 'o' else v55];
						v88[764] := v55;
						v87, v88[764], cf76, f23, f23 := v87, v55, cf76, cf8.f21, p3;
						var v89: multiset<C0> := multiset{cf8, cf8, cf8};
						v89 := (v89 - v89) + ((multiset{cf8, cf8, cf8, cf8, cf8})[cf8 := cf8.f22] - v89);
					case DC36(cf70) =>
						var v90: set<char> := {v55, v55};
						var v91: multiset<C3> := multiset{v37};
						var v92: set<int> := {|v91|, cf8.f22, p2};
						var v93: map<string, bool> := map[seq(0xd2, i12  => ('b')) := p3];
						var v94 := DC22(f23, 0x74, |p1|);
						var v95: map<int, bool> := map[|p1| := cf8.f21];
						var v96: array<bool> := new bool[14] [v92 >= {cf8.f22, |p0|, -DC22(false, cf8.f22, cf8.f22).cf45, cf8.f22}, p3, cf8.f22 < -cf8.f22, if ((seq(87, i13  => ('d'))) in v93) then v93[seq(87, i13  => ('d'))] else p0[0x22b], p3, p3, cf8.f21, true, (if (cf8.f21 in v32) then v32[cf8.f21] else true) || f23, cf8.f21, cf8.f21, v94.cf44, v95 !in map[v95 := false], cf8.f21];
						v96[222] := cf8.f21;
						var v98: seq<array<bool>> := [v96];
						v90, v96[222], v96 := set v97 : char | v97 in map[v55 := -cf8.f22] :: (v97), cf8.f21, v98[v37.fm3(p2, globalState)];
						cf8.f22 := cf8.f22;
						var v99 := new C3();
		...
		match (v31) {
			...
			case DC28(cf53, cf54) =>
				...
				var v112: array<string> := new string[10] [p1, p1 + (if (cf54 in v106) then v106[cf54] else p1), fm28(|v111|, globalState), p1, (if (cf53 in v106) then v106[cf53] else p1) + p1, p1, p1, p1, ("gd")[p2 := v107] + "bcsrv", p1];
				v112[802] := p1;
				v112[802] := p1;
				var v113 := new C0(if (cf53) then true else cf54, p2 + v30[v30[|p0|]]);
				var v114: array<bool> := new bool[9];
				v114[538] := cf54;
				v114[538] := "lba" < fm12(globalState);
				v114[538] := !f23;
			case DC29(cf55, cf56, cf57, cf58) =>
				cf57 := cf58;
				cf57 := p2;
				if (cf55) {
					...
					var v120 := DC15(v117);
					v115[693], v119 := v120, v119;
					var v121: multiset<bool> := multiset{f23, true};
					var v122: map<bool, int> := map[cf55 := v30[|v121|]];
					var v123: array<bool> := new bool[1] [p0[|v122|]];
					v123[275] := f23;
					v123[275] := cf56 != -0x41;
					v37.m1(globalState);
					...
					v36[286] := -cf56;
					v124, v36[286] := v124, cf57;
				} else {
					var v125: map<int, int> := map[cf57 := cf57];
					var v127: map<bool, map<int, int>> := map[f23 := v125];
					var v128: multiset<int> := multiset{824};
					var v129: array<map<int, int>> := new map<int, int>[9] [v125, (map v126 : int | v126 in {cf57, |(seq(153, i14  => ({false})))|} :: (v126 % p2) := (|map[v30 := false]|)) + v125, (if (f23 in v127) then v127[f23] else v125) + v125, (map[p2 := p2])[cf56 := cf57], v125[cf58 := 0x95], fm18(cf57, f23, cf55, v128, globalState), v125, map[cf56 := cf56], v125];
					v129[904] := map v130 : int | (990 <= v130) && (v130 < -0x79) :: (v130 % p2) := (|[{p3}]|);
					v129[904] := v125;
					var v131 := DC40(-cf57);
					v131 := DC40(cf58);
					cf55 := f23;
			...
			case DC26(cf50) =>
				...
				v135 := v135[v133 := v134];
				var v136: map<int, bool> := map[0x1ca / v134.f22 := if (p3) then !v134.f21 else p3];
			...
			case DC30(cf59) =>
				...
				v137[810] := if (p3) then v140 else v140;
				v36[352] := |((seq(0x12c, i16  => ('y'))) + p1)|;
				var v141: set<seq<bool>> := {p0};
				var v142: seq<map<bool, bool>> := [fm19(globalState), v32, v32];
				v137[810], globalState.f2, globalState.f2, v36[352] := fm52(v141, f23, p1, fm53(false, p2, globalState), globalState), |([fm19(globalState)] + (v142 + (seq(0x28f, i17  => (v32)))))|, p2, 0x119 - p2;
				var v143 := 'm';
				var v144 := DC0(v143);
				var v145: map<int, D0> := map[v36[352] := v144];
				if (v37.fm2(v145, -0x27a / v37.fm3(-0x3c3, globalState), v36[352], globalState)) {
					var v146: map<int, bool> := map[|p0| := !p3];
					var v147: map<seq<int>, map<int, bool>> := map[[p2, v36[352], p2] + v30 := v146];
					v147 := v147[v30 := fm46(|p1|, p3, |(map v148 : char | v148 in [v143] :: (v148) := (p2))|, p3, globalState)];
					v36[352] := fm3(-p2, globalState);
					globalState.f2 := p2;
					v37.m1(globalState);
					var v149: map<int, char> := map[p2 := v143];
					var v150 := DC8(-855, v36[352], true, v149, p1);
					var v151: seq<string> := ["dsmbhiv", p1, p1, p1];
					var v152: multiset<int> := multiset{v36[352]};
					var v153: array<string> := new string[28] [p1, p1, p1, p1, seq(17, i18  => (v143)), fm12(globalState), p1, p1[v36[352] := v143], seq(0x110, i19  => (v143)), p1, p1, p1, p1, "on", v150.cf13, p1, p1, p1, p1, "dexmmxe", p1, p1, p1, p1, p1, v151[if (-v36[352] in v152) then v152[-v36[352]] else p2], seq(390, i20  => ('e')), p1];
					...
					var v158 := DC8(v36[352], p2, v157.cf22, v155, p1);
					var v159 := new C4(!f23, v158);
					f23 := v36[352] > fm3(|p1|, globalState);
					var v160 := DC47([p2, -p2], p3 <==> fm2(v145, v36[352], v36[352], globalState), p2);
				...
				var v163 := DC8(v36[352], p2, p3, map[-|v162| := 'k'], p1);
				var v164 := new C4(f23 <==> v37.fm2(v145, |p0|, p2, globalState), v163);
				globalState.f2 := v36[352];
		}
	...
	method m0(p0: seq<int>, p1: int, p2: string, p3: bool, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
		...
		for i0 := p1 to -0x319 - p1 {
			...
			match (v1) {
				...
				case DC17(cf31, cf32, cf33, cf34) =>
					...
					v5[617] := v4.fm3(p1 * |v6|, globalState);
					var v7 := 'p';
					globalState.f1 := v7;
				case DC18(cf35) =>
					f23 := true;
				...
				case DC15(cf26) =>
					...
					var v20: map<set<T0>, int> := map[v19 * v19 := 0x224 % p1];
					v20 := v20 + v20;
					globalState.f2 := i0 - (i0 % |[f23]|);
		...
		v31[462] := v30.fm2(v33, p1, |p0|, globalState);
		var v34: array<seq<string>> := new seq<string>[12];
		v34[64] := seq(104, i2  => ("cfsbbcaev"));
		var v35: seq<string> := [p2];
		var v36: seq<string> := [p2 + v35[p1]];
		v31[462], r0, v34[64] := p3 && p3, 479 / p1, v36;
		globalState.f2 := if (v31[462]) then v30.fm25(p3, f23, -p1, globalState) else p1;
		var i3 := 0;
		while (false)
			decreases 100 - i3
		{
			...
			i3 := i3 + 1;
			globalState.f1 := 'n';
			var v37: map<bool, int> := map[v31[462] := p1];
			v37 := map[f23 := p1 - p1];
			f23 := p3;
			f23 := f23;
		}
	...
	method m1(globalState: GlobalState) {
		...
		if (!fm2(v7, v0, v0, globalState)) {
			...
			v19[799] := v4;
			var v20 := "dcjse";
			var v21: array<int> := new int[16] [if (f23) then v0 else v0, -v0, -|fm0(v0, v20, |v20|, globalState)|, 249, 387 - v0, v0, v0 - v0, v0, |v4|, v0, v0, |v20[v0 := 'h']| - -v0, v0, v0, v0 + 0x47, 964];
			v21[577] := v0 / v0;
			var v22: array<string> := new string[16](i2 => "xufoohmo" + v20);
			var v23: map<int, char> := map[v0 := v5];
			var v24 := DC8(v0, v0, f23, v23, v20);
			f23, v21[577], v22, v20 := f23, |(if (f23) then v4 else v4)|, v22, (v20 + v20) + v24.cf13;
			globalState.f2 := (v21[577] - |v20|) + 301;
		}
		...
		for i3 := v0 to v0 {
			...
			v5, v0, v25[601], globalState.f2, globalState.f2 := 'a', i3, |({v0, i3, v0, fm3(v0, globalState)} + v26)| * v0, |v27|, |((v28[f23 := v5] + (if (i3 in v30) then v30[i3] else v28)) + v28)|;
			var v31 := new C5();
			var v32: array<bool> := new bool[12];
			var v33 := DC6(v32);
			var v34: map<bool, D3> := map[false := v33];
			var v35: seq<int> := [|DC82(v34).cf148|, v25[601]];
			var v36 := DC3(f23, v35[v0]);
		...
		if (f23) {
			...
			v0 := v0;
			var v41: array<int> := new int[21](i4 => i4 / v0);
			v41[972] := v0;
			globalState.f2, v41[972] := v0 % (v0 * v0), v0;
			v41[972] := v41[972];
			if (v41[972] > v41[972]) {
				var v42: map<bool, int> := map[f23 := v41[972]];
				var v43: multiset<char> := multiset{if (f23) then v5 else v5, v5, v5, v5, v39[|v42|]};
				...
				v43, f23, f23, f23 := multiset{fm27(f23, f23, v41[972], globalState)}, |v46| >= |v39|, fm2(v7, -v41[972], 659, globalState), f23 ==> f23;
				var v47: seq<bool> := [true];
				var v48: map<seq<bool>, bool> := map[v47 + v47 := f23];
				var v49 := DC77(f23, v0, f23, f23, f23);
				v48 := v48[[f23, f23, if (v49.cf141 in v4) then v4[v49.cf141] else f23, f23, f23] := DC63(f23, v5, f23, v41[972], true).cf118];
				var v50: seq<int> := [v0, v41[972]];
				f23 := (v50[v0] - fm3(|v39|, globalState)) != fm3(v0, globalState);
				var v51: set<bool> := {f23};
				var v53: map<int, char> := map[v0 := v5];
				var v54 := DC70(f23, map v52 : int | v52 in v53 :: (v52 + v0) := (f23), |v39|, fm2(v7, v0, v41[972], globalState), 0x2f9);
				...
				var v60 := DC8(|v39|, v0, f23, map v56 : int | v56 in v59 :: (v56 % v41[972]) := (v5), v39);
				var v61: C4 := new C4(fm2(v7, v0, v41[972], globalState), v60);
				var v62: map<C4, array<int>> := map[v61 := v41];
				var v63: multiset<int> := multiset{|v62|};
				var v64: array<bool> := new bool[20] [f23, f23, f23, f23 in v46, f23, f23, (seq(942, i5  => (v0))) != v50, v51 == v51, f23, f23, !f23, f23, v0 < |"k"|, f23, f23, v55 != v55, v47[if (v0 in v63) then v63[v0] else v0], v57.f21 ==> !f23, v47[0xd9], v41[972] >= v57.f22];
				v64[784] := !f23;
				var v65: array<array<int>> := new array<int>[15];
				v65[304] := v41;
				f23, v41[972], v64[784], v65[304] := v57.fm8(globalState), if (v61.f24) then v41[972] else -0x107, v57.f21, v41;
				v57.f22 := if (v64[784]) then |multiset((seq(404, i6  => (v57.f22))) + [|v47|, fm3(v57.f22, globalState)])| else |v39|;
			} else {
				f23 := f23;
				v39 := "fevbgvhrj";
				var v66: seq<int> := [v41[972]];
				f23 := {-v0, v66[|v39|]} >= {v0};
				f23 := f23;
				var v67: array<string> := new string[8] [v39, seq(0x34a, i7  => (v5)), v39, (fm55(v40, f23, 0x156, globalState) + v39)[v0 := v5], v39, v39, v39, seq(-0x21f, i8  => ('r'))];
				v67[313] := v39;
				var v68: map<bool, char> := map[f23 := v5];
				f23, v67[313], f23 := !(v5 in v39), v39, (if (f23 in v68) then v68[f23] else v5) in "lpljopcs";
			}
			
		} else {
			var v69 := "wctjttxts";
			f23 := v69 <= (seq(-0x17, i9  => (v5)));
			...
			decreases 100 - i10
		{
			...
			var v88 := DC5(v84);
			var v89: array<set<int>> := new set<int>[13] [v84, set v85 : int | (-0x49 <= v85) && (v85 < 0x19) :: (v85 / |DC52(map[f23 := v0]).cf105|), v84, {v0}, v84 + {v0, v0}, v84, v84, {v0, v0}, (if (f23 in v87) then v87[f23] else v84) - v84, v84 - v84, v84, v88.cf6, {v0}];
			v89[447] := (fm51(v88, globalState)).cf6 + (set v90 : int | v90 in v86 :: (v90 - 533));
			v79, v89[447] := (seq(0x233, i11  => (v5))) + "smveyptny", if (true in v87) then v87[true] else {-v0, v0};
			f23 := f23;
		}
	}
}
...
class C9 extends T2 {
	...
	function fm5(p0: bool, p1: char, p2: int, p3: bool, globalState: GlobalState): int {
		|("xyux" + (seq(0xd5, i0  => ('p'))))| * (if (false) then |"vysryjqo"| else |[multiset([0xfd, 0x211])]|)
	}
	function fm6(p0: D2, p1: int, p2: int, p3: bool, globalState: GlobalState): int {
		--(-0x2e6 * |{|(seq(0x3c3, i0  => (521)))|}|) + (-DC3(false, -|{false}|).cf4 - |[true]|)
	}
	method m4(p0: int, p1: set<map<char, int>>, p2: int, p3: bool, globalState: GlobalState) returns (r0: int, r1: bool) {
		var v0: map<int, int> := map[p2 := p0];
		var v2 := DC5(set v1 : int | v1 in v0 :: (v1 / |[true, false]|));
		match (if (p3 || p3) then v2 else v2) {
			case DC6(cf7) =>
				...
				var v6 := DC6(cf7);
				v3[603] := v6.cf7;
				var v7: array<string> := new seq<char>[2](i0 => (seq(0x342, i1  => ('d'))) + "mksjsojw");
				var v8 := "en";
				v7[81] := v8 + v8;
				...
				v12[630] := p0 + p2;
				var v13 := DC2(!v5.f21);
				v7[81], r1, v12[630] := v8, v13.cf2, (p2 / --p0) % -0x21;
			case DC5(cf6) =>
				...
				match (DC5((if (p3 in v14) then v14[p3] else cf6) - cf6)) {
					case DC6(cf7) =>
						...
						cf7[215] := fm2(v21, |(seq(0x0, i2  => (|map[p3 := map[p0 := p3]]|)))|, p2, globalState);
						v16, cf7[215] := fm28(0x37a, globalState), p3;
					case DC5(cf6) =>
						var v22: map<bool, bool> := map[p3 := p3];
						var v23: seq<bool> := [p3, p3];
						var v24: array<map<bool, bool>> := new map<bool, bool>[8] [v22, v22, v22, v22 + v22, v22, v22, v22 + v22, v22 + v22[v23[p2] := false]];
						v24[16] := v22;
						v24[16] := v22;
						v23 := fm74(p0, p2 / -|(map v25 : int | (221 <= v25) && (v25 < 0x91) :: (v25 % |{true, p3}|) := (p0))|, p0, globalState);
						var v26 := 'e';
						var v27 := DC0(v26);
		...
		globalState.f2, v36[800], v38, v40 := 737 * p2, (p2 / |v41|) / p0, v38 * multiset{v37, v37}, match if (p3) then fm81(|v40|, p0, p0, globalState) else DC49(|v40|, v44, p0, 604, |{p3, p3, p3, p3}|) {
			case DC47(cf89, cf90, cf91) => v40
			case DC48(cf92, cf93, cf94) => v40 + ['b']
			case DC49(cf95, cf96, cf97, cf98, cf99) => seq(247, i3  => (v39))
			case DC46(cf88) => if (p3) then seq(-0x2f3, i4  => (v39)) else v40
		};
		var i5 := 0;
		while (true)
			decreases 100 - i5
		{
			...
			var v45: map<array<int>, int> := map[v36 := v36[800]];
			v45 := v45[v36 := p0];
			var v46: map<int, bool> := map[p0 := p3];
			var v47: map<map<int, bool>, bool> := map[v46 := !p3];
			var v48 := DC53(|v46|);
			v47 := v47[fm61(v36[800], v48.cf106, globalState) := fm2(map v49 : int | (0x25a <= v49) && (v49 < 0x181) :: (v49 * p2) := (DC0(v39)), v36[800], v36[800], globalState)];
			var v50 := new C0(p3, p0);
		}
		if (!p3 <== p3) {
			r1 := -p2 >= p0;
			var v51 := DC0(v39);
			var v52: array<char> := new char[29] [v39, v40[v36[800]], v39, v39, v39, v39, v39, v40[v36[800]], v39, 'x', v39, v39, v39, v39, if (p3) then v39 else 'm', v39, v39, 'i', v39, 't', v39, v39, 'g', v39, v39, fm17(globalState), v39, v51.cf0, v39];
			var v53: array<string> := new string[20];
			var v54 := DC15(fm29(p3, p2, fm56(p3, globalState), globalState));
			v52, v53, v54, v40, r1 := v52, v53, v54, v40[v36[800] * |v40| := v39], p3;
			var v55: seq<set<bool>> := [fm1(seq(0x300, i6  => (v39)), p3, p3, globalState)];
			r1 := !(v55 != v55);
			var v56 := new C0(p3, fm3(v36[800], globalState) - 0x100);
			var v57 := new C8(p3);
		} else {
			var v58 := new C2();
			var v59: seq<int> := [p0];
			...
			v58.m1(globalState);
			var v61: map<int, array<int>> := map[v36[800] := v36];
			var v62 := DC49(p0, fm82(p2, globalState), p2, v36[800], |v61|);
			...
			if (v63.cf44) {
				...
				globalState.f2 := 0x80 + (v36[800] / -p0);
				var v65: array<bool> := new bool[12];
				globalState.f5 := v65;
				globalState.f1 := v39;
				var v66: multiset<int> := multiset{v59[v36[800]], p2, |v42|};
				var v67: seq<bool> := [p3];
				v66 := (multiset{p2, 331})[-712 := |v67|] + multiset{p0, p2};
		...
		var v80: map<bool, int> := map[p3 <== p3 := |(v77[|v79|] + v77[p0])|];
		v80 := v80[p3 := v36[800]];
		var v81: map<int, bool> := map[v36[800] := p3];
		var v82 := DC70(p3, v81, p2, p3, v36[800]);
		globalState.f2 := match fm83(p3, v82.cf132, false, p3, globalState) {
			case DC20(cf37, cf38, cf39) => v36[800]
			case DC21(cf40, cf41, cf42, cf43) => p0
			case DC22(cf44, cf45, cf46) => -cf46
			case DC19(cf36) => -(v36[800] - -p2)
		};
		r0 := match fm81(295, v36[800], p0, globalState) {
			case DC47(cf89, cf90, cf91) => v36[800] % cf91
	...
	method m5(globalState: GlobalState) returns (r0: int, r1: seq<set<bool>>, r2: int, r3: int) {
			...
			decreases 100 - i1
		{
			...
			if (v3) {
				...
				var v17: array<int> := new int[25] [v5, v5, -v5 / v5, v5, v5, v5 * |(DC13(v1, v7.f26, v9, true).(cf21 := "rnisxkwck")).cf21|, v5, |v10|, v5, v5, if (v11 in v12) then v12[v11] else v5, v5 * |v14|, v5, v5, v15[v5], |v16|, v5, |(v1 + v1)|, v5, v5 + -202, v5, v5, v5, v5, v5];
				v17[69] := 0x335;
				var v18: seq<bool> := [false];
				v17[69] := (if (v7.f26) then --v5 else |v18|) / v5;
				r0 := v17[69];
				var v19: set<int> := {v5};
				r2 := -(v5 / |v19|);
				v13[154] := !v7.f26;
				var v20: C0 := new C0(v7.f26, v17[69]);
				var v21 := DC83(v20, v20.f22, v9, -0x2, v9);
				var v22: seq<D32> := [v21];
				var v23: multiset<D32> := multiset{v21, v21, v21, v21};
				v13[154] := (multiset(v22) * v23) == (v23 + v23);
			} else {
		...
		if (v38.f21) {
			...
			var v43: seq<T2> := [v42[v38.f22], v41, v41];
			var v44: seq<seq<T2>> := [v42, v42];
			var v45: seq<bool> := [v38.f21];
			v44 := [((if (v38.f21) then v42 else v43)[v5 := v41])[|v45| := v41]];
			var v46: array<bool> := new bool[22];
			v46[887] := !true;
			v46[887] := true;
			var v47: map<bool, int> := map[v38.f21 := v5];
			v47 := v47[v38.f21 := v38.f22 / v41.fm3(v38.f22, globalState)];
			var v48: set<int> := {0x34b};
			if ((v38.f22 % fm6(v4, v5, v38.f22, !true, globalState)) in v48) {
				v46[887] := false;
				var v49 := new C4(v46[887], v40);
				var v50 := new C0(v38.f21, v38.f22);
				var v51: array<seq<string>> := new seq<string>[10];
				var v52: seq<string> := [seq(-564, i3  => ('t'))];
				var v53 := DC17(!v38.f21, v38.f22, v46[887], v3);
				v51[299] := v52 + [v1, fm72(v38.f22, v38.f21, v53, v49.f24, globalState)];
				v51[299] := [v1];
				var v54: C2 := new C2();
				var v55 := DC78(v54);
				var v56 := DC80(v55);
				...
				globalState.f1, v58, v46[887] := v1[v38.f22], v58 - {v57[v56.(cf146 := v55) := v5]}, v38.f21;
			} else {
				v3, globalState.f5, v1, r2, v38.f22 := v38.f21, v46, "ofp", |("tdyquynuv" + v1)|, -0x9;
				var v59: set<bool> := {if (v46[887]) then false else v38.f21, v46[887]};
				v59 := v59;
				v46[887] := v38.f21;
				var v60 := new C3();
				v46 := v46;
			...
			v61[96] := v38.f22;
			v61[96] := v38.f22 % v38.f22;
			var v62 := 'i';
			r3 := fm5(v3, if (v3) then v62 else v62, -v61[96], if (v38.f21) then v38.f21 else v38.f21, globalState);
		}
		...
		r0 := |v65| - v38.f22;
		r1 := fm85(v38.f22 / v38.f22, v38.f22, v38.f22 % v5, globalState);
		r2 := v38.f22;
		r3 := 0xbb;
	}
	method m2(p0: bool, p1: int, p2: int, globalState: GlobalState) returns (r0: array<int>, r1: bool, r2: seq<bool>) {
		...
		for i0 := |v0| to p1 {
			...
			var v9 := DC8(i0, 0x346, v4[p2], v8, v0);
			...
			decreases 100 - i1
		{
			...
			v14[60] := !((if (false) then 0x2c1 else |v15[p2]|) <= p2);
			globalState.f2 := p1;
		}
	...
	method m3(p0: seq<bool>, p1: string, p2: int, p3: bool, globalState: GlobalState) {
		...
		var v1: map<seq<int>, int> := map[seq(-0x16, i0  => (p2)) := |v0|];
		var v3 := DC2(p3);
			...
			decreases 100 - i1
		{
			...
			var v15: array<seq<bool>> := new seq<bool>[21] [p0, v11[p2], [p3], if (p3 in v12) then v12[p3] else p0, p0, p0, p0 + [fm2(v13, p2, p2, globalState)], p0, [fm2(map v14 : int | (994 <= v14) && (v14 < 0x118) :: (v14 / 128) := (DC0('g')), p2, v4[|v0|], globalState)], p0, p0, p0, fm36(v6, v6, globalState), p0, p0, p0, [p3, p3, v6], p0, p0 + p0, p0, fm74(p2, p2, p2, globalState)];
			v15[178] := p0;
			var v16 := "ougjdo";
			var v17: map<bool, int> := map[p3 := p2];
			v6, v9, v15[178], v6, v16 := DC29(p3, p2, if (true in v17) then v17[true] else p2, p2).cf55, v9, (p0 + p0) + p0, fm2(v13, p2, p2, globalState), (v16[p2 := v7] + v16) + p1;
			v6 := v6;
			var v18: map<int, seq<bool>> := map[-(p2 + p2) := v15[178]];
			v18 := v18[-fm5(p3, v7, p2, v6, globalState) := v15[178]];
			if (!(p2 > p2)) {
				var v19: array<int> := new int[5];
				v19[595] := p2;
				v19[595] := fm5(v6, v7, |p1| + fm3(p2, globalState), {p3} > v10, globalState);
				v6 := fm2(v13, p2, v19[595] - p2, globalState);
				var v20: map<int, string> := map[-v19[595] := v16];
				v17 := v17[p3 := |(if (v19[595] in v20) then v20[v19[595]] else v16)|];
				globalState.f2, globalState.f2, v6, globalState.f2 := v19[595], p2, p3, v19[595];
				v6 := v6;
			} else {
				...
				var v24 := DC8(v21[322], v21[322], p3, v23, v16);
				var v25 := new C4(p3, if (p3) then DC8(p2, p2, v6, v23, p1) else v24);
				var v26: map<int, bool> := map[v21[322] := true];
		...
		for i3 := -343 to p2 {
			if (true) {
				...
				var v40 := 'r';
				globalState.f2 := fm5(|p1[i3 := 'c']| == |v39|, v40, 26, i3 <= i3, globalState);
			} else {
				var v41 := 'b';
				globalState.f1 := v41;
				var v42: map<bool, bool> := map[v6 := p0[i3]];
				globalState.f2 := p2 % (fm3(|v42|, globalState) + -i3);
				var v43: multiset<multiset<bool>> := multiset{multiset(p0) + v0};
				var v44: set<int> := {p2, i3};
				v43 := fm44(-|{map[p2 := |v44|]}|, true, globalState) - v43;
				var v45: C0 := new C0(v6, v4[p2]);
				v45 := v45;
			...
			if (v48 !in v52) {
				...
				var v60: array<int> := new int[26] [p2, |(v54 + fm86(v51.f24, {p3}, |p1|, v6, globalState))|, i3 - i3, i3, i3, p2, -|fm12(globalState)|, |((seq(0x34e, i5  => ('k'))) + "xssxq")|, i3, p2, p2, p2, if (p3) then |v58| else p2, p2, p2, p2, i3, p2, |(map v59 : int | (0x334 <= v59) && (v59 < 459) :: (v59 * i3) := (v48))|, i3, if (p3) then i3 else i3, i3, |(seq(0x275, i6  => (i3)))| - i3, p2, p2, i3];
				v60[696] := |p0[i3 := v51.f24]|;
				var v61: map<bool, bool> := map[v51.f24 := v6];
				var v62 := DC37(p0, v6, true);
				v60[696], globalState.f2, v6 := if ((i3 / |v61|) in v57) then v57[i3 / |v61|] else i3, 743, (v62.cf71 + p0) <= p0;
				v56.f26 := v51.f24;
				var v63 := "kw";
				v63 := (seq(0xd, i7  => ('q'))) + v63;
		...
		if (!match v67 {
			...
			if (p3 && (v6 ==> p3)) {
				...
				globalState.f2, v85, globalState.f2 := p2, v85, |v86[p2]|;
			...
			match (v99) {
				case DC34(cf65, cf66) =>
					var v100 := "cfes";
					var v101 := 'r';
					cf66, globalState.f2, globalState.f2, v100 := -p2, |[p3, p3]|, cf66, ((((seq(210, i8  => ('f'))) + v100) + (seq(-0xfe, i9  => ('f'))))[cf66 := v101])[|p1| := fm56(p3, globalState)];
					globalState.f2 := -fm3(cf66, globalState) % p2;
				...
				case DC35(cf67, cf68, cf69) =>
					...
					var v114: seq<bool> := [cf68, p0[|v113|], v6, v6, p2 == p2];
				...
				case DC33(cf64) =>
					...
					var v126: array<int> := new int[13] [p2, if (v117 in v118) then v118[v117] else |v119|, p2, p2, p2, |v120|, -|v121| - |v4[p2 := p2]|, p2 - -p2, |v123.cf164|, |v125[v6 := v124]|, p2, -|p1|, p2];
					v126[698] := p2;
					var v127 := DC77(p3, p2, p3, v6, v6);
					var v128: set<bool> := {v6};
					var v129: map<set<bool>, string> := map[v128 := p1];
					var v130 := DC49(p2, v129, p2, p2, p2);
					v126[698] := |map[-p2 := v127.cf141 < -v130.cf98]|;
					var v131 := DC32(v126[698] % v126[698], false, v6);
					v131 := v131;
					v6 := p3;
					globalState.f2 := p2;
			}
			...
			var v136 := DC8(p2, p2, v6, v134, p1[|v135| := v133]);
			v132[474] := |fm31(v136, p2, p3, globalState)| % 0x55;
			var v137: C8 := new C8(v6);
			var v138: multiset<int> := multiset{p2, |p1|, p2};
			var v139 := DC39(v138);
			v132[474], v133, v137, v138, v135 := |p1|, if (if (v137.f23) then v137.f23 else false) then v133 else p1[p2], v137, v139.cf77, (v135 - v135) * v135;
			var v140 := new C8(true);
			var v141: map<bool, bool> := map[v137.f23 := !v137.f23];
			v141 := v141[v132[474] <= v132[474] := v140.f23];
			if (v137.f23) {
				v140.f23 := v132[474] <= p2;
				globalState.f1 := fm17(globalState);
				var v142 := DC0(v133);
				var v143: map<int, D0> := map[|(seq(0x25f, i11  => (v133)))| := v142];
				...
				var v147: map<bool, map<int, array<int>>> := map[v137.f23 := v146 + v146[v132[474] := v132]];
				globalState.f2 := -|(if (v137.f23 in v147) then v147[v137.f23] else map[v132[474] := v132] + (if (v137.f23 in v147) then v147[v137.f23] else v146))|;
				var v148 := DC0(v133);
				var v149: map<int, D0> := map[p2 := v148];
				var v150: set<char> := {fm64(v132[474], p2, v137.fm2(v149, p2, v132[474], globalState), p2, globalState), v133, v133};
				var v151: map<int, bool> := map[p2 := v137.f23];
				var v152: map<char, bool> := map['d' := if (v132[474] in v151) then v151[v132[474]] else p3];
				v150 := fm88(if (v133 in v152) then v152[v133] else true, -v132[474], globalState);
				var v153: set<int> := {-|v152|};
				var v154: array<bool> := new bool[24] [p3, v140.f23, v140.f23, !v140.f23, v137.f23, v137.f23, true, true, v137.f23, false, false, true, v140.f23, v6, false, !false, v140.f23, if (p2 in v151) then v151[p2] else p3, v140.f23, v6, v137.fm2(v149, v132[474], v132[474], globalState), if ((seq(0x31d, i12  => (v132[474]))) in v5) then v5[seq(0x31d, i12  => (v132[474]))] else v140.f23, v137.f23, v140.f23];
				var v155: map<seq<int>, array<bool>> := map[v4[p2 := |v153|] := v154];
				v6 := v4 !in (if (false) then v155 else v155);
				v145 := ((seq(938, i13  => (p1[p2]))) + (v145 + "xgefi")[p2 := v133])[v132[474] * |p1| := v145[v132[474]]];
			}
			
			var v156: array<array<int>> := new array<int>[23] [v132, v132, DC9(v132, v137.f23, v132[474]).cf14, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132, v132];
			v156[565] := v132;
			v156[565] := v132;
		}
		
		var v157: array<int> := new int[16];
		v157[251] := |(multiset{p2, p2} + multiset{|p0|})|;
		var v158 := DC62();
		var v159 := 'g';
		var v160 := DC0(v159);
		v157[251], v158, globalState.f6, globalState.f2, globalState.f2 := p2 % --p2, match v160.(cf0 := v159) {
			case DC0(cf0) => v158
		}, v157, 393, p2 * -(|v4| - p2);
	}
	method m0(p0: seq<int>, p1: int, p2: string, p3: bool, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
		if (true) {
			...
			var v6: seq<bool> := [v5[v0.f22]];
			var v7: map<int, bool> := map[v0.f22 := v2.f23];
			var v8 := DC0('o');
			var v9: map<int, D0> := map[p0[|v7|] := v8];
			var v10: array<bool> := new bool[3] [p3, v6[631], fm2(v9, v0.f22, v0.f22, globalState)];
			v10[380] := -v0.f22 > p1;
			v10[380] := v0.f21;
			var v11 := new C3();
		} else {
			...
			var v13 := DC2(p3);
			v12[78] := -(p1 / fm6(v13, p1, |p2|, p3, globalState));
			var v14: multiset<int> := multiset{p1};
			var v15: map<multiset<int>, bool> := map[v14 := true];
			v12[78] := p1 + |v15[v14 := p3]|;
			var v16: map<int, string> := map[0x3bf := p2];
			var v17 := 'n';
			v16 := v16[v12[78] := p2[v12[78] := v17] + p2];
			var v18: multiset<string> := multiset{seq(-0x31e, i0  => (v17))};
			v18 := v18 - DC94(multiset{p2}).cf171;
			var v19 := DC0(v17);
			var v20: map<int, D0> := map[551 := v19];
			var v21: map<int, char> := map[-p1 := v17];
			var v22 := DC8(v12[78], |p0|, p3, v21, p2);
			var v23: T2 := new C4(!p3, v22);
			var v24: map<T2, int> := map[v23 := 0x2db];
			var v25: set<int> := {p1, p1};
			var v26 := DC5(v25);
			var v27: set<char> := {v17, v17};
			var v28: array<bool> := new bool[13] [p1 > v12[78], fm2(v20, 0x160, v12[78], globalState), fm2(v20, |(seq(-118, i1  => (v17)))|, v12[78], globalState), p3 && p3, p3, fm2(v20, v12[78], |v24|, globalState), p3, v26.cf6 != v26.cf6, p3, p3, |v27| > v12[78], p3, p3];
			v28[641] := p3;
			v28[641] := p3;
			if (p3) {
				var v29: set<bool> := {p3, false};
				v29, globalState.f5, v12[78], v28[641], v28[641] := v29 - {v28[641], true}, v28, v12[78], p3, !v28[641];
				v28[641] := false;
				var v30 := "vfdanqd";
				v30 := p2 + v30;
				v28[641] := v12[78] != v12[78];
				var v31: multiset<bool> := multiset{v28[641], v28[641]};
				v31 := multiset{v28[641] ==> !p3, p3};
			} else {
				v28[641] := p2 < ((seq(0x4c, i2  => (v17)))[v12[78] := v17] + p2[p1 := v17]);
				var v32: array<array<int>> := new array<int>[1] [v12];
				var v33 := DC66(v32);
				v28[641], v28[641], globalState.f2, v33 := v28[641], v28[641], p1, if (p3) then v33 else if (p3) then DC66(v32) else v33;
				globalState.f2 := -v23.fm3(p1, globalState);
				var v34 := "n";
				v34 := ((seq(0x107, i3  => (v17))) + v34) + (seq(0x8c, i4  => (v17)));
				v28[641], v28[641], globalState.f2, v12[78] := v28[641], v28[641] <== (p3 && v23.fm2(map[v12[78] := v19], |p2|, p1, globalState)), -p1, -p1 - v12[78];
			}
		...
		if (p3) {
			...
			if (p3) {
				...
				var v48 := 'v';
				var v49: map<int, char> := map[p1 := v48];
				var v50 := DC8(p1, 0x1b3, v45[351], v49, p2);
				...
				v57.f24 := v52[if (p3) then v53.f22 else v53.f22];
			}
			...
			var v71 := DC8(p1, |(seq(0x3aa, i5  => ('y')))|, p3, v70, p2);
			...
			v75.f24, v73, v77[440], globalState.f5, r0 := p3, p3 ==> v75.fm2((map v79 : int | (-0x33d <= v79) && (v79 < 0x376) :: (v79 / p1) := (DC0(v69)))[p1 := v80], |v81|, fm5(v75.f24, v69, p1, p3, globalState), globalState), v78[v75.f24 := v75.f24], v83[fm3(-p1, globalState)], p1;
			if (p3) {
				v75.f24 := true;
				...
				v87[728] := if (fm89(p1, |p2|, v89, globalState) !! v91) then v88 else v88;
				var v92: map<int, bool> := map[p0[p1] := p1 <= p1];
				v75.f24 := if (p1 in v92) then v92[p1] else p3;
			} else {
				var v93: array<int> := new int[28](i6 => i6 / p1);
				...
				var v103: array<multiset<bool>> := new multiset<bool>[3] [((if (|v100| in v99) then v99[|v100|] else v98)[p3 := 0xeb])[v73 := if (|p0| in v101) then v101[|p0|] else |[p1]|], multiset(v102), v98];
				v103[632] := multiset(if (v73) then v102 else v102);
				v103[632] := v98;
			}
		...
		var v105 := 's';
		var v106 := DC0(v105);
		var v107: map<int, D0> := map[|fm24(p1, DC13(seq(-0x3e5, i7  => ('b')), v104, 'f', p3), |p0|, globalState)| := v106];
		...
		var v113: array<int> := new int[28] [fm6(v109, |(seq(0x1a4, i8  => (v105)))|, p1, p3, globalState), p1, p1, p1, p1, p1, p1, p1, p1, |multiset{p3}|, p1, p1, p1, -0x3e3, |v110|, p1, p1, p1, p1, p1, p1, p1, |p2|, p1, 0x3c7, -0x15c, |p0|, |v112|];
		var v114 := DC9(v113, v104, p1);
		var v115: map<int, map<D4, bool>> := map[|p0| := map[v114 := v104]];
		var v116: map<D4, bool> := map[v114 := v104];
		v115 := v115[p1 := v116];
		var v117 := DC85(p3);
		var v118: map<D33, bool> := map[v117 := p1 == p0[p1]];
		if (if ((if (p3) then v117 else v117.(cf155 := v104)) in v118) then v118[if (p3) then v117 else v117.(cf155 := v104)] else p3) {
			...
			var v122: map<seq<int>, array<int>> := map[p0 := v121[p1]];
			var v123: set<bool> := {p3};
			var v124: seq<map<seq<int>, array<int>>> := [v122];
			var v125: array<map<seq<int>, array<int>>> := new map<seq<int>, array<int>>[18] [v122, v122, v122, v122, v122 + v122, v122, v122, map[p0 := v113] + map[[|v123|] := v113], v122, v124[-p1], v122, v122, v122, v122, v122 + map[seq(-0x3c5, i9  => (p1)) := v113], v122, v122, v122];
			v125[541] := v122;
			v125[541] := v122;
			var v126 := "gkywu";
			...
			var v129: map<int, array<int>> := map[p1 / p1 := v113];
			...
			var v137: multiset<map<char, int>> := multiset{v135, (map v136 : char | v136 in (seq(0x18b, i10  => (v105))) :: (v136) := (p1))[v105 := 0x285]};
			v129, v130[937], v132, v104 := v129 + (if (v104 in v133) then v133[v104] else v129), if (v104 || v104) then v131 else v131, v132 - ({v108} * (set v134 : map<int, bool> | v134 in v132 :: (v134))), v137 !! v137[map v138 : char | v138 in multiset{v105} :: (v138) := (p1) := p1];
		} else {
			globalState.f2 := p1;
			var v139: set<int> := {p1};
			var v140: map<int, set<int>> := map[p1 := v139];
			globalState.f2 := |(({p1} * (if (|v139| in v140) then v140[|v139|] else set v141 : int | (502 <= v141) && (v141 < 0x343) :: (v141 / p1))) * fm20(p3, p3, globalState))|;
			v104 := v104;
			...
			v145 := [seq(0x185, i11  => (p1)), p0, p0, p0] + v145;
	...
	method m1(globalState: GlobalState) {
		var v0 := true;
		var v1: array<int> := new int[29](i0 => i0 / |map[|map[v0 := -402]| := DC79()]|);
		var v2 := 0x1a9;
		var v3: map<bool, int> := map[v0 := v2];
		v1[583] := |v3|;
		var v4: set<int> := {v2};
		var v5: map<int, bool> := map[v2 := v0];
		var v7: seq<int> := [-|(set v6 : int | v6 in v5 :: (v6 / |map[true := 0x283]|))|];
		v0, v1[583], globalState.f2, v4 := v0, v2, v2 * v7[v7[v2]], set v8 : int | (0xd3 <= v8) && (v8 < -0x151) :: (v8 * v2);
		var v9 := DC77(v0, 0x118, v0, v0, !!v0);
		for i1 := v9.cf141 to -v1[583] {
			var v10: seq<bool> := [v0, v0, !v0];
			v1, v0 := if (v0) then v1 else if (v0) then v1 else v1, false in multiset(v10);
			v2 := |"yflrvj"|;
			globalState.f2 := -v7[i1 * 0x18e];
			v2 := |[v2, 0x320, -v1[583] - |(seq(0x10a, i2  => (v1[583])))|]|;
		}
			...
			decreases 100 - i3
		{
			...
			i3 := i3 + 1;
			var v12: map<int, map<int, bool>> := map[v1[583] := v5];
			var v13: array<bool> := new bool[29];
			var v14: map<int, array<bool>> := map[-|v12| := v13];
			v14 := v14[0x1f7 := v13];
			var v15: multiset<int> := multiset{|[v0]|};
			v13[390] := multiset{v1[583], v1[583]} !! v15;
			v13[428] := if (v0) then v0 else !v0;
			v13[304] := v0;
			var v16: multiset<array<int>> := multiset{v1, v1, v1, v1, v1};
			v13[181] := v16 !! multiset{v1};
			var v17: multiset<bool> := multiset{v0};
			var v18 := DC2(v0);
			var v19: seq<bool> := [v0];
			v1[583], v13[390], v13[428], v13[304], v13[181] := fm5(v0, 'n', |v17|, v0, globalState), v2 >= fm6(v18, v2, v2, v0, globalState), v1[583] >= v2, v0, (fm5(v0, fm27(v0, v0, v2, globalState), v2, v0, globalState) + v1[583]) <= |v19|;
			var v20: array<seq<bool>> := new seq<bool>[5] [(v19 + v19)[v1[583] := v13[390]], v19 + v19, v19, v19, v19];
		...
		if (v0) {
			var v24: array<multiset<int>> := new multiset<int>[23](i4 => multiset{v1[583]} + multiset(v7));
			var v25: multiset<int> := multiset{v2};
			v24[470] := v25;
			v24[470] := v25;
			var v26 := new C0(v25 !! v25, v2 - v1[583]);
			var v27: array<bool> := new bool[7](i5 => |[v0, v26.f21]| >= |[v0]|);
			v27[717] := v26.f21;
			v27[717] := !v26.f21;
			v7 := v7 + v7;
			var v28: map<seq<int>, bool> := map[[-v2] := v26.f21];
			globalState.f5 := new bool[9] [v1[583] < v2, v26.f21, if (v7 in v28) then v28[v7] else v27[717], true, v26.f21, false, v26.f21, v27[717] <==> v0, !!v26.f21];
		} else {
			var v29: multiset<int> := multiset{v1[583]};
			var v30: set<multiset<int>> := {multiset{737} * multiset{v2}, multiset(v7), v29};
			v2 := |v30|;
			v2 := v1[583];
			if (v0) {
				v0 := v1[583] > v2;
				var v31 := DC3(v0, v1[583]);
				...
				globalState.f2, v36, v35, v32.f26 := v2, ([v32.f26] + v36) + (v36[v2 := v0] + v36), seq(-0x352, i6  => (v33)), v0;
			} else {
				var v37 := DC0('r');
				var v38: map<int, D0> := map[0x298 := v37];
				var v39: set<bool> := {v0, v0, v0, v0, v0};
				v0 := fm2(v38, v1[583], |v39|, globalState);
				var v40 := DC2(v0);
				var v41 := 'k';
				var v42 := "c";
				var v43: array<bool> := new bool[16] [!v0, v0, v0, !true, v1[583] > v1[583], v0, v0, v0, v0, v0, v0, v0, v1[583] >= fm6(v40, |v7|, v2, v0, globalState), v41 !in v42, v0 || v0, v0];
				var v44: map<int, char> := map[|v39| := v41];
				var v45 := DC8(v1[583], |map[v0 := v0]|, v0, v44, v42);
				var v46: C4 := new C4(v0, v45);
				var v47: set<C4> := {v46};
				v43[906] := v47 > v47;
				v43[906] := v46.f24 && v46.f24;
				globalState.f2 := v2;
				var v48 := new C7();
				var v49: multiset<bool> := multiset{v43[906], v0, v43[906], v0, v0};
				v43[906] := !v43[906] ==> (v43[906] in v49);
			}
			
			var v50: set<bool> := {v0};
			v0 := v50 > (v50 + v50);
			var v51: set<char> := {'g'};
			var v53: seq<set<int>> := [{v1[583]}];
			var v56: array<set<int>> := new set<int>[20] [v4, v4, v4, v4, v4, v4, v4, v4, v4, DC5(v4).cf6, v4, set v52 : int | v52 in v4 :: (v52 % |map[false := [|map['y' := 'l']|]]|), v53[|{!v0}|], {v2}, set v54 : int | (0x15d <= v54) && (v54 < -42) :: (v54 * v2), fm10(v2, v3, -v1[583], v2, globalState), v4, v4, set v55 : int | (0x22d <= v55) && (v55 < 0x12e) :: (v55 - v1[583]), v4];
			var v57: map<int, array<set<int>>> := map[|v51| - v1[583] := v56];
			v57 := v57[v1[583] := v56];
		}
		...
		if (v0) {
			...
			v63 := map[v61[339] := map[v2 := v1[583]]];
			var v64: seq<bool> := [v61[339]];
			var v65: map<int, seq<bool>> := map[v2 := v64];
			v64, globalState.f5, v1[583] := if (-0x305 in v65) then v65[-0x305] else v64, v61, v2 * fm3(v2, globalState);
		} else {
			v1[583] := v2;
			var v66 := 'c';
			...
			v0 := if (v2 in v5) then v5[v2] else v1[583] > |v4|;
			v70.f24 := v70.f24 ==> !v70.f24;
			var v71: map<char, bool> := map[v66 := v70.f24];
			var v72 := DC46(map[v1[583] := v71]);
...
class C10 extends T2 {
	...
	method m4(p0: int, p1: set<map<char, int>>, p2: int, p3: bool, globalState: GlobalState) returns (r0: int, r1: bool) {
		...
		var v18 := 'g';
		r1, v6, r1, v14[54], v16 := p3 && (v17 > v17), DC4(DC4(v5)), !p3, v18 !in v15, v16;
		globalState.f2 := if (if (!p3) then v16.cf3 else v14[54]) then p2 else DC3(v14[54], p0).cf4;
		var v19: array<seq<int>> := new seq<int>[29];
		forall i2 | 0 <= i2 < v19.Length {
			v19[i2] := [|v9|] + ([|(map v20 : int | v20 in {p0, f20, |multiset([f20])|} :: (v20 - |(seq(0x1ad, i3  => (p0)))|) := (true))|, f20] + (seq(-0x7f, i4  => (p0))));
		}
	...
	method m5(globalState: GlobalState) returns (r0: int, r1: seq<set<bool>>, r2: int, r3: int) {
		...
		var v6: seq<int> := [|v3|, fm3(f20, globalState), |(map v4 : int | v4 in v5.cf6 :: (v4 / 0x206) := (f20))|, f20];
		var v7 := 'r';
		var v8 := DC0(v7);
		var v9: map<int, D0> := map[|v6| := v8];
		var v10: array<bool> := new bool[3] [v0, false, fm2(v9, v6[f20], f20, globalState)];
		var v11 := DC6(v10);
		...
		v12[701] := v10;
		v12[701] := if (v0) then v10 else v10;
		var v13: T2 := new C2();
		var v14: map<T2, bool> := map[v13 := v0];
		v14 := v14 + map[v13 := v0];
		v0 := !(v7 in (seq(220, i0  => (v7))));
		var v15 := "haina";
		for i1 := f20 to |((seq(0x34a, i2  => (v7))) + v15)| {
			var v16: seq<seq<int>> := [v6[f20 := i1]];
			r3, r2, v1 := |v16[i1]|, |((v15 + v15) + v15)|, v1;
			if (v13.fm2(map[i1 := v8], fm3(i1, globalState) % i1, i1 / -f20, globalState)) {
				r2 := i1 / f20;
				r0 := i1;
				...
				v18[420] := v19;
				v18[420] := v19;
				v0, globalState.f2, globalState.f1, v0 := false, (-562 + |v6|) + v17[715], v7, false <== !true;
			} else {
				v15 := v15;
				var v20 := DC35('j', v0, v0);
				var v21: map<bool, char> := map[v0 := v7];
				var v22: array<char> := new char[8] [v20.cf67, v15[|v1|], if (v0 in v21) then v21[v0] else 'w', v7, 'p', v7, v7, v7];
				v22[525] := v7;
				v22[525] := v7;
				v2 := v2 * v2;
				var v24: multiset<int> := multiset{i1};
				var v25: multiset<bool> := multiset{v0};
				v0 := !fm2(map v23 : int | (-0x2c6 <= v23) && (v23 < -0x1a2) :: (v23 * f20) := (v8), -(if (f20 in v24) then v24[f20] else v6[|v25|]), 0x236, globalState);
				var v26 := DC21(v10, v0, v0, v22[525]);
				var v27: array<D8> := new D8[8] [v26, DC21(v10, v0, v0, v22[525]), v26, v26, v26, v26, v26, DC21(v12[701], v0, v0, v22[525])];
		...
		if (!match v30 {
			...
			var v31: array<string> := new string[9] [v15, seq(0xbb, i3  => (v7)), if (v0) then v15 else "cukhard", v15, "nggpjgg", v15[|v15| := v7] + v15, v15[f20 := v7], seq(-0x321, i4  => (v7)), seq(0x2ca, i5  => (v7))];
			v31 := v31;
			var v32 := DC61(v9);
			var v33: multiset<bool> := multiset{v0, true};
			var v36: array<D24> := new D24[20] [v32, v32, fm90(([v0, v0, v0, v0])[f20 := v0], v0, [v0, v0], globalState), DC61(v9).(cf115 := (map[f20 := v8])[|v33| := v8]), v32, v32, v32, fm90([v0, v0], !v13.fm2(map v34 : int | v34 in v29 :: (v34 + -0x2bb) := (DC0(v7)), f20, -141, globalState), v1, globalState), DC61(map v35 : int | (0x39b <= v35) && (v35 < 0x165) :: (v35 / f20) := (v8)), DC61(map[f20 := v8]), v32, v32, DC61(v9), fm90(v1, v1[f20], v1, globalState), v32, DC61(v9), v32, DC61(v9), v32, v32];
			v36 := v36;
			var v37 := DC96();
			match (v37) {
				case DC95(cf172, cf173, cf174) =>
					...
					var v45: map<bool, bool> := map[cf174 != v38[42] := v44 != v44];
					var v46 := DC22(!cf172, v38[42], f20);
			...
			v65[418] := v64 * ((if (v66 in v67) then v67[v66] else multiset{0x3c4}) + v64);
			if (true) {
				var v68: map<bool, bool> := map[v0 := v0];
				v68 := (v68 + v68) + map[v0 := v0];
				var v69: array<int> := new int[12];
				v69[59] := v66.fm3(f20, globalState) - 431;
				var v70: seq<seq<int>> := [v6[f20 := f20], v6];
				var v71 := DC37(v1, true, v0);
				var v72: map<D14, string> := map[v71 := v15];
				var v73: map<bool, string> := map[v0 := "brk"];
				v69[59], v0, f20, v70 := (f20 * |v15|) / v62.fm25(v0, v0, f20, globalState), (if (v0) then |v2| else -f20) > 48, 0x3ca, (v70[f20 := v6])[f20 := fm0(f20, if (v71 in v72) then v72[v71] else if (v0 in v73) then v73[v0] else "jpvlpqbq", f20, globalState)] + v70;
				v0 := (v69[59] - 0x1b5) != f20;
				v10[295] := v0;
				v10[295] := v0;
				var v74: C0 := new C0(v13.fm2(v9, f20, v69[59], globalState), |fm36(true, v0, globalState)|);
			...
			match (fm49(f20, globalState)) {
				case DC27(cf51, cf52) =>
					var v77 := new C1();
					v15 := (seq(-702, i6  => (v7))) + v15[f20 := v7];
					v1 := v1[f20 := v0];
					globalState.f2 := cf52;
				case DC28(cf53, cf54) =>
					var v78: map<array<bool>, char> := map[v10 := v15[|v6[f20 := |v15|]|]];
					v78 := v78[v12[701] := 'o'];
					var v79: set<int> := {-v66.fm3(f20, globalState)};
					var v80: seq<set<int>> := [{f20, f20, f20}, v79, {-391}, if (v0) then v79 else {f20}];
					globalState.f1, v80, cf54 := v7, (seq(661, i7  => (v79 + {f20, f20})))[f20 % f20 := v79], ((v15[f20 := 'i'])[-f20 := v7] + "binmsr") <= (v15 + v15);
					var v81: array<int> := new int[7];
					v81[315] := f20 / f20;
					var v82: multiset<seq<int>> := multiset{v6, v6};
					v81[315] := |(fm19(globalState))[v82 !! v82 := v0]|;
				...
				case DC29(cf55, cf56, cf57, cf58) =>
					...
					v84[794] := |v85[true := cf58]| % cf56;
					v84[794] := (cf57 + |map[v0 := cf55]|) % cf57;
					v1 := v1 + [cf55, cf55, cf55, false];
					cf55 := (if (v0) then v15 else v15) == v15;
				case DC26(cf50) =>
					var v86: multiset<bool> := multiset{v0};
					var v87: multiset<array<bool>> := multiset{v10, v12[701], v12[701], v10};
					var v88: set<int> := {f20, f20};
					var v90: multiset<set<bool>> := multiset{cf50};
					var v91: array<int> := new int[27] [f20, f20, f20, -|v15[f20 := v7]|, |v29|, 0x3b9, |v86| * f20, f20, f20 - f20, f20, f20 % f20, v62.fm3(f20, globalState), f20, f20, f20 / f20, f20 / f20, f20, f20, |v87|, f20, 1, 0x80, |map[v0 := v0]|, 0x2a8, 731, f20, |v88| + |(map v89 : set<bool> | v89 in v90 :: (v89) := (|v64|))|];
					globalState.f6 := v91;
				...
				case DC30(cf59) =>
					...
					var v103: array<int> := new int[20] [f20, f20, |"dyi"|, |v3|, f20, f20, |v15|, -f20, |v65[418]| * f20, f20, |v100|, |(v15 + "orpdmnkk")|, f20, |(if (v0) then v15 else v15)|, v95.fm3(0x3ad, globalState), v13.fm3(-859, globalState) * f20, f20, |fm72(|v64|, v0, v101, v0, globalState)|, f20 - |(map v102 : int | v102 in v6 :: (v102 % f20) := (v0))|, f20];
					v96[488] := f20;
					v103[485] := 595;
					var v104 := DC54(v0, v0, f20);
					v96[488], globalState.f2, v103[485] := (f20 % 0x23a) % |v15|, v13.fm3(f20, globalState), v104.cf109;
					v0 := v0;
			}
		...
		var v109: map<array<bool>, int> := map[v12[701] := f20];
		r3 := if (v0) then |v108| else f20 / -|v109|;
	}
	method m2(p0: bool, p1: int, p2: int, globalState: GlobalState) returns (r0: array<int>, r1: bool, r2: seq<bool>) {
		if (!p0) {
			...
			var v6: seq<int> := [f20];
			var v7 := DC1(v6);
			v0, v2 := if (|(v6[p2 := 763] + v6)| in v5) then v5[|(v6[p2 := 763] + v6)|] else v0, fm91(v7, p0, globalState);
			var v8: map<int, seq<int>> := map[f20 := seq(0x210, i0  => (|multiset{p0}|))];
			var v9: seq<map<int, seq<int>>> := [v8];
			var v10: map<int, int> := map[|(v8[f20 := v6[f20 := f20]] + v9[f20])| := p2];
			v10 := v10[|v6| := f20];
			var v11: seq<bool> := [p0];
			var v12: array<bool> := new bool[9] [p0, p0, !p0 && p0, p0, p0, p0, p0, v11[f20], p0];
			v12[84] := p0;
			var v13 := DC11(p0, p0);
			var v14: seq<D5> := [DC11(p0, !p0), v13, v13.(cf18 := p0)];
			var v15: map<char, int> := map[v1 := fm3(p1, globalState)];
			globalState.f2, v12[84], v6, v14 := |v15|, (p2 < f20) in v11, v6, (v14 + (v14 + v14))[f20 % p2 := v13.(cf19 := p0)];
			var v16 := new C8(v12[84]);
			v10 := v10[p1 := p2];
		} else {
			var v17 := new C7();
			var v18: set<int> := {p2};
		...
		v27 := v27 + map[p0 := p0];
		var v28: set<bool> := {p0};
		v24[877] := if (p0) then |v28| else p2;
		v24[877] := p1;
		var v29 := new C8(p0);
		for i2 := if (v29.f23) then -v24[877] else p1 to v24[877] {
			var v30: array<bool> := new bool[28];
			globalState.f5 := v30;
			globalState.f2 := DC89(p2, 0xa5, v24[877]).cf163;
			v28, v30 := v28 + v28, v30;
		...
		r2 := match v34 {
			...
			case DC18(cf35) => if (p0) then ([p0])[p1 := v29.f23] else [v29.f23]
	...
	method m3(p0: seq<bool>, p1: string, p2: int, p3: bool, globalState: GlobalState) {
		...
		if (p3 ==> true) {
			...
			match (DC57(v2)) {
				case DC58(cf112, cf113) =>
					...
					var v8: map<array<bool>, seq<bool>> := map[v7 := fm36(p3, p0[0x363], globalState)];
					v7[294] := v0;
					v8, v0, v7[294] := v8, true, v0;
					var v9 := "pyd";
					v9, globalState.f2, v7[294] := v9, v5[0x287], cf112 == p2;
					var v10 := DC47(v5, v7[294], -732);
					v10 := v10;
				case DC59() =>
					var v11 := 's';
					var v12 := DC0('v');
				...
				case DC57(cf111) =>
					...
					var v23 := 'e';
					var v24 := DC8(|p1|, |v20[p2]|, v0, (map[|v22| := v23])[f20 := v23], p1);
					var v25: C4 := new C4(p3, v24);
					var v26: map<char, C2> := map[v23 := v19];
					v18[666], v19, globalState.f1, v25 := p3, if (v23 in v26) then v26[v23] else v19, v23, v25;
					var v27: multiset<bool> := multiset{v18[666]};
					var v28: map<multiset<bool>, int> := map[v27[v0 := p2] := |p1|];
					v28 := (v28 + v28) + v28;
			}
			...
			v29[531], globalState.f2, v33, f20 := (v32 + v32) + v32, f20, v33, |(multiset{v34.cf172} - v35)|;
		} else {
			var v36 := true;
			v36 := |map[|p0| := f20]| <= f20;
			var v37: C2 := new C2();
			...
			v36, globalState.f1, globalState.f2, f20, v37 := p1 <= (p1 + (seq(0x14a, i0  => ('k')))), v38, |v40|, |(p0 + p0)| / |v42|, v37;
			var v43: C1 := new C1();
			...
			if (!((v45 * v45) >= v45)) {
				...
				globalState.f2, v50[789] := |(v55 + v55)|, v52;
				var v56: array<int> := new int[23](i2 => i2 / f20);
				var v57: multiset<char> := multiset{v38};
				v56[23] := |v57|;
				v56[23] := p2;
				var v58: map<bool, array<bool>> := map[p3 := v39];
				globalState.f2, v56[23], v58, v56[23], v36 := v43.fm3(|p1|, globalState), f20 + f20, v58, p2, p3;
				v36 := p0[|(v54 * v54)|];
				var v59 := DC0(v38);
				var v60: map<int, D0> := map[v56[23] := v59];
				var v61 := new C8(v43.fm2(v60, f20, v56[23], globalState));
			}
		...
		if (p3) {
			...
			match (v89) {
				case DC76(cf138, cf139) =>
					var v90: seq<int> := [p2];
					var v91: map<int, bool> := map[v90[fm3(p2, globalState)] := cf138];
					v91 := v91[|(if (true) then p1 else "edaukoy")| := p0[fm3(228, globalState)]];
				...
				case DC77(cf140, cf141, cf142, cf143, cf144) =>
					cf140 := !cf143;
					var v95: seq<int> := [-884];
					globalState.f2 := v95[cf141] * (p2 - p2);
					var v96 := DC47(v95, cf144, 0x2c2);
					cf144 := !(v96.cf89 <= (if (cf142) then v95 else seq(996, i5  => (p2))));
		...
		for i7 := |(p1 + v119[p2])| to |v121| * p2 {
			...
			v130 := (v131[v129.fm3(f20, globalState)])[v129 := p2 % |p1|];
		}
	...
	method m0(p0: seq<int>, p1: int, p2: string, p3: bool, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
		...
		var v11: array<bool> := new bool[21](i2 => map['e' := map['h' := |multiset{237}|]] == map['y' := map['r' := |p2[f20 := 'v']|]]);
		v11[199] := !(p1 >= f20);
		var v12 := true;
		var v13 := 'v';
		...
		v11[199], v12, v12, r0 := v15 <= v15[p1 := p3], p2 <= p2, v12, |((v16 + v16) + v17[p1])|;
		var v18 := DC21(v11, p3, p3, v13);
		var v19: set<bool> := {v18.cf41};
		var v20: map<set<bool>, bool> := map[v19 := v15[|(seq(795, i3  => (p1)))|]];
		var v21: seq<map<set<bool>, bool>> := [v20];
		var v22: map<map<set<bool>, bool>, bool> := map[v21[|v15|] := true];
		var v25: set<int> := {|map[v12 := p3]|};
		v22 := v22[(map v23 : set<bool> | v23 in (map v24 : set<bool> | v24 in v20 :: (v24) := (f20)) :: (v23) := (!p3)) + map[v19 := v11[199]] := v25 !! v25];
		var v26: array<map<bool, bool>> := new map<bool, bool>[25](i4 => map[v12 := p3] + map[v15[-p1] := v12]);
		f20, globalState.f1, v26 := f20, v13, v26;
		v0[790] := |p2| * |v19|;
		v0[790] := fm3(|"oitqth"|, globalState);
	...
	method m1(globalState: GlobalState) {
		...
		match (v5) {
			case DC27(cf51, cf52) =>
				...
				if (v0 in v10) {
					...
					var v14: array<int> := new int[24] [f20, cf52, |(seq(367, i1  => (0x188)))|, f20, cf52, f20, f20, f20, cf52, cf52, f20, cf52, cf52, cf52, |v7|, cf52, |v13|, cf52, 864, cf52, |v11[585]|, fm3(cf52, globalState), cf52, fm3(cf52, globalState)];
					var v15: multiset<array<int>> := multiset{v14};
					var v16: map<int, int> := map[cf52 := f20];
					var v17: set<int> := {cf52, |v16|};
					var v18: multiset<int> := multiset{|v6[693]|};
					var v19 := DC39(v18);
					var v20: map<bool, char> := map[true := v1];
					var v21 := DC107(v20);
					var v22: map<bool, int> := map[v0 := f20];
					var v23: map<bool, map<int, int>> := map[cf51 := v16];
					var v24: array<int> := new int[25] [cf52, f20, cf52, cf52, f20, cf52, |v7|, |(v15 * v15)|, f20 / f20, ---f20, cf52, 0x350, cf52, |v17| / |v10|, |v7| * fm3(cf52, globalState), f20, cf52 / f20, if (|fm92(v19, globalState)| in v16) then v16[|fm92(v19, globalState)|] else |v21.cf189|, f20, -cf52, cf52, if (v0 in v22) then v22[v0] else cf52, -cf52, |(if (cf51 in v23) then v23[cf51] else v16)|, fm3(|v11[585]|, globalState)];
					v14[715] := f20;
					var v25: seq<multiset<int>> := [v18];
					v14[715], f20, cf52, v25, globalState.f2 := 0x1b3, cf52 * -cf52, fm3(cf52, globalState), if (cf51) then fm23(map[cf51 := v0], DC11(cf51, v0), f20, globalState) else [fm35(!cf51, v0, globalState), v18], cf52 - (f20 % -0xc3);
					globalState.f2 := 0x268;
					v11[585] := ((seq(0x51, i2  => (v14[715]))) + v11[585]) + v12;
					v7 := "mtgddakn";
				} else {
					var v26: array<int> := new int[1];
					v26[48] := cf52;
					v26[48] := |(([v0, cf51] + v10) + (if (v0) then v10 else v10))|;
					var v27: seq<int> := [v26[48]];
					v27 := v27;
					v7 := seq(-5, i3  => (v1));
					var v28 := DC77(v0, v26[48], cf51, v0, v0);
					cf51 := if (false) then fm2(v3, cf52, v28.cf141, globalState) else v0 ==> v0;
					var v29 := DC14(DC12(v6));
					cf51, v6[693], v10, v29 := v0, v6[693][f20 := v1] + "qmlhm", v10, v29;
				}
			...
			case DC28(cf53, cf54) =>
				...
				globalState.f2 := -(f20 / ((if (v0 in v33) then v33[v0] else 0x332) * |v34|));
				var v35: seq<int> := [f20];
				cf54 := (seq(0x360, i4  => (f20))) != ([f20, -0x2c] + v35);
				var v36 := "fgphc";
				v36 := "rmtbia" + v36;
			case DC29(cf55, cf56, cf57, cf58) =>
				if (cf55) {
					...
					v0 := fm2(v3 + v3, 0x169 / f20, |v42|, globalState);
				} else {
					var v43 := "svqg";
					...
					v53 := v53;
					var v54: seq<int> := [f20];
					var v55: seq<int> := [v50, if (v45) then -|fm12(globalState)| else cf57, cf56, v46 * cf56, cf56 / |v54|];
					globalState.f2 := v54[v46];
					var v56: C2 := new C2();
					var v57: seq<C2> := [v56, v56, v56, v56];
					var v58: seq<bool> := [v0];
					var v59 := DC22(v58[cf56], v50, |(seq(301, i5  => (v1)))|);
					...
					var v68: array<int> := new int[25] [cf56, |map[fm3(f20, globalState) := v43]|, |v57[v46 := v56]|, cf57, v53.fm3(v59.cf45, globalState) * (if (v51 in v60) then v60[v51] else f20), -cf56, |(v61 + (map[v0 := v53.f26])[v53.f26 := !v47])|, 961 % |[v53.f26, v51]|, -cf58, |v62|, cf56 * 0x164, cf56, -cf58 - v50, if (|v66| in v63) then v63[|v66|] else cf56, cf57 / f20, f20 + -0x23e, v56.fm25(v49, v45, f20, globalState), cf56, |v58|, v46, v46 * v53.fm3(|v67|, globalState), cf57, 0x17c * 0x164, -cf58, v46];
					globalState.f6 := v68;
				}
				...
				var v77 := DC88(v76);
				var v78: map<int, D34> := map[-v75[cf57] := v77];
				v71 := fm93(|v74|, cf56 * 592, v78, globalState);
			case DC26(cf50) =>
				v0 := v0 <==> (f20 > f20);
				var v79: multiset<bool> := multiset{v0};
				var v80: seq<bool> := [v0, v0, v0, v0];
				var v81: array<multiset<bool>> := new multiset<bool>[7] [v79, fm41(v1, v80[f20], globalState), v79 + v79, multiset{v0, !v0} * v79, v79, v79, multiset{v0} * v79];
				v81[908] := v79;
				var v82: multiset<int> := multiset{-f20, f20};
				var v83: array<bool> := new bool[5];
				v0, v0, globalState.f5, v0, v81[908] := fm2(v3, f20, f20, globalState), !fm2(v3, f20, fm3(|v82|, globalState), globalState), v83, !v0, v79;
				var v84 := "yah";
			...
			case DC30(cf59) =>
				...
				var v93: map<multiset<bool>, int> := map[v92[f20] := f20];
				var v94: T1 := new C5();
				var v95: array<bool> := new bool[14];
				var v96: map<T1, array<bool>> := map[v94 := v95];
				v93 := v93[v91 := f20 % |v96|];
				v95[626] := !v0;
				v95[626] := !(if (v0) then false else v0);
		}
			...
			decreases 100 - i6
		{
			...
			v104 := v104[fm2(v3, if (f20 in v105) then v105[f20] else f20, f20, globalState) := v106[f20]];
			v1 := v1;
			var v107 := "hhdiir";
			var v108: seq<string> := [v107 + v107, v107, v107, v107[0x89 := 'r'] + v107, v107];
			v108 := v108;
		...
		if (!v0) {
			...
			v113 := (map v114 : int | v114 in v115 :: (v114 % f20) := (v0)) + map[f20 := v101.f26];
			var v116 := new C8(f20 > f20);
			v101.f26 := v112[44];
			v101.f26 := v101.f26;
		} else {
			v101.f26 := !v101.fm2(if (v101.f26) then map v117 : int | (0x5c <= v117) && (v117 < 0x232) :: (v117 / |[map[v101.f26 := false], map[v101.f26 := false]]|) := (v2) else v3, f20, f20, globalState);
			var v118 := "qf";
			...
			match (v119) {
				...
				case DC10(cf17) =>
					...
					v125[377] := fm2(v3, v122.fm3(f20, globalState), v126.cf61, globalState);
					v125[377] := v5.cf50 !! {v101.f26};
			}
			...
			v132[888] := fm39(v101.f26, |v133[v127.f22 := f20]|, f20, globalState);
			var v134: map<bool, int> := map[v101.f26 := v127.f22];
			v131, v132[888] := v131, v134;
			globalState.f2 := f20;
		}
	...
	method m6(p0: char, p1: int, p2: bool, globalState: GlobalState) returns (r0: D2, r1: bool, r2: int, r3: bool) {
		...
		if (v0 < v0) {
			...
			var v6: seq<array<seq<bool>>> := [v3, v3, v3, v4[|v5|]];
			...
			r2 := -|(v11 + v11)|;
			r3 := (f20 / p1) == (fm3(f20, globalState) + f20);
			...
			var v18: multiset<int> := multiset{|v13| * f20, f20, -|(v15 + (set v17 : int | v17 in v16 :: (v17 % 0x2c3)))|};
			v16 := v18;
			var v19 := DC42(p1 - |([|v0|, |v0|, p1])[-0x1d0 := f20]|, p1);
			match (v19) {
				case DC42(cf80, cf81) =>
					var v20: array<D12> := new D12[9];
					var v21 := DC32(0x14e, false, v14.f23);
					v20[986] := v21.(cf62 := false);
					v20[986] := v21;
					...
					var v28: array<bool> := new bool[17] [v14.f23, p1 > |v0|, v14.f23 ==> v22[|multiset{v14.f23, v14.f23, v14.f23, !v14.f23}|], multiset{v14.f23} == v23, -|multiset{fm1(v0, v14.f23, p2, globalState), v24}| != cf81, v14.f23, v14.f23, "sulaugp" == fm31(v25, -|v0|, v14.f23, globalState), p1 != p1, !v14.f23, v15 > v15, p2, v14.f23, v14.f23, v14.f23, v14.f23, v26 in v27];
					v28[673] := !v14.f23;
					v28[673] := v15 >= (v15 * v15);
					...
					var v34: seq<seq<int>> := [v29, seq(0x317, i3  => (|[v13, v13]|))];
					var v35: array<seq<int>> := new seq<int>[29] [v29, fm0(|v0[f20 := v0[cf81]]|, v0, cf80, globalState), v29, v29 + (if (v14.f23 in v30) then v30[v14.f23] else v29), v29, [|[0x1ee]|], v29, v29 + v29, [|v29|, |fm55(v31, p2, f20, globalState)|], seq(680, i1  => (if (cf81 in v18) then v18[cf81] else f20)), v29, v29, v29, v29, (if (v32 in v33) then v33[v32] else v29) + v29, v29, v29, v29, v29, [cf81, |v23|, f20], if (p2) then if (v28[673] in v30) then v30[v28[673]] else v29 else v29, seq(0x1dc, i2  => (0x3b4)), v29, v34[f20], if (v14.f23) then v29 else v29, [f20, cf81], (v29 + v29)[-p1 := 0x6], v29, v29];
					v35 := v35;
					...
					var v42, v43 := v14.m0(v29, |v15|, v0, v40 !! v41[cf81], globalState);
				case DC43(cf82, cf83, cf84, cf85) =>
					...
					v45[431] := p2;
					v45[431] := cf84;
					var v46: array<int> := new int[15](i4 => i4 / cf83);
					v46[511] := p1;
					var v47: array<string> := new string[27](i5 => v0);
					v47[432] := v0 + v0;
					var v48: T0 := new C1();
					var v49: map<T0, string> := map[v48 := v0];
					v47[385] := if (v48 in v49) then v49[v48] else "qdm";
					globalState.f2, v46[511], v0, v47[432], v47[385] := 513, p1, v0 + v0, v0 + "otigq", v0;
					var v50: C2 := new C2();
					v50 := v50;
					globalState.f2 := v46[511] * v46[511];
				case DC44(cf86) =>
					var v51: multiset<bool> := multiset{p2, p2};
					v14.f23 := !((multiset([v14.f23]) - multiset{p2, v14.f23, !cf86, p2, !cf86}) >= (v51 - v51));
					var v52: array<int> := new int[1](i6 => i6 / -f20);
					v52[541] := f20 - f20;
				...
				case DC41(cf79) =>
					...
					var v58: seq<int> := [p1];
					var v59: map<int, string> := map[v58[f20] := v0];
			...
			if (699 in {0x187, p1, f20, |v64|, p1}) {
				...
				var v67: array<C8> := new C8[10] [v14, v14, v14, DC113(v14).cf196, v66[p1], v14, v14, v14, v14, v14];
				var v68: map<int, bool> := map[p1 := !p2];
				var v69 := DC0(p0);
				var v70: map<int, D0> := map[p1 := v69];
				globalState.f2, v65, v67, v14.f23 := f20, v65, v67, !v14.f23 <== ((if (|(seq(727, i7  => (p0)))| in v68) then v68[|(seq(727, i7  => (p0)))|] else fm2(v70[f20 := v69], p1, f20, globalState)) || false);
				var v71: C9 := new C9();
				v71 := v71;
				r2 := f20;
				var v72: seq<int> := [f20];
				...
				var v80: seq<bool> := [!((v18[p1 := p1])[|"yaqwl"| := f20] > multiset(v79))];
				r3 := v80[p1];
				v14.f23 := v14.f23;
		...
		var v88: array<int> := new int[13] [p1, p1, p1, -(p1 / f20), 0x9c, --0xd6, -p1, 0x163, p1, f20, f20 * p1, |(v84 * ((multiset(if (p2 in v86) then v86[p2] else [v83, v83]))[[v82, v82] := v87[p1]])[[v82, v82] := f20])|, |v0| * f20];
		var v89: map<char, int> := map[p0 := p1];
		var v90 := DC116(DC115(p2, v89, p2));
		var v91 := DC116(v90);
		var v92: C7 := new C7();
		var v93: map<int, int> := map[|map[v91 := |map[p1 := v92]|]| := -f20];
		v88[425] := p1 / (if (|v0| in v93) then v93[|v0|] else |v0|);
		var v94: array<bool> := new bool[6];
		var v95: multiset<array<bool>> := multiset{v94, v94, v94, v94};
		v88[425] := |(v95 - (multiset{v94} + multiset{v94}))|;
		var v96, v97, v98 := v92.m2(p2, -f20, |v0| + 0x2e7, globalState);
		v88[425] := (p1 / f20) / (f20 + p1);
	...
	method m7(p0: int, p1: int, globalState: GlobalState) returns (r0: int, r1: bool, r2: bool, r3: bool) {
		...
		for i0 := p1 to |v0| * p0 {
			...
			var v9 := 'o';
			var v10: array<string> := new string[13] [v0, v0, v0 + "nwcnbhao", (if (f20 in v7) then v7[f20] else v0) + v0, v0, "sju", (v8[i0])[0x13c := v9], v0 + v0, seq(466, i1  => (v9)), v0, v0, seq(0x3a6, i2  => (v9)), v0];
			v10[628] := v0;
			v10[628] := "ykkro";
			r1 := v3;
		}
		...
		var v16: array<int> := new int[24] [|(seq(298, i3  => (p0)))[|v0| := f20]|, |(fm96(v11, v12, |(seq(279, i4  => (v11)))|, globalState)).cf103|, p0, |v13|, f20, p1, -|(seq(539, i5  => (v11)))|, f20, f20, -0x29b, f20, |v14|, -0x2bb, p1, p1, p0, p0, fm3(p1, globalState), |v13|, p1, 0x26a, f20, |v15|, p1];
			...
			decreases 100 - i6
		{
			...
			if (v12) {
				...
				f20 := p0;
				globalState.f2 := |(v14 + (seq(0x6a, i7  => (-0xb7))))|;
			} else {
				...
				var v31: map<array<bool>, string> := map[v29 := v0 + v30[f20]];
				var v32: seq<array<bool>> := [v29];
				var v33: map<int, array<bool>> := map[p1 := v29];
				var v34: array<array<bool>> := new array<bool>[28] [v29, v29, v29, v29, v29, v29, v29, v29, v29, v29, v29, v29, if (v25[632]) then v29 else v29, v32[p0], if (v25[632]) then v29 else v29, v29, v29, v29, v29, if (v12) then v29 else v29, v29, v29, v29, v29, v29, v29, v29, if (p0 in v33) then v33[p0] else v29];
				v34[172] := v29;
				r2, v31, v34[172], r3 := v25[632], v31, v29, v12 ==> false;
				var v35: T0 := new C2();
				var v36: map<T0, map<bool, int>> := map[v35 := v23];
				r2 := if (v12 && v12) then v35 in v36 else v25[632];
				v29 := v29;
				var v37 := new C3();
			}
		...
		r1 := v12;
		r2 := v13[if (true) then p1 else p1];
...
method Main() {
	...
	if (!(v6 <= fm0(v5, v10, |v15|, globalState))) {
		...
		v17 := v18;
		v10 := v10 + (v10 + v10);
		v0[439] := fm1(v10, false, true, globalState) !! v3;
		v0[439] := v2;
	...
	v1[863] := v25.fm3(v5, globalState) + v25.fm3(v23.fm3(177, globalState), globalState);
	var v26: set<int> := {-|v12|, v5, 418};
	var v27: map<set<int>, bool> := map[v26 := v2];
	var v28: map<seq<bool>, int> := map[v12 := v5];
	globalState.f2, globalState.f6, v25, v1[863], v27 := |v28[v12 := v5]|, v1, v25, (v5 - v5) % v5, map[v26 := v25.fm2(map[v5 := v14], v5, v5, globalState)] + v27;
	v0[895] := !!!v2;
	v0[895] := v25.f23;
	var v29 := DC85(v25.f23);
	match (v29) {
		case DC85(cf155) =>
			v25.f23 := !((v1[863] / v1[863]) <= v25.fm3(0x2cc, globalState));
			v25.f23 := v5 == (v23.fm3(|v12|, globalState) / -v5);
			var v30, v31 := v25.m0(v6, v5 / v1[863], v10 + v10, v2 in v7, globalState);
			var v32: map<bool, multiset<char>> := map[v0[895] := v15];
			var v33: map<bool, map<bool, multiset<char>>> := map[cf155 := v32 + v32];
			v33 := v33[cf155 := v32 + v32];
		case DC86(cf156, cf157, cf158) =>
			var v34 := DC35(v10[v1[863]], v25.f23, v0[895]);
			globalState.f1 := v34.cf67;
			var v35: map<bool, int> := map[if (v25.f23) then false else v25.f23 := cf158 % v1[863]];
			v35 := v35[v25.f23 := cf158];
			v0[895] := (cf157 + cf157) > v5;
			v2 := v25.f23;
		case DC87(cf159) =>
			var v36: C3 := new C3();
			var v37 := DC121(v36);
			var v38: map<bool, C3> := map[v0[895] := v37.cf213];
			var v39: set<string> := {v10};
			var v40: map<int, char> := map[-v5 := v13];
			var v41 := DC8(v6[v1[863]], v1[863], v25.f23, v40, [v13, v13]);
			var v42 := DC8(v5, v1[863], v25.f23, v41.cf12, v10);
			var v44: seq<set<string>> := [set v43 : string | v43 in multiset([fm31(v41, v1[863], v25.f23, globalState)]) :: (v43)];
			var v45, v46 := v25.m0(v6, |v38|, v10, v39 > v44[v1[863]], globalState);
			v45 := -((v45 / v1[863]) * |(seq(676, i1  => (v13)))|);
			v25.f23 := v12 != v12;
			v1[863] := 0x6e;
		case DC84(cf154) =>
			var v47: array<string> := new string[17](i2 => v10);
			v47[300] := v10[v1[863] := v13];
			v47[300] := v10;
			var v48: C5 := new C5();
			var v49 := DC110(v48);
			var v50: map<int, C5> := map[v1[863] := v48];
			var v51: array<C5> := new C5[28] [v48, v48, v48, v48, v48, v48, v48, v48, v48, v48, v49.cf193, v48, v48, v48, v48, v48, v48, v48, v48, v48, v48, if (v1[863] in v50) then v50[v1[863]] else v48, v48, v48, v48, v48, v48, v48];
			v51[57] := v48;
			v51[57] := v48;
			match (fm97(v5 * v1[863], globalState)) {
				case DC58(cf112, cf113) =>
					globalState.f1 := v47[300][cf112];
					var v52: map<bool, bool> := map[v25.f23 := v0[895]];
					var v53: map<int, string> := map[-cf112 + |v52| := v47[300] + v10];
					v53 := v53[v1[863] * v1[863] := v47[300] + v10[|v47[300]| := v13]];
					var v54: array<seq<bool>> := new seq<bool>[7](i3 => [v25.f23] + [v25.f23]);
					var v55: C7 := new C7();
					var v56: map<int, D0> := map[cf112 := v14];
					v54, v2, v55, v25.f23 := v54, !!v0[895], v55, v48.fm2(v56, v1[863], if (true) then v1[863] else |(map v57 : int | (-132 <= v57) && (v57 < 678) :: (v57 % -0x2b1) := (v1[863]))|, globalState);
					globalState.f2 := v1[863];
				case DC59() =>
					v0[895] := v25.f23;
					v6 := (v6 + v6) + v6;
					v47[300] := v47[300];
					v0[895] := v25.f23;
				case DC57(cf111) =>
					globalState.f2 := v1[863] * v5;
					v47[300] := v10;
					var v58: C2 := new C2();
					v58 := v58;
					var v59 := DC17(v25.f23, -538, v25.f23, v25.f23);
					var v60: set<string> := {fm72(v1[863], v25.f23, v59, v25.f23, globalState), v47[300], v47[300], v47[300], v10};
					v2 := v47[300] in v60;
			}
			
			var v61 := DC21(v0, v25.f23, !v0[895], v13);
			v13 := v61.cf43;
	}
	...
	match (v62) {
		case DC27(cf51, cf52) =>
			...
			v63 := v63[v10 := v13];
			v2 := if (true) then -v1[863] <= cf52 else v25.f23;
			...
			var v69: map<bool, int> := map[v25.f23 := v1[863]];
			v67 := v67[v68 := if (v0[895] in v69) then v69[v0[895]] else cf52];
		case DC28(cf53, cf54) =>
			var v70: map<int, D0> := map[v6[v5] := v14];
			v25.m3([false, !v25.fm2(v70, v1[863], |v12|, globalState)], v10 + v10, v1[863] % v1[863], v25.f23, globalState);
			var v71: map<array<int>, int> := map[v1 := v1[863] * v5];
			var v72: map<seq<bool>, array<int>> := map[v12 := v1];
			v71 := v71[if (cf54) then if (v12 in v72) then v72[v12] else v1 else v1 := v1[863]];
			var v73 := new C8(v25.f23);
			var v74 := DC81(multiset{v6});
			var v75: seq<D31> := [v74, v74];
			v0[895], v25.f23 := [v74] <= (v75[-v1[863] := v74] + v75), if (!v25.f23) then v73.f23 else v25.f23;
		case DC29(cf55, cf56, cf57, cf58) =>
			var v76 := DC81(multiset{v6, v6, v6, v6, v6});
			match (v76) {
				case DC81(cf147) =>
					globalState.f2, globalState.f6, cf55 := v23.fm3(if (v1[863] in v9) then v9[v1[863]] else cf57, globalState), v1, v25.f23;
					v25.f23 := cf55;
					var v77 := new C2();
					var v78: array<array<set<int>>> := new array<set<int>>[20];
					var v79: C5 := new C5();
					var v80: map<C5, set<int>> := map[v79 := {v1[863], 849, 893, v5}];
					var v83: map<bool, bool> := map[cf55 := v25.f23];
					var v84 := DC13(("ygokhfds")[cf58 := v13], v0[895], v13, false);
					var v85 := DC14(v84);
					var v86 := DC14(v84);
					...
					var v91: array<set<int>> := new set<int>[18] [v26, v26, v26, v26, v26, v26, v26, {v1[863], cf56}, v26, if (v79 in v80) then v80[v79] else set v81 : int | (-0x13c <= v81) && (v81 < -179) :: (v81 / cf56), v26, set v82 : int | (-108 <= v82) && (v82 < 0x2b8) :: (v82 * v1[863]), {|v83|, |v12|, cf56}, v26, {|[v25.f23, v25.f23]|, cf58, |v10|}, fm38(v2, globalState), if (DC120(v86, v87, v25.f23, v25.f23, v88) in v90) then v90[DC120(v86, v87, v25.f23, v25.f23, v88)] else v26, {|v9|}];
					v78[935] := v91;
					var v92: map<bool, string> := map[v25.f23 := seq(0x249, i4  => (v13))];
					var v93: map<int, string> := map[cf58 := v10];
					var v94: seq<string> := [v10 + v10];
					cf55, cf58, v10, v78[935], v1[863] := cf58 < cf56, |(if (v25.f23 in v92) then v92[v25.f23] else v10)| + v25.fm3(v88.f20, globalState), if ((v1[863] - v79.fm3(|v9|, globalState)) in v93) then v93[v1[863] - v79.fm3(|v9|, globalState)] else seq(0x1c, i5  => (DC13(v10, v25.f23, v13, cf55).cf23)), v91, |v94[if (|v83| in v9) then v9[|v83|] else v1[863]]|;
			}
			...
			var v98: seq<map<int, C0>> := [v97, map[v1[863] := v96]];
			v2, v0[895], v1[863], v25, cf58 := v25.f23, "u" != (seq(0x1b8, i6  => (v13))), v5, v25, |(v98 + v98[v5 := v98[|{v13}|]])|;
		case DC26(cf50) =>
			var v99: map<int, bool> := map[0x55 := v10 <= v10];
			v99 := v99[v1[863] / |v12| := v25.f23];
			var v100: array<string> := new string[10](i7 => "ysa");
			v100[747] := v10;
			v100[747] := v10;
			v25.f23 := v12[|(seq(0x2cd, i8  => (v13)))| * v1[863]];
			var v101: array<C5> := new C5[15];
			var v102: C0 := new C0(true, |map[v25.f23 := v101]|);
			var v103: seq<C0> := [v102];
			var v104: map<seq<C0>, int> := map[v103 + v103 := v1[863]];
			v104 := (v104 + v104) + v104;
		case DC30(cf59) =>
			if ((-v1[863] + v5) >= (|v12| - v5)) {
				var v105: C0 := new C0(v0[895], v1[863]);
				var v106 := DC83(v105, v1[863], v13, v105.f22, v13);
				var v107: map<int, int> := map[v1[863] := v106.cf150];
				globalState.f2 := if ((|(seq(-0x12a, i9  => (v105.f22)))| - v5) in v107) then v107[|(seq(-0x12a, i9  => (v105.f22)))| - v5] else v5;
				var v108: map<bool, bool> := map[false := !v12[-0x20b]];
				v0[895] := v2 || (if (v105.f21 in v108) then v108[v105.f21] else v25.f23);
				var v109: map<int, string> := map[v1[863] := v10];
				v25.m3(v12, if (|v3| in v109) then v109[|v3|] else v10, v5, v15 == v15[v13 := -v1[863]], globalState);
				v25.f23, v1[863], globalState.f2 := v25.f23, |v12|, v1[863];
				var v110 := new C2();
			} else {
				v25.f23 := !v25.f23;
				var v111: array<set<bool>> := new set<bool>[10] [v3, v3, v3, v3, v3, v3, v3, v3, v3, {v25.f23}];
				var v112: seq<array<set<bool>>> := [v111];
				var v113: map<int, int> := map[v1[863] := v5];
				var v114: map<bool, array<set<bool>>> := map[v0[895] := v111];
				var v115: array<array<set<bool>>> := new array<set<bool>>[25] [v111, v112[v1[863]], v111, v111, v111, v111, v111, v111, v111, v111, v111, v111, v111, v111, v111, v111, v112[v25.fm3(|v113|, globalState)], if (v25.f23 in v114) then v114[v25.f23] else v111, v111, v111, if (v0[895] in v114) then v114[v0[895]] else v111, v111, v111, v111, v111];
				v115[219] := v111;
				v115[219] := v111;
				var v116: C2 := new C2();
				var v117 := DC78(v116);
				...
				var v120: array<C2> := new C2[20] [v116, v117.cf145, v116, v116, v116, v116, v116, v116, v116, v116, v116, v118[|v12|], if (v116 in v119) then v119[v116] else v116, v116, v116, v116, v116, v116, v116, v116];
				v120[16] := v116;
				var v121: map<int, array<bool>> := map[v116.fm25(v0[895], v25.f23, v1[863], globalState) := v0];
				var v122: seq<string> := ["cnafjogt", v10, "oofj", v10, seq(0x199, i10  => (v13))];
				v120[16], v1[863], globalState.f5, v1[863], globalState.f2 := if (v25.f23) then v116 else v116, v5 - |v10|, if (v1[863] in v121) then v121[v1[863]] else if (!v2) then if (v5 in v121) then v121[v5] else v0 else v0, |v122[v1[863]]| + (0x117 / v1[863]), v5;
				v25.m1(globalState);
				v25.f23 := "b" == v10;
			}
			...
			v123 := v123[v13 := v1[863]];
			globalState.f2 := v1[863] / (v23.fm3(v1[863], globalState) % v5);
	}
	
	var v124: set<set<bool>> := {v3, {true, !false}, v3, v3, v3};
	v124 := v124;
	v0[895], v0[895], v0[895], v25.f23 := true <==> (-v5 >= v1[863]), v25.f23, |fm0(v5, v10, v1[863], globalState)| != (if (v25.f23 in v7) then v7[v25.f23] else v5), (v26 + v26) !! v26;
	if (v25.f23) {
		v10 := "ge";
		globalState.f2 := -v25.fm3(v5, globalState);
		var v125 := new C8(v5 >= -v5);
		if ((v9 - v9) > (v9 - multiset{|v26|})) {
			v5 := if (v25.f23) then v1[863] else v5 + v5;
			globalState.f6 := v1;
			v25.m1(globalState);
			var v126: map<int, seq<int>> := map[0x2b2 := v6];
			var v127, v128 := v25.m0(if (v5 in v126) then v126[v5] else v6, v1[863] - v1[863], v10, v125.f23, globalState);
			v5 := v1[863];
		} else {
			v2, v25.f23, v0[895] := !v25.f23, v25.f23, v2;
			globalState.f1, globalState.f2 := fm17(globalState), |v4|;
			v25.f23 := v2;
			v0[895] := v0[895];
			var v129: map<int, int> := map[v5 := |{!v25.f23, v125.f23, v0[895], !v25.f23}|];
			v125.f23 := !(map[v5 := v129] == map[v5 := map v130 : int | (-0x3da <= v130) && (v130 < -0x3c9) :: (v130 % v5) := (-v1[863])]);
		}
		...
		if (v25.f23) {
			...
			v125.m3(fm74(|v132|, v5, 0xa9, globalState), v10, v5 * v1[863], v2, globalState);
			var v133: T1 := new C8(v125.f23);
			var v134: map<int, T1> := map[v5 / v1[863] := v133];
			var v135: seq<multiset<int>> := [v9 - v9, v9, v9, multiset{-v1[863]}];
			v134, v135, globalState.f2, v23 := v134 + v134, v135 + v135, v1[863], v23;
			var v136: C3 := new C3();
			v136 := new C3();
			var v137: map<char, int> := map[v13 := v1[863] / v1[863]];
			v137 := v137;
		} else {
			var v138 := DC79();
			v138 := v138;
			var v139: map<bool, bool> := map[v0[895] := v25.f23];
			var v140, v141, v142 := v125.m2(v1[863] >= v1[863], |[v139]|, v5, globalState);
			var v143: map<int, bool> := map[v1[863] := v125.f23];
			v141 := v5 in v143;
			var v144: map<int, D0> := map[v5 := v14];
			v143 := v143[0x35f - v5 := v23.fm2(v144, v1[863], v1[863], globalState)];
			var v145 := new C1();
		}
	...
	var v150 := DC8(v1[863], 0x185, v2, v149, seq(0x3b8, i11  => (v13)));
	var v151: T2 := new C4(v25.f23, v150);
	var v152 := DC3(v25.f23, -381);
	var v153: C6 := new C6(true, v152);
	var v154: map<T2, C6> := map[v151 := v153];
	var v155: map<bool, int> := map[v0[895] := |map[|v154| := v5]|];
	v25.f23 := |v155| <= -0xcf;
	for i12 := v5 * |v10| to |v12| {
		v5 := -0x2fe;
		var v156: map<set<bool>, int> := map[v3 := |v6[|"a"| := i12]|];
		v1[863] := if (i12 in v9) then v9[i12] else v6[v23.fm3(if (v3 in v156) then v156[v3] else v5, globalState)];
		var v157: array<map<int, int>> := new map<int, int>[7](i13 => map[-v5 := v1[863]] + map[|v10| := 785]);
		var v158 := DC13(v10, false, v13, v25.f23);
		v157[687] := fm24(v1[863], v158, 0x32, globalState);
		var v159: map<int, int> := map[v1[863] := v1[863]];
		var v160: map<bool, bool> := map[v25.f23 := false];
		v157[687], v5, globalState.f2, v2, globalState.f2 := fm24(-v5, v158, i12, globalState) + (if (v2) then v159 else map[v5 := v5]), -(|v160| * i12) / v1[863], v25.fm3(0x317, globalState), if (v10 <= v10) then v153.f26 else v2, v23.fm3(i12, globalState) % |v12|;
		globalState.f2 := -(v1[863] - (v5 % v1[863]));
	}
}