...
function fm1(p0: bool, p1: bool, p2: int, globalState: GlobalState): bool {
	(multiset{"cjuvb"} + multiset{seq(0x135, i0  => ('u'))}) >= (multiset{"tpfhnsqey", "txfv", seq(732, i1  => ('v')), "utuau", "ay"} * multiset{seq(0x6d, i2  => (DC15('a', false).cf28))})
}
...
function fm3(p0: seq<bool>, p1: int, p2: char, p3: bool, globalState: GlobalState): seq<string> {
	(["vsc"] + ["cov", "vx"]) + ((seq(0x21e, i0  => ("gk"))) + ["ywtrdjrjo"])
}
...
function fm26(p0: bool, p1: int, p2: int, globalState: GlobalState): seq<char> {
	seq(491, i0  => (if (!false) then 'q' else 'l'))
}
...
function fm29(globalState: GlobalState): string {
	match DC41([seq(0xa9, i0  => ('e'))]) {
		case DC42(cf63, cf64) => "krsbrr" + (seq(0x49, i1  => ('e')))
		case DC43(cf65, cf66, cf67, cf68) => "dgeq"
		case DC41(cf62) => "wkuahmwrp" + (seq(0x3d9, i2  => ('d')))
	}
}
function fm30(p0: int, p1: bool, p2: int, p3: int, globalState: GlobalState): multiset<map<int, bool>> {
	multiset{map[-0x1a9 := false] + map[|multiset([!true, false, true, true, true])| := true], DC66(map v0 : int | v0 in [-720, |(seq(0x3ab, i0  => ('g')))|] :: (v0 * |(seq(-0x109, i1  => ('l')))|) := (!false)).cf107}
}
...
function fm32(globalState: GlobalState): map<bool, int> {
	map[true := |(map v0 : int | (-576 <= v0) && (v0 < 0x2a5) :: (v0 + |DC35(multiset{false, true}).cf56|) := (|{true, true}|))| / |{false}|]
}
...
function fm35(p0: bool, p1: map<bool, char>, globalState: GlobalState): map<int, int> {
	if (false) then (map v0 : int | v0 in multiset{601} :: (v0 % 214) := (289)) + (map v1 : int | (-0x2fd <= v1) && (v1 < 0x1e2) :: (v1 * |[true]|) := (-0x26b)) else map[|"dshb"| := -0xc4] + (map v2 : int | (-548 <= v2) && (v2 < 0x30f) :: (v2 + |multiset{-0x1c1, -0x145}|) := (952))
}
...
function fm38(p0: bool, p1: int, p2: multiset<int>, p3: map<bool, int>, globalState: GlobalState): string {
	(seq(0x27, i0  => ('h'))) + ("vbu" + "nlkkuwfva")
}
function fm41(p0: bool, p1: char, p2: bool, globalState: GlobalState): string {
	("cxrxnd" + "irrwukcqv") + ("snynaanoq" + (seq(0x11f, i0  => ('h'))))
}
function fm42(globalState: GlobalState): seq<int> {
	([|multiset(seq(0x3a0, i0  => (|map[false := {-|map[114 := |[false, false]|]|, 729, |map[0x3c4 := 0x2bd]|, 0x3d9, |[|[-0x32c]|]|}]|)))|] + [|"skeep"|, 0x195]) + [|map['n' := false]|, -156]
}
function fm43(globalState: GlobalState): multiset<seq<int>> {
	multiset([[|map[false := 0x1c8]|]]) * (multiset{[|[[true]]|]} - multiset{[|"uaj"|], [|[true]|, |"igtvf"|], [-|"jk"|], seq(0x59, i0  => (925))})
}
function fm44(p0: int, p1: seq<int>, globalState: GlobalState): set<bool> {
	{false, false, !((seq(0xfd, i0  => ("vobys"))) == (seq(374, i1  => ("gxrprxa"))))}
}
...
function fm46(globalState: GlobalState): D2 {
	DC5(if (false) then 'e' else 'b', !(0x1ac <= 916), -0x371, |("rotjhxp" + (seq(0x3bf, i0  => ('u'))))|)
}
function fm47(globalState: GlobalState): char {
	'y'
}
function fm48(globalState: GlobalState): seq<seq<int>> {
	if (false || true) then DC67(DC67([[|{|{0x36e}|, |[!false]|}|]]).cf108).cf108 else seq(31, i0  => ([|{true, true, true, true}|]))
}
...
function fm50(p0: D2, p1: int, p2: bool, globalState: GlobalState): map<int, int> {
	map[|map[0x16e := !true]| := 406] + (if (!false) then map[|[|{true, true, !false}|]| := |map[|(set v0 : int | (0x10b <= v0) && (v0 < 0x4d) :: (v0 % |"wowcjj"|))| := true]|] else map[|map[[false, false] := 0x35a]| := 31])
}
...
function fm52(p0: bool, globalState: GlobalState): multiset<int> {
	multiset(seq(0x4f, i0  => (|map[|(seq(0x31e, i1  => ('n')))| := true]|))) - multiset(seq(-0x148, i2  => (|multiset{false}|)))
}
...
function fm55(p0: char, p1: bool, p2: bool, p3: bool, globalState: GlobalState): map<bool, seq<D0>> {
	match DC54({false}) {
		case DC55(cf86, cf87, cf88) => map[cf88 := seq(465, i0  => (DC0(true, cf86, cf86)))] + map[cf88 := [DC0(cf87, cf86, |{cf87}|), DC0(cf87, |(seq(-0x2b5, i1  => (cf86)))|, cf86)]]
		case DC54(cf85) => map[!true := [DC0(true, 0x306, |{286, |(seq(0x388, i2  => ('p')))|}|)]] + map[true := [DC0(false, --|cf85|, 903)]]
	}
}
function fm56(globalState: GlobalState): set<seq<int>> {
	(set v1 : seq<int> | v1 in (map v0 : seq<int> | v0 in [seq(0x94, i0  => (|"lll"|)), [17], seq(0xc8, i1  => (|{--0x2cf, -0x187, -0x149}|)), [|[620, |"caiutom"|]|]] :: (v0) := (false)) :: (v1)) * {[0x2fd, 590], [|[-0xc3, |map[false := |[true]|]|, |multiset{|[false, true]|, 901}|]|, |[false, !true]|, --526, |map[0xfd := |multiset([774])|]|, |"xvljky"|]}
}
...
function fm58(p0: D1, p1: string, p2: bool, globalState: GlobalState): D1 {
	match DC39(map["ofia" := true]) {
		...
		case DC40(cf61) => if (true) then DC1(seq(0x321, i0  => ('m'))) else DC1("bcuy")
	}
}
...
function fm62(p0: int, p1: int, p2: map<int, int>, globalState: GlobalState): D7 {
	DC16(if (true) then true else !false, -915 - |[-0xc2]|, [false, !!false] == [false], true !in [true, false], !(multiset{0x320} == multiset{|(seq(-0xd5, i0  => (0x47)))|}))
}
...
function fm65(p0: bool, p1: map<int, multiset<bool>>, p2: int, p3: int, globalState: GlobalState): seq<multiset<int>> {
	seq(0x361, i0  => (multiset([827] + [0x26, -0x363])))
}
...
function fm68(globalState: GlobalState): map<seq<int>, bool> {
	match DC17(DC16(true, |"jdresf"|, true, true, true)) {
		case DC15(cf28, cf29) => map[[0x11c] := !cf29] + (map v0 : seq<int> | v0 in (map v1 : seq<int> | v1 in [seq(298, i0  => (-0x2fb)), [0x2f4, DC57(|map[cf29 := |(seq(746, i1  => (cf28)))|]|, 839).cf90, |multiset(seq(-876, i2  => (|"uqbpxmqu"|)))|, 650], [0xbb, |multiset([|map[|{cf28}| := cf29]|])|]] :: (v1) := ('r')) :: (v0) := (cf29))
...
method m10(p0: seq<int>, p1: bool, globalState: GlobalState) returns (r0: array<string>, r1: bool) {
	...
	var v6 := 660;
	var v7: map<int, bool> := map[|v4| := v5[v6]];
	var v8: set<int> := {fm0(|(set v3 : int | v3 in p0 :: (v3 % 549))|, p1, v7, v6, globalState)};
	var i1 := 0;
	while (v8 >= v8)
		decreases 100 - i1
	{
		...
		decreases 100 - i2
	{
		...
		v27[300] := map[if (p1 in v28) then v28[p1] else |p0| := |v4|] + v31;
		v27[300] := v31;
		...
		r1, v32[658], globalState.f11 := p1, p1, (if (v6 in v33) then v33[v6] else v6) - (-|v35| - 0x231);
		globalState.f11 := 415;
		var v36: array<int> := new int[18](i4 => i4 - v6);
		var v37 := DC2(v36, v29.f31[0x359], if (v6 in v27[300]) then v27[300][v6] else v6, !fm1(v23.f28, v23.fm7(v28, globalState), v6, globalState));
		r1, r1, v5, v22 := v32[658], (v4 + v29.f31) <= ("w" + v4), v5, v37.cf5;
	}
	...
	var v44 := DC18(v40[fm0(v6, p1, v7, |map[v43 := v6]|, globalState)]);
...
class C2 extends T0 {
	...
	function fm18(p0: int, globalState: GlobalState): bool {
		multiset((seq(-0x272, i0  => (869))) + (seq(0x3a8, i1  => (DC8(false, |[false]|, -|map['r' := 'j']|).cf17)))) !in (map v0 : multiset<int> | v0 in map[multiset{823, -334} := true] :: (v0) := ([true]))
	}
	...
	method m7(p0: map<bool, bool>, p1: bool, p2: int, globalState: GlobalState) returns (r0: bool, r1: bool, r2: char) {
			...
			decreases 100 - i1
		{
			...
			v1[76] := p2;
			var v2 := 'w';
			var v3 := DC6(DC5(v2, true, p2, v1[76]));
			var v4: seq<bool> := [p1];
			var v5: set<bool> := {p1};
			var v6: map<int, int> := map[|v4| := |v5|];
			v3 := v3.(cf14 := DC3(v6));
			var v7 := DC5(v2, fm1(p1, !fm1(p1, p1, p2, globalState), v1[76], globalState), -0x365, 0x1e8);
			var v8: multiset<map<int, int>> := multiset{v6, v6};
			var v9: map<bool, int> := map[true := p2];
			match (v7.(cf10 := v2, cf13 := -(|v8| * |v9|))) {
				case DC4(cf9) =>
					globalState.f22 := v1[76];
					r0 := cf9;
					var v10: map<int, bool> := map[-p2 := fm19('o', globalState)];
					var v11: seq<int> := [-v1[76], |p0[p1 := cf9]| / v1[76], fm0(p2, p1, v10, |v6|, globalState)];
					globalState.f11 := v11[p2];
					var v12: array<array<int>> := new array<int>[9];
					var v13 := DC5(v2, p1, 351, |f29|);
					v12, cf9, v3 := v12, !cf9, DC6(v13);
				case DC5(cf10, cf11, cf12, cf13) =>
					v1 := if (v4[v1[76]]) then v1 else v1;
					var v14: map<char, int> := map[cf10 := v1[76]];
					var v16: seq<int> := [|v6|, cf13];
					var v17: map<int, bool> := map[v1[76] := cf11];
					...
					globalState.f11 := -|v21|;
				case DC3(cf8) =>
					globalState.f22 := 0x275 / p2;
					globalState.f10 := p1;
					v2 := fm21(f29, p1, p2 - 187, v1[76] + -v1[76], globalState);
					globalState.f11 := v1[76];
				case DC6(cf14) =>
					v9 := v9[true := p2 + p2];
					v0[92] := !(p1 <==> p1);
					var v22: seq<int> := [-384];
					v0[92], globalState.f22 := p1, -fm0(|multiset(v22)|, p1, map[-p2 := p1], p2, globalState);
					v1[76] := p2 * |multiset(v4)|;
					globalState.f16 := (0x128 % |v6|) * -790;
			}
		...
		if (!p1) {
			...
			var v30: map<array<map<C0, bool>>, int> := map[v28 := |v29| / p2];
			v30 := v30[v28 := p2];
			globalState.f22 := p2;
			var v31 := DC9(v24);
			v31 := v31;
			...
			var v35: map<bool, string> := map[true := seq(622, i2  => (v23))];
			...
			var v42 := DC0(v39.f24, |(seq(86, i3  => (v23)))|, p2);
			var v43: multiset<bool> := multiset{v39.f24};
			var v44: map<bool, seq<D0>> := map[v39.f24 := seq(-0x1ae, i4  => (v42))];
...
class C3 extends T0, T3 {
	...
	function fm6(p0: D0, globalState: GlobalState): int {
		|map[|map[|(seq(421, i0  => (f27)))| := f24]| := f25]| * (-966 - |{f26}|)
	}
	function fm4(p0: multiset<bool>, p1: string, globalState: GlobalState): D0 {
	...
	method m2(p0: int, p1: D0, p2: bool, globalState: GlobalState) returns (r0: bool, r1: bool, r2: int, r3: int) {
			...
			decreases 100 - i1
		{
			...
			v0[214] := !(if (|(v5 + v5)| in v4) then v4[|(v5 + v5)|] else v0[214]);
		...
		r0 := v8[p0 / |(if (p0 in v10) then v10[p0] else v1)|];
		m9(globalState);
		v2[611] := p0;
		v2[611] := fm6(p1, globalState);
		var v11: map<array<int>, bool> := map[v2 := false];
		for i2 := if (false) then |f33| else -v2[611] to v2[611] * |v11| {
			var v12: multiset<seq<int>> := multiset{v7, v7};
			v12 := (v12 + v12) * (fm43(globalState) + fm43(globalState));
			if (f32) {
				var v13: map<seq<bool>, int> := map[v8 := |(seq(572, i3  => (f27)))|];
				globalState.f10 := (-|f33| - i2) != |v13|;
				var v14: map<bool, array<bool>> := map[0x4b >= 0x247 := v0];
				v14 := v14[if (!f25) then v0[214] else v0[214] := v0];
				var v15: map<int, int> := map[i2 := i2];
				var v16: map<array<bool>, int> := map[v0 := v7[if (-0x12d in v15) then v15[-0x12d] else p0]];
				v0[214], globalState.f16, f24, v2[611] := v2[611] <= v2[611], p0 + |v16|, f33 < f33, p0 % (p0 - v2[611]);
				globalState.f10 := v0[214];
				f27 := f27;
			} else {
		...
		r3 := |(map v24 : char | v24 in v25 :: (v24) := (p0))| + v2[611];
	}
	method m3(p0: int, p1: bool, p2: array<bool>, globalState: GlobalState) returns (r0: bool, r1: bool, r2: map<string, int>) {
		var v0: set<bool> := {f32};
		...
		forall i0 | 0 <= i0 < v9.Length {
			v9[i0] := if (f26) then {DC15(f27, f25), DC15(f33[p0], f32), DC15(f27, true), DC15(f27, f24)} - {DC15(f27, f25)} else {DC15(f27, f32), DC15(f27, f26)} + (set v10 : D7 | v10 in map[DC15(f27, f25) := p1] :: (v10));
		}
		var v11: map<int, bool> := map[-495 % p0 := f26];
	...
	method m0(globalState: GlobalState) returns (r0: bool, r1: array<bool>) {
		...
		for i0 := v1 to -v1 {
			...
			match (v2) {
				case DC15(cf28, cf29) =>
					...
					var v8: map<int, D0> := map[|f33| / v1 := v7];
					var v9: seq<bool> := [f25];
					...
					var v16: multiset<seq<int>> := multiset{seq(-0x37e, i1  => (i0)), [v1] + v13, if (cf29) then v15 else v15};
					var v17: seq<seq<int>> := [v15];
					v16 := v16 - (v16 - multiset(v17));
				case DC16(cf30, cf31, cf32, cf33, cf34) =>
					var v18 := "btfxa";
					v18 := if (cf34) then v18 + (seq(0x202, i3  => (f27))) else f33;
					globalState.f16 := i0;
					var v19 := new C2("skabkujph", v18);
					var v20: map<bool, int> := map[cf32 := i0];
					var v21: seq<bool> := [f26];
					var v22: seq<int> := [|v21|];
					v20 := v20[cf30 := if (f25) then |v22| else fm0(i0, cf32, map v23 : int | (0xd9 <= v23) && (v23 < 893) :: (v23 - cf31) := (cf33), |(seq(0x70, i4  => (f27)))|, globalState)];
				case DC14(cf27) =>
					var v24: seq<bool> := [true, f32];
					var v25 := DC7(v24);
					globalState.f11 := |(if (v1 >= |f33|) then [f26, f32, f26] + v24 else v25.cf15)|;
					globalState.f22, globalState.f11, globalState.f10 := v1 / v1, -((i0 + i0) % |f33|), !(i0 <= v1);
				...
				case DC17(cf35) =>
					...
					var v30: array<int> := new int[11](i6 => i6 / i0);
					...
					v38[710] := !true;
					v38[710] := v0.fm24(i0, fm40(f26, v37, globalState), i0 / -0x395, |fm41(f26, f27, !fm39(v37, f32, globalState), globalState)|, globalState);
					globalState.f7 := v30;
					v36 := v36.(cf21 := v35);
			}
			...
			if (fm1(f32 && f32, v1 > i0, v41[i0] % --0x2c4, globalState)) {
				var v42: array<int> := new int[15](i7 => i7 / i0);
				v42[950] := i0 + i0;
				var v43 := DC5('g', false && f24, v1, -v41[|f33|]);
				v42[950], globalState.f11, v43, globalState.f11 := 0x3d6 - (i0 / v1), --v1, fm46(globalState), (i0 % v1) - i0;
				var v44: array<seq<int>> := new seq<int>[4];
				v44[2] := v41;
				var v45: seq<string> := [f33, f33, f33, "irfek", f33];
				globalState.f10, v44[2], f27, f27, r0 := f32, [-0x12c, v1, -v42[950], |v45|, v42[950]], fm47(globalState), f27, if (f24) then f25 else v1 <= i0;
				var v46 := "dreorfuot";
				v46 := fm41(false, f27, f24, globalState) + f33;
				var v47 := DC0(f32, i0, v42[950]);
				var v48: seq<D0> := [v47];
				var v49: map<bool, seq<D0>> := map[false := v48];
				globalState.f11 := (fm5(v49, f32, f26, globalState) + fm5(v49, true, f24, globalState)) * v42[950];
				r1 := new bool[7](i8 => if (f26) then f26 else f26);
			} else {
				var v50: array<char> := new char[8](i9 => f27);
				v50[167] := if (f32) then fm47(globalState) else f27;
				v50[167] := 'l';
				globalState.f10 := !!f25;
				var v51 := new C2(seq(81, i10  => (DC5(v50[167], f32, i0, v1).cf10)), f33);
				...
				var v57: T0 := new C2(seq(-0x76, i11  => (v50[167])), seq(0x10a, i12  => (v50[167])));
				var v58: map<T0, bool> := map[v57 := f24];
				var v59: array<int> := new int[28] [i0, v1, |v52|, if (!f25 in v54) then v54[!f25] else v1, -i0, |v55|, v1, v1, v1, |v56|, i0, i0, v41[369], v1, 0x9f, v1, |map[i0 := v1]|, v1, |v58|, v1, 0x133, -i0, i0, v1, v1, v1, v1, v1];
				var v60: seq<array<int>> := [v59, v59];
				var v61: seq<array<int>> := [v60[i0], v59];
				globalState.f7 := v61[-v1];
			...
			decreases 100 - i13
		{
			...
			var v68: array<int> := new int[9](i14 => i14 / v1);
			v67[578] := v68;
			...
			v65, v66, globalState.f10, v67[578], f24 := v65[v71.cf11 := -0x27b], "ep" + "gdeamwx", {v1} > {|v72|}, v68, f32;
			...
			decreases 100 - i15
		{
			...
			i15 := i15 + 1;
			globalState.f16 := v1;
			var v76: array<int> := new int[5] [v1 - v1, -v1 % v1, 0x1f, v1, v1];
			v76[731] := v1;
			...
			v76[731], globalState.f11, v77[896], v79 := v80[v80[v1]], v1 + |f33|, v76, v79;
	...
	method m1(p0: set<int>, globalState: GlobalState) returns (r0: char, r1: seq<bool>) {
			...
			decreases 100 - i0
		{
			...
			if (-v0 > |v5|) {
				...
				var v9 := DC6(DC6(v8));
				var v10 := DC11(v9, f26);
				var v11: multiset<bool> := multiset{v4[839], f25, fm7(v7, globalState), if (v4[839]) then f26 else v10.cf22};
				globalState.f16, globalState.f22 := |v11|, v0;
				globalState.f16, v0, f24, globalState.f16, v4[839] := v0, -v0, v0 >= -v0, v0, !(f24 ==> f32);
				var v12 := DC12(v1);
				...
				v14[886] := f27;
				v14[886] := f27;
				f24 := if (!f26) then v4[839] else v1 == v1;
				var v15: seq<string> := ["esq"];
				var v16: seq<seq<string>> := [["gq"], [f33]];
				var v17: map<bool, int> := map[f24 := 0x197];
				var v18: array<seq<string>> := new seq<string>[16] [seq(0xf, i2  => (f33)), v15, (seq(0x13f, i3  => (f33[0x3e1 := f27]))) + (seq(989, i4  => (f33))), v16[v0], v15 + v15, v15, v15 + v15, [f33], v15, ([seq(0x132, i5  => (v14[886]))])[v0 := f33], fm3(([true, !fm7(v17, globalState)])[v0 := f25], 0x171, 'm', f26, globalState), v15, ["ch"] + v15, v15, v15, v15];
				v18[503] := v15;
				v18[503] := v15;
			...
			r1 := (if (f32) then fm49(true, !f32, |v19|, v4[839], globalState) else v5) + v5;
		}
		...
		v20[347], v23[584], v25[988], globalState.f11 := v22, v27[-v0], v26, v0;
		var i7 := 0;
		while (f26)
			decreases 100 - i7
		{
			...
			i7 := i7 + 1;
			if (f27 in (f33[v0 := f27] + "wbjcugj")) {
				var v28 := DC0(f25, 0x106, v0);
				var v29: seq<D0> := [DC0(f24, v0, v1[v0]), v28];
				...
				globalState.f11 := |(v1[|f33| := v0])[|v34| := v0]|;
				var v35 := new C0(f33);
				var v36: multiset<bool> := multiset{f25};
				globalState.f16 := v0 / (if (f32 in v36) then v36[f32] else |v1|);
				globalState.f22 := v0;
				var v37: multiset<int> := multiset{v0};
				var v38: map<int, multiset<int>> := map[v0 := v37];
				v38 := v38;
			} else {
				var v39: map<int, bool> := map[v1[v0] / |fm50(DC6(DC4(f24)), v0, false, globalState)| := f32];
				v39 := v39[v0 := f24];
				...
				var v42 := new C1(v21[777]);
			}
			
			v21[897] := f32;
			var v43: map<bool, int> := map[f32 := v0];
			v21[897] := fm7(map[f25 := v0] + v43, globalState);
			if (v21[897]) {
				...
				var v48 := "leerl";
				v48 := (if (v21[897]) then v48 else f33) + (if (f32) then "uvggjar" else f33);
				f24 := fm1(f32, f25, v0, globalState) <==> v21[897];
			} else {
				v21[897] := f26;
				var v49: map<bool, bool> := map[f24 := v21[897]];
				...
				v63 := v63[(seq(273, i8  => ('h')))[v0 := f27] := f27];
				f24 := f25;
			}
			...
			var v74: map<char, multiset<multiset<bool>>> := map[v73.cf10 := v72[v71 := |multiset{v0}|]];
			globalState.f11, v3, v21[897], v64, v65 := fm5(v69, v0 <= v1[fm0(v0, f26, map[|(map[f26 := v0])[!(if (f27 in v70) then v70[f27] else v21[897]) := v0]| := v21[897]], v0, globalState)], fm39(v1, v21[897], globalState), globalState), v3, v72[v71 := v0] >= (if (f27 in v74) then v74[f27] else v72), (if (v21[897]) then v64 else f33) + f33, v65;
		}
		...
		if (f25) {
			...
			v21[436] := !!(if (f25) then f25 else |fm51(f27, globalState)| != |v82|);
			var v83 := new C0(f33);
			v80 := v80;
			var v84: map<bool, int> := map[v21[436] := v0];
	...
	method m8(p0: bool, globalState: GlobalState) returns (r0: int, r1: string, r2: array<int>, r3: string) {
		...
		for i0 := |f33| to v0 - |v1| {
			...
			var v6: map<int, set<int>> := map[|f33[|v4| := f27]| := v4];
			var v7: set<bool> := {false, f26, f25, f24};
			var v9: seq<set<int>> := [v4, set v5 : int | (0x2a0 <= v5) && (v5 < 0x2d9) :: (v5 % |multiset([false, !f32])|), if (|v7| in v6) then v6[|v7|] else set v8 : int | (0x1db <= v8) && (v8 < 0xd3) :: (v8 - v0), v4, v4];
			var v10: array<string> := new string[8](i1 => "ekgjc");
			v10[577] := f33;
			var v11: array<bool> := new bool[26];
			v3[860], v9, globalState.f2, v10[577] := v2 - v2[|v1| := v0], v9, v11, "ymb";
			globalState.f10 := true;
			var v12: seq<int> := [v0];
			globalState.f16 := |(v3[860] + multiset(v12))| % i0;
			var v13: set<string> := {v10[577]};
		...
		v17[826] := f33;
		v17[826] := (f33 + (seq(-0x2f2, i2  => (f27))))[v0 := f27];
		globalState.f16 := -(v0 + (|v1| + v0));
		var v18: seq<bool> := [f26];
		var v19 := DC1(v17[826]);
		var v20: set<bool> := {f25};
		var v22 := DC0(f26, v0, |v17[826]|);
		var v23: seq<int> := [v0, v0, |f33|];
		var v24: map<int, bool> := map[v0 := f25];
		var v25: array<int> := new int[24] [v0, v0 % v0, v0 - |v18|, v0, -0x1cf - v0, v0, v0, v0, -|v19.cf3| * v0, |v20|, v0 / v0, v0 + -0x279, |(fm53(f27, f27, globalState) + (set v21 : int | (0x30a <= v21) && (v21 < 0x1f9) :: (v21 - 0x3c8)))|, -v0, v0, fm6(v22, globalState), v0, v0, |v23|, |(v24 + v24)|, v0, 0x179, -(|(seq(-0x1a3, i3  => (f27)))| - v0), v0];
		v25[9] := v0 * |(seq(-0x59, i4  => (f27)))|;
		v25[9] := v0;
		r0 := if (f27 in (seq(0x289, i5  => (f27)))) then v0 else v0;
		r1 := v17[826] + "ohus";
		r2 := v25;
		r3 := seq(-574, i6  => (f27));
	}
	method m9(globalState: GlobalState) {
		...
		globalState.f10 := !(v28 > v28[f32 := v0]);
		for i1 := fm6(DC0(f24, -v0, v0), globalState) to v0 {
			...
			var v37: array<int> := new int[23] [v0, |((map v29 : int | v29 in v1 :: (v29 / i1) := (f25)) + v32)|, i1 + i1, v34.cf25, -|({false} * v35)|, v0, |f33|, v2[|v36[f24 := v0]|], -0x29, |f33|, |v32| - v0, v0, DC13(v33, |"wgnjr"|, f26).cf25, -fm6(DC0(f32, v0, v0), globalState), i1 / 0x1b, 0xb3, v0, v0, v0 / i1, i1, |v36|, v0, i1];
			v37[477] := v0;
			v37[477] := v0;
			...
			if (fm1(!f25, false, v37[477] / v0, globalState)) {
				v33[695] := v33 !in map[v33 := v0];
				v35, v39, v33[695], globalState.f10 := fm44(-|v2|, v2, globalState), v39, f32, if (f25) then f26 else f32;
				globalState.f10 := (f33 + (seq(0x39d, i3  => (f27)))) <= f33;
				var v43: array<array<array<bool>>> := new array<array<bool>>[18];
				var v44: array<array<bool>> := new array<bool>[9];
				v43[61] := v44;
				f24, globalState.f10, globalState.f11, v43[61] := f27 !in f33, (if (f24) then i1 else v37[477]) < v0, (v37[477] % i1) * v2[0x2e0], if (f24) then v44 else v44;
				globalState.f10 := f32;
				globalState.f7 := v37;
			} else {
				...
				f24, v46[437], v48[182] := f26, v47, if (!f24 ==> f26) then f33 else f33[|(if (f32 in v49) then v49[f32] else v28)| := f27];
				globalState.f11 := |v2| - 600;
				var v50: map<bool, bool> := map[f25 := !f26];
				var v51: seq<map<bool, bool>> := [v50, map[f24 := f32]];
				var v52: seq<map<bool, bool>> := [v51[v37[477]], map[v30[v0] := f32]];
				var v53 := new C2(v48[182], v45[if (!!f32 in v28) then v28[!!f32] else |v51|] + v48[182]);
			}
			...
			decreases 100 - i5
		{
			...
			globalState.f16 := v0;
			v62[268] := v0;
			globalState.f16, v62[268] := v60, 0x288 / v60;
		}
	}
}
...
class C4 extends T2 {
	...
	function fm37(p0: map<bool, D2>, globalState: GlobalState): int {
		(if (f25) then |map[false := -0x344]| else |(seq(-0x234, i0  => ({|multiset{f24}|})))|) * |((set v0 : char | v0 in map['r' := |"iwaa"|] :: (v0)) * (set v1 : char | v1 in multiset(['f', 'p']) :: (v1)))|
	}
	method m0(globalState: GlobalState) returns (r0: bool, r1: array<bool>) {
		f24 := f24 ==> (f25 || f24);
		...
		match (if (f24) then v3 else DC2(v0, v1, v2, f24)) {
			case DC2(cf4, cf5, cf6, cf7) =>
				...
				if (|v8| > (v2 % cf6)) {
					...
					v13 := v13[v2 * v2 := |v14| / |v15|];
					globalState.f10 := cf7;
					globalState.f11 := (v2 + |v9|) + (if (cf7) then cf6 else 0x1b1);
					var v16: array<bool> := new bool[6];
					var v17: map<array<bool>, array<char>> := map[v16 := v5[431]];
					v5[431] := if (v16 in v17) then v17[v16] else v5[431];
				} else {
					var v18 := DC15('g', cf7);
					...
					var v24: seq<bool> := [v22[92]];
					v23 := (v23 + multiset(v24)) + v23;
					cf7, globalState.f16 := if (f24) then v22[92] else !cf7, cf6;
					var v25: map<int, bool> := map[202 := f24];
					var v26 := DC13(v22, cf6, v22[92]);
					var v27: map<bool, string> := map[f24 := v8];
					var v28: map<bool, seq<D0>> := map[v22[92] := [DC0(fm1(f24, cf7, v2, globalState), |(if (v22[92] in v27) then v27[v22[92]] else v8)|, |v8|)]];
			...
			case DC1(cf3) =>
				...
				v35[396] := if (false) then v37 else v37;
				globalState.f11, r0, v1, globalState.f16, v35[396] := |[f24, f25, !f25]|, f25, v1, if (f25) then v2 else v2, v37;
		...
		for i1 := v38[v2] to v2 {
			match (DC2(v3.cf4, v1, v2, f24)) {
				case DC2(cf4, cf5, cf6, cf7) =>
					...
					var v40: map<int, int> := map[|v39[cf6 := f24]| := cf6];
					var v42 := "aiu";
					...
					globalState.f10, v47, cf7, v42 := f24, if (cf7) then v48[cf6] else v47, f25 ==> f25, seq(0x36a, i3  => (cf5));
				case DC1(cf3) =>
					var v49 := DC5(v1, f24, v2, v2);
					f24 := v49.cf11;
					var v50: map<int, int> := map[v2 := i1];
					v50 := v50[v2 % -|[f25]| := |(cf3 + cf3)|];
					...
					var v56: array<C0> := new C0[19] [v52, v52, v52, v52, v52, v52, v52, v52, v52, v52, v52, if (v55[v2] in v54) then v54[v55[v2]] else v52, v52, v52, v52, v52, v52, v52, v52];
			...
			v57[506] := f24;
			v57[506] := false;
			var v58: map<int, bool> := map[if (!true) then -0x2ba else i1 := v57[506]];
			v58 := v58;
		...
		var v62 := DC4(v60.f26);
		var v63: map<T3, D2> := map[v61[v2] := v62];
		v63 := v63;
	...
	method m1(p0: set<int>, globalState: GlobalState) returns (r0: char, r1: seq<bool>) {
		var v0 := 0x17d;
		var v1: seq<int> := [v0];
		v1 := v1 + (seq(-0x222, i0  => (|(seq(810, i1  => ('g')))|)));
		var v2: seq<bool> := [f25, f24];
		var v3 := DC0(f25, 0x227, v0);
		var v4: seq<D0> := [v3, v3];
		r1 := v2[fm5(map[f25 := v4], f24, f24, globalState) := f24] + [f25];
		for i2 := --v0 to |(p0 * {|v2|})| {
			var v5: array<string> := new seq<char>[25](i3 => seq(0x115, i4  => ('c')));
			...
			v7[613] := p0 + ({i2} + {|(if (i2 in v8) then v8[i2] else v9[v0])|});
			var v10: seq<seq<int>> := [(fm42(globalState))[i2 := i2]];
			var v11 := "hueldcmmw";
			v1 := v10[|(v11 + "opovedu")|];
		...
		if (v0 != (if (f24) then v0 else v0)) {
			var v13 := "v";
			v13 := v13;
			var v14: array<set<bool>> := new set<bool>[23](i6 => {f25} - {v2[v0]});
			var v15: set<bool> := {f25, false};
			v14[545] := v15;
			v14[545] := v15 * (fm44(|"fitm"|, v1, globalState) + v15);
			...
			var v21 := DC7(v2);
			var v22: array<seq<bool>> := new seq<bool>[20] [v2, v2, v2 + v2, [f24] + v2, fm49(true, f24, v0, f25, globalState), v2[v0 := v16[149]], (v21.(cf15 := v2)).cf15, v2, v2, v2, fm49(v16[149], f24, v0, f24, globalState), [!f25, f25], if (v16[149]) then v2 else v2, v2, [v3.cf0, f24, f24], v2, v2, [f25, f25], v2, v2];
			v22[51] := v2;
			v18, v19, v20, v22[51] := v18, multiset{v2}, v20, [v16[149], v2[|v13|]];
		} else {
			var v23 := "kgqjtvxw";
			var v24: map<int, bool> := map[|v23| := f24];
			var v25: set<map<int, bool>> := {v24, v24};
			globalState.f11, f24 := v0 / (v0 + v0), f24 <== ({v24} <= v25);
			var v26 := new C1(f24);
			...
			v28[212] := v27;
			v28[212], v0, globalState.f10, globalState.f22 := v27, v0, fm1(f24, f25, |v2|, globalState), (v0 - v0) * -(v0 / v0);
		...
		if (f25) {
			...
			match (v35) {
				case DC4(cf9) =>
					...
					v36 := v36;
					globalState.f16, globalState.f11 := -(fm5(map[cf9 := v4], f24, cf9, globalState) / v0), v0 + -(if (f25) then 0x333 else -v0);
					globalState.f16 := -v0;
				case DC5(cf10, cf11, cf12, cf13) =>
					var v37: array<int> := new int[29](i10 => i10 * -cf13);
					v37[100] := cf12 + |map[f24 := v0]|;
					var v38: array<char> := new char[29];
					var v39: map<seq<int>, bool> := map[v1 := f24];
					var v40 := "ok";
					v37[100], globalState.f11, v0, globalState.f10, v38 := |v39|, cf12 % (|v40| % cf13), cf13, f25, v38;
					...
					v43, globalState.f11, v48 := v43, fm5(fm55(cf10, cf11, v41.fm19(cf10, globalState), f24, globalState), cf11, true, globalState), multiset{seq(0x1a2, i11  => (v44)), v45, v45 + v45, v45[|v2| := DC15(cf10, f25)]};
				...
				case DC3(cf8) =>
					...
					v54, v52, globalState.f10, globalState.f22, v69[450] := v54[v0 := v56.f29], if (f24) then v52 else v52, f25, |(if ((v68 !! v68) in v70) then v70[v68 !! v68] else v2)|, 0x315;
					f24 := v62 < v62;
					var v71 := 'i';
					var v72 := new C3(f25, v55, v0 != v0, v71, f25, f25);
				case DC6(cf14) =>
					var v73 := 'j';
					var v74: T3 := new C3(f24, seq(188, i13  => ('w')), false, v73, true, f25);
			...
			var v91: array<string> := new string[12] [(v88[v0] + "psi")[v0 := v89], v87, v87, ("ormdo")[|v2| := v89], "drm", v87, v87 + v87, v87 + v87, v87, if (v0 in v90) then v90[v0] else v87, v87, v87];
			v91 := v91;
			...
			match (v93) {
				case DC8(cf16, cf17, cf18) =>
					globalState.f10 := v2[v0];
					v91[505] := v87;
					v91[505] := v87 + v87;
					var v94 := DC19(f25, cf17);
					var v95: map<D8, bool> := map[v94 := f24];
					v95 := v95;
					globalState.f11, v92[582], r0 := cf17, v92[582], v89;
				case DC7(cf15) =>
					v87 := v87 + v87;
...
class C5 extends T3 {
	...
	function fm4(p0: multiset<bool>, p1: string, globalState: GlobalState): D0 {
		DC0(0x2da > 0x333, 0x5b, |map[0x266 := f24]| / 0x2b3)
	}
	function fm5(p0: map<bool, seq<D0>>, p1: bool, p2: bool, globalState: GlobalState): int {
		(-233 / |{f26}|) * (|map[[-0x192, |(seq(0x148, i0  => (f27)))|, 0x390] := true]| % 0x143)
	}
	method m2(p0: int, p1: D0, p2: bool, globalState: GlobalState) returns (r0: bool, r1: bool, r2: int, r3: int) {
		...
		var v5 := "rmsenfy";
		r3, f27, r0, v1[838], f27 := (if (p0 in v4) then v4[p0] else p0) * (fm6(p1, globalState) - |v5|), f27, !p2, v0, f27;
		var v6 := DC8(p2, p0, p0);
		f24 := v6.cf16;
		var v7 := new C0(v5);
		var v8: array<int> := new int[14](i1 => i1 % p0);
		var v9: seq<array<int>> := [v8, v8];
		var v10: multiset<array<int>> := multiset{v9[p0]};
		var v11: map<multiset<array<int>>, int> := map[v10 := v6.cf17];
		f27 := v5[if (v10 in v11) then v11[v10] else p0];
			...
			decreases 100 - i2
		{
			...
			v8[733] := p0;
			v1[838][804] := p2;
			globalState.f11, v8[733], globalState.f16, v1[838][804] := fm0(p0, f25, map[p0 := f26], p0 / -0x36c, globalState), -p0, p0, v12[385];
			var v14: map<bool, int> := map[!false := v8[733]];
			r2 := if ((true <== true) in v14) then v14[true <== true] else v8[733];
			var v15: set<int> := {p0};
			var v16: map<bool, bool> := map[v15 >= v15 := f25];
			var v17: multiset<int> := multiset{v8[733]};
			v16 := v16[v17 >= v17 := if (v1[838][804]) then f24 else f25];
			v5 := fm29(globalState) + v5;
		}
		var v18 := DC9(v1);
		var v19 := DC0(false, 356, p0);
		var v20: array<seq<int>> := new seq<int>[3];
		var v21: map<array<int>, seq<int>> := map[v8 := (seq(0x2aa, i3  => (p0)))[p0 := p0]];
		var v22: seq<int> := [p0, p0];
		v20[366] := if (v8 in v21) then v21[v8] else v22;
		v18, globalState.f11, globalState.f16, v19, v20[366] := v18, fm5(v3, f26, f25, globalState), p0, p1, (v22 + (seq(-171, i4  => (p0)))) + [p0, p0, p0];
		r0 := f25;
		r1 := p2;
		r2 := p0;
		r3 := p0;
	...
	method m3(p0: int, p1: bool, p2: array<bool>, globalState: GlobalState) returns (r0: bool, r1: bool, r2: map<string, int>) {
			...
			decreases 100 - i0
		{
			...
			p2[132] := p1;
			var v9 := "hfdsbjpl";
			var v10: map<bool, char> := map[f25 := v9[p0]];
			var v11: map<char, bool> := map[f27 := f26];
			var v12: array<map<bool, char>> := new map<bool, char>[23] [v10, map[true := v9[|v9|]], v10, v10, v10 + v10, v10, v10, map[f25 := f27] + v10, v10, fm31(globalState), fm31(globalState), v10, v10, fm31(globalState), map[f26 := f27] + v10, v10[f24 := f27], v10, v10, if (if (f27 in v11) then v11[f27] else f24) then v10 else v10, v10 + v10, v10, v10, v10];
			var v13: set<int> := {-0x196};
			globalState.f22, globalState.f2, globalState.f22, p2[132], v12 := p0, p2, p0, -0x386 > |(v13 * v13)|, v12;
		}
		r1 := p1;
		...
		var v18 := DC0(p0 <= |v17[p0]|, p0, |fm32(globalState)|);
		match (v18) {
			case DC0(cf0, cf1, cf2) =>
				var v19 := new C1(f25);
				...
				v20[816] := fm0(p0, f24, v21 + v21, cf2, globalState);
				var v22 := "pqnv";
				v22 := v22;
		...
		for i2 := p0 to |[f26, f26]| {
			var v26 := "u";
			var v27 := new C2(v26, ("krn" + (seq(0x346, i3  => (f27))))[p0 := f27]);
			match (v14) {
				case DC4(cf9) =>
					f24 := (v27.f29 <= v26[p0 := f27]) && (i2 < i2);
					var v28: array<map<int, char>> := new map<int, char>[27];
					v28[785] := map v29 : int | (0x1f0 <= v29) && (v29 < 854) :: (v29 + i2) := (f27);
					var v30: map<int, char> := map[i2 := f27];
					v28[785] := v30;
					p2[821] := fm1(p1, cf9, p0, globalState);
					p2[821] := f26;
				...
				case DC3(cf8) =>
					...
					r1 := if (v36[p0] in v35) then v35[v36[p0]] else !f24;
					...
					globalState.f7 := new int[26] [469, i2, (v41.(cf7 := f26)).cf6, |(v44 + v44)|, i2 + p0, v36[0x102], -0x1d3, i2 + i2, |v47|, 78, i2 - i2, if (p1) then if (i2 in cf8) then cf8[i2] else i2 else i2, p0, -i2, i2, |v48.cf20| * |v49|, if (f25) then p0 else |v50|, p0 / |map[f26 := p0]|, i2 / v36[p0], i2, 491, |v49| % 0x278, i2, |(v51 - v51)|, p0 % i2, 0x3cc];
					var v52: seq<bool> := [f24, f26];
					globalState.f22, v50, v36 := |v52| / i2, v50, v36;
				case DC6(cf14) =>
			...
			var v62: map<int, int> := map[p0 % v60.fm25(v27.f30, v60.fm24(i2, v61, i2, 0x3be, globalState), globalState) := i2];
			v62 := v62[p0 := i2];
		}
		var v63 := "u";
		for i4 := p0 to -|v63| % p0 {
			globalState.f10 := (-0x382 % |v63|) > i4;
			...
			p2[417] := (set v69 : map<int, int> | v69 in v68 :: (v69)) <= v70;
			p2[417] := f26;
			...
			v73[394] := v71;
			var v74: multiset<bool> := multiset{fm1(f25, f26, i4, globalState), f25, f24, f25, p2[417]};
			v73[394] := v71[|v74| := f25];
		}
		if (f25) {
			...
			var v77: seq<D0> := [v18];
			var v78: map<bool, seq<D0>> := map[p1 := v77[p0 := v18]];
			var v79: seq<bool> := [f26];
			var v81: map<int, bool> := map[p0 := p1];
			var v82: multiset<int> := multiset{p0};
			var v83 := DC2(v76, f27, fm0(fm5(v78, !v79[p0], f24, globalState), false, map v80 : int | v80 in v81 :: (v80 / 0xb7) := (f24), |map[!f25 := v82]|, globalState), f26);
			v75 := v75[!(p0 >= p0) := v83.cf7];
			var v84: map<int, int> := map[-p0 := p0];
			var v85: seq<int> := [p0, p0, v83.cf6];
			var v86 := DC8(!f25 && (if (|v84| in v81) then v81[|v84|] else false), p0, |((seq(160, i6  => (|v63|))) + v85)|);
			match (v86) {
				case DC8(cf16, cf17, cf18) =>
					...
					var v89: set<seq<char>> := {[f27], v88[v18.cf2]};
				...
				case DC7(cf15) =>
					...
					v93 := v93[v63 := fm0(-0x38e, f26, v81, p0, globalState) / 868];
			...
			r0 := p1;
			var v95: set<int> := {|v82[p0 := |v63|]|, |map[v85[p0 := p0] := false]|};
			var v97 := DC14(v95);
			var v100: T1 := new C1(f26);
			var v101: map<bool, T1> := map[f26 := v100];
			var v102: seq<map<int, int>> := [map[|v101| := p0], v84];
			var v103: map<bool, char> := map[v100.f24 := v63[p0]];
			var v108: array<map<int, int>> := new map<int, int>[28] [v84, map v96 : int | v96 in v97.cf27 :: (v96 % p0) := (|(map v98 : map<int, int> | v98 in {map[p0 := p0]} :: (v98) := (|v79|))|), map v99 : int | v99 in v81 :: (v99 - p0) := (-p0), v102[p0], v84, fm35(false, v103, globalState) + (map v104 : int | v104 in v81 :: (v104 % p0) := (p0)), v84, v84, map v105 : int | v105 in v85 :: (v105 * p0) := (|v79|), v84, v84, v84[p0 := |[|v79|, p0, |[p2]|, p0, p0]|], (map v106 : int | (0x25c <= v106) && (v106 < 0x270) :: (v106 + p0) := (|[|[p0]|]|))[-|v95| := |[f24, v100.f24]|], v84, v84 + v84, v84, v84, v84, v84, v84, v84, v84 + map[-90 := |fm31(globalState)|], v84, map v107 : int | v107 in v95 :: (v107 % p0) := (p0), v84[p0 := p0], v84, v84, fm35(f25, v103, globalState)];
			p2[327] := false;
			var v109: map<seq<int>, set<int>> := map[seq(0x161, i7  => (-p0)) := v95];
			var v110: map<bool, int> := map[f25 := 0x17e];
			v95, r0, v108, p2[327] := if (v85 in v109) then v109[v85] else v95, fm7(v110, globalState), v108, (p0 - fm0(p0, !v100.f24, v81, |v82|, globalState)) != |(v79 + [v100.f24])[p0 := f25]|;
			p2[327] := v100.f24;
		} else {
	...
	method m0(globalState: GlobalState) returns (r0: bool, r1: array<bool>) {
		...
		if (v0 > v0) {
			...
			match (if (fm36(|v16|, false, f25, globalState) in v17) then v22 else DC10(v21)) {
				case DC11(cf21, cf22) =>
					...
					var v27: set<seq<int>> := {v13, seq(0x254, i1  => (v0))};
					var v28 := DC16(f26, v0, f26, v26[v0], fm56(globalState) >= v27);
		...
		if ((v35.fm6(v36, globalState) * 0x136) != v0) {
			...
			var v43: map<string, int> := map[v38 + v38 := |v42| / v0];
			var v44: map<int, bool> := map[v0 := true];
			var v46: map<int, int> := map[|(map v45 : int | v45 in v44 :: (v45 / v0) := (v38))| := v0];
			v43 := v43[(seq(0x195, i2  => (DC5(f27, f26, v0, v0).cf10)))[v0 := 'p'] := |fm57(-v0, v44, set v47 : int | v47 in v46 :: (v47 - 0x347), globalState)|];
			...
			v49[362], globalState.f11, globalState.f11 := (if (v0 in v53) then v53[v0] else v39) - v39, v0, --((|map[v0 := f25]| - v0) * |(map[f25 := v0] + v41)|);
			var v54 := DC4(f26);
			var v55: multiset<D2> := multiset{v54, v54};
			var v56 := DC8(f26, 0x27a, |v55|);
			var v57: seq<D0> := [v36, v36];
			var v58: map<bool, seq<D0>> := map[f25 := v57];
			globalState.f11 := v56.cf18 / (v35.fm5(v58[false := seq(0x2b9, i3  => (v36))], f26, f26, globalState) * v0);
		} else {
			...
			match (fm58(DC1(fm38(f26, v60.cf6, v61[|(seq(0x1e0, i4  => (545)))| := v0], v63, globalState)), v62, f26, globalState)) {
				case DC2(cf4, cf5, cf6, cf7) =>
					var v64: map<bool, string> := map[f25 := v62];
					v64 := v64[cf7 := v62];
					v59[541] := v35.fm6(v36, globalState);
					var v65: set<char> := {cf5, f27, cf5, f27, cf5};
					v59[541] := v0 + |(v65 - v65)|;
					var v66: C3 := new C3(v61 < v61, seq(507, i5  => (cf5)), cf7, f27, cf7, cf7);
					v66 := v66;
					f27 := f27;
				case DC1(cf3) =>
			...
			var v74: C3 := new C3(f25, v62, !f25, if (f26) then f27 else f27, fm1(v73[|{f24, false}|], f25, v0, globalState), f24);
			v74 := v74;
		...
		if (!f25) {
			...
			var v84: array<bool> := new bool[23] [f25, f25, f25, if (f25) then f24 else true, false, f26, v0 > v0, if (f25) then f26 else f24, v0 > v0, f26, v0 !in v79, f25, if (v81 in v82) then v82[v81] else true, true, f26, f24, f24, f25, f25, f25, !(v78[fm0(v0, f26, v79, v0, globalState)] == |v78|), if (false in v83) then v83[false] else f26, {f24, f25, f24, true} != v34];
			v84[252] := false;
			v84[252] := f25;
			globalState.f16 := |v34| + -v0;
			var v85: map<bool, int> := map[!!f26 := v0];
			var v86 := DC10(multiset{v85});
			var v87: map<D5, bool> := map[v86 := v84[252]];
			var v89: multiset<D5> := multiset{v86, v86};
			var v90: multiset<map<bool, int>> := multiset{v85, v85};
			v87 := map v88 : D5 | v88 in v89[DC10(v90) := v0] :: (v88) := (v84[252]);
			if (!f25) {
				f24 := if (fm7(fm45(f25, globalState), globalState)) then |"xvtxs"| != v0 else v84[252];
				var v91: set<int> := {|"sc"|};
				...
				globalState.f11 := -|v96[if (f26) then v0 else v0]|;
				v34 := v34;
				var v97: array<map<D5, int>> := new map<D5, int>[23](i7 => map[DC10(multiset(seq(0x182, i8  => (v85)))) := v78[v0]]);
			...
			match (v106) {
				case DC4(cf9) =>
					...
					v112[66] := v105;
					v108, globalState.f10, v111, v112[66], v0 := v108, f26, v111 + v111, v105 + (v105 + v105), v0;
					var v114: array<map<int, string>> := new map<int, string>[12](i9 => map[|[v0]| := v112[66]] + (map v113 : int | (0x398 <= v113) && (v113 < 361) :: (v113 - v0) := (v105)));
					var v115: seq<string> := [v105];
					var v116: multiset<int> := multiset{v0, v0};
					var v117: map<int, string> := map[0x125 := v115[|v116|]];
					v114[850] := v117 + v117;
					v114[850] := v117;
				...
				case DC5(cf10, cf11, cf12, cf13) =>
					...
					var v127: C3 := new C3(cf11, v105 + (seq(-0x65, i10  => (cf10))), false, f27, !f24, v126 <= v126);
					var v128: seq<C3> := [v127, v127, v127];
					v127 := v128[-v0];
					var v129 := new C1(f24);
					v124[417] := v0;
					v124[417] := v127.fm5(map[f26 := [v36]] + fm55(cf10, false, f25, f26, globalState), f24, v126[cf13], globalState);
					cf12, v124[417] := |(v127.f33 + (seq(-837, i11  => (cf10))))| - v124[417], v124[417] % cf13;
				case DC3(cf8) =>
					var v130 := new C2("egvtbhsk", v105 + (seq(0x92, i12  => (f27)))[v0 := f27]);
					...
					var v141: array<bool> := new bool[28] [f26, f24, f24 ==> f26, (set v135 : char | v135 in v105 :: (v135)) > v136, f26, f26, f26, f24, f25, f24, f25, v137[v0], f25, v0 < |fm59(fm5(map[f25 := v138], f24, f24, globalState), f25, f24, globalState)|, f26, v137[v139[0x295]], !f24, |v136| in [v0], v130.fm19(f27, globalState), f25, f25 || !f24, f25, v140 >= fm51(f27, globalState), v139 < v139, f26, f24, v0 !in v139, f25];
					var v142: multiset<int> := multiset{|[v0, v0]|, v0, |v130.f29|};
					v141[2] := !(v142 !! v142);
					v141[2] := ("iw" + v105) == v130.f30;
				...
				case DC6(cf14) =>
					...
					var v157: array<bool> := new bool[22] [v147.cf7, !v148[|v145|], f24, [v146, v146] != v149, f26, f25, if (f26) then f26 else v152.cf22, f25, v148 <= v148, v105 != "gtqtq", f24, f25, !(v154 < (seq(-0x156, i13  => (v153)))), f26, f26, f26 || f24, f25, |v105| <= |v155|, v156 >= v156, f24 <==> f24, if (f25) then false else f25, f25];
					v157[418] := f25;
					v157[418] := !f25;
					var v158: map<int, int> := map[0x170 := v0 / v0];
					v158 := v158[v0 := if (f24) then v0 else 0x1ce];
					globalState.f11 := (v0 / v0) - v0;
			}
		...
		var v164: map<array<char>, int> := map[v161 := |v163[392]| - v0];
		v164 := v164[v161 := v0];
	...
	method m1(p0: set<int>, globalState: GlobalState) returns (r0: char, r1: seq<bool>) {
		...
		if (v1 >= v1) {
			...
			v3[637] := if (f24) then if ((seq(373, i0  => (f27))) in v5) then v5[seq(373, i0  => (f27))] else v2 else |v8|;
			v3[637] := v2;
			var v9: seq<string> := [v6.f30];
			globalState.f11 := -(-|(v9 + v9[v3[637] := "hujqf"])| * -|"begj"|);
			globalState.f10 := f24;
		} else {
			...
			v10[147] := v11;
			globalState.f10 := f26;
			var v12: map<int, int> := map[v11 - |v0| := v10[147]];
			var v13 := "neiyfvffm";
			var v14: C0 := new C0(v13);
			var v15: array<char> := new char[24] [f27, f27, f27, f27, f27, 'h', v14.f31[v11], 'v', f27, f27, f27, f27, f27, f27, f27, f27, 'v', 'c', f27, f27, f27, f27, f27, f27];
			var v16: seq<array<char>> := [v15];
			var v17: multiset<int> := multiset{|v16|};
			var v18: map<C0, int> := map[v14 := |v17|];
			v12 := v12[v10[147] := |(v18 + map[v14 := v10[147]])|];
			...
			var v22 := DC5(f27, if (!f24) then fm7(v20, globalState) else f26, if (v13 in v21) then v21[v13] else v10[147], v10[147]);
			var v23: array<string> := new string[13](i2 => v14.f31);
			var v24: map<int, array<string>> := map[v10[147] := v23];
			var v25 := DC20(if (v10[147] in v24) then v24[v10[147]] else v23);
		...
		var v27 := 0x16b;
		for i3 := v27 / -0xbc to v27 {
			var v28: array<int> := new int[5];
			var v29: multiset<bool> := multiset{f24, f26};
			v28[676] := if (f26 in v29) then v29[f26] else v27;
			var v30: multiset<char> := multiset{f27, f27};
			v28[676] := 0x3a7 + ((if (f27 in v30) then v30[f27] else v27) / i3);
			var v31: array<bool> := new bool[17](i4 => f25);
			var v32: seq<array<bool>> := [v31, v31];
			var v33: seq<bool> := [f24, f25];
			match (DC13(v32[-0x1f2], -|v33|, f24)) {
				case DC13(cf24, cf25, cf26) =>
					v27 := (i3 / |v32|) % (v28[676] * -v27);
					var v34 := "xf";
					v34 := v34[v28[676] := f27];
					v31[241] := cf26 ==> !cf26;
					v31[763] := v28[676] in multiset{0x195};
					var v35: map<int, seq<bool>> := map[v27 := v33];
					globalState.f10, globalState.f11, v31[241], v31[763] := v33[cf25], if (f26) then i3 else |(map[818 := v33] + v35)|, (v34 + v34) == v34, f24;
					var v36: array<map<bool, char>> := new map<bool, char>[6];
					var v37 := DC21(v36);
					v36 := v37.cf40;
				case DC12(cf23) =>
					var v38: multiset<int> := multiset{v28[676], |multiset{v28[676]}|, v28[676]};
					var v39: seq<char> := [f27];
					globalState.f16, globalState.f7, globalState.f16 := i3, v28, -((if (0x23b in v38) then v38[0x23b] else |v39|) % v28[676]) / i3;
					var v40: map<bool, set<multiset<bool>>> := map[f24 := {multiset{true, f25}}];
					var v41: map<int, multiset<bool>> := map[|multiset{i3}| := v29];
					var v42: set<multiset<bool>> := {if (v28[676] in v41) then v41[v28[676]] else (multiset{f24, f26})[f26 := -438]};
					v40 := v40[f24 := v42];
					globalState.f11 := v28[676];
					v39 := ("gusxkuor" + v39)[v27 - i3 := if (f25) then f27 else f27];
			}
			
			var v43: map<int, bool> := map[v28[676] := f25];
			var v44: map<bool, int> := map[true := v28[676]];
			var v45: multiset<int> := multiset{--|(if (f25) then v43 else v43)|, v27 / -fm0(v28[676], f26, v43[0x36b := fm7(v44, globalState)], -i3, globalState)};
			var v46 := "fieqtkeqr";
			v45 := v45 - (v45[-v28[676] := |v46|] - v45);
			var v47 := new C4(fm1(f25, !f24, v28[676], globalState), f26);
		}
		var v48: multiset<int> := multiset{v27, v27, 0x47};
		var v49: map<int, int> := map[v27 := v27 / |v48|];
			...
			decreases 100 - i5
		{
			...
			var v55: multiset<multiset<int>> := multiset{(multiset{v27})[v27 := v27], v48, v48, v48};
			var v57 := "ksrgpvm";
			var v58: multiset<string> := multiset{v57, v57};
			v54[174] := -fm0(v27, !fm1(!f25, f26, |v55|, globalState), map v56 : int | v56 in v49 :: (v56 * |v48|) := (DC5(f27, f25, v27, v27).cf11), |v58|, globalState);
			v54[174] := 810;
			...
			if (f24) {
				...
				v27 := fm0(-v54[174], f24 <== f25, v60, 0x380, globalState);
				var v61: seq<string> := [v57[|v48| := f27]];
				var v62: seq<bool> := [f26, !f24];
				var v63: C3 := new C3(v50 >= v50, v61[|v61|], v62[v27], f27, false, v50 >= v50);
				var v64 := DC25(v63);
				v63 := v64.cf46;
				v57 := v63.f33 + v57;
			} else {
			...
			if (-(v54[174] / v27) == v27) {
				globalState.f10 := v27 == (v54[174] - v54[174]);
				v49 := v49[v54[174] := 0xd4];
				var v69: C3 := new C3(f25, v57, false, 'g', f24, false);
				var v70 := DC25(v69);
				var v71: map<D11, int> := map[v70 := v54[174]];
				globalState.f11, globalState.f16 := if (DC25(v69) in v71) then v71[DC25(v69)] else v27, v54[174];
				v54[174] := v54[174];
				...
				var v76 := DC21(v75);
				var v77: map<int, array<bool>> := map[v54[174] := v73[905]];
				v73[905], globalState.f10, v76, f24 := if (!f26) then v73[905] else if (v27 in v77) then v77[v27] else v74, f25, DC21(v75), f25;
				globalState.f11 := |(seq(0x2d3, i8  => (|[v27, v54[174]]|)))|;
				var v78 := DC22(v27, f26);
				var v79 := DC24(v78);
				f24, globalState.f11, v79, v54[174] := f26, -v27, DC24(v78), v27;
				var v80: array<string> := new string[25];
				var v81: map<array<bool>, array<string>> := map[v73[905] := v80];
				v81 := v81[v73[905] := v80];
			}
...
class C6 extends T1, T3 {
	...
	method m2(p0: int, p1: D0, p2: bool, globalState: GlobalState) returns (r0: bool, r1: bool, r2: int, r3: int) {
		...
		var v1: map<int, int> := map[p0 / p0 := |v0|];
			...
			decreases 100 - i1
		{
			...
			r2, v8, f24, r0 := 0x196, v8[-p0 := f24], (v8 <= v8) || v8[if (-0x32a in v9) then v9[-0x32a] else 0x1ab], fm17(f24, f28, globalState);
			var v10: array<int> := new int[10];
			v10[799] := p0;
			v10[799] := (p0 / p0) * --0x399;
			if (p2 && f28) {
				var v11: map<bool, seq<D0>> := map[p2 := seq(0x187, i2  => (p1))];
				var v12 := "mi";
				var v13: multiset<string> := multiset{v12, v12, v12, v12, v12};
				var v14: map<bool, int> := map[f25 := |(seq(-0x245, i3  => (f27)))|];
				var v15: seq<int> := [p0];
				globalState.f22, r2, globalState.f22 := v10[799], if (f25) then fm6(DC0(p2, -p0, v10[799]), globalState) % v10[799] else fm5(v11, p2, f28, globalState) % |v13|, if ((v15 <= v15) in v14) then v14[v15 <= v15] else p0;
				r0 := false;
				var v16 := DC4(!(f25 || p2));
				v16 := DC4(v0 !! v0);
				...
				var v17 := new C0(seq(-0x8d, i4  => (f27)));
				globalState.f10 := p2;
				...
				r3 := v19.fm5(v21[p2 := v20] + map[true := seq(0x53, i5  => (p1))], p2, p2, globalState);
			}
	...
	method m3(p0: int, p1: bool, p2: array<bool>, globalState: GlobalState) returns (r0: bool, r1: bool, r2: map<string, int>) {
		...
		r1, v0[500], globalState.f10 := f25, v4 + v4, fm7(v1, globalState) && v7.cf7;
		f24 := f25;
		match (DC1("yxq")) {
			case DC2(cf4, cf5, cf6, cf7) =>
				v2 := v2;
			...
			case DC1(cf3) =>
				...
				v6[597] := p0;
				var v10 := DC1(cf3);
				var v11: array<string> := new string[7] [v10.cf3, cf3, cf3, cf3, if (p1) then cf3 else cf3[v6[597] := f27], ("usp")[v6[597] := f27], cf3];
				v11[319] := cf3;
				f24, v11[319] := p1, ((seq(0x313, i0  => (f27))) + "napupek") + (cf3 + cf3);
				globalState.f10 := f28;
				cf3 := ("rdauad")[fm0(-0x69, f28, v2, v6[597], globalState) := f27];
		}
		...
		match (match v12 {
			...
			case DC2(cf4, cf5, cf6, cf7) =>
				...
				if (!f24) {
					...
					var v14: seq<bool> := [p1];
					var v15: map<int, multiset<bool>> := map[cf4[624] := v13];
					var v16: seq<multiset<bool>> := [if (p0 in v15) then v15[p0] else v13, v13];
					var v17: array<multiset<bool>> := new multiset<bool>[23] [v13, v13, v13, multiset(v14), multiset{!f24, false}, v13 + fm27(f25, globalState), fm27(cf7, globalState), v13, v16[|fm28(globalState)|], v13, fm27(f25, globalState) + v13, if (false) then v13 else v13, v13 + v13, v13, (multiset{f24})[f24 := p0], multiset(v14) * multiset{cf7, p1, f25, p1}, v13, multiset{cf7} * v13[f28 := cf4[624]], v13, if (f24) then v13 else v13, multiset{p1}, v13, v13 * v13];
					v17[283] := v13 * multiset{p1, true};
					v17[283] := v13 * (v13 - v13);
					globalState.f22 := p0;
					p2[689] := p1;
					p2[689] := !v14[p0 % p0];
					var v18: set<bool> := {f24, p2[689]};
					globalState.f16 := (|v18| + p0) % |map[true := p2[689]]|;
				} else {
					var v19 := "dvxphtst";
					var v20: C0 := new C0(if (p1) then seq(-157, i1  => (f27)) else v19);
					var v21: array<map<int, bool>> := new map<int, bool>[15](i2 => v2 + v2);
					v21[499] := v2;
					v1, v20, v21[499] := v1 + (v1 + v1), v20, v2 + v2;
					v6[934] := p0;
					var v22: T3 := new C5(f25, f27, cf7, cf7);
					var v23: map<bool, T3> := map[true := v22];
					v6[934] := fm0(|v23|, if (false) then !f24 else f26, (map v24 : int | (955 <= v24) && (v24 < 0x139) :: (v24 - p0) := (v22.f24))[v0[500][cf6] := v22.f24] + v2, p0, globalState);
					var v25: seq<string> := [v19, v19];
					globalState.f10 := (v20.f31 + "trgbqclqk") <= v25[|(map v26 : string | v26 in v25 :: (v26) := (if (f25 in v1) then v1[f25] else p0))|];
					r1 := !(0x129 != p0);
					var v27 := DC0(!p1, fm0(v6[934], f24, map[cf6 := p1], cf6, globalState), cf6);
			...
			decreases 100 - i3
		{
			...
			var v40 := DC29(v39[p0 := |(seq(0x175, i5  => (f27)))|]);
		...
		var v47 := DC6(v46);
		r2 := match v47 {
			case DC4(cf9) => map[seq(0xce, i6  => (f27)) := p0]
			case DC5(cf10, cf11, cf12, cf13) => map["an" := cf13]
			case DC3(cf8) => (map v48 : string | v48 in map[seq(376, i7  => (f27)) := p1] :: (v48) := (p0)) + map[DC1("oek").cf3 := p0]
	...
	method m0(globalState: GlobalState) returns (r0: bool, r1: array<bool>) {
		...
		for i0 := v0 to v0 {
			...
			v3, globalState.f10, v6[308], globalState.f11, v11 := v3, DC5(f27, f25, i0, i0).cf11, v10, 597 % -922, (v11 + v11) + v11;
			var v12: array<seq<int>> := new seq<int>[1](i2 => (seq(0xe1, i3  => (519))) + [v0]);
			var v13: C3 := new C3(f24, "bylgvmw", !f28, f27, f24, true);
			var v14 := DC25(v13);
			var v15: map<D11, bool> := map[v14 := f28];
			globalState.f10, f24, v12, v15, globalState.f22 := v3 <= v13.f33, f28 <== v13.f32, if (f28) then v12 else v12, v15 + (v15 + map[v14 := v13.f32]), (v0 + v0) / v0;
			r0 := f24;
		...
		for i5 := v0 to fm0(|v27|, if (v0 in v28) then v28[v0] else f26, v28, v0, globalState) / v0 {
			var v29 := "iw";
			...
			v30[467], v31[155], v32, f24, globalState.f10 := if (i5 != i5) then |(if (f24) then v27 else v27)| else v0, v29, v32, f26, f26;
			...
			if (!(|v33| <= i5)) {
				...
				var v41 := DC3(v37);
				globalState.f11, globalState.f7, v37, v17[11], globalState.f2 := i5 * |fm60(v35.fm5(v40[f25 := v39], false, v35.f24, globalState), globalState)|, v30, v41.cf8, fm0(v0, f26, v28, v0, globalState) < v0, v17;
				var v43: array<map<string, int>> := new map<string, int>[20](i6 => map v42 : string | v42 in map["vq" := v0] :: (v42) := (0x2c5));
				var v44: map<string, int> := map[v31[155] := v30[467]];
				v43[305] := v44;
				var v45: set<seq<bool>> := {v2, if (f25) then fm33('d', v30[467], v35.f24, false, globalState) else v2};
				var v46: set<array<int>> := {v30};
				globalState.f7, v43[305], v45, v17[11], v30[467] := v30, v44, v45, {v30} !! (v46 + v46), |map[v34 := v0]|;
				...
				v48[821] := v49;
				v48[821], v0, globalState.f22, globalState.f10 := v49, v0 + i5, |v31[155]|, !(f24 ==> !v17[11]);
			} else {
				var v50: seq<int> := [v0, --v30[467], v30[467], |"clgdsdkx"| / -0x354, |"fg"|];
				v50 := v50 + v50;
				...
				v0 := v0;
				globalState.f16 := v30[467];
				var v57: map<bool, int> := map[f26 := i5];
				var v58: C5 := new C5(v56.f32, f27, v2[i5], fm7(v57, globalState));
			...
			if (f25) {
				...
				var v64 := DC14(v1);
				globalState.f22, v1, v62, v30[467], v63 := |v29|, v64.cf27, map[v17 := v30[467]], v0, v63;
				...
				var v73: map<array<bool>, bool> := map[v72[v0] := f28];
				v17[991] := if (v17 in v73) then v73[v17] else f28;
				var v74 := DC0(v17[991], 942, v0);
				...
				v76 := v76[f25 := if (f24) then v75.(cf2 := 0x19).(cf1 := |fm38(f25, v30[467], multiset(v77), v79[v30[467]], globalState)|) else v74];
		...
		globalState.f16, globalState.f16, v17[590], v85 := v0, v0 / (v0 * v0), f25, {v81} * v85;
		r0 := f25;
		r1 := v17;
	}
	method m1(p0: set<int>, globalState: GlobalState) returns (r0: char, r1: seq<bool>) {
		...
		for i0 := --v2[v1] to v1 {
			...
			match (v6) {
				...
				case DC23(cf43, cf44) =>
					...
					var v16 := new C2(fm41(!f26, v14[|v15|], v4[|multiset{v1}|], globalState), v14);
					...
					var v22: seq<C2> := [v16];
					globalState.f10, globalState.f11, v20 := v21 !! {f25, cf44, f25, f28, f25}, (fm61(f25, i0, fm42(globalState), f28, globalState)).cf49, if (f26) then v22[v1] else v16;
				case DC21(cf40) =>
					...
					var v29: array<int> := new int[19] [|v23|, |fm53(f27, f27, globalState)| - |map[i0 := f25]|, i0, |(set v24 : int | (0x283 <= v24) && (v24 < -926) :: (v24 - |"bvaem"|))|, -i0 % i0, v1, v1, -v1 * v1, |v25|, v1, i0, v1, (if (i0 in v26) then v26[i0] else v28.cf25) - i0, i0, -v1, 0x2e9 - -|v23|, |"lnuul"|, v1, v1 / i0];
					v29[561] := 841;
					v29[561] := v1;
					...
					v27[715] := f24;
					var v34: seq<seq<int>> := [[852, i0, |p0|, v29[561]], v2[v29[561] := |fm41(v32.f32, f27, f25, globalState)|]];
					var v35: seq<seq<seq<int>>> := [v34];
					var v36: array<seq<seq<int>>> := new seq<seq<int>>[9] [v34, v35[v29[561]], v34 + v34, v34, v34, v34, [[v29[561], i0]], v34, v34];
					v36[483] := v34 + v34;
					var v37: seq<array<int>> := [v29, v29];
					var v38: array<array<int>> := new array<int>[15] [v29, v29, v29, v29, v29, v37[v29[561]], v29, v37[|v31|], v29, v29, v29, v29, v29, v29, v29];
					v38[84] := v29;
					v27[715], v36[483], v38[84], v29 := f24, v34, v29, v29;
				case DC24(cf45) =>
					...
					var v45: map<seq<bool>, bool> := map[v4 := v39[249]];
					var v46: multiset<bool> := multiset{v39[249], f24, f25, v39[249], f24};
		...
		if (f28) {
			...
			v48[436] := if (f25) then fm1(f25, f26, |v50[v1]|, globalState) else f24;
			v48 := v48;
			var v51 := "jearwwh";
			v51 := v51 + v51;
			...
			var v54: map<int, int> := map[if (v48[436]) then v1 else 0x187 := v1];
			var v55 := DC12(v2);
			var v56 := DC0(f24, v1, |multiset(v55.cf23)|);
			var v57: seq<D0> := [v56.(cf1 := v1)];
			var v58: map<bool, seq<D0>> := map[f26 := v57];
			v54 := v54[(fm62(-v1, fm5(v58, true, f25, globalState), map v59 : int | v59 in [v1] :: (v59 + 950) := (|(seq(0x349, i5  => (DC15(v51[-0x33c], v52.f24).cf28)))|), globalState)).cf31 := v1];
		} else {
			...
			var v66: map<D11, int> := map[if (f24) then v65 else v65 := -v2[v1] / v1];
			v66 := v66;
			globalState.f22 := 651;
		...
		var v70 := new C3(v67[-v1], fm38(f24, v1, fm52(f24, globalState), v69[v1], globalState), f26, fm47(globalState), f26, f26);
			...
			decreases 100 - i6
		{
			...
			if (f25) {
				...
				var v86 := DC1(v70.f33);
				var v87: array<string> := new string[26] [v70.f33 + v70.f33, v70.f33, v70.f33, v70.f33, v70.f33, v70.f33, v70.f33 + (seq(0x2ff, i7  => (f27))), if ('v' in v85) then v85['v'] else v70.f33, v70.f33, "wpcjvwum" + v70.f33, v86.cf3 + "hjtpq", v70.f33, v70.f33, "kttic", v70.f33 + v70.f33, v70.f33, v70.f33, v70.f33, v70.f33, v70.f33 + v70.f33, v70.f33, v70.f33 + v70.f33, v70.f33 + v70.f33, "axgf", if (v70.f32) then v70.f33 else v70.f33[v1 := f27], v70.f33 + v70.f33];
				v87 := v87;
				...
				var v94: map<int, int> := map[|([v93, v93, v93, v93, v93] + (seq(279, i8  => (v93))))| := |p0|];
				var v95: seq<D2> := [DC4(v70.f32)];
				v92[293], v94, globalState.f10 := v90, v94, |v95| != -v1;
				var v96: array<int> := new int[22](i9 => i9 + v1);
				v96[151] := v1;
				var v97: map<bool, char> := map[v70.f32 := fm21(v70.f33, v70.f32, v1, |"dpebkoc"|, globalState)];
				v96[151] := |(v74 - multiset{0x2f9})[v1 + v1 := |v97| % v1]|;
			} else {
				globalState.f10 := false;
...
class C7 extends T1 {
	...
	function fm5(p0: map<bool, seq<D0>>, p1: bool, p2: bool, globalState: GlobalState): int {
		(|[|"ehhsxxmh"|]| / -176) * (|(set v0 : map<int, int> | v0 in [map[-0x51 := 0x248], map[--|"bcufgk"| := |map[f24 := false]|]] :: (v0))| - 0x2d1)
	}
	function fm15(p0: seq<bool>, globalState: GlobalState): map<int, bool> {
		(map[|map[f24 := f24]| := f24] + map[|map[|[--0x356, |"gdglbboxy"|]| := DC5('j', f24, |[f24]|, |(seq(0x17c, i0  => ('b')))|).cf11]| := f24]) + (map[705 := f24] + map[DC0(f24, |"spnokcg"|, 0x2bc).cf2 := f24])
	}
	method m5(p0: bool, p1: D2, p2: set<bool>, globalState: GlobalState) returns (r0: int, r1: multiset<int>) {
		...
		var v8: array<multiset<bool>> := new multiset<bool>[16] [v0, v0, v0, v0 + v0, fm16(0x176, v4, f24, {v1, if (f24 in v0) then v0[f24] else -v1}, globalState), v0, v0, v0, multiset(v5) - v0, v0, (v0[f24 := v1])[f24 := |v7|], (multiset{f24})[f24 := v1], v0, multiset{p0} + v0, v0, v0 - v0];
		...
		var v12: seq<multiset<bool>> := [multiset{false}, v0, v0, multiset(v10[v1] + (if (f24 in v11) then v11[f24] else v5)), v0];
		r0 := -|v12|;
		var i1 := 0;
		while (f24)
			decreases 100 - i1
		{
			...
			var v23: array<int> := new int[16] [if (v1 in v20) then v20[v1] else |fm41(f24, v9, fm1(f24, fm1(f24, p0, v1, globalState), v1, globalState), globalState)|, |v0| % v1, -|v21|, v1, v1 / v1, |v7|, v1, v1, v1, |v7|, |(if (p0 in v22) then v22[p0] else p2)|, v1 * v1, 0x3c, 993 / v1, v1, --380];
			v23[880] := v1;
			v1, globalState.f11, v17[846], v23[880], globalState.f10 := v1, v1, v18, v1, (|map[f24 := |v5|]| + |v13|) != v1;
			var v25: set<int> := {--0xda, v1, v23[880], -0x2c1, v23[880]};
			var v26: array<char> := new char[17];
			var v27: multiset<array<char>> := multiset{v26};
			var v28 := new C3((set v24 : int | v24 in v6 :: (v24 / 0x19a)) < v25, seq(447, i3  => (v9)), v23[880] < v1, v9, f24, v27 > v27);
		}
	...
	method m6(p0: int, p1: array<map<bool, int>>, globalState: GlobalState) returns (r0: set<seq<bool>>) {
		...
		if (|v12| == -p0) {
			...
			match (v15) {
				case DC22(cf41, cf42) =>
					...
					f24 := {v16[cf42 := cf42]} != {v17[p0], v16, v16, v16, v16};
					...
					v22[523] := if (v23 !in v24) then map[true := if (cf41 in v25) then v25[cf41] else cf42] else map[!!f24 := f24];
				...
				case DC23(cf43, cf44) =>
					...
					globalState.f7 := new int[28] [p0, p0 + p0, |("ifvsn" + v27)|, p0, p0, |(v27 + "ppwatrie")|, fm5(v30, f24, f24, globalState), 373 + |(seq(-0x5f, i1  => ('q')))|, p0, p0, |(v31[p0 := p0] - multiset(v33[p0]))|, p0, p0, |cf43|, p0, p0, p0, p0, p0, p0, -p0, p0, |v31|, |[p0, p0]|, -p0, p0, p0, p0];
					var v34 := 'a';
					v34 := 'h';
				...
				case DC21(cf40) =>
					var v36 := "xnwor";
					v36 := (v36 + v36) + v36;
					globalState.f11 := p0 / (p0 * p0);
					var v37: multiset<int> := multiset{p0};
					var v38 := 'e';
					var v39: set<bool> := {(seq(0x1c, i2  => (v38))) < v36};
					var v40: map<int, bool> := map[p0 := f24];
					var v42: set<int> := {p0, p0};
					var v44: map<int, int> := map[fm0(|v12|, f24, v40, p0, globalState) := -p0];
					var v45: array<int> := new int[25] [p0, if (f24) then |v36| else |v0|, -183, 0x245, |v39| * p0, |v40|, p0 * p0, |v37|, --|("qcahaefbq")[124 := v38]| % p0, p0, |DC3(map v41 : int | v41 in fm63(globalState) :: (v41 + DC5(v38, f24, |"t"|, |{p0}|).cf13) := (p0)).cf8|, p0 / p0, -p0, p0 - p0, p0, p0, p0, -p0, p0, p0, |(if (f24) then v36 else "bx")|, 0x194, |v42|, |((map v43 : int | (0x2e3 <= v43) && (v43 < 0x38f) :: (v43 + p0) := (p0)) + v44)|, p0];
					v45[271] := p0;
					v37, v38, v39, v45[271], f24 := v37 - v37, v38, v39, p0, if (f24) then f24 else f24;
					f24, f24 := v45[271] == (-v45[271] + 0x322), f24;
				case DC24(cf45) =>
					...
					var v56: array<int> := new int[20] [p0, p0, p0, p0, p0 / (v49.(cf6 := p0, cf5 := v48, cf7 := f24)).cf6, p0, p0, if (f24) then 0x2e9 else p0, |fm26(f24, |v0|, p0, globalState)|, |(v50 + v50)|, p0, p0, p0, if (f24) then p0 else fm0(p0, !true, v51, p0, globalState), p0, p0 - |v52|, p0, |v51|, if (p0 in v55) then v55[p0] else p0, p0];
					v47[10] := p0;
					var v57: map<int, int> := map[p0 - p0 := |v50|];
					var v58: seq<int> := [p0, 713];
					var v59: map<int, array<bool>> := map[p0 := v53];
					v47[10], globalState.f11, v53, globalState.f11 := -|v57|, |(if (f24) then v58 else seq(0x346, i4  => (-0x19c)))| / (p0 * p0), if (p0 in v59) then v59[p0] else if (f24) then v53 else v53, 820;
			...
			if (f24) {
				...
				globalState.f11 := -(p0 / p0) * fm0(p0, false, v61, |v62|, globalState);
				...
				var v75: array<multiset<array<int>>> := new multiset<array<int>>[29] [multiset{v71, v71}, v72, multiset{v71, v71}, v72, v72[v71 := p0] * v72, v72, v72 - v72, v72, v72, v72, v72 - v72, v72, v72[v71 := p0], if (v73 in v74) then v74[v73] else v72, v72, v72 * v72, multiset{v71}, if (f24) then v72 else v72, v72, multiset{v71} * v72, v72, v72, v72, v72, v72 + v72, v72, v72, v72, v72];
				v75[584] := v72 * multiset{v71};
				v75[584] := v72[v71 := p0 * -v67];
			...
			var v87: array<bool> := new bool[4] [v0[p0], p0 <= |v86|, f24, f24];
			v87[396] := fm1(!true, fm1(f24, f24, p0, globalState), p0, globalState);
			v87[396] := f24 <==> (|"umpkhd"| >= p0);
			var v88: seq<int> := [p0];
		...
		var v89: seq<int> := [p0, p0, p0 % p0];
		v89 := if (f24 <==> !f24) then v89[p0 := p0] else [p0, 0x155];
		f24 := f24;
		v89 := v89;
		var v90: array<array<bool>> := new array<bool>[4];
		var v91: array<array<D10>> := new array<D10>[5];
		var v92 := DC22(|(seq(-533, i6  => ('a')))|, false);
...
class C8 extends T2 {
	...
	function fm4(p0: multiset<bool>, p1: string, globalState: GlobalState): D0 {
		DC0(f25, 0x1da / -0xbd, if (f24) then 273 else 984)
	}
	function fm5(p0: map<bool, seq<D0>>, p1: bool, p2: bool, globalState: GlobalState): int {
	...
	function fm13(p0: int, p1: bool, globalState: GlobalState): bool {
		DC5('u', f25, |[-898]|, |map[0x198 := 641]|).cf10 !in multiset(seq(-0x208, i0  => ('y')))
	}
	method m0(globalState: GlobalState) returns (r0: bool, r1: array<bool>) {
		var v0 := 0x1c;
		globalState.f16 := v0;
		var v1: array<map<map<D1, int>, bool>> := new map<map<D1, int>, bool>[8];
		v1[587] := fm14(globalState);
		var v2 := "mwgmksent";
		var v3 := DC1(v2);
		...
		var v7: map<int, int> := map[v6[v0] := v0];
		var v8 := DC3(v7);
		...
		var v18: array<bool> := new bool[28] [v11.cf9, f24, f25, true, f25, f25, f24, false, f25, f24, f25, f24, f25, f25, DC2(v12, v13, v14.cf12, f25).cf7, f25, f25, v10[v0], f25, false, if (v16 in v17) then v17[v16] else f25, f24, f24, true, f24, f25, !f25, f25];
		...
		match (v19) {
			case DC2(cf4, cf5, cf6, cf7) =>
				...
				var v25 := new C3(f25, v22[|v23|], if (f25 in v24) then v24[f25] else false, cf5, cf7, !((if (f24 in v24) then v24[f24] else f25) || fm13(cf6, f24, globalState)));
				v18[930] := cf7;
			...
			case DC1(cf3) =>
				if (f25 && f24) {
					var v27 := DC7(v10);
					var v28: seq<D3> := [v27];
					v28 := seq(-0x3bf, i0  => (DC7(v10)));
					var v29: seq<string> := [cf3];
					globalState.f11 := if (|v29| in v5) then v5[|v29|] else -v0 % v0;
					var v30: multiset<array<bool>> := multiset{v18};
					var v31: map<string, int> := map[cf3[if (v18 in v30) then v30[v18] else 0x373 := v13] + cf3 := -v0];
					...
					v33, v39, globalState.f22 := v33, fm53(v13, v13, globalState), if (if (f24) then f25 else f24) then 0x287 else fm0(v0, v32.f28, if (f25 in v40) then v40[f25] else v34, v0, globalState) / v0;
					v12[339] := v0;
					v12[339] := v0;
				} else {
					...
					globalState.f16 := (if (f25) then v0 else v0) / |v43|;
				...
				if (v46.cf37) {
					...
					v49 := v49;
					var v50 := DC6(DC4(v10[v0]));
					var v52 := DC11(v50, v10[|(map v51 : int | v51 in v5 :: (v51 / v0) := (v0))|]);
					var v53: array<char> := new char[27](i1 => v13);
					v53[38] := v13;
					v18[894] := f24;
					v2, v52, globalState.f10, v53[38], v18[894] := cf3, v52, fm13(-v0 * v0, f25, globalState), v13, !fm1(f24, true, v0, globalState);
					v0 := v0 - v0;
					...
					var v60: C3 := new C3(f25, v55.f31, v56 !! v56, v55.f31[v0], f24, v59 > v59);
				...
				v12[501] := |(v61 + v61)|;
				v18[16] := f24;
				globalState.f10, cf3, v12[501], v7, v18[16] := f25, "chp", |v15|, v8.cf8, (v0 + v0) == (if (v0 in v5) then v5[v0] else 0x10e);
				var v62: set<int> := {-v0};
				var v63: C3 := new C3(false, v2[|v62| := v13], f25, v13, v18[16], f24);
				var v64: multiset<C3> := multiset{v63};
				var v65: seq<C3> := [v63, v63, v63];
				v64 := v64 + multiset{v65[v12[501]]};
		}
		
		var v66: array<D3> := new D3[1](i2 => DC8(f25, 250, v0));
		var v67: map<array<D3>, int> := map[v66 := v0];
		v67 := map[v66 := v0] + map[v66 := |{v10[v0], !f25}|];
		r0 := fm1(f24, v15 > v15, v0 % -v0, globalState);
	...
	method m1(p0: set<int>, globalState: GlobalState) returns (r0: char, r1: seq<bool>) {
		...
		for i0 := v0 to |v2| {
			...
			var v12 := "ycyxxta";
			var v13 := DC33(v0);
			v3, v5[554], globalState.f16, globalState.f10, globalState.f11 := v3, fm0(-i0, DC26(f24, true, |(set v10 : int | (-0x3b9 <= v10) && (v10 < 0x230) :: (v10 % i0))|, f25).cf50, v11, i0, globalState), 104 * |([|v12|])[-i0 := 0x60]|, v13.cf55 > fm5(v8, f25, f24, globalState), fm0(i0, f25 <==> !f24, map v14 : int | (0x2ff <= v14) && (v14 < -0x3d6) :: (v14 - i0) := (f24), v0, globalState);
			...
			v17[631] := fm1(false, f24, -fm5(v8, f25, f24, globalState), globalState);
			v15[26], globalState.f10, f24, v5[554], v17[631] := v16, f24, v5[554] <= |v12|, -i0, f24;
			globalState.f10 := (0x32c - v5[554]) <= (|{i0}| + 0x2b);
		}
		var i2 := 0;
		while (f24)
			decreases 100 - i2
		{
			...
			v0, f24, globalState.f10, v20[695], v21 := 0x2d7 * -v0, f24, f24, v19.f31, v21;
			var v22: array<int> := new int[16](i3 => i3 / v0);
			v22[230] := v0;
			v22[230] := v0 / v0;
			var v23: array<char> := new char[3](i4 => if (f24) then 'g' else 's');
			var v24 := 'n';
			v23[248] := v24;
			var v25: set<string> := {seq(0x2db, i5  => (v24))};
			v22[230], v20[695], globalState.f10, v22[230], v23[248] := v0, v20[695], v25 != {v18}, v0, v24;
		...
		match (v32) {
			case DC9(cf19) =>
				...
				var v40: array<char> := new char[6] ['n', v36, v36, fm47(globalState), if (f25 in v39) then v39[f25] else v37[|v1|], v36];
				v40[1] := v36;
				var v41: C7 := new C7(f25);
				v35, v40[1], globalState.f16, globalState.f10, v41 := v35, v36, v0 * -0x70, ([v0, v0, v0, v0, v0] + [v0]) <= DC12(v1).cf23, v41;
		...
		globalState.f22 := |(seq(0x134, i7  => ('v')))|;
		var v44 := DC22(if (fm1(f24, f24, v0, globalState)) then --v0 else v0, f24);
		match (v44) {
			case DC22(cf41, cf42) =>
				var v45: seq<bool> := [f24, f24];
				var v46: map<bool, int> := map[f25 := v0];
				r1, globalState.f22, v1, f24, v1 := ([f25, !f24] + v45) + (v45 + DC7([cf42]).cf15), v0 * (cf41 / |v46|), v1, false <==> cf42, v1;
				if (cf42) {
					var v47 := 'k';
					...
					var v53: array<string> := new string[2] [v50, v50[290 := v47]];
					...
					v52, globalState.f22, v53, globalState.f10, v54[396] := fm53(v47, v47, globalState) - (v52 - {|v58|, cf41}), if (v59 in v60) then v60[v59] else |v63| % -0x137, v53, !(v0 >= cf41), v57;
					globalState.f10 := cf42;
					var v64: map<int, bool> := map[|v1| := false];
					var v65 := DC0(true, v0, cf41);
					var v66: map<bool, seq<int>> := map[!false := v1[|v50| := fm6(v65, globalState)]];
					var v67: set<bool> := {f24};
					var v68: array<seq<int>> := new seq<int>[19] [v1 + v1, v1, v1, v1, v1 + v1, v1 + v1[cf41 := v0], [-v0, |v64|], [v0] + v1, seq(0x362, i8  => (cf41)), if (f25 in v66) then v66[f25] else v1, v1, v1, v1, v1, v1[v0 := 0x15c] + v1, v1, ([|v67|, v0, cf41, v0, if (fm13(|v1|, cf42, globalState) in v46) then v46[fm13(|v1|, cf42, globalState)] else cf41])[752 := v0], v1, [v0]];
					v68[125] := v1 + v1;
					v68[125] := v1;
				} else {
					...
					var v75: array<int> := new int[19] [|v45|, v0, cf41, |v1[v0 := |{cf41, fm0(-913, f25, v69, v0, globalState)}|]|, -|v71|, |"rulfk"|, cf41, |[0x330]|, cf41, v0, v0, |v74|, cf41, cf41, 0x7a, cf41, -237, cf41, v0];
					var v76: seq<char> := [v70, v70];
					var v77 := DC2(v75, v76[0xe2], v0, f24);
					r0 := v77.cf5;
					var v78: set<array<int>> := {v75};
					var v79 := new C4(cf42, v78 > v78);
					v75[985] := |(map v80 : int | v80 in fm53(v70, v70, globalState) :: (v80 - 0x2f5) := (true))| * cf41;
					v75[985] := v0;
					var v81: multiset<string> := multiset{v76};
					var v82: array<char> := new char[4](i9 => v70);
					v82[320] := v70;
					v81, f24, globalState.f10, v82[320] := v81, cf42, cf42, v70;
					f24 := f25 ==> f25;
				}
				...
				if (cf42 <== f24) {
					...
					var v85: map<bool, map<bool, int>> := map[f25 := v46 + v84[v0]];
					v85 := v85[f24 := v46];
					var v86: seq<string> := [seq(868, i10  => ('u'))];
					...
					globalState.f10, globalState.f16, v96[726], globalState.f22, globalState.f16 := cf41 > v0, |v97|, if (true) then v97 else v97 + v97, |p0|, (v0 / |v99|) - cf41;
					v96[726] := v97[0x79 := v98];
				}
			...
			case DC23(cf43, cf44) =>
				if (f25) {
					...
					var v102 := DC12(v1);
					var v103: array<seq<int>> := new seq<int>[15] [(v1 + v1)[-v0 := v0], v1, v1, if (f24) then v1[v0 := v0] else [v0, v0], if (f24) then v1 else v1, v1 + fm42(globalState), v1, fm42(globalState) + v1, v1, (v1 + v1)[v0 := v0], [|(map v101 : int | v101 in DC3(map[v0 := v0]).cf8 :: (v101 * v0) := (true))|], v1, v1, v1, v102.cf23];
					v103 := v103;
					var v104: map<bool, int> := map[f25 := v0];
					var v105: seq<map<bool, int>> := [v104];
					var v106 := 'm';
					var v107: seq<seq<bool>> := [cf43, cf43];
					var v108: multiset<seq<bool>> := multiset{cf43, (cf43 + cf43)[|v105| := f25], fm33(v106, v0, f24, f24, globalState) + cf43, cf43, cf43 + v107[|v104|]};
					v108 := v108[[f24, f25, f25] := v44.cf41 / v0];
					cf44 := cf44;
				...
				var v122: array<int> := new int[26](i12 => i12 * |map[true := seq(0x58, i13  => ('c'))]|);
				var v123 := "kledxy";
				var v124: map<int, bool> := map[v0 := f25];
				var v125 := 'g';
				v122[486] := |(if (cf44) then v123 else v123)[|v124| := v125]|;
				var v126: multiset<bool> := multiset{p0 > p0, cf44, f25};
				var v127: multiset<int> := multiset{-v0, v0};
				globalState.f10, v121, v122[486], v126 := f25, v121, v0 - |v127|, (v126 * v126) * v126;
			...
			case DC21(cf40) =>
				globalState.f11 := v0;
				globalState.f22 := v0 - |{v1[v0], v0, v0}|;
				var v132: seq<array<bool>> := [v30, v30, v30, v30, v30];
				globalState.f2 := v132[v0 % v0];
			...
			case DC24(cf45) =>
				...
				globalState.f7, v30[62], f24, globalState.f16, v136 := v137, v138 == v138, f25, v0 * v0, v136;
				var v139: array<array<int>> := new array<int>[10];
				var v140 := DC16(f25, v0, v30[62], f24, v30[62]);
				v30[62], globalState.f22, globalState.f11, v139 := v30[62], -v0 + (v140.cf31 + v0), v0 + (v0 % v0), if (f25) then v139 else v139;
				v2 := v2;
		}
		...
		for i14 := v0 to DC33(v0).cf55 % |(if (v0 in v146) then v146[v0] else {[v143, v143, v143, v143]})| {
		...
		r1 := match v151 {
			...
			case DC10(cf20) => ([!f25, f24] + [v143.f32])[v0 := v143.f32]
		};
	}
}
...
class C9 extends T3 {
	...
	function fm6(p0: D0, globalState: GlobalState): int {
		--((-0x370 + DC0(f25, 0xc2, 0x35f).cf2) / 675)
	}
	function fm4(p0: multiset<bool>, p1: string, globalState: GlobalState): D0 {
	...
	method m2(p0: int, p1: D0, p2: bool, globalState: GlobalState) returns (r0: bool, r1: bool, r2: int, r3: int) {
		...
		var v4: set<int> := {p0, p0 / p0, p0 - |(if (v2 in v3) then v3[v2] else v2)|};
		v4 := {0x359 + p0, p0};
		var v5: array<int> := new int[4](i0 => i0 % p0);
		v5[241] := if (f25) then p0 else p0;
		...
		v5[241] := -(if ((v2 + (if (p0 in v9) then v9[p0] else v2)) in v6) then v6[v2 + (if (p0 in v9) then v9[p0] else v2)] else p0);
		var v10: multiset<bool> := multiset{f24, !f25, f26};
		if ((|v10| * p0) < v5[241]) {
			var v11: multiset<int> := multiset{v5[241]};
			v11 := v11;
			v5[241] := v5[241];
			if (f26) {
				globalState.f10 := v2[v5[241]];
				globalState.f16 := p0;
				v1 := v1[p2 := p2];
				...
				globalState.f22, f24 := v5[241] / (p0 * |(seq(-0x309, i1  => (f27)))|), |(if (!true) then v7 else v7)| != p0;
				var v13: map<int, int> := map[p0 := p0];
				r0 := fm1(f24, p0 < (if (p0 in v11) then v11[p0] else if (f25 in v8) then v8[f25] else v5[241]), |v13|, globalState);
				r0 := fm8(p2, globalState);
				v5[241] := p0 / p0;
				var v14: set<bool> := {!f24, f26, p2, fm1(f26, f24, p0, globalState), f26};
				var v15: map<bool, map<int, int>> := map[f26 := v13];
				var v16: seq<int> := [p0];
				v5[241], r0, globalState.f7, globalState.f10 := v5[241], v2[|v14| / -v5[241]], v5, |(if (f24 in v15) then v15[f24] else v13)| > v16[v5[241]];
			}
			
			r1, r1, f24, v2, v11 := f25, f26, if (f25 in v1) then v1[f25] else f26, (v2 + v2[-962 := f25]) + v2, multiset{-192, |v7|};
			match (p1) {
				case DC0(cf0, cf1, cf2) =>
					var v17: map<int, bool> := map[cf2 := f24];
					globalState.f10 := if (f25) then !f25 else if (p0 in v17) then v17[p0] else cf0;
					var v18: seq<int> := [v5[241], cf2];
					var v19 := DC0(cf0, v5[241] + -cf2, |v18|);
					v19 := p1;
					r0 := if (cf0) then v2[p0] else v2[p0];
					cf2 := -cf2;
			}
		...
		r2 := if (fm0(v5[241], true, v30, v5[241], globalState) in v29) then v29[fm0(v5[241], true, v30, v5[241], globalState)] else v5[241];
		r3 := v5[241];
	}
	method m3(p0: int, p1: bool, p2: array<bool>, globalState: GlobalState) returns (r0: bool, r1: bool, r2: map<string, int>) {
		globalState.f11 := (p0 / p0) + p0;
		if (f24) {
			...
			var v1: array<int> := new int[5](i0 => i0 % |[f24, !f26]|);
			...
			v7 := v7[DC5(f27, f26, p0, p0) := if (f26) then |v8| else p0];
		} else {
			...
			var v14: seq<bool> := [v13[p0], f26, if (f25) then f24 else true, p1 <==> v9.f25, v9.f25];
			if (v14[p0]) {
				p2[972] := v9.f24;
				var v15 := "a";
				var v16 := DC0(f25, p0, --|v15|);
				p2[972] := p0 > fm6(v16, globalState);
				v9.f24 := p1;
				var v17: array<C4> := new C4[7];
				var v18: C4 := new C4(fm1(f26, v9.f24, p0, globalState), p2[972]);
				v17[5] := v18;
				...
				var v26: seq<array<int>> := [v25[p0]];
				f27, v17[5], globalState.f7, v22, v23[593] := if ((if (v9.f25) then true else v9.f24) in v24) then v24[if (v9.f25) then true else v9.f24] else f27, v18, v25[p0 % 299], v22, p0;
				globalState.f11 := v23[593];
				var v27 := DC22(p0, f24);
				r0, f27 := v9.f25, DC2(v23, 'f', v27.cf41, v9.f24).cf5;
			} else {
				var v28: array<int> := new int[18](i1 => i1 / 0xd4);
				v28[952] := 0x367 / p0;
				v28[952] := p0;
				...
				var v37 := DC19(p1, v28[952]);
				var v38: map<bool, map<T3, int>> := map[f26 := v32];
				var v39: array<map<T3, int>> := new map<T3, int>[29] [v32, v32, v32 + v32, v32, if (v9.f24) then v32 else v32, v32[DC36(v31).cf57 := v28[952]], v32, v32, v32 + v32, v32, v32, v32, map[v31 := v28[952]] + v32, map[if (false in v33) then v33[false] else v31 := -|v34|], v32, v32, v32, map[v31 := |v29|] + v32, v32 + v32, v32, v35.cf58, v32, v36[v37.cf38], v32 + v32, v32, v32, v32, map[v31 := -377], if (false in v38) then v38[false] else v32];
				v39[773] := v32;
				var v40 := DC0(v31.f24, p0, p0);
				v28[952], v30, v39[773] := v31.fm6(v40, globalState), v30, v32 + v32;
				var v41: C3 := new C3(v13[|map[v31.f26 := v9.f24]|], v29, v14[0x32], f27, v31.f25, v31.f24);
				var v42 := DC25(v41);
				var v43: map<D11, bool> := map[v42 := !v31.f25];
				v43 := v43[v42 := v31.f25];
				var v44: seq<T3> := [v31];
				var v45 := DC36(v44[-v28[952]]);
				v45 := DC36(v31);
			}
			...
			if (f24) {
				...
				var v48: set<multiset<int>> := {v47[p0]};
				var v50 := "nrl";
				...
				p2[343] := f26;
				var v55: seq<int> := [p0];
				var v56: array<bool> := new bool[23] [v9.f24, v9.f25, f25, v9.f24, v9.f25, v9.f25, v9.f24, false, f24, f26, v9.f25, f24, true, f26, v9.f24, true, fm1(p2[343], !p1, p0, globalState), v13[-v55[p0]], f25, p1, f24, v9.f24, v9.f25];
				...
				p2[343], globalState.f11, globalState.f16, globalState.f10, p2[343] := p2[343] <== !(if (v56 in v57) then v57[v56] else f26), p0, (if (v9.f25) then fm6(v61, globalState) else |[p1]|) % p0, f25, !(v62 >= {p0, |v64|, 58, p0});
			...
			if (v9.f25) {
				...
				globalState.f11 := (p0 - v9.fm6(v69, globalState)) / p0;
				var v70 := DC19(v9.f24, p0);
				globalState.f16 := (p0 * p0) / v70.cf38;
				var v71 := DC8(v9.f25, p0, p0);
				var v72 := DC31(map[!v9.f25 := v71]);
				var v73: array<seq<string>> := new seq<string>[29](i3 => ["rgqehf", seq(0xc6, i4  => (f27))]);
				var v74 := "kldidyrjv";
				var v75: seq<string> := [v74, v74];
				v73[228] := (v75 + (seq(0x1fb, i5  => (v74))))[p0 := v74];
				var v76: multiset<bool> := multiset{v9.f24, if (p0 in v12) then v12[p0] else p1};
				var v77: map<int, multiset<bool>> := map[p0 := v76];
				v72, v73[228], globalState.f16, v76, globalState.f10 := DC31(map[!v9.f24 := v71]), fm3(v13, p0, if (p1) then f27 else f27, p1, globalState), p0, if (p0 in v77) then v77[p0] else v76 + multiset{v9.f24, p1, p1}, false;
			} else {
			...
			if (v9.f24) {
				...
				v84[638] := p2;
				v84[638] := p2;
				v12 := v12[|multiset{p0, |(seq(-0x1b8, i7  => (f27)))|}| := p1];
				...
				var v100: seq<seq<int>> := [([p0, p0])[p0 := p0]];
				v98[422] := new seq<int>[7] [v99, v99, v99, (seq(-0x5b, i9  => (-p0))) + v99, v99, v100[p0], v99];
		...
		if (f24) {
			...
			if (v101 > v101) {
				var v102 := "xmyh";
				var v103 := new C3(p1, if (f25) then v102 else seq(0xe9, i10  => (f27)), f24, 'j', p1 && !f24, f25);
				var v104: map<bool, int> := map[f25 := p0];
				var v105: map<int, int> := map[p0 + |v104| := p0 / p0];
				v105 := v105[p0 := p0];
				...
				v107[782] := map v108 : int | (637 <= v108) && (v108 < 0x2c4) :: (v108 % -100) := (v104);
				...
				globalState.f7 := v110;
				v110 := v110;
				p2[85] := fm8(f26, globalState) <== f26;
				var v111 := "xqnxyikmi";
				var v112: seq<string> := [v111];
				var v113: set<bool> := {f24};
				p2[85], v111, v112, globalState.f16 := !p1 <== f25, "lfjmvuh", DC41(v112).cf62[|v113| := v111 + v111], p0;
				globalState.f11 := p0;
			...
			var v119 := DC44(v116[|v118|]);
			var v120: multiset<C8> := multiset{v115, v119.cf69};
			v114[829] := v120 + v120;
			v114[829] := multiset(v116);
			...
			var v130 := "dgpevljab";
			var v131: array<int> := new int[17] [-(|v126| * p0), p0, p0, p0, p0, p0, |(map[f26 := |v127|] + v128)|, p0, p0 * |v129|, p0 % p0, |v127|, p0, p0, p0, |v130|, fm0(p0, f24, v126, 497, globalState), p0];
			v131[205] := p0;
			var v132: map<bool, string> := map[fm1(p1, f25, p0, globalState) := v130];
			var v133: seq<array<bool>> := [p2, p2, p2];
			var v134 := DC47(v133);
			globalState.f10, v131[205], v132, globalState.f10, f24 := (v134.cf72 + v133) != v133, p0, v132, (p0 + -fm0(p0, false, v126, p0, globalState)) == p0, f25;
			var v135: T0 := new C2(v130, v130);
			var v136: multiset<T0> := multiset{v135};
			var v137: seq<multiset<T0>> := [v136];
			globalState.f10 := v137[406] >= multiset{v135};
			var v138: array<seq<int>> := new seq<int>[20];
			var v139: seq<int> := [p0];
			v138[104] := v139;
			var v140 := DC12(v139);
			var v141: map<bool, seq<int>> := map[f24 := v140.cf23];
			v138[104] := if (p1 in v141) then v141[p1] else v139;
			r1 := !(p0 > v131[205]);
		}
		...
		for i11 := p0 to fm0(p0, fm1(f24, p1, |["hbmmyrqy"]|, globalState), map[p0 := p1], p0, globalState) {
			...
			if (!(if (p1) then p0 >= p0 else f26)) {
				...
				var v155: map<char, bool> := map['g' := !((seq(-348, i12  => (f27))) <= (seq(-450, i13  => (f27))))];
				var v156: T3 := new C6(f25, v142.f24, v142.f24, f27, false);
				var v157: map<T3, char> := map[v156 := v156.f27];
				v155 := v155[v144[|v157|] := v142.f24];
				var v158: C4 := new C4(f24, v142.f24);
				var v159: set<C4> := {v158};
				v151 := v151[p1 := if (f24) then i11 else -|v159|];
				var v160: seq<int> := [p0];
				var v161: map<seq<int>, int> := map[[|v160|] + (seq(0x380, i14  => (0x3db))) := p0];
				v161 := v161[v160 := p0];
			} else {
				v148 := v148;
				v151 := v151[v147[945] := -0x2f2];
				var v162: seq<int> := [i11];
				var v163: array<string> := new string[21] [v144, v144 + "vbrt", v144 + v144, (seq(-0x95, i15  => (f27))) + v144, v144, "ehjcihrpi", v144 + v144, fm29(globalState), v144 + v144, v144, "yhr", v144[i11 := f27] + v144, "gj", "ahwwypl", v144, "haka", "uwnexvq", v144, "bgyypakx", ("u")[|v162| := f27] + "hmh", v144];
				v163[469] := v144 + v144;
				v163[469] := v144;
	...
	method m0(globalState: GlobalState) returns (r0: bool, r1: array<bool>) {
		...
		if (fm7(v2, globalState)) {
			if (f26) {
				var v3 := new C0(seq(258, i0  => (f27)));
				var v4: seq<bool> := [true, f25];
				var v5: C1 := new C1(v4[|fm27(f26, globalState)|]);
				v5 := v5;
				var v6: map<seq<bool>, string> := map[([f26])[|v1| := f24] := seq(477, i1  => (v3.f31[v0]))];
				...
				f24, v2, globalState.f11 := f25, v2, 494 / v0;
				var v11: array<int> := new int[18] [v0, v0, 435, v0, v0, |(v4[v0 := f25] + v4)|, v0, v0 / |v2|, v0, v0, v0 + v0, v0, v0, -v1[v0], v0, v0, v5.fm5(v9, f24, f25, globalState), if (false) then v0 else v1[v0]];
				v11[525] := |v3.f31| + v0;
				v11[525] := v0;
			} else {
				globalState.f10 := false;
				...
				var v15: C6 := new C6(fm8(f25, globalState), v12[v0 := true] != v12, !(-|v14| >= v0), f27, if (false) then f24 else f25);
			...
			match (v24) {
				case DC26(cf47, cf48, cf49, cf50) =>
					var v25: array<bool> := new bool[21](i2 => v20[v0]);
					v25[960] := f24;
					var v26 := DC0(cf47, v0, cf49);
					var v27 := DC0(cf50, cf49, fm6(v26, globalState));
					v25[960] := v1 <= ((seq(0x125, i3  => (-0x22e)))[-v0 := -|v23.f33|])[cf49 := v23.fm6(v27, globalState)];
					globalState.f10 := v25[960];
					globalState.f16 := -cf49;
					var v28 := DC23(v20, v25[960]);
					var v29 := DC24(v28);
					var v30: map<D10, int> := map[v29 := cf49];
					var v31: multiset<map<D10, int>> := multiset{v30};
					var v32: map<seq<seq<bool>>, multiset<map<D10, int>>> := map[seq(0xf1, i4  => (v20)) := v31];
				...
				case DC27() =>
					...
					v22 := "jwx" + v36.f31;
				case DC28() =>
					var v41: map<bool, string> := map[f25 := (seq(0xeb, i6  => (f27)))[v0 := f27]];
					var v42: seq<string> := [v22, v23.f33, v23.f33, v23.f33, seq(0x3f, i7  => (f27))];
					v22 := (if (v23.f32 in v41) then v41[v23.f32] else v23.f33) + v42[v0];
				...
				case DC25(cf46) =>
					...
					globalState.f22 := (|v49| / v0) % v0;
					var v50: array<C3> := new C3[9];
					v50[681] := cf46;
					v50[681] := cf46;
					var v51: multiset<int> := multiset{v0};
					var v52: multiset<int> := multiset{if (v0 in v51) then v51[v0] else v0, |(v23.f33 + v22)|, |(seq(481, i8  => ({v0, v0})))| * v0, -(if (v23.f32) then v0 else v0), v0};
					v51 := v51;
					var v53 := new C1(v23.f32);
			}
			
			var v54: array<int> := new int[3];
			var v55: multiset<int> := multiset{if (f25 in v2) then v2[f25] else v1[|v23.f33|], v0, v0, v0};
			v54[631] := if (v0 in v55) then v55[v0] else -v0;
			var v56: array<char> := new char[24];
			v56[92] := f27;
			var v57: set<bool> := {f25};
			var v58 := DC0(v23.f32, |v57|, v0);
			var v59: seq<D0> := [v58.(cf0 := v23.f32, cf2 := |v1|)];
			v54[631], globalState.f22, f24, v56[92] := |v1|, -(v0 * fm5(map[fm8(f26, globalState) := v59], v23.f32, f26, globalState)), (v0 % |[v0]|) == v0, f27;
	...
	method m1(p0: set<int>, globalState: GlobalState) returns (r0: char, r1: seq<bool>) {
			...
			decreases 100 - i0
		{
			...
			var v6: map<int, set<bool>> := map[v4 % v4 := v5];
			globalState.f10, globalState.f16, v2, v3[892] := v4 != v4, |v6|, v2, f26;
			if (f25) {
				globalState.f10 := f24;
				...
				globalState.f11 := fm5(v15, v8.f24, f24, globalState) / v4;
				v5 := v5;
			} else {
			...
			globalState.f22 := if (v3[892]) then v4 else v4 / fm0(|v27|, true, v28, v4, globalState);
		}
		var i2 := 0;
		while ([f25, f24] < [false])
			decreases 100 - i2
		{
			...
			if (v32.cf42) {
				...
				v38[597] := v37.f31 + v1.f30;
				var v39: seq<string> := ["vosc" + v1.f30];
				var v40 := DC38(v29);
				v38[597] := v39[v40.cf59];
				...
				v42[581] := v34.f25;
				var v45: seq<int> := [v29];
				globalState.f22, v44, v42[581] := (v29 / -v29) + -v29, v44, -v29 !in v45;
			} else {
				globalState.f16 := v29;
			...
			var v52: array<bool> := new bool[8] [!f25, f24, f25, f24, f25, f26, v1.fm18(|v50|, globalState), v51[v29]];
			var v53: set<array<bool>> := {v52};
			if (v53 != v53) {
				var v54: T1 := new C7(f26);
				v52[5] := v54.f24;
				v29, globalState.f16, v54, globalState.f11, v52[5] := v29, |p0|, v54, v29, f25;
				var v55: array<array<int>> := new array<int>[2];
				var v56: array<int> := new int[21];
				v55[630] := v56;
				var v57: seq<int> := [v29, v29, v29];
				v55[630] := if (v57 < v57) then v56 else v56;
				var v58 := DC15(f27, f25);
				var v59 := DC26(v54.f24, v52[5], v29, v54.f24);
				...
				v63[92], globalState.f11, v65, globalState.f22 := -v29, v29, v65 + v65, -(if (f24) then if (f26 in v66) then v66[f26] else 0x256 else v29) - (-v29 * v29);
				var v67: array<seq<string>> := new seq<string>[25];
				var v68: seq<string> := [v0];
				v67[414] := v68;
				v67[414] := fm3(v51, v29, f27, f24, globalState) + v68;
				var v69: map<multiset<bool>, string> := map[v66 := v1.f30];
				var v70: array<string> := new string[13] [v68[v29], v0, "rsqsmtehn", "beprrpet", v1.f30, if (multiset(v51) in v69) then v69[multiset(v51)] else "enogwbmp", v1.f29, v1.f30, v1.f30, v1.f29, v1.f30, v1.f30, "tpmhlluw"];
				var v71: map<array<string>, int> := map[v70 := v63[92]];
				v71 := v71[v70 := 0xde - -v63[92]];
		...
		for i3 := v77 % v77 to v77 {
			...
			globalState.f10 := (p0 - {if (v79 in v80) then v80[v79] else v77}) == v86[-0x31a];
			globalState.f10 := f26;
			...
			var v91 := new C6(v87 in v88, v82[if (v89 in v90) then v90[v89] else i3], f25, f27, |p0| >= i3);
			v79.f24 := f26;
		}
		var v92 := DC22(v77, f24);
		var v93 := DC5(f27, f24, v77, v77);
		var v94: array<int> := new int[2] [v92.cf41, v93.cf12 % v77];
		v94[847] := --|v1.f29|;
		var v95: C6 := new C6(DC2(v94, f27, 0x3d9, true).cf7, f24, f26, f27, v77 == v77);
		var v96: seq<bool> := [f26];
		var v97: map<bool, C6> := map[v96[|(seq(0x2cf, i5  => (f27)))|] := v95];
		var v98: seq<C6> := [v95, if (v95.f28 in v97) then v97[v95.f28] else v95];
		var v99 := DC14(p0);
		var v100 := DC17(v99);
		v94[847], globalState.f16, v95, globalState.f10 := (v77 % |v96|) - |v0|, v77 / (v77 + v77), if (f25) then v98[-v77] else v95, match v100 {
	...
	method m4(p0: set<string>, p1: bool, p2: array<array<char>>, p3: int, globalState: GlobalState) {
		...
		if (true <==> v4[p3]) {
			...
			var v8: array<int> := new int[13] [0x1e2, if (f25) then p3 else p3, p3, p3, |v4|, --p3, p3, p3, |(set v5 : int | v5 in (seq(0x39d, i0  => (p3))) :: (v5 / 0x39b))| * p3, fm0(p3, f25, v0, |v2|, globalState), p3, |v7| / |v6|, p3];
			v8[863] := 0x298;
			v8[863] := p3 + p3;
			...
			globalState.f11 := |(v12 * v12)| % fm0(if (f25 in v1) then v1[f25] else -p3, f25, v0[fm0(|v6|, fm1(p1, f26, v8[863], globalState), map v13 : int | (-0x1dd <= v13) && (v13 < 0x142) :: (v13 * 0x40) := (f25), p3, globalState) := p1], |v6|, globalState);
			match (fm4(v1, v6, globalState)) {
				case DC0(cf0, cf1, cf2) =>
					var v14: array<bool> := new bool[22](i1 => f26);
					v14[577] := f27 in v6;
					var v15 := DC0(p1, |fm28(globalState)|, p3);
					v14[577] := (if (f25) then cf0 else v15.cf0) || (|v4| < v8[863]);
					var v16 := new C4(!(if (f24) then v14[577] else f25), v3 >= v3);
					var v17: array<C5> := new C5[11];
					var v18: C5 := new C5(f25, f27, f26, f25);
					v17[28] := v18;
					cf0, v17[28], globalState.f11 := cf1 >= v8[863], v18, cf2;
					f24 := fm26(if (-0xeb in v0) then v0[-0xeb] else v14[577], v8[863], |v6|, globalState) <= v6;
			}
			
			var v19 := DC0(f26, v8[863], v8[863]);
			var v20: seq<D0> := [v19];
			...
			if (fm5(v21, f26, p1, globalState) <= p3) {
				...
				var v28: map<int, array<map<int, int>>> := map[|(multiset(v4))[v22.f32 := v8[863]]| := v27];
				var v29: array<array<map<int, int>>> := new array<map<int, int>>[21] [v27, v27, v27, if (!!false) then v27 else v27, v27, v27, v27, v27, v27, v27, v27, v27, v27, v27, v27, v27, if (v4[p3]) then if (v8[863] in v28) then v28[v8[863]] else v27 else v27, v27, v27, v27, v27];
				v29[318] := v27;
				f24, v29[318] := true, v27;
			} else {
				var v30: array<bool> := new bool[28](i3 => f24);
				var v31: set<array<bool>> := {v30, v30};
				var v32: map<bool, set<array<bool>>> := map[f24 := v31 + v31];
				v32 := v32[p1 := v31];
				globalState.f11 := v8[863];
				v8[863] := p3;
				var v34: array<map<D3, char>> := new map<D3, char>[4](i4 => (map v33 : D3 | v33 in (seq(0x241, i5  => (DC8(f24, |[|map[-555 := v8[863]]|]|, 0x1dd)))) :: (v33) := (f27)) + map[DC8(f25, |map[f27 := v8[863]]|, v8[863]) := f27]);
				var v35: array<char> := new char[18];
				v35[889] := if (f24) then f27 else f27;
				var v36: array<seq<bool>> := new seq<bool>[29];
				var v37 := DC42(f26, v36);
				v34, v35[889], v37, v8[863] := v34, f27, v37, p3 - v8[863];
				v8[863] := v2[p3] - p3;
			}
			
		} else {
			f24 := fm29(globalState) == (seq(816, i6  => (f27)));
			globalState.f16, globalState.f11 := -649, p3;
			globalState.f16, f24 := p3, f25;
			var v38: C1 := new C1(!p1);
			var v39 := DC29(v3);
			match (DC43(423 / p3, v38, v39.cf51, p3 % p3)) {
				case DC42(cf63, cf64) =>
					var v40: array<bool> := new bool[6] [false, true, p1, true, v4[p3], f25];
					v40[554] := !f24;
					v40[554] := cf63;
					globalState.f11 := p3;
		...
		if (v61 in v62) {
			...
			match (if (f25) then DC10(v66) else v67) {
				case DC11(cf21, cf22) =>
					...
					var v78: array<int> := new int[17] [p3 % |v69|, if (f25) then |v1| else 971, p3, |(v70 + v70)|, 0x316, p3 - p3, p3, |v71| / |v73|, v74.cf38, |v71|, 0x262, p3, p3, p3, v61.fm6(v77, globalState), p3, p3];
					v78[53] := p3;
					var v79: array<D6> := new D6[25];
					var v80 := DC16(f24, p3, cf22, p1, cf22);
					var v81: C2 := new C2(v71, v71);
					v68[143], v78[53], v79, v80, v81 := if (cf22) then f24 else p1, fm0(-p3, p1, v0, p3, globalState) - p3, v79, v80.(cf32 := v4 !in [v4], cf33 := v72.f24, cf31 := p3), v81;
					var v82: map<int, int> := map[v78[53] := v78[53]];
					var v83: C8 := new C8(if (v78[53] in v0) then v0[v78[53]] else v72.f24, true);
					...
					var v89: map<map<int, int>, map<int, seq<bool>>> := map[(map v86 : int | v86 in (if (true in v87) then v87[true] else seq(0x11e, i11  => (v78[53]))) :: (v86 % p3) := (p3)) + v82 := v88];
					v89 := v89[v82 := v88];
				case DC10(cf20) =>
					var v90: map<bool, bool> := map[v61.fm7(map[p1 := p3], globalState) := p1];
					var v91: array<int> := new int[12](i14 => i14 * 0x97);
					globalState.f10, globalState.f7 := (seq(0x14e, i12  => (multiset(v4))))[|v90| := v1] <= (seq(0x35c, i13  => (multiset([f26])))), v91;
					f24 := v2 <= [p3];
					f24 := f25;
			...
			var v95: map<bool, seq<D0>> := map[f25 := seq(112, i15  => (DC0(f25, p3, p3)))];
			...
			if (p1 ==> f26) {
				...
				var v105 := DC13(v104[p3], p3, v99.f24);
				var v106: set<int> := {p3, |v102|, v105.cf25, p3, p3};
				globalState.f16 := |DC50(v100).cf78| + |((set v101 : int | (0x24e <= v101) && (v101 < 0x209) :: (v101 % p3)) + v106)|;
				var v107 := DC27();
				var v108 := DC33(p3);
				var v109 := "patlt";
			...
			var v118: array<seq<bool>> := new seq<bool>[25] [v4, v4, v4, v4, v4, v4, v4, v4, v4, v4, v4, [if (p3 in v0) then v0[p3] else f26, f25], v4, v4, v4, v4[0x3dc := f26], v4, v4, v4, [f25, true], v4, fm9(v65[f26 := p3], p3, fm21("t", false, p3, |v2|, globalState), p3, globalState), v4, v4, [p1]];
			...
			v122[564] := v120;
			v122[564] := if (f24) then v120 else v120;
			...
			var v132: array<seq<int>> := new seq<int>[12] [v2, v2, v2, v2, fm42(globalState), if (f24) then v2 else v2[p3 := p3], seq(0x116, i18  => (0x217)), v2, v2 + v2, v2[p3 := p3], [|v125|, if (|v129[v127 := p1]| in v3) then v3[|v129[v127 := p1]|] else p3, p3, fm6(v131, globalState)], [p3, p3]];
			v132[482] := v2;
			globalState.f10, v123[724], v132[482], v126, globalState.f10 := f24, v2[867] + (|map[|v2| := p3]| / |(seq(468, i19  => (f27)))[p3 := fm47(globalState)]|), v2 + v2, (v126[|v126| := v124.f27])[p3 := v124.f27], if (f26) then v124.f26 else v2 < v2;
			...
			decreases 100 - i20
		{
			...
			var v145: array<set<bool>> := new set<bool>[18] [v139, fm44(p3, v2, globalState), v139, {false}, v139, v139, v139, v139 * v139, v139, v139, v139, v139 * {f25, p1}, v140.cf85 + {f24, DC11(v143, p1).cf22}, v139 - v139, v139, v139, v144[p3], {f26}];
			...
			if (p1) {
				...
				var v152: set<int> := {|(map v148 : int | (0x21f <= v148) && (v148 < -527) :: (v148 / p3) := (f25))|, fm0(fm0(p3, f25, v0, p3, globalState), p1, map[|"rqabx"| := f24], p3, globalState), p3, |(v1 * v1)|, v61.fm6(v151, globalState)};
				v152 := (v152 * {p3}) * (set v153 : int | v153 in v2 :: (v153 % |"lplftua"|));
				var v154: seq<string> := [v141];
				var v155: array<string> := new string[24] ["sdf", v141, if (p1) then "j" else v141, v154[p3], (seq(0x144, i21  => (f27))) + fm29(globalState), v141 + v154[-0xb4], seq(531, i22  => (f27)), fm26(p1, -p3, p3, globalState), seq(0x37c, i23  => (f27)), v141, seq(0x0, i24  => (f27)), "a" + v141[p3 := f27], v141, seq(0xf, i25  => (f27)), v141, v141, if (f25) then v141 else v141, v141, v141, v154[fm5(fm55('x', p1, f24, f26, globalState), p1, p1, globalState)], v141, v141, v141, v141 + v141];
				v155[101] := v141;
				f27, v147, globalState.f10, globalState.f16, v155[101] := 'j', v147, f25, |(v2 + (v2 + v2))|, v141;
				v0 := v0[p3 := f25];
				...
				v158[80] := v4 + [f26, f24];
				v158[80] := v4[p3 / 0x76 := f26];
				var v159: array<int> := new int[5](i26 => i26 - |v141|);
				var v160: C8 := new C8(true, f25);
				var v161: seq<C8> := [v160];
				var v162: multiset<C8> := multiset{v161[p3]};
				globalState.f10, globalState.f7, f24, globalState.f22, globalState.f10 := !!(p3 !in v2[p3 := p3]), v159, f24 !in multiset(v158[80]), v157.fm23(p1, f27, p3 != p3, |v139|, globalState), (multiset{v160, v160, v160, v160} <= v162) && f25;
				globalState.f22 := p3;
		...
		if (f25) {
			...
			v173 := map v174 : int | (0xaa <= v174) && (v174 < 47) :: (v174 % p3) := (v172);
			...
			if (true) {
				...
				v179 := v179;
				f24 := fm1((if (|(seq(0x16f, i27  => (0x32d)))| in v0) then v0[|(seq(0x16f, i27  => (0x32d)))|] else !f24) ==> !f25, f24, p3, globalState);
				var v180 := "xpfnvs";
				...
				globalState.f16 := |map[p3 := "dq" != ("qjcwnh")[|v185| := f27]]|;
			}
			...
			decreases 100 - i28
		{
			...
			var v195 := "ght";
			var v196: set<bool> := {p1, p1, if (v195[p3] in v194) then v194[v195[p3]] else f26};
			...
			var v210: array<set<int>> := new set<int>[26] [v193, v193, v193, v193 + v193, v193, v193, v197[p3], (set v198 : int | v198 in v3 :: (v198 + |{true, false}|)) * v193, set v199 : int | (-0x14c <= v199) && (v199 < 0x28a) :: (v199 + p3), v193, v193 + v193, {|v2|, 0x16}, v193 * (set v200 : int | (0x356 <= v200) && (v200 < -817) :: (v200 * p3)), v193, v193 * (set v201 : int | (0x33 <= v201) && (v201 < 0xf2) :: (v201 - p3)), v193, v193 * (if (v191.fm5((map[f26 := v204])[p1 := v204], true, p1, globalState) in v203) then v203[v191.fm5((map[f26 := v204])[p1 := v204], true, p1, globalState)] else v193), (set v205 : int | v205 in v3 :: (v205 + 134)) * {p3, p3, fm5(v206, f24, f26, globalState), |v195|}, v193, if (f24) then v193 else v193, DC14(v193).cf27 * v193, {p3}, set v207 : int | (0x313 <= v207) && (v207 < 0x1f0) :: (v207 - p3), v193, v193, {p3, 162, |(if (0xc3 in v209) then v209[0xc3] else v196)|}];
			v210[430] := v193;
			v210[430] := {p3} + v197[p3];
...
method Main() {
	...
	var v9 := "ylhvnb";
	var v10: map<int, string> := map[392 := seq(837, i0  => ('j'))];
	var v11: array<string> := new string[22] [v9, v9, "g", v9, v9, v9, "hi", v9, v9, v9, v9, v9, "npevt", v9, v9, v9, if (0x2ba in v10) then v10[0x2ba] else v9[v1 := v6], v9, v9, "dh", v9, v9];
	...
	v3 := !!v3;
	globalState.f10 := v9 != (seq(-755, i1  => (v6)));
	var i2 := 0;
	while (v3 <== v3)
		decreases 100 - i2
	{
		...
		match (v12) {
			case DC0(cf0, cf1, cf2) =>
				...
				v5[18], v1, v13[748] := |v15|, (cf2 + cf2) / cf1, if (v12 in v16) then v16[v12] else v5;
				var v17: map<int, int> := map[v5[18] := v5[18]];
				var v18: set<map<int, int>> := {map[|"umttmamld"| := v1], v17 + v17, map[v5[18] := cf1] + v17, v17};
				v18 := v18 - v18;
		}
		
		if (v1 == fm0(v1, fm1(false, v3, |(seq(-112, i4  => (v1)))|, globalState), map v19 : int | v19 in map[|(seq(0x2fe, i5  => (|map[v1 := v1]|)))| := v1] :: (v19 + v1) := (v3), v1, globalState)) {
			var v20: multiset<int> := multiset{|(seq(0xaf, i6  => (v1)))|};
			var v21: map<bool, multiset<int>> := map[false := v20];
			var v22: seq<int> := [|v21|, v1, v1];
			globalState.f11 := v22[v1];
			var v23: map<int, bool> := map[|v4| := v3];
			var v24: seq<map<int, bool>> := [v23];
			var v25: map<bool, bool> := map[v3 := v3];
			var v26: map<map<bool, bool>, int> := map[fm2([v3, v3, v3], v3, -|v9|, fm0(v1, !v3, v24[v1], v1, globalState), globalState) + v25 := |v9[fm0(v1, !v3, v23, v1, globalState) := v6]|];
			v6, v3, v26, v9, globalState.f22 := v6, v3, v26 + v26, "ekekwwlsn", |((seq(183, i7  => (v9))) + fm3(v2, v1, v6, v3, globalState))|;
			...
			var v37: array<seq<bool>> := new seq<bool>[26] [[v3, false], [v3, v3], v2, v2, [v3], v2, v2, v2, v2, [v3], v2, [v3], v2, v2, v2, v2[v1 := v3], [v3, v3], v2, [v3], v2, v2, v2, v2, v2, fm9(map[v3 := |v36|], v1, v6, |v34.f30|, globalState), v2];
		...
		var v45: seq<C9> := [v44];
		globalState.f16 := v1 / (|v45| - v1);
	}
	var v47: array<set<int>> := new set<int>[23](i8 => set v46 : int | v46 in multiset{|(seq(242, i9  => (v6)))|, v1} :: (v46 * 0x1b4));
	v47 := v47;
	if (v1 > (v1 - v1)) {
		...
		match (v53) {
			...
			case DC49(cf73, cf74, cf75, cf76, cf77) =>
				v0[243] := cf73 > 229;
				var v66: set<int> := {v1};
				v0[243] := (if (cf75) then v66 else v66) !! v66;
				globalState.f16 := -v1;
				var v67: C7 := new C7(v3);
				v11, v6, cf76, v67, v0[243] := v11, 'o', (if (v3) then v6 else v6) !in (seq(0x217, i10  => ('j'))), v67, v48.fm22(!(multiset{v48.fm22(cf76, globalState), false} !! multiset([cf75])), globalState);
				cf73 := -cf73;
			case DC47(cf72) =>
		...
		v80[31] := v81;
		v80[31] := (map v82 : int | (62 <= v82) && (v82 < 0x1a4) :: (v82 / v1) := (v1))[v1 := v1];
		...
		v86[497] := v87;
		v85[307], v86[497] := v0, v87;
		v5[643] := -934;
		v5[643] := v1 % -|v86[497]|;
		var v88: multiset<bool> := multiset{v3, v3};
		globalState.f10, v6 := (v3 && fm1(false, true, v1, globalState)) in v88, v6;
		var v89 := DC52(|v80[31]|, v3, v3);
		var v90: map<int, D22> := map[0x239 % v5[643] := v89.(cf83 := v3, cf82 := v3)];
		v90 := v90[v5[643] := v89];
	}
	...
	if ((v91 + v91) !! v91) {
		...
		v97[117] := fm36(v1, !fm1(v3, !false, v1, globalState), v3, globalState);
		var v98: set<int> := {v1};
		var v99: seq<int> := [v1];
		globalState.f22, v3, v97[117] := 0x375 / (0x352 % |v98|), (v1 !in v99) <== v3, v6;
		var v100 := DC55(v1, v3, v3);
		v100 := DC55(|v9|, v3, true);
		globalState.f22 := 0x13f;
		v6 := if (v3) then v6 else v97[117];
	} else {
		...
		v106[718] := v107;
		v105[479], v106[718] := v104, if (v107.f25) then v107 else v107;
	...
	globalState.f11, v112[85], globalState.f10 := (-v1 % 0x256) * |"kpsdwsoss"|, v11, if (v3) then v3 else v2[|v113|];
	if (0x125 == (v1 - v1)) {
		var v114 := DC33(v1 * v1);
		match (v114) {
			case DC33(cf55) =>
				...
				var v119: map<C7, int> := map[v117 := v118[v1]];
			...
			case DC34() =>
				...
				globalState.f11 := fm0(v1, v0[433], v122, v1, globalState) / |v9|;
				var v123: set<int> := {0x29e};
				v5[469] := |(v123 * v123)|;
				v5[469] := v1;
				var v124: seq<string> := [v9];
				var v125 := new C0(v124[|v2|]);
			case DC32(cf54) =>
				var v126: T0 := new C2(v9, v9);
				var v127: map<bool, T0> := map[true := v126];
				var v128: multiset<T0> := multiset{v126, v126, v126, v126, if (v2[v1] in v127) then v127[v2[v1]] else v126};
				v5[345] := |v128|;
				v5[345] := v1;
				...
				v131 := v131[map[v129 := v132[v5[345]]] + v130 := v6];
		}
		
		var v133: T0 := new C2("ryvrbqnqv", v9);
		var v135: map<T0, int> := map[v133 := |(map v134 : int | (0x23 <= v134) && (v134 < 11) :: (v134 / v1) := ("oka"))|];
		...
		decreases 100 - i13
	{
		...
		var v144: set<T3> := {v143[v1], v142, v142};
		var v145: seq<int> := [|v140|, fm0(0x2b5, v3, v141, -|v144|, globalState), v1];
		var v146 := DC54(v139);
		globalState.f16, v139, globalState.f11 := v145[-(v1 % v1)], v146.cf85, -(v1 - v1) * v1;
		v0[425] := v3;
		v0[425] := v3;
		globalState.f11 := |(set v147 : int | (0x212 <= v147) && (v147 < 621) :: (v147 % v1))|;
		if (v142.f25) {
			var v148: C4 := new C4(v142.f24, v0[425]);
			...
			v150[308] := if (v151 in v152) then v152[v151] else v148;
			v3 := v142.f25;
			globalState.f16 := -(-(if (v1 in v113) then v113[v1] else v1) * -(v1 / |multiset{v142.f26}|));
			...
			v1, v5[170], v0[425] := -(v1 % (if (v142.f24) then v1 else v1)), if (!v142.f24) then if (true) then |v141| else -v1 else v1, !fm1("ff" < v9, v142.f26 <==> v142.f25, |v155|, globalState);
			var v156: map<bool, bool> := map[multiset([v1]) <= v113 := v0[425]];
			v156 := v156[v142.f24 := true];
			var v157, v158 := m10(v145 + v145, v142.f25, globalState);
			var v159: C9 := new C9(v0[425], v6, v1 > |multiset{v142.f25}|, v142.f26);
			v159 := v159;
		}
	...
	for i14 := |v161| to |v162| {
		...
		var v169: map<int, int> := map[|(seq(0x3bb, i15  => (v6)))| := v1];
		var v170: multiset<map<int, int>> := multiset{v169, v169};
		var v171: seq<int> := [i14];
		if (v170 !! (if (v167) then v170 else (multiset{map[i14 := fm0(|v9|, v3, v165, i14, globalState)]})[map[v171[v1] := |v171|] := |v4|])) {
			globalState.f7 := new int[23](i16 => i16 % i14);
			var v172, v173 := m10(v171, v3, globalState);
			var v174 := new C0(v9 + v9);
			v171 := if (v173) then seq(23, i17  => (0x55)) else v171 + v171;
			...
			var v180 := DC12(v171);
			v180 := DC12(v171).(cf23 := seq(0x388, i18  => (DC0(v167, i14, i14).cf2)));
			globalState.f16 := v171[v1] * v1;
			v168[337] := v167;
			v168[337] := v178;
	...
	globalState.f22 := (v160.fm6(v182, globalState) % v1) + fm0(v1, v3, fm57(v1, v183, v185, globalState), v1, globalState);
	...
	v188 := v188[v2[|v190|] := fm68(globalState)];
