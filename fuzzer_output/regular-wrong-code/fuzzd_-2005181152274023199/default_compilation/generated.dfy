...
function fm0(globalState: GlobalState): seq<map<int, bool>> {
	[map[|(seq(0xfd, i0  => (0x165)))| := true], map[|[-341]| := true] + map[|{'q'}| := false]]
}
...
function fm7(p0: int, p1: bool, globalState: GlobalState): map<bool, int> {
	(map[true := 0x4c] + map[false := -449]) + (map[false := |(map v0 : int | (692 <= v0) && (v0 < 418) :: (v0 / |[!!true]|) := (false))|] + map[true := 0x2c9])
}
...
function fm9(p0: bool, globalState: GlobalState): set<bool> {
	{true <==> true, false, ["qylrshxjb"] < (seq(340, i0  => ("tp"))), true <== true, false}
}
...
function fm16(p0: int, p1: bool, p2: char, p3: int, globalState: GlobalState): string {
	(seq(0x1dd, i0  => ('m'))) + ((seq(866, i1  => ('u'))) + (seq(0x2db, i2  => ('t'))))
}
function fm23(p0: set<int>, p1: int, globalState: GlobalState): bool {
	!(0xe4 >= -DC78(0xec, |(map v0 : int | (861 <= v0) && (v0 < 0xf3) :: (v0 + 0x3bf) := (|(seq(0x363, i0  => ('p')))|))|, map[true := 0x39e]).cf135)
}
...
function fm28(p0: D0, globalState: GlobalState): D1 {
	DC3(|(seq(-560, i0  => ('e')))| % |"fvskreag"|, |[--0x5c, -0x383, |{0x53}|, |map[-0x200 := 'w']|, 0x179]| % |"nf"|, 0x1 / -868, 636, -(-|['i']| - -0xcb))
}
function fm29(p0: bool, p1: int, p2: string, globalState: GlobalState): map<bool, int> {
	(map[true := |map[|{map[false := 0x134], map[false := -0x169]}| := true]|] + map[true := 0x138]) + (map[true := -DC3(0x70, |map[!true := true]|, |map[|"wasaeyknb"| := -0x195]|, |map['r' := seq(0x27, i0  => ('f'))]|, -633).cf8] + map[false := 0x1d9])
}
function fm30(p0: seq<multiset<bool>>, p1: map<string, bool>, globalState: GlobalState): map<int, int> {
	map v0 : int | v0 in ([0x4c] + [|(set v1 : int | v1 in [0x1c3, |map[true := 0x33d]|] :: (v1 + |{false, !false}|))|]) :: (v0 + -|[|map[true := false]|, |"rg"|]|) := (530 + |[459, |[|(seq(-0x367, i0  => ('n')))|, -|map[true := false]|, -|[false]|, 0x3f]|]|)
}
function fm31(p0: int, p1: bool, p2: D1, globalState: GlobalState): multiset<D3> {
	multiset([DC13(|(map v0 : int | (471 <= v0) && (v0 < 0x17b) :: (v0 / |(set v1 : int | v1 in (seq(-833, i0  => (|"vcf"|))) :: (v1 * -187))|) := (false))|, "fahmrgw", |{0x162, 0x1d}|, |"dnvpfxa"|, 's')] + [DC13(0x16c, seq(-0x39a, i1  => ('y')), -0x205, |(map v2 : int | (-0x261 <= v2) && (v2 < 922) :: (v2 / |map[true := -0x1fe]|) := (true))|, 'q'), DC13(|map[true := true]|, "tosl", |[false]|, 972, 'q')]) + (multiset{DC13(0x33e, "xg", |{true, true}|, |[|{map v3 : int | (438 <= v3) && (v3 < 0x255) :: (v3 - |(seq(0x3c3, i2  => ('x')))|) := (true)}|]|, 'r')} + multiset{DC13(|map[true := |{0xca, 0x1c9}|]|, "aiq", 0x52, |"aynd"|, 'p'), DC13(0x1ed, "uw", 462, |[true, true]|, 'f')})
}
...
function fm35(p0: int, p1: bool, globalState: GlobalState): D0 {
	DC1(false, |"mphupn"|, ['x', 'v'], !(368 in (seq(0x1eb, i0  => (DC40(|{false, false}|, 0x1c2, 0xd3, |map[|[true, true]| := false]|).cf72)))), -(0x2c6 % 0xe))
}
...
function fm39(p0: bool, p1: int, p2: int, globalState: GlobalState): seq<string> {
	((seq(829, i0  => (seq(0x1ee, i1  => ('p'))))) + ["cykrhjkg", "fe"]) + ["cwmhocws", "boaslvno"]
}
...
function fm43(p0: bool, p1: bool, globalState: GlobalState): map<bool, int> {
	(if (true) then map[true := 0x2cb] else map[!!!!true := 0xa3]) + (map[false := |[0x1a7, 0x20f, -|[true, false, true, false, false]|]|] + map[true := |map[|map[|multiset([0x17])| := |(map v0 : int | v0 in {|multiset{true, false, false}|, 71} :: (v0 % |"bgpxdfu"|) := (true))|]| := |"gmkulofbe"|]|])
}
...
function fm45(globalState: GlobalState): set<string> {
	({"frqcl", "njtvu", "lb", seq(13, i0  => ('i'))} * {seq(588, i1  => ('b')), seq(933, i2  => ('f')), "anl"}) + DC79(set v0 : string | v0 in [seq(242, i3  => ('q')), "kvx", "wkyiedaj"] :: (v0)).cf137
}
...
function fm50(p0: bool, p1: string, p2: map<bool, bool>, p3: char, globalState: GlobalState): map<map<int, bool>, bool> {
	map[map[0x359 := true] := "yog" !in ["rmbiql", seq(-0x3b4, i0  => ('q'))]]
}
function fm51(p0: bool, p1: int, p2: bool, p3: D10, globalState: GlobalState): seq<map<int, int>> {
	[(map v0 : int | (0x252 <= v0) && (v0 < 461) :: (v0 / 0x14b) := (-0x1bf)) + map[0x15e := |map[-0x1f9 := false]|]]
}
function fm52(p0: int, p1: int, globalState: GlobalState): multiset<string> {
	multiset(["u", "dsxh", "yagibnn", seq(947, i0  => ('b')), seq(0x29f, i1  => ('g'))]) * (multiset{"seqmtp", "fi", seq(0x14f, i2  => ('x'))} * multiset(["aahqhps", "ie"]))
}
function fm53(globalState: GlobalState): seq<D0> {
	([DC1(false, 952, seq(572, i0  => ('c')), true, |[0x1ae]|), DC1(false, 0x1e2, ['b'], false, 280)] + (seq(0x3b2, i1  => (DC1(false, 943, ['n', 'a', 'n', 'f', 'w'], true, 0x1b))))) + ([DC1(true, 0x336, ['j'], true, |(seq(983, i2  => ('h')))|), DC1(true, |[|map[|map[-0x380 := true]| := true]|]|, seq(400, i3  => ('w')), true, |(map v0 : int | (-0xf1 <= v0) && (v0 < 903) :: (v0 * DC80(false, {|multiset{true}|}, true, -938).cf141) := (|[|(map v1 : char | v1 in ['e'] :: (v1) := (-0x356))|]|))|)] + [DC1(!!!false, 0x1ae, ['s', 'r'], !true, 36), DC1(!!!false, |[|(seq(-918, i4  => ('u')))|]|, ['r', 'm'], false, 0x1b3), DC1(false, 0xa7, ['a'], false, -0x3dc), DC1(true, 0x14e, ['o'], true, |[true, true]|)])
}
function fm54(p0: int, p1: int, globalState: GlobalState): map<map<char, int>, int> {
	map v0 : map<char, int> | v0 in [map v1 : char | v1 in ['m', 'a'] :: (v1) := (|map[|[false, true]| := true]|), map['q' := 0x1f3], map['j' := |(seq(0x303, i0  => ('f')))|]] :: (v0) := (|(seq(-0x287, i1  => (|multiset{multiset([false])}|)))| + |multiset([0x142, |[false]|])|)
}
function fm55(p0: int, p1: bool, p2: int, p3: bool, globalState: GlobalState): D9 {
	if (!(multiset([|{|"ptwyd"|, --0x179, |map[false := !false]|, 263, -|"qnmsblqx"|}|]) >= multiset{0x37a})) then DC23(false, !false, -|map[!false := -|(seq(0x3b2, i0  => ('f')))|]|) else DC23(true, true, |[map[false := true]]|)
}
function fm56(p0: bool, p1: bool, p2: int, p3: string, globalState: GlobalState): D13 {
	match DC42(true) {
		...
		case DC41(cf73) => if (!false) then DC33(false, |[!false]|, false, 203) else DC33(false, -0x26e, true, -|map[-|(seq(0x139, i0  => ('u')))| := multiset{--|map[false := |[true]|]|}]|)
	}
}
...
function fm59(globalState: GlobalState): D3 {
	DC13(|[false, false]| - |[false, false]|, seq(0x95, i0  => ('s')), if (false) then 654 else |"wblaoerrr"|, 0x4d % -|[|DC67(true, 399, [|map[false := false]|, -158, |map[true := !false]|], true).cf118|]|, 'u')
}
...
function fm61(p0: bool, globalState: GlobalState): D21 {
	match DC53(true, DC50(true).cf84, 'l') {
		...
		case DC54(cf90) => DC55({map[|(seq(-0x186, i0  => ('t')))| := |{true}|]})
	}
}
...
function fm65(globalState: GlobalState): map<string, int> {
	map v0 : string | v0 in ((seq(-0x199, i0  => ("hw"))) + [seq(-614, i1  => ('c'))]) :: (v0) := (|("isiwmcovd" + "dkdiy")|)
}
method m0(globalState: GlobalState) returns (r0: int, r1: int, r2: array<int>, r3: map<int, bool>) {
	...
	if (v3.cf61) {
		...
		var v8: C5 := new C5(seq(0x3d4, i0  => ('j')), "lf", fm65(globalState) != v7);
		var v9: multiset<int> := multiset{v6 * v6};
		var v10: array<int> := new int[15];
		var v11: seq<array<bool>> := [v1];
		var v12: multiset<seq<array<bool>>> := multiset{[v1], v11, v11[fm1(fm1(v6, v0, v0, globalState), v0, v0, globalState) := v1]};
		v5, v8, r2, v6, v9 := v5, v8, v10, if ((v11 + v11) in v12) then v12[v11 + v11] else v6 + v6, v9;
		var v13 := 'n';
		v13 := fm46(globalState);
		v0 := (v6 - v6) == v6;
		var v14: seq<bool> := [v0];
		var v15: map<int, int> := map[v6 := |(seq(100, i1  => (v13)))|];
		...
		var v20 := 0x339;
		var v21: C0 := new C0(|map[fm1(v20, v0, v0, globalState) := v0]|, |(seq(0x2b4, i2  => ('m')))|);
		...
		if (v27.cf92) {
			...
			var v29 := DC39(v0, 306);
			v1[373] := v29.cf67;
			var v30 := new C3(fm33(v1[373], globalState), "xj", (seq(807, i3  => (v24))) + v25, v26.f18);
			var v31: set<bool> := {!v26.f18, v26.f18};
			v1[373] := v31 >= {v1[373], v0};
			...
			var v39 := new C6([v26.f18], |v26.f19|, v26.f18, seq(-0x22, i4  => ('w')), v26.f20);
		}
	...
	if (v0) {
		...
		var v66: map<multiset<bool>, int> := map[v65[0x317] := v55];
		var v67: array<string> := new string[25] [v2, v2, (v2 + v2)[v60.f35[-v55] := v61], "cgsgthyqi", seq(0x3b8, i6  => (v61)), "mfhtk" + "fkjwvty", v2, v2, v2[v55 := v61], "gwuxuka", "yftjej", fm16(v55, true, v61, v55, globalState), v2, v2 + v2, "nvtv", (if (v0) then seq(0x271, i7  => (v61)) else "nnpdry")[if (-v55 in v63) then v63[-v55] else v55 := v61], "vbke", if (fm23({v55}, |v66[v64 := |(seq(459, i8  => (0x3ba)))|]|, globalState)) then v2 else v2, v60.fm37(v55, globalState), seq(0x1fe, i9  => (v61)), v2 + v2[v55 := v61], seq(0x261, i10  => (v61)), v2 + (seq(0xdb, i11  => (v61))), v2, v2];
		v67 := v67;
		var v68 := new C0(v55, v55);
		v0, v55 := v0, v55;
	} else {
		var v69: set<bool> := {v0};
		var v70: T0 := new C5(v2, seq(853, i12  => ('k')), v69 >= v69);
		v70 := v70;
		var v71: C5 := new C5(v2, seq(0x335, i13  => ('b')), if (v70.f18) then v70.f18 else v70.f18);
		v71 := v71;
		var v72 := 999;
	...
	var i14 := 0;
	while (fm23(if (v0 in v78) then v78[v0] else v77, |((seq(-0x1a8, i15  => ('d'))) + v2)|, globalState))
		decreases 100 - i14
	{
		if (i14 >= 100) {
			break;
		...
		var v83: array<int> := new int[3] [v75, |v80|, -(|v2| / |v82|)];
		v81[385] := v75;
		...
		v81[703] := fm1(|multiset(v87)|, false, v0, globalState);
		var v88 := 'n';
		var v89: map<int, int> := map[|([v0, v0])[398 := v0]| := v75];
		...
		var v104 := DC5(DC3(v75, v75, |v89|, |multiset{v92.f18}|, v103[v75]));
		var v105 := DC5(v104);
		var v106 := DC5(v105);
		var v107 := DC5(DC5(v106));
		var v108: map<D1, int> := map[v107 := v75];
		v81[385], v81[703], v96, v1[825] := if ((v99 + v99) in v100) then v100[v99 + v99] else |v102|, if (DC5(v105) in v108) then v108[DC5(v105)] else v75, v96, !false;
		var v109 := DC40(v81[385], v75, v81[385], v81[385]);
		var v110: map<int, D16> := map[v81[385] := v109];
		var v113: array<map<int, D16>> := new map<int, D16>[9] [v110, v110, v110, v110, v110, map v111 : int | (0x3e0 <= v111) && (v111 < -0x94) :: (v111 / -0x2c7) := (v109), v110, map v112 : int | v112 in v89 :: (v112 * v75) := (v109), v110];
		var v114 := DC70(v75, v81[385], v0, v113);
		var v115: C5 := new C5(v2, v2, v91.f18);
		var v116: seq<C5> := [v115, v115, v115];
		var v117: map<bool, bool> := map[v91.f18 := v0];
		var v118: array<D26> := new D26[21] [v114, DC70(v75, 0x269, !v91.f18, v113), v114, v114.(cf123 := 0x367), v114, v114, DC70(-v75, v81[385], v91.f18, v113), DC70(v75, |v116|, false, v113), v114, v114, v114, DC70(|{v1[825]}|, |v80|, v91.f18, v113), DC70(|v89[v81[385] := v81[385]]|, v75, v91.f18, v113), DC70(|[v1[825]]|, |v117|, v0, v113), v114, v114, v114, v114, v114, v114, v114];
		v118[212] := if (v115.fm15(|v77|, v75, globalState)) then v114 else v114;
		var v119: array<bool> := new bool[19];
		var v120: map<array<bool>, char> := map[v119 := v88];
		v118[212] := DC70(v75, |v120|, v0, v113);
		v88 := v88;
	}
	r0 := -v75;
	r1 := --v75;
	...
	var v129: map<T0, int> := map[v128 := |(seq(0x69, i18  => (v126)))|];
	...
	var v133: array<int> := new int[26] [0x57, |(v123 + map[false := seq(-0x15, i17  => (v75))])|, v75, -(if (v124 in v125) then v125[v124] else v75), v75, |("sihya")[v75 := v126]|, --(v75 % |v122|), v75, 0x2df, v75, |v2| * v75, -0x195, 0xe8, -|v127| + (if (v128 in v129) then v129[v128] else -v75), |v2|, v75, |(v121 + v121)|, v130.cf42, v75 * v75, v75, v75, |v131|, 0x2be / |v132|, if (|v2| in v131) then v131[|v2|] else |fm49(v75, v75, v75, globalState)|, v75 / -|v2|, v75 + v75];
	r2 := v133;
	r3 := map v134 : int | v134 in v77 :: (v134 % v75) := (v0);
}
trait T0 {
	const f18 : bool
...
class C1 extends T0 {
	...
	function fm21(p0: int, p1: string, p2: map<bool, set<char>>, p3: seq<bool>, globalState: GlobalState): string {
		(if (f18) then "fsarcis" else seq(-683, i0  => ('h'))) + ("k" + "cjfcrawcl")
	}
	function fm22(p0: string, p1: D0, globalState: GlobalState): int {
		0x396 % -(-0x3b7 / |map[f18 := 0x26a]|)
	}
	method m1(p0: string, p1: int, p2: string, globalState: GlobalState) {
		var i0 := 0;
		while (f18 ==> !f18)
			decreases 100 - i0
		{
			...
			v9[840] := f18;
			var v10: set<int> := {p1};
			globalState.f0, v8[893], v9[840] := p1, v5, fm23(v10, fm1(|(seq(0xbc, i3  => ('j')))|, f18, f18, globalState) * p1, globalState);
		}
		...
		v20[910] := f18 in map[f18 := v19];
		v17, v18[568], v20[910], globalState.f2 := fm26(v13, p1 * p1, globalState), v19, f18, p1 == (fm1(p1, f18, f18, globalState) - -fm1(|p2|, f18, f18, globalState));
		v20[910] := f18;
		if (v16.cf16) {
			var v21 := DC1(f18, p1, if (v20[910]) then ['l'] else [v14, v14, v14, v14], f18, p1);
			v21, v16, v20[910] := v21, DC7(p1, v20[910], p1).(cf15 := p1 / p1, cf16 := v20[910]), true;
			...
			globalState.f0 := v25;
			v27[301] := |p0[|p2| := v14]|;
			var v30: multiset<bool> := multiset{v20[910]};
			var v31 := DC3(v25, v26, v25, if (f18 in v30) then v30[f18] else v26, v26);
			var v32 := DC5(DC5(v31));
			var v33: map<bool, map<bool, int>> := map[v20[910] := v11[f18 := v25]];
			var v34: seq<map<bool, int>> := [v11, v11];
			var v35 := DC1(!v20[910], |(if (true in v33) then v33[true] else v34[if (f18 in v30) then v30[f18] else v25])|, fm21(|p2|, p2, fm27(globalState), v19, globalState), false, |v13|);
			v26, globalState.f0, v20[910], v27[301], v32 := v26, v25, false, fm22(p2 + p0, v35, globalState), v32;
		}
...
class C2 extends T0 {
	...
	function fm20(p0: int, p1: map<int, char>, p2: bool, globalState: GlobalState): set<string> {
		{"qa", "pj", "wx"} * ({seq(0x191, i0  => ('f')), "wpnvrbl"} * {"lftn", "ogsxkngw"})
	}
	method m1(p0: string, p1: int, p2: string, globalState: GlobalState) {
		for i0 := p1 to fm1(f33, f18, f18, globalState) {
			...
			match (v3.(cf12 := f18)) {
				case DC3(cf7, cf8, cf9, cf10, cf11) =>
					...
					var v8: array<bool> := new bool[10] [f18, v4[-0x36c], f18, fm19(v5, globalState), cf9 > |p2|, f18, f18, !(v7 !! v7), !f18, f18];
					v8[104] := f18;
					...
					v8[104], v9 := p0 <= (p2 + p0), map[f18 := v10[|v12|]];
					cf7 := cf8;
					var v13: set<char> := {if (f18) then p0[0x2dc] else v2, v2, v2, v2, v2};
					v13 := {v2} + (v13 * v13);
					v6[451] := p1;
					v6[451] := fm1(|v0|, v8[104], f18, globalState);
				case DC4(cf12) =>
					...
					v15[492] := if (true in v16) then v16[true] else cf12;
					var v17: T0 := new C1(v15[492]);
					v17 := new C1(v17.f18);
				case DC2(cf6) =>
					var v19: map<int, int> := map[i0 := -697];
					var v20: map<int, int> := map[|v19| := |p2|];
					var v21: seq<map<int, int>> := [v20];
					var v22: multiset<seq<int>> := multiset{seq(0x30b, i2  => (p1))};
					globalState.f3 := |(map v18 : int | v18 in (v20 + v21[if ([i0, i0] in v22) then v22[[i0, i0]] else i0]) :: (v18 / -0x328) := (f18))|;
					globalState.f12 := p0;
					...
					globalState.f0, v23[927], v25 := |v26| % ((if (f18 in v1) then v1[f18] else p1) - |v27|), v24, v25;
				...
				case DC5(cf13) =>
					...
					v33[430] := v29;
					var v34 := DC0(f18);
					v33[430] := fm28(v34.(cf0 := f18), globalState);
					globalState.f0 := -f33;
					globalState.f3 := p1;
			...
			var v41: array<int> := new int[10] [p1, i0, i0, -i0 / fm1(|p2|, f18, f18, globalState), -i0, i0 * |v39|, f33, f33, i0, i0 / |v40|];
			v41[765] := 0x4f;
			...
			v37, v41[765], globalState.f0, globalState.f2 := v37, i0 * f33, |(f34[i0 := |v0|] + f34)| - (if (v43 in v44) then v44[v43] else i0), f18 <== fm19(v45, globalState);
		}
		...
		var v55: seq<bool> := [f18, v50[p1], if (-|v54| in v52) then v52[-|v54|] else !true];
		var v56 := DC3(-p1, f33, p1, f33, -|v50|);
		var v57: map<bool, int> := map[f18 := |v53|];
		var v58: set<int> := {|v46|};
		var v59: array<int> := new int[23] [f33, |fm29(f18, f33, "deqxsk", globalState)| - f33, -(if (f18 in v46) then v46[f18] else |v48|), fm1(f33, f18, v47.cf16, globalState), |v49|, |v50|, p1, fm1(f33, f18, f18, globalState), (v56.(cf11 := -0x118, cf10 := f33)).cf10, f33, f33, p1, f33, f34[|v57|], |(p0 + (seq(0x251, i4  => ('f'))))|, f33, f33 + f33, -(|v58| - -(if (f18 in v46) then v46[f18] else |p2|)), f33, p1, p1, |f34|, if (f18) then f33 else p1];
		v59 := v59;
		...
		v63[408] := if (f33 in v52) then v52[f33] else true;
		var v64 := DC8(f18, false, f18);
		var v65: map<D2, array<bool>> := map[v64 := v63];
		var v66 := 'n';
		var v67: map<char, int> := map[v66 := 0x316];
		v60, v61, globalState.f10, v63[408], v65 := DC10(v60).cf22, v61 + v61, f34 + ([|p0[if (v66 in v67) then v67[v66] else p1 := v66]|] + f34), (v58 !! (set v68 : int | v68 in v48 :: (v68 % |"u"|))) <== fm19(v47, globalState), v65;
		var i5 := 0;
		while (true)
			decreases 100 - i5
		{
			...
			if (f33 > (|p2| * -0x1b8)) {
				...
				var v69: set<multiset<bool>> := {v46, v46[f18 := p1], v46};
				var v70 := new C0(f33, |v69|);
				var v71 := new C0(|v50|, f33);
				var v72: map<array<bool>, bool> := map[v63 := v63[408]];
				v72 := v72[v63 := f18];
				...
				var v76: map<bool, map<bool, int>> := map[v63[408] := v57];
				...
				v79 := (v79 + map[false := v63[408]]) + v79[true := true];
				...
				v80[879] := if (v63[408]) then v56 else DC3(v75.f32, |v81|, f33, p1, p1);
				v80[879] := if (f18) then v56 else v56;
			}
			
			globalState.f2 := f18;
		}
		for i6 := fm1(f33, f18, v63[408], globalState) to f33 {
			globalState.f3 := f33;
			var v82: map<bool, array<bool>> := map[true := v63];
			var v83: seq<map<bool, array<bool>>> := [v82, v82, v82];
			var v84: map<int, map<bool, array<bool>>> := map[0x373 := v82 + v83[p1]];
			v84 := v84[i6 := map[!v63[408] := v63]];
			var v85 := new C0(|(seq(-0x44, i7  => (v66)))| * p1, f33);
			v63[408] := v63[408];
		}
		globalState.f2 := f18;
	}
...
class C3 extends T1 {
	...
	method m2(p0: map<array<int>, map<bool, int>>, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int) {
		...
		if (multiset([p1, p1, p2, p1]) == v0) {
			...
			v1[126] := if (false) then v3 else v3;
			v1[126] := v3;
			globalState.f2 := f18;
			var v4 := 640;
			...
			var v7: array<multiset<bool>> := new multiset<bool>[19] [v0, v0, multiset{f18} - v0, v0[p1 := v4], v0, v0, v0, v0, v0, multiset([f18]) * v0, v0 * v0, v0 - v0, v0 * v0, v0, v0, multiset(v5), (multiset(v5))[if (v4 in v6) then v6[v4] else p1 := v4], v0, v0];
			var v8 := DC17(v7);
			v7 := v8.cf38;
		} else {
			...
			if (p2) {
				...
				var v16: set<bool> := {(map v14 : int | (0x135 <= v14) && (v14 < 139) :: (v14 / v10) := (v10)) != v15, f18};
				var v17: map<map<bool, int>, set<bool>> := map[v11 + v9[679] := v16];
				v13[172], v16, globalState.f15 := v10, if (v11 in v17) then v17[v11] else v16, v10;
				...
				globalState.f10 := (fm40(v21, globalState))[|f19| := v10];
				var v22: seq<bool> := [p1];
				var v23: map<seq<bool>, bool> := map[v22 := p1];
				v10 := if (if (v22 in v23) then v23[v22] else p2) then v13[172] else v10;
				var v24: seq<array<bool>> := [v12];
				var v25: array<array<bool>> := new array<bool>[22] [v12, v12, v12, v12, v12, v12, v12, v12, v12, v12, v12, v12, v12, v12, v12, v12, v12, v24[|(seq(839, i1  => (v10)))|], v12, v12, v12, v12];
				v25[212] := v12;
				...
				v25[212], globalState.f9, globalState.f15, globalState.f15 := if (p2) then v12 else v12, (seq(0x254, i2  => ('u')))[|v26[f18 := v22]| := 'j'], if (v9[679] in v27) then v27[v9[679]] else if (v10 in v29) then v29[v10] else |f19|, if ((p2 <== !!p2) in v0) then v0[p2 <== !!p2] else v10;
			} else {
				var v30 := new C1(p2);
				var v31 := new C1(!p1);
				globalState.f3 := (-0x382 * v10) / v10;
		...
		globalState.f12, v35, globalState.f3 := (f19 + f19) + ("jnb" + f19), v35, match v36.(cf23 := p1) {
			...
			case DC12(cf25, cf26, cf27, cf28, cf29) => cf27 / cf27
			case DC13(cf30, cf31, cf32, cf33, cf34) => cf33 / |{p2}|
			...
			decreases 100 - i5
		{
			...
			v41[306], v44, v37 := v37, v45[if (v37 in v46) then v46[v37] else v37], |(v42 + v42[0x170 := p2])|;
			v41[306] := f35[0x303] % (0x11f - 0x247);
			globalState.f2 := false;
			var v47: map<bool, array<bool>> := map[true := v43];
			var v48: map<int, seq<int>> := map[0xe3 := f35];
			var v49 := new C2(|v47|, (seq(0x306, i7  => (f35[|f35|]))) + (if (v41[306] in v48) then v48[v41[306]] else f35), f18);
		}
	...
	method m9(globalState: GlobalState) returns (r0: string, r1: bool, r2: bool) {
		...
		var v3 := DC4(f18);
		var v4: array<bool> := new bool[16] [f18, f18, v3.cf12, f18, f18, true, (fm41(globalState)).cf0, f18, f18, f18, v1[v0], f18, !f18, f18, f18, f18];
		...
		v6[854] := if (f18) then v5 else v5[v4 := v4];
		v6[854] := v5;
			...
			decreases 100 - i2
		{
			...
			i2 := i2 + 1;
			var v9: map<int, int> := map[v7[306] := v7[306]];
			var v10: set<bool> := {f18, false};
			var v11 := 'b';
			var v12 := DC13(v7[306], f19, -0x207 % v7[306], if (v0 in v9) then v9[v0] else |v10|, v11);
			match (v12) {
				case DC11(cf23, cf24) =>
					globalState.f15 := v0;
					...
					v13[620] := v14 + multiset{seq(0x1ef, i3  => (v11))};
					v13[620] := v14 - multiset{f19, f19, f19, fm37(v0, globalState)};
				...
				case DC13(cf30, cf31, cf32, cf33, cf34) =>
					globalState.f3 := --cf30;
					var v15: map<int, seq<int>> := map[v7[306] / cf30 := seq(816, i4  => (v0))];
					v15 := v15[-cf30 := f35];
					var v16: array<map<int, int>> := new map<int, int>[2];
					v16[193] := map[cf32 := |[v11, v11]|];
					v16[193] := v9;
					globalState.f10 := (fm42(f18, f18, f18, globalState)).cf43;
				case DC10(cf22) =>
					v4[202] := fm2(false, true, f18, globalState);
					var v17 := DC1(false, |map[-fm38(f19, globalState) := -v0]|, f19, f18, fm38(f19, globalState));
					v0, v7[306], v4[202] := v7[306], v17.cf2, true;
					var v18: seq<array<int>> := [v7];
					var v19: array<char> := new char[1](i5 => v11);
					v19[10] := v11;
					var v20: map<bool, seq<array<int>>> := map[f18 := v18];
					v18, v7[306], v19[10], v4[202] := (v18 + v18) + (if (v4[202] in v20) then v20[v4[202]] else v18)[v7[306] := v7], v0 + v7[306], v11, v4[202];
					v7[306] := |fm37(v0 - v0, globalState)|;
					var v21 := new C0(|f19|, if (v7[306] in v9) then v9[v7[306]] else -v0);
				case DC14(cf35) =>
					var v22: multiset<char> := multiset{v11, 'f', v11};
					v0 := v7[306] + (if ('u' in v22) then v22['u'] else if (v0 in v9) then v9[v0] else v0);
					var v23: multiset<int> := multiset{v7[306], v0, |"vuiukyo"|, |v1|};
					var v24: seq<seq<bool>> := [v1, v1];
					r1 := (if (v0 in v23) then v23[v0] else |multiset(v24)|) <= (v0 / v7[306]);
					var v25: array<multiset<bool>> := new multiset<bool>[18];
					v25[718] := multiset{f18};
					globalState.f2, globalState.f10, r1, globalState.f15, v25[718] := [v7[306], v0, -0x195] != f35, [0x2ea], !f18, v0, multiset{f18};
					var v26: array<seq<int>> := new seq<int>[20];
					v26[247] := f35;
					var v27: map<int, bool> := map[|f35| := f18];
					r2, globalState.f0, globalState.f2, v26[247], v10 := f18, v0, !(f18 || f18), f35 + ([|v27|] + f35), v10;
			}
			
			v10 := {true, !f18 || !f18, f18, f18};
			v7[306] := v7[306];
			v7[306] := -145 * (v0 - v7[306]);
		}
		globalState.f10 := f35;
	...
	method m10(p0: bool, globalState: GlobalState) returns (r0: set<bool>, r1: int, r2: int, r3: string) {
		...
		if (!((v0.cf47 * v1) == v1)) {
			...
			var v14 := DC3(v5.cf46, 0x24d, fm1(-v3.f33, true, v4[596], globalState), |v9| + v2, |v10| % v13.cf2);
			v14 := v14;
			globalState.f3 := v3.f33 / (-|(seq(53, i0  => (-v2)))| % |f19|);
		} else {
			globalState.f2 := f18;
			globalState.f2 := f18;
			var v15: multiset<bool> := multiset{p0};
			var v16: C2 := new C2(|v15|, f35, f18);
			v16 := new C2(v16.f33 / v16.f33, v16.f34, p0);
			...
			var v24: array<array<bool>> := new array<bool>[25] [v19, v19, v19, v19, if (p0) then v19 else v19, v19, v19, v19, v19, v19, v19, v20[v16.f33], v21.cf39, v19, v19, v19, v19, v19, if (v22 in v23) then v23[v22] else v19, v19, v19, v19, v19, v19, v19];
			v24 := v24;
		}
		...
		v25[341] := p0;
		var v26 := 0x59;
		var v27: C0 := new C0(v26 / v26, v26);
		v27 := v27;
		var v28: seq<bool> := [p0];
		var v29: map<int, bool> := map[|v28| := v25[341]];
		if ((-v27.f32 !in v29) || v25[341]) {
			globalState.f0 := 0x340;
			var v30: map<array<bool>, int> := map[if (f18) then v25 else v25 := f35[571]];
			v30 := v30[v25 := -v26];
			if (if (f18) then p0 else v27.f32 == v26) {
				...
				v25[341] := false;
				var v31 := 'm';
				var v32: map<bool, char> := map[v25[341] in v28 := v31];
				v32 := v32[v25[341] := v31];
				globalState.f2, v25[341] := if (f18) then v28[|fm43(v25[341], p0, globalState)|] else p0, v25[341];
				var v33: array<int> := new int[19](i2 => i2 - v27.f32);
				var v34: map<array<int>, array<int>> := map[v33 := v33];
				v34 := v34[if (v25[341]) then v33 else v33 := v33];
			}
			...
			var v39: array<int> := new int[24] [fm1(if (v27.f32 in v35) then v35[v27.f32] else v27.f31, fm2(v25[341], v25[341], v25[341], globalState), v25[341], globalState), |(v28 + v28)|, v27.f31 - |f19|, v27.f31 / -|f35|, v27.f31, v27.f31, v27.f31, v27.f31, v27.f32, v38.cf42, v27.f31, v27.f31, v27.f32, v27.f31, v26 + v27.f31, v27.f31, v26 / 0x3da, v27.f31 + -v27.f32, v27.f31, v27.f32, v27.f31, 0x330, |f19|, v27.f31];
			v39[892] := |v29|;
			v39[892] := v27.f32;
			var v40 := DC4(f18);
			match (v40) {
				case DC3(cf7, cf8, cf9, cf10, cf11) =>
					...
					var v59: array<map<D8, D2>> := new map<D8, D2>[13] [v50, map v51 : D8 | v51 in v52 :: (v51) := (v49), map[v38 := v49] + v50, v50, v50, v50, v50, map v53 : D8 | v53 in ([v38])[|v56.cf50| := v38] :: (v53) := (v49), v50, v50, (if (v57 in v58) then v58[v57] else map[v38 := v49])[v38 := v49], v50, map[v38 := v49]];
					v59[882] := v50 + map[v38 := v49];
					v59[882] := v50 + v50;
				...
				case DC4(cf12) =>
					...
					var v63: map<bool, map<seq<int>, array<int>>> := map[|(map[v26 := v27.f32])[v27.f32 := v39[892]]| < v27.f32 := (if (|f19| in v62) then v62[|f19|] else v61) + map[f35 := v39]];
					v63 := v63[f18 := v61 + v61];
				...
				case DC2(cf6) =>
					...
					v65[533], v66, v39[892] := v36, multiset(v28) + v66, v27.f32;
					globalState.f3 := if (true in v66) then v66[true] else -0x19e / v27.f32;
					v25[341] := f18;
					r3 := ("uyagbxxr" + f19) + f19;
				case DC5(cf13) =>
					var v67: map<int, seq<bool>> := map[v27.f31 := v28];
					var v68: map<bool, seq<bool>> := map[v25[341] := v28];
					var v69 := DC30(v28);
					var v70: array<seq<bool>> := new seq<bool>[25] [v28, v28, [f18], v28, [v25[341]], if (v27.f32 in v67) then v67[v27.f32] else v28, v28, fm44(v27.f31, f18, v36, |multiset{fm1(v27.f32, v25[341], p0, globalState), v27.f31, -v27.f31}|, globalState), v28 + v28, v28, v28, fm44(v27.f31, f18, v36, v26, globalState), v28[v39[892] := v28[v26]], v28 + v28, v28 + v28, v28[-0x273 := !f18], v28 + v28, (if (f18 in v68) then v68[f18] else v28) + v28, v28, v28 + v28, v69.cf53, v28, v28, v28 + v28, v28];
					var v71: set<string> := {f19};
					var v73: array<set<string>> := new set<string>[21] [{f19}, v71, v71, v71 - v71, {"dr"}, v71, {seq(0xeb, i4  => (v36)), f19}, fm45(globalState), v71, v71, fm45(globalState) - (set v72 : string | v72 in map[f19 := v27.f32] :: (v72)), {f19}, v71, v71, v71, v71 * {f19}, v71, v71, v71, v71, v71];
					v73[902] := v71;
					var v74: map<array<bool>, bool> := map[v25 := false];
					var v75 := DC32(v74);
					var v76: map<bool, int> := map[p0 := |(v74 + v75.cf54)|];
					v0, v70, v73[902], globalState.f3, v76 := v0, v70, v71, v27.f31, v76[fm2(fm2(false, v25[341], p0, globalState), fm2(v25[341], !p0, p0, globalState), true, globalState) := v27.f31];
					v36 := v36;
					var v77: array<array<bool>> := new array<bool>[13];
					v77[938] := v25;
					var v78: array<D4> := new D4[27];
					var v79: map<bool, array<D4>> := map[!f18 := v78];
					v77[938], v39[892], globalState.f2, v79 := if (v25[341]) then v25 else v25, 0x14f, f18, v79;
					var v80: array<seq<int>> := new seq<int>[23];
					v80[751] := f35;
					globalState.f2, globalState.f2, v80[751], globalState.f9 := v25[341] <== !v28[v26], p0, f35 + f35, seq(0x251, i5  => (f19[v27.f31]));
			}
			...
			if (fm2(p0, f18, f18, globalState)) {
				...
				var v82: map<string, int> := map[seq(178, i6  => ('u')) := v27.f31];
				v81 := v81[v25 := |(v82 + v82)|];
				var v83 := new C1(p0);
				...
				v25 := new bool[29];
				v25[341] := if (false) then f18 else f18;
				globalState.f2, globalState.f15 := v27.f31 == v27.f32, fm38("rsjphe", globalState);
				var v85 := 'w';
				var v86: multiset<char> := multiset{v85, fm46(globalState), f19[v27.f32], v85};
				v86 := (v86 - multiset(f19)) + v86;
				globalState.f2 := p0;
			}
			
			globalState.f2 := v28[v27.f32];
			if (!v25[341]) {
				var v87 := new C1(v25[341]);
				globalState.f2 := v25[341];
				globalState.f3 := v27.f32;
				v87.m1(f19, fm1(v27.f32, f18, false, globalState) / v27.f31, "hkpnyc", globalState);
				var v88: set<int> := {-v27.f31};
				var v89: map<set<int>, bool> := map[v88 := p0];
				v89 := v89[v88 := false];
			} else {
				globalState.f2 := f18;
				r1 := fm1(v27.f31, fm3(|v28|, globalState), v25[341], globalState) / v27.f31;
				...
				var v95: set<D11> := {v94};
				v91[566] := v95;
				globalState.f15, v90[22], v91[566], globalState.f2 := (v27.f32 / v26) / |f19|, fm1(v26, fm2(false, f18, p0, globalState), f18, globalState), v95 + v95, f18;
				globalState.f12 := f19;
				var v96 := 'v';
				v96 := v96;
			}
...
class C4 extends T0, T1 {
	...
	method m1(p0: string, p1: int, p2: string, globalState: GlobalState) {
		if (|"pcybxl"| < f30) {
			...
			if (v13.cf16) {
				globalState.f3 := (f30 + p1) % (if (f18) then f30 else p1);
				var v14: set<int> := {f30};
				...
				v17[726] := |p2|;
				v17[726] := f30;
				var v18: map<array<int>, bool> := map[v17 := v16[98]];
				var v19: seq<bool> := [f18];
				var v20 := DC3(0x135, |v19|, -878, p1, v17[726]);
				v16[98], globalState.f2, globalState.f2, globalState.f0 := if (v17 in v18) then v18[v17] else v16[98], f18, true, --v20.cf8;
			} else {
			...
			if (f18) {
				...
				var v33: array<char> := new char[22] [f29, f29, f29, f29, f29, f29, f29, if (|p2| in v32) then v32[|p2|] else 'q', 'q', f29, f29, f29, f29, f29, f29, f29, f19[|v7|], f29, f29, f29, f29, 'x'];
				...
				var v38: array<int> := new int[24] [f30, |v35|, f30, fm1(p1, true, f18, globalState), p1, f30, |v9| % f30, -f30, f30, 0x2f3 * |p0|, f30 - 0x15, |f19|, -f30, 0x39a, p1, p1, p1 - f30, if (f18 in v36) then v36[f18] else 614, f30, f30, |v37|, p1 / p1, f30, f30];
				...
				v29, v30, globalState.f0, v31, v38 := v39[p1], v30, |p0| - f30, if (v40[f30]) then v31.(cf21 := v41) else v31, v38;
				v30[55] := 0xfe <= |"vd"|;
				v30[55] := fm23({p1}, 0x112, globalState) ==> (v12 < v12);
				var v42 := 'v';
				v42 := f29;
				v30[55] := f18;
				var v43 := DC0(f18);
				...
				var v46 := DC4(f18);
				var v47: array<int> := new int[23] [f30 % p1, p1 - p1, f30, |fm31(|DC15(v45).cf36|, f18, v46, globalState)|, f30, f30 / f30, 0xc5, f30, f30, -(p1 % p1), f30, f30, f30, |p2|, f30, |p0|, 672, f30 * |v7|, p1, |v7|, |p2[|v12| := f29]|, p1, f30];
				v47[343] := p1;
				var v48: multiset<string> := multiset{"nla" + f19, seq(0x26d, i1  => (f29)), seq(0xe4, i2  => (f29))};
				globalState.f2, globalState.f3, v47[343], v48 := f18 || f18, (f30 - p1) + -p1, |v12|, (if (f18) then v48 else v48[seq(0x4d, i3  => (f29)) := p1]) - v48[p2 := f30];
				var v49: array<map<int, int>> := new map<int, int>[26](i4 => map[f30 := v47[343]]);
				...
				v51[726] := !fm3(|(map[745 := v47])[v47[343] := if (f18 in v53) then v53[f18] else v47]|, globalState);
				v51[726] := f18;
				v51[726] := v51[726];
			}
		...
		for i5 := |v55| to 0x335 {
			...
			v56[910] := |(if (f18 in v58) then v58[f18] else v57)| / i5;
			var v59: array<map<bool, int>> := new map<bool, int>[28](i6 => map[f18 := f30] + map[true := p1]);
			var v60: seq<int> := [p1];
			var v61: map<bool, int> := map[!f18 := |v60[f30 := i5]|];
			v59[137] := v61;
			var v62: seq<multiset<bool>> := [multiset{f18}, fm32(globalState)];
			var v64: seq<string> := [p2];
			var v65: multiset<bool> := multiset{true};
			var v66: map<int, int> := map[i5 := v60[|v65|]];
			globalState.f0, v56[910], globalState.f15, v59[137] := |(fm30(v62, map v63 : string | v63 in v64 :: (v63) := (f18), globalState) + (if (f18) then v66 else v66))|, -(i5 - i5), i5, v61;
			globalState.f2 := fm2(f18, false, f18, globalState);
			if (f18) {
				var v67: T0 := new C2(fm1(v56[910], f18, f18, globalState), [|p2|], !v57[|multiset{|[f30, f30]|, f30}|]);
				var v68 := DC2(v67);
				...
				var v73: multiset<map<int, int>> := multiset{v66[v56[910] := 0x2b7], v66[i5 := f30]};
				v66 := v66[f30 := |v73|];
				v56[910] := v56[910] / p1;
				v56 := v56;
			} else {
			...
			var v76 := new C0(p1, |DC1(fm3(v56[910], globalState), i5, p2, f18, |v75|).cf3| % v56[910]);
		}
		globalState.f2 := f18;
			...
			decreases 100 - i8
		{
			...
			globalState.f2 := f18;
			if (!f18) {
				var v78: seq<int> := [-(161 / p1)];
				var v79: seq<bool> := [f18, f18, f18, f18];
				globalState.f10, globalState.f2, globalState.f15, globalState.f2, globalState.f0 := v78[|(f19 + "tdxtowp")| := p1], fm1(f30, f18, true, globalState) >= -p1, -(972 * f30), f18 <==> f18, |v79[p1 := f18]| * p1;
				...
				globalState.f0 := f30 % (-|v89| * f30);
				var v90: array<bool> := new bool[15](i10 => f18);
				var v91: seq<bool> := [true, f18];
				v90[31] := v91[p1];
				globalState.f12, globalState.f2, globalState.f15, globalState.f2, v90[31] := "vqgd", f18, p1, !f18 <== true, fm2(f18, f18, f18, globalState);
				globalState.f2 := v90[31];
		...
		for i11 := 528 to |v98| {
			var v99: array<bool> := new bool[4] [f18, f18, f18, v97[i11]];
			...
			var v105: array<int> := new int[29] [f30, p1, |"btuvfidmd"|, -309, fm17(-f30, v101, globalState), p1, 725, -p1, p1, i11, p1, |p2[i11 := f29]|, p1, p1, |v102|, f30, f30, i11, f30, fm18(globalState), i11, |(set v103 : int | (711 <= v103) && (v103 < 0x25e) :: (v103 - i11))|, p1, -f30, f30, p1, |v104|, p1, p1];
			var v106: map<bool, array<int>> := map[v97[0x3a4] := v105];
			v106 := v106[f18 := v105];
			globalState.f15 := i11;
			var v107: map<bool, bool> := map[f18 := v97[i11]];
			v107 := v107[false := f18];
		}
	...
	method m2(p0: map<array<int>, map<bool, int>>, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int) {
		...
		match (v0) {
			case DC7(cf15, cf16, cf17) =>
				...
				match (v1) {
					case DC11(cf23, cf24) =>
						...
						globalState.f0 := cf15 / |fm32(globalState)|;
					case DC12(cf25, cf26, cf27, cf28, cf29) =>
						var v4: seq<bool> := [f18];
					...
					case DC13(cf30, cf31, cf32, cf33, cf34) =>
						...
						v19[870] := v20;
						cf16, v19[870], globalState.f2 := !(p2 && f18), v20, p2;
					...
					case DC10(cf22) =>
						...
						v22[415], globalState.f12, globalState.f2, v24 := p2, f19, f30 !in v23, v24;
						var v25: map<bool, bool> := map[f18 <==> cf16 := cf15 == cf17];
						var v26: multiset<int> := multiset{v23[cf15]};
						var v27: set<bool> := {cf16, v26 > v26[cf15 := fm1(444, f18, cf16, globalState)], p2, false};
						var v28: multiset<bool> := multiset{!fm3(f30, globalState)};
						var v29: seq<bool> := [f18, p1];
						var v30: map<bool, seq<bool>> := map[f18 := v29];
						var v31 := DC36(v30);
						globalState.f2, v25, v27 := true, (fm47(v28, cf17, |v31.cf64|, v22[415], globalState))[v22[415] := f18], v27;
						var v32: array<int> := new int[10];
						v32[687] := f30;
						v32[687] := cf15 + cf17;
						var v33: map<char, array<int>> := map[f29 := v32];
						var v34 := DC7(cf17, false, |v33|);
						var v35 := new C2(cf15 * v32[687], v23 + v23, v34.cf16);
					case DC14(cf35) =>
						var v36: seq<bool> := [false, cf16, f18];
						var v37: map<bool, seq<bool>> := map[cf16 := v36[cf15 := true]];
						v37 := v37[p1 := v36 + [p1]];
						...
						globalState.f3 := fm1(f30 + fm1(|v41|, cf16, p1, globalState), cf16, if (f19[-cf17] in v42) then v42[f19[-cf17]] else p1, globalState);
						var v43: array<bool> := new bool[22];
						v43[325] := f19 == fm48(globalState);
						v43[325] := fm3(|(fm49(0x2c4, f30, cf15, globalState) * {0x19e, cf17, cf17})|, globalState);
				}
				...
				match (v45.(cf22 := v44)) {
					case DC11(cf23, cf24) =>
						...
						var v48: set<int> := {|[f29]|};
						var v50: map<array<int>, set<int>> := map[v47 := set v49 : int | v49 in v48 :: (v49 % |[false]|)];
						v50 := v50 + v50;
					...
					case DC12(cf25, cf26, cf27, cf28, cf29) =>
						...
						var v58: map<bool, multiset<int>> := map[map[cf17 := fm18(globalState)] != v56 := v57[cf17]];
					...
					case DC13(cf30, cf31, cf32, cf33, cf34) =>
						...
						v65 := (if (cf16) then v65 else v65)[0xf3 := p1];
						cf16 := p2;
						var v66: seq<int> := [547, cf32, cf30];
						var v67 := new C3(v66, cf31, seq(728, i3  => (map[cf30 := |multiset{f29}|])), p2);
					case DC10(cf22) =>
						var v68 := 'v';
						...
						var v73: array<int> := new int[5](i4 => i4 % |map[p2 := v72.f31]|);
						...
						var v81: seq<string> := [v80[v69.f31]];
						var v82: array<bool> := new bool[24] [p2, p1, p2, v74 <= fm44(|v75|, p1, v68, v72.f32, globalState), cf16, multiset(v74) > DC37(v75).cf65, p1, p1, !(f18 ==> f18), p2, true, f18, v69.f31 > |map[v77.cf27 := v78]|, v79 <= v79, !v74[cf17], !p2, cf16, -v72.f31 < -v72.f31, f19 < f19, f18, f19 !in v81, true, p1, p1];
						v82[995] := p1;
						v68, globalState.f0, globalState.f15, v73, v82[995] := 'v', fm1(cf15, -v72.f32 == v72.f32, cf16, globalState), if (p2) then v72.f31 else cf17, v73, "tv" != f19;
					case DC14(cf35) =>
						globalState.f0 := cf17;
						cf16 := f18 ==> false;
						globalState.f2 := f18;
						...
						globalState.f2 := (cf15 / cf17) <= |v84[|f19|]|;
				}
				
				globalState.f9 := "oqwb";
				var v85: seq<int> := [cf15, cf15, 0x94, 0x1f9, cf15];
				var v86 := new C3(v85, f19, seq(935, i5  => (map[cf17 := f30])), cf16);
			case DC8(cf18, cf19, cf20) =>
				...
				var v96: array<bool> := new bool[20] [cf20, cf20, cf18, cf19, cf19, p2, f18, p1, !f18, if (-fm1(f30, cf20, cf18, globalState) in v94) then v94[-fm1(f30, cf20, cf18, globalState)] else cf20, p2, cf18, v91[|f19|], cf18, if (p2 in v95) then v95[p2] else cf19, p2, cf20, cf19, f18, !cf20];
			...
			case DC6(cf14) =>
				...
				globalState.f2, globalState.f3 := f18, f30 * (f30 % f30);
				...
				var v109 := 'l';
				v109 := f19[|fm48(globalState)| + f30];
			case DC9(cf21) =>
				var v110: multiset<int> := multiset{f30};
				var v111: seq<int> := [f30, |f19|, f30, |v110|];
				var v112 := new C0(f30, v111[f30]);
				var v113 := new C1(p1);
				var v114: array<bool> := new bool[19];
				var v115 := DC1(p1, v112.f32, seq(78, i8  => (f29)), true, v112.f32);
				v114[673] := v115.cf4;
				v114[673] := p2 || p1;
				var v116: seq<bool> := [v114[673]];
				v114[673], v116 := p1, [p1] + v116;
		}
			...
			decreases 100 - i9
		{
			...
			var v119: map<map<char, string>, C1> := map[map v118 : char | v118 in f19[f30 := 'r'] :: (v118) := (f19) := v117];
		...
		var v128 := DC24(v125);
		var v129 := new C3(v126, f19, fm51(p2, f30, v127[-f30], v128, globalState), p1 ==> f18);
		var v130: array<map<int, string>> := new map<int, string>[22];
		var v131: map<int, string> := map[f30 := f19];
		v130[724] := v131 + v131;
		v130[724] := map[f30 / f30 := f19];
		var v132 := new C1(p1);
		globalState.f2 := p2;
		r0 := f30 + |v127|;
	}
	method m8(p0: int, globalState: GlobalState) returns (r0: string) {
		...
		var v3: map<int, int> := map[p0 := |(v1 + v2[p0])|];
		v3 := map v4 : int | (584 <= v4) && (v4 < 0x116) :: (v4 / -p0) := (|[0x37f, p0]|);
		...
		var v13: map<bool, char> := map[v12[p0] := f29];
...
class C5 extends T0 {
	...
	method m1(p0: string, p1: int, p2: string, globalState: GlobalState) {
		...
		var v4: seq<bool> := [f18, f18];
		globalState.f0, globalState.f3, globalState.f2, globalState.f2, globalState.f2 := |v3| + p1, p1 / |v4|, f18, f18, f18;
		if (p1 <= 0x246) {
			globalState.f15 := p1;
			...
			if (v4[p1]) {
				var v7 := 'w';
				var v8: set<int> := {p1};
				var v9: array<string> := new string[24] [("m")[224 := v7], fm16(p1, f18, f27[p1], p1, globalState), p2, p2, f27, p0, f27, p0, f28, f28, "t" + f28, "gdjcvu", "y", ("kxv")[p1 := v7] + p0, p0 + "tclvbcm", "c", f27, "dsde" + f28, p2 + f28, f28, if (false) then f28 else p2, seq(0x39, i0  => (v7)), f28, f27[|v8| := v7]];
				v9[94] := p2;
				var v11: map<int, bool> := map[p1 := true];
				var v13: seq<int> := [p1, -p1, p1, p1, p1];
				var v14: map<int, map<int, bool>> := map[p1 := map[p1 := true]];
				var v15: multiset<map<int, bool>> := multiset{(map v10 : int | (120 <= v10) && (v10 < 247) :: (v10 + 0x17) := (v6[52])) + v11, if (v6[52]) then map v12 : int | v12 in v13 :: (v12 / 0xa3) := (v6[52]) else v11, (if (p1 in v14) then v14[p1] else v11) + v11};
				var v16: seq<string> := [p0];
				v9[94], globalState.f12, v15 := p0, (seq(0x1db, i1  => (p2[|map[v7 := f18]|]))) + v16[|f27|], v15;
				globalState.f0 := DC7(|v8|, f18, p1).cf15;
				globalState.f15 := (p1 * p1) + p1;
				var v17: array<int> := new int[9] [p1, p1, if (true) then p1 else p1, p1, p1, p1 * |map[v6[52] := v7]|, p1, fm1(p1, v6[52], v6[52], globalState), |(p2 + f27)|];
				v17[608] := p1 - p1;
				v17[608] := (p1 / p1) % -138;
				globalState.f0, v7 := p1, if (!f18) then 's' else v7;
			} else {
				v6[52] := v6[52];
				var v18 := new C1(v6[52]);
				var v19: map<seq<bool>, int> := map[v4 := p1 * p1];
				var v20: array<char> := new char[17](i2 => 'n');
				var v22: multiset<bool> := multiset{true};
				var v23: set<int> := {DC12(v20, f18, -p1, map v21 : multiset<bool> | v21 in multiset{v22} :: (v21) := (v6[52]), f18).cf27, -0xc6, 0x23};
				globalState.f0 := if (v4 in v19) then v19[v4] else |f28| / |v23|;
			...
			var v26 := DC23(v4[p1], f18, p1);
			match (DC1(v4[p1], p1, p0, v26.cf44, |fm43(f18, false, globalState)|)) {
				case DC1(cf1, cf2, cf3, cf4, cf5) =>
					...
					var v32: seq<int> := [|(seq(0x1f3, i3  => ('e')))|, cf2];
					var v33: set<int> := {v32[0x2de]};
					var v34: array<bool> := new bool[21](i4 => !DC28(f18).cf51);
					var v35: map<array<bool>, bool> := map[v34 := false];
					var v36: array<int> := new int[26] [cf2, p1, cf5, cf2 % |v33|, cf2 - p1, cf2 % |map[cf1 := false]|, fm1(|p0|, !cf1, f18, globalState) + p1, cf2, cf2, p1, cf2, v32[fm1(p1, true, f18, globalState)] + p1, |v32|, |"kseym"| - cf5, cf2 / cf5, cf2, |v29|, if (0x45 in v3) then v3[0x45] else p1, |(v32 + v32)|, cf5, cf2, 0x2a8, p1 + p1, 0x108, |v35|, cf2];
					v36[607] := cf5;
					v36[607] := cf2;
				case DC0(cf0) =>
					...
					var v41 := new C2(998, v40.f35, if (f18) then f18 else v4[p1]);
			...
			var v56 := 'w';
			var v57: array<map<int, int>> := new map<int, int>[25] [v3, v3, v3, v3, map v49 : int | (-0x3aa <= v49) && (v49 < 0x97) :: (v49 / p1) := (p1), v3[|v50| := p1], v3[p1 := fm1(p1, !f18, f18, globalState)], map[-p1 := p1], v3, v3, map v51 : int | (0x1f8 <= v51) && (v51 < -0x255) :: (v51 / p1) := (p1), v3[p1 := |"pxqm"|], map[p1 := if (false in v52) then v52[false] else 0x2bb], v3, v3, v3, if (|fm16(|v55|, f18, v56, 0x35a, globalState)| in v54) then v54[|fm16(|v55|, f18, v56, 0x35a, globalState)|] else v3, v3, map[p1 := -p1], v3 + v3, v3, v3, v3, v3, v3];
			v57[393] := v3;
			v57[393] := v3;
			if (f18) {
				...
				v58 := v58[p1 + |v60[|v61|]| := false];
				var v62: seq<map<bool, int>> := [fm43(f18, !f18, globalState), v1, v1, v1];
				var v63: C0 := new C0(p1, p1);
				var v64: map<C0, bool> := map[v63 := f18];
				var v65: array<map<bool, int>> := new map<bool, int>[11] [v1, v1, v1, v1, v1, v1[false := p1], v62[|(if (p1 in v54) then v54[p1] else map[p1 := p1])|], fm29(true, p1, "rsbldutix", globalState), v1 + v1, map[f18 := |v64|], v1[!f18 := 988]];
				v65 := v65;
			} else {
				globalState.f15 := p1;
				globalState.f2 := f18;
				var v66: multiset<seq<int>> := multiset{v55};
				var v67: array<int> := new int[19] [0x1e8 + -0x303, |"gmvtc"|, --0xba, -800, p1, p1, p1, p1, p1 / (if (v55 in v66) then v66[v55] else p1), -0x28, -0x321, -|v55|, 577, -((if (f18 in v1) then v1[f18] else p1) % -|f27|), --p1, -p1, p1, p1, 426];
				var v68: array<char> := new char[9] [v56, p0[p1], v56, v56, v56, v56, v56, v56, fm46(globalState)];
				var v69: seq<multiset<bool>> := [v52];
				var v70: map<multiset<bool>, bool> := map[v69[100] := true];
				var v71 := DC12(v68, !!f18, p1, v70, f18);
				var v72: map<int, bool> := map[p1 := f18];
				var v73: map<D2, char> := map[DC6(v1) := v56];
				var v74: T1 := new C4(v56, p1, f18, p2, seq(574, i5  => (v57[393])));
				var v75: map<int, T1> := map[if (true in v1) then v1[true] else |([f18, f18])[-p1 := f18]| := v74];
				var v76: seq<map<int, T1>> := [v75];
				var v77 := DC33(v74.f18, p1, f18, -|v55|);
				v67 := new int[28] [p1, p1 % 836, p1, p1, if (f18) then p1 else p1, |v1| - p1, 0x1e6 / p1, if (f18) then p1 else p1, v71.cf27, p1, |v72|, |v73|, -0x3c3 + p1, p1, |v50|, p1 - p1, 0x3ae, -(0x17b + |v76|), if (p1 in v50) then v50[p1] else |v55|, -p1, -p1 - -p1, v77.cf56, p1 % p1, p1, p1, p1, p1 / p1, p1 + |v55|];
			...
			v84[173] := |p0|;
			globalState.f12, v84[173] := seq(0x22c, i6  => (v56)), p1;
		}
	...
	method m7(p0: bool, p1: int, globalState: GlobalState) {
		...
		if (f18 || p0) {
			...
			v4[588] := v5;
			v4[588] := v5;
			globalState.f3, v3[291], globalState.f2, v3[291], v3 := p1 % p1, f18, p0 !in (v2 + v2), f27 < "gnynibgy", v3;
			if (false) {
				var v6: map<array<bool>, bool> := map[v3 := v3[291] in v2];
				v6 := v6[v3 := p0];
				var v7: set<bool> := {f18, f18, f18};
				var v8: map<bool, set<bool>> := map[v3[291] := v7];
				v8 := v8[v3[291] := {v3[291], !!f18} - v7];
				v3[291] := v3[291];
				v3[291] := v3[291];
				globalState.f0 := if (f18) then p1 else |(v0 * v0)|;
			} else {
				var v9: seq<int> := [p1 - p1];
				globalState.f0 := v9[p1];
				globalState.f15 := p1;
				var v10: array<seq<int>> := new seq<int>[16](i1 => v9);
				v10[389] := v9;
				v10[389] := v9 + ([p1] + v9);
				var v11: seq<set<int>> := [v0];
				globalState.f2 := fm23(v11[p1], -p1, globalState);
				var v12 := 'w';
				var v13: multiset<string> := multiset{f28, f27[p1 := v12], f27[98 := v12]};
				...
				v14[919] := v13;
				var v15 := DC31();
				globalState.f2, v14[919], globalState.f2, v15 := !f18, multiset{f28, f28, f28} - v13, v12 in f27, v15;
			}
			...
			v16[131] := v2;
			var v17: seq<seq<bool>> := [[f18], v2 + [p0], v2[p1 := p0], v2 + v2];
			var v18: seq<int> := [p1];
			globalState.f2, globalState.f2, v16[131], v0 := p0, f18 && f18, v17[fm1(|v18|, p0, p0, globalState)], (v0 - v0) - v0;
			...
			var v23: array<seq<D0>> := new seq<D0>[7] [v21[p1 := v20], v21, fm53(globalState), v21, v22[p1], v21 + v21, v21[p1 := v20]];
			v23[789] := fm53(globalState);
			var v24 := 'j';
			...
			var v31: C4 := new C4(v24, |multiset(v18)|, f18, (v25[if (f18 in v26) then v26[f18] else p1])[|map[|v29| := p0]| := v24], v30);
			var v32: map<C4, bool> := map[v31 := p0];
			v19[294], v23[789] := |v32| > |fm16(v31.f30, f18, v31.f29, v31.f30, globalState)|, (seq(975, i2  => (DC1(f18, 484, f28, p0, p1).(cf2 := -v31.f30, cf4 := p0)))) + v21;
			var v33 := new C0(|v16[131]|, v31.f30);
			var v34: map<array<bool>, int> := map[v19 := p1];
			globalState.f15 := v33.f32 % |v34|;
			globalState.f3 := v33.f32 % p1;
		}
		
		var v35: array<int> := new int[25];
		var v36: multiset<array<int>> := multiset{v35};
		var v37: seq<array<int>> := [v35];
		for i3 := if (v37[-p1] in v36) then v36[v37[-p1]] else p1 to p1 % p1 {
			var v38: array<bool> := new bool[23];
			v38[177] := p0;
			globalState.f12, v38[177] := f28 + (f28 + f28), (v1 == v1) ==> (f27 < f28);
			...
			decreases 100 - i5
		{
			...
			globalState.f3, v51[248], v52, v2, v54 := |fm36(!(f18 ==> f18), p1, -p1 < 0x1bf, globalState)|, v1 - multiset{if (p0 in v55) then v55[p0] else f18}, v52, v2, v54[f18 := v53];
			var v56: seq<int> := [p1];
		...
		for i6 := p1 to p1 {
			...
			match (v60) {
				...
				case DC32(cf54) =>
					...
					var v80: array<bool> := new bool[27] [f18, p0, p0, f28 == f27, (set v74 : int | v74 in v73 :: (v74 % 0x2e6)) !! v0, f18, f18, v75 == v75, p0, fm23(set v76 : int | v76 in v0 :: (v76 % |"gm"|), v73[i6], globalState), p0, f18, f18, false, fm23(set v77 : int | (0x364 <= v77) && (v77 < 822) :: (v77 % i6), p1, globalState), fm15(0xb3, i6, globalState), p0, fm23(v0, |f27|, globalState), p0, v72.fm22("bm", v78, globalState) < |v73|, p0, f18, f18, if (f18) then p0 else !f18, fm23(v0, -p1, globalState), fm25(p1, globalState) !! v79, true];
					v80[576] := p0;
					var v81 := DC23(p0, f18, |(seq(-0x66, i7  => (p1)))|);
					var v82: map<bool, int> := map[false := |f27|];
					globalState.f2, globalState.f2, v1, globalState.f0, v80[576] := f18, (if (p0) then v81 else v81).cf44, v1, p1 * p1, if (f18) then (if (p0 in v82) then v82[p0] else p1) >= i6 else f18;
					globalState.f2 := p0;
					globalState.f3 := if (fm15(i6, i6, globalState)) then i6 else p1;
				case DC35(cf63) =>
					var v83: array<D0> := new D0[17];
					var v84 := DC0(p0);
					v83[494] := if (false) then v84 else v84;
					var v85 := 'j';
					var v86: seq<int> := [p1];
					globalState.f2, globalState.f2, v83[494], globalState.f2 := p0 ==> f18, |fm44(p1, p0, v85, i6, globalState)| <= |(v86 + v86)|, v84.(cf0 := true), p0;
					...
					var v90: seq<string> := [seq(926, i8  => ('x'))];
					var v91 := new C4(v85, p1, false, f27 + v90[i6], seq(0x318, i9  => (v59)));
			}
			
			globalState.f3 := p1;
			var v92: seq<int> := [p1, 0x23b, if (p1 in v59) then v59[p1] else i6, |fm54(i6, p1, globalState)|];
			globalState.f2 := !((seq(-129, i10  => (|f27|))) < v92);
		}
		match (v43) {
			case DC37(cf65) =>
				var v93 := 'l';
				var v94: multiset<string> := multiset{f28[|f27| := v93], f27, "mnrphd", f28, f28};
				globalState.f0 := p1 % (if (f28 in v94) then v94[f28] else p1);
				v35[49] := p1 + p1;
				v35[49] := p1;
				var v95: array<seq<int>> := new seq<int>[14];
				var v96: seq<int> := [v35[49], |multiset{p0, f18}|, v35[49]];
				v95[95] := v96;
				globalState.f2, v95[95], globalState.f2 := !p0, v96 + v96, p0;
				var v97: array<D13> := new D13[8];
				var v98: array<bool> := new bool[25];
				v98[836] := p0;
				v97, v98[836], globalState.f2, globalState.f15 := v97, p0, p0, p1;
		}
...
class C6 extends T0, T1 {
	...
	method m1(p0: string, p1: int, p2: string, globalState: GlobalState) {
			...
			decreases 100 - i0
		{
			...
			match (v2) {
				case DC1(cf1, cf2, cf3, cf4, cf5) =>
					...
					var v4: seq<int> := [cf5];
					var v5 := 'f';
					var v6: array<int> := new int[14] [f26 * fm1(p1, cf1, cf1, globalState), |v3| / fm1(-f26, f18, f18, globalState), -p1 % -|multiset{|v1|}|, cf2, |((seq(0x34e, i1  => ('k'))) + fm13(cf4, p1, globalState))[v4[f26] := v5]|, p1, if (cf4 in v1) then v1[cf4] else p1, cf2, if (cf1) then cf2 else -cf5, cf5, f26, |f25|, cf5, f26];
					v6[171] := -(cf5 + f26);
					v6[171] := f26;
					v3 := v3;
					...
					var v12: array<seq<array<bool>>> := new seq<array<bool>>[20] [v10 + v10, v10, v10 + v10, v10 + v10[v6[171] := v8], v10, v10, v10, v10, v10, v10, v10, v10 + v10, (if (cf1 in v11) then v11[cf1] else v10) + v10[f26 := v8], v10, v10 + v10, v10, v10, v10[627 := v8], v10, [v8]];
					v12[160] := v10;
					v12[160] := v10;
					var v13: array<seq<bool>> := new seq<bool>[7] [f25, fm44(v6[171], cf1, v5, |[f18, cf4]|, globalState), f25, f25, f25, f25, f25];
			...
			var v25: map<set<string>, D11> := map[if (true) then v22 else v22 := DC29(v24[p1])];
		...
		match (v33) {
			case DC28(cf51) =>
				...
				globalState.f0, f26, v36, globalState.f12 := f26 / (v37[|v34|] - p1), p1, v36, (seq(0x1a8, i2  => ('v'))) + (seq(0x347, i3  => ('r')));
				if (!false <== f18) {
					...
					v41[957] := f18;
					var v42: T1 := new C3([|v39.f34|], "cutdr", f20 + (seq(0x27f, i4  => (v38[p1 := f26]))), f25[0x147]);
					var v43: array<set<int>> := new set<int>[8];
					v43[898] := fm49(|v42.f19|, v39.f33, p1, globalState);
					cf51, v41[957], v42, globalState.f3, v43[898] := v42.f18, v42.f18, v42, f26, v0;
					globalState.f0 := (f26 + v39.f33) - fm1(|v43[898]|, !!v41[957], v42.f18, globalState);
					var v44: array<string> := new string[25];
					var v45 := 'p';
					v44[116] := f19[v39.f33 := v45];
					v44[116] := if (v42.f18) then f19 else p0;
					...
					globalState.f2, globalState.f2, cf51, v46[653] := f18 || false, if (map[f18 := v47] in v49) then v49[map[f18 := v47]] else cf51, cf51, v35 + v35;
					globalState.f15 := |v35|;
					globalState.f2 := f18;
					v34 := v34 * v34;
					globalState.f2, globalState.f2, globalState.f3 := f18, !f18, --(---0x300 / 0x1c5);
				}
				...
				if (cf51) {
					...
					var v53: array<array<bool>> := new array<bool>[26] [v50, v50, v50, v50, v50, v50, v51[|f25|], v50, v51[|v52|], v50, v50, v50, v50, v50, v50, v50, v50, v50, v50, v50, v50, v50, v50, v50, v50, v50];
					v53[29] := v50;
					v53[29] := v50;
					var v54 := 'f';
			...
			case DC27(cf50) =>
				...
				if (p1 <= |p0|) {
					...
					var v67 := DC20(v0);
					globalState.f2 := if ((f26 % p1) in v66) then v66[f26 % p1] else fm23(v67.cf40, f26, globalState);
					v61[451] := v61[451] + f25;
					var v68: array<char> := new char[29];
					v68[792] := v63;
					v68[792] := v63;
					var v69: map<bool, bool> := map[v61[451][f26] := f18];
					var v70: multiset<map<bool, bool>> := multiset{v69, v69};
					var v71: multiset<bool> := multiset{f18};
					var v72: array<bool> := new bool[19] [f19 < p0, f18, f18 <== v61[451][403], f25[p1], f18, fm23(v0, p1, globalState), f26 != f26, if (f18 in v69) then v69[f18] else f18, f18, f18, f18, f18, p1 >= |v70|, f18, true, v71 > v71, f18, f18, f18];
					v72[488] := if (f18) then f18 else fm2(f18, f18, !f18, globalState);
					v72[488] := !f18;
					var v73: map<int, map<int, bool>> := map[0x37a := v66];
					globalState.f2 := (v66 + v66) != (if (f26 in v73) then v73[f26] else v66)[f26 := v72[488]];
				}
			...
			case DC29(cf52) =>
				...
				if (!(|p2| == --f26)) {
					...
					var v82: map<int, int> := map[v77[|v80|] := |v81|];
					var v83: set<seq<int>> := {v77};
					var v84: map<int, string> := map[|v83| := v79.f19];
					globalState.f2, globalState.f2, globalState.f10, globalState.f0 := false, !(f26 <= f26), ((seq(700, i10  => (p1)))[|v75| := v77[f26]])[if (f26 in v82) then v82[f26] else p1 := |v84|], f26 - -0x33a;
		...
		for i12 := 2 to |"tsjsavsim"| {
			...
			var v99: array<int> := new int[21] [-(|f25| % f26), f26, i12 - p1, i12, fm1(f26, f18, f18, globalState), i12, p1, i12, f26, p1, f26, f26, f26, p1, 487, f26, v30.fm22(f19, v96, globalState), i12, i12, |DC1(f18, |v97|, fm48(globalState), f18, i12).cf3| * |(if (f18 in v98) then v98[f18] else p0[p1 := 'a'])|, p1];
			v99 := v99;
			var v100 := DC0(f18);
		...
		var v106: array<string> := new string[26] [p2 + p0, p2, if (f18) then "mamkhfj" else p2, (seq(387, i13  => (v95)))[p1 := v95], p0, f19, p2, p2, v30.fm21(|p0|, f19, v105, f25, globalState), if (f18) then f19[|(seq(120, i14  => (-f26)))| := v95] else "b", f19, f19, if (f18) then p0 else fm48(globalState), "yntu", (seq(-0x3e0, i15  => (v95))) + p2, f19, f19, p0[|{0xb3}| := fm46(globalState)] + "ohly", "mssdb", p0 + p2, "leqkgaus", (seq(-0x2a5, i16  => (v95))) + f19, "nkvl", p2, p2 + p2, "ocvvqga"];
		v106[165] := p0;
		v106[165] := if (f18 ==> f18) then f19 else p2;
	}
	method m2(p0: map<array<int>, map<bool, int>>, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int) {
		if (false) {
			if (f18) {
				...
				var v2: map<bool, seq<D3>> := map[p2 := (seq(238, i0  => (DC13(-f26, "hpkoolb", f26, 0x329, 'f')))) + [v1, v1, v1, v1]];
				var v3: map<int, bool> := map[f26 := f18];
				var v4: map<map<int, bool>, int> := map[v3 := |f19|];
				var v5: seq<D3> := [DC13(f26, seq(0x3d8, i1  => (v0)), f26, if (v3 in v4) then v4[v3] else 0x1f9, v0), v1];
				v2 := v2[true := v5 + v5];
				var v6: multiset<int> := multiset{f26, f26};
				var v7: set<bool> := {f18, p1};
				var v8: seq<int> := [-0x35, |v7|, fm1(f26, p2, p2, globalState), f26];
				var v9: set<int> := {f26};
				globalState.f0, globalState.f2, globalState.f2 := f26, p1, (v6 * v6) >= multiset(v8[f26 := |v9|] + v8);
				var v10: map<int, multiset<string>> := map[f26 + -f26 := fm52(f26, v8[f26], globalState)];
				var v11: multiset<string> := multiset{f19, f19, f19, f19, seq(0x3db, i2  => (f19[f26]))};
				v10 := v10[f26 := v11 + v11];
				...
				v14[423], globalState.f0, v17 := f26, -(v13 % v13), {v16};
			} else {
				...
				globalState.f2, v23[467], globalState.f12, globalState.f2, globalState.f3 := f18, (-|v24| % f26) - f26, "fphpatnfw", f26 < |fm36(!f18, f26, p1, globalState)|, f26;
				var v25 := 'f';
				var v26: T1 := new C4(v25, --f26, p1, f19, f20);
				var v27: seq<T1> := [v26];
				var v28: T1 := new C3([f26, |v27|], v26.f19, f20, p1);
				var v29: map<bool, T1> := map[fm1(f26, p1, p1, globalState) < |(seq(0x349, i3  => (f26)))| := v26];
				var v30: map<bool, bool> := map[v28.f18 := v28.f18];
				v29 := v29[v26.fm2(DC0(v26.f18).cf0, v28.f18, f25[|map[|v30| := v25]|], globalState) := v26];
				...
				var v33 := DC39(!(v32 !! multiset([f26, -v23[467]])), 0x3b7);
				var v34 := DC8(p2, f18, true);
				var v35: multiset<bool> := multiset{p1, v34.cf19, p1, v26.f18};
				var v36: map<T1, char> := map[v26 := v25];
				globalState.f15, v31, v33, v35, globalState.f15 := v23[467], v31, v33.(cf67 := v23[467] > f26), fm32(globalState) - (multiset{v26.fm2(false, v26.f18, v28.f18, globalState)} + v35[p2 := |v36|]), (fm55(f26, p2, |v30|, v28.f18, globalState)).cf46;
				globalState.f9, globalState.f3 := v28.f19, -v23[467] * -479;
			}
			
			var v37: set<int> := {f26};
			var v38: map<seq<bool>, bool> := map[f25 := p1];
			var v39: seq<int> := [|v37|, f26 % f26, f26, fm1(|f25|, if (f25 in v38) then v38[f25] else p1, f18, globalState)];
			globalState.f0 := v39[f26 * f26];
			var v40 := new C1(p1);
			globalState.f9 := (seq(838, i4  => ('u'))) + f19;
			...
			if (p1) {
				...
				var v53: set<int> := {|v52|, |(seq(-716, i6  => (v44.f29)))|, v44.f30};
				v51[58] := v53;
				v51[58] := v53;
			...
			v62[724] := 'h';
			var v63 := DC33(v44.fm3(f26, globalState), v44.f30 % 0x314, p1, f26 + v44.f30);
			v61, v43, v62[724], v63 := v61, v44.f29, if (f18) then v43 else v44.f29, fm56(p2, f18, f26, seq(798, i8  => (v44.f29)), globalState);
		...
		if (p2 || (!f18 || !f18)) {
			...
			if (--v69.f33 == |f19|) {
				...
				var v80: array<int> := new int[29] [f26, 432, v69.f33, if (v67[501] in v70) then v70[v67[501]] else f26, v69.f33, |[v71, v71]|, v69.f33, fm1(v69.f34[f26], p2, v67[501], globalState) + |multiset{v68, fm33(true, globalState)}|, 0x374, f26 % v69.f33, v69.f33, v72.cf46 / v69.f33, v69.f33, DC44(v67, v69.f33).cf78, |v73| % v69.f33, f26, |map[true := v77.cf76]|, f26, f26, v69.f33 - |(seq(0x143, i10  => (v71)))|, f26, if (f18 in v70) then v70[f18] else fm1(-f26, v67[501], p2, globalState), v69.f33, v69.f34[v69.f33], if (v67[501]) then if (f18 in v78) then v78[f18] else 0x38 else fm1(0x387, f18, v69.fm19(v79, globalState), globalState), v69.f33, v69.f33, 0x34d - v69.f33, f26];
				v80[145] := v69.f33;
				v80[145] := v69.f33;
				...
				var v83 := DC21(v71, |[p2]|);
				v71 := v83.cf41;
				v75 := v75[|f25| - v80[145] := v69.f33];
			} else {
				globalState.f0 := f26;
				globalState.f2 := f18;
				v67[501] := v67[501];
				v67[501] := f25[|(if (p2) then v68[--208 := |v68|] else v68)|];
				var v84 := 'd';
			...
			match (fm24(v86 >= v86, v88, f26 == v69.f33, globalState)) {
				case DC3(cf7, cf8, cf9, cf10, cf11) =>
					...
					var v93: array<map<D6, bool>> := new map<D6, bool>[22] [v91, v92[cf9], v91, v91, v91 + v91, v91 + v91, (map[v90 := p2])[v90 := p1], v91 + v91, map[v90 := v67[501]], v91[v90 := v67[501]], v92[cf8], v91[v90 := f18], v91 + v91, v91, v91, if (v67[501]) then v91 else v91, if (v67[501]) then v91 else map[v90 := f18], v91, v91, v91 + v91, v91 + map[DC17(v89) := v67[501]], v91[DC17(v89) := v67[501]]];
				...
				case DC4(cf12) =>
					...
					var v97: array<seq<int>> := new seq<int>[18](i12 => ([-0x392])[v69.f33 := v69.f33] + v69.f34);
					var v98: map<int, bool> := map[f26 := p1];
					var v99: map<int, bool> := map[f26 := if (564 in v98) then v98[564] else cf12];
					v97[594] := fm33(if (|f19| in v98) then v98[|f19|] else p2, globalState);
					v97[594] := if (p1) then v69.f34 + v69.f34 else v68 + v68;
					v99 := v99[f26 := v69.f33 != |f19|];
					var v100 := DC38(v96);
					v100 := v100;
				case DC2(cf6) =>
					globalState.f15 := |f19|;
					v67[501] := -v69.f33 == --fm1(v69.f33, p2, f18, globalState);
					v67[501] := !DC23(v67[501], p1, -0x323).cf44;
					var v101: array<int> := new int[7];
					v101[901] := f26;
					v101[901] := (if (!p2) then f26 else f26) % v69.f33;
				case DC5(cf13) =>
					var v102: map<bool, array<bool>> := map[v67[501] := if (true) then v67 else v67];
					v102 := map[f18 := v67] + v102;
					var v103: map<char, int> := map[v87 := v69.f33];
					v103 := v103['s' := v69.f33];
					v69.m1(f19 + f19, f26, f19, globalState);
					var v104: array<C2> := new C2[10];
					v67[501] := |map[f26 := v104]| >= 696;
			}
			...
			var v110 := DC45(v108);
			var v111: seq<seq<multiset<D8>>> := [v108, v108[|v109| := v107], seq(50, i13  => (multiset{DC21(v105, f26), v106})), v110.cf79];
			v108 := v111[f26 - f26];
			...
			if (f25[f26]) {
				...
				v115[745] := if ([v114, 0x390, f26, v114] <= v116) then p2 else false;
				var v117: multiset<bool> := multiset{p1};
				var v118: seq<multiset<bool>> := [v117, v117];
				var v119: map<int, bool> := map[f26 := v118[v114] > v117];
				v115[745], v114 := v115[745], |v119|;
				v115[745] := !f18 <==> fm23(v109, v114, globalState);
			} else {
				...
				v122[664] := p1;
				var v123 := DC42(f18);
				v122[664], globalState.f2, globalState.f12, v123, v122[664] := v122[664], v122[664], (seq(-0x21e, i14  => (v112[521]))) + (f19 + f19), v123, v122[664];
				var v124: map<int, string> := map[f26 := f19];
				globalState.f0 := -f26 % |v124|;
			}
		...
		if (f18) {
			...
			globalState.f2, v132[915], v131[611] := f26 < f26, f26, fm2(p2, -0x94 < |v133|, -fm1(|[f26]|, v131[611], v131[611], globalState) < f26, globalState);
			var v134: set<bool> := {v131[611]};
			var v135: map<bool, bool> := map[false := v134 !! v134];
			v135 := v135[p2 := false && p2];
			var v136: array<array<int>> := new array<int>[20];
			var v137 := DC16(v132);
			v136[461] := if (false) then v137.cf37 else v132;
			v136[461] := v132;
			var v138 := 'd';
			var v139: set<char> := {v138};
			r0 := if (p1) then v132[915] / |v139| else f26;
		} else {
			globalState.f0 := f26;
			...
			v151[61] := !fm23(v152, f26, globalState);
			v151[61] := !p1;
			var v153: seq<seq<bool>> := [f25];
			var v154: set<seq<bool>> := {v153[f26]};
			var v155: map<bool, set<seq<bool>>> := map[f26 != f26 := v154];
			v155 := v155[!v151[61] := v154];
			var v156: array<int> := new int[23];
			var v157: map<string, int> := map["rfnihgwpf" := v148.cf58];
			v156[85] := |v157|;
			v156[85] := |fm58(f26 >= f26, p1, globalState)|;
			globalState.f2 := if (v143 >= v143) then true <==> f18 else v151[61];
		}
		...
		if (fm3(f26, globalState)) {
			...
			var v163 := new C3(v161, (v162[|v159|])[0x4 := v158], f20, 0x122 <= f26);
			...
			v168[74] := p2;
			v168[74] := fm2(true, p2, p1, globalState);
			r0 := 0x1e7;
			var v169 := 'y';
			var v170: seq<string> := [f19, f19];
			var v171: array<string> := new string[26] ["xlqtclqmw", f19, f19, (seq(610, i17  => ('v')))[f26 := v169], f19[f26 := v169] + f19, "rskscm", fm48(globalState), f19, seq(618, i18  => (v169)), (seq(0x2d8, i19  => (v169))) + f19, v170[237], f19 + "jfpoexew", "qkvw", f19, fm16(f26, false, v169, f26, globalState), f19, f19, f19, f19, f19 + (seq(0x357, i20  => ('c'))), seq(0x28, i21  => (v169)), f19, f19[|f19| := v169] + f19, f19, f19, f19];
			v171[195] := f19;
			globalState.f2, globalState.f9, globalState.f2, v171[195], v169 := false, (f19 + f19) + "mrmigu", -f26 > f26, (seq(0xa2, i22  => (v169))) + f19, v169;
			v169 := v169;
	...
	method m6(globalState: GlobalState) returns (r0: bool, r1: int) {
		...
		var v4: array<seq<int>> := new seq<int>[12] [v2, v2 + v2, [f26, f26, f26, |f19|] + v2, (if (v2 in v3) then v3[v2] else v2) + v2, (seq(0x3c4, i0  => (f26))) + v2, v2, v2, [|f19|, f26, f26], v2, if (v0.f18) then seq(0x2b8, i1  => (f26)) else v2[0x52 := f26], v2, v2];
		v4 := v4;
		for i2 := |(if (v0.f18) then f19 else f19)| to f26 {
			r0 := if (f18) then f18 else v0.f18 <==> v0.f18;
			globalState.f0 := f26;
			var v5 := 'a';
			globalState.f12 := f19 + f19[0x21a := v5];
			r0 := fm2(v0.f18, f18, f26 >= i2, globalState);
		}
		if (f18) {
			var v6: array<bool> := new bool[22](i3 => f25 < f25);
			var v7: set<int> := {f26};
			v6[110] := fm23(v7, |f19|, globalState);
			v6[110] := f18;
			v6[110] := !!(f19 < f19);
			globalState.f9 := f19 + (seq(0x334, i4  => ('a')));
			...
			if (v0.f18) {
				...
				v14[209] := |(if (f26 in v15) then v15[f26] else map[f26 := 888])|;
				v14[209] := f26 + f26;
				v14[209] := v14[209];
			} else {
				var v16: seq<seq<bool>> := [f25[f26 := v0.f18]];
				var v17: map<seq<seq<bool>>, int> := map[v16 := f26];
				v17 := v17[v16 := f26];
				var v18: map<bool, bool> := map[v6[110] := f18];
				var v19: map<int, int> := map[|v18| := f26];
				var v20: seq<seq<int>> := [(v2[|v19| := f26])[f26 := f26], v2];
				var v21 := new C3(v20[f26], f19 + f19, f20, f18);
			...
			if (false) {
				...
				globalState.f2, v29[31], globalState.f2, globalState.f2, globalState.f3 := v0.f18, f26, v31 !! v31, !(|v32| == f26), (f26 + v2[|f19|]) % f26;
				var v33 := DC19(v30);
				var v34: map<D7, bool> := map[v33 := v0.f18];
				r0 := !(if (DC19(v30) in v34) then v34[DC19(v30)] else v0.f18);
				var v35: array<char> := new char[14] [v27, f19[v29[31]], v27, if (false) then v27 else v27, v27, v27, v27, v27, v27, v27, v27, v27, v27, 'v'];
				v35[479] := f19[v29[31]];
				v35[479] := fm46(globalState);
				v29, v27 := v29, v27;
				v29[31] := fm1(v29[31] * 623, false, v0.f18, globalState);
			} else {
				globalState.f3 := f26;
				var v36 := DC30(f25);
				var v37: seq<D12> := [v36];
				...
				var v43: multiset<int> := multiset{f26, |v41|, |(seq(-538, i6  => (-0x395)))|};
				var v44: array<int> := new int[27] [f26, |v39|, f26, 0x12e, |v40|, f26, fm1(|v41|, v0.f18, !v0.f18, globalState), f26, f26, fm1(f26, v0.f18, v0.f18, globalState), f26, -(|v41| % f26), f26, if (v0.f18) then f26 else f26, |v28| % f26, f26 * f26, f26 * f26, f26, f26, |v24|, |(if (v0.f18 in v42) then v42[v0.f18] else [|v43|])|, if (f18) then f26 else f26, -f26, |f25|, 822, |fm32(globalState)|, f26];
			...
			var v55: multiset<seq<char>> := multiset{v54[-f26], f19};
			v24 := fm44(f26, fm23(v53, f26, globalState), v27, v2[if ([v27] in v55) then v55[[v27]] else fm1(f26, v0.f18, v0.f18, globalState)], globalState);
		}
			...
			decreases 100 - i9
		{
			...
			v60 := v60 + v60;
			var v61 := 'i';
			var v62 := new C5(v56[138] + f19, v56[138] + v56[138], v61 !in f19);
			var v63: map<char, int> := map[v61 := |[f18, f18]|];
			v63 := v63[fm46(globalState) := f26];
			var v65 := new C4(v61, f26, f26 == v2[f26], "wgfvxeb" + v62.f28, f20[f26 := map v64 : int | v64 in [f26] :: (v64 - f26) := (|f25|)]);
		}
...
class C7 extends T0 {
	...
	method m1(p0: string, p1: int, p2: string, globalState: GlobalState) {
		...
		v10[80] := !(f18 <==> !false);
		v10[80] := !(if (f18) then fm10(f18, f18, f18, globalState) else p1 <= f24);
		v3 := v3;
		var v11: map<bool, bool> := map[f18 := f18];
		var v12: map<bool, bool> := map[f18 := if (f18 in v11) then v11[f18] else true];
		var v13: set<int> := {|v12|};
		var v14: map<bool, int> := map[v10[80] := p1];
		var v16: seq<int> := [f24];
		var i1 := 0;
		while ((v13 == v13) <==> (v14[v10[80] := |(map v15 : int | v15 in v16 :: (v15 - p1) := (f18))|] != v14))
			decreases 100 - i1
		{
			if (i1 >= 100) {
				break;
			...
			var v18: array<string> := new string[28] [p2, p2, p0, p2 + p0, p2, seq(470, i2  => (v3)), p0, p0 + p2, p0, p2, "gvm" + "qmjimlo", seq(0xda, i3  => (v3)), p0, p2, p2, p2 + v17[|p2|], p0, fm12(|(seq(0x355, i4  => (v3)))|, v10[80], globalState) + p0, p0, p0 + "r", "etpqxgf", p0, p2, "nuxgycs", p2, ((seq(-679, i5  => (v3))) + fm12(f24, true, globalState))[p1 := v3], p2, seq(0x3b, i6  => (v3))];
			var v19: seq<bool> := [f18, !v10[80]];
			v18[678] := (p2[p1 := 's'])[|v19| := v3];
			v18[678] := p0[p1 := v3];
			globalState.f3 := if (f18) then p1 else |(seq(0x24a, i7  => (map[f24 := 512])))|;
			var v20: multiset<bool> := multiset{p1 > p1};
			v20 := v20;
			var v21: map<int, int> := map[0x155 := fm1(p1, v10[80], v10[80], globalState)];
			var v22: seq<map<int, int>> := [v21, v21];
			var v23: T1 := new C6([v10[80], v10[80]], f24, !(v18[678] == p2), p2, v22 + v22);
	...
	method m5(p0: multiset<string>, p1: int, p2: multiset<int>, p3: int, globalState: GlobalState) {
		...
		v0 := v0;
		globalState.f2 := !f18;
		for i0 := f24 / 0x257 to p3 {
			var v1: seq<int> := [|fm43(f18, !f18, globalState)|, p1 % p3];
			globalState.f10 := v1;
			var v2 := "dqugt";
			var v3: map<string, bool> := map[v2 := f18];
			var v4 := new C0(if (p1 in p2) then p2[p1] else p1, v1[|v3|]);
		...
		if (v12[f24 - |v20|]) {
			...
			v24[903] := v22;
			var v25: C6 := new C6(v12 + [f18, false, f18], p1, f18, v11, v18[f24 := v17]);
			var v26 := DC33(f18, v19.f30, !f18, v14.f33);
			var v27 := DC11(f18, f18);
			globalState.f2, globalState.f2, globalState.f12, v24[903], v25 := f18 <==> v19.fm2(f18, f18, f18, globalState), !v26.cf57, v11[|v17| := v10], DC44(v9, if (!v27.cf24) then v21.f31 else f24), v25;
			...
			v32[283] := DC34(v9, v11, f18, f18).cf60;
			var v33: seq<string> := [v11];
			var v34: map<bool, string> := map[f18 := v33[v14.f33]];
			v32[283] := if ((v8[914] < |v14.f34|) in v34) then v34[v8[914] < |v14.f34|] else "lnxxapdgd";
			var v35: array<map<int, int>> := new map<int, int>[12](i2 => map[p3 := 86] + v17);
			v35[951] := v17;
			var v36: seq<multiset<bool>> := [v20];
			v35[951] := fm30(v36, map v37 : string | v37 in v33 :: (v37) := (f18), globalState);
			var v38: set<bool> := {f18};
			var v39 := DC24(v38);
			...
			match (v40) {
				case DC25(cf48) =>
					...
					var v42: map<bool, seq<bool>> := map[f18 := v41[v19.f30]];
					var v43: map<array<bool>, bool> := map[v9 := !!true];
					v12 := (if (cf48 in v42) then v42[cf48] else v12 + v12)[|v43| := cf48];
					var v44: map<int, bool> := map[v14.f33 := cf48];
					v44 := v44[v19.f30 := v12[|v13|] || f18];
					globalState.f2 := if (f18) then f18 else f18;
					globalState.f3 := v8[914];
				case DC24(cf47) =>
					var v45 := new C4(v19.f29, v19.f30, f18, v32[283], v18);
					globalState.f2, v8[914], globalState.f2, v14.f33 := f18, |v11|, false <==> f18, v19.f30 + |v32[283]|;
					globalState.f0 := 265 + v14.f33;
					v35[951] := v35[951][v45.f30 + p1 := p1];
				case DC26(cf49) =>
					v8[914] := fm1(v19.f30, f18, f18, globalState) / (v14.f33 - 0x2a);
					var v46: seq<seq<int>> := [v14.f34, v14.f34];
					globalState.f2 := (v14.f34 + v13) in v46;
					globalState.f0 := v14.f34[v14.f33] * v14.f33;
					globalState.f2 := f18;
			}
	...
	method m1(p0: string, p1: int, p2: string, globalState: GlobalState) {
		...
		for i0 := p1 to 486 + |v0| {
			...
			if (!!fm6(v4 + v4, false, v5 - v5, p1, globalState)) {
				...
				var v18: array<string> := new string[11] [p0, p0, p0 + p2, p0, "e", seq(747, i1  => (v1)), p0, p0, DC1(f18, |p2[f23 := v1]|, p2, f18, f23).cf3, p0, p2];
				v18[818] := p2 + p2;
				v18[818] := p2 + "msaxakyd";
			} else {
				...
				var v21: T0 := new C4(v1, 0x263, f18, seq(842, i2  => (v1)), [v20, v20]);
				var v22 := DC2(v21);
		...
		if (f18) {
			...
			if (f18) {
				...
				var v39: map<int, string> := map[f23 := v37[-|v38|]];
				...
				var v47: array<multiset<bool>> := new multiset<bool>[20] [v42, (multiset{f18, f18})[!f18 := f23], multiset(v44.cf53) * v42, v45.cf65, v42, v42[false := v34.f24], v42, v42 * v42, v42, if (multiset(v36) in v46) then v46[multiset(v36)] else v42, v42 + multiset{f18, false, !f18}, fm32(globalState), v42, v42, multiset{f18}, v42, v42, multiset(v43), v42[f18 := |DC38(v41).cf66|], multiset(v43) + v42];
				v47[667] := multiset{f18, false, f18};
				v47[667] := multiset{if (f18) then !f18 else f18};
				var v48 := 's';
				var v49 := DC13(v34.f24, p2, f23, fm1(f23, f18, f18, globalState), v48);
				var v50: map<char, bool> := map[v49.cf34 := f18 !in v47[667]];
				v50 := v50;
				var v51: array<bool> := new bool[3] [f18, true, fm23(v0, p1, globalState)];
				v51[18] := false;
				v51[18] := ((seq(0x1d8, i4  => (v48))) + p2) <= "afcymv";
				globalState.f2 := !f18;
			}
			...
			match (v53) {
				...
				case DC4(cf12) =>
					...
					v34.m5(v56 * multiset{"sbqo", p0}, v34.f24 % p1, fm25(-0x300, globalState), |(p0 + p2)|, globalState);
					...
					globalState.f12, v57, v34.f24, globalState.f3, globalState.f2 := p0 + (seq(-0x1, i6  => ('k'))), v57, if (|v36| in v58) then v58[|v36|] else -v34.f24, v35.fm22(seq(0x2d1, i7  => ('l')), v59, globalState), cf12;
					var v60: array<D20> := new D20[14](i8 => DC52(v58));
					var v63 := DC52(map v61 : int | (-108 <= v61) && (v61 < 0x1ae) :: (v61 % f23) := (|(map v62 : int | (424 <= v62) && (v62 < 0x30b) :: (v62 + p1) := (true))|));
					v60[965] := v63;
					v60[965] := v63;
				case DC2(cf6) =>
					globalState.f2 := f18;
					var v64 := 'f';
					var v65: multiset<string> := multiset{p0, p0, (if (false) then p0 else p0)[f23 := v64], "dlsydos", p0 + p2};
				...
				case DC5(cf13) =>
					...
					v70[919] := [fm23(v0, v34.f24, globalState), f18, !f18, !f18, f18];
			...
			match (v72) {
				case DC18() =>
					...
					globalState.f3 := -(v36[p1] - v76.cf27) * f23;
					var v77 := 'i';
					...
					var v85: map<int, int> := map[f23 := |v83[|v84|]|];
					var v86: map<int, map<int, int>> := map[p1 := v85];
					var v87: seq<map<int, map<int, int>>> := [v86];
					var v90: array<map<int, map<int, int>>> := new map<int, map<int, int>>[25] [v86, v86 + v86[|v85| := v85], v86, v86, map[v34.f24 := v85], v87[0x3b7], v86, map[f23 := v85], v86, v86, if (v84[v34.f24]) then map[fm1(f23, f18, f18, globalState) := map v88 : int | (-0x92 <= v88) && (v88 < -0xc3) :: (v88 * -|v85|) := (v36[|v84|])] else v86, v86 + v86, v86, v86, fm60(v84, f18, f18, globalState), map v89 : int | (0x374 <= v89) && (v89 < 0x359) :: (v89 % v34.f24) := (v85), v86 + v86, v86, v86, v86, v86, v86 + v86, v86, map[p1 := map[v34.f24 := |"babsckc"|]], v86[v34.f24 := v85]];
					v90[164] := v86;
					var v91 := DC7(v34.f24, f18, 956);
					v80, globalState.f0, v81[718], globalState.f10, v90[164] := v80, -0x29 * 0x3ce, DC4(!f18), fm40(v91, globalState), v86;
				case DC17(cf38) =>
					globalState.f2, globalState.f3, globalState.f2, globalState.f9 := f18, v34.f24, f18, seq(0x2fb, i11  => ('k'));
					globalState.f2 := f18;
					globalState.f2 := f18;
					var v92, v93, v94, v95 := m0(globalState);
			}
			...
			var v102: T1 := new C3(v101, p0, seq(0xcb, i12  => (map[v34.f24 := |{f18, f18, f18}|])), f18);
			var v103: map<string, int> := map["ljsg" := v34.f24];
			var v104: array<int> := new int[17] [|[v96, v96]|, if ((if (|p0| in v98) then v98[|p0|] else v96[0x3cd]) in v97) then v97[if (|p0| in v98) then v98[|p0|] else v96[0x3cd]] else |v101|, p1, v34.f24, |[v102]|, f23, |map[v102.f18 := v102.f18]|, |v103|, -v34.f24, v34.f24, f23, |map[v34.f24 := v34.f24]|, |v96|, p1, |v97|, 0x175, v34.f24];
			var v105: map<array<int>, string> := map[v104 := p2];
			v105 := v105 + map[v104 := p0];
			v104[337] := if (v102.f18) then v34.f24 else p1;
			v104[337] := 0x129 * v34.f24;
			var v106 := 'm';
			...
			v112[170] := v113.cf48;
			v108[935], v111, v112[170], globalState.f2 := v109, v111, f18, f18;
		}
		...
		if (f18) {
			...
			match (v126) {
				case DC11(cf23, cf24) =>
					...
					globalState.f2, v137, globalState.f0 := f18, v138[v34.f24], -(|p2| * p1);
					v118[649] := |p0| - v34.f24;
					v118[649] := 0x32d;
				case DC12(cf25, cf26, cf27, cf28, cf29) =>
				...
				case DC13(cf30, cf31, cf32, cf33, cf34) =>
					...
					var v141: array<char> := new char[23] [cf34, cf34, v125, v125, p2[f23], cf34, cf34, cf34, v126.cf34, v125, v125, cf34, cf34, cf34, cf34, 'n', v125, cf31[cf32], 'b', cf34, v125, v125, v125];
					v140[43] := v141;
					v140[43] := v141;
					v0, globalState.f2, cf34 := {cf30, fm1(-cf30, f18, f18, globalState), cf33, f23 % v34.f24, p1 / v34.f24}, f18, v125;
					globalState.f9 := p2;
					f23 := 0x1ba;
				case DC10(cf22) =>
					globalState.f2 := ((seq(0x138, i18  => (v125)))[v34.f24 := 'l'] == "hbxr") || f18;
	...
	method m1(p0: string, p1: int, p2: string, globalState: GlobalState) {
			...
			decreases 100 - i0
		{
			...
			match (v1) {
				case DC3(cf7, cf8, cf9, cf10, cf11) =>
					var v3: set<bool> := {f18};
					var v4: seq<int> := [p1 % cf8, if (true) then cf10 else |v3|, cf10];
					var v5: map<bool, int> := map[f18 := p1];
					var v6 := DC6(v5);
					globalState.f3 := v4[cf8 * fm1(|v6.cf14|, f18, f18, globalState)];
					globalState.f2 := |p0| != cf10;
					var v7 := 'r';
					var v8: set<char> := {v7};
					var v9 := DC60(v8);
					var v10 := new C7(|(v9.cf98 + v8)|, f18);
					globalState.f15 := (cf7 / cf11) / (-p1 / p1);
				case DC4(cf12) =>
					var v11: array<string> := new string[19] [p2, p0, p0, p0, p2, p2 + p0, p2, seq(-357, i1  => ('r')), p2, "bm", "nvgq" + p0, seq(-59, i2  => ('u')), p0, p0, p0 + p2, p2 + p2, seq(349, i3  => ('u')), seq(-196, i4  => ('j')), (p2 + p0)[|(seq(-0x30d, i5  => ('p')))| := 'i']];
					v11[946] := p2;
					...
					var v18 := DC63(v13);
					var v19: seq<int> := [p1];
					var v20: seq<int> := [p1, v19[|v12|]];
					var v22: array<set<set<int>>> := new set<set<int>>[17] [v13, if (!f18 in v14) then v14[!f18] else {fm49(p1, p1, p1, globalState)}, if (f18) then v13 else v17[p1], v13, v18.cf104, v13, v13, v13, {set v21 : int | v21 in v20 :: (v21 - |['j', 'q']|), v12}, {v12, {p1}}, v13, v13, v13 * v13, v13 - v13, if (f18) then v13 else v13, v13, v13];
					v22[873] := v13;
					v11[946], globalState.f3, v22[873] := p0, p1, if (f18) then v13 + v13 else v13;
					globalState.f15 := -p1 / p1;
				...
				case DC5(cf13) =>
					...
					var v42 := DC38(v41);
					var v43: array<multiset<bool>> := new multiset<bool>[15] [v37[f18 := p1], v37, v37, v37 + multiset{f18, true, f18, f18, f18}, v37, v37, multiset{true}, v38.cf65, multiset{f18, f18} - v37, multiset{fm6(v39, f18, v42.cf66, p1, globalState)}, v37, v37 - (fm32(globalState))[f18 := 0xd5], fm32(globalState) - multiset(v40), v37, v37];
					v43[377] := v37;
					var v44: array<int> := new int[9](i6 => i6 / p1);
					v44[978] := -p1;
					var v45: C0 := new C0(p1, |v39|);
					var v46: seq<C0> := [v45];
					v44[48] := v45.f32;
					v43[377], v44[978], v46, globalState.f2, v44[48] := v37, v45.f31, v46, f18, -p1;
					globalState.f3 := v44[978];
					var v47 := new C1(f18);
					var v48 := 'c';
		...
		var v59: map<bool, seq<int>> := map[f18 := v55[p1 := 0x12e]];
		var v60: array<multiset<int>> := new multiset<int>[16] [multiset([p1, |(v52.(cf73 := map[p1 := f18]).(cf73 := v53)).cf73|, |p2|]), multiset{p1}, v54, multiset{p1}, v54, fm25(-p1, globalState), v54, v54, v54, if (fm6(v56, f18, v54, |v57|, globalState)) then multiset{v55[p1], p1, p1} else v54, v58[p1], multiset((if (f18 in v59) then v59[f18] else v55)[p1 := p1]) * v54, multiset{p1}, v54, if (true) then v54 else v54, multiset{-p1}];
		var v61 := 'y';
		var v62: multiset<char> := multiset{v61};
		v60[968] := multiset{if (v61 in v62) then v62[v61] else |{v61, v61}|, 0x6c, 0x111, p1};
		v60[968] := multiset(v55) + (v54 - v54);
		for i7 := p1 to p1 {
...
class C10 extends T1 {
	...
	function fm3(p0: int, globalState: GlobalState): bool {
		DC1(f21, |f19|, f19[0xcf := 't'], f18, -0x271).cf1
	}
	method m2(p0: map<array<int>, map<bool, int>>, p1: bool, p2: bool, globalState: GlobalState) returns (r0: int) {
		var i0 := 0;
		while (f21)
			decreases 100 - i0
		{
			...
			if (p1) {
				...
				var v0 := -0x348;
				var v1: map<bool, int> := map[(seq(357, i1  => ('b'))) == (seq(136, i2  => ('g'))) := v0];
				v1 := v1[p2 := v0];
				...
				globalState.f15, v2[904], globalState.f2, v0 := -(161 + v0), v3, f18, if (v0 in v8) then v8[v0] else 52 * v0;
				var v9: multiset<bool> := multiset{p2};
				var v10: map<string, bool> := map[f19 := p2];
				var v11: set<int> := {v0, v0};
				var v12: seq<set<int>> := [v11, v11, v11];
				var v13: map<bool, set<int>> := map[v4.f18 := v12[v0]];
				...
				globalState.f2 := p2;
				var v25 := 'm';
				var v26: array<char> := new char[13] [f19[|v22|], v25, 'e', v25, v25, fm46(globalState), v25, v25, v25, v25, v25, v25, v25];
				v26[350] := v25;
				v26[350] := if (p1) then v25 else v25;
				var v27: map<bool, char> := map[false := v25];
				v20 := v20 / |map[v20 := if (!fm3(v20, globalState) in v27) then v27[!fm3(v20, globalState)] else v25]|;
				var v28: array<int> := new int[9](i3 => i3 % 0x198);
				v28[446] := v20;
				...
				v28[446] := (if (p2) then |v29| else v20) + |v33|;
			}
			...
			match (v35) {
				case DC11(cf23, cf24) =>
					...
					var v43: multiset<int> := multiset{|{v41[v39.f26]}|, |v42|};
					var v44 := 'p';
					var v45: set<bool> := {v40.f18, p1};
					globalState.f12 := (f19[0x327 + (if (v37 in v43) then v43[v37] else v39.f26) := v44])[v37 - |v45| := v44];
				...
				case DC12(cf25, cf26, cf27, cf28, cf29) =>
					...
					var v56: set<D3> := {v35};
					v50 := if (v53 in v52) then v52[v53] else v50[|map[([v55])[v55.f30 := v55] := v55.f30]| := v56];
					var v57: map<bool, int> := map[f21 := cf27];
					var v58 := DC13(v55.f30, f19, v55.f30, -495, f19[if (true in v57) then v57[true] else v55.f30]);
				...
				case DC10(cf22) =>
					...
					var v78: map<int, array<map<T1, int>>> := map[v75 / |f19[v75 := v76]| := if (f21) then v77 else v77];
					v78 := v78[v75 := v77];
					...
					var v83: set<bool> := {p2};
					var v84: C4 := new C4(v76, v75, p2, v82[|v83|], f20);
					var v85: map<bool, C4> := map[false := v84];
					var v86: seq<C4> := [v84];
					var v87: seq<bool> := [f18];
					var v88: array<C4> := new C4[24] [v84, v84, v84, v84, if (false in v85) then v85[false] else v84, v84, v84, v84, v84, v84, v84, v84, v84, v86[v75], v84, v84, v84, if (f18) then v84 else v84, v84, v84, if (fm3(|v87|, globalState)) then v84 else v84, v84, v84, v84];
					v88[933] := v84;
					v88[933] := v84;
			...
			var v111: seq<int> := [v97, v110[v97], |f19|, -v97];
			var v112: C2 := new C2(v97, v110, p2);
			var v113: map<bool, C2> := map[v109.cf0 := v112];
			var v114: array<int> := new int[23] [v97, v97, |v98| + |multiset{v99}|, if ([p1] in v101) then v101[[p1]] else v97, v97 / v97, v97, v97, v97, |(seq(0x378, i6  => (|map[[v97, 0x6f] := p1]|)))|, 0x77 % v97, |f19|, if (-v97 in v103) then v103[-v97] else v97, -|v106|, |({-|(map v107 : char | v107 in v108 :: (v107) := (f21))|, |v113|, |f19|} + v104)|, -(v112.f33 % v112.f33), v112.f33, 0x16e, v112.f33, v112.f33 / v97, v112.f33, v97, |v100| + v97, 0x123];
			v114[0] := v97 + v97;
			var v115: map<bool, bool> := map[f21 := p1];
			var v116: map<map<bool, bool>, int> := map[v115 := v97];
			v114[0] := if ((v115 + v115[fm23(v104, v97, globalState) := f18]) in v116) then v116[v115 + v115[fm23(v104, v97, globalState) := f18]] else v112.f33;
		}
		var v117 := DC49(!f21);
		var v118: T0 := new C1(v117.cf83);
		var v119: set<T0> := {v118};
		var v120 := -0x393;
		globalState.f0 := -(|v119| / v120);
		if (v118.f18) {
			var v122: array<int> := new int[18](i7 => i7 - |(set v121 : int | (0x3d4 <= v121) && (v121 < -0x12f) :: (v121 / v120))|);
			v122[897] := v120;
			var v123 := 'x';
			var v124: map<char, map<bool, char>> := map[v123 := fm63(globalState) + map[v118.f18 := v123]];
			var v125 := DC39(v118.f18, 0x229);
			v122[897], globalState.f15, globalState.f2, v124 := v120, v120, v125.cf67, map v126 : char | v126 in map[v123 := v123] :: (v126) := (map[f18 := v123]);
			v122[897] := 703;
			var v127 := DC18();
			match (v127) {
				case DC18() =>
					...
					var v132 := DC16(v122);
					v128[427] := if (v130 in v131) then v131[v130] else v132.cf37;
					var v133: array<C0> := new C0[12];
					var v134: seq<array<C0>> := [v133, v133];
					var v135: set<int> := {|v134|};
					var v136: map<bool, int> := map[false := v122[897]];
					globalState.f2, v135, globalState.f2, v122[897] := false, v135 * ({v120} + {|v136|}), p2, v122[897];
					var v137: map<int, bool> := map[|([p2])[v120 := p1]| := f21];
					v137 := v137;
					var v138: seq<int> := [v120, 0x1f0];
					var v139: map<int, int> := map[v122[897] := v120];
					var v140: multiset<bool> := multiset{true};
					var v141: map<multiset<bool>, bool> := map[v140 := v118.f18];
					var v142: map<int, set<int>> := map[|v141| := {|map[map[!p1 := v122[897]] := v123]|}];
					var v143: C5 := new C5(f19, f19, v118.f18);
					var v144: multiset<int> := multiset{v122[897]};
				...
				case DC17(cf38) =>
					var v147 := DC58(v122[897]);
					v147 := v147;
					var v148: array<C5> := new C5[24];
					var v149: C5 := new C5(f19[v122[897] := v123], seq(0x3bd, i8  => ('s')), v118.f18);
					v148[293] := DC64(!f18, v118.f18, v149, v122[897], v122[897]).cf107;
					v148[293] := v149;
					var v150: array<multiset<set<C5>>> := new multiset<set<C5>>[13];
					var v151: set<C5> := {v148[293]};
					var v152: seq<C5> := [v148[293]];
					var v153: multiset<set<C5>> := multiset{v151, {v152[v122[897]]}, v151};
					v150[166] := v153;
					var v154: multiset<bool> := multiset{p1};
					var v155: map<int, set<C5>> := map[if (!false in v154) then v154[!false] else v122[897] := v151];
					var v156: seq<set<C5>> := [v151, v151, v151, {v148[293], v149}];
					var v157 := DC64(p2, !f18, v149, v122[897], v122[897]);
					var v158: seq<seq<set<C5>>> := [[if (v120 in v155) then v155[v120] else v151], v156 + v156, [v151, {v157.cf107, v149}]];
					v150[166] := multiset(v158[v120]);
					v120 := -|(f19 + "csklyy")|;
			}
			
			globalState.f2 := fm3(if (!f18) then v122[897] else v120, globalState);
			...
			var v163 := DC56(v162 <= v162, v120, 778 * v160[663]);
			match (v163) {
				case DC56(cf92, cf93, cf94) =>
					...
					var v166: multiset<multiset<bool>> := multiset{v165[v118.f18 := v160[663]]};
					var v167 := DC47(v166);
					var v168: C1 := new C1(f18);
					var v169: map<D19, C1> := map[v167 := v168];
					var v170: seq<int> := [cf93, v120, cf93, |v169| * cf94];
					globalState.f3 := v170[fm1(cf94, p2, v118.f18, globalState)];
					...
					var v178: array<bool> := new bool[23] [true, v118.f18, p2, v118.f18, v118.f18, v118.f18, v118.f18, false, cf94 <= v120, f21 <==> f18, cf92, cf92, cf92, v118.f18, v118.f18, v118.f18, v162[v120], cf92, v118.f18, v118.f18, f18, !f18, v177 != v177];
					v178[673] := true || p1;
					v178[673] := f21;
				case DC55(cf91) =>
			...
			if (true) {
				...
				var v185: C4 := new C4(v183, if (v160[663] in v184) then v184[v160[663]] else v160[663], f18, "jgqnwe", (f20 + (seq(0x2c, i10  => (map[v160[663] := -0x1cb]))))[v160[663] := map[v160[663] := |f19[v120 := 'c']|]]);
				v185 := v185;
				var v186: array<bool> := new bool[26];
				v186[451] := |[v118.f18]| <= v160[663];
				globalState.f2, v186[451] := p2 <== f21, v118.f18;
				globalState.f2 := f21;
				...
				var v194: map<array<bool>, seq<char>> := map[v193 := seq(0x337, i12  => (v191))];
				var v195 := DC1(if (p2 in v192) then v192[p2] else f18, v160[663], if (v193 in v194) then v194[v193] else f19, v118.f18, v120);
				var v196: map<bool, bool> := map[v195.cf1 := f21];
				var v197: map<int, int> := map[v120 := v160[663]];
				var v198 := new C4(v191, v120, if (v118.f18 in v192) then v192[v118.f18] else v118.f18, f19, ([v197])[|v196| := map[v120 := v160[663]]]);
				var v199: map<bool, int> := map[if (p1 in v192) then v192[p1] else v118.f18 := |v162|];
				var v200: map<char, map<bool, int>> := map[v191 := v199];
				v160[663] := fm1(0x2ed, v118.f18, (map[v191 := v199])['b' := v199] != v200, globalState);
				var v201: set<int> := {v160[663]};
				...
				var v206 := DC66(v205);
				var v207: map<multiset<set<int>>, map<bool, map<bool, char>>> := map[v202[{v120} := if (v118.f18 in v203) then v203[v118.f18] else v120] := if (p2) then v206.cf115 else v205];
	...
	method m1(p0: string, p1: int, p2: string, globalState: GlobalState) {
		...
		var v5: map<int, array<char>> := map[0x104 := v4[--0x162]];
		...
		globalState.f15 := |(seq(0x1ef, i0  => (f19)))| / p1;
	}
	method m3(p0: int, p1: map<bool, set<int>>, p2: bool, globalState: GlobalState) {
		var v0 := 'h';
		var v1: map<int, char> := map[p0 := v0];
		var v2: seq<int> := [p0];
		var v3: T0 := new C4(if (|v2| in v1) then v1[|v2|] else v0, p0, f21, ("pjdlrtgp" + f19)[p0 := v0], f20);
		...
		var v10: map<bool, array<int>> := map[[v7] != v9[fm1(p0, false, true, globalState)] := v5];
		...
		for i0 := p0 + -fm1(p0, true, v3.f18, globalState) to |(v6.f27 + "rwis")| {
			var v11: map<int, bool> := map[p0 % -i0 := v6.f28[p0 := 'w'] != v6.f27];
			...
			var v14: array<bool> := new bool[29] [!f21, !v13[p0], f18, f21, f18, f18, v3.f18, f18, f18, false, v3.f18, v3.f18, p2, v3.f18, true, false, p2, !f21, f21, v3.f18, !v3.f18, p2, f21, v3.f18, v3.f18, v3.f18, p2, p2, v3.f18];
	...
	method m4(p0: bool, p1: multiset<char>, globalState: GlobalState) returns (r0: bool, r1: bool, r2: bool) {
		...
		var v5: C3 := new C3(seq(0x362, i1  => (v1.f24)), f19, f20, p0);
		var v6: seq<C3> := [v5, v5];
		v6 := v6 + [v5, v5];
		...
		v9[513] := v11 + v11;
		var v12: set<int> := {v0};
		var v13: map<multiset<char>, int> := map[multiset(seq(-0x181, i3  => (v10))) := v0];
		var v15: map<char, bool> := map[v10 := true];
		r2, globalState.f15, v8, globalState.f0, v9[513] := fm23(v12, |v13|, globalState), v0, {f19, "pewnmqk", "cswc", "erclvwh", f19} + v8, v0, (fm64(v10, v1.f24, p0, p0, globalState) + v11) + (map v14 : char | v14 in v15 :: (v14) := (-v0));
		var v16 := DC42(f18);
		match (v16) {
			case DC42(cf74) =>
				...
				if (v22.cf112) {
					...
					var v24: seq<seq<int>> := [seq(0x29, i6  => (|{!v17[145]}|)), v5.f35[v0 := |v23|]];
					var v25: map<seq<int>, bool> := map[v24[|f19|] := cf74];
					v25 := (v25 + v25[v5.f35 := p0]) + (if (f18) then v25 else v25);
					var v26: array<C2> := new C2[6];
					var v27: C2 := new C2(v1.f24, [v1.f24, 0x388], false);
					v26[468] := v27;
					v26[468] := v27;
					globalState.f2 := (v27.f33 % -v1.f24) != -v5.fm38("dqcnaoqeg", globalState);
					v21[460] := v1.f24;
					var v28: map<array<bool>, bool> := map[v17 := v17[145]];
					var v29 := DC32(v28);
					...
					v30[271] := v29;
					v21[460], v30[271], v17[145], cf74, globalState.f10 := 658 + 0xec, if (f21 <== cf74) then v29 else v29.(cf54 := v28), f18, v10 !in f19, v5.f35;
					v21[460] := v1.f24;
				} else {
					r2 := f21;
					var v31: seq<bool> := [f18, false];
					var v32: C5 := new C5(f19, f19, true);
					var v33 := DC64(v31[314], cf74, v32, |f19|, v1.f24 % -v0);
			...
			case DC43(cf75, cf76) =>
				...
				v44 := (v44 + v44) + (if (f21) then v44 else v44[v0 := f18]);
			case DC44(cf77, cf78) =>
				v10 := v10;
				r1 := f18;
				var v48 := DC50(!f18);
				match (DC51(v48)) {
					case DC48(cf82) =>
						...
						var v51: seq<string> := [f19, f19[v1.f24 := v10], f19];
						globalState.f3 := |(if (f21) then f19 + "eofv" else v51[v1.f24])|;
					case DC49(cf83) =>
					...
					case DC50(cf84) =>
						...
						var v54: multiset<string> := multiset{f19, f19, f19, f19[|v53| := v10], seq(186, i8  => (v10))};
						v1.m5(v54, v1.f24, v7, v1.f24 / 0x30e, globalState);
						...
						var v63 := DC73(v60);
						var v64: array<multiset<seq<T1>>> := new multiset<seq<T1>>[20] [v58, v58 - v58, v58, v58, v58 - v58, v58, v58, v58, v58, v58 + v61[v1.f24], v58 - multiset{if (cf84 in v62) then v62[cf84] else v59}, v58, multiset{v59}, v58, v58, v58[v59 := |v57.f19|], v58, multiset{[v57, v57], v59[v0 := v57]}, v58 - multiset(v63.cf130), v58 * v58];
						var v65: map<bool, multiset<seq<T1>>> := map[cf84 := v58];
						v64[88] := if (f21 in v65) then v65[f21] else v58;
						var v66 := DC74(v58);
						globalState.f9, v64[88] := fm12(|v5.f35|, v57.f18, globalState), v66.cf131;
						var v67: array<map<bool, bool>> := new map<bool, bool>[10];
						var v68 := DC53(f18, v57.f18, v10);
						v67[291] := (map[cf84 := p0])[v68.cf87 := cf84];
						var v69: seq<bool> := [f21];
						var v70: seq<bool> := [v69[|f19|]];
						var v71: map<bool, bool> := map[v69[-v1.f24] := cf84];
						var v72: seq<map<bool, bool>> := [v71];
						var v73: seq<map<bool, bool>> := [v71 + v71, map[f18 := f21], v71, v72[v1.f24], v71];
						v67[291] := v72[|multiset{v57.f18}| - |f19|];
					case DC47(cf81) =>
						...
						var v95 := DC59(v94);
						v88[179][885] := v1.f24;
						r1, v95, cf77, v88[179][885] := f21, v95, cf77, -(v1.f24 % (if (f21) then fm1(v1.f24, p0, p0, globalState) else cf78));
						...
						v98 := v98[v1.f24 := v97];
					case DC51(cf85) =>
						globalState.f3 := v1.f24 % (if (p0) then cf78 else |(set v99 : int | v99 in multiset{v0} :: (v99 * -|[[DC43(multiset{map[|map[false := true]| := -0x3c1]}, false).cf76, false, true, false], [false, false]]|))|);
						globalState.f15 := cf78;
						r1 := false;
						var v100 := DC67(f21, |f19|, v5.f35, f21);
						var v101: array<seq<int>> := new seq<int>[6] [v5.f35, v5.f35, v5.f35, v5.f35 + [v1.f24, cf78], (seq(0x25e, i9  => (|map[v1.f24 := true]|))) + [v1.f24], v100.cf118];
						v101[452] := v5.f35 + v100.cf118;
						v101[452] := ([|f19|, |map[f21 := v1.f24]|])[v0 + v1.f24 := cf78 + |(set v102 : int | v102 in (seq(0x2ba, i10  => (|v5.f35|))) :: (v102 - 0x155))|];
				}
			...
			case DC41(cf73) =>
				...
				if (multiset(if (f18) then v103 else fm44(v0, f18, if (v10 in v104) then v104[v10] else v10, 0xad, globalState)) != (if (p0 in v106) then v106[p0] else v105)) {
					...
					globalState.f0 := 0x2a3 % 0x305;
				} else {
					v1.f24 := v1.f24;
					v1.m1(f19, v1.f24, f19, globalState);
					globalState.f12, v1.f24 := (if (f18) then "qnpnbbeug" else f19) + fm12(v1.f24, p0, globalState), (v1.f24 % v0) / v1.f24;
					r1 := f18;
				...
				var v110: array<char> := new char[24] [f19[v1.f24], v10, 'm', v10, v10, v10, v10, v10, 'a', v10, v10, v10, f19[-v0], v10, v10, v10, if (f18) then v10 else v10, v10, v10, v109.cf89, v10, v10, v10, 'k'];
				v110 := v110;
...
method Main() {
	...
	for i0 := v2 to |v14| {
		if (v3) {
			...
			v19[212] := if (0x98 in v20) then v20[0x98] else v11;
			v19[212] := v11;
			globalState.f0 := i0;
			var v21, v22, v23, v24 := m0(globalState);
		} else {
			v11[331] := v3;
			v11[331] := if (v3) then v3 else !true;
			var v25, v26, v27, v28 := m0(globalState);
			v27[698] := v25;
			v27[698] := fm1(if (v3) then fm1(-0x2d, v11[331], false, globalState) else v2, v3, v3, globalState);
			v11[331] := v3 <==> true;
			var v29: map<int, int> := map[i0 := v27[698]];
			v29, v11[331], v7 := (v29 + v29) + v29, v25 <= (0x2ec * v25), v7[-(0xd2 - |v28|) := v11[331]];
		}
	...
	if (!(v2 == v2) ==> v3) {
		...
		globalState.f3 := |{seq(404, i1  => (v12)), v0}|;
		...
		globalState.f2, globalState.f0, v45 := v3, |[v3, v2 >= v47.cf128, v3]|, if (v48 <= multiset{v2}) then v45 else v49[v2];
		var v50: map<int, int> := map[v2 := v2 % v2];
		v50 := map[v2 := 30];
	...
	for i2 := v2 - v2 to v2 {
		...
		if (fm23(v64, v2, globalState)) {
			...
			v68[992] := v64;
			var v69: seq<seq<int>> := [v5, v5, seq(-0x10b, i3  => (i2)), v5];
			v68[992], globalState.f15, v69, v8 := v64, fm1(i2, v3, false, globalState), v69, v8;
			v8[12] := i2 + v2;
			var v70: multiset<string> := multiset{(DC13(v2, "ppao", i2, 281, v12).(cf33 := 0x33c)).cf31};
			v8[12] := if (!true) then if (v3) then v2 else i2 else |v70| - v2;
			...
			v73[120] := v0 + (seq(519, i5  => (v12)))[v2 := v12];
			globalState.f15, v72, globalState.f0, v73[120] := v8[12] % v2, v72, v5[v8[12]], fm16(|v0| + 661, v14 !! v14, v12, v8[12], globalState);
			var v74: set<seq<int>> := {v5};
			var v75: C6 := new C6(v7, i2, !v3, seq(0x28c, i6  => (v12)), seq(0x3e7, i7  => (map[i2 := i2])));
		...
		match (v84) {
			...
			case DC68(cf120) =>
				...
				globalState.f9 := (v0 + (v0 + v0))[(if (v12 in v94) then v94[v12] else v2) * -v96.cf123 := v12];
				var v97 := DC16(v8);
				...
				var v103: map<T0, int> := map[v102[-i2] := 0x9b];
				v103 := v103[v101 := i2];
		}
	...
	v8[173] := v2;
	v8[173] := v2;
	var v108: C7 := new C7(v5[v2], v3);
	v108 := v108;
	globalState.f0 := v8[173];
	globalState.f15 := v2;
	var v109: map<bool, string> := map[v3 := v0];
	var v110: array<string> := new string[11] [v0, v0, seq(-0x162, i9  => (v12)), v0, v0, v0, (if (v3 in v109) then v109[v3] else "fbdttde") + v0, v0 + v0, "mp", if (v3) then "sstrrcv" else v0, v0];
	v110[283] := v0;
	v110[283] := v0;
	var v111 := DC42(v3);
	var v112: map<int, bool> := map[v2 := v111.cf74];
	for i10 := |{v3}| / |v112| to if (v3) then v108.f24 else 0x82 {
		v108.m1("scoeuba", i10, v110[283] + v0, globalState);
		var v113: C5 := new C5(v110[283], v110[283], v3);
		var v114: map<int, int> := map[v8[173] := v2];
	...
	var v118: map<char, string> := map[v12 := v110[283]];
	var v119: array<T0> := new T0[24];
	var v120: map<array<T0>, string> := map[v119 := v110[283]];
	...
	v11[635], v121[325], v122, v3, globalState.f2 := (if (v3) then v122 else v122) !! multiset{false}, v12, multiset(if (false) then v7 else v7) + v122, fm23(v123, -0x1d7, globalState), v0 < v0;
	globalState.f2 := v111.cf74;
	var v124: multiset<map<bool, int>> := multiset{map[v11[635] := 871] + map[v11[635] := |v122|]};
	v124 := v124[map[false := v8[173]] := v2];
	v8[173] := v108.f24 + fm1(v2, v3, !v3, globalState);
}