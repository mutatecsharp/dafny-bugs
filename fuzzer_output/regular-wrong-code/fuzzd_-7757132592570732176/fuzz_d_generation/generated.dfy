...
function fm0(globalState: GlobalState): seq<int> {
	(seq(0x36c, i0  => (|[--0x102, |[false]|, -|"ohtdbkn"|, 0x382, |{0x1da, |[-0xee]|}|]|))) + ([|(seq(0x10f, i1  => ('g')))|, |[|"ajayh"|, |"li"|, 0x3d4]|, |(seq(-0x15f, i2  => ('y')))|] + (seq(0x381, i3  => (-0x9c))))
}
function fm1(p0: int, p1: bool, p2: int, globalState: GlobalState): bool {
	!((set v0 : seq<char> | v0 in (seq(0x2c9, i0  => ([DC26('o').cf48]))) :: (v0)) != {['p']}) <==> (if (false) then true else false)
}
...
function fm3(globalState: GlobalState): map<bool, map<int, bool>> {
	(map[true := map[|(seq(0x55, i0  => ([|"ypcfvnmt"|, |(seq(0xb, i1  => (|map[true := 't']|)))|])))| := true]] + map[false := map[|"jfvw"| := false]]) + (map[false := map[|multiset{!false, true, false, !false, false}| := true]] + map[true := map[973 := true]])
}
...
function fm15(p0: bool, p1: int, p2: seq<bool>, p3: bool, globalState: GlobalState): D0 {
	DC1(433, (seq(296, i0  => ('a'))) + "sogpqka", |multiset(seq(0x278, i1  => (|multiset{-0x7a}|)))| % -|{270}|, !false)
}
function fm17(globalState: GlobalState): D2 {
	DC7([|multiset([false])|, 97, |(seq(0x3b1, i0  => ('t')))|, 929, -|map[|{false, true}| := |{|multiset{0x24e}|, |"enf"|}|]|] + [|map[true := |map[|map[!true := false]| := |map[true := 'q']|]|]|, 0x77])
}
...
function fm22(p0: map<int, int>, globalState: GlobalState): set<string> {
	if (false) then {seq(-0x3d6, i0  => ('h'))} else {"ctdyvhdge", "xrw"} * (set v0 : string | v0 in ["wa", "ucjpv"] :: (v0))
}
...
function fm36(p0: int, p1: int, p2: char, globalState: GlobalState): string {
	if ({false, false, false} != {!false, true, false, true, DC3(0xe9, true).cf8}) then seq(0x278, i0  => ('t')) else (seq(-0x351, i1  => ('c'))) + "hen"
}
...
function fm38(p0: char, p1: char, globalState: GlobalState): map<int, int> {
	match DC35(-0x2c0) {
		...
		case DC32(cf57) => if (true) then map[844 := |(seq(0x8a, i0  => ('n')))|] else map v0 : int | v0 in map[-112 := 0x35e] :: (v0 % |map[50 := |"rlsvjvt"|]|) := (|{false, true}|)
	}
}
...
function fm41(globalState: GlobalState): seq<D6> {
	seq(0x15f, i0  => (DC18(true)))
}
function fm42(p0: int, p1: int, globalState: GlobalState): map<bool, bool> {
	map[!false := |[false]| == |map[|(seq(-0x240, i0  => ([false])))| := !true]|]
}
function fm43(p0: int, p1: bool, p2: int, p3: string, globalState: GlobalState): D6 {
	DC16((map v0 : int | (0x172 <= v0) && (v0 < 0x2b7) :: (v0 + 956) := (DC15(-0x228, |"polbyirqn"|, -0x141))) + map[0x33a := DC15(|(seq(288, i0  => ('w')))|, |(seq(0x120, i1  => ('n')))|, |(seq(877, i2  => ('q')))|)])
}
...
function fm46(p0: bool, p1: bool, globalState: GlobalState): D4 {
	DC12(multiset{'p'} >= multiset(seq(0x15, i0  => ('v'))), 0x150, |map[!!!!true := |"vul"|]|)
}
...
function fm48(p0: int, p1: char, p2: int, p3: bool, globalState: GlobalState): multiset<D0> {
	(multiset{DC3(-0x26a, !true)} * multiset([DC3(|map[false := |map[!!true := true]|]|, false)])) - multiset(seq(0x3dd, i0  => (DC3(0x34a, !true))))
}
...
function fm50(globalState: GlobalState): set<int> {
	(if (!false) then {|"rxvistgq"|} else {0x2e1, -0x1e6}) - ({-904, |multiset{map[true := false], map[false := false]}|, |multiset(seq(0x3e2, i0  => (|multiset([false])|)))|} * {-|(map v0 : char | v0 in (set v1 : char | v1 in ['s', 's'] :: (v1)) :: (v0) := (|multiset{DC47(map v2 : char | v2 in map['k' := 'd'] :: (v2) := (false)), DC47(map['a' := true])}|))|, -460, -|[false]|, -|map[false := |[|multiset{true}|]|]|, -|(seq(387, i1  => ('f')))|})
}
function fm51(p0: int, p1: int, p2: int, p3: char, globalState: GlobalState): map<multiset<int>, int> {
	(map[multiset{|(seq(0x2fd, i0  => (DC44({DC41(true)}))))|} := |"hct"|] + map[multiset(seq(0x12a, i1  => (|{true}|))) := |(seq(-0x10c, i2  => (-0x24a)))|]) + map[multiset{-0x33a, |"lvbhnjfvo"|, 40, |map[|[|multiset{'s'}|]| := true]|, 0x256} := 0x1a8]
}
function fm52(globalState: GlobalState): multiset<D3> {
	multiset{DC9(|multiset{|[|map[false := -0x10f]|, |multiset{|multiset{|[true]|, -0x3b1, 163}|, |[true]|}|, |map[false := true]|, |{|(seq(-984, i0  => (|"qdnyphax"|)))|, 315}|]|}|, map v0 : int | (0x283 <= v0) && (v0 < 0xc8) :: (v0 + 0x2a3) := (map[|{true}| := |(seq(0x299, i1  => ('t')))|]), |"eaevsmgns"|, DC45(0x288, {--451, -|multiset{[|{347, |multiset{'k', 'x'}|}|, 0xd9]}|}).cf69), DC9(582, map[|{true}| := map[0x205 := |map['o' := false]|]], -0x3a, |map[[false, true] := |map[false := false]|]|)} * (multiset{DC9(|multiset{|[true, true, !false, false]|}|, map[DC51(0xa0, |[0xb9]|).cf78 := map[-0x234 := DC33(|[true]|).cf58]], 0x1b2, |map['d' := 'l']|), DC9(0x21e, map[0xf6 := map[|map[|map[0x5 := false]| := 0x2f7]| := 769]], |map[true := false]|, |"pf"|)} - multiset{DC9(-0x269, map[|[-0xe4]| := map[337 := -|"hjucomr"|]], |"hckgvkew"|, |multiset{false}|), DC9(-585, map[-|{false}| := map v1 : int | v1 in map[-411 := |[|[|map[-0x332 := -0x279]|, -|map[0x3a7 := true]|]|]|] :: (v1 - 0x2e8) := (0x237)], -0x1ea, |map[0x187 := true]|), DC9(|"iduro"|, map[-0x3db := map[|"a"| := |[false]|]], 348, |(map v2 : int | v2 in [0xd5, |"sokhgatk"|] :: (v2 / |multiset{-745}|) := (true))|)})
}
function fm53(p0: map<int, int>, p1: int, globalState: GlobalState): D3 {
	DC9(0x240, map[|{false}| := map[|"mvqhguysd"| := |(seq(0x3e1, i0  => ('i')))|]], 377, -(if (false) then -0x10a else -394))
}
function fm54(p0: bool, globalState: GlobalState): map<bool, map<int, int>> {
	map[|{false, true, false}| !in multiset([0x96]) := if (false) then map[0x261 := |multiset([|(seq(0x139, i0  => (-0x2c7)))|, |(map v0 : int | v0 in [-0x2ea] :: (v0 % |"xflc"|) := (|map["rbjpcvxsh" := true]|))|])|] else map[|{DC19(!true, true, false, true, [!true])}| := 0x29a]]
}
...
function fm56(p0: int, p1: bool, p2: bool, p3: bool, globalState: GlobalState): seq<map<int, bool>> {
	(seq(0x137, i0  => (map[-0x3d := true]))) + [map[0x314 := false], map v0 : int | (0x3ce <= v0) && (v0 < -0x1d1) :: (v0 % |{false, !!true}|) := (false)]
}
function fm57(p0: int, p1: bool, globalState: GlobalState): map<map<int, bool>, seq<bool>> {
	(map[map v0 : int | (0x86 <= v0) && (v0 < 0x23b) :: (v0 - |(map v1 : int | (0x23e <= v1) && (v1 < -0x2fb) :: (v1 - |map[[false] := false]|) := (true))|) := (false) := [true]] + map[map[48 := false] := [false]]) + ((map v2 : map<int, bool> | v2 in map[map v3 : int | (652 <= v3) && (v3 < 0xa2) :: (v3 % 0x3bb) := (false) := true] :: (v2) := ([false])) + (map v4 : map<int, bool> | v4 in map[map v5 : int | v5 in {|{-|[!true]|, 0x2d9}|} :: (v5 * |map[false := |map[true := |"a"|]|]|) := (true) := |{0x2c8}|] :: (v4) := ([!false])))
}
function fm58(p0: int, globalState: GlobalState): D15 {
	DC41(multiset{-|map["jk" := DC63([DC37(), DC37()]).cf92]|, 880, |(map v0 : int | (0x3b5 <= v0) && (v0 < 0x397) :: (v0 / |{false, true}|) := (|multiset{false}|))|} >= multiset{0x13d, |"cjorlfrb"|, 636})
}
function fm59(p0: int, p1: int, p2: bool, p3: int, globalState: GlobalState): map<map<int, int>, int> {
	map[map v0 : int | v0 in {170, |map[|"xuifnsqld"| := 197]|} :: (v0 * 437) := (0xf2) := 0x224] + (map[map v1 : int | (0x2ea <= v1) && (v1 < -0x3b5) :: (v1 / 0x393) := (607) := 0x1c1] + map[map[|multiset([multiset{0x187}, multiset([0x2bd])])| := |multiset([-|map["hcxgjmau" := true]|])|] := 737])
}
...
function fm63(globalState: GlobalState): seq<multiset<int>> {
	[multiset([642]), multiset{|[map v0 : int | (360 <= v0) && (v0 < 0x3e) :: (v0 + |"ihi"|) := (|[0x5a]|)]|, 0xa, 0x331}, multiset([-0x1f8])] + ([multiset{0x1ee, |map[true := 0x2ea]|, |[false, true]|}] + (seq(-0x33d, i0  => (multiset{|(seq(0x392, i1  => ('m')))|}))))
}
...
function fm71(p0: bool, p1: bool, p2: int, p3: bool, globalState: GlobalState): set<seq<int>> {
	{[0x2ca], seq(710, i0  => (754))} - (set v0 : seq<int> | v0 in [[|{false}|]] :: (v0))
}
method m0(globalState: GlobalState) returns (r0: array<multiset<bool>>, r1: bool) {
	var v0 := 0x33a;
	var v1: seq<int> := [v0];
	var v2: set<int> := {v1[v0], v0};
	...
	match (if (v5 >= v5) then v4 else DC2(v0, v3)) {
		case DC1(cf1, cf2, cf3, cf4) =>
			if (!!(cf4 && cf4)) {
				...
				globalState.f8, r1, r1, cf3, v9 := v10[v0], cf4, -(0x33d % v0) != v0, v0 * (cf3 % v1[cf1]), if (cf4) then v9 else v9;
				...
				v12[772] := v15[if (cf4 in v16) then v16[cf4] else cf1];
				cf3 := -0x44;
				globalState.f8 := fm1(cf3 - cf1, cf4, v0 * |v2|, globalState);
				v12[772], cf3, v12[772] := !cf4, v0, v12[772];
			}
			...
			v17[662] := (v19 + v19[|fm50(globalState)| := cf4]) == [fm1(cf3, cf4, v0, globalState)];
			var v20: seq<seq<bool>> := [v19];
			var v21: C7 := new C7(|fm0(globalState)|, cf1);
			var v22: map<C7, seq<bool>> := map[v21 := v19];
			r1 := (v20[cf3])[v0 := cf4] !in [if (v21 in v22) then v22[v21] else [v17[662]]];
		case DC2(cf5, cf6) =>
			var v23 := true;
			var v24 := 'r';
		...
		case DC0(cf0) =>
			...
			var v39: seq<multiset<array<int>>> := [v38, (multiset{v3, v3})[v3 := -0x17a], v38];
			var v40: map<multiset<array<int>>, int> := map[v39[v0] := v0];
			var v41: map<char, int> := map[v37 := 911];
			var v42: seq<bool> := [v36];
			v40 := v40[if (fm1(0xac, v36, |v41|, globalState)) then v38 else v38 := |(if (v36) then v42 else v42[v0 := v36])|];
	...
	if (fm1(|v47|, false, v0, globalState)) {
		if (true) {
			...
			var v55: multiset<bool> := multiset{v45, v54[v48.f17], v48.f16, true, v45};
			var v56: T0 := new C12(v53, |v55| * v48.f17, v45, v48.f17, v3[294]);
			var v57: map<int, array<bool>> := map[v3[294] := v52];
			...
			globalState.f8, v56, r1, v48.f17, v58 := v56.f11, v56, v48.f16, v3[294], v58;
			v3[294] := |v1|;
			...
			v62[654] := v63;
			var v64 := "ncihubldw";
			var v65: C8 := new C8(seq(957, i3  => (v0)));
			...
			var v69: array<string> := new string[26] [v64, v64, "xkd", v64, "hrjw", v64, v64, fm36(v61.f25, v61.f25, v46, globalState), v64, if (v65 in v66) then v66[v65] else if (v64 in v67) then v67[v64] else v64, v64[v61.f25 := if (fm2(globalState) in v68) then v68[fm2(globalState)] else v46], v64, v64, v64, v64, v64, v64, v64, "n", (seq(537, i4  => (v46)))[v61.f25 := v46], "bljdb", v64, v64, v64, "xdgj", v64];
			...
			v61, v62[654] := if (v69 in v70) then v70[v69] else v61, v71[|v73|];
		...
		if (v45) {
			...
			v78[309] := if ((0x66 % v0) in v80) then v80[0x66 % v0] else v45;
			globalState.f8 := v45;
			v0 := v0;
			globalState.f8 := v78[309];
			var v81 := new C10(v78[309]);
		} else {
			v3[676] := v0;
			v3[676] := v0;
			var v82: map<seq<int>, bool> := map[v1 := v45];
			var v83: map<int, int> := map[v3[676] := 0xc4];
			...
			var v90 := "tf";
			var v91 := DC25((v89[v88 := |v90|])[v88 := v0]);
			var v92: map<D20, D8> := map[v87 := v91];
			var v93: set<map<D20, D8>> := {map[v87 := DC25(v89)], v92};
			var v94: array<bool> := new bool[16] [v0 > |v82|, !!!(fm2(globalState) in v83), false, v45, !v45, v45, v45, v0 >= 0x3d4, false, v0 < |v1|, fm2(globalState) != v3[676], (set v86 : map<D20, D8> | v86 in v85 :: (v86)) !! v93, v45, v45, v0 !in v1, v45];
			v94[730] := !v45;
			var v95: multiset<bool> := multiset{v45};
			v94[730] := (v0 + v3[676]) > (if (v45 in v95) then v95[v45] else v0);
			var v96 := DC41(v45);
			var v97: map<char, D15> := map[v46 := v96];
			v97 := v97[v46 := v96];
			var v98 := DC51(v3[676] - |v90|, if (v45 in v95) then v95[v45] else v3[676]);
			v98 := if (v95 != v95) then v98 else if (v94[730]) then v98 else v98;
			var v99: array<char> := new char[28];
			v99[648] := v46;
			v3[676], v99[648], v94[730] := v3[676], 'w', false && v94[730];
		}
		
		var v100: C2 := new C2(v0 * v0, ("mvjqaslaa")[-v0 := v46], v0, v0, fm1(v0, false, -0xd8, globalState));
		v100 := v100;
		v3[981] := -865;
		v3[981] := v0;
		...
		match (v102) {
			...
			case DC35(cf60) =>
				var v111 := "gmrtyw";
				cf60 := |((v111 + v111) + (seq(0x24f, i7  => (v46))))|;
				v0 := v0;
				...
				v114 := fm69(v115 + map[v45 := map[v0 := v45]], v0 % v0, cf60, map[v116 := map v117 : char | v117 in v47 :: (v117) := (343)] != v118, globalState);
				globalState.f8 := v45;
			case DC32(cf57) =>
	...
	if (v45 <==> (if (v142 in v143) then v143[v142] else v45)) {
		...
		v140 := (multiset([v0, v142.f13]))[v142.f12 := v142.f12] + fm12(-v142.f12, v0, |{v45, v45}|, v45, globalState);
		v45 := v45;
		var v151 := new C3(if (v45) then v142.f12 else 0x216, |v1|);
		v0 := -313 / (v142.f13 % v142.f13);
	} else {
		var v152: C3 := new C3(v142.f13, v142.f12);
		v152 := v152;
		var v153: array<string> := new string[20] [v142.f20, v142.f20 + v138, v138, v138 + v138, v142.f20, v138, v138[v142.f12 := v46], if (v45) then v142.f20 else seq(0x2bd, i11  => (v46)), v142.f20 + v142.f20, v142.f20, v138, v142.f20, v138, "sjuxrcwi", v142.f20[v142.f13 := v46], v142.f20, v138 + v142.f20, v138, v142.f20, v142.f20];
		v153[903] := v142.f20 + (seq(141, i12  => (DC26(v46).cf48)));
		v153[903] := v142.f20;
		if (DC17(v45, v0).cf30 && (v45 <==> v45)) {
			...
			v156 := v156[v45 := v142];
			var v157: map<int, seq<int>> := map[v142.f12 := seq(0x29e, i13  => (v142.f13))];
			v157 := v157[|v1| := (seq(0x1fb, i14  => (|v1|))) + v1];
		...
		v163, v165, v166[761] := v163, if (v45 <== v45) then v165 else v165, DC7([v142.f12, |map[|map[v142 := v167]| := v45]|, |map[v142.f12 := v0]|, v142.f12, v142.f13]).cf13;
		v3 := v3;
	}
...
class C1 {
	...
	method m24(globalState: GlobalState) {
		var v0 := "mbgh";
		var v1 := 'y';
		var v2: array<string> := new string[28] [v0, v0, v0, v0, v0, v0 + (seq(931, i0  => ('c'))), v0, v0, (seq(229, i1  => ('h'))) + "deoknfa", v0 + v0, v0, v0, v0, v0, fm36(f27, |v0|, v1, globalState), v0, v0, v0, v0, "qscv", v0 + v0, v0, v0, v0 + v0, v0, "beqc", v0, v0];
		v2[599] := v0;
		var v3: map<char, string> := map[v1 := v0];
		var v4 := true;
		v2[599] := (if (v1 in v3) then v3[v1] else fm36(f27, -|{v4}|, v1, globalState)) + v0;
		var v5 := DC13(f27);
		v5 := v5;
		var v6 := new C0(f27, v4, f27 % f27, f27);
		...
		if (if (v6.f29 in v7) then v7[v6.f29] else v6.f28 >= v6.f28) {
			...
			if (v6.f29) {
				...
				var v15: multiset<int> := multiset{-DC9(v6.f28, map[-v10[44] := v14], v6.f28, v10[44]).cf17};
				var v16: map<multiset<int>, int> := map[v15 := v6.f28];
				v12, v12, v13, v1 := v12, v12, multiset{v4} + (v13 - multiset(v8)), v0[v6.f28 + |(set v17 : multiset<int> | v17 in v16 :: (v17))|];
				f27 := -0xb6;
			} else {
				var v18: set<bool> := {v6.f29};
				var v19: seq<set<bool>> := [v18];
				var v20: map<set<bool>, bool> := map[v18 := v6.f29];
				var v21: multiset<int> := multiset{v10[44], |v20|};
				var v22: array<bool> := new bool[11] [false, v6.f29, v18 >= v19[v6.f28], v18 == v18, v18 <= v18, !fm1(v6.f28, v6.f29, v10[44], globalState), v4, v21 > v21, v6.f29, v6.f29, !!v4];
				v22[151] := v6.f28 != v6.f28;
				v22[151], f27 := !(v6.f28 >= v10[44]), f27;
				var v23: array<array<int>> := new array<int>[25];
				v23 := v23;
				v10 := v10;
				v2 := v2;
				var v24: map<bool, string> := map[v6.f29 := v2[599]];
				globalState.f8, v4 := v4 !in (v24 + v24), v6.f29;
			}
			
		} else {
			var v25: array<bool> := new bool[6];
			v25[311] := v6.fm7(globalState);
			var v26 := DC3(|[v6.f29, true]|, false);
			var v27: map<D0, bool> := map[v26 := v6.f29];
			var v28: seq<int> := [v6.f28];
			var v29: seq<bool> := [if (DC3(|v2[599][f27 := v1]|, fm1(|map[true := |v28|]|, v6.f29, v6.f28, globalState)) in v27) then v27[DC3(|v2[599][f27 := v1]|, fm1(|map[true := |v28|]|, v6.f29, v6.f28, globalState))] else v6.f29, v4, v6.f29];
			v25[311] := v29[f27 + v6.f28];
			if (v2[599] <= v0) {
				...
				v34 := v34[v33 := v25];
				var v35: array<seq<bool>> := new seq<bool>[4] [v29 + [v25[311]], [false], v29[f27 := true], v29];
				v35[310] := v29 + v29;
				var v36: set<bool> := {v4};
				f27, v35[310], v25[311] := v6.f28 - v6.f28, v29 + (v29 + v29), v6.f29 || fm1(f27, false, |v36|, globalState);
				var v37: multiset<int> := multiset{v31.f28};
				v37 := v37 - (DC5(multiset{v6.f28}, v4).cf10 - multiset{v6.f28});
			} else {
				var v38: array<int> := new int[24](i5 => i5 % v6.f28);
				var v39 := DC2(|v28|, v38);
				f27 := v39.cf5;
				...
				v41 := (v41 + (seq(81, i6  => (DC11(v40))))) + (v41 + v41[f27 := v43]);
			...
			v2[599] := (v0 + (v47.cf2 + v0))[|{|(seq(0x284, i7  => (|map[v6.f28 := v6.f29]|)))|}| := v1];
		...
		v0 := (match v55 {
			...
			case DC2(cf5, cf6) => v2[599]
			case DC3(cf7, cf8) => "a"
			case DC0(cf0) => v2[599]
		})[|v56[f27 := |multiset(v57)|]| * v6.f28 := v1];
		...
		v58[999], v59, f27 := |v60|, v59, -v6.f28;
	}
}
...
class C2 extends T2, T0 {
	...
	function fm32(p0: int, globalState: GlobalState): map<map<int, int>, bool> {
		match DC3(|f20[f12 := 'o']|, f11) {
			case DC1(cf1, cf2, cf3, cf4) => if (cf4) then map[map v0 : int | (65 <= v0) && (v0 < 0x152) :: (v0 % f26) := (f26) := f11] else map[map[cf3 := |map[false := f12]|] := f11]
			...
			case DC0(cf0) => map[map[f12 := f12] := f11] + map[map v3 : int | (0x329 <= v3) && (v3 < 501) :: (v3 / f26) := (f13) := f11]
		}
	}
	...
	method m11(p0: int, p1: int, p2: int, p3: array<int>, globalState: GlobalState) returns (r0: array<bool>, r1: int, r2: set<int>) {
		if (f11) {
			...
			v3 := if (fm6(f11, v5[0x144], |v6|, globalState)) then v3 - {f11} else {f11};
			f11 := fm33(f12 - p0, globalState);
		} else {
			var v7 := 's';
			var v8: array<int> := new int[13](i0 => i0 / f13);
		...
		var v17: map<bool, int> := map[(v15.(cf21 := f11, cf22 := |(seq(-0x220, i1  => (p1)))|)).cf21 := |(if (f11) then v16 else v16)|];
		var v18: seq<map<bool, int>> := [v17, v17];
		var v19 := DC1(p1, f20, f26, f11);
		v17 := if (f11) then v18[f26] else map[f11 := |v19.cf2|] + v17;
		...
		if (!(f11 <==> (if (v20 in v22) then v22[v20] else f11))) {
			...
			if (f11 && f11) {
				...
				v23[638], p3[204], globalState.f8, v24[963], v23 := f11, |(map[p3[204] := f11] + v25)| * p2, (f12 < 0x2ff) <==> !f11, if (true) then v15 else DC12(f11, |f20|, -p2), if (f11) then v23 else v23;
				var v26 := "b";
				v26 := v26;
			} else {
				r1 := 758;
				...
				v27 := new int[10] [p3[204], p2 * p2, p3[204], --f12, 0x1f9, p1, p2, 344, f26, |{f11}|];
			...
			if (f11) {
				...
				globalState.f5 := v31;
				var v32 := new C0(fm2(globalState), f11, p3[204], p2);
			} else {
				r1 := f12;
				...
				var v35 := 's';
				var v36: map<string, string> := map[(seq(0x369, i5  => ('w')))[f12 := v35] := f20];
				v36 := v36;
				var v37 := DC8(v17);
				...
				v40 := v40[v39 := v35];
				var v41: seq<int> := [f13];
				var v42: multiset<seq<int>> := multiset{v41[p3[204] := p0], v41};
			...
			var v47: array<seq<int>> := new seq<int>[4](i6 => seq(-821, i7  => (0x28c)));
			var v48 := DC0(v47);
			v46[990] := v48;
			v46[990] := DC0(v48.cf0);
		} else {
			...
			var v54: seq<string> := [seq(350, i8  => ('l'))];
			v50 := v50[multiset(v53 + v53) := v54 < v54];
			if (f11) {
				var v55: array<int> := new int[1] [v53[f12]];
				...
				var v60: map<int, int> := map[f12 / -f13 := fm2(globalState)];
				var v61 := 'f';
				v60 := v60[f13 * p0 := |f20[0x2 := v61]|];
				globalState.f5 := v53;
				p3[439] := f26;
				p3[439] := -f13;
				...
				var v68: set<int> := {-78};
				globalState.f8, p3[439], p3[439], v62, globalState.f8 := !(-p3[439] > p0), p3[439], if (v67 in v66) then v66[v67] else |v51|, v62, !((v62 > v62) && (v68 <= v68));
		...
		var v75 := DC10(v72);
		r1 := match v75 {
			case DC9(cf15, cf16, cf17, cf18) => cf18 % |multiset{f11, f11}|
			case DC8(cf14) => f13
			case DC10(cf19) => p2
		};
	...
	method m23(p0: bool, p1: int, p2: int, globalState: GlobalState) returns (r0: string, r1: D3, r2: int, r3: bool) {
			...
			decreases 100 - i2
		{
			...
			match (fm37(fm6(p0, |fm36(f12, p2, v2, globalState)|, p2, globalState), v3.cf1, fm33(p2, globalState), p2, globalState)) {
				case DC15(cf26, cf27, cf28) =>
					cf27 := f13;
					var v4: seq<int> := [cf28];
					var v5: array<int> := new int[17] [cf27, 0x3d, cf26, cf27, f26, v4[|v1|], |f20|, 975, f12, f26, f26, p1, cf27, fm2(globalState), cf26, f13, cf28];
					...
					var v12: array<int> := new int[19] [f13, cf28, |(v6[v5 := f26] + v6)|, f26 * |f20|, cf27, p2, p2, f26, cf26, p1 % cf26, -p2 / fm2(globalState), -(if (f11) then f13 else -cf27), |(v8 - v8)|, p1, if (v2 in v9) then v9[v2] else f13, if (f11 in v10) then v10[f11] else -p1, |v7| * f13, p1 / (if (v3 in v11) then v11[v3] else cf27), f26];
					v12 := new int[27];
					...
					v12 := v5;
				case DC14(cf25) =>
					var v15: array<int> := new int[4] [f12, |(seq(0x2d6, i3  => (multiset([f26]))))|, -f13, f13];
			...
			r2, v21[157], v26 := f12 % |(if (f11) then fm38(v2, v2, globalState) else v27)|, v22, v26 + v26;
			r2 := -0x1f1;
		...
		if (!(false ==> (v30 in v32))) {
			...
			if (v34.f29) {
				var v49 := new C0(-(v34.f28 / f13), v34.f29, f26, v48[f26]);
				v49.f29 := f11;
				r2 := v49.f28 * f12;
				...
				var v53: array<map<int, D5>> := new map<int, D5>[27](i6 => DC16(map[v34.f28 := DC15(v34.f28, v34.f28, |(map v52 : int | (916 <= v52) && (v52 < 0x2ef) :: (v52 % 0x30d) := (f20))|)]).cf29);
			...
			globalState.f8, r2, v58, v29 := if (v36[0x1d5]) then f11 else p0, f26, {v36[p1]}, v29;
		} else {
			f11 := p0;
			if (!fm7(globalState)) {
				...
				v59[991], f11, r3, v61, r3 := f26, (if (DC5(v62, v61).cf10 in v63) then v63[DC5(v62, v61).cf10] else f12) >= -(if (p0 in v64) then v64[p0] else p2), v61, p0, false;
				v59[991] := f13;
				var v65 := DC13(-v59[991]);
				v65 := v65;
			} else {
				v29 := v29;
				...
				v72 := v73[-p1];
				r2 := |((seq(0x3c, i8  => (v29))) + (f20 + f20))[p2 := f20[f12]]|;
				var v74: seq<bool> := [f11, f11, p0];
				var v75: array<int> := new int[23](i9 => i9 - v68[0x155]);
				...
				v76[705] := v75;
				var v77 := DC18(p0);
				globalState.f8, globalState.f8, globalState.f8, v74, v76[705] := f11, true, f11, ([p0, p0, p0, v77.cf32] + v74)[f13 := f11], v75;
			}
			...
			globalState.f8 := v80 > ({if (p0 in v81) then v81[p0] else v79, v79, seq(0x35c, i10  => (f13))} * v80);
			r2 := fm2(globalState);
			var v82: seq<bool> := [p0];
			v82 := (v82 + v82) + v82;
		}
		...
		var v84 := DC8(v83);
		v84 := fm40(v29 in f20[f12 := 'y'], globalState);
		var v85: map<string, bool> := map[f20 := f26 == 905];
		var v87: map<string, int> := map[(seq(-0x137, i11  => (v29)))[|"a"| := v29] := f13];
...
class C3 extends T1 {
	...
	method m22(p0: multiset<multiset<int>>, globalState: GlobalState) returns (r0: bool, r1: array<int>, r2: array<map<int, bool>>, r3: set<D2>) {
		...
		match (v4) {
			case DC9(cf15, cf16, cf17, cf18) =>
				...
				if (!({'g', v5, v5, v5, v5} !! v6)) {
					...
					var v15: array<seq<bool>> := new seq<bool>[21] [[v7, v7], v13, if (v11.f29) then v13 else v13, v13, [true, false, v7, v11.f29], v13, v13, [v7, v7], v13, v13, v13, v13, v13, v13 + v13, v13, [v7, v11.f29], v13[v11.f28 := v11.f29], v13, v13, v13, (v13 + v13)[|p0[v14 := cf17]| := v7]];
					...
					v23[938] := v24 + v24;
					var v25 := false;
					var v26: array<bool> := new bool[6] [fm1(f12, v25, |(seq(0x50, i2  => (v5)))|, globalState), v25, v25, v25, v25, v25];
					v26[225] := v25;
					var v27: multiset<int> := multiset{cf15, -0x37d};
					var v28: array<int> := new int[8] [|[if (cf17 in v27) then v27[cf17] else cf17]|, cf15, f12, cf15, 0x152, f13, |("a")[f12 := 'w']|, 625];
					var v29: array<array<int>> := new array<int>[7] [v28, v28, v28, v28, if (v25) then v28 else v28, v28, v28];
					v29[950] := v28;
					globalState.f8, v26[225], v29[950] := v25, !v25, v28;
					var v30: map<bool, int> := map[v26[225] := cf18];
					var v31: multiset<map<bool, int>> := multiset{v30};
					var v32 := new C0(cf18, v26[225], cf15, |v31|);
				...
				v40[504] := if (v36) then v41 else v41;
				v40[504] := v41 + fm42(cf15, f12, globalState);
				var v42: array<bool> := new bool[2];
				v42[789] := -cf15 == cf15;
				var v43: array<int> := new int[10];
				var v44: seq<int> := [|v0|];
				cf18, v42[789] := f12, ([|{v43}|, f13, cf18, 0x1dc, f13] + fm0(globalState)) == (v44 + v44);
			case DC8(cf14) =>
				...
				var v56 := DC20(v55);
				r0, r0, globalState.f8, v52 := v47.f29, v51 == ("vqio" + v51)[v47.f28 := v48], fm44(globalState) >= v54, DC20(v56);
				if (v47.f29) {
					var v57 := 494;
					...
					v67, r0, v57 := v67, v47.f29, v63[0x36c];
					var v68: map<int, array<int>> := map[-872 := v46];
					var v69 := DC9(f13, v1, |v64|, v47.f28);
					r1 := if (v62[v69.cf17] in v68) then v68[v62[v69.cf17]] else v58[v47.f28];
					...
					v51 := v47.fm8(|v71|, map[v48 := v47.f29], v72, globalState) + (seq(0x14c, i4  => ('y')));
					var v73: map<int, bool> := map[-v47.f28 := v47.f29];
					var v74: seq<bool> := [true];
					var v75: multiset<bool> := multiset{if (v47.f28 in v73) then v73[v47.f28] else v47.f29, v47.fm7(globalState), v47.f29, v74[f12], v47.f29};
					var v76 := DC21(v75[v47.f29 := f13]);
					v75 := v76.cf39;
					var v77 := 0xef;
					...
					v74, v47.f29, v77, v78, v79 := [!DC23(v45, v74, v47.f29, |v80|).cf44, v47.f29, v47.f29 <== v47.f29], if (v47.f29) then false else fm7(globalState), fm2(globalState), v81[-f12], v83.cf47 * multiset{v82};
			...
			case DC10(cf19) =>
				...
				v87[920] := true;
				var v88 := "ghpgbut";
				v86, v86, v87[920] := f12, (-0x146 % f13) * (-0x366 + fm2(globalState)), v86 >= |v88|;
				v87 := v87;
				var v89: T0 := new C2(if (v87[920]) then -0xf7 else -417, v88 + (seq(0x366, i5  => (v88[f12]))), f13, v86, 463 < v86);
				v89 := v89;
				var v90: seq<bool> := [v89.f11, false, v87[920]];
		...
		var v99 := DC19(true, !(v94 !! v94), {|(map[fm2(globalState) := seq(0x2f8, i6  => ('x'))])[f13 := v95]|, f12, f13} > v98, v93, v96 + v96);
		match (v99) {
			case DC17(cf30, cf31) =>
				cf31 := -((f13 / 0x193) * f12);
				globalState.f8, cf31, cf31 := v97[f13] > f12, |v97|, |v98|;
				var v100: array<int> := new int[25](i7 => i7 - |v96|);
				var v101: seq<array<int>> := [v100, v100];
				var v102: map<bool, array<int>> := map[cf30 := v100];
				var v103: array<array<int>> := new array<int>[29] [v100, v100, v100, v100, v101[317], v100, v100, v100, v100, v100, v100, v100, v100, v100, v100, v100, v101[|v98|], v100, v100, if (!!v93 in v102) then v102[!!v93] else v100, v100, v100, v100, v100, v100, v100, v100, v100, v100];
				var v104 := DC22(f12, v103);
				var v105: map<int, D7> := map[f12 := if (v93) then DC22(cf31, v103) else v104];
				v105 := v105[v97[if (v93 in v94) then v94[v93] else f12] := v104];
				var v106: map<bool, int> := map[true := -733];
				var v107 := 'q';
				var v108: map<char, bool> := map[v107 := v93];
				var v109 := DC23(v103, v96, v93, f13);
				var v110 := new C2(|v106| * cf31, fm8(f13, v108, {f12}, globalState), v109.cf45, 0x4, v96[f13]);
			case DC18(cf32) =>
				...
				var v116 := 'h';
				var v117: multiset<string> := multiset{v95[|v115| := v116]};
				v113 := map v114 : string | v114 in v117 :: (v114) := (v96);
			case DC19(cf33, cf34, cf35, cf36, cf37) =>
				var v118 := new C0(636 - 0x192, v93, f13, 788 / 350);
				var v119 := 0x2cc;
				v119 := f12;
			...
			case DC16(cf29) =>
				...
				v127[76] := if ((584 - |v131[v130 := |[v132, DC11(v0[if (v93 in v94) then v94[v93] else |"ajqx"| := f13])]|]|) in v128) then v128[584 - |v131[v130 := |[v132, DC11(v0[if (v93 in v94) then v94[v93] else |"ajqx"| := f13])]|]|] else f13 % f12;
				globalState.f8 := v93;
				globalState.f5, v127[76] := v97, v127[76];
			case DC20(cf38) =>
		...
		var v139 := 0x3ce;
		v139 := f12;
		var v140: map<multiset<int>, int> := map[(multiset(v97))[f12 := f13] := |v97|];
		v139 := |(v97[6 := f13] + [v139, -|v140|, f13])|;
		...
		v145[829] := v97[|map[v93 := v147]|];
		v93, v93, v141, v144, v145[829] := !v93, f13 < f12, fm43(-f13, v93 && v93, f12, v95 + v95, globalState), [v95], v139;
		r0 := v93;
		r1 := v145;
		var v148: array<map<int, bool>> := new map<int, bool>[16];
		r2 := v148;
		var v149 := DC7(seq(898, i9  => (|[DC21(v94), DC21(v94), DC21(v94)]|)));
		r3 := {v149};
	}
}
...
class C4 extends T0 {
	...
	method m21(p0: string, p1: array<bool>, p2: int, p3: int, globalState: GlobalState) returns (r0: int, r1: int, r2: array<bool>) {
		...
		v1 := map[v0 := |v0|] + (v1 + v1);
		var v3: map<int, map<int, int>> := map[p2 := map v2 : int | (0x299 <= v2) && (v2 < 0x376) :: (v2 % p3) := (p3)];
			...
			decreases 100 - i0
		{
			...
			r0, v9, r1, r1, v10 := -fm2(globalState), v11[|v10| * p3], p2 - p3, p2 % fm2(globalState), p0 + p0;
			r1 := |v10|;
		}
		...
		if (!(fm30(f11, !f11, |v13|, globalState) < fm30(f11, false, f25, globalState))) {
			...
			var v20: seq<string> := [v19[DC2(p3, v16).cf5], v18, v18[p3 := p0[fm2(globalState)]]];
			v18 := v20[f25 / p3];
			match (DC3(p2, f11)) {
				case DC1(cf1, cf2, cf3, cf4) =>
					var v21: map<bool, int> := map[f11 := 0xa7];
					var v22: seq<int> := [|"gmgibfqjo"|];
					r0, globalState.f8, globalState.f8 := (if (cf4 in v21) then v21[cf4] else |v0|) + |p0|, v13[f11 := v22[|cf2|]] !! v13, if (cf4) then f11 else f11;
					var v23: set<int> := {cf3};
				...
				case DC2(cf5, cf6) =>
					...
					var v29 := DC6(v28);
					v29 := v29;
					cf5 := if ((if (!f11 in v15) then v15[!f11] else f11) in v13) then v13[if (!f11 in v15) then v15[!f11] else f11] else cf5 % p2;
					var v30: seq<int> := [p2];
					var v31 := new C2(if (f11) then p2 else f25, p0, -v30[p2], 0x323, !true);
				case DC3(cf7, cf8) =>
					v14 := v14[p2 := p3];
				...
				case DC0(cf0) =>
					v18 := p0;
					var v35 := DC1(p3, seq(0x2f9, i2  => ('t')), p3, f11);
			...
			if (!f11) {
				...
				p1[174] := f11;
				p1[174] := if (fm1(p2, f11, 0x323, globalState)) then !!!f11 else f11 <==> f11;
				var v44: map<int, bool> := map[p2 := p1[174]];
				var v45: map<map<int, bool>, bool> := map[v44 := !f11];
				var v46: array<bool> := new bool[5];
				var v47: map<map<map<int, bool>, bool>, array<bool>> := map[v45 + map[map[f25 := p1[174]] := !fm6(!!f11, p2, p3, globalState)] := v46];
				v47 := v47[v45 := v46];
			} else {
				var v48: seq<int> := [p2];
				r1 := v48[f25 * f25];
				var v49: seq<array<int>> := [v16, v17[399], v16];
				var v50 := DC4([v17[399], v16]);
				var v51: seq<D1> := [DC4(v49), v50.(cf9 := v49).(cf9 := v49)];
				v51 := v51;
				f11 := (fm46(f11, v0[0x66], globalState)).cf21;
		...
		for i3 := fm2(globalState) - p3 to f25 {
			if (true) {
				...
				v62 := v62;
				globalState.f8 := (if (f11) then seq(0x94, i4  => ('t')) else p0) == (if (f11) then p0 else seq(0x53, i5  => ('m')));
				...
				var v73: array<seq<int>> := new seq<int>[6] [v71, v71, v71, v71, [p2, i3, |(seq(91, i7  => (-349)))|, v71[|v72|]], v71];
				var v74 := DC0(v73);
				var v75: set<D0> := {v74};
				p1[77], v68[207], v70, v67[104], f11 := v75 >= v75, v69, v70, v67[104], p0 != p0;
				var v76 := DC2(i3, v67);
				v76 := v76;
			}
			...
			v77, v78[729], globalState.f8, r1 := (v77 + v77) * v77, v79[p3], f11, f25;
			p1[182] := f11;
			p1[182] := f11;
			p1[182] := p1[182];
		}
...
class C5 {
	...
	function fm26(p0: bool, p1: int, p2: bool, p3: bool, globalState: GlobalState): set<int> {
		(set v0 : int | v0 in (seq(0x3c6, i0  => (|[|multiset([!false, true])|]|))) :: (v0 / 11)) + ((set v1 : int | (-0x275 <= v1) && (v1 < 0x1a7) :: (v1 * f24)) * {f24})
	}
	function fm27(p0: int, p1: string, p2: bool, globalState: GlobalState): bool {
		(f24 % f24) > (f24 - DC1(f24, "hgoabqmce", |"ir"|, true).cf1)
	}
	method m20(p0: seq<array<bool>>, globalState: GlobalState) returns (r0: int, r1: int, r2: string) {
		...
		match (v2) {
			case DC9(cf15, cf16, cf17, cf18) =>
				...
				var v8: multiset<int> := multiset{|(map v7 : int | (0x146 <= v7) && (v7 < 0x385) :: (v7 % v5.f28) := (v5.f29))|};
				v6[332] := v8;
				var v9: array<array<int>> := new array<int>[12];
				var v10 := DC23(v9, v3, false, -0x20);
				var v11: seq<int> := [f24];
				v6[332], r0, v10, globalState.f8 := v8 - (multiset(v11))[|v3| := cf15], cf18, v10, v5.f29 && v0;
				v10 := v10;
			case DC8(cf14) =>
				if (v0) {
					...
					v13[452] := f24;
					var v14 := "ihtnsyxyb";
					v12, globalState.f8, v13[452], r0 := v12, v0, f24, f24 - |v14|;
					var v15: map<bool, bool> := map[false := v0];
					v15 := v15[v0 ==> v0 := v0];
					var v16: seq<bool> := [!v0];
					var v17 := DC19(v0, v0, v0, v0, v16);
					var v18 := DC20(v17);
					r0, v18, r0 := f24 / v13[452], v18, |v15|;
					...
					var v25 := DC24(DC22(v13[452], v24));
					var v26: map<int, array<array<int>>> := map[f24 := v24];
					var v27: seq<int> := [f24];
					v25 := DC24(DC21(v19)).(cf46 := DC23(if (|map[v27[v13[452]] := !v0]| in v26) then v26[|map[v27[v13[452]] := !v0]|] else v24, v16, v0, |v14|));
				} else {
					...
					var v38: seq<int> := [v37[if (0x1d9 in v36) then v36[0x1d9] else f24]];
					var v39 := DC5(v36[f24 := f24] - multiset(v37), v0);
					v39 := v39.(cf11 := v0 || v0);
			...
			case DC10(cf19) =>
				...
				v42[286] := v0;
				v42[286] := v0;
				var v43: array<D5> := new D5[1](i1 => DC15(f24, |(seq(0x110, i2  => ('e')))[f24 := 'b']|, f24));
				...
				var v48 := DC15(|map[fm1(|[v42[286], v0, v0, v0, v42[286]]|, v42[286], f24, globalState) := |[f24]|]|, f24, f24);
		...
		if (v54.cf4) {
			...
			var v56 := 'r';
			r1 := |(v53[f24 := v56])[-(-fm2(globalState) - f24) := v56]|;
			var v57 := DC26(v56);
			...
			v65[933] := v0 && (if (v0) then v0 else v65[933]);
		}
		...
		var v71 := 'j';
		v68 := v68[v69 := v53[if (0x1e3 in v70) then v70[0x1e3] else f24 := v71]];
		...
		match (v85.(cf19 := v84)) {
			case DC9(cf15, cf16, cf17, cf18) =>
				...
				v92[957] := true;
				v92[957] := v0 !in v66;
				var v93 := new C2(cf17, seq(0x171, i5  => (v71)), f24, 943, v0);
				var v94: seq<int> := [fm2(globalState)];
				cf15 := -((cf18 / v94[f24]) * fm2(globalState));
				r1 := -cf17 % f24;
			case DC8(cf14) =>
				...
				if (!v0 && v0) {
					...
					v95[146] := v97;
					v95[146] := fm1(0x13b, v97 ==> v0, f24 * |(map[v97 := f24])[v97 := f24]|, globalState);
					var v98 := new C1(0x3a);
					r1 := f24;
					var v99 := new C1(|v53|);
				} else {
					r0 := fm2(globalState);
					r0 := -f24;
					var v100: map<map<bool, bool>, int> := map[v79 := f24 % f24];
					var v101: seq<int> := [f24, f24];
					var v102: multiset<seq<int>> := multiset{v101, seq(315, i6  => (f24))};
			...
			case DC10(cf19) =>
				...
				var v107 := DC26(v53[|v69|]);
				match (v107) {
					case DC27(cf49, cf50, cf51, cf52) =>
						r0 := -((cf51 * cf51) * fm2(globalState));
						v1 := v1[true := f24];
						var v108 := new C4(fm2(globalState) % |[true]|, v0);
					...
					case DC26(cf48) =>
						...
						var v121: array<array<bool>> := new array<bool>[13] [v116, v116, v116, v116, v116, v116, v116, v116, if (!v0) then v116 else v116, v116, v117[0x177], v116, v116];
						v121 := if (v0) then v121 else v121;
					case DC28(cf53) =>
						var v122: array<string> := new string[16];
						v122[9] := v53;
						v122[9] := seq(-765, i9  => (v71));
						v71 := v122[9][|("rdjws")[-0xc1 := v71]| % f24];
				...
				match (v130) {
					case DC17(cf30, cf31) =>
						...
						var v134: array<int> := new int[18](i10 => i10 / 744);
						v134[205] := -cf31 + cf31;
						v134[205] := f24;
						r2 := (v53 + v53) + (v53 + v53);
					case DC18(cf32) =>
					...
					case DC19(cf33, cf34, cf35, cf36, cf37) =>
						...
						var v140: array<seq<int>> := new seq<int>[4] [v139, [fm2(globalState), f24, f24, |v70|, f24], v139, seq(0x60, i11  => (v139[f24]))];
						var v141 := DC0(v140);
						v141 := v141.(cf0 := v140);
						r0 := f24;
					...
					case DC16(cf29) =>
						...
						v146[219] := if (-f24 in v147) then v147[-f24] else fm1(0x29f, v0, |v148|, globalState);
						v146[219] := false;
						var v149: array<int> := new int[22];
						v149 := v149;
						var v150: array<seq<bool>> := new seq<bool>[3] [[v0] + v66, v66, v66 + v66];
						v150[877] := v66 + fm39(v146[219], v146[219], globalState);
						var v151: map<char, seq<bool>> := map[v71 := v66];
						v150[877] := v66 + (if (v71 in v151) then v151[v71] else fm39(v146[219], v0, globalState));
					case DC20(cf38) =>
						...
						v152[977] := v0;
						globalState.f8 := !v0;
						var v154 := DC5(multiset{f24, |("okfmdc")[f24 := v71]|}, v0);
						v70 := v70 - v154.cf10;
						r0 := |v53| + f24;
				}
		...
		r0 := f24;
		r1 := f24;
		var v155: multiset<map<bool, bool>> := multiset{v79, v79, (map[v0 := v66[f24]])[v0 := v0]};
		var v156 := DC25(v155);
		r2 := (match v156 {
			case DC25(cf47) => v53 + "sywamaofb"
		})[-f24 := if (v0) then v53[f24] else v71];
	}
}
...
class C6 {
	...
	method m18(globalState: GlobalState) returns (r0: bool, r1: int, r2: multiset<array<bool>>, r3: D0) {
		...
		if (v0) {
			v1 := 0xb7;
			var v7: array<bool> := new bool[12] [v1 != v1, fm1(v1, v0, -v1, globalState), v0, v5[v1], if (v0) then v0 else !v0, v0, v0, v0, v0, v0, v0, v0];
			v7[966] := v0;
			v7[966] := fm1(v1, v0, v1, globalState);
			globalState.f8 := fm1(0x34a, v7[966], v1, globalState);
			var v8: map<int, bool> := map[v1 * v1 := v7[966]];
			v8 := v8[v1 := v7[966]];
			var v9: map<char, int> := map['a' := fm2(globalState)];
			var v10 := 'o';
			var v11: map<string, int> := map[v2[|v9| := v10] + v2 := v1];
			v11 := v11[v2 := v1];
		} else {
			globalState.f8 := v0;
			var v12 := 't';
			...
			var v21: map<int, array<string>> := map[v19[972] := v20];
			v21 := v21[-v19[972] := v20];
			var v22: seq<int> := [0x3b8];
			var v23: map<bool, seq<int>> := map[v0 := v22];
			globalState.f5 := (if (v0 in v23) then v23[v0] else [v1, v19[972], v19[972], v19[972], v1]) + v22;
		}
		...
		if (v0) {
			...
			var v31: map<bool, multiset<bool>> := map[if (v28.f29) then v0 else !true := v30[v28.f28]];
			v31 := if (v0) then v31 else v31;
			globalState.f8 := v28.f29;
			var v32: map<int, int> := map[v28.f28 := 0xb7 % -0x338];
			...
			globalState.f8, v0, r0, v5, v38[199] := v0 <==> (v1 <= v1), v0, v0, [!(if (v38 in v39) then v39[v38] else v0), v0], v1 > (v1 - v1);
			v33 := v33;
			var v40: array<string> := new string[12](i3 => v2);
			var v41 := DC14(v40);
			var v42: map<D5, bool> := map[v41 := v38[199]];
			var v43: seq<int> := [-|v42[v41 := v38[199]]|, v1, v1];
			var v44: map<char, bool> := map[v33 := v43[v1] <= v1];
	...
	method m19(p0: set<int>, p1: array<int>, p2: int, p3: multiset<bool>, globalState: GlobalState) returns (r0: array<bool>) {
		...
		v4[601] := true;
		var v5 := "r";
		if (v5 != (if (v4[601]) then v5 else v5)) {
			var v6: set<int> := {p2, |(v5 + v5)|};
			var v7 := DC29(p0);
			v6 := v7.cf54;
			var v8 := DC13(473);
			var v9: map<D4, multiset<bool>> := map[v8 := multiset{v1}];
			var v10: map<map<D4, multiset<bool>>, int> := map[if (v1) then map[DC13(461) := p3] else v9 := |v5[if (v4[601] in p3) then p3[v4[601]] else p2 := v3]| % p2];
			v10 := v10[v9 := p2 + p2];
			var v11: array<array<set<bool>>> := new array<set<bool>>[5];
			var v12: array<set<bool>> := new set<bool>[6](i0 => {v1, !v4[601]});
			v11[798] := v12;
			v11[798] := v12;
			p1[227] := fm2(globalState);
			var v13: map<int, array<int>> := map[p2 := p1];
			var v14: map<bool, bool> := map[v1 := true];
			var v15: seq<bool> := [v4[601], !v4[601], v4[601], v1];
			var v16: set<map<bool, bool>> := {v14, if (v15[|v5|]) then v14 else v14, map[v4[601] := v1]};
			var v17: seq<int> := [p2, 0x379];
			var v18: set<bool> := {v1, v1};
			globalState.f8, p1[227], v13, v16, globalState.f8 := v15[p2 % |v17|], |v18|, v13, v16, v4[601];
			p1[227] := v17[p2];
		} else {
			var v19 := 0x2bb;
			v19 := 970;
			var v20: seq<bool> := [v1];
			var v21 := DC19(v1, v1, v20[p2], v1, v20);
			v1 := !v21.cf33;
			v4[601] := v4[601];
			if (v1) {
				var v22: set<bool> := {v1, !v4[601]};
				var v23: map<set<bool>, string> := map[{v4[601], v1} - v22 := "qaswmji"];
				v23 := v23[v22 - v22 := (seq(0xc6, i1  => (v3))) + v5];
				p1[557] := v19;
				p1[557] := p2;
				var v24: array<D6> := new D6[19];
				var v25: multiset<array<D6>> := multiset{v24};
				var v26 := DC30(v25);
				v3, v25 := v3, v25 + (v25[v24 := p2] * v26.cf55);
				v4[601] := v4[601];
				var v27: map<int, char> := map[p1[557] := v3];
				var v28: map<bool, int> := map[v4[601] := |{|v27|, p1[557]}|];
				v4[601] := |v28| == v19;
			} else {
				var v29: map<string, bool> := map[v5 := v4[601]];
				var v30: map<bool, int> := map[if (v5 in v29) then v29[v5] else v4[601] := v19];
				v30 := v30;
				var v31: seq<string> := ["ssylvffrk"];
				var v32 := DC1(p2, v5, p2, !v1);
				v31 := [seq(-907, i2  => (v3)), v5, v32.cf2, v5, v5];
				v19 := v19;
				v19 := p2;
				p1[532] := v19;
				var v33: seq<seq<bool>> := [v20];
				var v34: seq<int> := [if (v1 in p3) then p3[v1] else v19];
				var v35: C2 := new C2(p2, v5, p2, v34[p2], v1);
				var v36: multiset<C2> := multiset{v35};
				p1[532], v19, v19, v19 := p2, -(p2 * (|v5| + |v33|)), v19 / (p2 / (if (v35 in v36) then v36[v35] else v19)), (p2 * p2) - v19;
			}
			
			p1[681] := p2;
			var v37: map<bool, multiset<bool>> := map[v4[601] := (multiset{v1, v1})[true := p2]];
			var v38 := DC32(v37);
			p1[681], v37 := v19 + v19, v38.cf57;
		}
		...
		match (v40) {
			case DC5(cf10, cf11) =>
				...
				var v45: set<bool> := {v1};
				globalState.f8 := (v45 !! v45) <== v4[601];
				var v46 := -0x3b4;
				v46 := if (v1) then p2 else p2;
			case DC4(cf9) =>
				var v47 := new C0(p2 % p2, v1, p2, 0x1e3);
				var v48: map<bool, bool> := map[true := true];
				var v49 := new C2(p2 / p2, fm36(v47.f28, v47.f28, v3, globalState), v47.f28, -p2, if (v4[601] in v48) then v48[v4[601]] else v1);
				v1 := (multiset{v4[601], v47.f29} != p3) && v47.f29;
				var v50: map<int, bool> := map[v49.f26 + 732 := fm1(p2, v4[601], 0x1d5, globalState)];
				v50 := v50[v47.f28 / v49.f26 := v47.f28 <= p2];
			case DC6(cf12) =>
				...
				var v54: seq<int> := [p2 + p2, |"ivvcj"| % |v53|, -p2 % p2];
				var v55: set<bool> := {v4[601], v1, v1};
				v52 := v54[|(v55 * {v1})|];
				v55 := v55;
		}
		
		v5 := ((v5 + v5) + v5)[p2 - p2 := v3];
		var v56: seq<bool> := [v4[601]];
		var v57 := DC1(fm2(globalState), v5, p2, v56[p2]);
...
class C7 extends T1 {
	...
	method m16(p0: bool, p1: string, p2: array<bool>, p3: int, globalState: GlobalState) returns (r0: array<int>, r1: array<bool>) {
		for i0 := f13 to f13 {
			...
			p2[272] := !(!p0 <==> v2[i0]);
			p2[272] := !p0;
			var v3: array<map<bool, int>> := new map<bool, int>[13](i1 => DC8(map[false := v1]).cf14 + map[p2[272] := f12]);
			var v4: map<bool, int> := map[p2[272] := -p3];
			v3[874] := v4;
			v3[874] := v4 + v4;
		}
		var i2 := 0;
			...
			decreases 100 - i2
		{
			...
			match (v10) {
				...
				case DC10(cf19) =>
					...
					globalState.f8, globalState.f8, v5, v5, v5 := p0, p0, f13 % f12, --p3, |v27|;
					globalState.f8 := p0;
			}
		...
		var v40: seq<int> := [f13];
		for i3 := f13 to |fm22(map v39 : int | v39 in v40 :: (v39 / 0x11d) := (p3), globalState)| {
			globalState.f8 := !p0;
			var v41 := 0x389;
			v41 := v41;
		...
		var v46: array<int> := new int[14] [p3, p3, |v43|, if (v38) then f12 else p3, f13, |DC5(v44[p3 := f12], v38).cf10|, p3, -0x5, f13, p3 + 0x5a, f13, p3, f12 + f13, p3];
		r0 := v46;
		r1 := p2;
	}
		...
		match (v10) {
			case DC7(cf13) =>
				...
				if (p1) {
					...
					var v13: map<seq<int>, bool> := map[(v9[0x285 := f13])[f13 := |v6|] := p1];
					v13 := v13;
					r1 := p0 / p0;
					globalState.f8 := (if (p1) then |{v3, v3}| else f12) != f12;
					var v14: array<int> := new int[1];
					v14[453] := |fm0(globalState)|;
					v14[453] := f13;
				...
				var v19 := new C2(-f12 % f12, if (p1 in v18) then v18[p1] else "oblpvb", p0, f13, p1);
		}
...
class C8 {
	...
	method m14(globalState: GlobalState) returns (r0: bool, r1: array<int>, r2: D0) {
			...
			decreases 100 - i0
		{
			...
			var v3 := "ypssnf";
			var v4 := new C0(-v1 + |fm36(v1, v1, fm45(|v2|, globalState), globalState)|, v0, |v3| / v1, v1);
			...
			decreases 100 - i1
		{
			...
			if ((v1 == v1) && v0) {
				...
				var v30: array<string> := new string[21] [v25 + v25, v25, v25[v1 := v26] + "yyrd", (seq(0x2db, i4  => (v26))) + (seq(-0x113, i5  => ('v'))), v25 + v25, v25, v25, if (v0 in v27) then v27[v0] else seq(0x22, i6  => (v26)), v25 + v25, v25 + v25, v25, v25, fm36(|v28|, v1, fm45(v1, globalState), globalState), v25, v25, v25 + "mrlgffk", v25, fm36(v1, |v29|, v26, globalState), (seq(-0x1ab, i7  => (v26))) + v25, v25, v25];
				v30 := v30;
				var v31 := DC19(v0, v0, v0, v0, v29);
				v17[447] := v29 == v31.cf37;
				...
				v26, v17[447], globalState.f8, v26, v32[858] := 'l', if (v17 in v16[525]) then v16[525][v17] else v0, fm1(v1, v0, -|v35[v1]|, globalState), if (true) then 'i' else if (fm1(|v36|, if (v1 in v38) then v38[v1] else v0, v1, globalState)) then v26 else v26, v1;
				var v39: map<int, char> := map[v32[858] := v26];
				var v40 := new C7(v32[858], f23[|v39|]);
				v32[858] := v32[858];
			}
		...
		var v62 := DC3(v1, v0);
		var v63: seq<bool> := [v0];
		r2 := v62.(cf7 := if (true in v59) then v59[true] else |v63[v1 := v0]|);
	}
	method m15(globalState: GlobalState) returns (r0: multiset<bool>, r1: int, r2: int, r3: map<bool, bool>) {
		...
		v0[668], v5, globalState.f8, v6[329], v3 := v1, fm39(!v4, v4, globalState), (v3 % 390) <= 136, |(seq(0x125, i1  => ('f')))|, 0x2fd % (v3 * f23[|v7[v4 := v3]|]);
		var v8: map<int, D3> := map[v6[329] := DC8(v7)];
		var v9: set<bool> := {v4, v4, v4};
		var v10: set<int> := {|v9|, |f23|};
		var v11: map<int, int> := map[v6[329] := 548];
		var v12: map<int, map<int, int>> := map[v3 := v11];
		var v13 := DC10(if (|v10| in v8) then v8[|v10|] else DC9(v3, v12, v6[329], f23[v3]));
		v13 := v13;
		...
		var v16: map<int, multiset<int>> := map[v6[329] := v15];
		r1 := -(if (v6[329] in v15) then v15[v6[329]] else |v16| % (if (v6[329] in v11) then v11[v6[329]] else v3));
		...
		r3 := (if (v4) then v20 else fm42(v6[329], v6[329], globalState)) + map[false := v4];
	}
}
...
class C9 extends T1, T0, T2 {
	...
	function fm8(p0: int, p1: map<char, bool>, p2: set<int>, globalState: GlobalState): string {
		if (f11) then f20 else seq(912, i0  => ('x'))
	}
	function fm5(p0: int, globalState: GlobalState): D0 {
	...
	method m11(p0: int, p1: int, p2: int, p3: array<int>, globalState: GlobalState) returns (r0: array<bool>, r1: int, r2: set<int>) {
		...
		if (-(f12 % f22) != f22) {
			...
			var v9: array<bool> := new bool[24] [f11, f11, f11, f11, !f11 && f11, map[!f11 := f11] != v1, f11, f11, f11, f11, f11, f11, true, f11, true || !f11, f11, v8[f13], f11, f11 || f11, f11 || f11, false, f11, if (f13 in v2) then v2[f13] else f11, !(f11 <== f11)];
			r0, v7 := v9, v7;
			...
			r1, r1, f11, r1, f22 := (p0 - |fm20(f22, |f20|, |v1|, globalState)|) + ---f13, if ((p2 / p0) in v12) then v12[p2 / p0] else f12, (if (f11) then f11 else f11) <==> f11, p2 * p2, |((if (f12 in v18) then v18[f12] else v17) + (v17 + v17))|;
			var v19: set<int> := {f13};
			...
			var v24: set<array<bool>> := {v23[p0]};
			var v25: map<char, bool> := map[v16 := f11];
			var v26: map<seq<int>, string> := map[v5 := fm8(|v24|, v25, v19, globalState)];
			v20, v9[98], f22, f11, f22 := (v20 + ([v19, v19] + v20))[fm2(globalState) := (if (f13 in v22) then v22[f13] else {if (f11 in v4) then v4[f11] else 577, p1}) + v19], f11, |v26|, v8[|v0|], -p2;
			if (f11) {
				p3[669] := f22;
				p3[669] := |multiset{f11, v9[98]}| + f13;
				var v27: map<bool, string> := map[f11 := v0];
				var v28 := DC1(|{|v5|}|, f20, f13, v9[98]);
				...
				var v32: array<seq<array<int>>> := new seq<array<int>>[18] [v31, v31, v31 + v31, v31, v31 + v31, v31, v31, [v30, v30] + v31, v31 + v31, v31, v31, v31, v31, v31, v31 + v31, v31, v31[p1 := v30], ([v30])[p2 := v30]];
				v32[407] := v31 + [v30, v30];
				v32[407] := [v30, v30, v30, v30, v30];
			} else {
				globalState.f8 := v8[f13];
				var v33 := DC7(v5);
				var v34: map<D2, int> := map[v33 := f22];
				v34 := v34[v33 := p2];
				var v35 := new C2(if (f11) then |[f22]| else p2, "tqwuuwg", f22 % fm2(globalState), |v12|, !(p2 <= |f20|));
				v9[98] := f11;
				var v36 := DC17(false, p1);
				var v37 := DC1(v36.cf31, f20, v35.f26, v9[98]);
			...
			if (f11) {
				...
				globalState.f8 := !((v42 + ([f11, f11])[f22 := f11])[|v42| := f11] <= v42);
			...
			if (f11) {
				...
				r1 := -DC23(v50, v51, f11, f22).cf45 % (-p0 - p0);
				r1 := (p0 / p0) / f13;
				...
				v0 := f20;
				p3[253] := p2;
				f11, f22, p3[253] := f11, f22, -p1;
				var v53: array<bool> := new bool[22];
				v53[581] := true;
				var v54 := 'n';
				var v55: map<int, int> := map[f13 := p1];
				var v56 := DC1(f13, f20, |v55|, f11);
				v0, v0, v53[581] := (v0 + v0[-f22 := v54]) + (v0 + v56.cf2), if (false) then v0 else f20, 'i' in (seq(0x18a, i3  => (v54)));
			}
			
			var v57: set<bool> := {f11};
			var v58: seq<string> := [v0];
			globalState.f8, r1, f11, v0, v57 := f22 == fm2(globalState), f13, f12 == p2, v58[-f22] + (seq(0x2c9, i4  => ('o'))), v57;
			var v59 := 'p';
			var v60: array<array<int>> := new array<int>[2];
			var v61 := DC22(p0, v60);
			var v62 := DC27(!f11, v61, p0, f11);
			var v63: seq<bool> := [f11, f11];
			var v64: map<map<set<char>, bool>, bool> := map[map[{v59} := f11] := v62.cf49 || v63[|"l"|]];
			...
			var v75, v76 := m12(v0, v71, v74[f22], globalState);
		}
		
		for i5 := p1 to |v0| {
			var v77: seq<bool> := [f11, true, f11];
			var v78: seq<bool> := [v77[|multiset(v5)|], f11];
			var v79: array<bool> := new bool[11] [false, true || fm7(globalState), f11, f11, f11, f11, if (f11) then f11 else f11, if (v77[p0]) then f11 else f11, !f11, f11, f11 || !f11];
			v79[600] := f11;
			var v80: set<bool> := {!f11, f11};
			var v81: map<set<bool>, int> := map[v80 := p1];
			v79[600] := (if (v80 in v81) then v81[v80] else f22) < (f22 + |v1|);
			v79[600] := v79[600];
			v79[600] := (seq(733, i6  => (-205))) == v5;
			var v82 := DC2(|[f11]|, p3);
			var v83: seq<array<int>> := [p3, p3];
			var v84: array<array<int>> := new array<int>[29] [p3, p3, p3, p3, p3, p3, p3, p3, p3, p3, p3, p3, p3, p3, p3, p3, p3, p3, p3, v82.cf6, p3, p3, v83[f22], p3, p3, p3, p3, p3, p3];
			var v85 := DC22(f12, v84);
			match (v85.(cf41 := v84)) {
				case DC22(cf40, cf41) =>
					var v86 := new C8(v5 + v5);
					cf40 := v86.f23[if (v79[600]) then p2 else f12];
					var v87: map<string, array<bool>> := map[v0 := v79];
					var v88: seq<array<bool>> := [v79];
					v87 := v87[f20 + f20 := v88[f13]];
					var v89: multiset<map<int, bool>> := multiset{map[i5 := !v79[600]]};
					var v91: map<int, int> := map[|(set v90 : map<int, bool> | v90 in v89 :: (v90))| := f22];
					var v92 := DC3(f12, v79[600]);
					var v93 := 'c';
				...
				case DC23(cf42, cf43, cf44, cf45) =>
					v79[600] := cf44;
					var v96: array<int> := new int[2](i7 => i7 % |multiset{f20}|);
					v96, v0 := v96, v0;
					var v97 := DC4(v83);
					var v98 := DC6(v97);
				...
				case DC24(cf46) =>
					...
					var v106: map<bool, int> := map[v79[600] := p1];
					r0 := v104[|(v105.cf14 + v106)|];
					var v107 := 's';
		...
		if (v110[273]) {
			...
			match (fm53(v113 + v113, p0 % f12, globalState)) {
				case DC9(cf15, cf16, cf17, cf18) =>
					...
					v122[327] := v120;
					v122[327] := v120;
					globalState.f5 := seq(0x392, i8  => (cf17));
				case DC8(cf14) =>
					globalState.f8 := fm6(v110[273], 0x2dc, f22, globalState) <==> f11;
					var v123: array<map<int, bool>> := new map<int, bool>[9](i9 => v2);
					v123[460] := v3;
					v123[460] := v2;
					var v124: array<int> := new int[11](i10 => i10 - f13);
					v124 := if (v110[273]) then v124 else p3;
					...
					v125[102] := ((if (|v128| in v127) then v127[|v128|] else v4) + v4)[multiset{f11} < fm30(!v110[273], v110[273], 0x221, globalState) := p1 * fm2(globalState)];
				case DC10(cf19) =>
					var v129: array<string> := new string[1];
					v129[513] := f20;
					var v130: seq<bool> := [v110[273]];
					var v131: array<array<int>> := new array<int>[12];
					var v132 := DC22(|v130|, v131);
					var v133 := DC27(f11, v132, -258, f11);
					var v134 := DC28(v133);
					v129[513], r1 := f20, |map[v134 := f22]|;
					v110[273] := !(v110[273] && (p2 >= f13));
					...
					var v141 := DC1(-f22, v140[f12], f12, v110[273]);
			...
			var v143 := DC39(v110);
			v142[983] := v143.cf63;
			var v144: set<bool> := {v110[273], true};
			v110[273] := fm2(globalState) >= |(v144 + {v110[273], f11})|;
			f22 := (if (v110[273]) then p2 else f12) - (p2 * p2);
			var v145: seq<bool> := [v110[273]];
			v145, r1, f11, globalState.f8, v142[983] := v145 + [v110[273]], if (v110[273]) then |(v144 + v144)| else f22, !((v4 * v4) !! (fm30(v110[273], v110[273], f22, globalState))[f11 := 877]), v110[273] <== !true, if (v110[273]) then v142[983] else v142[983];
			m13(f12 + p1, v110[273], p3, globalState);
		}
		
		r0 := v110;
		r1 := p1 + (f12 + p2);
		var v147: set<int> := {f13};
		r2 := ((set v146 : int | (0x228 <= v146) && (v146 < 0x255) :: (v146 / |v0|)) * v147) + (set v148 : int | v148 in v5 :: (v148 % |map[true := !true]|));
	}
	method m12(p0: string, p1: D0, p2: array<array<bool>>, globalState: GlobalState) returns (r0: bool, r1: bool) {
		f11, f22 := !false, f22 * (f22 % f22);
		var v0: array<bool> := new bool[7];
		p2[646] := v0;
		var v1: seq<bool> := [false && f11];
		r0, r1, p2[646] := !f11, v1[f12 / f22], v0;
		...
		for i0 := f13 to -(-f13 - f13) {
			...
			if (false) {
				...
				v8, v13 := v8, fm54(if (v1[-|v7|]) then !false else f11, globalState);
				var v14: array<int> := new int[25];
				v14[531] := |DC1(0x17e, p0, f13, true).cf2| / f22;
				var v15: array<array<multiset<int>>> := new array<multiset<int>>[26];
				var v16: array<multiset<int>> := new multiset<int>[26](i1 => multiset{f12});
				v15[566] := v16;
				v14[340] := f12;
				v14[531], f11, v15[566], v8, v14[340] := f13, fm50(globalState) == v7, v16, v8, fm2(globalState);
			...
			if (f11) {
				...
				var v26: array<int> := new int[22] [fm2(globalState), |v3|, f12, f12, f12, |v21|, f12, f13, f13, f22, 0x3e0, i0, |(seq(-0x1c6, i2  => ('m')))|, i0, f12, f13, f22, |v24|, |v25|, |f20|, f12, |v1|];
				var v27: seq<array<int>> := [v26];
				var v28: map<int, seq<array<int>>> := map[0x0 := v27];
				v20[993] := if (f13 in v28) then v28[f13] else v27[-0x85 := v22];
				v20[993] := v27 + v27;
				...
				v7 := v7 * (set v37 : int | v37 in v24 :: (v37 * -|(seq(0xc9, i3  => ('u')))|));
				...
				f22 := |v39|;
				v0[64] := f11;
				v0[64] := if (f11) then true else if (f11) then f11 else f11;
				var v40: array<int> := new int[3];
				var v41: seq<array<int>> := [v40];
				var v42: array<array<int>> := new array<int>[9] [v40, v41[f22], v40, v40, v40, v40, v40, v40, v40];
				var v43 := DC27(f11, DC22(-DC15(|v7|, i0, f12).cf27, v42), f22 * f13, false);
				v43 := v43;
				globalState.f8 := v0[64];
				var v44, v45 := m0(globalState);
			}
			
			if ((|multiset(seq(271, i4  => (i0)))| * f13) > fm2(globalState)) {
				var v46: array<array<bool>> := new array<bool>[21];
				v46 := p2;
				f22 := f13;
				p2[646][863] := f11;
				p2[646][863] := true ==> true;
				f22 := f22;
				...
				p2[646], v48[694], v50, v49.f27 := p2[646], f20, [v49, v49, v49], -688;
		...
		for i5 := f12 to 0x9d {
			...
			var v54 := 'j';
			var v55: array<string> := new string[13] ["omgjypkmn", f20, "kpqqegm", fm36(f12, |v53|, DC26(v54).cf48, globalState), seq(853, i6  => (v54)), f20, p0, f20, f20, f20, seq(767, i7  => (v54)), p0, "rnwrb"];
		...
		match (v3) {
			case DC29(cf54) =>
				...
				if (false) {
					...
					var v13: array<bool> := new bool[13] [true, f11, p2[821] == f22, v8 !in (if (v7.f29 in v10) then v10[v7.f29] else v9), (if (f11 in v12) then v12[f11] else v11) >= multiset{v7.f28}, f11, p1, !p1 <== p1, f22 <= f12, 'd' in f20, f12 != f12, f13 <= f12, false];
					v13[979] := false;
					v13[979] := !p1;
					...
					p2[821] := fm2(globalState);
					var v17 := new C8(seq(305, i0  => (f12)));
					globalState.f8 := !p1;
					var v18 := "jlq";
					var v19: multiset<int> := multiset{0x1c9};
					var v20: multiset<int> := multiset{if (f22 in v19) then v19[f22] else 0x2e1};
					var v21 := DC31(f11);
					var v22: map<D11, int> := map[v21 := p2[821]];
					p2[821], f22, v18, f11, p2[821] := p2[821], p2[821], "uk", f11, (if (|v22| in v20) then v20[|v22|] else |v18|) / (f13 - p0);
					f22 := f13 % (p0 % p0);
				}
				
				var v23: map<bool, int> := map[!f11 := p2[821]];
				globalState.f8 := p1 in v23;
				var v24: map<multiset<bool>, bool> := map[v4 := p1];
				var v25: multiset<int> := multiset{p2[821], |v24|, f12, if (p1) then f13 else |f20|};
				f22 := if (fm2(globalState) in v25) then v25[fm2(globalState)] else 0x26 / p0;
		}
		
		for i1 := f13 to f13 {
			var v26: multiset<int> := multiset{p2[821], p2[821]};
			var v27 := 'j';
			var v28: map<char, bool> := map[v27 := f11];
			var v29: map<bool, int> := map[p1 := i1];
			var v30: seq<int> := [p2[821], f12];
			var v31: map<bool, seq<int>> := map[f11 := seq(569, i2  => (p2[821]))];
			var v32: map<int, int> := map[f13 := f12];
			var v33: multiset<map<int, int>> := multiset{v32, v32};
			var v34: map<int, bool> := map[p2[821] := p1];
			var v35: set<bool> := {p1};
			var v36: array<bool> := new bool[23] [f11, false, v26 > multiset{p2[821], f12}, f11, f11, if (v27 in v28) then v28[v27] else f11, |v29| > p2[821], v30 != v30, true, |v0| !in (if (f11 in v31) then v31[f11] else v30), p1, f11, p1 ==> f11, f11, p1, true, fm1(p0, f11, f13, globalState), !p1, v33 >= v33, if (|map[p1 := p2[821]]| in v34) then v34[|map[p1 := p2[821]]|] else f11, v35 != v35, !(v32 != v32[0x140 := |f20|]), p1];
			var v37: array<array<int>> := new array<int>[23];
			var v38 := DC23(v37, v0, p1, f12);
			v36[805] := true <== v38.cf44;
			v36[805] := p1;
			p2[821] := f22;
			v29 := v29[v36[805] := -f13 - f13];
			p2[821], f22, globalState.f8 := |f20| + (308 - |fm8(-|f20|, map[v27 := p1], v1, globalState)|), (|f20| * f13) - fm2(globalState), f11;
		}
		var v39: array<string> := new string[16];
		var v40 := DC14(v39);
		var v41: multiset<D5> := multiset{v40};
		var v42: set<multiset<D5>> := {multiset{v40, v40}, v41};
		var v43: seq<int> := [-p2[821]];
		var v44: multiset<string> := multiset{f20};
		var v45: set<bool> := {f11};
		var v46: T0 := new C4(531, p1);
		var v47: map<T0, int> := map[v46 := p2[821]];
		var v48: array<int> := new int[20] [p0, -|v42|, f13 / |"ggj"|, -fm2(globalState), |v43| - (if (f20 in v44) then v44[f20] else f13), f22, -p0, f13 / |v45|, f13, f13, f13, -p2[821], f13 / p0, f13, f12, |f20|, f22, p0, if (v46 in v47) then v47[v46] else |"uidytrhx"|, p0];
		v48 := v48;
		if (f11) {
			p2[821] := f22;
			var v49 := 'i';
			...
			var v65: array<map<multiset<char>, int>> := new map<multiset<char>, int>[2] [v63, map[v64 := p2[821]] + v63];
			v65[75] := v63;
			...
			v61, v65[75], v62, f22, v46.f11 := (if (v46.f11) then DC40(map[v0 := v59]) else v66).cf64, v63, v62, if (|v70| in v69) then v69[|v70|] else |f20|, v46.f11 !in (if (v46.f11) then v70 else map[f11 := v46.f11]);
			var v71: multiset<int> := multiset{p0 / f13, f13};
			v71 := (multiset{p2[821]} - v71) + multiset([525, f13, p2[821], -f12]);
			var v72: array<map<int, seq<int>>> := new map<int, seq<int>>[9];
			var v73: seq<seq<int>> := [[f22]];
			v72[574] := map[f12 := v73[p2[821]]];
			var v74: map<int, seq<int>> := map[f13 := v43 + [p0]];
			f22, v72[574], v46.f11, f22, v62 := |map[f13 := v46.f11 ==> v46.f11]|, v74, p1, p0 * 202, v62;
			var v75 := "sdeejae";
			v75 := f20;
		}
		
		var v76: array<seq<bool>> := new seq<bool>[25](i3 => [v0[|multiset{'y'}|], !DC41(f11).cf65]);
		v76 := v76;
	}
}
...
class C10 extends T0 {
	...
	function fm5(p0: int, globalState: GlobalState): D0 {
		DC1(|[|(seq(0x169, i0  => ('m')))|, -0x26]|, "ecqjml" + "fojk", |("omya" + "qfg")|, true)
	}
	function fm6(p0: bool, p1: int, p2: int, globalState: GlobalState): bool {
		f11
	}
	function fm16(p0: int, p1: int, globalState: GlobalState): bool {
		(DC3(|multiset(DC7(seq(0x145, i0  => (|[f11]|))).cf13)|, f11).cf8 <== !f11) !in map[f11 := -|map[f11 := f11]|]
	}
	method m9(p0: int, p1: array<int>, globalState: GlobalState) {
			...
			decreases 100 - i1
		{
			...
			var v4 := DC7(v3[p0 := p0]);
			var v5: map<string, bool> := map[seq(-0x27e, i2  => ('r')) := v0[0]];
			var v6: map<D2, int> := map[v4.(cf13 := fm0(globalState)) := |v5|];
			var v7 := "ptqylp";
			v6 := v6[v4.(cf13 := seq(0x8e, i3  => (p0))) := -|v7| * p0];
			var v8: map<set<int>, int> := map[v1 := |v2|];
			v8 := v8[v1 := p0];
		}
		var v9: map<int, bool> := map[p0 := v0[0]];
		var v10: multiset<int> := multiset{fm2(globalState), -0x309};
		var v11: seq<map<int, bool>> := [map[--(if (p0 in v10) then v10[p0] else p0) := fm6(v0[0], p0, p0, globalState)], v9, (map[|v1| := f11])[p0 := false]];
		v9 := v11[p0 % -618];
		var v12: seq<bool> := [f11];
		var v13: map<bool, int> := map[fm6(v0[0], |v12|, p0, globalState) := |v1|];
		if (v12[|(map[!v0[0] := p0] + v13)|]) {
			var v14 := 0x2d4;
			var v15 := 'n';
			...
			if (f11) {
				...
				var v20: array<D2> := new D2[27] [v19, fm17(globalState).(cf13 := seq(0x3b0, i4  => (-p0))), v19, v19, v19, v19, v19, v19, v19, DC7(seq(0x324, i5  => (|map[v19 := DC3(|"c"|, v0[0])]|))), v19, v19, v19, v19, v19, v19, v19, DC7([p0, v14, v14]), v19, v19, DC7(v18), DC7(v18), v19, v19, v19, v19.(cf13 := v18), v19];
				var v21, v22, v23 := m10(f11, v20, p0, globalState);
				v14 := -fm2(globalState);
				f11 := false !in v12;
				globalState.f8 := !((f11 || f11) || f11);
			} else {
				var v24: multiset<bool> := multiset{f11, f11, !!v0[0]};
				var v25: seq<multiset<bool>> := [v24];
				v14, v25 := -v14, v25 + v25;
				var v26: map<bool, char> := map[v0[0] := v15];
				var v27: seq<int> := [-|v26|];
				var v28 := new C8(v27);
				var v29 := "nvdlfmlk";
				var v30: seq<string> := [v29, v29 + "sslfvi"];
				v30 := [v29];
				p1[59] := fm2(globalState);
				p1[59] := (|v29| * p0) / -v14;
			...
			var v33: map<array<int>, array<bool>> := map[v32[p0] := v0];
			v33 := v33[v32[0x396] := v0];
			var v34 := 0x2d5;
			var v35: map<int, int> := map[v34 := p0];
			var v36 := DC31(v0[0]);
			var v37: seq<D11> := [v36];
			v34 := -(((if (|v37| in v35) then v35[|v37|] else -p0) * v34) % |v12|);
			var v38 := 'r';
			v38 := v38;
			v34 := -v34;
		...
		if (if (if (|v13| in v9) then v9[|v13|] else v0[0]) then v12 < v12 else f11) {
			var v40 := new C6();
			var v41 := "hgnjjve";
			var v42: map<int, int> := map[fm2(globalState) * |v41| := p0 / p0];
			v42 := v42[p0 + -p0 := p0 - p0];
			var v43: set<map<int, int>> := {v42, v42, v42};
			v43 := {v42, v42, v42} + v43;
			var v44 := new C4(0x1c9 / p0, fm1(-p0, f11, p0, globalState));
			var v45: seq<int> := [v44.f25];
			var v46 := DC7(v45);
			var v47: map<bool, D2> := map[v0[0] := v46];
			v47 := v47[f11 := v46];
		} else {
			v0[0] := !f11;
			var v48: map<map<int, bool>, seq<bool>> := map[v9 := v12];
			var v50: map<array<bool>, map<map<int, bool>, seq<bool>>> := map[v0 := v48[map v49 : int | (-0x33e <= v49) && (v49 < 0x38f) :: (v49 / p0) := (v0[0]) := [v0[0], v0[0]]]];
			var v53: map<map<int, bool>, int> := map[v9 := p0];
			var v55: seq<map<map<int, bool>, seq<bool>>> := [v48, v48, v48, map v54 : map<int, bool> | v54 in fm56(p0, v0[0], f11, v0[0], globalState) :: (v54) := (v12)];
			var v56: array<map<map<int, bool>, seq<bool>>> := new map<map<int, bool>, seq<bool>>[25] [v48, v48 + (if (v0 in v50) then v50[v0] else map[v9 := [f11]]), map[map v51 : int | v51 in v10 :: (v51 - p0) := (f11) := [v0[0]]], v48, v48, v48, map[v9 := v12], (map v52 : map<int, bool> | v52 in v53 :: (v52) := (v12)) + v48, v48, v48, v48, map[v9 := v12], DC43(map[v9 := v12]).cf67 + v48, v48, v55[p0], v48, v48, map[map[fm2(globalState) := v0[0]] := v12], v48, v48, if (v0[0]) then v48 else v48, v48, v48, v48, v48];
			v56[93] := fm57(-0x29f, f11, globalState);
			var v57 := -0xb1;
			var v58 := "ehm";
			v56[93], f11, v57, globalState.f8 := v48, v58 < v58, p0 + v57, f11;
			globalState.f8 := true;
			v57 := p0;
			p1[138] := p0;
			p1[138] := -v57;
	...
	method m10(p0: bool, p1: array<D2>, p2: int, globalState: GlobalState) returns (r0: map<int, string>, r1: int, r2: map<array<bool>, bool>) {
			...
			decreases 100 - i0
		{
			...
			var v6 := DC9(p2, v5, -|(seq(0x1de, i1  => (p2)))|, -|v3|);
		...
		var v20 := "cwra";
		var v21: T0 := new C2(p2, v20, p2, p2 / p2, p0);
		v21 := v21;
...
class C11 extends T1 {
	...
	method m8(globalState: GlobalState) returns (r0: seq<int>, r1: bool) {
		...
		var v10: array<int> := new int[27] [f12, f12, f12, f13, |v1|, |(seq(825, i0  => ('l')))|, f13, -f13, |multiset{f12}|, f13, if (v0) then f13 else (fm15(v0, f13, v2, v0, globalState)).cf1, |(v3 + v3)|, f13 * |v7|, if (true) then --f13 else |"fj"|, |(if (v0) then map[v0 := f12] else map[v0 := f12])|, f13 * f13, f12, 0xb6, f12, f12, f12, f13 * f13, f12, -69, |v9|, 230, f12 + f12];
		v10[746] := f12 % f12;
		v10[746] := f12;
		var v11 := "scvtajpa";
		v10[746] := |v11|;
		var v12: array<string> := new seq<char>[22](i1 => seq(0x31f, i2  => (v8)));
		v12[145] := v11;
		v12[271] := v11;
		var v13: multiset<bool> := multiset{v0};
		var v14: multiset<int> := multiset{v10[746], v10[746]};
		v10[746], v10[746], v12[145], v0, v12[271] := f12, match DC1(-|v13[v0 := f12]|, v11, f13, v0) {
			case DC1(cf1, cf2, cf3, cf4) => cf3 - -932
			case DC2(cf5, cf6) => v10[746]
			...
			decreases 100 - i3
		{
			...
			i3 := i3 + 1;
			var v15: map<char, bool> := map[v8 := fm1(v10[746], v0, v10[746], globalState)];
			var v16: map<int, int> := map[v10[746] := v10[746]];
			var v17: set<int> := {|v16|, v10[746]};
			var v18: set<int> := {|fm8(f12, v15, v17, globalState)|};
			var v19: seq<multiset<int>> := [v14];
			var v20: array<multiset<int>> := new multiset<int>[10] [v14, v14, multiset{|v18|, v10[746], f12, |v2|}, v14, v19[f12], v14, v14, v14[f12 := f13], v14, v19[v10[746]]];
			var v21: map<bool, array<multiset<int>>> := map[v0 := v20];
			v21 := v21[v0 := v20];
			v11 := (v12[145] + v11[0x24d := v8]) + v11;
			var v22 := new C3(f13, f13);
			v9 := v9[v8 := |(map[f12 := f13] + v16)|];
		}
		for i4 := if (v0) then f12 else f13 to f12 {
			var v23: map<int, seq<bool>> := map[271 := v2];
			var v24: array<seq<bool>> := new seq<bool>[14] [v2, v2, [v0], v2, [fm7(globalState), v0], v2 + v2, v2, v2, v2, v2[i4 := fm1(0x90, v0, |"nwqyqwpb"|, globalState)] + v2, v2, ((if (i4 in v23) then v23[i4] else [v0, false])[-f12 := v0] + v2)[f12 := v0], v2, v2];
			var v25: map<int, int> := map[|v12[145][f12 := v8]| := |[i4, v10[746], v10[746]]|];
			var v26: seq<array<int>> := [v4, v10, v4];
			v24, v10[746], v10[746] := v24, if (v0) then |v25| else f12, |(v26 + ([v10] + v26))|;
			var v27 := new C2(f13, v12[145], i4 + |[v0, v0]|, f12 - f13, v0);
			v0 := v11 !in (map v28 : string | v28 in (map v29 : string | v29 in [v12[145], "arocdqm", v12[145]] :: (v29) := (|v25|)) :: (v28) := ({v27.f26}));
			var v30 := DC26(v8);
			var v31: array<char> := new char[7] [v8, v8, v8, v8, v30.cf48, v8, 'q'];
			var v32: multiset<array<int>> := multiset{v4, v10};
			v10[746], v31 := v10[746], if (v32 <= v32) then v31 else v31;
		}
		r1 := v0;
		var v33 := DC19(v0, v0, v0, true, v2);
		r0 := match v33 {
			case DC17(cf30, cf31) => if (v0) then [f12] else [f12, |v5|, v10[746], -cf31, cf31]
			case DC18(cf32) => if (!DC12(cf32, f13, v10[746]).cf21) then seq(-0xc4, i5  => (f13)) else [-548, f12]
			case DC19(cf33, cf34, cf35, cf36, cf37) => [v10[746], |v11|]
			case DC16(cf29) => [|v5|, f12]
			case DC20(cf38) => (seq(0x33a, i6  => (v10[746]))) + [|[f13]|, f13]
		};
		r1 := v0;
	}
}
...
class C12 extends T0, T1 {
	...
	function fm5(p0: int, globalState: GlobalState): D0 {
		match DC3(|[f19, f19, f13, |"gw"|, f13]|, f11) {
			case DC1(cf1, cf2, cf3, cf4) => DC1(|(map v0 : int | (772 <= v0) && (v0 < 0x5f) :: (v0 + cf3) := ([f11]))|, ("xyqxy")[f19 := 'x'], f12, false)
	...
	method m7(p0: array<int>, p1: int, globalState: GlobalState) returns (r0: int, r1: bool) {
		...
		p0[893] := v1.f25;
		p0[893] := --(fm2(globalState) / f13);
		if (f11) {
			globalState.f8 := !fm1(p1, true, f12 + p0[893], globalState);
			var v2: map<bool, bool> := map[f11 := f11];
			var v3: set<bool> := {f11, false, f11};
			var v4: map<int, set<bool>> := map[17 := v3];
			globalState.f8 := if (!fm6(if (true in v2) then v2[true] else fm1(p0[893], f11, p0[893], globalState), -v1.f25, |v4|, globalState) || true) then if (f11) then f11 else f11 else !(-p0[893] < f12);
			...
			if (if (f11 <== !f11) then f11 else true) {
				...
				var v10 := new C1(-(v1.f25 % f12));
				globalState.f8 := !f11;
				globalState.f8 := !f11;
				var v11: map<int, array<int>> := map[v1.f25 := v5];
				v11 := v11 + map[v10.fm34(globalState) := v5];
			} else {
				r0 := f19 + (if (fm1(v1.f25, f11, -204, globalState)) then v0[0x1e1] else f19);
				p0[893] := 13 + f19;
				var v12 := "nvld";
				var v13 := 'j';
				var v14: T2 := new C2(fm2(globalState), (v12 + v12)[v1.f25 := v13], 0x1a6, -f12 - |v12|, !f11);
				v14 := v14;
				var v15: map<bool, int> := map[f11 := -p0[893]];
				var v16: map<int, int> := map[p0[893] := (if (v6.fm27(v1.f25, seq(0x357, i1  => (v13)), f11, globalState) in v15) then v15[v6.fm27(v1.f25, seq(0x357, i1  => (v13)), f11, globalState)] else v1.f25) % |v15|];
			...
			var v22 := DC5(v19[v1.f25 := |v21|] * v19, f11);
			v22 := v22;
			...
			var v32: map<char, bool> := map[v20 := v29[|(set v31 : int | v31 in v30 :: (v31 * |[false]|))|]];
			var v33: set<int> := {p0[893]};
			var v34: array<string> := new seq<char>[28] [(v21 + v21)[-fm2(globalState) := v24.cf48], v21, v21, if (f11) then v21[v26.cf5 := v20] else "llmohotq", v27[-v1.f25], (seq(0x15, i2  => ('d')))[fm2(globalState) := 'o'], v21, (v21[if (f11 in v28) then v28[f11] else f12 := v20] + "vx")[f12 := 'q'], seq(619, i3  => (v20)), seq(-35, i4  => (v20)), v21, v21, v21, v21, v21, v21, v21, v21, v21, v21, v21, v21, v21 + (seq(153, i5  => (v20))), v21, (fm8(v1.f25, v32, v33, globalState))[-v1.f25 := v20], "tc", if (f11) then v21 else v21, v21];
			v34[772] := v21;
			var v35: map<int, int> := map[v1.f25 := v1.f25];
			var v36: map<int, map<int, int>> := map[375 := v35];
			var v37 := DC9(f19, v36, p1, f13);
			v23, v34[772], v20, f11, r0 := v23, v21, v20, (v21 + v21) == (if (true) then ("nxs")[v1.f25 := 't'] else v21), (|{v37}| + v1.f25) - p1;
			p0[893] := -f19;
			p0[893] := (v0[f13] % f13) * f19;
		}
		
		var v38: array<bool> := new bool[12];
		v38[886] := f11;
		var v39: array<int> := new int[25](i6 => i6 / f19);
		v38[886], v39 := f11, v39;
		var v40 := DC35(p1 / fm2(globalState));
		match (v40) {
			case DC33(cf58) =>
				v39 := v39;
				var v41: C10 := new C10(f11);
				v41 := if (v38[886]) then v41 else v41;
				var v42: C0 := new C0(p0[893], f11, v1.f25, p1);
				var v43: map<bool, C0> := map[f11 := v42];
				var v44: seq<bool> := [v42.f29];
				var v45: multiset<int> := multiset{|map[v43 := v44[v1.f25]]|};
				var v46 := "mdwgotbd";
				var v47 := DC1(p1, v46, v1.f25, v42.f29);
				var v48: map<int, bool> := map[|{v47.cf2, v46}| := v38[886]];
				v38[886] := (|v45| >= |v48|) <== v42.f29;
				v46 := "xjd";
			case DC34(cf59) =>
				var v49: multiset<bool> := multiset{p0[893] < |v0|, false};
				p0[893], p0[893] := if (f11 in v49) then v49[f11] else p1, p1;
				...
				v52 := v52;
				p0[893] := (v1.f25 - f12) / v1.f25;
				var v53 := 'j';
				var v54: seq<char> := [v53];
				var v55: map<multiset<bool>, int> := map[v49 := 415];
				var v57: seq<array<int>> := [p0, cf59];
				var v58: map<int, int> := map[|v57| := p1];
				p0[893], v54, v55, v0, r0 := f13, v54, map v56 : multiset<bool> | v56 in [v49, multiset{v38[886]}] :: (v56) := (v1.f25), v0, (f13 + v1.f25) * ((if (v1.f25 in v58) then v58[v1.f25] else |v58|) - v1.f25);
			case DC35(cf60) =>
				v39 := v39;
				var v59, v60 := m0(globalState);
				p0[893] := v1.f25 % f13;
				var v61: seq<array<int>> := [p0, v39];
				var v62: array<array<int>> := new array<int>[8] [v61[|fm50(globalState)|], v39, p0, p0, v39, p0, v39, p0];
				v62[203] := p0;
				var v63 := "hate";
				var v64: set<int> := {v1.f25, f19, p1, p1};
				var v66 := 'j';
				var v67: map<char, bool> := map[v66 := f11];
				var v68: map<int, bool> := map[v1.f25 := v38[886]];
				v62[203], p0[893], v63, p0[893] := p0, v1.f25, fm8(f12 / |(set v65 : int | v65 in v64 :: (v65 % |[false, true]|))|, v67 + map[v66 := f11], {p1, p0[893], f19, |v68|, -|v68|}, globalState), fm2(globalState);
			case DC32(cf57) =>
				if (false) {
					v38 := v38;
					var v69 := "hsy";
					...
					v70 := v70[v1.f25 := if (v38[886]) then fm8(v1.f25, v72, v73, globalState) else v69];
					var v74 := new C6();
					var v75: array<array<int>> := new array<int>[4];
					var v76 := DC12(f11, -0xfb, p0[893]);
					var v77: seq<bool> := [f11];
					var v78 := DC23(v75, v77, v38[886], p0[893]);
					v75 := if (v76.cf21) then v78.cf42 else v75;
					var v79: map<string, bool> := map[v69 := v38[886]];
					f11 := v69 in v79;
				} else {
					v38[886] := v38[886];
					var v80 := 'a';
					var v81: map<bool, bool> := map[v38[886] := false];
					var v82: set<map<bool, bool>> := {v81};
					var v83: map<int, int> := map[|(seq(472, i7  => (v80)))| := fm2(globalState)];
					var v84: set<int> := {-|v82| / v1.f25, v1.f25, fm2(globalState), |(if (!f11) then v83 else v83)|, f12 + v1.f25};
					var v85: map<bool, char> := map[v38[886] := v80];
					v80, v84 := if (v38[886] in v85) then v85[v38[886]] else v80, {f12, v1.f25, p1, p0[893]} + fm50(globalState);
					var v86: map<int, bool> := map[|v81| := f11];
					v86 := v86[-393 % v1.f25 := f11];
					var v87: set<array<int>> := {if (v38[886]) then p0 else p0, v39, p0, p0};
					v87 := v87;
					r1 := !!fm13(fm1(v1.f25, v38[886], f13, globalState), globalState);
				}
				
				if (!f11) {
					var v88 := new C6();
					var v89: map<bool, bool> := map[f11 := v38[886]];
					var v90 := 'd';
					var v91: seq<char> := ['g', fm45(|v89|, globalState), fm45(v1.f25, globalState), v90, v90];
					v91 := seq(0x118, i8  => (v90));
					...
					var v99 := DC44(v98);
					v38[886] := v98 >= v99.cf68;
				} else {
					var v100 := 't';
					var v101: map<seq<char>, int> := map[[v100] := p0[893]];
					var v103: map<seq<char>, bool> := map[[v100] := f11];
					r1 := v101 == ((map v102 : seq<char> | v102 in v103 :: (v102) := (v1.f25)) + v101);
					var v104: map<bool, int> := map[v38[886] := p1];
					...
					r0, v109 := v1.f25, v109;
					v109[828] := v38;
					var v110: multiset<seq<int>> := multiset{seq(0x351, i9  => (v1.f25))};
					f11, f11, globalState.f8, p0[893], v109[828] := f11, f11, v110 > v110, if (v108[p0[893]]) then p0[893] else v1.f25, if (if (v38[886]) then f11 else v38[886]) then v38 else v38;
				}
				
				var v111: map<int, int> := map[v1.f25 := 0x2cf];
				var v112: map<map<int, int>, int> := map[v111 := v0[v1.f25]];
				var v113: seq<map<int, int>> := [v111, v111];
				v112 := map[v113[f13] := p0[893]] + (v112 + fm59(f13, v1.f25, f11, v1.f25, globalState));
				var v114: map<set<bool>, bool> := map[{f11} := true];
				var v115: set<bool> := {v38[886]};
				v114 := v114[v115 * {v38[886]} := v38[886]];
		}
		
		globalState.f8 := v38[886];
		r0 := -(f19 + p0[893]);
		var v116 := "iowpq";
		r1 := v116 != "fvs";
	}
	...
	method m6(globalState: GlobalState) returns (r0: bool, r1: map<bool, bool>, r2: int, r3: seq<bool>) {
		...
		match (v7) {
			case DC1(cf1, cf2, cf3, cf4) =>
				...
				v6 := if ((cf1 / cf3) in v9) then v9[cf1 / cf3] else !v6;
				if (cf4) {
					var v10 := 'r';
					...
					var v15: array<int> := new int[4](i1 => i1 % |map[|"jf"| := v0]|);
					var v16 := DC2(|v14|, v15);
			...
			case DC2(cf5, cf6) =>
				...
				cf6[96] := 0x324;
				v44, cf6, globalState.f8, cf6[96], v6 := v44, if (v6) then cf6 else if (true) then cf6 else cf6, v6, (fm2(globalState) * v0) + v0, v6;
				v6 := v6;
			case DC3(cf7, cf8) =>
				...
				v50[799] := v48[v46 := v49[-0x316 := true]];
				...
				v50[799], globalState.f8, v6, cf7, r1 := (if (v6) then v48 else map[seq(0x39, i3  => (cf7)) := fm47(true, cf8, v51, v0, globalState)])[v46 := fm47(true, cf8, map[cf8 := |v52|], cf7, globalState)], v6 in (v45 + v45), v6, (v0 % cf7) * (if (v53 in v54) then v54[v53] else cf7), v55 + (v55 + v55);
				if (v6) {
					var v56 := new C1(v0);
					var v57: array<bool> := new bool[19];
					var v58: map<array<bool>, bool> := map[v57 := v6];
					var v59: map<seq<int>, bool> := map[v46 := v58 == v58];
					v59 := v59[([v56.f27, v0])[|{v56.f27}| := |(seq(0x385, i4  => (v53)))|] := v45 == v45];
					globalState.f8 := v45[cf7];
					v6 := cf8;
					v53 := 'n';
				} else {
					r2 := cf7;
					...
					v62 := v62[v61 + (map v63 : int | v63 in v49 :: (v63 - v46[v0]) := (v53)) := -v0];
					var v64 := "nivaav";
				...
				var v72 := DC40(v71);
				v69[562] := v72;
				v69[562] := v72.(cf64 := fm55(0x1e8, cf7, v0, globalState));
			case DC0(cf0) =>
				var v73: multiset<int> := multiset{v0 + v0, if (!!v6) then v0 else |map[v0 := v0]|, -v0};
				var v74: multiset<bool> := multiset{v6, true};
				v73 := if (v74 == v74[v6 := v0]) then v73 - v73 else v73 * v73[v0 := 84];
				var v75, v76 := m0(globalState);
				globalState.f8 := (fm2(globalState) - v0) != (-|"ja"| * v0);
				var v77: map<bool, bool> := map[v76 := v6];
				var v78: array<int> := new int[26] [v0, v0, v0, v0, v0, v0, |v77|, v0, v0, |v77|, 0x2ce, v0, v0, 0x149, 895, v0, 0x339, -284, v0, v0, v0, fm2(globalState), v0, |(seq(0x3d9, i6  => ('y')))|, 0x1d4, 0x73];
				...
				r2, r0, globalState.f8, v76, v76 := |v84| - |(seq(0x221, i7  => ('s')))|, (if (v6 in v74) then v74[v6] else -v0) > v0, v6, !(if (v85 in v86) then v86[v85] else !(v88 !! multiset{v87})), v76;
		}
		...
		match (v102) {
			case DC22(cf40, cf41) =>
				...
				if (!(|map[v0 := fm1(v0, v6, v0, globalState)]| == cf40) <== (if (v6) then v6 else v6)) {
					...
					v108[18] := v101 + ([v6])[v0 := v6];
					var v109 := "oevt";
					var v110: map<bool, bool> := map[v6 := v6];
					var v111: seq<map<bool, bool>> := [v110, v110];
					var v112: seq<int> := [|(v109 + v109)|, 0x31e, |v111[v0]|];
					globalState.f8, globalState.f5, globalState.f8, v108[18] := (cf40 > v0) || v6, v112, (if (v6) then v6 else v6) <== v6, v101;
					v6 := v6;
					r2 := cf40;
					...
					var v115: seq<multiset<bool>> := [v114, v114[true := cf40], multiset(v101)];
					v99[736] := |v115|;
				} else {
					v99[464] := v0;
					v99[549] := cf40;
					...
					v99[464], v99[549], v117[959], v121 := (-cf40 / cf40) - -cf40, v0, v6, v121;
					var v122: map<int, int> := map[v0 / cf40 := v0];
					v122 := v122[118 := -368 / v0];
					globalState.f8 := v117[959] && false;
					var v123: multiset<int> := multiset{cf40, |v122|};
					var v124: multiset<multiset<int>> := multiset{v123};
					var v125 := "qm";
					var v126 := new C2(|v124|, v125, cf40, v99[464], v117[959]);
					var v127: multiset<bool> := multiset{v99[464] > v126.f26};
					v127 := v127;
				}
				
				v100[794] := v99;
				v100[794] := if (!v6) then v99 else v99;
				var v128 := "fqbfsoug";
			...
			case DC23(cf42, cf43, cf44, cf45) =>
				...
				var v132: seq<int> := [cf45, v130[v0], cf45, |v101|, if (v0 in v131) then v131[v0] else v0];
				var v133 := new C2(v0 + cf45, v129, |v130|, v130[cf45] % cf45, cf44);
				...
				if (v6) {
					...
					var v142: array<D0> := new D0[27] [DC1(v130[370], "oabhtcmf", |v135|, cf44), v136.(cf2 := v129, cf3 := -|v129|), v133.fm5(cf45, globalState), v136, v136, v136, v136.(cf1 := v0, cf2 := fm36(fm2(globalState), fm2(globalState), v137, globalState)), v136, v136, v136, if (v6) then v136.(cf4 := !cf44) else DC1(v133.f26, v129, |v138|, v6), DC1(|fm0(globalState)|, v129, 774, cf44), v136, v136, DC1(|v135|, "nfcbcyxby", 0x85, v6), DC1(cf45, v129, |map[cf44 := v6]|, v6), v136, v136, v136, v136, v136, fm15(cf44, v133.f26, [v6], v6, globalState), v136, v133.fm5(v0, globalState), DC1(|v129|, v129, -v133.f26, v6), DC1(|v141|, ("du")[v133.f26 := v137], cf45, v6), fm15(v6, v133.f26, cf43, v6, globalState)];
					v142[214] := DC1(|v129|, "e", cf45, v6);
					v142[214] := v136;
					var v143: multiset<string> := multiset{v129};
					globalState.f8 := !(v143 !! (v143 + v143));
					var v144: set<bool> := {multiset{v0, v0, v0} !! v131};
					r0, v131, r1, v144 := v101[-v0 * v133.f26], v131, map[v144 !! v144 := cf44], v144 * (v144 + v144);
					v99 := v99;
				} else {
			...
			case DC21(cf39) =>
				...
				var v155 := new C0(v0, !(v0 == |v153|), -v0, v154[0x355]);
				r0 := v155.f29;
			...
			case DC24(cf46) =>
				...
				var v163: map<array<bool>, multiset<seq<D11>>> := map[v157 := v161[v162 := |v101|]];
				...
				v99[325] := |v172[v171 := v0]|;
				var v174 := DC29(set v173 : int | (0x3c8 <= v173) && (v173 < 0x191) :: (v173 / v0));
				v157[806] := v6;
				var v175: set<int> := {-0x26d, v0};
				v99[325], v174, v6, v157[806], r2 := (v0 / v0) * v0, DC29(v175), v6, false, v0 - v0;
		}
		...
		var v180 := new C2(297 / v0, v176, -|v179|, v0, v6);
		...
		r0 := -|(if (fm1(|(set v184 : string | v184 in v183 :: (v184))|, v6, -|v185|, globalState)) then v187[v180.f26] else v176)[v180.f26 := v186]| !in ({v180.f26} + v188);
		var v189: map<bool, bool> := map[v6 := v6];
		r1 := v189 + v189;
		r2 := v180.f26;
		r3 := (v101 + v101[v0 := false]) + v101;
	}
}
...
class C14 extends T0 {
	...
	function fm5(p0: int, globalState: GlobalState): D0 {
		DC1(|{f11}| + |map[f11 := 0x9e]|, "wxliobhom" + (seq(0x3b9, i0  => ('q'))), |[f11, f11, DC3(|"gbesth"|, f11).cf8, f11, false]| + --0x9d, false)
	}
	function fm6(p0: bool, p1: int, p2: int, globalState: GlobalState): bool {
		(|[false, true]| + |(seq(0x363, i0  => ('d')))|) > |("ehqwd" + "viqa")|
	}
	...
	method m5(p0: bool, p1: set<int>, p2: seq<array<int>>, globalState: GlobalState) returns (r0: int, r1: bool) {
		...
		for i0 := v0 to DC2(-v0, v4).cf5 {
			...
			match (DC33(v0).(cf58 := i0).(cf58 := |[v4]|).(cf58 := 0xe2)) {
				...
				case DC32(cf57) =>
					...
					var v22: map<D1, string> := map[DC5(v8, f11) := seq(0x60, i2  => (v5))];
					...
					globalState.f8, v22, f11, v23[547], v24 := f11, map[v25 := "whowf" + v2], v26 >= (v26 + v26), if (f11) then v5 else v5, v24;
					v2 := v2;
					var v27: map<char, bool> := map[v23[547] := f11];
					v2 := v21.fm8(v9.f27, v27, p1, globalState) + (seq(699, i5  => (v5)));
			}
		...
		v28[399] := if (f11 in v30) then v30[f11] else v29;
		v28[399] := v29;
			...
			decreases 100 - i7
		{
			...
			globalState.f8 := f11;
			var v34 := new C10(p0);
			var v35: multiset<char> := multiset{v28[399], v28[399]};
			var v36: map<multiset<char>, bool> := map[v35 := p0];
			var v37 := DC50(v36);
			var v39: array<map<multiset<char>, bool>> := new map<multiset<char>, bool>[6] [(map[multiset{v28[399]} := p0])[multiset{'c'} := false], v37.cf76, v37.cf76, map[v35 := fm1(|(seq(-0x1d0, i8  => (v28[399])))|, p0, |multiset{!f11, p0}|, globalState)], map[multiset(v2) := p0] + v36, v36 + (map v38 : multiset<char> | v38 in ([multiset{v28[399]}, v35[v29 := v0]])[v0 := v35] :: (v38) := (false))];
			v39[682] := v36[v35 := p0];
			var v40 := DC41(f11);
			r1, v4, v39[682], globalState.f8, v2 := f11, v4, v36, v40.cf65, v2;
			...
			v41[805], r0, f11, v2, v0 := (v43 + v43) != (v43 + v43), if (multiset{p0} >= v44) then v0 else v0, f11, v2, (v0 * |[v0]|) * v0;
		}
		var v45 := DC12(f11, -0x249, |(seq(-0x121, i9  => ('o')))|);
...
class C15 extends T0 {
	...
	method m4(p0: int, p1: int, p2: seq<bool>, globalState: GlobalState) returns (r0: D0, r1: int, r2: map<bool, map<bool, int>>) {
		...
		f11, f11, r1, v3 := f16, !match v4 {
			...
			case DC16(cf29) => !f16
			case DC20(cf38) => v3.f11 !in map[f11 := p2[p1]]
		}, fm2(globalState), v3;
		var i1 := 0;
		while (v3.f11)
			decreases 100 - i1
		{
			...
			i1 := i1 + 1;
			v0[529] := f16;
			v0[529] := fm2(globalState) == p0;
			var v5: seq<int> := [p1];
			var v6 := new C3(v5[p1], p1);
			var v7: set<bool> := {f16};
			var v8: map<int, seq<bool>> := map[p1 := p2[f17 := v3.f11]];
			...
			v12[625] := |v16|;
			var v17: C2 := new C2(f17, v2, f17, f17, v0[529]);
		...
		var v28: map<bool, seq<multiset<int>>> := map[p2[p0] := v21[f17 := multiset(v24)]];
		var v29: map<int, seq<multiset<int>>> := map[p1 := v21];
		var v30: array<seq<multiset<int>>> := new seq<multiset<int>>[24] [[multiset{p1}], v21, v21, v21 + v21, v21, v21, v21[f17 := multiset(v24)], v21, v21, v21, v21 + v21[|v26[-|v27|]| := v20], if (f16 in v28) then v28[f16] else v21, seq(0x1ce, i2  => (multiset(v24))), [v20, DC5(multiset{|v2|}, false).cf10] + v21, v21, v21, v21 + [v20, v20], v21, v21, v21, (if (p0 in v29) then v29[p0] else v21) + v21, fm63(globalState), v21, v21];
		v30[873] := [multiset{p0}, v20] + (seq(0x3dd, i3  => (v20)));
		var v31 := 'y';
		v30[873], globalState.f8, v31, v2 := seq(-0x2bf, i4  => (DC5(v20, f16).cf10 - multiset(v24))), false, v31, (seq(0x2fa, i5  => ('f'))) + v2;
		var i6 := 0;
		while (f16)
			decreases 100 - i6
		{
			...
			match (fm43(p0, if (f16) then f16 else v3.f11, p0, v38[0x52], globalState)) {
				case DC17(cf30, cf31) =>
					var v39 := DC7(v24);
					var v40: set<D2> := {v39};
				...
				case DC18(cf32) =>
					...
					v46[915] := f17;
					v46[915] := (p1 / p0) + p1;
					var v47 := DC21((multiset{false, !v3.f11, f11})[false := f17]);
					var v48 := DC24(v47);
					v46[915], f17, v48, r1, v46[915] := p0, |((p2 + p2) + (p2 + p2))|, v48, p0, p0;
				case DC19(cf33, cf34, cf35, cf36, cf37) =>
					...
					v0[49] := cf34;
					var v68: array<seq<int>> := new seq<int>[9](i7 => [p1, |(seq(0x364, i8  => (v31)))|]);
					var v69: map<int, seq<bool>> := map[v60.f22 := p2];
					v3.f11, v0[49], v68, f11, f17 := (p2 + cf37) < (if (f17 in v69) then v69[f17] else p2), cf37[p0], v68, !cf36, v60.f22;
				...
				case DC16(cf29) =>
					...
					v73[654] := (if (f17 in v74) then v74[f17] else |(if (v31 in v75) then v75[v31] else v76)|) + f17;
					var v77 := DC55(v1);
					...
					v80[216] := map[|v2| := v78.f19];
					var v81 := DC58(v78);
					v72, v73[654], v78, v80[216] := v72, p0, v81.cf86, (v74 + fm38(v31, v31, globalState)) + v74[p0 := p1];
					v73[654] := -|(v20 - (v20 - v20))|;
				case DC20(cf38) =>
					v0[392] := v3.f11;
					v0[392] := v3.f11;
					...
					v82[759] := p0;
					v82[759] := p1;
					globalState.f8 := p2[|v2| := f11] < fm39(p2[f17], f11, globalState);
					v82[759] := (-v82[759] - p1) * v82[759];
			}
			...
			if (v3.f11 ==> v3.f11) {
				...
				var v92: array<int> := new int[12] [f17 % f17, p1, p1, f17, -p0 / p0, |p2| * v91.cf45, p0, p1, p1, |v2|, p0, -(p0 % f17)];
				v92[790] := |v2|;
				var v93: C6 := new C6();
				var v94: map<C6, map<bool, int>> := map[v93 := map[v3.f11 := p1]];
				v27, v0, v92[790] := v27 + (if (v93 in v94) then v94[v93] else v27), v0, p0 + p0;
				r1 := -v92[790];
				var v95: multiset<bool> := multiset{v3.f11, v3.f11, f16};
				var v96: set<int> := {v92[790] - v92[790], p1, if (fm6(f16, v92[790], f17, globalState) in v27) then v27[fm6(f16, v92[790], f17, globalState)] else |v95|};
				v3.f11, v92[790], v96 := !v3.f11, -f17, v96 + v96;
		...
		var v105: array<map<int, int>> := new map<int, int>[25](i9 => (map v102 : int | (0x5 <= v102) && (v102 < -0x17e) :: (v102 - 94) := (|map[true := DC8(v27)]|)) + (map v103 : int | v103 in (map v104 : int | (0x381 <= v104) && (v104 < 68) :: (v104 % p0) := (v3.f11)) :: (v103 / f17) := (|"sfj"|)));
		var v106: array<int> := new int[23];
		var v107: map<array<int>, int> := map[v106 := p1];
		v105, f17, v2, r1 := v105, if (f16) then if (v3.f11) then -p1 else f17 else p0, ("nosncc" + (seq(548, i10  => (v31))))[|v107| := v31], -match DC41(f11) {
			case DC41(cf65) => p1
			case DC40(cf64) => |multiset{v3.f11}|
			case DC42(cf66) => p0
		};
		var v108: array<char> := new char[17];
		v108[56] := fm25(f17, f17, globalState);
		r1, v0, v108[56], globalState.f8 := fm2(globalState), v0, v31, v3.f11;
...
class C16 extends T0, T1 {
	...
	function fm8(p0: int, p1: map<char, bool>, p2: set<int>, globalState: GlobalState): string {
		if (f11) then (seq(0x6c, i0  => ('o'))) + (seq(0x21e, i1  => ('i'))) else "qcgbxqc"
	}
	...
	method m3(p0: string, p1: D0, p2: set<multiset<bool>>, globalState: GlobalState) returns (r0: bool, r1: multiset<bool>, r2: D0, r3: map<int, bool>) {
			...
			decreases 100 - i1
		{
			...
			if (f11) {
				...
				v0[76] := 709;
				v0[76] := -(f12 - (if (f11) then f13 else |multiset{v11, v22.f24}|));
				var v23: array<bool> := new bool[7];
				var v24: multiset<array<bool>> := multiset{v23};
				var v25: C12 := new C12(v24, f12, f11, v0[76], v3.f12);
				...
				globalState.f8, v32, v33, v0[76], v23[905] := (v19[707] + fm0(globalState)) < v19[707], v32, v33, |(v34 + v34)|, v8.fm6(v35 == (set v36 : int | (-0x14a <= v36) && (v36 < 581) :: (v36 / f12)), v3.f13, if (-|v40| in v38) then v38[-|v40|] else v3.f13, globalState);
			}
		...
		for i5 := f12 to v3.f13 {
			...
			if (v3.fm7(globalState)) {
				...
				var v48 := "sutgkklap";
				var v49: map<bool, int> := map[f11 := |(seq(0x254, i7  => ('d')))|];
				var v50 := 'p';
				v0[663], r3, r0, r0, v48 := (f12 * f12) / v3.f12, fm47(f11, false, v49, v3.f12, globalState) + v2, f14, (v0[663] + f15) != (if (f14) then fm2(globalState) else v3.f13), v48 + (v3.f20 + "ggglnxi")[v3.f13 := v50];
				var v51: seq<bool> := [f11, f14, f14, true, f11];
				f11 := (f11 in v49) !in (v51 + ([f11, f11])[f12 := true]);
			} else {
				v41[445] := if (fm9(f14, fm7(globalState), globalState)) then f14 else !f11;
				v41[445] := f11;
				var v52: array<multiset<int>> := new multiset<int>[19];
				var v53: multiset<int> := multiset{fm2(globalState)};
				v52[552] := v53;
				var v54: array<D0> := new D0[5];
				v54[856] := if (v41[445]) then p1 else p1;
				var v55: seq<bool> := [f11, true, f14];
				var v56: map<int, multiset<int>> := map[|v55| := v53];
				v41[445], v52[552], v41[445], v54[856] := f14, if ((0x309 % f12) in v56) then v56[0x309 % f12] else v53[v3.f12 := |v55|], f14, p1;
				var v57: array<bool> := new bool[2](i8 => false);
				var v58: map<bool, bool> := map[f14 := f14];
				v57 := if (if (v41[445] in v58) then v58[v41[445]] else f11) then v57 else v57;
				var v59: T1 := new C3(v3.f13, |(v55 + v55)|);
				var v60: seq<seq<bool>> := [v55];
				var v61: C10 := new C10(f11);
				var v62 := DC19(f14, false, false, f11, v60[|map[v61 := f14]|]);
				var v63: map<bool, D6> := map[f14 := v62];
				v59 := new C7(|(v63 + map[v41[445] := v62])|, (if (f12 in v53) then v53[f12] else v3.f12) % v3.f12);
				v0[663] := if (v3.f13 in v52[552]) then v52[552][v3.f13] else |"bthuoibuq"|;
			}
			...
			if (f11) {
				...
				v0[663], v41[361], v0[663], v0[663], globalState.f8 := v3.f13, f14, |(v64 - {f11, true})| - v3.f13, if (if (f14) then f11 else f11) then v3.f13 / v69[f15] else 0x34f, v70 !in v71;
				...
				v0[663] := -(if (v74 in v75) then v75[v74] else if (v3.f13 in v73) then v73[v3.f13] else v3.f12);
				var v76: seq<string> := [v3.f20, v3.f20];
				var v78: map<bool, int> := map[!f11 := v0[663]];
				var v79: seq<int> := [|v78|];
				var v80: seq<bool> := [f11];
				var v81: set<bool> := {!v80[v3.f12], f11};
				var v82: map<string, set<bool>> := map[v76[|(map v77 : int | v77 in v79 :: (v77 % f15) := (|{f11, f11, f11}|))|] := v81];
				v82 := v82 + v82;
				var v83: array<string> := new string[24](i9 => p0);
				v83[872] := v3.f20 + p0;
				v83[872] := seq(0x43, i10  => ('e'));
				v8 := v8;
				var v84 := DC22(v3.f13, v1);
				var v85: array<D7> := new D7[15] [v84.(cf40 := v0[663]), DC22(v3.f13, v1).(cf40 := f12), DC22(v7.fm14(globalState), v1), v84, v84, DC22(f12, v1), DC22(v3.f13, v1), v84, v84, v84, v84, v84, v84, v84, v84];
			...
			decreases 100 - i11
		{
			...
			v87[510] := (if (f11) then v3.f20 else v3.f20) <= v88.cf2;
			v1[5] := v0;
			v1[5] := new int[26](i12 => i12 % v3.f13);
...
class C17 extends T0 {
	...
	function fm5(p0: int, globalState: GlobalState): D0 {
		DC1(|({-0x52} * {|multiset{|"vldiolxvg"|, |[0x106, 0x1f1]|}|, 0x186, |[|map[-0x43 := 0x20e]|, 0x7a, |map[map[f11 := 0x2d5] := -0x1df]|, |{--|multiset([f11, f11])|, 36}|]|})|, "xnpq", --(|"ahuf"| / |(map v0 : int | v0 in (map v1 : int | (-642 <= v1) && (v1 < 0x361) :: (v1 + 0x1f8) := ([f11])) :: (v0 + 770) := (-852))|), f11)
	}
	function fm6(p0: bool, p1: int, p2: int, globalState: GlobalState): bool {
		f11
	}
	method m1(p0: bool, p1: int, p2: bool, p3: array<bool>, globalState: GlobalState) returns (r0: bool, r1: int, r2: char, r3: bool) {
		...
		if ((if (f11 in v0) then v0[f11] else p2) && p0) {
			...
			var v16: map<bool, int> := map[p2 := |v15[p1]|];
			var v17: C4 := new C4(p1, p0);
			var v19: map<int, int> := map[v17.f25 := p1];
			var v20: multiset<bool> := multiset{true};
			var v21 := "w";
			var v22: array<int> := new int[22] [-0x1bc, p1, |map[p0 := v17]|, 0x176, p1, |(map v18 : int | v18 in v19 :: (v18 / v17.f25) := (p0))|, p1, 0x11f, |v20|, v17.f25, p1, v17.f25, p1, v17.f25, |v21|, p1, p1, -v17.f25, v17.f25, -p1, p1, v17.f25];
			...
			globalState.f8, globalState.f5, r1, r1, r1 := p0, (if (p2) then v28 else v28) + (v28 + [p1, v17.f25]), p1, v17.f25 + -|(v20 + v20)|, v28[v17.f25];
			var v29 := DC2(p1, v22);
			r1 := (if (false) then v29.cf5 else p1) - v17.f25;
			v22[326] := (if (true in v16) then v16[true] else p1) % -p1;
			...
			var v38: seq<C11> := [v37];
			var v39: multiset<int> := multiset{0x3a9, (if (v17.f25 in v19) then v19[v17.f25] else p1) % v17.f25, v17.f25 * v36.cf69, |v38|};
			v22[326], r0, v34, v39, globalState.f8 := v37.fm14(globalState), f11, v34, if (f11) then fm12(v17.f25, v17.f25, p1, p2, globalState) else v39 - v39, !p2;
		}
		
		var v40: set<int> := {p1, |multiset{p2}|, p1, p1};
		var v41: array<C8> := new C8[24];
		var v42 := "od";
		v40, r1, globalState.f8, v41 := v40 * v40, -((|v42| % p1) % p1), (0x32e == -0x1f) <==> p2, v41;
		for i0 := p1 to p1 % p1 {
			r1 := i0;
			if (p1 >= i0) {
				p3[98] := p2;
				...
				globalState.f8, p3[903], p3[191] := f11, v54 == (if (f11) then v54 else v54), if (f11 || f11) then p2 else p0;
				...
				v56 := v56 + (if (p3[903] in v57) then v57[p3[903]] else v56);
				r1 := i0 - i0;
			}
		...
		var v71: map<char, int> := map['k' := |(seq(72, i2  => (v68)))[p1 := v68]|];
		var v72: array<int> := new int[23] [0x342, p1, p1, if (fm1(p1, p0, p1, globalState)) then p1 else p1, 0x3c9, p1, p1, |v42| / -p1, p1, p1 - fm2(globalState), -|v67|, p1, if (p0) then p1 else p1, 0x2d7, |[v68, v68]|, |v42|, |v70|, p1 - p1, p1, p1 + p1, |v42|, p1, |v71|];
		v72 := v72;
		f11 := v40 <= v40;
		r0 := p1 > p1;
		r1 := p1;
	...
	method m2(globalState: GlobalState) returns (r0: bool, r1: bool, r2: int) {
		...
		r2 := fm2(globalState) % |DC59(v9).cf87|;
		var i1 := 0;
		while (f11)
			decreases 100 - i1
		{
			...
			if (f11) {
				...
				v12[636], v14, v16 := f11, v14, (v16 + (seq(0x154, i2  => (v15)))) + v17[v0];
				v0 := -v0 % 424;
				var v18: array<D18> := new D18[16](i3 => DC48());
				v18[200] := fm64(globalState);
				v18[200] := fm64(globalState);
			} else {
				...
				v22 := v23[v0];
				var v24: C3 := new C3(v0, v0 % v0);
				var v25: seq<set<int>> := [v19];
				r2, v22, r0, v24, v25 := v0 / v0, v22, f11, v24, (v25 + v25)[v0 := v19];
				var v26: seq<int> := [0x249, -v0];
				var v27 := new C5(v26[|map[v8 := v0]|]);
			...
			decreases 100 - i4
		{
			...
			var v38: map<string, map<string, array<bool>>> := map[seq(0x1fc, i5  => (v8)) := v37];
			var v39 := DC60(v37);
			var v40: seq<map<string, array<bool>>> := [v37, v37, v37];
			var v41: array<map<string, array<bool>>> := new map<string, array<bool>>[29] [v37, v37, v37, v37, v37, v37, v37 + v37, v37, v37, if (fm36(v0, v0, v8, globalState) in v38) then v38[fm36(v0, v0, v8, globalState)] else v37, map[v35 := v30] + v37, v37, v37, v37, v37, v37 + v37, v37, v37 + v37, v39.cf88, if (!f11) then v39.cf88 else v37, v37, v37 + v37, v40[v0], v37, map[v35 := v30], v37, v37, v37 + v37, if (f11) then v37 else v37];
			v41[4] := v37;
			v41[4] := v37;
...
method Main() {
	...
	var v11: set<map<char, bool>> := {map[v6 := v4], v7, map['r' := v4], v10[v1]};
	var globalState := new GlobalState(886, 0x21e, false, 596, v0 + v0, ((v3[v1])[v1 := v1])[|v5| := v1], 0x11d, 0x21e, true, v11, 0x323);
	var i0 := 0;
	while (v4)
		decreases 100 - i0
	{
		...
		var v15: array<seq<int>> := new seq<int>[14] [v2, [v1], v2, v2, seq(289, i1  => (v1)), v2[v1 := v1], seq(-0x160, i2  => (v1)), v2 + v2, v2, v2, v2[v1 := |v14|], v2, v2 + fm0(globalState), [v1, |v0|, v1, v1, v1]];
		var v16 := DC0(v15);
	...
	var v18: multiset<string> := multiset{v0[|v17| := v6]};
	var v19: seq<string> := [v0, v0];
	var v20: map<bool, bool> := map[fm1(0x25, v4, v1, globalState) := v4];
	var v21: multiset<int> := multiset{v1};
	var v22: seq<bool> := [v4];
	var v23: array<bool> := new bool[27] [v4, v4, v4, v18 !! multiset(v19), v4, v4, v4, v4, if (v4 in v20) then v20[v4] else v4, v4, fm1(v1, fm1(v1, v4, v1, globalState), v1, globalState) || v4, v4, v4 || v4, v4, true, v4, v4, v4, v4, DC3(|v21|, true).cf8, v4, v22[-828], v0 < v0, v4, v1 <= v1, v4, !v4 || fm1(v1, v4, v1, globalState)];
	forall i3 | 0 <= i3 < v23.Length {
		v23[i3] := v4;
	}
	v23[50] := v4;
	v23[50] := v1 <= (fm2(globalState) * v1);
	var v24: map<seq<int>, bool> := map[v2 := v23[50]];
	var v25: seq<map<seq<int>, bool>> := [v24];
	globalState.f8 := !!(v4 ==> (map[v2 := v23[50]] == v25[v1]));
	v6 := v6;
	v1 := if (v23[50]) then v1 else v1;
	for i4 := v1 to v1 / v1 {
		v1 := 0x7a;
		v22 := v22;
		var v26: set<int> := {v1, i4 / i4, i4, |v0|};
		var v27: map<int, set<int>> := map[i4 := v26];
		v26 := (if (v1 in v27) then v27[v1] else {i4}) * v26;
		v4 := |fm3(globalState)| > (|map[fm2(globalState) := v1]| / i4);
	}
	v1 := v1;
	v22 := v22;
	var i5 := 0;
		...
		decreases 100 - i5
	{
		...
		i5 := i5 + 1;
		var v28 := DC3(v1, v4);
		v17 := fm4(-0x15c, v28, v1 % v1, 'o', globalState);
		v0 := v0;
		v23[50] := v4;
